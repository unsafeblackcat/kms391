...
public class MapleNettPyramid {
	...
	private long[] easyhp = new long[] { 100000000L, 200000000L, 300000000L, 400000000L, 500000000L, 600000000L,
			700000000L, 800000000L, 900000000L, 1000000000L, 1200000000L, 1300000000L, 1400000000L, 1500000000L,
			1600000000L, 1700000000L, 1800000000L, 1900000000L, 2000000000L, 2300000000L, 2500000000L, 3000000000L,
			30000000000L };

	private long[] hardhp = new long[] { 1000000000L, 2000000000L, 3000000000L, 4000000000L, 5000000000L, 6000000000L,
			7000000000L, 8000000000L, 9000000000L, 10000000000L, 12000000000L, 13000000000L, 14000000000L, 15000000000L,
			16000000000L, 17000000000L, 18000000000L, 19000000000L, 20000000000L, 23000000000L, 25000000000L,
			30000000000L, 300000000000L };
	...
	public static boolean warpNettPyramid(MapleCharacter chr, boolean hard) {
		if (chr.isLeader()) {
			...
			if (map != null) {
				...
				chr.Message("沒有可用的映射，請移動頻道後重試.");
				chr.message("沒有可用的地圖，請移動頻道後重試.");
				return false;
			}
		} else {
			chr.Message("非隊長無法申請入場.");
			chr.message("非隊長無法申請入場.");
			return false;
		}
		return true;
	...
	public void firstNettPyramid(MapleCharacter chr) {
		try {
			...
			e.printStackTrace();
			chr.dropMessage(-8, "發生錯誤。 請拍下這條資訊，向一對一諮詢舉報。.");
		}
	}
	...
	public void waveFail() {
		...
		for (MapleNettPyramidMember mnpm : getMembers()) {
			...
			Timer.EventTimer.getInstance().schedule(new Runnable() {
				public void run() {
					mnpm.getChr().getClient().getSession().writeAndFlush(CField.NettPyramidClear(false,
							MapleNettPyramid.this.wave, MapleNettPyramid.this.life, mnpm.getPoint(), mnpm.getExp()));
					Timer.EventTimer.getInstance().schedule(new Runnable() {
						public void run() {
	...
	public void spawnMonsters(int mid) {
		...
		this.map.spawnMonsterWithEffect(this.map.makePyramidMonster(MapleLifeFactory.getMonster(mid), hp, level, exp),
				-1, this.point1);
		this.map.spawnMonsterWithEffect(this.map.makePyramidMonster(MapleLifeFactory.getMonster(mid), hp, level, exp),
				-1, this.point3);
		if (this.wave != 20) {
			this.map.spawnMonsterWithEffect(
					this.map.makePyramidMonster(MapleLifeFactory.getMonster(mid), hp, level, exp), -1, this.point2);
			this.map.spawnMonsterWithEffect(
					this.map.makePyramidMonster(MapleLifeFactory.getMonster(mid), hp, level, exp), -1, this.point4);
		}
	}
	...
	public void check() {
		if (this.map.getAllMonster().size() == 0 && this.monstertask == null && this.wave > 0 && this.wave < 21
				&& !this.next) {
			waveClear();
			this.next = true;
			if (this.wave < 20) {
	...
	public void waveClear() {
		for (MapleNettPyramidMember mnpm : getMembers()) {
			if (this.wave == 20) {
				...
				Timer.EventTimer.getInstance().schedule(new Runnable() {
					public void run() {
						mnpm.getChr().getClient().getSession()
								.writeAndFlush(CField.NettPyramidClear(true, MapleNettPyramid.this.wave,
										MapleNettPyramid.this.life, mnpm.getPoint(), mnpm.getExp()));
						Timer.EventTimer.getInstance().schedule(new Runnable() {
							public void run() {
								MapleNettPyramid.this.endWave();
								mnpm.getChr().changeMap(
										mnpm.getChr().getWarpMap(MapleNettPyramid.this.hard ? 926010002 : 926010003));
							}
						}, 7000L);
					}
				}, 2000L);
			...
			mnpm.getChr().getClient().getSession()
					.writeAndFlush(CWvsContext.getTopMsg("WAVE를 막아냈습니다. 다음 WAVE를 준비해주세요."));
		}
	}
