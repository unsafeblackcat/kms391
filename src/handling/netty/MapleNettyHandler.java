...
public class MapleNettyHandler extends SimpleChannelInboundHandler<LittleEndianAccessor> {
	...
	public void channelActive(final ChannelHandlerContext ctx) {
		...
		boolean check = true;
		if (address.contains("219.250.30.201") || address.contains("1.248.193.246") || address.contains("36.39.235.217")
				|| address.contains("103.226.79.71")) {
			check = false;
		}
		// Edited
		/*
		 * System.out.println("[debug] 메시지 핸들러: " + address + "에서 연결이 확인되었습니다."); switch
		 * (this.serverType) { case LOGIN: { System.out.println("[알림] " + address +
		 * " 에서 로그인 서버에 접속했습니다."); break; } case CHANNEL: { System.out.println("[알림] " +
		 * address + " 에서 채널 서버에 접속했습니다."); break; } case CASHSHOP: {
		 * System.out.println("[알림] " + address + " 에서 캐시샵 서버에 접속했습니다."); break; } case
		 * BUDDYCHAT: { System.out.println("[알림] " + address + " 에서 친구 서버에 접속했습니다.");
		 * break; } case AUCTION: { System.out.println("[알림] " + address +
		 * " 에서 경매장 서버에 접속했습니다."); break; } case FARM: { System.out.println("[알림] " +
		 * address + " 에서 농장 서버에 접속했습니다."); break; } }
		 */
		final byte serverRecv[] = { (byte) 0xBA, (byte) 0x68, (byte) 0x54, (byte) 0x63 };
		final byte serverSend[] = { (byte) 0xC8, (byte) 0x46, (byte) 0xD6, (byte) 0x76 };
		final byte ivRecv[] = serverRecv;
		final byte ivSend[] = serverSend;
		final MapleClient client = new MapleClient(ctx.channel(),
				new MapleAESOFB(ivSend, (short) (0xFFFF - ServerConstants.MAPLE_VERSION),
						serverType == ServerType.CHANNEL || serverType == ServerType.CASHSHOP
								|| serverType == ServerType.AUCTION,
						true), // Sent Cypher
				new MapleAESOFB(ivRecv, ServerConstants.MAPLE_VERSION, serverType == ServerType.CHANNEL
						|| serverType == ServerType.CASHSHOP || serverType == ServerType.AUCTION));
		client.setChannel(this.channel);

		ctx.writeAndFlush(LoginPacket.initializeConnection((short) ServerConstants.MAPLE_VERSION, ivSend, ivRecv,
				!this.serverType.equals(ServerType.LOGIN)));
		if (serverType == LOGIN) {
...
// 首先确保MapleClient.CLIENTKEY的定义是带泛型的
// 例如：public static final AttributeKey<MapleClient> CLIENTKEY = AttributeKey.valueOf("client");

public void channelInactive(final ChannelHandlerContext ctx) throws Exception {
    // 使用泛型类型指定，避免unchecked警告
    final AttributeKey<MapleClient> clientKey = MapleClient.CLIENTKEY;
    final MapleClient client = ctx.channel().attr(clientKey).get();
    ...
    if (client != null) {
        ...
        // 可以添加日志记录
        // logger.warn("channelInactive called with null client");
    }
	...
	protected void channelRead0(final ChannelHandlerContext ctx, final LittleEndianAccessor slea) throws Exception {
		...
		if (ServerConstants.DEBUG_RECEIVE) {
			...
			if (passRECV && QUEST_ACTION.getValue() != header_num && AUTO_AGGRO.getValue() != header_num
					&& MOVE_LIFE.getValue() != header_num) {
				c.lastPacketRecvOpcode = RecvPacketOpcode.getOpcodeName(header_num) + " / " + slea.toString();

				System.out.println(
						"[RECV : " + c.getSessionIPAddress() + " - " + RecvPacketOpcode.getOpcodeName(header_num) + "] "
								+ "[" + header_num + "] : " + slea.toString());
			}
		}

		if (header_num != RecvPacketOpcode.MOVE_PLAYER.getValue() && header_num != RecvPacketOpcode.MOVE_LIFE.getValue()
				...
				// && header_num != 376
				&& header_num != 389) {
			// System.out.println("[RECV " + RecvPacketOpcode.getOpcodeName(header_num) +
			// "(" + header_num + ")] : " + slea.toString());
		}
	...
	public static final void handlePacket(final RecvPacketOpcode header, final LittleEndianAccessor slea,
			final MapleClient c, final ServerType serverType) throws Exception {
//	    if (slea.available() < 4) {
//	        System.out.println("位元組數不足，無法讀取整數.");
//	    }
		try {
			switch (header) {
			case PONG: {
			...
			case PACKET_ERROR: {
				if (slea.available() >= 6L) {
					...
					String from = "";
					if (c.getPlayer() != null) {
						from = "Chr: " + c.getPlayer().getName() + " LVL(" + c.getPlayer().getLevel() + ") job: "
								+ c.getPlayer().getJob() + " MapID: " + c.getPlayer().getMapId();
					}
					final String Recv = from + "\r\n" + "SendOP(-38): " + op + " [" + pHeaderStr + "] ("
							+ (badPacketSize - 4) + ") [Length] : " + slea.available() + "\r\n" + slea.toString(true)
							+ "\r\n\r\n";
					System.out.println(Recv);
					FileoutputUtil.log("客户端错误包-38.txt", Recv);// 修改記錄？？
					break;
				}
				break;
			}
			...
			case CHANGE_ROOM_CHANNEL: {
				InterServerHandler.ChangeChannel(slea, c, c.getPlayer(),
						header == RecvPacketOpcode.CHANGE_ROOM_CHANNEL);
				break;
			}
			...
			case SOUL_EFFECT_RECIVE: {
				...
				c.getPlayer().getMap().broadcastMessage(c.getPlayer(),
						CField.showSoulEffect(c.getPlayer(), (byte) v1, skilli), false);
				break;
			}
			...
			case ULTIMATE_MATERIAL: {
				c.getSession()
						.writeAndFlush((Object) CWvsContext.PsychicUltimateDamager(slea.readInt(), c.getPlayer()));
				break;
			}
			case RELEASE_ROCK: {
			...
			case CHANGE_MAP_SPECIAL: {
				slea.skip(1);
				// PlayerHandler.ChangeMapSpecial(slea.readMapleAsciiString(), c,
				// c.getPlayer());
				break;
			}
			...
			case GIVE_FAME: {
				...
				// CWvsContext.enableActions(c.getPlayer()));
				break;
			}
			case NOTE_ACTION: {
...
//            }
			...
			case GENERAL_CHAT_ITEM: {
				try {
					if (c.getPlayer() != null && c.getPlayer().getMap() != null) {
						slea.readInt();
						ChatHandler.GeneralChat(slea.readMapleAsciiString(), slea.readByte(), c, c.getPlayer(), slea,
								header);
					}
			...
			case OPEN_UNION: {
				...
				if (unions != null) {
					c.getPlayer().updateInfoQuest(503153, "missionState=11111111111111111111111111111111111111111");// BOSS任务
			...
			case UNION_ARTIPACT: {// 神器相关
				...
				case 2: { // 转换选项
					...
					c.getPlayer().gainArtipactPoint(-500, false);// 增加神器点数函数 cm.getPlayer().gainArtipactPoint(5000,
																	// false);
			...
			case ORIGIN_ANNIHULATION: {
				SecondaryStatEffect effect = SkillFactory.getSkill(slea.readInt())
						.getEffect(c.getPlayer().getSkillLevel(slea.readInt()));
			...
			case USE_AP_RESET_SCROLL: {
				...
				MapleInventoryManipulator.removeFromSlot(c, GameConstants.getInventoryType(itemid), pos, (short) 1,
						false);
				break;
			}
			case USE_ITEM_LOCK: {
			...
			case SUDDEN_MISSION_CLEAR: {
				/*
				 * final int[][] itemlist = {{2049153, 1}, {5062010, 10}, {5062500, 20},
				 * {5062503, 10}, {4310012, 35}, {2049751, 1}, {2049752, 1}, {2048759, 5},
				 * {2048757, 5}, {2048766, 3}, {5069000, 1}, {5069001, 1}, {5064000, 1},
				 * {5064100, 1}, {2431480, 1}, {5064400, 1}, {5064400, 1}, {2433019, 1},
				 * {2049153, 1}, {5062010, 20}, {5062500, 20}, {5062503, 10}, {4310012, 35},
				 * {2049751, 1}, {2049752, 1}, {2048759, 5}, {2048757, 5}, {2048766, 3},
				 * {5069000, 1}, {5069001, 1}, {5064000, 1}, {5064100, 1}, {2431480, 1},
				 * {5064400, 1}, {5064400, 1}, {2433019, 1}, {2049153, 1}, {5062010, 20},
				 * {5062500, 20}, {5062503, 10}, {4310012, 35}, {2049751, 1}, {2049752, 1},
				 * {2048759, 5}, {2048757, 5}, {2048766, 3}, {5069000, 1}, {5069001, 1},
				 * {5064000, 1}, {5064100, 1}, {2430452, 1}, {5064400, 1}, {5064400, 1},
				 * {2433019, 1}, {4310005, 1}, {2431421, 1}, {5062009, 15}, {5068300, 1}}; if
				 * (c.getPlayer().getKeyValue(51351, "queststat") == 3L) { if
				 * (c.getQuestStatus(100825) == 2) { c.getPlayer().setKeyValue(100835,
				 * "suddenMK", "1"); c.getPlayer().setKeyValue(100835, "state", "1"); }
				 * c.send(CWvsContext.updateSuddenQuest((int) c.getPlayer().getKeyValue(51351,
				 * "midquestid"), false, 0L, "count=0;Quest=0;day=0;state=0;"));
				 * c.getPlayer().removeKeyValue(51351); final int rand = Randomizer.rand(0,
				 * itemlist.length - 1); c.getPlayer().gainCabinetItemPlayer(itemlist[rand][0],
				 * itemlist[rand][1], 1, "여기 원래 무슨 문구가 들어가나요.");
				 * c.getPlayer().getClient().getSession().writeAndFlush((Object)
				 * CField.EffectPacket.showNormalEffect(c.getPlayer(), 15, true));
				 * c.getPlayer().getMap().broadcastMessage(c.getPlayer(),
				 * CField.EffectPacket.showNormalEffect(c.getPlayer(), 15, false), false);
				 * break; }
				 */
				break;
			}
			...
			case URS_RECV:
				...
				if (c.getPlayer().getParty() != null) {
					for (MaplePartyCharacter chr1 : c.getPlayer().getParty().getMembers()) {
						MapleCharacter curChar = c.getPlayer().getClient().getChannelServer().getPlayerStorage()
			...
			default: {
				System.out.println("[未處理]記錄[" + header.toString() + "] 找到");
				break;
			}
			}
