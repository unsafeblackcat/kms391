...
public class MarriageEventAgent {
	...
	public void registerEvent(MapleCharacter chr) {
		...
		World.Broadcast.broadcastMessage(CWvsContext.serverNotice(5, "", this.runningDataEntry.getGroomName() + "님과 "
				+ this.runningDataEntry.getBrideName() + "님의 결혼이 " + this.channel + "채널 대성당에서 시작되려 합니다."));
	...
	public void checkEnterMap(MapleCharacter chr) {
		if (this.endTime > 0L) {
			chr.getClient().getSession()
					.writeAndFlush(CField.getClock((int) ((this.endTime - System.currentTimeMillis()) / 1000L)));
		}
	}

	public void checkLeaveMap(MapleCharacter chr, int newMap) {
		if (this.runningDataEntry != null && !isWeddingMap(newMap) && (chr.getId() == this.runningDataEntry.getGroomId()
				|| chr.getId() == this.runningDataEntry.getBrideId())) {
			endEvent(true);
		}
	}
				...
				if (this.count == 0) {
					...
					for (MapleCharacter chr : hall.getCharacters()) {
						MapleItemInformationProvider.getInstance()
								.getItemEffect(
										MarriageEventAgent.this.runningDataEntry.getTicketType().getBuffEffectItemId())
								.applyTo(chr, true);
					}
	...
	public final void doNextFinale() {
		if (this.finaleLevel == 0) {
			...
			for (MapleCharacter chr : this.cserv.getMapFactory().getMap(680000400).getCharacters()) {
				if (chr.getId() != this.runningDataEntry.getBrideId()
						&& chr.getId() != this.runningDataEntry.getGroomId()) {
