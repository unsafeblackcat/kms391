...
public final class MapleMap {
	...
	private class MapleMapManagement implements Runnable {
		...
		public void handleBurningStatus(long time) {
			if (MapleMap.this.getAllNormalMonstersThreadsafe().size() > 0 && !MapleMap.this.isTown()
					&& !GameConstants.로미오줄리엣(MapleMap.this.getId()) && !GameConstants.사냥컨텐츠맵(MapleMap.this.getId())
					&& MapleMap.this.isSpawnPoint() && !GameConstants.보스맵(MapleMap.this.getId())
					&& !GameConstants.isContentsMap(MapleMap.this.getId())) {
				if (MapleMap.this.getAllCharactersThreadsafe().size() == 0) {
					...
					if (MapleMap.this.getBurning() > 0) {
						MapleMap.this.broadcastMessage(CField.EffectPacket
								.showBurningFieldEffect("#fn나눔고딕 ExtraBold##fs26#          버닝 " + MapleMap.this.burning
										+ "단계 : 경험치 " + (MapleMap.this.burning * 10) + "% 추가지급!!          "));
					} else {
						MapleMap.this.broadcastMessage(CField.EffectPacket
								.showBurningFieldEffect("#fn나눔고딕 ExtraBold##fs26#          버닝필드 소멸!          "));
					}
				}
			}
		}
			...
			while (chrs.hasNext()) {
				...
				if (chr.isAlive()) {
					if (MapleMap.this.getId() >= 105200520 && MapleMap.this.getId() < 105200530
						...
						chr.addHP(minushp);
						chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, 0, minushp, 36,
								0, 0, (byte) 0, true, null, null, null));
						chr.getMap().broadcastMessage(chr, CField.EffectPacket.showEffect(chr, 0, minushp, 36, 0, 0,
								(byte) 0, false, null, null, null), false);
					}
					if (chr.getMapId() > 450008000 && chr.getMapId() < 450009000 && MapleMap.this.getNumMonsters() > 0
							&& chr.getMoonGauge() < 100 && chr.getSkillCustomValue(8880302) == null) {
					...
					if (chr.getBuffedValue(13111024) || chr.getBuffedValue(13120007)) {
						...
						Rectangle bounds = chr.getBuffedEffect(skillid).calculateBoundingBox(summon.getTruePosition(),
								summon.isFacingLeft());
						if (summon != null) {
							for (MapleMonster mob : chr.getMap().getAllMonster()) {
								if (bounds.contains(mob.getTruePosition())) {
									if (skillid == 13120007) {
										mob.applyStatus(chr.getClient(), MonsterStatus.MS_IndiePdr,
												new MonsterStatusEffect(skillid, 4000),
												-chr.getBuffedEffect(skillid).getW(), chr.getBuffedEffect(skillid));
									}
									mob.applyStatus(chr.getClient(), MonsterStatus.MS_Speed,
											new MonsterStatusEffect(skillid, 4000), -100, chr.getBuffedEffect(skillid));
								}
							}
						}
					}
				...
				if (chr.getSkillCustomValues().size() > 0) {
					...
					for (Map.Entry<Integer, SkillCustomInfo> sci : customInfo.entrySet()) {
						if (((SkillCustomInfo) sci.getValue()).canCancel(System.currentTimeMillis())) {
							...
							if (((Integer) sci.getKey()).intValue() == 9110
									&& (chr.getMapId() / 100000 == 9530 || chr.getMapId() / 100000 == 9540)) {
								chr.warp(951000000);
								continue;
							}
							if (((Integer) sci.getKey()).intValue() == 24220
									&& (chr.getMapId() == 450008950 || chr.getMapId() == 450008350)) {
								chr.playerIGDead();
								continue;
							}
							if (((Integer) sci.getKey()).intValue() == 450001401) {
								if (MapleMap.this.getId() == 450001400) {
									chr.getClient().send(SLFCGPacket.ErdaSpectrumArea(new int[] { 33, 22962, 0, 2500,
											-120, -128, 120, 5, Randomizer.rand(-250, 1200), 47 }));
					...
					if (mist.getSourceSkill() != null) {
						if (mist.getSourceSkill().getId() == 400041008 && mist.getDuration() != 1800) {
							...
							for (MapleMist mistz : MapleMap.this.getAllMistsThreadsafe()) {
								if (mistz.getSourceSkill().getId() == 400040008
										|| mistz.getSourceSkill().getId() == 400041008) {
									spearSize++;
								}
							}
							MapleMist spear = new MapleMist(
									new Rectangle((mist.getBox()).x, (mist.getBox()).y + 25, (mist.getBox()).width,
											(mist.getBox()).height + 25),
									mist.getOwner(), SkillFactory.getSkill((spearSize >= 5) ? 400041008 : 400040008)
											.getEffect(mist.getSkillLevel()),
									1800, (byte) 0);
							spear.setPosition(mist.getTruePosition());
							MapleMap.this.spawnMist(spear, false);
						} else if (mist.getSourceSkill().getId() == 400051025) {
							MapleMist icbm = new MapleMist(
									new Rectangle((mist.getBox()).x, (mist.getBox()).y, (mist.getBox()).width,
											(mist.getBox()).height),
									mist.getOwner(), SkillFactory.getSkill(400051026).getEffect(mist.getSkillLevel()),
									15000, (byte) 0);
		...
		public void handleMobs(long time) {
				...
				for (MapleMonster mapleMonster : MapleMap.this.getAllMonster()) {
					if (mapleMonster.getId() == 8220028) {
						...
						for (int i = 0; i < 4; i++) {
							...
							MapleMap.this.spawnFlyingDrop(chr, mapleMonster.getPosition(), mapleMonster.getPosition(),
									toDrop);
							if (Randomizer.isSuccess(5)) {
								toDrop = new Item(2432398, (short) 0, (short) 1, 0);
								MapleMap.this.spawnFlyingDrop(chr, mapleMonster.getPosition(),
										mapleMonster.getPosition(), toDrop);
							}
						}
					}
				}
					...
					if (!chr.getMap().getWillPoison().isEmpty()) {
						while (pos2.hasNext()) {
							...
							if (pos.x - 100 < (chr.getPosition()).x && pos.x + 100 > (chr.getPosition()).x
									&& chr.isAlive() && chr.getBuffedEffect(SecondaryStat.NotDamaged) == null
									&& chr.getBuffedEffect(SecondaryStat.IndieNotDamaged) == null) {
								chr.addHP(-(chr.getStat().getCurrentMaxHp() / 100L) * 44L);
								chr.getMap().broadcastMessage(CField.playerDamaged(chr.getId(),
										(int) (chr.getStat().getCurrentMaxHp() / 100L) * 44));
							}
						}
					}
				}
			}
			if (MapleMap.this.rune != null && !MapleMap.this.getAllCharactersThreadsafe().isEmpty()
					&& MapleMap.this.isSpawnPoint() && !GameConstants.보스맵(MapleMap.this.mapid)
					&& !GameConstants.isContentsMap(MapleMap.this.mapid) && MapleMap.this.getRuneCurse() < 4) {
				...
				if (MapleMap.this.runeCurseTime >= 120) {
					...
					for (MapleCharacter allchr : MapleMap.this.getAllCharactersThreadsafe()) {
						if (!allchr.getBuffedValue(80002282)) {
							allchr.getClient()
									.send(CField.runeCurse("必須解放符文以解除精英BOSS的詛呪!!\\n詛呪階段 " + MapleMap.this.getRuneCurse()
											+ " :  經驗埴獲取率、掉落率 " + MapleMap.this.getRuneCurseDecrease() + "% 降低效果生效中",
											true));
						}
					}
				}
			...
			if ((monster = MapleMap.this.getMonsterById(8220028)) != null && !MapleMap.this.isElitebossrewardmap()) {
				...
				for (int i = 0; i < 3; i++) {
					...
					int itemid = itemlist[Random];
					Item toDrop = new Item(itemid, (short) 0, (short) 1, 0);
					MapleMap.this.spawnFlyingDrop(monster.getController(), monster.getPosition(), monster.getPosition(),
							toDrop);
				}
			...
			if (((monster = MapleMap.this.getMonsterById(8880100)) != null
					|| (monster = MapleMap.this.getMonsterById(8880101)) != null
					|| (monster = MapleMap.this.getMonsterById(8880110)) != null
					|| (monster = MapleMap.this.getMonsterById(8880111)) != null)
					&& time - MapleMap.this.lastIncinerateTime >= 30000L) {
				if (MapleMap.this.lastIncinerateTime != 0L) {
					MapleMap.this.spawnIncinerateObject(new MapleIncinerateObject(
							Randomizer.nextInt(MapleMap.this.getRight() - MapleMap.this.getLeft())
									+ MapleMap.this.getLeft(),
							16));
				}
			...
			while (lifes.hasNext()) {
				...
				for (Pair<MonsterStatus, MonsterStatusEffect> cancel : life.getStati()) {
					if (!MapleMap.this.getAllChracater().contains(((MonsterStatusEffect) cancel.getRight()).getChr())
							&& !((MonsterStatusEffect) cancel.getRight()).isMobskill()) {
				...
				if ((life.getId() == 8950100 || life.getId() == 8950000) && life.getBuff(MonsterStatus.MS_Laser) != null
						&& !life.isSkillForbid()) {
					if (Randomizer.isSuccess(1) && life.getCustomValue(2287) == null
							&& life.getCustomValue(22878) == null) {
						MobSkillFactory.getMobSkill(228, 7).applyEffect(life.getController(), life, true, true);
					}
					if (Randomizer.isSuccess(3) && life.getCustomValue(2286) == null
							&& life.getCustomValue(2287) == null) {
						...
						life.getMap().broadcastMessage(MobPacket.LaserHandler(life.getObjectId(), life.getEnergycount(),
								life.getEnergyspeed(), life.isEnergyleft() ? 0 : 1));
					}
				}
						...
						if (chr != null) {
							...
							if (chr.getSkillCustomValue(143144) == null) {
								if (chr.getTruePosition().getX() > life.getTruePosition().getX() - pix
										&& chr.getTruePosition().getX() < life.getTruePosition().getX() + pix) {
									inside = true;
								}
								Map<SecondaryStat, Pair<Integer, Integer>> statups = new HashMap<>();
								statups.put(SecondaryStat.MobZoneState, new Pair<>(Integer.valueOf(inside ? 1 : 0),
										Integer.valueOf(life.getObjectId())));
								chr.getClient().getSession()
										.writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups, null, chr.getPlayer()));
				...
				if (life.getId() == 8870100) {
					...
					if (size <= 0 && hardhilla != null) {
						hardhilla.setLastSkillUsed(MobSkillFactory.getMobSkill(200, 251), System.currentTimeMillis(),
								(Randomizer.rand(30, 40) * 1000));
						while (hardhilla.getBuff(MonsterStatus.MS_ExchangeAttack) != null) {
							hardhilla.cancelStatus(MonsterStatus.MS_ExchangeAttack,
									hardhilla.getBuff(MonsterStatus.MS_ExchangeAttack));
						}
					}
				}
				if (MapleMap.this.getId() == 240060200 || MapleMap.this.getId() == 240060201) {
					...
					boolean clear = false;
					for (MapleMonster m : MapleMap.this.getAllMonster()) {
						if ((m.getId() >= 8810002 && m.getId() <= 8810009)
								|| (m.getId() >= 8810102 && m.getId() <= 8810109)) {
							clear = false;
							break;
						}
				...
				if (life.getId() == 8880102) {
					if (life.getCustomValue(8880102) == null && life.getController() != null
							&& MapleMap.this.getAllChracater().size() > 1) {
					...
					if (demian != null) {
						...
						for (MapleCharacter chr : MapleMap.this.getAllCharactersThreadsafe()) {
							if (chr.getSkillCustomValue(143144) == null) {
								...
								Map<SecondaryStat, Pair<Integer, Integer>> statups = new HashMap<>();
								statups.put(SecondaryStat.MobZoneState, new Pair<>(Integer.valueOf(damaged ? 1 : 0),
										Integer.valueOf(demian.getObjectId())));
								chr.getClient().getSession()
										.writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups, null, chr.getPlayer()));
				...
				if ((life.getId() == 8500001 || life.getId() == 8500002 || life.getId() == 8500011
						|| life.getId() == 8500012 || life.getId() == 8500021 || life.getId() == 8500022)
						&& life != null) {
					if (life.getPatten() > 0) {
						...
						if (MapleMap.this.PapulratusPatan == 0) {
							if (life.getPatten() <= 0) {
								...
								MapleMap.this.broadcastMessage(MobPacket.SpeakingMonster(life,
										(life.getId() == 8500001 || life.getId() == 8500011 || life.getId() == 8500021)
												? 1
												: 2,
										0));
								...
								MapleMap.this.broadcastMessage(
										MobPacket.BossPapuLatus.PapulLatusTime(life.getPatten() * 1000, true, false));
							}
							...
							if (life.getPatten() <= 0) {
								...
								MapleMap.this.broadcastMessage(
										MobPacket.BossPapuLatus.PapulLatusTime(life.getPatten() * 1000, false, false));
							} else if (life.getSeperateSoul() <= 0 && (life.getMap()).PapulratusPatan == 1
									&& life.getCustomValue(8500001) == null) {
								MapleMap.this.broadcastMessage(MobPacket.BossPapuLatus.dropPaPul());
								life.setCustomInfo(8500001, 0, 2000);
							}
						}
					}
					if (MapleMap.this.getId() == 105200120 || MapleMap.this.getId() == 105200510
							|| MapleMap.this.getId() == 105200510 || MapleMap.this.PapulratusPatan == 1) {
						for (MapleCharacter chr : MapleMap.this.getAllCharactersThreadsafe()) {
							if (chr.isAlive() && chr.getBuffedValue(SecondaryStat.NotDamaged) == null
									&& chr.getBuffedValue(SecondaryStat.IndieNotDamaged) == null) {
								chr.addHP(-(chr.getStat().getCurrentMaxHp() * 10L) / 100L);
								chr.getClient().getSession()
										.writeAndFlush(CField.EffectPacket.showEffect(chr, 0,
												(int) -(chr.getStat().getCurrentMaxHp() * 10L) / 100, 36, 0, 0,
												(byte) 0, true, null, null, null));
								chr.getMap().broadcastMessage(chr,
										CField.EffectPacket.showEffect(chr, 0,
												(int) -(chr.getStat().getCurrentMaxHp() * 10L) / 100, 36, 0, 0,
					...
					if (buff != null && life.getHp() > 1L && buff.getInterval() > 0 && buff.getChr() != null) {
						...
						if (damage) {
							...
							if (buff.getSkill() == 25121006 && buff.getChr().getSkillLevel(25121006) > 0
									&& buff.getChr().isAlive()) {
								buff.getChr().getClient().getSession()
										.writeAndFlush(CField.EffectPacket.showEffect(buff.getChr(), 0, 25121006, 4, 0,
												0, (byte) (buff.getChr().isFacingLeft() ? 1 : 0), true,
												buff.getChr().getPosition(), null, null));
								buff.getChr().getMap().broadcastMessage(buff.getChr(),
										CField.EffectPacket.showEffect(buff.getChr(), 0, 25121006, 4, 0, 0,
												(byte) (buff.getChr().isFacingLeft() ? 1 : 0), false,
												buff.getChr().getPosition(), null, null),
										false);
								buff.getChr()
										.addHP(buff.getValue()
												* SkillFactory.getSkill(buff.getSkill())
														.getEffect(buff.getChr().getSkillLevel(25121006)).getX()
												/ 100L);
							}
						}
					}
		...
		public void run() {
			...
			if (MapleMap.this.characterSize() > 0) {
				...
				if (MapleMap.this.BattleGroundMainTimer > 0) {
					...
					if (MapleMap.this.BattleGroundMainTimer == 539) {
						MapleMap.this.broadcastMessage(
								CField.ImageTalkNpc(9001153, 5000, "무적 상태가 해제 되었습니다. 플레이어들끼리 전투가 가능합니다!"));
						...
						MapleMap.this.broadcastMessage(CField.ImageTalkNpc(9001153, 5000,
								"보스존에 #r홀로 드래곤#k이 스폰 되었습니다. 처치시 막대한 #b경험치와 골드#k를 얻을 수 있습니다!"));
						MapleMap.this.spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9303206),
								new Point(-380, -58));
			 ...
			 * MapleMap.this.elitetime--; if (MapleMap.this.elitetime % 10 == 0) {
			 * MapleMonster mob = null; for (MapleMonster mobs :
			 * MapleMap.this.getAllMonster()) { if (mobs.getId() == 8220022 || mobs.getId()
			 * == 8220023 || mobs.getId() == 8220024 || mobs.getId() == 8220025 ||
			 * mobs.getId() == 8220026) { mob = mobs; break; } } if (mob != null) {
			 * World.Broadcast.broadcastMessage(CWvsContext.eliteWarning(MapleMap.this.getId
			 * (), mob.getId(), MapleMap.this.getChannel() - 1)); } } if
			 * (MapleMap.this.elitetime <= 0) { MapleMap.this.stopEliteBossMap(); } }
			 */
			if (MapleMap.this.PapulratusTime > 0) {
				...
				if (MapleMap.this.PapulratusTime <= 0) {
					...
					if (MapleMap.this.getAllMonster().size() > 0 && MapleMap.this.PapulratusPatan == 0) {
						MapleMap.this.PapulratusTime = 50;
						MapleMap.this.broadcastMessage(
								MobPacket.BossPapuLatus.PapulLatusLaser(false, Randomizer.rand(1, 3)));
					}
				}
			}
			...
			if (MapleMap.this.getCustomValues().size() > 0) {
				...
				for (Iterator<Map.Entry<Integer, SkillCustomInfo>> iterator = customInfo.entrySet().iterator(); iterator
						.hasNext();) {
						...
						if (MapleMap.this.getChannel() == 1) {
							if (((Integer) sci.getKey()).intValue() == 921174101) {
								if (MapleMap.this.getCustomValue0(921174102) < 4) {
									...
									MapleMap.this.spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8880507),
											new Point(basex1 + MapleMap.this.getCustomValue0(921174102) * 165, -1423));
									MapleMap.this.spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8880508),
											new Point(basex2 - MapleMap.this.getCustomValue0(921174102) * 165, -1423));
									MapleMap.this.setCustomInfo(921174102, MapleMap.this.getCustomValue0(921174102) + 1,
											0);
									for (MapleCharacter chr : MapleMap.this.getAllChracater()) {
									...
									if (MapleMap.this.getCustomValue0(921174102) < 4) {
										pt.add(new Point(basex1 + MapleMap.this.getCustomValue0(921174102) * 165,
												-1423));
										pt.add(new Point(basex2 - MapleMap.this.getCustomValue0(921174102) * 165,
												-1423));
							...
							if (((Integer) sci.getKey()).intValue() == 993192500) {
								if (MapleMap.this.getCharactersSize() < 5) {
									for (MapleCharacter chr : MapleMap.this.getAllChracater()) {
										chr.warp((Integer.parseInt(chr.getPlayer().getV("returnM")) > 0)
												? Integer.parseInt(chr.getPlayer().getV("returnM"))
												: 993192000);
							...
							if (((Integer) sci.getKey()).intValue() == 993192602
									|| ((Integer) sci.getKey()).intValue() == 993192603
									|| ((Integer) sci.getKey()).intValue() == 993192604) {
								if (MapleMap.this.getCharactersSize() > 0) {
									String name = (((Integer) sci.getKey()).intValue() == 993192602) ? "trick01"
											: ((((Integer) sci.getKey()).intValue() == 993192603) ? "trick02"
													: "trick03");
									List<String> str = Arrays.asList(new String[] { name });
									List<Integer> foothold = new ArrayList<>();
									int cooltime = (((Integer) sci.getKey()).intValue() == 993192602) ? 4300
											: ((((Integer) sci.getKey()).intValue() == 993192603) ? 3500 : 3500);
									...
									int endtime = (((Integer) sci.getKey()).intValue() == 993192602) ? 2500
											: ((((Integer) sci.getKey()).intValue() == 993192603) ? 3500 : 3500);
							...
							if (((Integer) sci.getKey()).intValue() == 921174100) {
								...
								Timer.EtcTimer.getInstance().schedule(() -> {
									...
									MapleMap.this.broadcastMessage(CField.UIPacket
											.detailShowInfo("게임이 종료 되었습니다 잠시 후 퇴장맵으로 이동 됩니다.", 3, 20, 20));
	...
	public void MapiaWin(MapleCharacter player) {
		long fuck = (ChannelServer.getInstance(player.getClient().getChannel()).getMapFactory()
				.getMap(234567899)).mbating;
		int fuckingmapia = 0;
		...
		if (this.MapiaChannel == 1) {
			...
			World.Broadcast.broadcastMessage(
					CWvsContext.serverNotice(8, "", "[마피아 알림] " + chan + "세이상 채널에서 마피아의 승리로 게임이 종료 되었습니다."));
		} else {
			int chan = this.MapiaChannel + 1;
			World.Broadcast.broadcastMessage(
					CWvsContext.serverNotice(8, "", "[마피아 알림] " + chan + "채널에서 마피아의 승리로 게임이 종료 되었습니다."));
		}
	...
	public void CitizenWin(MapleCharacter player) {
		...
		if (this.MapiaChannel == 1) {
			int chan = 20;
			World.Broadcast.broadcastMessage(
					CWvsContext.serverNotice(8, "", "[마피아 알림] " + chan + "세이상 채널에서 시민의 승리로 게임이 종료 되었습니다."));
		} else {
			int chan = this.MapiaChannel + 1;
			World.Broadcast.broadcastMessage(
					CWvsContext.serverNotice(8, "", "[마피아 알림] " + chan + " 채널에서 시민의 승리로 게임이 종료 되었습니다."));
		}
	...
	public void MapiaCompare(MapleCharacter player) {
		...
		try {
			...
			if (voteamount[this.playern - 1] == voteamount[this.playern - 2]) {
				...
				for (MapleCharacter chr : getCharacters()) {
					if (charinfo[0] == chr.getName()) {
						chr.dropMessage(1, "主持人>>您因投票結果被處決.");
						chr.isDead = true;
					} else {
			...
			if (this.MapiaChannel == 1) {
				int chana = 20;
				World.Broadcast.broadcastMessage(
						CWvsContext.serverNotice(8, "", "[마피아 알림] " + chana + "세이상 채널에서 게임이 다시 활성화 되었습니다."));
			} else {
				int chana = this.MapiaChannel + 1;
				World.Broadcast.broadcastMessage(
						CWvsContext.serverNotice(8, "", "[마피아 알림] " + chana + " 채널에서 게임이 다시 활성화 되었습니다."));
			}
			this.MapiaIng = false;
			this.nightnumber = 0;
			for (MapleCharacter chr : getCharacters()) {
				chr.warp(ServerConstants.warpMap);
				chr.dropMessage(1, "錯誤，請聯系運營商.");
			}
			return;
		}
	}
	...
	public void MapiaNight(final MapleCharacter player) {
		final int[] maps = { this.citizenmap1, this.citizenmap2, this.citizenmap3, this.citizenmap4, this.citizenmap5,
				this.citizenmap6 };
		this.nightnumber++;
		final java.util.Timer m_timer = new java.util.Timer();
					...
					for (MapleCharacter chr : MapleMap.this.getCharacters()) {
						if (!chr.isDead) {
							...
							if (chr.mapiajob == "시민") {
								...
								chr.dropMessage(5, MapleMap.this.nightnumber
										+ "번째 밤이 되었습니다. 마피아, 경찰, 의사가 투표를 모두 할때까지 잠시만 기다려 주세요.");
								citizen++;
							} else if (chr.mapiajob == "마피아") {
								chr.warp(MapleMap.this.mapiamap);
								chr.isMapiaVote = true;
								chr.dropMessage(5,
										MapleMap.this.nightnumber + "번째 밤이 되었습니다. 바로 옆의 엔피시를 통해 암살할 사람을 지목해 주세요. 제한시간은 "
												+ MapleMap.this.nighttime + "초 입니다.");
							} else if (chr.mapiajob == "경찰") {
								chr.warp(MapleMap.this.policemap);
								chr.isPoliceVote = true;
								chr.dropMessage(5,
										MapleMap.this.nightnumber
												+ "번째 밤이 되었습니다. 바로 옆의 엔피시를 통해 마피아 일것 같다는 사람을 지목 해 주세요. 제한시간은 "
												+ MapleMap.this.nighttime + "초 입니다.");
								...
								chr.dropMessage(5,
										MapleMap.this.nightnumber
												+ "번째 밤이 되었습니다. 바로 옆의 엔피시를 통해 살리고 싶은 사람을 지목 해 주세요. 제한시간은 "
												+ MapleMap.this.nighttime + "초 입니다.");
							}
	...
	public final boolean isLevelMob(MapleCharacter chr) {
		...
		for (MapleMonster monster : getAllMonster()) {
			...
			if (!monster.getStats().isBoss() && monster.getStats().getLevel() - 21 <= chr.getLevel()
					&& chr.getLevel() <= monster.getStats().getLevel() + 21) {
				already = true;
				break;
			}
		}
	...
	public final void addMapObject(MapleMapObject mapobject) {
		...
		((ConcurrentHashMap<Integer, MapleMapObject>) this.mapobjects.get(mapobject.getType()))
				.put(Integer.valueOf(newOid), mapobject);
	}
	...
	public final Point calcPointBelow(Point initial) {
		...
		if (!fh.isWall() && fh.getY1() != fh.getY2()) {
			...
			if (fh.getY2() < fh.getY1()) {
				dropY = fh.getY1() - (int) (Math.cos(Math.atan(s2 / s1)) * Math.abs(initial.x - fh.getX1())
	...
	private void dropFromMonster(MapleCharacter chr, MapleMonster mob, boolean instanced) {
		...
		if (chr.getParty() != null) {
			if (mob.isHellMode() || mob.isExtreme()) {
				...
				int quantity = size == 1 ? 6
						: size == 2 ? 5 : size == 3 ? 4 : size == 4 ? 3 : size == 5 ? 2 : size == 6 ? 1 : 0;
				switch (mob.getId()) {
				case 8880405: // 진 힐라
				case 8645066: // 듄켈
				case 8644655: // 더스크
		...
		if (mob.getId() == 9390612 || mob.getId() == 9390610 || mob.getId() == 9390611 || mob.getId() == 8645066) {
			MapleCharacter pchr = chr.getClient().getChannelServer().getPlayerStorage()
					.getCharacterById(chr.getParty().getLeader().getId());
			if (pchr == null) {
		...
		if (chr.getV("arcane_quest_2") != null && Integer.parseInt(chr.getV("arcane_quest_2")) >= 0
				&& Integer.parseInt(chr.getV("arcane_quest_2")) < 6) {
			switch (mob.getId()) {
			case 8641000:
		...
		if (chr.getV("arcane_quest_3") != null && Integer.parseInt(chr.getV("arcane_quest_3")) >= 0
				&& Integer.parseInt(chr.getV("arcane_quest_3")) < 4) {
			switch (mob.getId()) {
			case 8642000:
			case 8642001:
			case 8642002:
		...
		if (chr.getV("arcane_quest_4") != null && Integer.parseInt(chr.getV("arcane_quest_4")) >= 0
				&& Integer.parseInt(chr.getV("arcane_quest_4")) < 4) {
			switch (mob.getId()) {
			case 8643000:
			case 8643001:
			case 8643002:
		...
		if (chr.getV("arcane_quest_5") != null && Integer.parseInt(chr.getV("arcane_quest_5")) >= 0
				&& Integer.parseInt(chr.getV("arcane_quest_5")) < 4) {
			switch (mob.getId()) {
			case 8644000:
			case 8644001:
			case 8644002:
		...
		if (chr.getV("arcane_quest_6") != null && Integer.parseInt(chr.getV("arcane_quest_6")) >= 0
				&& Integer.parseInt(chr.getV("arcane_quest_6")) < 4) {
			switch (mob.getId()) {
			case 8644400:
		...
		if (chr.getV("arcane_quest_7") != null && Integer.parseInt(chr.getV("arcane_quest_7")) >= 0
				&& Integer.parseInt(chr.getV("arcane_quest_7")) < 4) {
			switch (mob.getId()) {
			case 8644500:
		...
		int[] items = { 1004422, 1004423, 1004424, 1004425, 1004426, 1052882, 1052887, 1052888, 1052889, 1052890,
				1073030, 1073035, 1073032, 1073033, 1073034, 1082636, 1082637, 1082638, 1082639, 1082640, 1102775,
				1102794, 1102795, 1102796, 1102797, 1152174, 1152179, 1152176, 1152177, 1152178, 1212115, 1213017,
				1222109, 1232109, 1242116, 1242120, 1262017, 1272016, 1282016, 1292017, 1302333, 1312199, 1322250,
				1332274, 1342101, 1362135, 1372222, 1382259, 1402251, 1412177, 1422184, 1432214, 1442268, 1452252,
				1462239, 1472261, 1482216, 1492231, 1522138, 1532144, 1582017, 1592019 };
		int[] items2 = { 1004808, 1004809, 1004810, 1004811, 1004812, 1053063, 1053064, 1053065, 1053066, 1053067,
				1073158, 1073159, 1073160, 1073161, 1073162, 1082695, 1082696, 1082697, 1082698, 1082699, 1102940,
				1102941, 1102942, 1102943, 1102944, 1152196, 1152197, 1152198, 1152199, 1152200, 1212120, 1213018,
				1222113, 1232113, 1242121, 1242122, 1262039, 1272017, 1282017, 1292018, 1302343, 1312203, 1322255,
				1332279, 1342104, 1362140, 1372228, 1382265, 1402259, 1412181, 1422189, 1432218, 1442274, 1452257,
				1462243, 1472265, 1482221, 1492235, 1522143, 1532150, 1582023, 1592020 };
		...
		for (Pair<Integer, Integer> list : ServerConstants.NeoPosList) {
			if (((Integer) list.getLeft()).intValue() == mob.getId()) {
				if (ServerConstants.Event_MapleLive) {
					customs.add(new MonsterDropEntry(2633609, 9999999, ((Integer) list.getRight()).intValue(),
							((Integer) list.getRight()).intValue(), 0, 0));
				}
				break;
			}
		}
		...
		if (GameConstants.isContentsMap(getId()) || chr.getMapId() / 100000 == 9530 || chr.getMapId() / 100000 == 9540
				|| (mob.getId() >= 9833935 && mob.getId() <= 9833946)
				|| (mob.getId() >= 9833947 && mob.getId() <= 9833958)) {
			finals.clear();
		}
				...
				for (MaplePartyCharacter pc : chr.getParty().getMembers()) {
					if (pc.isOnline() && pc.getId() != chr.getId()) {
						...
						if (player != null) {
							if (GameConstants.getInventoryType(de.itemId) == MapleInventoryType.EQUIP
									&& (mob.isExtreme() || mob.isHellMode())) {
								double d2 = (de.chance < 250) ? de.chance
										: (250.0D * dropBuff / 100.0D * showdown / 100.0D);
								continue;
							}
							d1 = de.chance * dropBuff / 100.0D * showdown / 100.0D;
						}
		...
		if (realfinals != null && !realfinals.isEmpty()) {
			...
			for (MonsterDropEntry de : realfinals) {
				...
				if (de.privated == 0 && (mob.getStats().isBoss() || de.itemId == 4001847 || de.itemId == 4001849
						|| de.itemId == 2434851)) {
					if (chr.getParty() != null) {
						for (MaplePartyCharacter pc : chr.getParty().getMembers()) {
							if (pc.isOnline() && pc.getId() != chr.getId()) {
	...
	public void drop(boolean mesoDropped, MonsterDropEntry de, MapleMonster mob, MapleCharacter chr, Point pos,
			byte d) {
	...
	public void drop(boolean mesoDropped, MonsterDropEntry de, MapleMonster mob, MapleCharacter chr, Point pos, byte d,
		...
		byte droptype = (byte) ((mob.getStats().isExplosiveReward() && mob.getStats().isBoss()) ? 3
				: ((chr.getParty() != null) ? 1 : 0));
		...
		if (de.itemId == 2633304) {
			...
			int i = 0;
			for (Pair<Integer, Integer> list : ServerConstants.NeoPosList) {
				if (((Integer) list.getLeft()).intValue() == mob.getId() && Integer.parseInt(wishCoinCheck[i]) == 1) {
					return;
				}
				i++;
			}
		...
		if (de.itemId == 0) {
			...
			if (mesos > 0) {
				if (GameConstants.isLinkMap(chr.getMapId())) {
					...
					spawnMobMesoDrop((int) (mesos * mesobuff / 100.0D * chr.getDropMod() * cmServerrate),
			...
			if (GameConstants.getInventoryType(de.itemId) == MapleInventoryType.EQUIP) {
				...
				if (mob.isExtreme() || mob.isHellMode()) {
					int[] items = { 1004422, 1004423, 1004424, 1004425, 1004426, 1052882, 1052887, 1052888, 1052889,
							1052890, 1073030, 1073035, 1073032, 1073033, 1073034, 1082636, 1082637, 1082638, 1082639,
							1082640, 1102775, 1102794, 1102795, 1102796, 1102797, 1152174, 1152179, 1152176, 1152177,
							1152178, 1212115, 1213017, 1222109, 1232109, 1242116, 1242120, 1262017, 1272016, 1282016,
							1292017, 1302333, 1312199, 1322250, 1332274, 1342101, 1362135, 1372222, 1382259, 1402251,
							1412177, 1422184, 1432214, 1442268, 1452252, 1462239, 1472261, 1482216, 1492231, 1522138,
							1532144, 1582017, 1592019 };
					int[] items2 = { 1004808, 1004809, 1004810, 1004811, 1004812, 1053063, 1053064, 1053065, 1053066,
							1053067, 1073158, 1073159, 1073160, 1073161, 1073162, 1082695, 1082696, 1082697, 1082698,
							1082699, 1102940, 1102941, 1102942, 1102943, 1102944, 1152196, 1152197, 1152198, 1152199,
							1152200, 1212120, 1213018, 1222113, 1232113, 1242121, 1242122, 1262039, 1272017, 1282017,
							1292018, 1302343, 1312203, 1322255, 1332279, 1342104, 1362140, 1372228, 1382265, 1402259,
							1412181, 1422189, 1432218, 1442274, 1452257, 1462243, 1472265, 1482221, 1492235, 1522143,
							1532150, 1582023, 1592020 };
					for (int item : items) {
					...
					case 8880177:
						...
						break;
					case 8880302:
						if (idrop.getItemId() == 1162080 || idrop.getItemId() == 1162081 || idrop.getItemId() == 1162082
								|| idrop.getItemId() == 1162083) {
							Equip equip = (Equip) idrop;
			...
			if ((mob.getId() == 8820212 || mob.getId() == 8880302 || mob.getId() == 8880342 || mob.getId() == 8880150
					|| mob.getId() == 8880151 || mob.getId() == 8880155 || mob.getId() == 8644650
					|| mob.getId() == 8644655 || mob.getId() == 8880405 || mob.getId() == 8645009
					|| mob.getId() == 8645039 || mob.getId() == 8880504 || mob.getId() == 8880602
					|| mob.getId() == 8880614 || mob.getId() == 8820101 || mob.getId() == 8820001
					|| mob.getId() == 8860005 || mob.getId() == 8860000 || mob.getId() == 8810018
					|| mob.getId() == 8810122 || mob.getId() == 8840007 || mob.getId() == 8840000
					|| mob.getId() == 8840014 || mob.getId() == 8800102 || mob.getId() == 8500012
					|| mob.getId() == 8500002 || mob.getId() == 8880200 || mob.getId() == 8870000
					|| mob.getId() == 8800002 || mob.getId() == 8500022 || mob.getId() == 8870100
					|| mob.getId() == 8910100 || mob.getId() == 8930100 || mob.getId() == 8910000
					|| mob.getId() == 8930000 || mob.getId() == 8850011 || mob.getId() == 8850111
					|| mob.getId() == 8950102 || mob.getId() == 8950002 || mob.getId() == 8880111
					|| mob.getId() == 8880101 || (mob.getId() >= 8900100 && mob.getId() <= 8900102)
					|| (mob.getId() >= 8900000 && mob.getId() <= 8900002)
					|| (mob.getId() >= 8920000 && mob.getId() <= 8920003)
					|| (mob.getId() >= 8920100 && mob.getId() <= 8920103) || mob.getId() == 8920106
					|| mob.getId() == 8900103 || mob.getId() == 8880157 || mob.getId() == 8880167
					|| mob.getId() == 8880177 || mob.getId() == 8880614) && chr.getPlayer() != null) {
				FileoutputUtil.log(FileoutputUtil.首領獲得物品日誌,
						"[획득] 계정 번호 : " + chr.getClient().getAccID() + " | 파티번호 : " + chr.getPlayer().getParty().getId()
								+ " | 캐릭터 : " + chr.getName() + "(" + chr.getId() + ") | 보스 클리어 "
								+ MapleLifeFactory.getMonster(mob.getId()).getStats().getName() + "(" + mob.getId()
								+ ") | 드롭 : " + MapleItemInformationProvider.getInstance().getName(idrop.getItemId())
								+ "(" + idrop.getItemId() + ")를 [" + idrop.getQuantity() + "]개 획득");
			}
			if (idrop.getItemId() == 4001886) {
				idrop.setBossid(mob.getId());
			}
			if (de.privated == 0 && (mob.getStats().isBoss() || idrop.getItemId() == 4001847
					|| idrop.getItemId() == 4001849 || idrop.getItemId() == 2434851 || mob.getScale() > 100)) {
	...
	public final void killMonster(final MapleMonster monster, final MapleCharacter chr, boolean withDrops,
			boolean second, byte animation, int lastSkill) {
		...
		if (chr.getBattleGroundChr() != null) {
			...
			if (monster.getId() == 9300008 || monster.getId() == 9300014) {
				...
				if (q == 0) {
					...
					broadcastMessage(CField.startMapEffect("다크아이와 쉐도우 아이를 모두 퇴치하였습니다. 엘로그린 벌룬에게 말을 걸어 다음 스테이지로 이동해주세요!",
							5120018, true));
				}
			}
			...
			Timer.MapTimer.getInstance().schedule(new Runnable() {
				public void run() {
					if (!monster.getStats().getName().contains("드래곤킹")) {
						MapleMap.this.spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(monster.getId()),
								new Point((monster.getPosition()).x,
										(monster.getStats().getName().contains("도라지")
												|| monster.getStats().getName().contains("요괴선사"))
														? ((monster.getPosition()).y - 300)
		...
		if (monster.getBuff(MonsterStatus.MS_Treasure) != null) {
			...
			if (buffowner != null) {
				...
				MapleMist newmist = new MapleMist(
						a.calculateBoundingBox(buffowner.getPosition(), buffowner.isFacingLeft()), buffowner, a,
						(int) buffowner.getBuffLimit(400021096), (byte) (buffowner.isFacingLeft() ? 1 : 0));
		...
		if (this.Monstermarble == 20) {
			...
			broadcastMessage(CField.environmentChange("Map/Effect.img/quest/party/clear", 4));
			broadcastMessage(
					CField.startMapEffect("모든 몬스터 구슬을 획득하였습니다. 파티장이 엘린에게 말을 걸어 다음 스테이지로 이동하여 주세요.", 5120023, true));
			broadcastMessage(CField.achievementRatio(50));
		}
		...
		if (partyquest == 1) {
			...
			if (moonstak % 2 == 0) {
				...
				// 위 두줄 같음
				MapleMonster mob;
				for (MapleMapObject monstermo : getMapObjectsInRange(chr.getPosition(), Double.POSITIVE_INFINITY,
		...
		if (getId() == 450001400) {
			...
			chr.getClient().send(SLFCGPacket.ErdaSpectrumGauge((int) chr.getSkillCustomValue0(450001400),
					(int) chr.getSkillCustomValue0(8641018), (int) chr.getSkillCustomValue0(450001402)));
			chr.getClient()
					.send(SLFCGPacket.EventSkillOnEffect((monster.getPosition()).x, (monster.getPosition()).y, 2, 1));
		}
		if (this.RealSpawns.contains(monster)) {
			this.RealSpawns.remove(monster);
		}
		...
		if (monster.isExtreme() || monster.isHellMode()) {
			...
			if (itemArray.length > 0) {
				...
				if (toDrop != null) {
					if (prob >= rand) {
						// spawnItemDrop(monster, chr, toDrop, new Point((monster.getTruePosition()).x,
						// (monster.getTruePosition()).y), true, false);
...
//                }
		...
		try {
			...
			if ((monster.getId() >= 8850000 && monster.getId() <= 8850004)
					|| (monster.getId() >= 8850100 && monster.getId() <= 8850104)) {
			...
			for (String qlis : qlist) {
				if (chr.getV(qlis + "_" + monster.getId() + "_isclear") != null
						&& chr.getV(qlis + "_" + monster.getId() + "_count") != null
						&& chr.getV(qlis + "_" + monster.getId() + "_mobq") != null) {
					if (chr.getV(qlis + "_" + monster.getId() + "_isclear").equals("0")
							&& Integer.parseInt(chr.getV(qlis + "_" + monster.getId() + "_count")) < Integer
									.parseInt(chr.getV(qlis + "_" + monster.getId() + "_mobq"))) {
						int count = Integer.parseInt(chr.getV(qlis + "_" + monster.getId() + "_count")) + 1;
						chr.addKV(qlis + "_" + monster.getId() + "_count", "" + count);
						chr.dropMessage(-1,
								monster.getStats().getName() + " " + chr.getV(qlis + "_" + monster.getId() + "_count")
										+ " / " + chr.getV(qlis + "_" + monster.getId() + "_mobq"));
						if (Integer.parseInt(chr.getV(qlis + "_" + monster.getId() + "_count")) >= Integer
								.parseInt(chr.getV(qlis + "_" + monster.getId() + "_mobq"))) {
				...
				if (chr.getParty() != null) {
					if (chr.getParty().getMembers().size() >= 2) {
						for (MaplePartyCharacter partychar : chr.getParty().getMembers()) {
							if (partychar != null && partychar.getMapid() == chr.getMapId()
									&& partychar.getChannel() == this.channel) {
								MapleCharacter other = chr.getClient().getChannelServer().getPlayerStorage()
										.getCharacterByName(partychar.getName());
								if (other.getId() != chr.getId() && other != null && other.getGuild() != null
										&& other.getMapId() == chr.getMapId()) {
									if (other.getGuildId() == chr.getGuildId()) {
										party = true;
										World.Guild.gainContribution(other.getGuildId(),
												GameConstants.GPboss(monster.getId(), party, false), other.getId());
										continue;
									}
									World.Guild.gainContribution(other.getGuildId(),
											GameConstants.GPboss(monster.getId(), party, false), other.getId());
					...
					World.Guild.gainContribution(chr.getGuildId(), GameConstants.GPboss(monster.getId(), party, false),
							chr.getId());
				} else {
					World.Guild.gainContribution(chr.getGuildId(), GameConstants.GPboss(monster.getId(), party, false),
							chr.getId());
				...
				if (!next && end) {
					if (getElitechmptype() == 2) {
						...
						for (int i = 0; i < 10; i++) {
							...
							EliteChmp.setElitechmp(true);
							spawnMonsterOnGroundBelow(EliteChmp, new Point(
									(monster.getPosition()).x + Randomizer.rand(-150, 150), (monster.getPosition()).y));
						}
					...
					for (MapleMonster mob : getAllMonster()) {
						if (monster.getId() != mob.getId() && (mob.getId() == 8220123 || mob.getId() == 8220122
								|| mob.getId() == 8220124 || mob.getId() == 8220125
								|| (mob.getId() >= 8220100 && mob.getId() <= 8220108))) {
					...
					for (int i = 0; i < 3; i++) {
						...
						if (fh != null) {
							spawnMobPublicDrop(new Item(2023927, (short) 0, (short) 1, 0), new Point(x, y), monster,
									chr, (byte) 0, 0);
						}
					}
					...
					if (!monster.isElitemonster()) {
						...
						if (chr.getKeyValue(51351, "startquestid") == 49012L
								|| chr.getKeyValue(51351, "startquestid") == 49013L
								|| chr.getKeyValue(51351, "startquestid") == 49014L) {
							int maxmob = (chr.getKeyValue(51351, "startquestid") == 49012L) ? 1
									: ((chr.getKeyValue(51351, "startquestid") == 49013L) ? 2 : 3);
							String q = "";
							int count = Integer.parseInt(
									chr.getQuest(MapleQuest.getInstance((int) chr.getKeyValue(51351, "startquestid")))
											.getCustomData());
							count++;
							if (count < 10) {
								q = "00" + count;
							...
							if (count >= maxmob) {
								...
								chr.getClient().send(CWvsContext.updateSuddenQuest(
										(int) chr.getKeyValue(51351, "midquestid"), false,
					...
					broadcastMessage(
							CField.startMapEffect((this.eliteCount <= 15) ? "어두운 기운이 사라지지 않아 이곳을 음산하게 만들고 있습니다."
									: "이곳이 어둠으로 가득차 곧 무슨일이 일어날 듯 합니다.", 5120124, true));
				}
			}
			...
			if ((monster.getId() == 8500003 || monster.getId() == 8500004 || monster.getId() == 8500007
					|| monster.getId() == 8500008) && Randomizer.isSuccess(100)) {
				int itemcode = 0;
				...
				spawnItemDrop(monster, chr, toDrop,
						new Point((monster.getTruePosition()).x, (monster.getTruePosition()).y), true, false);
			}
			if (chr.getClient().getQuestStatus(100825) == 2) {
				if (chr.isGM() || (monster.getStats().getLevel() - 21 <= chr.getLevel()
						&& chr.getLevel() <= monster.getStats().getLevel() + 21 && isSpawnPoint()
						&& !GameConstants.isContentsMap(getId()) && !GameConstants.보스맵(getId())
						&& !GameConstants.사냥컨텐츠맵(getId()) && !GameConstants.튜토리얼(getId())
						&& !GameConstants.로미오줄리엣(getId()) && !GameConstants.피라미드(getId()))) {
					if (!monster.getStats().isBoss() && !monster.isElitemonster() && !monster.isElitechmp()
							&& !monster.isEliteboss()) {
					...
					if (chr.getBuffedValue(80003064) && chr.getSkillCustomValue0(100857) <= 0L) {
						if (chr.getKeyValue(100857, "feverCnt") >= 10L) {
							...
							chr.setKeyValue(100857, "count",
									(chr.getKeyValue(100857, "count") + (chr.isGM() ? 1L : 1L)) + "");
							if (chr.getKeyValue(100857, "count") == 250L) {
								if (Randomizer.isSuccess(50)) {
									chr.getClient().send(CField.enforceMsgNPC(9062549, 3000,
											"신나는 #r팡팡 리액션#k을 하기까지\r\n절반 정도 왔어! 준비하라고!"));
								} else {
									chr.getClient().send(CField.enforceMsgNPC(9062549, 3000,
											"벌써 #r250마리#k의 몬스터를 처치!\r\n나한테 사냥을 배워서 그런가?\r\n정말 빠른걸!"));
								}
								...
								if (chr.getClient().getCustomKeyValue(501567, "fever") < 200L) {
									chr.getClient().setCustomKeyValue(501567, "fever",
											chr.getClient().getCustomKeyValue(501567, "fever") + 1L + "");
								}
								chr.getClient().send(CField.enforceMSG(
										"\uba54\uc774\ud50c \ud321\ud321 \ub9ac\uc561\uc158 \ud0c0\uc784! \uc5b4\uc11c \ud6c4\uc6d0\ubc1b\uc740 \ud321\ud321 \uc8fc\uba38\ub2c8\ub97c \ud130\ub728\ub824!",
										341, 20000));
								chr.getClient().send(CField.enforceMsgNPC(9062549, 3000,
										"\ud6c4\uc6d0\uc774 \ud130\uc9c0\ub294 \uc774 \uc21c\uac04!\r\n\uba4b\uc9c4 #r\ud321\ud321 \ub9ac\uc561\uc158#k\uc744 \ubcf4\uc5ec\uc918!"));
								chr.setKeyValue(100857, "count", "0");
								...
								Timer.EtcTimer.getInstance().schedule(() -> {
									...
									Timer.EtcTimer.getInstance().schedule(() -> {
										if (chr.getSkillCustomValue(100857) != null) {
											chr.getClient().send(CField.enforceMsgNPC(9062549, 3000,
													"\uc815\ub9d0 \ubaa8\ub450\uc758 \uc18d\uc774 \ub2e4 \uc2dc\uc6d0\ud574\uc9c0\ub294 #r\ud321\ud321 \ub9ac\uc561\uc158#k\uc774\uc5c8\uc5b4!"));
											chr.getClient()
													.send(CField.UIPacket.detailShowInfo(
															"\uc624\ub298\uc758 \ub9ac\uc561\uc158 \ud69f\uc218 : "
																	+ chr.getKeyValue(100857, "feverCnt") + "/10\ud68c",
															3, 20, 20));
											chr.dropMessage(5, "\uc624\ub298\uc758 \ub9ac\uc561\uc158 \ud69f\uc218 : "
													+ chr.getKeyValue(100857, "feverCnt") + "/10\ud68c");
			...
			if (monster.getId() == 9833965 && chr.getSkillCustomValue0(100857) > 0L) {
				...
				if (chr.getSkillCustomValue0(100857) <= 0L) {
					...
					chr.getClient().send(CField.UIPacket
							.detailShowInfo("오늘의 리액션 횟수 : " + chr.getKeyValue(100857, "feverCnt") + "/10회", 3, 20, 20));
			...
			if (chr.getKeyValue(51351, "startquestid") >= 49001L && chr.getKeyValue(51351, "startquestid") <= 49003L
					&& monster.getStats().getLevel() - 21 <= chr.getLevel()
					&& chr.getLevel() <= monster.getStats().getLevel() + 21 && isSpawnPoint()
					&& !GameConstants.isContentsMap(getId()) && !GameConstants.보스맵(getId())
					&& !GameConstants.사냥컨텐츠맵(getId()) && !GameConstants.튜토리얼(getId()) && !GameConstants.로미오줄리엣(getId())
					&& !GameConstants.피라미드(getId())) {
				int maxmob = (chr.getKeyValue(51351, "startquestid") == 49001L) ? 100
						: ((chr.getKeyValue(51351, "startquestid") == 49002L) ? 200 : 300);
				String q = "";
				int count = Integer
						.parseInt(chr.getQuest(MapleQuest.getInstance((int) chr.getKeyValue(51351, "startquestid")))
								.getCustomData());
				count++;
				if (count < 10) {
					q = "00" + count;
			...
			if (monster.getBuff(2111010) != null) {
				for (int i = 0; i < 2; i++) {
					MapleSummon tosummon = new MapleSummon(chr, 2111010, monster.getTruePosition(),
							SummonMovementType.WALK_STATIONARY, (byte) 0, (int) chr.getBuffLimit(2111010));
			...
			if (isElitebossrewardmap() && getElitebossrewardtype() == 1 && monster.getId() == 8220027
				...
				spawnItemDrop(monster, chr, toDrop,
						new Point((monster.getTruePosition()).x + 25, (monster.getTruePosition()).y), true, false);
			}
			if (!isElitebossmap() && !isEliteChmpmap() && !monster.isElitemonster() && !isElitebossrewardmap()
					&& !monster.isEliteboss() && !monster.getStats().isBoss() && monster.getStats().getLevel() > 100
					&& monster.getStats().getLevel() - 21 <= chr.getLevel()
					&& chr.getLevel() <= monster.getStats().getLevel() + 21 && isSpawnPoint()) {
...
//                chr.checkLiveQuest(1, false);
				if (chr.getKeyValue(51351, "startquestid") == 49012L || chr.getKeyValue(51351, "startquestid") == 49013L
						|| chr.getKeyValue(51351, "startquestid") == 49014L) {
					int maxmob = (chr.getKeyValue(51351, "startquestid") == 49012L) ? 1
							: ((chr.getKeyValue(51351, "startquestid") == 49013L) ? 2 : 3);
					String q = "";
			...
			if (monster.getId() == 8800002) {
				World.Broadcast.broadcastMessage(CField.getGameMessage(4,
						"[" + chr.getParty().getLeader().getName() + "]님의 파티가 노말 자쿰을 격파하였습니다!"));
			}

			if (monster.getId() == 8860000) {
				World.Broadcast.broadcastMessage(CField.getGameMessage(8,
						"[" + chr.getParty().getLeader().getName() + "]님의 파티가 노말 아카이럼을 격파하였습니다!"));
			}

			if (monster.getId() == 8880010) {
				World.Broadcast.broadcastMessage(CField.getGameMessage(8,
						"[" + chr.getParty().getLeader().getName() + "]님의 파티가 이지 매그너스를 격파하였습니다!"));
			}
			if (monster.getId() == 8880002) {
				World.Broadcast.broadcastMessage(CField.getGameMessage(8,
						"[" + chr.getParty().getLeader().getName() + "]님의 파티가 노말 매그너스를 격파하였습니다!"));
			}
			if (monster.getId() == 8880000) {
				World.Broadcast.broadcastMessage(CField.getGameMessage(8,
						"[" + chr.getParty().getLeader().getName() + "]님의 파티가 하드 매그너스를 격파하였습니다!"));
			}

			if (monster.getId() == 8500022) {
				World.Broadcast.broadcastMessage(CField.getGameMessage(8,
						"[" + chr.getParty().getLeader().getName() + "]님의 파티가 카오스 파풀라투스를 격파하였습니다!"));
			}

			if (monster.getId() == 8950102) {
				World.Broadcast.broadcastMessage(CField.getGameMessage(8,
						"[" + chr.getParty().getLeader().getName() + "]님의 파티가 노말 스우를 격파하였습니다!"));
			}
			if (monster.getId() == 8950002) {
				World.Broadcast.broadcastMessage(CField.getGameMessage(8,
						"[" + chr.getParty().getLeader().getName() + "]님의 파티가 하드 스우를 격파하였습니다!"));
			}

			if (monster.getId() == 8880111) {
				World.Broadcast.broadcastMessage(CField.getGameMessage(8,
						"[" + chr.getParty().getLeader().getName() + "]님의 파티가 노말 데미안을 격파하였습니다!"));
			}
			if (monster.getId() == 8880101) {
				World.Broadcast.broadcastMessage(CField.getGameMessage(8,
						"[" + chr.getParty().getLeader().getName() + "]님의 파티가 하드 데미안을 격파하였습니다!"));
			}

			if (monster.getId() == 8880151) {
				World.Broadcast.broadcastMessage(CField.getGameMessage(8,
						"[" + chr.getParty().getLeader().getName() + "]님의 파티가 노말 루시드를 격파하였습니다!"));
			}
			if (monster.getId() == 8880153) {
				World.Broadcast.broadcastMessage(CField.getGameMessage(8,
						"[" + chr.getParty().getLeader().getName() + "]님의 파티가 하드 루시드를 격파하였습니다!"));
			}

			if (monster.getId() == 8880302) {
				World.Broadcast.broadcastMessage(CField.getGameMessage(8,
						"[" + chr.getParty().getLeader().getName() + "]님의 파티가 하드 윌을 격파하였습니다!"));
			}
			...
			if (monster.getId() == 8644655) {
				if (monster.isHellMode()) {
					...
					World.Broadcast.broadcastMessage(CField.getGameMessage(8,
							"[" + chr.getParty().getLeader().getName() + "]님의 파티가 헬 더스크를 격파하였습니다."));
				} else if (monster.isExtreme()) {
					World.Broadcast.broadcastMessage(CField.getGameMessage(8,
							"[" + chr.getParty().getLeader().getName() + "]님의 파티가 익스트림 더스크를 격파하였습니다."));
				} else {
					World.Broadcast.broadcastMessage(CField.getGameMessage(8,
							"[" + chr.getParty().getLeader().getName() + "]님의 파티가 카오스 더스크를 격파하였습니다."));
				}
			}
			...
			if (monster.getId() == 8645066) {
				if (monster.isHellMode()) {
					World.Broadcast.broadcastMessage(CField.getGameMessage(8,
							"[" + chr.getParty().getLeader().getName() + "]님의 파티가 헬 듄켈을 격파하였습니다."));
				} else if (monster.isExtreme()) {
					World.Broadcast.broadcastMessage(CField.getGameMessage(8,
							"[" + chr.getParty().getLeader().getName() + "]님의 파티가 익스트림 듄켈을 격파하였습니다."));
				} else {
					World.Broadcast.broadcastMessage(CField.getGameMessage(8,
							"[" + chr.getParty().getLeader().getName() + "]님의 파티가 친위대장 듄켈을 격파하였습니다."));
				}
			}
			...
			if (monster.getId() == 8880405) {
				if (monster.isHellMode()) {
					World.Broadcast.broadcastMessage(CField.getGameMessage(8,
							"[" + chr.getParty().getLeader().getName() + "]님의 파티가 헬 진힐라를 격파하였습니다."));
				} else if (monster.isExtreme()) {
					World.Broadcast.broadcastMessage(CField.getGameMessage(8,
							"[" + chr.getParty().getLeader().getName() + "]님의 파티가 익스트림 진힐라를 격파하였습니다."));
				} else {
					World.Broadcast.broadcastMessage(CField.getGameMessage(8,
							"[" + chr.getParty().getLeader().getName() + "]님의 파티가 고통의 미궁 : 진힐라를 격파하였습니다."));
				}
			}
			...
			if (monster.getId() == 8880504) {
				if (monster.isHellMode()) {
					World.Broadcast.broadcastMessage(CField.getGameMessage(8,
							"[" + chr.getParty().getLeader().getName() + "]님의 파티가 헬 검은 마법사를 격파하였습니다."));
				} else if (monster.isExtreme()) {
					World.Broadcast.broadcastMessage(CField.getGameMessage(8,
							"[" + chr.getParty().getLeader().getName() + "]님의 파티가 익스트림 검은 마법사를 격파하였습니다."));
				} else {
					World.Broadcast.broadcastMessage(CField.getGameMessage(8,
							"[" + chr.getParty().getLeader().getName() + "]님의 파티가 검은 마법사를 격파하였습니다."));
				}
			}
			...
			if (monster.getId() == 8880602) {
				if (monster.isHellMode()) {
					World.Broadcast.broadcastMessage(CField.getGameMessage(8,
							"[" + chr.getParty().getLeader().getName() + "]님의 파티가 헬 세렌을 격파하였습니다."));
				} else if (monster.isExtreme()) {
					World.Broadcast.broadcastMessage(CField.getGameMessage(8,
							"[" + chr.getParty().getLeader().getName() + "]님의 파티가 익스트림 세렌을 격파하였습니다."));
				} else {
					World.Broadcast.broadcastMessage(CField.getGameMessage(8,
							"[" + chr.getParty().getLeader().getName() + "]님의 파티가 선택받은 세렌을 격파하였습니다."));
				}
			}

			if (monster.getId() == 8880808) {
				World.Broadcast.broadcastMessage(
						CField.getGameMessage(8, "[" + chr.getParty().getLeader().getName() + "]님의 파티가 칼로스를 격파하였습니다."));
			}

			if (monster.getId() == 8880842) {
				World.Broadcast.broadcastMessage(
						CField.getGameMessage(8, "[" + chr.getParty().getLeader().getName() + "]님의 파티가 카링을 격파하였습니다."));
			}

			if (monster.getId() == 8881000) {
				World.Broadcast.broadcastMessage(CField.getGameMessage(8,
						"[" + chr.getParty().getLeader().getName() + "]님의 파티가 우르스 토벌에 성공 하였습니다."));
			}

			if (getCustomValue(8222222) == null && this.EliteMobCommonCount >= 700) {
				this.EliteMobCommonCount = 0;
				if (getEliteCount() == 5 && monster.getEliteGrade() <= -1 && isSpawnPoint() && !isEliteChmpmap()
						&& !isElitebossmap() && !isElitebossrewardmap()) {
					...
					case 3:
						...
						for (k = 0; k < 15; k++) {
							...
							mapleMonster.setElitehp(mapleMonster.getHp());
							spawnMonsterOnGroundBelow(mapleMonster,
									((Spawns) this.monsterSpawn.get(Rand)).getPosition());
						}
						setElitechmpfinal(true);
					...
					Timer.MapTimer.getInstance().schedule(() -> {
						if (isEliteChmpmap()) {
							...
							for (MapleMonster mob : getAllMonster()) {
								if (mob.getId() == 8220123 || mob.getId() == 8220111 || mob.getId() == 8220122
										|| mob.getId() == 8220124 || mob.getId() == 8220125
										|| (mob.getId() >= 8220100 && mob.getId() <= 8220108)) {
									killMonster(mob.getId());
									continue;
				...
				} else if (getEliteCount() < 20 && monster.getStats().getLevel() >= 100 && isSpawnPoint()
						&& !isElitebossmap() && !isElitebossrewardmap() && !isEliteChmpmap()
						&& !monster.isElitemonster() && monster.getEliteGrade() < 0) {
					...
					if (!alreadyspawn) {
						/*
						 * if (getEliteCount() >= 10) { if (!isElitebossmap() &&
						 * !isElitebossrewardmap()) { startEliteBossMap();
						 * broadcastMessage(CField.specialMapEffect(2, false, "Bgm36.img/RoyalGuard",
						 * "Effect/EliteMobEff.img/eliteMonsterFrame",
						 * "Effect/EliteMobEff.img/eliteMonsterEffect", "", ""));
						 * setEliteMobCommonCount(0); setEliteCount(0); setEliteMobCount(0);
						 * setElitebossmap(true); final int RandomI = Randomizer.rand(0, 4); final int
						 * BossID = 8220020 + RandomI + 2; final int EffectID = 5120120 + RandomI + 5;
						 * final String[] EffectMsgs = {"검은 기사 모카딘 : 위대한 분을 위하여 너를 처단하겠다.",
						 * "미친 마법사 카리아인 : 미천한 것들이 날뛰고 있구나. 크크크크...", "돌격형 CQ57 : 목표발견. 제거 행동을 시작한다.",
						 * "인간사냥꾼 줄라이 : 사냥감이 나타났군.", "싸움꾼 플레드 : 재미 있겠군. 어디 한 번 놀아볼까."};
						 * killAllMonsters(true); Timer.MapTimer.getInstance().schedule(new Runnable() {
						 * public void run() { MapleMonster eliteBoss =
						 * MapleLifeFactory.getMonster(BossID); int rand = Randomizer.rand(0,
						 * EliteMonsterGradeInfo.getThirdGradeInfo().size() - 1);
						 * eliteBoss.getEliteGradeInfo().add(new
						 * Pair<>(Integer.valueOf(((EliteMonsterGradeInfo)
						 * EliteMonsterGradeInfo.getThirdGradeInfo().get(Integer.valueOf(rand))).
						 * getSkillid()), Integer.valueOf(((EliteMonsterGradeInfo)
						 * EliteMonsterGradeInfo.getThirdGradeInfo().get(Integer.valueOf(rand))).
						 * getSkilllv()))); eliteBoss.setEliteType(3); eliteBoss.setEliteGrade(RandomI);
						 * eliteBoss.getStats().setLevel(monster.getStats().getLevel());
						 * eliteBoss.setHp(monster.getStats().getHp() * 750L);
						 * eliteBoss.setEliteboss(true); eliteBoss.setElitehp(eliteBoss.getHp());
						 * MapleMap.this.makeEliteMonster(monster.getId(), Randomizer.rand(0, 2),
						 * ((Spawns) MapleMap.this.monsterSpawn.get(Randomizer.rand(0,
						 * MapleMap.this.monsterSpawn.size() - 1))).getPosition(), false, true);
						 * MapleMap.this.makeEliteMonster(monster.getId(), Randomizer.rand(0, 2),
						 * ((Spawns) MapleMap.this.monsterSpawn.get(Randomizer.rand(0,
						 * MapleMap.this.monsterSpawn.size() - 1))).getPosition(), false, true);
						 * MapleMap.this.spawnMonsterOnGroundBelow(eliteBoss,
						 * monster.getTruePosition());
						 * MapleMap.this.broadcastMessage(CField.startMapEffect(EffectMsgs[RandomI],
						 * EffectID, true));
						 * World.Broadcast.broadcastMessage(CWvsContext.eliteWarning(MapleMap.this.getId
						 * (), eliteBoss.getId(), MapleMap.this.getChannel())); } }, 6000L); } } else {
						 * this.EliteMobCommonCount = 0; makeEliteMonster(monster.getId(),
						 * Randomizer.rand(0, 2), monster.getPosition(), false, false); }
						 */
					}
				}
			}
		...
		if (monster.getId() == 8910001) {
			MapleMap warp = ChannelServer.getInstance(chr.getClient().getChannel()).getMapFactory()
					.getMap(chr.getMapId() - 10);
			...
			Timer.MapTimer.getInstance().schedule(new Runnable() {
				public void run() {
					for (MapleCharacter chr : MapleMap.this.getAllCharactersThreadsafe()) {
						MapleMap zz = ChannelServer.getInstance(chr.getClient().getChannel()).getMapFactory()
								.getMap(chr.getMapId() - 10);
		...
		if (monster.getId() / 1000 == 9800 && monster.getMobExp() == 0L) {
			...
			chr.getClient().getSession()
					.writeAndFlush(CField.startMapEffect("경험치 보상 " + formatter.format(oneexp) + " 누적!", 5120162, true));
		}
		boolean dispel = false, killall = false;
		KoreaCalendar kc = new KoreaCalendar();
		String today = (kc.getYeal() % 100) + "/" + kc.getMonths() + "/" + kc.getDays();
		if (monster.getId() == 8820212 || monster.getId() == 8880302 || monster.getId() == 8880342
				|| monster.getId() == 8880150 || monster.getId() == 8880151 || monster.getId() == 8880155
				|| monster.getId() == 8644650 || monster.getId() == 8644655 || monster.getId() == 8880405
				|| monster.getId() == 8645009 || monster.getId() == 8645039 || monster.getId() == 8880504
				|| monster.getId() == 8880602 || monster.getId() == 8880614 || monster.getId() == 8820101
				|| monster.getId() == 8820001 || monster.getId() == 8860005 || monster.getId() == 8860000
				|| monster.getId() == 8810018 || monster.getId() == 8810122 || monster.getId() == 8840007
				|| monster.getId() == 8840000 || monster.getId() == 8840014 || monster.getId() == 8800102
				|| monster.getId() == 8500012 || monster.getId() == 8500002 || monster.getId() == 8880200
				|| monster.getId() == 8870000 || monster.getId() == 8800002 || monster.getId() == 8500022
				|| monster.getId() == 8870100 || monster.getId() == 8910100 || monster.getId() == 8930100
				|| monster.getId() == 8910000 || monster.getId() == 8930000 || monster.getId() == 8850011
				|| monster.getId() == 8850111 || monster.getId() == 8950102 || monster.getId() == 8950002
				|| monster.getId() == 8880111 || monster.getId() == 8880101
				|| (monster.getId() >= 8900100 && monster.getId() <= 8900102)
				|| (monster.getId() >= 8900000 && monster.getId() <= 8900002)
				|| (monster.getId() >= 8920000 && monster.getId() <= 8920003)
				|| (monster.getId() >= 8920100 && monster.getId() <= 8920103) || monster.getId() == 8920106
				|| monster.getId() == 8900103) {
			dispel = true;
			killall = true;
			if (chr.getParty() != null && !monster.isExtreme() && !monster.isHellMode()) {
				...
				for (MaplePartyCharacter chrs : chr.getParty().getMembers()) {
					if (chrs.getPlayer() != null && chr.getMapId() == chrs.getPlayer().getMapId() && chrs.isOnline()) {
						FileoutputUtil.log(FileoutputUtil.首領通關日誌,
								"[클리어] 계정 번호 : " + chrs.getPlayer().getClient().getAccID() + " | 파티번호 : "
										+ chrs.getPlayer().getParty().getId() + " | 캐릭터 : " + chrs.getPlayer().getName()
										+ "(" + chrs.getPlayer().getId() + ") | 클리어 보스 : "
										+ MapleLifeFactory.getMonster(monster.getId()).getStats().getName() + "("
										+ monster.getId() + ")");
						if (quest != 0) {
							if (quest == 34017) {
								(chrs.getPlayer()).Stigma = 0;
								Map<SecondaryStat, Pair<Integer, Integer>> dds = new HashMap<>();
								dds.put(SecondaryStat.Stigma,
										new Pair<>(Integer.valueOf((chrs.getPlayer()).Stigma), Integer.valueOf(0)));
								chrs.getPlayer().getClient().getSession()
										.writeAndFlush(CWvsContext.BuffPacket.cancelBuff(dds, chrs.getPlayer()));
								chrs.getPlayer().getMap().broadcastMessage(chrs.getPlayer(),
										CWvsContext.BuffPacket.cancelForeignBuff(chrs.getPlayer(), dds), false);
							}
			...
			if (chr.getParty() != null) {
				for (MaplePartyCharacter chrs : chr.getParty().getMembers()) {
					...
					chrs.getPlayer().updateInfoQuest((monster.getId() == 8880002) ? 3993 : 3992,
							"count=1;lasttime=" + today + "");
				}
			}
		}
		if (monster.getId() == 8880602 || monster.getId() == 8880140 || monster.getId() == 8880141
				|| monster.getId() == 8880142) {
			dispel = true;
			killAllMonsters(true);
		}
		...
		if (monster.getId() == 8880300 || monster.getId() == 8880303 || monster.getId() == 8880304
				|| monster.getId() == 8880340 || monster.getId() == 8880343 || monster.getId() == 8880344) {
			broadcastMessage(CField.enforceMSG("威爾變得認真了.我想鏡子深處會映照出威爾的真心.", 245, 7000));
		}
		if (monster.getId() == 8880301 || monster.getId() == 8880341) {
			broadcastMessage(CField.enforceMSG("威爾沒時間了.鏡子世界的最深處好像要暴露了.", 245, 7000));
		}
		...
		if (monster.getId() == 8860002 || monster.getId() == 8860006) {
			...
			spawnItemDrop(monster, chr, toDrop, new Point((monster.getTruePosition()).x, (monster.getTruePosition()).y),
					true, false);
		}
		if (monster.getId() >= 8920000 && monster.getId() <= 8920003 && withDrops) {
			...
			int size = 0;
			for (MapleMonster m : getAllMonster()) {
				if (m.getId() != 8820014 && m.getId() != 8820000 && m.getId() != 8820009 && m.getId() != 8820114
						&& m.getId() != 8820100 && m.getId() != 8820109) {
					size++;
				}
			}
			if (size == 0) {
				for (MapleMonster m : getAllMonster()) {
					if (m.getId() == 8820008 || m.getId() == 8820000 || m.getId() == 8820014 || m.getId() == 8820108
							|| m.getId() == 8820100 || m.getId() == 8820114) {
		...
		if (monster.getId() == 9390612 || monster.getId() == 9390610 || monster.getId() == 9390611
				|| monster.getId() == 8645066) {
			MapleCharacter pchr = chr.getClient().getChannelServer().getPlayerStorage()
					.getCharacterById(chr.getParty().getLeader().getId());
			if (pchr == null) {
				chr.dropMessage(5, "파티의 상태가 변경되어 골럭스 원정대가 해체됩니다.");
		...
		int[] linkMobs = { 9010152, 9010153, 9010154, 9010155, 9010156, 9010157, 9010158, 9010159, 9010160, 9010161,
				9010162, 9010163, 9010164, 9010165, 9010166, 9010167, 9010168, 9010169, 9010170, 9010171, 9010172,
				9010173, 9010174, 9010175, 9010176, 9010177, 9010178, 9010179, 9010180, 9010181 };
		...
		if (chr.getMap().getId() / 10000 == 92507 && getAllMonster().size() <= 0) {
			...
			if (floor >= 31 && floor <= 39) {
				...
				if (chr.getSkillCustomValue0(92507000) >= ((floor == 37 || floor == 38 || floor == 39) ? 3L : 2L)) {
					...
					chr.getClient().getSession()
							.writeAndFlush(CWvsContext.getTopMsg("포탈을 통해 다음 스테이지로 이동해야 클리어 기록이 랭킹에 등록 됩니다."));
					chr.getClient().getSession()
							.writeAndFlush(CField.getDojoClockStop(true, 900 - chr.getDojoStartTime()));
					...
					chr.MulungTimerTask = new TimerTask() {
						public void run() {
							...
							if (chr.getMap().getId() / 10000 == 92507) {
								chr.getClient().getSession()
										.writeAndFlush(CField.getDojoClockStop(false, 900 - chr.getDojoStartTime()));
							}
				...
				if (floor == 1 || floor == 13 || floor == 34 || floor == 35 || floor == 38 || floor == 39 || floor == 43
						|| floor == 52 || floor == 54) {
					int mapid = chr.getMapId();
					chr.getClient().send(SLFCGPacket.OnBomb(176, 39, monster.getPosition()));
					Timer.EtcTimer.getInstance().schedule(() -> {
						if ((monster.getPosition()).x + 150 > (chr.getPosition()).x
								&& (monster.getPosition()).x - 150 < (chr.getPosition()).x && mapid == chr.getMapId()) {
							chr.addHP(-(chr.getStat().getCurrentMaxHp() / 100L) * 10L);
							chr.getClient()
									.send(CField.DamagePlayer2((int) (chr.getStat().getCurrentMaxHp() / 100L) * 10));
							chr.getClient().send(CField.EffectPacket.showEffect(chr, 0, 176, 45, 39, 0,
		...
		if (chr.getParty() != null && GameConstants.price.containsKey(Integer.valueOf(monster.getId()))) {
			for (MaplePartyCharacter pc : chr.getParty().getMembers()) {
				MapleCharacter pz = chr.getClient().getChannelServer().getPlayerStorage()
		...
		if (!monster.getStats().isBoss() && chr.getBuffedValue(SecondaryStat.Reincarnation) != null
			...
			if (chr.getReinCarnation() == 0) {
				chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, 0, 1320019, 1, 0, 0,
						(byte) (chr.isFacingLeft() ? 1 : 0), true, chr.getPosition(), null, null));
				chr.getMap().broadcastMessage(chr, CField.EffectPacket.showEffect(chr, 0, 1320019, 1, 0, 0,
						(byte) (chr.isFacingLeft() ? 1 : 0), false, chr.getPosition(), null, null), false);
			}
			Map<SecondaryStat, Pair<Integer, Integer>> statups = new HashMap<>();
			statups.put(SecondaryStat.Reincarnation, new Pair<>(Integer.valueOf(1),
					Integer.valueOf((int) chr.getBuffLimit(chr.getBuffSource(SecondaryStat.Reincarnation)))));
			chr.getClient().getSession().writeAndFlush(
					CWvsContext.BuffPacket.giveBuff(statups, chr.getBuffedEffect(SecondaryStat.Reincarnation), chr));
		}
		...
		if (this.rune == null && !chr.getBuffedValue(80002282) && !isTown() && getAllMonster().size() > 0
				&& isSpawnPoint() && !GameConstants.isContentsMap(getId()) && !GameConstants.보스맵(getId())
				&& !GameConstants.사냥컨텐츠맵(getId()) && !GameConstants.튜토리얼(getId()) && !GameConstants.로미오줄리엣(getId())
				&& !GameConstants.피라미드(getId())) {
	...
	public final boolean containsNPC(int npcid) {
		Iterator<MapleMapObject> itr = ((ConcurrentHashMap) this.mapobjects.get(MapleMapObjectType.NPC)).values()
				.iterator();
		while (itr.hasNext()) {
	...
	public MapleMonster getMonsterById(int id) {
		...
		Iterator<MapleMapObject> itr = ((ConcurrentHashMap) this.mapobjects.get(MapleMapObjectType.MONSTER)).values()
				.iterator();
		while (itr.hasNext()) {
	...
	public MapleReactor getReactorById(int id) {
		...
		Iterator<MapleMapObject> itr = ((ConcurrentHashMap) this.mapobjects.get(MapleMapObjectType.REACTOR)).values()
				.iterator();
		while (itr.hasNext()) {
	...
	public final void spawnMonster_sSack(MapleMonster mob, Point pos, int spawnType) {
		mob.setPosition((calcPointBelow(new Point(pos.x, pos.y - 1)) == null) ? new Point(pos.x, pos.y)
				: calcPointBelow(new Point(pos.x, pos.y - 1)));
	...
	public final void spawnZakum(MapleCharacter chr, int x, int y) {
		...
		if (eim != null) {
			...
			broadcastMessage(MobPacket.getSmartNotice(mainb.getId(), 0, 5, 1, "紮庫姆擡起手臂準備往下打"));
	...
	public final void spawnRevives(final MapleMonster monster, final int oid) {
		...
		spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
			public final void sendPackets(MapleClient c) {
				if (monster.getOwner() == -1) {
					c.getSession().writeAndFlush(MobPacket.spawnMonster(monster,
							(monster.getStats().getSummonType() <= 1) ? -3 : monster.getStats().getSummonType(), oid));
		...
		if (monster.getId() == 8500001 || monster.getId() == 8500011 || monster.getId() == 8500021) {
			...
			monster.getMap()
					.broadcastMessage(MobPacket.BossPapuLatus.PapulLatusTime(monster.getPatten() * 1000, false, false));
			...
			monster.getMap()
					.broadcastMessage(MobPacket.BossPapuLatus.PapulLatusTime(monster.getPatten() * 1000, false, false));
		} else if (monster.getId() == 8810001 || monster.getId() == 8810000 || monster.getId() == 8810100
				|| monster.getId() == 8810101) {
			broadcastMessage(CWvsContext.serverNotice(6, "", "一個巨大的生物體正在從深洞穴中靠近."));
		}
	}
	...
	public void spawnMonster(final MapleMonster monster, final int spawnType, final boolean overwrite) {
		...
		if (monster.getSeperateSoul() == 0) {
			...
			if (monster.getId() == 8880360 || monster.getId() == 8880340 || monster.getId() == 8880300) {
				...
				this.broadcastMessage(MobPacket.BossWill.setWillHp(monster.getWillHplist(), this, monster.getId(),
						monster.getId() + 3, monster.getId() + 4));
				 ...
				 * mob0.setHp(90000000); spawnMonsterOnGroundBelow(mob0, new Point(352, 0));
				 * 
				 * MapleMonster mob1 = MapleLifeFactory.getMonster(8880322);
				 * mob1.setHp(90000000); spawnMonsterOnGroundBelow(mob1, new Point(352, -2020));
				 * 
				 * MapleMonster mob2 = MapleLifeFactory.getMonster(8880325);
				 * mob2.setHp(90000000); spawnMonsterOnGroundBelow(mob2, new Point(352, 0));
				 * 
				 * MapleMonster mob3 = MapleLifeFactory.getMonster(8880326);
				 * mob3.setHp(90000000); spawnMonsterOnGroundBelow(mob3, new Point(352, -2020));
				 */
			}
			if (monster.getId() == 8880301 || monster.getId() == 8880341) {
			...
			if (monster.getId() == 8880000 || monster.getId() == 8880002 || monster.getId() == 8880010) {
			...
			} else if (monster.getId() / 1000 == 8900 && monster.getId() % 10 < 3 && this.getNumMonsters() <= 2
					&& monster.getSeperateSoul() <= 0) {
				MapleBossManager.pierreHandler(monster);
			} else if (monster.getId() == 8950000 || monster.getId() == 8950001 || monster.getId() == 8950002
					|| monster.getId() == 8950100 || monster.getId() == 8950101 || monster.getId() == 8950102) {
				...
				for (MobSkill sk : monster.getSkills()) {
					...
					Timer.MobTimer.getInstance().schedule(() -> {
						...
						stats.add(new Pair<MonsterStatus, MonsterStatusEffect>(MonsterStatus.MS_Laser,
								new MonsterStatusEffect(223, (int) sk.getDuration())));
						monster.applyMonsterBuff(this, stats, sk);
					}, 3000L);
					break;
				}
				monster.setSchedule(Timer.MapTimer.getInstance().register(() -> {
					if (!(monster.getEventInstance() != null && monster.isAlive()
							|| this.getAllChracater().size() > 0)) {
						monster.getSchedule().cancel(true);
						monster.setSchedule(null);
			...
			} else if (monster.getId() == 8880100 || monster.getId() == 8880110 || monster.getId() == 8880101
				...
				if (monster.getId() == 8880101 || monster.getId() == 8880111) {
					...
					this.spawnMonsterWithEffect(mapleMonster, MobSkillFactory.getMobSkill(201, 182).getSpawnEffect(),
							monster.getPosition());
					time = 18000;
				} else {
				...
				this.broadcastMessage(CField.enforceMSG("無法預知戴米安會將烙印施加給誰.", 216, 30000000));
			...
			} else if (monster.getId() == 8880140 || monster.getId() == 8880141 || monster.getId() == 8880143
					|| monster.getId() == 8880150 || monster.getId() == 8880151 || monster.getId() == 8880153
					|| monster.getId() == 8880155 || monster.getId() == 8880166 || monster.getId() == 8880158) {
				// 루시드 1페 수정
			...
			if (monster.getId() >= 9833935 && monster.getId() <= 9833946) {
				monster.setCustomInfo(monster.getId(), monster.getId() == 9833946 ? 250
						: (monster.getId() == 9833944 || monster.getId() == 9833945 ? 100 : 50), 0);
			}
			...
			if (monster.getId() >= 9833070 && monster.getId() <= 9833074) {
				Timer.MapTimer.getInstance().schedule(() -> {
					...
					if (monster != null && monster.isAlive() && !this.getAllCharactersThreadsafe().isEmpty()
							&& (player = this.getAllCharactersThreadsafe().get(0)) != null) {
						Point pos = monster.getTruePosition();
	...
	public final int spawnMonsterWithEffect(final MapleMonster monster, final int effect, Point pos) {
		try {
			...
			if ((monster.getId() >= 8900000 && monster.getId() <= 8900002)
					|| (monster.getId() >= 8900100 && monster.getId() <= 8900103)) {
				MapleBossManager.pierreHandler(monster);
				if (monster.getId() == 8900002 || monster.getId() == 8900102) {
					...
					if (iterator.hasNext()) {
						...
						chrs.getMap()
								.broadcastMessage(CWvsContext.getTopMsg(name + "가 [" + chrs.getName() + "]를 추격합니다."));
				...
				monster.setSchedule(Timer.MapTimer.getInstance().register(() -> {
					if (monster.getHPPercent() <= 10) {
						...
						for (int i = 0; i < Randomizer.rand(4, 7); i++) {
							...
							Obstacle ob = new Obstacle(key, new Point(x, -210), new Point(x, 820), 25,
									(key == 22 || key == 25) ? 100 : ((key == 23 || key == 26) ? 50 : 33),
									Randomizer.rand(1100, 1500), Randomizer.rand(0, 128), 3, 653);
							obs.add(ob);
						}
		...
		spawnAndAddRangedMapObject(mda, new DelayedPacketCreation() {
			public void sendPackets(MapleClient c) {
				c.getSession().writeAndFlush(
						MobPacket.onDemianDelayedAttackCreate(skill.getSkillId(), skill.getSkillLevel(), mda));
			}
		});
	}
	...
	public final void spawnSummon(MapleSummon summon) {
		...
		spawnAndAddRangedMapObject(summon, c -> {
			if (summon != null && c.getPlayer() != null
					&& (!summon.isChangedMap() || summon.getOwner().getId() == c.getPlayer().getId())) {
		...
		if (Aobject != null) {
			...
			if (object != null) {
				...
				if (remover != null) {
					if (GameConstants.isDarkKnight(chr.getJob())) {
						if (chr.getBuffedValue(400011047)) {
							int healhp = (int) (chr.getStat().getCurrentMaxHp() / 100L
									* chr.getBuffedEffect(400011047).getX());
							int makeshiled = healhp / 100 * chr.getBuffedEffect(400011047).getV();
							int maxshiled = (int) (chr.getStat().getCurrentMaxHp() / 100L
									* chr.getBuffedEffect(400011047).getY());
				...
				if (chr.getBuffedValue(400051068)
					...
					chr.setSkillCustomInfo(400051068,
							chr.getSkillCustomValue0(400051068) + chr.getBuffedEffect(400051068).getY(), 0L);
	...
	public final void spawnRandomPortal(MapleRandomPortal portal) {
		 ...
		 * c.getSession().writeAndFlush(CField.specialMapSound(
		 * "Field.img/StarPlanet/cashTry")); if (portal.getPortalType() == 2) {
	...
	public final void spawnSummon(final MapleSummon summon, final int duration) {
		...
		if ((duration > 0 && duration < Integer.MAX_VALUE && summon.getSkill() != 400051046
				&& summon.getSummonType() != 7)
				|| (summon.getSkill() == 400021068 && summon.getMovementType() != SummonMovementType.SUMMON_JAGUAR)) {
			Timer.MapTimer.getInstance().schedule(new Runnable() {
				public void run() {
					if (summon.getSkill() == 500001002 || summon.getSkill() == 500001003
							|| summon.getSkill() == 500001004) { // HEXA 솔 야누스 구현
	...
	public void spawnEnergySphereListTimer(int objectId, int skillLevel, List<MapleEnergySphere> sps, int time) {
		Timer.MapTimer.getInstance()
				.schedule(() -> broadcastMessage(MobPacket.createEnergySphere(objectId, skillLevel, sps)), time);
	}
	...
	public final void disappearingItemDrop(MapleMapObject dropper, MapleCharacter owner, Item item, Point pos) {
		...
		broadcastMessage(CField.dropItemFromMapObject(this, drop, dropper.getTruePosition(), droppos, (byte) 3,
				(owner.getBuffedEffect(SecondaryStat.PickPocket) != null)), drop.getTruePosition());
	}
	...
	public final void spawnMesoDrop(int meso, Point position, final MapleMapObject dropper, final MapleCharacter owner,
			boolean playerDrop, byte droptype, final int delay) {
		...
		spawnAndAddRangedMapObject(mdrop, new DelayedPacketCreation() {
			public void sendPackets(MapleClient c) {
				c.getSession()
						.writeAndFlush(CField.dropItemFromMapObject(owner.getMap(), mdrop, dropper.getTruePosition(),
								droppos, (byte) 1, (owner.getBuffedEffect(SecondaryStat.PickPocket) != null), delay,
								(byte) (owner.getBuffedValue(4221018) ? 4 : 0)));
			}
		});
		if (!this.everlast) {
	...
	public final void spawnMobMesoDrop(int meso, final Point position, final MapleMapObject dropper,
			final MapleCharacter owner, boolean playerDrop, byte droptype) {
		final MapleMapItem mdrop = new MapleMapItem(meso, position, dropper, owner, droptype, playerDrop);
		spawnAndAddRangedMapObject(mdrop, new DelayedPacketCreation() {
			public void sendPackets(MapleClient c) {
				c.getSession()
						.writeAndFlush(CField.dropItemFromMapObject(owner.getMap(), mdrop, dropper.getTruePosition(),
								position, (byte) 1, (owner.getBuffedEffect(SecondaryStat.PickPocket) != null)));
			}
		});
		boolean magnetpet = false;
		for (int i = 0; i < (owner.getPets()).length; i++) {
			if (owner.getPets()[i] != null && (owner.getPets()[i].getPetItemId() == 5000930
					|| owner.getPets()[i].getPetItemId() == 5000931 || owner.getPets()[i].getPetItemId() == 5000932)) {
				magnetpet = true;
			}
		}
		mdrop.registerExpire(120000L);
	...
	public final void spawnFlyingDrop(final MapleCharacter chr, final Point startPos, final Point dropPos, Item idrop) {
		...
		spawnAndAddRangedMapObject(mdrop, new DelayedPacketCreation() {
			public void sendPackets(MapleClient c) {
				c.getSession().writeAndFlush(
						CField.dropItemFromMapObject(chr.getMap(), mdrop, startPos, dropPos, (byte) 1, false));
			}
		});
		mdrop.registerExpire(120000L);
	...
	public final void spawnMobFlyingDrop(final MapleCharacter chr, final MapleMonster mob, final Point dropPos,
			Item idrop) {
		...
		spawnAndAddRangedMapObject(mdrop, new DelayedPacketCreation() {
			public void sendPackets(MapleClient c) {
				c.getSession().writeAndFlush(CField.dropItemFromMapObject(chr.getMap(), mdrop, mob.getTruePosition(),
						dropPos, (byte) 1, false));
			}
		});
		mdrop.registerExpire(120000L);
		activateItemReactors(mdrop, chr.getClient());
	...
	public final void spawnMobDrop(Item idrop, final Point dropPos, final MapleMonster mob, final MapleCharacter chr,
			byte droptype, final int questid) {
		final MapleMapItem mdrop = new MapleMapItem(idrop, dropPos, mob, chr, droptype, false, questid);
		if (idrop.getItemId() / 1000000 == 1 && !GameConstants.isArcaneSymbol(idrop.getItemId())
				&& !GameConstants.isAuthenticSymbol(idrop.getItemId()) && idrop.getItemId() / 1000 != 1162
				&& idrop.getItemId() / 1000 != 1182) {
		...
		if (mdrop.getItemId() != 4001536) {
			spawnAndAddRangedMapObject(mdrop, new DelayedPacketCreation() {
				public void sendPackets(MapleClient c) {
					if (c != null && (questid <= 0 || c.getPlayer().getQuestStatus(questid) == 1) && mob != null
							&& dropPos != null) {
						c.getSession()
								.writeAndFlush(CField.dropItemFromMapObject(chr.getMap(), mdrop, mob.getPosition(),
										dropPos, (byte) 1, (chr.getBuffedEffect(SecondaryStat.PickPocket) != null)));
					}
				}
			});
		}
	...
	public final void spawnMobPublicDrop(Item idrop, final Point dropPos, final MapleMonster mob,
			final MapleCharacter chr, byte droptype, final int questid) {
		...
		if (mdrop.getItemId() != 4001536) {
			spawnAndAddRangedMapObject(mdrop, new DelayedPacketCreation() {
				public void sendPackets(MapleClient c) {
					if (c != null && (questid <= 0 || c.getPlayer().getQuestStatus(questid) == 1) && mob != null
							&& dropPos != null && c.getAccID() == chr.getClient().getAccID()) {
						c.getSession()
								.writeAndFlush(CField.dropItemFromMapObject(chr.getMap(), mdrop, mob.getPosition(),
										dropPos, (byte) 1, (chr.getBuffedEffect(SecondaryStat.PickPocket) != null)));
					}
	...
	public final void spawnItemDrop(final MapleMapObject dropper, final MapleCharacter owner, Item item, Point pos,
		...
		if (item.getItemId() == 2434851 || item.getItemId() == 4001849 || item.getItemId() == 4001847
				|| item.getItemId() == 2023927 || item.getItemId() == 2022570) {
			drop.setTouchDrop(true);
		}
		try {
			spawnAndAddRangedMapObject(drop, new DelayedPacketCreation() {
				public void sendPackets(MapleClient c) {
					c.getSession().writeAndFlush(CField.dropItemFromMapObject(owner.getMap(), drop,
							dropper.getTruePosition(), droppos, (byte) 1, false));
				}
			});
			broadcastMessage(CField.dropItemFromMapObject(owner.getMap(), drop, dropper.getTruePosition(), droppos,
					(byte) 0, false));
			if (!this.everlast) {
	...
	private void activateItemReactors(final MapleMapItem drop, final MapleClient c) {
		...
		for (final MapleMapObject o : this.mapobjects.get(MapleMapObjectType.REACTOR).values()) {
			...
			if (react.getReactorType() == 100
					...
					&& react.getArea().contains(drop.getTruePosition()) && !react.isTimerActive()) {
	...
	public final void addPlayer(MapleCharacter chr) {
		...
		if (GameConstants.isZero(chr.getJob())) {
			...
			if (weapon != null && subWeapon != null && weapon.getItemId() / 1000 == 1562
					&& subWeapon.getItemId() / 1000 == 1572) {
				chr.getInventory(MapleInventoryType.EQUIPPED).move((short) -10, (short) -11, (short) 1,
						chr.getClient());// 修改記錄？？增加了 chr.getClient() 参数
				chr.getClient().getSession().writeAndFlush(
						CWvsContext.InventoryPacket.updateInventoryItem(false, MapleInventoryType.EQUIP, weapon));
				chr.getClient().getSession().writeAndFlush(
						CWvsContext.InventoryPacket.updateInventoryItem(false, MapleInventoryType.EQUIP, subWeapon));
			}
		}
///*        if (isTown()) {
...
//        }*/
		 ...
		 * if (damageSkinCode != -1) {
		 * chr.getMap().broadcastMessage(CField.showForeignDamageSkin(chr,
		 * damageSkinCode)); }
		 */
			...
			if (chr.getBuffedValue(63101005)) {
				...
				for (MapleSecondAtom at : chr.getMap().getAllSecondAtoms()) {
					if (at.getSourceId() == 63101006 && at.getChr().getId() == chr.getId()) {
						int du = (int) (at.getSecondAtoms().getExpire() - System.currentTimeMillis()
								- at.getStartTime());
		...
		if (chr.getDeathCount() > 0 && chr.getEventInstance() != null) {
			...
			} else if (chr.getMapId() != 262031310 && chr.getMapId() != 262030310 && chr.getMapId() != 262031300
					&& chr.getMapId() != 262030100 && chr.getMapId() != 262031100 && chr.getMapId() != 262030200
					&& chr.getMapId() != 262031200) {
		...
		int eventskillid = (chr.getSkillLevel(80003064) > 0) ? 80003064
				: ((chr.getSkillLevel(80003046) > 0) ? 80003046
						: ((chr.getSkillLevel(80003025) > 0) ? 80003025
								: ((chr.getSkillLevel(80003016) > 0) ? 80003016 : 0)));
		if (eventskillid > 0) {
			String skillname = SkillFactory.getSkillName(eventskillid);
			int ui = (eventskillid == 80003046) ? 1297 : ((eventskillid == 80003016) ? 1291 : 0);
			boolean cast = true;
			if (eventskillid == 80003046 && chr.getKeyValue(100794,
		...
		case 450004550:
			chr.getClient().getSession()
					.writeAndFlush(MobPacket.BossLucid.changeStatueState(false, getLucidCount(), false));
			break;
		case 450008150:
		case 450008750:
		...
		case 993192000:
			...
			if (getCustomTime(993192000) != null) {
				...
				chr.getClient().send(CField.startMapEffect(bool1 ? "따사로운 봄 햇살이 쏟아져 내립니다." : "기분 좋게 시원한 봄바람이 살랑입니다.",
						bool1 ? 5121112 : 5121113, true));
			}
			break;
		case 993026900:
			...
			if (chr.getMap().getCustomTime(chr.getMap().getId()) != null) {
				chr.getClient()
						.send(CField.getClock(chr.getMap().getCustomTime(chr.getMap().getId()).intValue() / 1000));
			}
			break;
		case 310070140:
		case 310070220:
		case 310070450:
			...
			pos = (getId() == 310070140) ? new Point(1493, -62)
					: ((getId() == 310070220) ? new Point(1352, -459) : new Point(483, -573));
			already = false;
		...
		case 993194000:
			...
			if (getCustomTime(993194000) != null) {
				...
				chr.getClient().send(CField.ChangeSpecialMapEffect(eff));
				chr.getClient().send(CField.startMapEffect(sun ? "시청자로부터 블루 하트 선물이 쏟아집니다!" : "시청자로부터 핑크 하트 선물이 쏟아집니다!",
						sun ? 5121114 : 5121115, true));
			}
			break;
		// 거탐
		case 993073000:
			if (chr.getClient().getKeyValue("LIE_DETECTOR_WARP") != null) {
				...
				chr.getClient().send(CField
						.getTrueRoomClock(Integer.parseInt(chr.getClient().getKeyValue("LIE_DETECTOR_WARP")) * 1000));
			}
		...
		default:
			final int[] array;
			skilllist = array = new int[] { 80001427, 80001428, 80001432, 80001752, 80001753, 80001755, 80001878,
					80002888, 80002889, 80002890 };
		...
		if (chr.getSkillCustomValue(9110) != null) {
			if (chr.getSkillCustomTime(9110).intValue() > 0
					&& (chr.getMapId() / 100000 == 9530 || chr.getMapId() / 100000 == 9540)) {
		...
		for (int i = 0; i < 3; i++) {
			if (chr.getPet(i) != null) {
				chr.getClient().getSession()
						.writeAndFlush(PetPacket.updatePet(chr, chr.getPet(i),
								chr.getInventory(MapleInventoryType.CASH).getItem(chr.getPet(i).getInventoryPosition()),
		...
		if (chr.getParty() != null) {
			...
			chr.getClient().getSession().writeAndFlush(CWvsContext.PartyPacket.updateParty(chr.getClient().getChannel(),
					chr.getParty(), PartyOperation.SILENT_UPDATE, null, chr.getPlayer()));
		...
		if (chr.getEventInstance() != null && chr.getEventInstance().isTimerStarted()) {
			if (chr.inPVP()) {
				chr.getClient().getSession()
						.writeAndFlush(CField.getPVPClock(Integer.parseInt(chr.getEventInstance().getProperty("type")),
								(int) (chr.getEventInstance().getTimeLeft() / 1000L)));
			} else {
				chr.getClient().getSession()
						.writeAndFlush(CField.getClock((int) (chr.getEventInstance().getTimeLeft() / 1000L)));
			}
		}
		if (hasClock()) {
		...
		if (getMapTimer() > 0L) {
			chr.getClient().getSession()
					.writeAndFlush(CField.getClock((int) ((getMapTimer() - System.currentTimeMillis()) / 1000L)));
		}
		// 팅 의심
		// chr.getClient().getSession().writeAndFlush(CField.specialChair(chr, true,
		// true, true, null));
		if (isElitebossmap()) {
			chr.getClient().send(CField.getClock(this.elitetime));
			broadcastMessage(CField.specialMapEffect(2, true, "Bgm36.img/RoyalGuard",
					"Effect/EliteMobEff.img/eliteMonsterFrame", "Effect/EliteMobEff.img/eliteMonsterEffect", "", ""));
			...
			broadcastMessage(CField.specialMapEffect(3, true, "Bgm36.img/HappyTimeShort",
					"Map/Map/Map9/924050000.img/back", "Effect/EliteMobEff.img/eliteBonusStage", "", ""));
		}
		if (this.burning > 0 && getAllNormalMonstersThreadsafe().size() > 0 && !isTown()
				&& !GameConstants.로미오줄리엣(getId()) && !GameConstants.사냥컨텐츠맵(getId()) && isSpawnPoint()
				&& !GameConstants.isContentsMap(getId())) {
			chr.getClient().getSession().writeAndFlush(CField.playSound("Sound/FarmSE.img/boxResult"));
			chr.getClient().getSession()
					.writeAndFlush(CField.EffectPacket.showBurningFieldEffect("#fn나눔고딕 ExtraBold##fs26#          버닝 "
							+ this.burning + "단계 : 경험치 " + (this.burning * 10) + "% 추가지급!!          "));
			if (chr.getKeyValue(51351, "startquestid") == 49011L) {
				...
				chr.getClient()
						.send(CWvsContext.updateSuddenQuest((int) chr.getKeyValue(51351, "midquestid"), false,
								PacketHelper.getKoreanTimestamp(System.currentTimeMillis()) + 600000000L,
								"count=1;Quest=" + chr.getKeyValue(51351, "startquestid") + ";state=3;"));
				chr.getClient().send(CWvsContext.updateSuddenQuest((int) chr.getKeyValue(51351, "startquestid"), false,
						chr.getKeyValue(51351, "endtime"), "BTField=1;"));
			}
		}
		if (this.rune != null && !chr.getBuffedValue(80002282) && this.runeCurse > 0 && isSpawnPoint()
				&& !GameConstants.보스맵(this.mapid) && !GameConstants.isContentsMap(this.mapid)) {
			chr.getClient().getSession().writeAndFlush(CField.runeCurse("必須解放符文以解除精英BOSS的詛呪!!\\n詛呪階段 " + this.runeCurse
					+ " :  經驗埴獲取率、掉落率 " + getRuneCurseDecrease() + "% 降低效果生效中", false));
		}
		if (chr.getKeyValue(210416, "TotalDeadTime") > 0L && isSpawnPoint()) {
			chr.getClient().getSession()
					.writeAndFlush(CField.PenaltyMsg("經驗埴獲得，掉落率80%降低效果中!\r\n使用護符道具可立刻解除.", 338, 10000, 180));
		}
		...
		if (getNumMonsters() > 0 && (this.mapid == 280030001 || this.mapid == 240060201 || this.mapid == 280030000
				|| this.mapid == 240060200 || this.mapid == 220080001 || this.mapid == 541020800
		...
		if (getNodez().getEnvironments().size() > 0 && this.mapid != 450004250 && this.mapid != 450004550
				&& this.mapid != 450003920) {
		...
		if (chr.getBuffedValue(SecondaryStat.RepeatEffect) != null) {
			...
			if (GameConstants.isAngelicBlessBuffEffectItem(skillid)) {
				...
				broadcastMessage(CWvsContext.BuffPacket.giveForeignBuff(chr, statups,
						chr.getBuffedEffect(SecondaryStat.RepeatEffect)));
			}
		}
	...
	private void sendObjectPlacement(MapleCharacter c) {
		...
		for (MapleMapObject o : getMapObjectsInRange(c.getTruePosition(), c.getRange(),
	...
	public final void loadMonsterRate(boolean first) { // 몹 젠
		...
		if (first && spawnSize > 0) {
			...
			if (this.barrierArc == 0 && this.barrierAut == 0 && getId() != 310070200 && getId() != 310070210
					&& getId() != 310070220) {
				this.createMobInterval = (short) (this.createMobInterval / 3);
			} else if (getId() != 450006000 && getId() != 450006010 && getId() != 450006020 && getId() != 450006030
					&& getId() != 450006040) {
	...
	public final void updateMapObjectVisibility(MapleCharacter chr, MapleMapObject mo) {
		...
		if (!chr.isMapObjectVisible(mo)) {
			if (mo.getType() == MapleMapObjectType.MIST || mo.getType() == MapleMapObjectType.EXTRACTOR
					|| mo.getType() == MapleMapObjectType.SUMMON || mo.getType() == MapleMapObjectType.RUNE
					|| mo.getType() == MapleMapObjectType.MagicSword || mo instanceof MechDoor
					|| mo.getTruePosition().distanceSq(chr.getTruePosition()) <= mo.getRange()) {
				chr.addVisibleMapObject(mo);
				mo.sendSpawnData(chr.getClient());
			}
		} else if (!(mo instanceof MechDoor) && mo.getType() != MapleMapObjectType.MIST
				&& mo.getType() != MapleMapObjectType.EXTRACTOR && mo.getType() != MapleMapObjectType.SUMMON
				&& mo.getType() != MapleMapObjectType.RUNE && mo.getType() != MapleMapObjectType.MagicSword
				&& mo.getTruePosition().distanceSq(chr.getTruePosition()) > mo.getRange()) {
	...
	public void movePlayer(MapleCharacter player, Point newPosition) {// mush
		...
		for (MapleMapObject mo : this.getMapObjectsInRange(player.getTruePosition(), player.getRange())) {
			if (mo == null || player.getVisibleMapObjects().contains(mo)
					|| mo.getType() == MapleMapObjectType.MagicSword) {
				continue;
			}
					...
					Timer.MapTimer.getInstance().schedule(new Runnable() {
						public void run() {
							ActivateItemReactor.this.reactor.forceHitReactor((byte) 0,
									ActivateItemReactor.this.c.getPlayer().getId());
						}
	...
	public void respawn(boolean force, long now) {
		...
		if (force) {
			...
			int numShouldSpawn = (GameConstants.isForceRespawn(this.mapid) ? this.monsterSpawn.size()
					: this.maxRegularSpawn) - this.spawnedMonstersOnMap.get();
			if (numShouldSpawn > 0) {
				int spawned = 0;
		...
		for (Pair<Point, Integer> guard : getNodez().getGuardians()) {
			if (((Integer) guard.right).intValue() == team || ((Integer) guard.right).intValue() == -1) {
				...
				for (MapleReactor r : react) {
					if ((r.getTruePosition()).x == ((Point) guard.left).x
							&& (r.getTruePosition()).y == ((Point) guard.left).y && r.getState() < 5) {
						found = true;
						break;
					}
				}
	...
	public Collection<MapleCharacter> getNearestPvpChar(Point attacker, double maxRange, double maxHeight,
		...
		for (MapleCharacter a : this.characters) {
			if (chr.contains(a.getClient().getPlayer())) {
				...
				if (isLeft) {
					if (attacker.x < attackedPlayer.x && distanceX < maxRange && distanceX > 1.0D
	...
	public void startCatch() {
		if (this.catchstart == null) {
			...
			this.catchstart = Timer.MapTimer.getInstance().schedule(new Runnable() {
				public void run() {
					MapleMap.this.broadcastMessage(CWvsContext.serverNotice(1, "",
					...
					for (MapleCharacter chr : MapleMap.this.getCharacters()) {
						...
						if (chr.isCatching) {
							chr.changeMap(chr.getClient().getChannelServer().getMapFactory().getMap(910040005),
									chr.getClient().getChannelServer().getMapFactory().getMap(910040005).getPortalSP()
											.get(0));
							chr.isWolfShipWin = false;
							continue;
						}
						chr.changeMap(chr.getClient().getChannelServer().getMapFactory().getMap(910040004), chr
								.getClient().getChannelServer().getMapFactory().getMap(910040004).getPortalSP().get(0));
	...
	public List<MapleMonster> getAllButterFly() {
		...
		for (final MapleMapObject mmo : this.mapobjects.get(MapleMapObjectType.MONSTER).values()) {
			...
			if ((monster.getId() == 8880175 || monster.getId() == 8880178 || monster.getId() == 8880179)
					&& (this.mapid == 450004250 || this.mapid == 450004550)) {
				ret.add(monster);
			} else {
				if ((monster.getId() != 8880165 && monster.getId() != 8880168 && monster.getId() != 8880169)
						|| (this.mapid != 450004150 && this.mapid != 450004450)) {
					continue;
				}
				ret.add(monster);
			}
	...
	public final void spawnSoul(MapleMapObject dropper, MapleCharacter chr, Item item, Point pos, Item weapon) {
		...
		broadcastMessage(
				CField.dropItemFromMapObject(chr.getMap(), drop, dropper.getPosition(), droppos, (byte) 0, false));
	...
	public Integer getCustomTime(int skillid) {
		if (this.customInfo.containsKey(Integer.valueOf(skillid))) {
			return Integer.valueOf((int) (((SkillCustomInfo) this.customInfo.get(Integer.valueOf(skillid))).getEndTime()
	...
	public void makeEliteMonster(int mobid, int grade, Point pos, boolean rune, boolean eliteboss) {
		...
		eliteMonster.getEliteGradeInfo()
				.add(new Pair<>(Integer.valueOf(eg.getSkillid()), Integer.valueOf(eg.getSkilllv())));
	...
	public void startEliteBossReward() {
		this.elitetime = 0;
		broadcastMessage(CField.specialMapEffect(3, false, "Bgm36.img/HappyTimeShort",
				"Map/Map/Map9/924050000.img/back", "Effect/EliteMobEff.img/eliteBonusStage", "", ""));
	...
	public final List<SpiderWeb> getAllSpiderWeb() {
		return getWebInRange(new Point(0, 0), Double.POSITIVE_INFINITY,
				Arrays.asList(new MapleMapObjectType[] { MapleMapObjectType.WEB }));
	}
	...
	public void getObtacles(int rand) {
		...
		if (getId() == 450013300) {
			if (rand == 0) {
				Obstacle ob = new Obstacle(77, new Point(-987, -600), new Point(-983, -239), 95, 50, 0, 487, 1000, 123,
						7, 360);
				obs.clear();
				obs.add(ob);
				...
				Obstacle ob = new Obstacle(77, new Point(323, -600), new Point(-365, 88), 95, 50, 0, 415, 1000, 85, 5,
						973);
				obs.clear();
				obs.add(ob);
				broadcastMessage(MobPacket.CreateObstacle2(obs));
			...
			Obstacle ob = new Obstacle(77, new Point(-853, -600), new Point(-976, 85), 95, 50, 0, 498, 1000, 129, 6,
					695);
			obs.clear();
			obs.add(ob);
			broadcastMessage(MobPacket.CreateObstacle2(obs));
			...
			Obstacle ob = new Obstacle(77, new Point(-938, -600), new Point(-979, 85), 95, 50, 0, 425, 1000, 90, 6,
					685);
			obs.clear();
			obs.add(ob);
			broadcastMessage(MobPacket.CreateObstacle2(obs));
	...
	public void BloomingMoment(boolean active, boolean justshow) {
		...
		if ((((new Date()).getMinutes() >= 30 && (new Date()).getMinutes() < 40) || justshow)
				&& (getCustomValue(993192000) == null || justshow)) {
			...
			if (getCharactersSize() > 0) {
				...
				broadcastMessage(CField.ChangeSpecialMapEffect(eff));
				if (active) {
					broadcastMessage(CField.startMapEffect(sun ? "따사로운 봄 햇살이 쏟아져 내립니다." : "기분 좋게 시원한 봄바람이 살랑입니다.",
							sun ? 5121112 : 5121113, true));
				}
			}
		}
	}
