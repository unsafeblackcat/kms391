...
public class PlayersHandler {
	...
	public static void UseRandomDoor(LittleEndianAccessor slea, MapleCharacter chr) {
		MapleRandomPortal portal = (MapleRandomPortal) chr.getMap().getMapObject(slea.readInt(),
				MapleMapObjectType.RANDOM_PORTAL);
		if (portal != null && portal.getCharId() == chr.getId() && chr.getMapId() == portal.getMapId()) {
			if (portal.getPortalType() == 2) {
				NPCScriptManager.getInstance().start(chr.getClient(), portal.ispolo() ? 9001059 : 9001060,
						portal.ispolo() ? "poloEnter" : "FrittoEnter");
	...
	public static void FollowReply(LittleEndianAccessor slea, MapleClient c) {
		if (c.getPlayer().getFollowId() > 0 && c.getPlayer().getFollowId() == slea.readInt()) {
			...
			if (tt != null && tt.getTruePosition().distanceSq(c.getPlayer().getTruePosition()) < 10000.0D
					&& tt.getFollowId() == 0 && tt.getId() != c.getPlayer().getId()) {
	...
	public static void TouchReactor(LittleEndianAccessor slea, MapleClient c) {
		...
		if ((reactor.getMap().getId() >= 105200310 && reactor.getMap().getId() <= 105200317)
				|| (reactor.getMap().getId() >= 105200710 && reactor.getMap().getId() <= 105200719)) {
			String text = "";
			boolean send = false;
		...
		if (reactor.getTouch() == 2) {
			...
			if (reactor.getReactorType() == 100) {
				int itemid = GameConstants.getCustomReactItem(reactor.getReactorId(),
						((Integer) reactor.getReactItem().getLeft()).intValue());
				if (c.getPlayer().haveItem(itemid, ((Integer) reactor.getReactItem().getRight()).intValue())) {
					if (reactor.getArea().contains(c.getPlayer().getTruePosition())) {
						MapleInventoryManipulator.removeById(c, GameConstants.getInventoryType(itemid), itemid,
								((Integer) reactor.getReactItem().getRight()).intValue(), true, false);
	...
	public static void DoRing(MapleClient c, String name, int itemid) {
		int newItemId = (itemid == 2240000) ? 1112803
		...
		chr.getClient().getSession()
				.writeAndFlush(CWvsContext.sendEngagementRequest(c.getPlayer().getName(), c.getPlayer().getId()));
	}
		...
		if (mode == 0) {
			...
			if (accepted) {
				...
				try {
					...
					chr.getClient().getSession()
							.writeAndFlush(CWvsContext.sendEngagement((byte) 13, newItemId, chr, c.getPlayer()));
				...
				if (invatation != null && invatation.getItemId() == data.getTicketType().getInvitationItemId()) {
					...
					if (chr != null) {
						...
						if (MapleInventoryManipulator.checkSpace(chr.getClient(), type.getInvitedItemId(), 1, "")) {
							MapleCharacterUtil.sendNote(receiver, c.getPlayer().getName(),
									"Congratulations! 당신은 결혼식에 초대되었습니다! 기타창을 확인해주세요.", 0, 6, 0);
							...
							MapleInventoryManipulator.removeById(c, MapleInventoryType.ETC, invatation.getItemId(), 1,
									true, false);
						} else {
							c.getSession().writeAndFlush(CWvsContext.sendEngagement((byte) 25, 0, null, null));
							c.getSession().writeAndFlush(CWvsContext.enableActions(c.getPlayer()));
			...
			if (item != null && item.getItemId() == itemid && item.getMarriageId() > 0) {
				...
				if (data != null) {
					c.getSession().writeAndFlush(CWvsContext.showWeddingInvitation(data.getGroomName(),
							data.getBrideName(), data.getTicketType().getItemId() - 5251004));
					return;
				}
			}
			...
			if (data != null && data.getStatus() == 1 && data.getWeddingStatus() >= 1 && data.getWeddingStatus() < 8) {
				...
				if (data.getGroomId() == c.getPlayer().getId()) {
					MarriageTicketType type = data.getTicketType();
					if (MapleInventoryManipulator.checkSpace(c, type.getInvitationItemId(),
							type.getInvitationQuantity(), "")) {
				...
				if (data.getWeddingStatus() == 7) {
					...
					MapleInventoryManipulator.addById(c, type.getInvitationItemId(),
							(short) type.getInvitationQuantity(), "");
					int channel = World.Find.findChannel(data.getPartnerId(c.getPlayer().getId()));
					if (channel >= 0) {
						MapleCharacter chr = ChannelServer.getInstance(channel).getPlayerStorage()
								.getCharacterById(data.getPartnerId(c.getPlayer().getId()));
						if (chr != null) {
							MapleInventoryManipulator.addById(chr.getClient(), type.getInvitationItemId(),
									(short) type.getInvitationQuantity(), "");
							chr.getClient().getSession()
									.writeAndFlush(CWvsContext.sendEngagement((byte) 18, 0, null, null));
						}
					}
	...
	public static void Report(LittleEndianAccessor slea, MapleClient c) {
		...
		if (target != null) {
			if (System.currentTimeMillis() - (c.getPlayer()).lastReportTime >= 600000L) {
				...
				String data = c.getPlayer().getName() + " 캐릭터가 " + target.getName() + " 캐릭터를 " + target.getMapId()
						+ " 맵에서 신고 | 타입 : " + type + " / 내용 : " + desc + "\r\n\r\n";
	...
	public static final void TouchRune(LittleEndianAccessor slea, MapleCharacter chr) {
		...
		for (MapleCoolDownValueHolder m : chr.getCooldowns()) {
			if (m.skillId == 80002282 && !chr.isGM()) {
				chr.getClient().getSession().writeAndFlush(
						CField.RuneAction(2, (int) (m.length + m.startTime - System.currentTimeMillis())));
				return;
			}
		}
		if (GameConstants.isYeti(chr.getJob()) || GameConstants.isPinkBean(chr.getJob())) {
			...
			if (chr.getBuffedValue(80002282) && !chr.isGM()) {
				chr.dropMessage(1, "在符文冷卻時間內不能使用符文.");
				chr.getClient().getSession().writeAndFlush(CWvsContext.enableActions(chr));
				return;
			}
			...
			case 3: // 어둠의 룬 80001754
				...
				if (!spawn) {
					List<MapleMapObject> monstersInRange = chr.getMap().getMapObjectsInRange(chr.getPosition(), 2.0E7D,
					...
					if (!monstersInRange.isEmpty()) {
						...
						if (monster != null && !monster.getStats().isBoss()) {
							for (int i = 0; i < 3; i++) {
								chr.getMap().makeEliteMonster(monster.getId(), Randomizer.rand(0, 2),
										((Spawns) (chr.getMap()).monsterSpawn
												.get(Randomizer.rand(0, (chr.getMap()).monsterSpawn.size() - 1)))
												.getPosition(),
										true, false);
							}
						}
			...
			if (chr.getMap().getRuneCurse() > 0) {
				...
				chr.getMap().broadcastMessage(CField.runeCurse("精英老大的詛呪解除了!!", true));
			}
			...
			if (chr.getKeyValue(51351, "startquestid") == 49010L) {
				...
				chr.getClient().send(CWvsContext.updateSuddenQuest((int) chr.getKeyValue(51351, "startquestid"), false,
						chr.getKeyValue(51351, "endtime"), "RunAct=1;"));
			}
	...
	public static void WeddingPresent(LittleEndianAccessor slea, MapleClient c) {
		...
		if (mode == 7) {
			...
			if (entry != null) {
				List<Item> items = (c.getPlayer().getGender() == 0) ? entry.getGroomPresentList()
						: entry.getBridePresentList();
				if (null != items)
					try {
						Item item = items.get(slot);
						if (item != null && MapleInventoryType.getByType(invtype) == GameConstants
							...
							if (MapleInventoryManipulator.checkSpace(c, item.getItemId(), item.getQuantity(), "")) {
								...
								c.getPlayer().dropMessage(1,
										"인벤토리 공간이 부족합니 void WeddingPresent(LittleEndianAccessor slea, MapleClient c) {\n"
												+ "    byte mode = slea.readByte();\n" + "    if (mode == 7) {\n"
												+ "      byte invtype = slea.readByte();\n"
												+ "      byte slot = slea.readByte();\n"
			...
			if (agent != null) {
				...
				if (dataEntry != null && item != null && item.getItemId() == itemid && item.getQuantity() >= quantity) {
					...
					if (c.getPlayer().getWeddingGive() == 0) {
						...
						c.getSession().writeAndFlush(CWvsContext.showWeddingWishGiveToServerResult(
								dataEntry.getGroomWishList(), type, dataEntry.getGroomPresentList()));
					} else {
						dataEntry.getBridePresentList().add(item2);
						c.getSession().writeAndFlush(CWvsContext.showWeddingWishGiveToServerResult(
								dataEntry.getBrideWishList(), type, dataEntry.getBridePresentList()));
					}
				}
			}
		}
	...
	public static void ColorCardHandler(LittleEndianAccessor slea, MapleClient c) {
		...
		if (chr.getColorCardInstance() != null) {
			...
			c.send(SLFCGPacket.ColorCardPacket.ColorCardMain(chr.getColorCardInstance().getCombo(),
					chr.getColorCardInstance().getGauge(), chr.getColorCardInstance().getPoint()));
		}
	}
	...
	public static void MapleYutHandler(LittleEndianAccessor slea, MapleClient c) {
		...
		if (c.getPlayer().getMultiYutInstance() != null) {
			...
			case 1:
				...
				c.getPlayer().getMultiYutInstance().getPlayer(c.getPlayer()).MoveHorse(horseposition, movecount,
						yuttype + 1);
				break;
			}
		}
	...
	public static void MapleTyoonKitchenSuc(LittleEndianAccessor slea, MapleClient c) {
		...
		if (type == 0) {
			...
			c.getPlayer().getMap().broadcastMessage(SLFCGPacket.TyoonKitchenPacket
					.Menu(c.getPlayer().getMtk().getRecipes(), MapleTyoonKitchen.getAllMtk(c.getPlayer())));
		} else if (type == 2) {
			...
			if (recipe.getDestination() == id) {
				...
				c.getPlayer().getMap().broadcastMessage(SLFCGPacket.TyoonKitchenPacket.Effect(c.getPlayer(),
						"Effect/OnUserEff.img/aquarisTower/success", 0));
				c.getPlayer().getMap().broadcastMessage(CField.playSound("Sound/Item.img/03015651/Appear"));
				c.getPlayer().getMap()
						.broadcastMessage(CField.environmentMove("cookPlate" + (recipe.getMakenum() + 1), 0));
				...
				if (MapleTyoonKitchen.getAllMoney(c.getPlayer()) >= 500) {
					...
					c.getPlayer().getMap().broadcastMessage(
							CField.environmentChange("Effect/EventEffect.img/2021BloomingRace/success", 16));
