...
public class BattleGroundGameHandler {
		...
		if (map != null && map.getAllChracater().size() > 0) {
			for (MapleCharacter chr : map.getAllChracater()) {
				if (chr.getBattleGroundChr() == null) {
					...
					// chr.getMap().broadcastMessage(BattleGroundPacket.ChangeAvater(gchr,
					// GameConstants.BattleGroundJobType(gchr)));
					// chr.getClient().send(BattleGroundPacket.UpgradeMainSkill(gchr));
					// chr.getClient().send(BattleGroundPacket.AvaterSkill(gchr,
					// GameConstants.BattleGroundJobType(gchr)));
					// chr.getClient().send(BattleGroundPacket.SelectAvaterOther(chr, 1, 1));
					MapleBattleGroundCharacter t = gchr;
					t.setTeam(2);
					chr.getMap().broadcastMessage(chr,
							BattleGroundPacket.ChangeAvater(t, GameConstants.BattleGroundJobType(t)), false);
				}
			}
			NotDamage = true;
			spawnMob(map);
			map.broadcastMessage(CField.ImageTalkNpc(9001153, 5000,
					"전투 시작. 몬스터와 보스 몬스터를 처치해 강해질 수 있습니다. 게임시작 후 1분간은 #r무적#k 상태 입니다."));
	...
	public static void spawnMob(MapleMap map) {
		...
		for (i = 0; i < 6; i++) {
			...
			map.spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(mobid),
					new Point(Randomizer.rand(-876, 80), 825));
		}
		for (i = 0; i < 9; i++) {
			int mobid = (i < 4) ? 9303224 : ((i < 7) ? 9303225 : 9303228);
			map.spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(mobid),
					new Point(Randomizer.rand(-1117, 396), 1495));
		}
		for (i = 0; i < 6; i++) {
			int mobid = (i == 5 || i == 6) ? 9303227 : 9303222;
			map.spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(mobid),
					new Point(Randomizer.rand(-679, -83), 2004));
		}
	}
