...
public class FieldSkillFactory {
	...
	public void load() {
		...
		System.out.println("添加到wz_fieldskill數據中.....");
				...
				if (summonData != null) {
					for (MapleData sumData : summonData) {
						for (int i = 0; i > -1 && sumData.getChildByPath(String.valueOf(i)) != null; i++) {
							...
							Point sumPosition = MapleDataTool.getPoint("pt", sumData.getChildByPath(String.valueOf(i)),
									new Point(0, 0));
				...
				if (attackData != null) {
					for (MapleData atkData : attackData) {
						for (int i = 0; i > -1 && atkData.getChildByPath(String.valueOf(i)) != null; i++) {
							int duration = MapleDataTool.getInt("duration", atkData.getChildByPath(String.valueOf(i)),
									0);
							int interval = MapleDataTool.getInt("interval", atkData.getChildByPath(String.valueOf(i)),
									0);
							int angleMin = MapleDataTool.getInt("angleMin", atkData.getChildByPath(String.valueOf(i)),
									0);
							int angleMax = MapleDataTool.getInt("angleMax", atkData.getChildByPath(String.valueOf(i)),
									0);
							int attackDelay = MapleDataTool.getInt("attackDelay",
									atkData.getChildByPath(String.valueOf(i)), 0);
							...
							if (lt != null && rb != null && pos != null) {
								footHolds.add(new FieldSkill.FieldFootHold(duration, interval, angleMin, angleMax,
										attackDelay, z, (short) set, (short) 0,
										new Rectangle(lt.x, rb.y, rb.x - lt.x, rb.y - lt.y), pos, false));
							}
						}
					}
				}
