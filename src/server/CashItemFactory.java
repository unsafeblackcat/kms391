...
public class CashItemFactory {
	...
	public void initialize() {
		...
		for (MapleData field : cccc) {
			...
			CashItemInfo stats = new CashItemInfo(MapleDataTool.getIntConvert("ItemId", field, 0),
					...
					MapleDataTool.getIntConvert("LimitMax", field, 0));
			if (SN > 0) {
		...
		try {
			...
			rs = ps.executeQuery();
			if (rs.next()) {
				CashItemInfo.CashModInfo ret = new CashItemInfo.CashModInfo(rs.getInt("serial"),
						rs.getInt("discount_price"), rs.getInt("mark"), (rs.getInt("showup") > 0), rs.getInt("itemid"),
						rs.getInt("priority"), (rs.getInt("package") > 0), rs.getInt("period"), rs.getInt("gender"),
