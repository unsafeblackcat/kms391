...
public class MapleMapFactory {
	...
	private final MapleDataProvider source = MapleDataProviderFactory
			.getDataProvider(new File(System.getProperty("wz") + "/Map.wz"));

	private final MapleData nameData = MapleDataProviderFactory
			.getDataProvider(new File(System.getProperty("wz") + "/String.wz")).getData("Map.img");
			...
			try {
				...
				for (MapleData life : mapData.getChildByPath("life")) {
					...
					if ((npcs || !type.equals("n")) && !limited.equals("Stage0")) {
						AbstractLoadedMapleLife myLife = loadLife(life,
				...
				if (!map.isTown() && map.getAllMonster().size() > 0 && map.isSpawnPoint()
						&& !GameConstants.보스맵(map.getId()) && !GameConstants.사냥컨텐츠맵(map.getId())
						&& !GameConstants.튜토리얼(map.getId()) && !GameConstants.로미오줄리엣(map.getId())
						&& !GameConstants.피라미드(map.getId()) && !GameConstants.isContentsMap(map.getId())) {
				...
				map.setUserEnter((mapid == 180000002) ? "jail"
						: MapleDataTool.getString(mapData.getChildByPath("info/onUserEnter"), ""));
				if (reactors && map.monsterSpawn.size() > 0 && map.isSpawnPoint() && !GameConstants.보스맵(map.getId())
						&& !GameConstants.사냥컨텐츠맵(map.getId()) && !GameConstants.튜토리얼(map.getId())
						&& !GameConstants.로미오줄리엣(map.getId()) && !GameConstants.피라미드(map.getId())
						&& !GameConstants.isContentsMap(map.getId())) {
				...
				try {
					map.setMapName(MapleDataTool.getString("mapName",
							this.nameData.getChildByPath(getMapStringName(omapid.intValue())), ""));
				...
				if (mapData.getChildByPath("monsterDefense/wave") != null) {
					...
					for (MapleData wave : wavedata.getChildren()) {
						...
						for (MapleData mid : wave.getChildren()) {
							if ((Integer.parseInt(wave.getName()) != 20 && (i == 0 || i % 4 == 0))
									|| (Integer.parseInt(wave.getName()) == 20 && (i == 0 || i % 2 == 0))) {
								...
								((List<Integer>) monster.get(Integer.valueOf(Integer.parseInt(wave.getName()))))
										.add(Integer.valueOf(MapleDataTool.getInt(mid.getChildByPath("mobID"), 0)));
							}
							i++;
						}
					}
	...
	public MapleMap CreateInstanceMap(int mapid, boolean respawns, boolean npcs, boolean reactors, int instanceid) {
		...
		for (MapleData life : mapData.getChildByPath("life")) {
			...
			if ((npcs || !type.equals("n")) && limited.equals("")) {
				AbstractLoadedMapleLife myLife = loadLife(life, MapleDataTool.getString(life.getChildByPath("id")),
						type);
				if (myLife instanceof MapleMonster && !GameConstants.isNoSpawn(mapid)) {
					MapleMonster mob = (MapleMonster) myLife;
		...
		try {
			map.setMapName(
					MapleDataTool.getString("mapName", this.nameData.getChildByPath(getMapStringName(mapid)), ""));
			map.setStreetName(
					MapleDataTool.getString("streetName", this.nameData.getChildByPath(getMapStringName(mapid)), ""));
		} catch (Exception e) {
			map.setMapName("");
			map.setStreetName("");
		}
	...
	private MapleNodes loadNodes(int mapid, MapleData mapData) {
		...
		if (mapData.getChildByPath("nodeInfo") != null) {
			for (MapleData node : mapData.getChildByPath("nodeInfo")) {
				try {
					...
					MapleNodes.MapleNodeInfo mni = new MapleNodes.MapleNodeInfo(Integer.parseInt(node.getName()),
			...
			if (mapData.getChildByPath(String.valueOf(i)) != null && mapData.getChildByPath(i + "/obj") != null) {
				for (MapleData node : mapData.getChildByPath(i + "/obj")) {
					if (node.getChildByPath("SN_count") != null && node.getChildByPath("speed") != null) {
						...
						MapleNodes.MaplePlatform mni = new MapleNodes.MaplePlatform(name,
								MapleDataTool.getIntConvert("start", node, 2), speed,
								MapleDataTool.getIntConvert("x1", node, 0), MapleDataTool.getIntConvert("y1", node, 0),
								MapleDataTool.getIntConvert("x2", node, 0), MapleDataTool.getIntConvert("y2", node, 0),
					...
					if (node.getChildByPath("SN_count") != null) {
						String name = MapleDataTool.getString("name", node, "");
						MapleNodes.Environment ev = new MapleNodes.Environment(name,
								MapleDataTool.getIntConvert("x", node, 0), MapleDataTool.getIntConvert("y", node, 0));
		...
		if (mapData.getChildByPath("directionInfo") != null) {
			...
			for (MapleData area : mc) {
				MapleNodes.DirectionInfo di = new MapleNodes.DirectionInfo(Integer.parseInt(area.getName()),
