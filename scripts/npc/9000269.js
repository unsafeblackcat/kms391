			...
			//	NormalReward[i]['item'] == 2630128 || NormalReward[i]['item'] == 2630129 || NormalReward[i]['item'] == 1190303) {
                for (ac = 0; ac < ab; ++ac) {
                    if (NormalReward[i]['item'] == 1190303) {
                        World.Broadcast.broadcastMessage(CField.getGameMessage(8, cm.getPlayer().getName() + " 您在轉盤高級使用券上 STAR : 獲得了會徽."));
                    } else {
                        World.Broadcast.broadcastMessage(CField.getGameMessage(8, cm.getPlayer().getName() + " 님이 메소 뽑기에서 " + cm.getItemName(AdvancedReward[i]['item']) + "獲得了."));
                    }
                    if (ab >= 2) {
                        return;
                    }
			...
			//	AdvancedReward[i]['item'] == 2630128 || AdvancedReward[i]['item'] == 2630129 || AdvancedReward[i]['item'] == 1190303) {
                for (ac = 0; ac < ab; ++ac) {
                    if (AdvancedReward[i]['item'] == 1190303) {
                        ...
                        World.Broadcast.broadcastMessage(CField.getGameMessage(8, cm.getPlayer().getName() + " 在高級拔法中 " + cm.getItemName(AdvancedReward[i]['item']) + "獲得了."));
                    }
                    if (ab >= 2) {
                        return;
                    }
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        var chat = "#fs15#"
        chat += "#h0#您好? 歡迎訪問Fancy服務器!" + enter
        chat += "挑戰普通使用券或高級使用券抽籤!" + enter + enter
        chat += "#L0#普通抽取方法#l" + enter
        chat += "#L1#高級抽取方法#l"
        cm.sendSimple(chat);
    } else if (status == 1) {
        switch (selection) {
