...
public class FrittoEagle {
							...
							public void run() {
								if (c != null && c.getPlayer() != null) {
									c.getSession().writeAndFlush(
											SLFCGPacket.SetIngameDirectionMode(false, true, false, false));
									c.getSession()
											.writeAndFlush(SLFCGPacket.InGameDirectionEvent("", new int[] { 10, 0 }));
	...
	public void start(final MapleClient c) {
		...
		c.getSession().writeAndFlush(
				CField.startMapEffect("독수리를 침착하게 한 마리씩 잡도록 해! 아참, 대머리 독수리는 아무 쓸모 없으니 잡지마!", 5120160, true));
