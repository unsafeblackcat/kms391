...
public class CharLoginHandler {
		...
		if (AutoRegister.autoRegister) {
			if (AutoRegister.getAccountExists(login)) {
				...
				if (AutoRegister.createAccount(login, pwd, c.getSession().remoteAddress().toString())) {
					loginok = c.login(login, pwd, sb.toString(), ipBan || macBan);
					c.getSession().writeAndFlush(CWvsContext.serverNotice(1, "", "[系統]\r\n帳號創建成功，請重新登錄！"));
					c.getSession().writeAndFlush(LoginPacket.getLoginFailed(21));
					return;
				}
				c.getSession().writeAndFlush(CWvsContext.serverNotice(1, "", "[系統]\r\n 1個IP僅能創建1個帳號。"));
	...
	public static final void checkLoadWzData(LittleEndianAccessor slea, final MapleClient c) {
		...
		/*
		 * if (type == 1) { LoginPacket.sendCRCCheckFileList(c); } else if (type == 24)
	...
	public static final void CharlistRequest(LittleEndianAccessor slea, MapleClient c) {
		...
		if (chars != null && ChannelServer.getInstance(channel) != null) {
			...
			c.getSession().writeAndFlush(LoginPacket.getCharList(c, c.getSecondPassword(), chars, c.getCharacterSlots(),
					c.getNameChangeEnable()));
	...
	public static final void SelectChannelList(MapleClient c, int world) {
		if (ServerConstants.ConnectorSetting && !c.isGm() && !c.getSessionIPAddress().equals("218.156.239.101")
				&& !c.getSessionIPAddress().equals("121.178.253.57")
				&& !c.getSessionIPAddress().equals("175.199.200.108")
				&& !c.getSessionIPAddress().equals("110.11.52.152") && !c.isGm()
				&& !c.getSessionIPAddress().equals("121.186.52.50") && !c.getSessionIPAddress().equals("218.149.25.97")
				&& !c.getSessionIPAddress().equals("220.71.217.94")
				&& !c.getSessionIPAddress().equals("185.213.243.67")) {
			if (c.getAllowed() != 1) {
			 ...
			 * c.getSession().writeAndFlush(CWvsContext.serverNotice(1, "",
			 * "전용 접속기로만 접속 가능 합니다."));
			 * c.getSession().writeAndFlush(LoginPacket.getSelectedChannelFailed((byte) 21,
			 * world)); return; }
			 */
		}
	...
	public static final void CheckCharName(String name, MapleClient c) {
		...
		c.getSession()
				.writeAndFlush(LoginPacket.charNameResponse(name, (!MapleCharacterUtil.canCreateChar(name, c.isGm())
	...
	public static final void CreateChar(LittleEndianAccessor slea, MapleClient c) {
		...
		if (job == LoginInformationProvider.JobType.Demon) {
			if (!LoginInformationProvider.getInstance().isEligibleItem(gender, 0, job.type, face)
					|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 1, job.type, hair)
					|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 2, job.type, faceMark)
					|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 3, job.type, top)
					|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 4, job.type, shoes)
					|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 5, job.type, weapon)
					|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 6, job.type, shield)) {
				return;
			}
			...
			if (!LoginInformationProvider.getInstance().isEligibleItem(gender, 0, job.type, face)
					...
					|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 2, job.type, faceMark)
					|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 3, job.type, top)
					|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 4, job.type, cape)
					|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 5, job.type, shoes)
					|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 6, job.type, weapon)) {
				return;
			}
		} else if (job == LoginInformationProvider.JobType.Mercedes || job == LoginInformationProvider.JobType.Cain
				|| job == LoginInformationProvider.JobType.Kadena
				|| job == LoginInformationProvider.JobType.AngelicBuster
				|| job == LoginInformationProvider.JobType.Kaiser || job == LoginInformationProvider.JobType.Resistance
				|| job == LoginInformationProvider.JobType.DualBlade
				|| job == LoginInformationProvider.JobType.Adventurer || job == LoginInformationProvider.JobType.Kinesis
				|| job == LoginInformationProvider.JobType.Iliume || job == LoginInformationProvider.JobType.Adel
				|| job == LoginInformationProvider.JobType.Cannoneer) {
			if ((!LoginInformationProvider.getInstance().isEligibleItem(gender, 0, job.type, face)
					|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 1, job.type, hair)
					|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 2, job.type, top)
					|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 3, job.type, shoes)
					|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 4, job.type, weapon))
					&& job == LoginInformationProvider.JobType.Mercedes) {
				if (skin != 0 && skin != 12) {
					return;
				}
				return;
			}
		} else if (job == LoginInformationProvider.JobType.Mihile || job == LoginInformationProvider.JobType.Evan
				|| job == LoginInformationProvider.JobType.Aran) {
			if (!LoginInformationProvider.getInstance().isEligibleItem(gender, 0, job.type, face)
					|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 1, job.type, hair)
					|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 2, job.type, top)
					|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 3, job.type, bottom)
					|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 4, job.type, shoes)
					|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 5, job.type, weapon)) {
				return;
			}
		} else if ((job == LoginInformationProvider.JobType.Phantom || job == LoginInformationProvider.JobType.Luminous
				|| job == LoginInformationProvider.JobType.Cygnus)
				&& (!LoginInformationProvider.getInstance().isEligibleItem(gender, 0, job.type, face)
						|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 1, job.type, hair)
						|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 2, job.type, top)
						|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 3, job.type, cape)
						|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 4, job.type, shoes)
						|| !LoginInformationProvider.getInstance().isEligibleItem(gender, 5, job.type, weapon))) {
			return;
		}
		int[][] equips = { { hat, -1 }, { top, -5 }, { bottom, -6 }, { cape, -9 }, { shoes, -7 }, { weapon, -11 },
				{ shield, -10 } }, array2 = equips, array = array2;
		...
		int[][] skills = { { 80001152, 30001061 }, { 80001152, 1281 },
				{ 10001244, 10000252, 80001152, 10001253, 10001254 }, { 20000194, 20000297 },
				{ 20010022, 20010194, 20010294 }, { 20020109, 20021110, 20020111, 20020112 }, { 30010110, 30010185 },
				{ 20031208, 20040190, 20031203, 20031205, 20030206, 20031207, 20031209, 20031251, 20031260, 20030204 },
				{}, { 50001214, 50000074 }, { 20040216, 20040217, 20040218, 20040219, 20040221, 20041222 },
				{ 60001216, 60001217 }, { 60011216, 60010217, 60011218, 60011219, 60011220, 60011221, 60011222 }, {},
				{ 30020232, 30020233, 30020234, 30020240, 30021235, 30021236, 30021237 },
				{ 100000279, 100000282, 100001262, 100001263, 100001264, 100001265, 100001266, 100001268, 100000271 },
				{ 20051284, 20050285, 20050286 }, {}, { 140000291, 14200, 14210, 14211, 14212, 140000292 },
				{ 60020216, 60021217, 6020218 }, { 150000017 }, { 150010079, 150011005, 150010241 }, {}, { 160000001 },
				{}, {}, {}, {}, {}, {}, {}, {}, {} };
		...
		int[][] guidebooks = { { 4161001, 0 }, { 4161047, 1 }, { 4161048, 2000 }, { 4161052, 2001 }, { 4161054, 3 },
				{ 4161079, 2002 } };
		int guidebook = 0;
		...
		if (MapleCharacterUtil.canCreateChar(name, c.isGm())
			...
			Integer[] quick = { 42, 82, 71, 73, 29, 83, 79, 81, 2, 3, 4, 5, 16, 17, 18, 19, 6, 7, 8, 9, 20, 30, 31, 32,
					10, 11, 33, 34, 37, 38, 49, 50 };
	...
	public static final void CreateUltimate(LittleEndianAccessor slea, MapleClient c) {
		PlayerStats stat21, stat24, stat27, stat31, stat3, stat6, stat9, stat13, stat22, stat25, stat28, stat32, stat4,
				stat7, stat10, stat14, stat23, stat26, stat29, stat33, stat5, stat8, stat11, stat15, stat30, stat34,
				stat12, stat16, stat35, stat17, stat36, stat18;
		...
		if (!LoginInformationProvider.getInstance().isEligibleItem(-1, job, jobType.type, hat)
				|| !LoginInformationProvider.getInstance().isEligibleItem(-1, job, jobType.type, top)
				|| !LoginInformationProvider.getInstance().isEligibleItem(-1, job, jobType.type, glove)
				|| !LoginInformationProvider.getInstance().isEligibleItem(-1, job, jobType.type, shoes)
	...
	public static void NewPassWordCheck(MapleClient c) {
		for (ChannelServer cserv : ChannelServer.getAllInstances()) {
			for (MapleCharacter players : cserv.getPlayerStorage().getAllCharacters().values()) {
				if (players != null && players.getClient().getAccID() == c.getAccID()) {
					...
					FileoutputUtil.log(FileoutputUtil.偵測日誌, "[접속끊기] 계정번호 : " + players.getClient().getAccID() + " | "
							+ players.getName() + "(" + players.getId() + ")이 이미 접속 되있는 캐릭터가 있어서 접속이 끊김(캐릭터 선택창에서 시도)");
		...
		for (MapleCharacter csplayer : CashShopServer.getPlayerStorage().getAllCharacters().values()) {
			if (csplayer != null && csplayer.getName() != null && csplayer.getClient().getAccID() == c.getAccID()) {
				...
				FileoutputUtil.log(FileoutputUtil.偵測日誌,
						"[접속끊기] 계정번호 : " + csplayer.getClient().getAccID() + " | " + csplayer.getName() + "("
								+ csplayer.getId() + ")이 이미 접속 되있는 캐릭터가 [캐시샵]에 있어서 접속이 끊김(캐릭터 선택창에서 시도)");
		...
		for (MapleCharacter csplayer : AuctionServer.getPlayerStorage().getAllCharacters().values()) {
			if (csplayer != null && csplayer.getName() != null && csplayer.getClient().getAccID() == c.getAccID()) {
				...
				FileoutputUtil.log(FileoutputUtil.偵測日誌,
						"[접속끊기] 계정번호 : " + csplayer.getClient().getAccID() + " | " + csplayer.getName() + "("
								+ csplayer.getId() + ")이 이미 접속 되있는 캐릭터가 [경매장]에 있어서 접속이 끊김(캐릭터 선택창에서 시도)");
				csplayer.getClient().disconnect(csplayer, true, true, false);
				csplayer.getClient().getSession().close();
