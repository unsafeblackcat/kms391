...
var SCRIPT_TITLE = "#e#b<一鍵星火系統>#n#k";
var enter = "\r\n";
...
function action(mode, type, selection) {
    ...
    chat = "";
    chat += "#fs14#";
    chat += SCRIPT_TITLE + enter;
    if (status == 0) {
        chat += "#b楓之谷便利一鍵星火系統,";
        chat += "#b請選擇要使用的輪回火焰#n#k\r\n#fs12##r注意：需要星火强化過的裝備才能進行快速强化#n#k#fs15#." + enter;
        ...
        if (selectedEquipment != null && selectedRebirthFireItemId != null) {
            if (checkCost(selectedRebirthFireItemId)) {
                ...
                chat += "#L" + SELECTION_APPLY + "#再次使用" + enter;
                chat += "#L" + SELECTION_EXIT + "#結束使用" + enter;
            } else {
...
function formattedMeso(meso) {
    ...
    if (upperMeso >= 1) mesoString += upperMeso + "億";
    if (lowerMeso > 0) mesoString += lowerMeso + "萬";
