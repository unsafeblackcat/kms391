...
public class PlayerInteractionHandler {
	...
	public static final void PlayerInteraction(final LittleEndianAccessor slea, final MapleClient c,
		...
		case CHAT: {
			...
			if (chr.getTrade() != null) {
				...
				FileoutputUtil.log(FileoutputUtil.交易聊天日誌,
						"[교환] [" + c.getChannel() + "채널] 상대방이름 : " + chr.getTrade().getPartner().getChr().getName()
								+ " | " + c.getPlayer().getName() + " : " + message);
				break;
			} else if (chr.getPlayerShop() != null) {
				IMaplePlayerShop iMaplePlayerShop = chr.getPlayerShop();
				iMaplePlayerShop.broadcastToVisitors(PlayerShopPacket.shopChat(chr, chr.getName(), chr.getId(),
						chr.getName() + " : " + message, iMaplePlayerShop.getVisitorSlot(chr)));
				...
				if (iMaplePlayerShop instanceof MaplePlayerShop) {
					...
					etc = "주인 : " + iMaplePlayerShop.getOwnerName() + " / 상점명 : " + iMaplePlayerShop.getDescription()
							+ " / 수신 : " + iMaplePlayerShop.getMemberNames();
				} else if (iMaplePlayerShop instanceof MapleMiniGame) {
					chatType = LogType.Chat.MiniGame;
					etc = "주인 : " + iMaplePlayerShop.getOwnerName() + " / 게임명 : " + iMaplePlayerShop.getDescription()
							+ " / 암호 : "
							+ ((iMaplePlayerShop.getPassword() == null) ? "없음"
									: ("있음 - " + iMaplePlayerShop.getPassword()))
							+ " / 수신 : " + iMaplePlayerShop.getMemberNames();
				}
				AdminTool.addMessage(8, "[" + c.getChannel() + "채널] " + c.getPlayer().getName() + " : " + message);
				FileoutputUtil.log(FileoutputUtil.小遊戲聊天日誌,
						"[미니게임] [" + c.getChannel() + "채널] | " + c.getPlayer().getName() + " : " + message);
		...
		case EXIT: {
			...
			if (chr.getOneCardInstance() != null) {
				chr.getOneCardInstance().sendPacketToPlayers(
						SLFCGPacket.leaveResult(chr.getOneCardInstance().getPlayer(chr).getPosition()));
		...
		case OPEN: {
			...
			if (c.getChannelServer().isShutdown()) {
				chr.dropMessage(1, "由於服務器即將關閉，無法建立房間.");
				c.getSession().writeAndFlush((Object) CWvsContext.enableActions(c.getPlayer()));
		...
		case ADD_ITEM1: {
			...
			if ((ii2.isDropRestricted(ivItem.getItemId()) || ii2.isAccountShared(ivItem.getItemId())
					|| ItemFlag.UNTRADEABLE.check(flag)) && !ItemFlag.KARMA_EQUIP.check(flag)
			...
			if (GameConstants.getLowestPrice(ivItem.getItemId()) > price) {
				c.getPlayer().dropMessage(1,
						"The lowest you can sell this for is " + GameConstants.getLowestPrice(ivItem.getItemId()));
				c.getSession().writeAndFlush((Object) CWvsContext.enableActions(c.getPlayer()));
				return;
			}
			...
			if (subpacket == 19 && type3 == 7) {
				if (c.SecondPwUse == 1) {
					...
					if (c.SecondPwUse == 1) { // c.CheckSecondPassword(secondPassword) ||
						if (chr != null && chr.getTrade() != null && chr.getTrade().getPartner() != null
								&& chr.getTrade().getPartner().getChr() != null) {
							MapleTrade.visitCashTrade(chr, chr.getTrade().getPartner().getChr());
						} else {
		...
		case REMOVE_ITEM: {
			...
			if (shop5 == null || !shop5.isOwner(chr) || shop5 instanceof MapleMiniGame || shop5.getItems().size() <= 0
					|| shop5.getItems().size() <= slot2 || slot2 < 0) {
				return;
			}
		...
		case START: {
			...
			if (chr.getTrade() != null && chr.getTrade().getPartner() != null) {
				...
				chr.getTrade().getPartner().getChr().getClient().getSession()
						.writeAndFlush((Object) PlayerShopPacket.StartRPS());
				break;
			}
			break;
		}
		case REQUEST_TIE: {
			...
			if (game2.isOwner(chr)) {
				...
				game2.getMCOwner().getClient().getSession()
						.writeAndFlush((Object) PlayerShopPacket.getMiniGameRequestTie());
			}
		...
		case REQUEST_REDO: {
			...
			if (game2.isOwner(chr)) {
				...
				game2.getMCOwner().getClient().getSession()
						.writeAndFlush((Object) PlayerShopPacket.getMiniGameRequestRedo());
			}
			break;
		}
		case ANSWER_REDO: {
			...
			if (slea.readByte() > 0) {
				ips3.broadcastToVisitors(
						PlayerShopPacket.getMiniGameSkip((int) ((ips3.getVisitorSlot(chr) == 0) ? 1 : 0)));
				game2.nextLoser();
			} else {
			...
			if (ips3 != null && ips3 instanceof MapleMiniGame) {
				...
				if (chr.getTrade() != null && chr.getTrade().getPartner() != null) {
					...
					Timer.ShowTimer.getInstance().schedule(new Runnable() {
						@Override
						public void run() {
							final byte result = PlayerInteractionHandler.getResult(chr.getTrade().getPRS(),
									chr.getTrade().getPartner().getPRS());
							if (result == 2) {
								chr.dropMessage(1, "雖然很可惜，但是剪刀石頭布輸了！");
								chr.addFame(-1);
							} else if (result == 0) {
								chr.dropMessage(1, "恭喜！剪刀石頭布贏了！");
								chr.addFame(1);
							}
							c.getSession().writeAndFlush(
									(Object) PlayerShopPacket.FinishRPS(result, chr.getTrade().getPartner().getPRS()));
						}
					}, 1000L);
					break;
				}
		...
		case SELECT_CARD: {
			...
			if (turn == 1) {
				...
				if (game2.isOwner(chr)) {
					...
					game2.getMCOwner().getClient().getSession()
							.writeAndFlush((Object) PlayerShopPacket.getMatchCardSelect(turn, slot3, fs, turn));
				}
				game2.setTurn(0);
				return;
			}
		...
		case ONECARD: {
			...
			case 0: {
				...
				if (oc.getLastCard().getType() == 9 || oc.getLastCard().getType() == 8
						|| (oc.getLastCard().getType() == 12 && oc.getLastCard().getColor() == 3)) {
					nextPlayer = oc.getLastPlayer();
				} else {
					if (oc.getLastCard().getType() == 10) {
						nextPlayer = oc.setNextPlayer(oc.setNextPlayer(oc.getLastPlayer(), oc.isbClockWiseTurn()),
								oc.isbClockWiseTurn());
					} else {
				...
				if (oc.getLastCard().getType() == 12) {
					if (oc.getLastCard().getColor() == 1) {
						oc.setFire(0);
						oc.sendPacketToPlayers(
								SLFCGPacket.OneCardGamePacket.onShowScreenEffect("/Effect/screeneff/michael"));
						...
						oc.sendPacketToPlayers(
								SLFCGPacket.OneCardGamePacket.onShowScreenEffect("/Effect/screeneff/hawkeye"));
						...
						oc.sendPacketToPlayers(
								SLFCGPacket.OneCardGamePacket.onShowScreenEffect("/Effect/screeneff/irina"));
						for (OneCardGame.OneCardPlayer bp : oc.getPlayers()) {
							...
							if (bp.getCards().size() == 0 || bp.getCards().isEmpty()) {
								...
								oc.sendPacketToPlayers(
										SLFCGPacket.OneCardGamePacket.onShowScreenEffect("/Effect/screeneff/lastcard"));
							}
						}
						...
						oc.sendPacketToPlayers(
								SLFCGPacket.OneCardGamePacket.onShowScreenEffect("/Effect/screeneff/icart")); // 이카르트
					}
				}
				...
				if (nextPlayer != null) {
					...
					if (oc.getLastCard().getType() == 8) {// || (oc.getLastCard().getType() == 12 &&
															// oc.getLastCard().getType() == 3)) {
						...
						if (oc.getLastCard().getColor() == 4) {
							for (OneCardGame.OneCard card : nextPlayer.getCards()) {
								if (oc.getFire() == 0) {
									...
									if (card.getType() == 6 || card.getType() == 7
											|| (card.getType() == 12 && card.getColor() == 0)) {
										possibleCards.add(card);
									}
								}
							}
							...
							for (OneCardGame.OneCard card : nextPlayer.getCards()) {
								if (card.getType() <= 5) {
									if ((card.getColor() == oc.getLastCard().getColor()
											|| card.getType() == oc.getLastCard().getType()) && oc.getFire() == 0) {
										possibleCards.add(card);
									}
								} else if (card.getType() <= 7) {
									if (oc.getFire() == 0) {
										if (card.getColor() == oc.getLastCard().getColor()
												|| card.getType() == oc.getLastCard().getType()) {
											possibleCards.add(card);
										}
									} else {
										if (oc.getLastCard().getType() == 6) {
											if (card.getType() == 6) {
												possibleCards.add(card);
													...
													&& card.getColor() == oc.getLastCard().getColor()) {
												possibleCards.add(card);
											}
										} else if (oc.getLastCard().getType() == 7) {
											if (card.getType() == 7) {
						...
						if (oc.getOneCardDeckInfo().size() == 0 || oc.getOneCardDeckInfo().isEmpty()) {
							...
							oc.sendPacketToPlayers(
									SLFCGPacket.OneCardGamePacket
											.onUserPossibleAction(nextPlayer, possibleCards,
													(possibleCards.size() == 0 && nextPlayer.getCards().size() == 16)
															|| nextPlayer.getCards().size() < 16,
													oc.isbClockWiseTurn()));
						} else {
							oc.sendPacketToPlayers(
									SLFCGPacket.OneCardGamePacket
						...
						nextPlayer.getPlayer().getClient().getSession()
								.writeAndFlush(CField.playSound("Sound/MiniGame.img/oneCard/myturn"));
						nextPlayer.getPlayer().getClient().getSession()
								.writeAndFlush(CWvsContext.enableActions(nextPlayer.getPlayer()));
						nextPlayer.getPlayer().getClient().getSession()
								.writeAndFlush(SLFCGPacket.onShowText("당신의 턴입니다."));
					}
			...
			case 1: {
				...
				if (oc.getFire() > 0) {
					oc.sendPacketToPlayers(
							SLFCGPacket.onShowText(chr.getName() + "님이 " + oc.getFire() + "의 피해를 입었습니다."));
				}
				...
				if (ocp.getCards().size() >= 17) { // 파산
					oc.setFire(0);
					oc.sendPacketToPlayers(
							SLFCGPacket.OneCardGamePacket.onShowScreenEffect("/Effect/screeneff/gameover"));
				...
				if (nextPlayer != null) {
					...
					if (oc.getOneCardDeckInfo().size() == 0 || oc.getOneCardDeckInfo().isEmpty()) {
						oc.resetDeck();
						oc.sendPacketToPlayers(SLFCGPacket.OneCardGamePacket.onEffectResult(0, 0, 0, false));
						oc.sendPacketToPlayers(SLFCGPacket.OneCardGamePacket.onUserPossibleAction(nextPlayer,
								possibleCards, nextPlayer.getCards().size() < 17, oc.isbClockWiseTurn()));
					} else {
						oc.sendPacketToPlayers(SLFCGPacket.OneCardGamePacket.onUserPossibleAction(nextPlayer,
								possibleCards, nextPlayer.getCards().size() < 17, oc.isbClockWiseTurn()));
					}
			...
			case 2: {
				...
				oc.sendPacketToPlayers(
						SLFCGPacket.OneCardGamePacket.onChangeColorResult(oc.getLastCard().getType() == 12, color));
		...
		case ONECARD_EMOTION: {
			...
			chr.getOneCardInstance()
					.sendPacketToPlayers(SLFCGPacket.OneCardGamePacket.onEmotion(chr.getId(), emotionId));
			chr.getClient().getSession().writeAndFlush(CWvsContext.enableActions(chr));
			break;
		}
		default: {
			System.out.println(
					"Unhandled interaction action by " + chr.getName() + " : " + action + ", " + slea.toString());
			break;
		}
		}
	}
	...
	public enum Interaction {
		SET_ITEMS1(0), SET_ITEMS2(1), SET_ITEMS3(2), SET_ITEMS4(3), SET_MESO1(4), SET_MESO2(5), SET_MESO3(6),
		SET_MESO4(7), CONFIRM_TRADE1(8), CONFIRM_TRADE2(9), CONFIRM_TRADE_MESO1(10), CONFIRM_TRADE_MESO2(11),
		CREATE(16), VISIT(19), INVITE_TRADE(21), DENY_TRADE(22), CHAT(24), OPEN(26), EXIT(28),
		HIRED_MERCHANT_MAINTENANCE(29), RESET_HIRED(30), ADD_ITEM1(31), ADD_ITEM2(32), ADD_ITEM3(33), ADD_ITEM4(34),
		BUY_ITEM_HIREDMERCHANT(35), PLAYER_SHOP_ADD_ITEM(36), BUY_ITEM_PLAYER_SHOP(37), BUY_ITEM_STORE(38),
		REMOVE_ITEM(47), MAINTANCE_OFF(48), MAINTANCE_ORGANISE(49), CLOSE_MERCHANT(50), TAKE_MESOS(52),
		VIEW_MERCHANT_VISITOR(53), VIEW_MERCHANT_BLACKLIST(54), MERCHANT_BLACKLIST_ADD(55),
		// MERCHANT_BLACKLIST_REMOVE(56),
		ADMIN_STORE_NAMECHANGE(59), REQUEST_TIE(56), ANSWER_TIE(57), GIVE_UP(58), REQUEST_REDO(60), ANSWER_REDO(61),
		EXIT_AFTER_GAME(62), CANCEL_EXIT(63), READY(64), UN_READY(65), EXPEL(66), START(67), GAME_RESULT(68), SKIP(69),
		MOVE_OMOK(70), SELECT_CARD(71), WEDDING_START(105), WEDDING_END(108), INVITE_ROCK_PAPER_SCISSORS(83), // 377
		ONECARD(155), ONECARD_EMOTION(156);
