    ...
    if (status == 0) {
        ...
        if (parseInt(qm.getClient().getCustomData(100795, "start")) == 1) {
            if (selection == 1) {
                ...
                if (qm.getPlayerCount(993192800) >= 1) {
                    qm.sendNext("이미 블루밍 포레스트에 도전중인 유저가 있습니다.");
                    qm.dispose();
                    return;
                }
                qm.warp(993192700, 0);
                ...
                //상점오픈
                if (qm.getClient().getCustomData(501368, "spoint") != null) {
                    qm.getPlayer().setKeyValue(501368, "point", qm.getClient().getCustomData(501368, "spoint")+"")
                }
                qm.openShop(41);
                qm.dispose();
            }
