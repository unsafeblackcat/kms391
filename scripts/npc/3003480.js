...
var daily = [ // 첫번째
	{'mobid' : 8644401, 'qty' : 201, 't' : 1}, 
{'mobid' : 8644402, 'qty' : 202, 't' : 1},
{'mobid' : 8644403, 'qty' : 203, 't' : 1},
{'mobid' : 8644405, 'qty' : 204, 't' : 1},
{'mobid' : 8644409, 'qty' : 205, 't' : 1}, 
{'mobid' : 8644411, 'qty' : 206, 't' : 1}
]
	...
	if (status == 0) {
		...
		if (isclear == 2 || isclear == 5) {
			if ((isitem && (cm.haveItem(getk("mobid"), getk("mobq")))) || (!isitem && (getk("count") >= getk("mobq")))) {
				...
				if (isclear == 5) {
					msg += "여기, 이건 오늘의 보상입니다!\r\n보상을 받기전 장비창을 20칸 이상 비워주세요."+enter;
					msg += getReward()+enter;
				}
		...
		switch (isclear) {
			case 1:
				var msg = "모라스의.. 아름다움을 유지해다오..\r\n#r#e*보상을 받기전 장비창을 20칸 이상 비워주세요.#n#k"+enter+enter;
				msg += "#fUI/UIWindow2.img/UtilDlgEx/list1#\r\n#d#L1#(Lv."+제한레벨+") #b#e[일일 퀘스트]#n#k #d모라스";

				cm.sendSimple(msg);
			break;
			case 2:
				var msg = "처리해줘야 할 임무는 아래와 같습니다!"+enter+enter;
				msg += "#fUI/UIWindow2.img/UtilDlgEx/list0#\r\n#r#e[일일 퀘스트] : 모라스#n#k"+enter;
				msg += "처치 대상 : #b#o"+getk("mobid")+"# "+getk("mobq")+"마리#k"+enter+enter;
				msg += "그럼, 일이 끝난 후에 다시 찾아와주십시오!\r\n#r#e*보상을 받기전 장비창을 20칸 이상 비워주세요.#n#k";

				cm.sendOk(msg);
				cm.dispose();
			break;
			case 4:
				var msg = "모라스의.. 아름다움을 유지해다오.."+enter+enter;
				msg += "#fUI/UIWindow2.img/UtilDlgEx/list1#\r\n#d#L1#(Lv."+제한레벨+") #b#e[일일 퀘스트]#n#k #d모라스 2";
		...
		if (getk("isclear") == 1 || getk("isclear") == 4) {
			var msg = "처리해줘야 할 임무는 아래와 같습니다!"+enter+enter;
			msg += isitem ? "#r#e[일일 퀘스트] : 모라스#n#k"+enter : "#fUI/UIWindow2.img/UtilDlgEx/list0#\r\n#r#e[일일 퀘스트] : 모라스#n#k"+enter;
		...
		var msg = "처리해줘야 할 임무는 아래와 같습니다!"+enter+enter;
		msg += isitem ? "#fUI/UIWindow2.img/UtilDlgEx/list0#\r\n#r#e[일일 퀘스트] : 모라스#n#k"+enter : "#fUI/UIWindow2.img/UtilDlgEx/list0#\r\n#r#e[일일 퀘스트] : 모라스#n#k"+enter;
		msg += isitem ? "\r\n목표 전리품 : #b#i"+getk("mobid")+"##z"+getk("mobid")+"# "+getk("mobq")+"개#k"+enter+enter : "처치 대상 : #b#o"+getk("mobid")+"# "+getk("mobq")+"마리#k"+enter+enter;
		msg += "그럼, 일이 끝난 후에 다시 찾아와주십시오!";
		cm.sendOk(msg);
		cm.dispose();
