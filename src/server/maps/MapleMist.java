...
public class MapleMist extends MapleMapObject {
	...
	public MapleMist(Rectangle mistPosition, MapleCharacter owner, SecondaryStatEffect source, int duration,
			byte rltype) {
		this.mistPosition = mistPosition;
		this.owner = owner;
		...
		this.skilllevel = (source.getSourceId() == 25111012) ? owner.getSkillLevel(25111005)
				: owner.getTotalSkillLevel(GameConstants.getLinkedSkill(source.getSourceId()));
		setDuration(duration);
		setRltype(rltype);
		setStartTime(System.currentTimeMillis());
		// if (owner.isGM())
		// owner.dropMessage(6, "skillid " + this.source.getSourceId() + " mist lv : " +
		// this.skilllevel + " / duration : " + this.duration);
