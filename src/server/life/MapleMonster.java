...
public class MapleMonster extends AbstractLoadedMapleLife {
	...
	private long lastDamageTaken; // 新增字段
	...
	private boolean alive; // 新增 alive 字段

	public MapleMonster(int id, MapleMonsterStats stats) {
		super(id);
		initWithStats(stats);
		this.alive = true; // 初始化 alive 字段为 true
	}
	...
	public final void addHp(long hp, boolean brodcast) {
		...
		// 更新怪物的当前血量，将传入的血量值加到当前血量上
		this.hp = getHp() + hp;

		// 检查更新后的血量是否超过了怪物的最大血量
		if (this.hp > getStats().getHp()) {
			// 如果超过最大血量，则将当前血量设置为最大血量，避免血量溢出
			this.hp = getStats().getHp();
		}

		// 检查是否需要广播怪物的血量信息
		if (brodcast) {
			// 如果需要广播，则通过地图对象广播显示怪物血量的消息包
		...
		if (this.hp <= 0L) {
			// 输出死亡怪物的信息到控制台
			System.out.println("怪物已死亡，信息如下：");
			System.out.println("怪物ID: " + getId());
			System.out.println("怪物名称: " + getStats().getName());
			System.out.println("怪物所在地图ID: " + getMap().getId());

			// 如果血量小于等于0，则调用地图对象的killMonster方法击杀怪物
			// 传入当前怪物对象、控制器（可能是击杀该怪物的角色）、是否掉落物品、是否有额外效果、死亡类型和额外参数
	...
	public final void damage(MapleCharacter from, long damage, boolean updateAttackTime, int lastSkill) {
		...
		if (getId() == 8880505) { // 8880505 로 1페 클리어 여부
			if (this.getMap().getId() / 1000 == 450017
					&& (this.getMap().getMobsSize(8880520) > 0 || this.getMap().getMobsSize(8880521) > 0)) { // 창조와 파괴의
																												// 기사 살아
																												// 있을 시
																												// 8880505
																												// 에 데미지
		...
		if (GameConstants.isUnionRaid(from.getMapId())) {
			...
			case 9833205:
				...
				for (MapleMonster monster : getMap().getAllMonster()) {
					if (monster.getOwner() == from.getId() && monster.getId() == getId() - 100
							&& monster.getBuff(MonsterStatus.MS_PowerImmune) == null) {
						List<Pair<MonsterStatus, MonsterStatusEffect>> stats = new ArrayList<>();
						stats.add(
								new Pair<>(MonsterStatus.MS_PowerImmune, new MonsterStatusEffect(146, 210000000, 1L)));
		...
		if (getId() == 8880305) {
			...
			if (Randomizer.isSuccess(30)) {
				for (int i = 0; i < Randomizer.rand(1, 3); i++) {
					...
					Obstacle ob = new Obstacle(key, new Point(x, y - 601), new Point(x, y), 40, (key == 64) ? 0 : 60,
							1208, 111, 3, 599);
					obs.add(ob);
				}
		...
		if (getId() == 9500805) {
			...
			if (totaldamage >= getStats().getHp() / 1000L * 10L) {
				...
				// from.getPlayer().setKeyValue(221121, "point", "" +
				// (from.getPlayer().getKeyValue(221121, "point") + totaladdpoint) + "");
				from.getClient()
						.send(CField.PunchKingPacket(from, 3, new int[] { getObjectId(), Randomizer.rand(25, 58) }));
		...
		if (from.getMapId() == 941000200 && (getId() >= 9500800 && getId() < 9500805)) {
			...
			if (getHp() - damage <= 0L) {
				if (getCustomValue0(getId()) > 0L) {
					...
					// from.getPlayer().setKeyValue(221121, "point", "" +
					// (from.getPlayer().getKeyValue(221121, "point") + getCustomValue0(getId())) +
					// "");
					from.getClient().send(CField.PunchKingPacket(from, 3,
							new int[] { getObjectId(), (int) getCustomValue0(getId()) }));
					from.getClient().send(CField.PunchKingPacket(from, 2, new int[] { po1 }));
				}
			} else {
				from.setSkillCustomInfo(getId(), from.getSkillCustomValue0(getId()) + damage, 0L);
				...
				if (totaldamage >= getStats().getHp() / 100L * 10L) {
					...
					// (from.getPlayer().getKeyValue(221121, "point") + totaladdpoint) + "");
					from.getClient().send(
							CField.PunchKingPacket(from, 3, new int[] { getObjectId(), Randomizer.rand(25, 58) }));
					from.getClient().send(CField.PunchKingPacket(from, 2, new int[] { po1 }));
				}
			}
			if (nextStage) {
		...
		if (from.getBuffedEffect(SecondaryStat.Reincarnation) != null && this.stats.isBoss()
			...
			if (from.getReinCarnation() == 0) {
				from.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(from, 0, 1320019, 1, 0, 0,
						(byte) (from.isFacingLeft() ? 1 : 0), true, from.getPosition(), null, null));
				from.getMap().broadcastMessage(from, CField.EffectPacket.showEffect(from, 0, 1320019, 1, 0, 0,
						(byte) (from.isFacingLeft() ? 1 : 0), false, from.getPosition(), null, null), false);
			}
			...
			statups.put(SecondaryStat.Reincarnation, new Pair<>(Integer.valueOf(1),
					Integer.valueOf((int) from.getBuffLimit(from.getBuffSource(SecondaryStat.Reincarnation)))));
			from.getClient().getSession().writeAndFlush(
					CWvsContext.BuffPacket.giveBuff(statups, from.getBuffedEffect(SecondaryStat.Reincarnation), from));
		}
		...
		if (this.map.isElitebossrewardmap() && this.map.getElitebossrewardtype() == 1 && getId() == 8220027) {
			...
			for (int i = 0; i < Randomizer.rand(1, 3); i++) {
				...
				this.map.spawnItemDrop(this, getController(), toDrop,
						new Point((getTruePosition()).x + ((i == 1) ? 25 : ((i == 2) ? 50 : 0)), (getTruePosition()).y),
						true, false);
			}
		}
		...
		if (cap != null && ((cap.getValue() == 100 && (getId() == 8900001 || getId() == 8900101))
				|| (cap.getValue() == 200 && (getId() == 8900002 || getId() == 8900102)))) {
		...
		if (from.getMap().getId() / 10000 == 92507) {
			...
			if ((stage == 9 || stage == 13 || stage == 15 || stage == 45) && damage >= getHp()
					&& getCustomValue0(getId()) == 0L) {
				...
				Timer.EtcTimer.getInstance()
						.schedule(() -> this.map.killMonster(this, from, true, false, (byte) 1, lastSkill), 5000L);
				return;
			}
		}
		if (from.getBuffedValue(80002902) && !isAlive()
				&& (getId() == 8644653 || getId() == 8644656 || getId() == 8644654 || getId() == 8644657)) {
			...
			if (from.getSkillCustomValue0(8644651) <= 0L) {
				...
				for (MapleMonster m : getMap().getAllMonster()) {
					if (m.getOwner() == from.getId() && (m.getId() == 8644653 || m.getId() == 8644656
							|| m.getId() == 8644654 || m.getId() == 8644657)) {
			...
			if (getCustomValue0(8880111) == 1L) {
				...
				if (getCustomValue0(8880112) >= 1000000000L) {
					getMap().broadcastMessage(
							MobPacket.demianRunaway(this, (byte) 1, MobSkillFactory.getMobSkill(214, 14), 0, false));
		...
		if (getId() == 8641018 && getCustomValue(8641018) == null
				&& !GameConstants.is_forceAtom_attack_skill(lastSkill)) {
			int mobid = (getController().getSkillCustomValue0(450001402) == 0L) ? 8641019
					: ((getController().getSkillCustomValue0(450001402) == 1L) ? 8641020 : 8641021);
			MapleMonster mob = MapleLifeFactory.getMonster(mobid);
			getMap().spawnMonsterOnGroundBelow(mob,
					new Point((getPosition()).x + Randomizer.rand(-150, 150), (getPosition()).y));
		...
		if (getId() == 8800002 || getId() == 8800102) {
			...
			for (MapleMonster mob : getMap().getAllMonster()) {
				if ((mob.getId() >= 8800003 && mob.getId() <= 8800010)
						|| (mob.getId() >= 8800103 && mob.getId() <= 8800110)) {
					mobsize++;
				}
			}
		...
		if (from.getMapId() == 993192800
				&& ((getId() >= 9833935 && getId() <= 9833946) || (getId() >= 9833947 && getId() <= 9833958))) {
			boolean nextStage = (from.getMap().getAllMonster().size() == 1 && getHp() - damage <= 0L);
			int stage = (int) from.getSkillCustomValue0(993192801);
			if (getHp() - damage <= 0L) {
				if (getCustomValue0(getId()) > 0L) {
					from.getClient().setCustomData(100795, "point",
							"" + (Integer.parseInt(from.getClient().getCustomData(100795, "point"))
									+ getCustomValue0(getId())) + "");
					from.getClient().send(CField.PunchKingPacket(from, 3,
							new int[] { getObjectId(), (int) getCustomValue0(getId()) }));
					from.getClient().send(CField.PunchKingPacket(from, 2,
							new int[] { Integer.parseInt(from.getClient().getCustomData(100795, "point")) }));
				}
			} else {
				from.setSkillCustomInfo(getId(), from.getSkillCustomValue0(getId()) + damage, 0L);
				long totaldamage = from.getSkillCustomValue(getId()).longValue();
				...
				if (totaldamage >= getStats().getHp() / 100L * 10L) {
					...
					addSkillCustomInfo(getId(), -totaladdpoint);
					from.getClient().setCustomData(100795, "point", ""
							+ (Integer.parseInt(from.getClient().getCustomData(100795, "point")) + totaladdpoint) + "");
			...
			if (nextStage) {
				if (stage + 1 > 12) {
					...
					from.getClient()
							.send(SLFCGPacket.OnYellowDlg(9062507, 100, "#rSTAGE " + (stage + 1) + "#k 시작한담!", ""));
					...
					for (int i = -140; i <= 180; i += 80) {
						...
						getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9833947 + stage),
								new Point(i, 581));
					}
					stage++;
		...
		if (this.stats.getSelfD() != -1) {
			...
			if (this.sponge.get() != null && ((MapleMonster) this.sponge.get()).hp > 0L) {
				...
				if (((MapleMonster) this.sponge.get()).hp <= 0L
						&& (((MapleMonster) this.sponge.get()).getId() == 8810122
								|| ((MapleMonster) this.sponge.get()).getId() == 8810018)) {
					for (MapleMonster m : getMap().getAllMonster()) {
				...
				if (((MapleMonster) this.sponge.get()).hp <= 0L) {
					...
					this.map.broadcastMessage(MobPacket.showBossHP(((MapleMonster) this.sponge.get()).getId(), -1L,
							((MapleMonster) this.sponge.get()).getMobMaxHp() * getMobMaxHp()));
					if (send) {
			...
			if (this.hp > 0L) {
				...
				if (getStats().isMobZone()) {
					if (getId() == 8880101 || getId() == 8880111) {
						...
						if (size < maxcount) {
							...
							getMap().spawnMonsterWithEffect(m, MobSkillFactory.getMobSkill(201, 182).getSpawnEffect(),
									getPosition());
						}
						if (this.phase != phase) {
							setPhase(phase);
				...
				if (this.hp > 0L) {
					...
					if (this.map.getLucidCount() + this.map.getLucidUseCount() < count
							&& (getId() == 8880140 || getId() == 8880141 || getId() == 8880150 || getId() == 8880151)) {
						if (this.map.getLucidCount() < 3) {
							this.map.setLucidCount(this.map.getLucidCount() + 1);
							this.map.broadcastMessage(
									CField.enforceMSG("나팔동상 근처에서 '채집' 키를 눌러 사용하면 루시드의 힘을 억제할 수 있습니다!", 222, 2000));
						}
						this.map.broadcastMessage(
								MobPacket.BossLucid.changeStatueState(false, this.map.getLucidCount(), false));
					}
					switch (getId()) {
					case 8880300:
					case 8880303:
					case 8880304:
						...
						if (will != null) {
							...
							this.map.broadcastMessage(
									MobPacket.BossWill.setWillHp(hps, this.map, 8880300, 8880303, 8880304));
						}
						break;
					case 8880340:
					case 8880343:
					case 8880344:
						...
						if (will != null) {
							List<Integer> hps = will.getWillHplist();
							this.map.broadcastMessage(
									MobPacket.BossWill.setWillHp(hps, this.map, 8880340, 8880343, 8880344));
						}
						break;
					case 8880301:
					case 8880341:
				...
				if (getStats().isBoss() && from.getSkillLevel(131001026) > 0 && !from.getBuffedValue(131003026)
						&& !from.skillisCooling(131001026)) {
				...
				if (this.hp <= 0L) {
					...
					if (from.getSkillLevel(131001026) > 0 && !from.getBuffedValue(131003026)
							&& !from.skillisCooling(131001026)) {
						SkillFactory.getSkill(131001026).getEffect(1).applyTo(from);
					}
					...
					if (this.stats.getHPDisplayType() == 0) {
						this.map.broadcastMessage(
								MobPacket.showBossHP(getId(), -1L, (long) (getMobMaxHp() * bonusHp())),
								getTruePosition());
					}
					this.map.killMonster(this, from, true, false, (byte) 1, lastSkill);
	...
	public final void killed() {
		for (Pair<MonsterStatus, MonsterStatusEffect> skill : this.stati) {
			if (((MonsterStatusEffect) skill.getRight()).getSchedule() != null
					&& !((MonsterStatusEffect) skill.getRight()).getSchedule().isDone()) {
	...
	private final void giveExpToCharacter(MapleCharacter attacker, long exp, boolean highestDamage, int numExpSharers,
			byte pty, byte Class_Bonus_EXP_PERCENT, byte Premium_Bonus_EXP_PERCENT, int lastskillID) {
		...
		if (exp > 0L) { // 레벨별 배율
			...
			if (!attacker.getBuffedValue(80002282) && attacker.getMap().getRuneCurse() > 0
					&& !GameConstants.보스맵(getMap().getId()) && !GameConstants.isContentsMap(getMap().getId())) {
				attacker.getClient().getSession().writeAndFlush(
						CField.runeCurse("必須解放符文以解除精英BOSS的詛呪!!\\n詛呪階段 " + attacker.getMap().getRuneCurse()
								+ " :  經驗埴獲取率、掉落率 " + attacker.getMap().getRuneCurseDecrease() + "% 降低效果生效中", false));
				long curse = exp * attacker.getMap().getRuneCurseDecrease() / 100L;
				exp -= curse;
				attacker.dropMessage(6, "因符文之詛呪經驗埴 " + exp + " 獲得中");
			}
			if (attacker.getLevel() >= 200) {
				int level = 20;
				if (attacker.getMap().isSpawnPoint() && !getStats().isBoss()
						&& (getStats().getLevel() - level > attacker.getLevel()
								|| attacker.getLevel() > getStats().getLevel() + level)) {
					exp -= exp * 80L / 100L;
					if (attacker.getSkillCustomValue0(60524) == 0L) {
						attacker.setSkillCustomInfo(60524, 1L, 0L);
						attacker.getClient().getSession().writeAndFlush(
								CField.UIPacket.detailShowInfo("狩獵等級範圍以外的怪物時，經驗埴和物品獲得量大幅降低.", 3, 20, 20));
					}
				}
			}
	...
	public final int killBy(MapleCharacter killer, int lastSkill) {
		...
		// 사냥코인 배율
		if (killer != null && killer.getPosition() != null && killer.getKeyValue(501661, "point") < 99999
				&& !(getId() >= 8644101 && getId() <= 8644112)) {
		...
		if (killer != null && killer.getPosition() != null && (getId() < 8644101 || getId() > 8644112)
				&& getStats().getLevel() >= killer.getLevel() - 20 && getStats().getLevel() <= killer.getLevel() + 20) {
			if (killer.getBuffedValue(80003025)) {
				...
				if (killer.getKeyValue(100722, "today") < maximum) {
					...
					if (givecoin) {
						...
						killer.getClient().getSession().writeAndFlush(SLFCGPacket.SkillfromMonsterEffect(80003025, 0,
								(getTruePosition()).x, (getTruePosition()).y));
						int nowcoin = (int) killer.getKeyValue(100722, "cnt");
						if (nowcoin == 5) {
							if (Randomizer.isSuccess(15)) {
								killer.getClient().getSession()
										.writeAndFlush(CField.enforceMsgNPC(9062453, 2000, "나는 널 그리워 할꺼야.\r\n 항상..."));
							} else if (Randomizer.isSuccess(15)) {
								killer.getClient().getSession().writeAndFlush(
										CField.enforceMsgNPC(9062453, 2000, "에르다를 조금만 더 모으면 거대한 마법의 종이 나타날 거야."));
							} else if (Randomizer.isSuccess(15)) {
								killer.getClient().getSession()
										.writeAndFlush(CField.enforceMsgNPC(9062453, 2000, "아아, 이 세계가 에르다로 가득해."));
							} else if (Randomizer.isSuccess(15)) {
								killer.getClient().getSession()
										.writeAndFlush(CField.enforceMsgNPC(9062453, 2000, "네오 캐슬을 지나~\r\n늪을 건너~"));
							} else if (Randomizer.isSuccess(15)) {
								killer.getClient().getSession().writeAndFlush(
										CField.enforceMsgNPC(9062453, 2000, "네오 캐슬에 다시 오고 싶다면 9와 4분의 3번째 에르다를 찾으렴."));
							} else {
								killer.getClient().getSession().writeAndFlush(
										CField.enforceMsgNPC(9062453, 2000, "부디 눈꽃 순록들에게\r\n종소리가 닿기를..."));
							}
						} else if (nowcoin == 10) {
							List<MapleMonster> monsters = getMap().getAllMonster();
							killer.setKeyValue(100722, "cnt", "0");
							killer.getClient().getSession().writeAndFlush(
									CField.enforceMsgNPC(9062453, 900, "에르다의 기운이 모여서\r\n거대한 #r마법의 종#k이 나타났어."));
							killer.getClient().getSession()
									.writeAndFlush(CField.enforceMsgNPC(9062453, 900, "어서 종을 울려서\r\n#b네오 스톤#k을 모아보렴."));
				...
				if (killer.getEventKillingMode()) {
					if (killer.getKeyValue(100711, "today") < 20000L) {
						if (killer.getEventMobCount() > 0) {
							...
							killer.getClient().getSession()
									.writeAndFlush(SLFCGPacket.EventSkillSetCount(80003016, killer.getEventMobCount()));
							killer.getClient().getSession().writeAndFlush(SLFCGPacket.SkillfromMonsterEffect(80003016,
									0, (getTruePosition()).x, (getTruePosition()).y));
							if (killer.getEventMobCount() <= 0) {
								killer.setEventKillingMode(false);
								killer.getClient().getSession().writeAndFlush(
										CField.enforceMsgNPC(9062453, 1300, "마법구슬의 힘으로 네오 스톤을\r\n#r모두#k 찾았어!"));
								killer.getClient().getSession().writeAndFlush(
										CField.enforceMsgNPC(9062453, 1300, "이것이 #b에르다#k로 빚어낸 마법!\r\n너무 아름다워..."));
							...
							killer.setEventKillingMode(false);
							killer.getClient().getSession()
									.writeAndFlush(SLFCGPacket.DreamBreakerMsg("오늘 획득 가능한 네오 스톤을 모두 획득햇습니다."));
							killer.getClient().getSession()
									.writeAndFlush(CField.enforceMsgNPC(9062453, 1300, "후후, 오늘은 이 정도면 충분할 것 같아. 고마워!"));
							killer.getClient().getSession().writeAndFlush(SLFCGPacket.EventSkillEnd(80003016));
					...
					if ((Randomizer.isSuccess(5) || v) && !getStats().isBoss()) {
						if (killer.getKeyValue(100711, "today") < 30000L) {
							...
							if (v) {
								killer.getClient().getSession().writeAndFlush(SLFCGPacket.SkillfromMonsterEffect(
										80003016, 1, (getTruePosition()).x, (getTruePosition()).y));
								killer.getClient().getSession().writeAndFlush(
										CField.enforceMsgNPC(9062453, 1300, "마법구슬에 #b에르다#k 급속충전된거같아! 준비해!"));
								killer.setKeyValue(100708, "fever", "100");
							} else {
								killer.getClient().getSession().writeAndFlush(SLFCGPacket.SkillfromMonsterEffect(
										80003016, 0, (getTruePosition()).x, (getTruePosition()).y));
								killer.setKeyValue(100708, "fever", (killer.getKeyValue(100708, "fever") + 5L) + "");
							}
							if (killer.getKeyValue(100708, "fever") == 25L) {
								killer.getClient().getSession().writeAndFlush(
										CField.enforceMsgNPC(9062453, 1300, "마법구슬의 #b에르다#k가 차오르기 시작했어."));
							} else if (killer.getKeyValue(100708, "fever") == 50L) {
								killer.getClient().getSession().writeAndFlush(
										CField.enforceMsgNPC(9062453, 1300, "마법구슬에 #b에르다#k가 절반정도 찬거같아. 조금더 힘내!"));
							} else if (killer.getKeyValue(100708, "fever") == 75L) {
								killer.getClient().getSession().writeAndFlush(
										CField.enforceMsgNPC(9062453, 1300, "마법구슬에 #b에르다#k가 거의 차오른거같아. 화이팅!"));
								...
								if (!killer.getEventKillingMode()) {
									...
									killer.getClient().getSession().writeAndFlush(
											SLFCGPacket.DreamBreakerMsg("르네의 마법구슬이 빛을 발하며 온 몸에 에르다의 힘이 깃듭니다"));
									killer.getClient().getSession().writeAndFlush(
											CField.enforceMsgNPC(9062453, 1300, "마법구슬이 가득 차오른 지금!\r\n#b네오 크리스탈 파워!#k"));
									killer.getClient().getSession()
											.writeAndFlush(SLFCGPacket.EventSkillEffect(80003016, 30000));
									killer.getClient().getSession()
											.writeAndFlush(SLFCGPacket.EventSkillStart(80003016, 30000));
									killer.getClient().getSession()
											.writeAndFlush(SLFCGPacket.EventSkillSetCount(80003016, count));
									if (killer.getEventSkillTimer() == null) {
										ScheduledFuture<?> qwer = Timer.ShowTimer.getInstance().schedule(() -> {
											if (killer != null) {
												killer.setEventKillingMode(false);
												killer.getClient().getSession()
														.writeAndFlush(SLFCGPacket.EventSkillEnd(80003016));
												killer.getClient().getSession().writeAndFlush(CField.enforceMsgNPC(
														9062453, 1300, "마법구슬의 힘으로 네오 스톤을\r\n#r일부#k 찾았어!"));
												killer.getClient().getSession().writeAndFlush(CField.enforceMsgNPC(
														9062453, 1300, "이것이 #b에르다#k로 빚어낸 마법!\r\n너무 아름다워..."));
												killer.getEventSkillTimer().cancel(true);
												killer.setEventSkillTimer((ScheduledFuture<?>) null);
											}
										}, 30000L);
								...
								killer.getClient().getSession()
										.writeAndFlush(SLFCGPacket.DreamBreakerMsg("에르다를 찾아 네오 스톤 1개를 찾았습니다. (구슬 게이지: "
												+ killer.getKeyValue(100708, "fever") + "%)"));
							}
						} else {
							killer.getClient().getSession().writeAndFlush(SLFCGPacket.EventSkillEnd(80003016));
							killer.getClient().getSession()
							...
							killer.cancelEffectFromBuffStat(SecondaryStat.EventSpecialSkill, 80003016);
							killer.getClient().getSession().writeAndFlush(CField.UIPacket.closeUI(1291));
							killer.dropMessage(11, "露娜的魔法寶珠已停用.");
						}
					}
					if (killer.getKeyValue(100711, "today") >= 30000L) {
						killer.getClient().getSession().writeAndFlush(SLFCGPacket.EventSkillEnd(80003016));
					...
					Timer.ShowTimer.getInstance().schedule(() -> {
						if (killer != null) {
							if (Randomizer.isSuccess(50)) {
								killer.getClient().getSession()
										.writeAndFlush(CField.enforceMsgNPC(9062453, 700, "와! 눈꽃 순록들이 달려오고 있어!"));
								killer.getClient().getSession().writeAndFlush(
										CField.enforceMsgNPC(9062453, 2000, "눈꽃 순록들이 달리는 모습!\r\n너무 아름다웠어!"));
							} else {
								killer.getClient().getSession().writeAndFlush(
										CField.enforceMsgNPC(9062453, 700, "종 소리가 울려 퍼지는 지금!\r\n눈꽃 순록 러쉬!"));
								killer.getClient().getSession().writeAndFlush(
										CField.enforceMsgNPC(9062453, 2000, "고마워.\r\n눈꽃 순록들도 즐거워 하고 있어."));
							}
						}
					}, 1500L);
				}
		...
		if (!FieldLimitType.Event.check(this.map.getFieldLimit()) && !this.map.isEliteField()
				&& !this.map.isElitebossmap() && !this.map.isElitebossrewardmap() && !this.map.isElitechmpfinal()
				&& !GameConstants.isContentsMap(getMap().getId()) && !GameConstants.보스맵(getMap().getId())
				&& !GameConstants.사냥컨텐츠맵(getMap().getId()) && !GameConstants.튜토리얼(getMap().getId())
				&& !GameConstants.로미오줄리엣(getMap().getId()) && !GameConstants.피라미드(getMap().getId())
				&& ((getStats().getLevel() >= killer.getLevel() - 20 && getStats().getLevel() <= killer.getLevel() + 20)
						|| killer.isGM())) {
			...
			if (this.map.getCustomValue0(9930005) >= 3000) {
				if ((Randomizer.nextInt(10000) < 3 && this.map.getPoloFrittoPortal() == null) || killer.isGM()) {
					...
					if (target.characterSize() == 0 || target2.characterSize() == 0) {
						MapleRandomPortal portal = new MapleRandomPortal(2, getTruePosition(), this.map.getId(),
								killer.getId(), Randomizer.nextBoolean());
				...
				if (Randomizer.nextInt(10000) < 2 && this.map.getFireWolfPortal() == null) {
					MapleRandomPortal portal = new MapleRandomPortal(3, getTruePosition(), this.map.getId(),
							killer.getId(), false);
		...
		if (killer.getMapId() == 993000500) {
			MapleMap target = ChannelServer.getInstance(killer.getClient().getChannel()).getMapFactory()
					.getMap(993000600);
		...
		if (getSeperateSoul() <= 0) {
			...
			if (oldSponge != null && oldSponge.isAlive()) {
				...
				for (MapleMapObject mon : this.map.getAllMonstersThreadsafe()) {
					...
					if (mons.isAlive() && mons.getObjectId() != oldSponge.getObjectId()
							&& mons.getStats().getLevel() > 1 && mons.getObjectId() != getObjectId()
		...
		case 8820113: {
			...
			for (MapleMonster i : mobs) {
				if (i.getId() == 8820000 || i.getId() == 8820100) {
					...
					int type = i.getId() >= 8810002 && i.getId() <= 8810009
							|| i.getId() >= 8810102 && i.getId() <= 8810109 ? -2 : -1;
				...
				if (mob.getId() == 8220102 || mob.getId() == 8220104) {
					if (map.getAllMonster().size() > 0) {
						...
						if (iterator.hasNext()) {
							...
							mob.setHp((long) ((double) (mons.getStats().getHp()
									* (long) (mob.getId() == 8220102 ? 30 : 50)) * mob.bonusHp()));
						}
					} else {
						mob.setHp((long) ((double) (mob.getStats().getHp() * (long) (mob.getId() == 8220102 ? 30 : 50))
								* mob.bonusHp()));
					}
				}
	...
	public final void sendSpawnData(MapleClient client) {
		...
		if (this.map != null && !this.stats.isEscort() && client.getPlayer() != null && client.getPlayer()
				.getTruePosition().distanceSq(getTruePosition()) <= GameConstants.maxViewRangeSq_Half()) {
	...
	public final int getBurnedBuffSize(int skillid) {
		...
		while (itr.hasNext()) {
			Pair<MonsterStatus, MonsterStatusEffect> skill = itr.next();
			if (((MonsterStatusEffect) skill.getRight()).getSkill() == skillid
					&& skill.getLeft() == MonsterStatus.MS_Burned) {
				size++;
			}
		}
		return size;
	...
	public void applyStatus(MapleClient c, List<Pair<MonsterStatus, MonsterStatusEffect>> datas,
		...
		for (Pair<MonsterStatus, MonsterStatusEffect> data : datas) {
			Ignition ig = null;
			int maxSuperPos = (((MonsterStatusEffect) data.getRight()).getSkill() == 4120011
					|| ((MonsterStatusEffect) data.getRight()).getSkill() == 4220011
					|| ((MonsterStatusEffect) data.getRight()).getSkill() == 4340012) ? 3 : effect.getDotSuperpos();
			int dotSuperpos = 0;
			...
			if (data.getLeft() == MonsterStatus.MS_Freeze) {
				re = data;
				bind = true;
				if (getBuff(MonsterStatus.MS_Freeze) != null && effect.getSourceId() != 100001283
						&& effect.getSourceId() != 101120110) {
					alreadybind = true;
					continue;
				}
			}
			CancelStatusAction action = new CancelStatusAction(this, data.getLeft(), data.getRight());
			ScheduledFuture<?> schedule = Timer.BuffTimer.getInstance().schedule(() -> action.run(),
					((MonsterStatusEffect) data.getRight()).getDuration());
			...
			if (data.getLeft() == MonsterStatus.MS_Burned) {
				...
				if ((effect.getDotSuperpos() == 0 && dotSuperpos <= 0) || dotSuperpos < maxSuperPos) {
					...
					if (c.getPlayer().getSkillLevel(2110000) > 0) {
						SecondaryStatEffect extremeMagic = SkillFactory.getSkill(2110000)
								.getEffect(c.getPlayer().getSkillLevel(2110000));
						((MonsterStatusEffect) data.getRight())
								.setDuration(((MonsterStatusEffect) data.getRight()).getDuration()
										* (100 + extremeMagic.getX()) / 100);
					}
					...
					ig = new Ignition(c.getPlayer().getId(), ((MonsterStatusEffect) data.getRight()).getSkill(),
							(value > 0L) ? value : ((MonsterStatusEffect) data.getRight()).getValue(), interval, du);
				...
				mob.applyStatus(c, MonsterStatus.MS_SeperateSoulC, data.getRight(),
						(int) ((MonsterStatusEffect) data.getRight()).getValue() + 2000, effect);
				mob.applyStatus(c, MonsterStatus.MS_Stun, data.getRight(),
						(int) ((MonsterStatusEffect) data.getRight()).getValue() + 2000, effect);
			...
			if ((!isResist() || effect.getSourceId() == 100001283 || effect.getSourceId() == 101120110)
					&& data.getLeft() == MonsterStatus.MS_Freeze) {
		...
		if (bind && effect.getSourceId() != 100001283 && effect.getSourceId() != 101120110) {
			if (alreadybind || getStats().getIgnoreMovable() > 0) {
				...
				datas.remove(re);
				c.getSession().writeAndFlush(MobPacket.monsterResist(this, c.getPlayer(),
						(int) (90L - (System.currentTimeMillis() - this.lastBindTime) / 1000L), effect.getSourceId()));
	...
	public void applyStatus(MapleClient c, MonsterStatus status, MonsterStatusEffect effect, int value,
			SecondaryStatEffect eff) {
		if (isBuffed(MonsterStatus.MS_PCounter) || isBuffed(MonsterStatus.MS_MCounter)
				|| isBuffed(MonsterStatus.MS_PImmune) || isBuffed(MonsterStatus.MS_MImmune)) {
			return;
		}
		if (effect.getSkill() != 80001227 && (status == MonsterStatus.MS_Stun || status == MonsterStatus.MS_Seal)
				&& getStats().isBoss() && !isBuffed(MonsterStatus.MS_SeperateSoulC)) {
			return;
		}
		...
		int igkey = 0;
		int dotSuperpos = 0;
		int maxSuperPos = (effect.getSkill() == 4120011 || effect.getSkill() == 4220011 || effect.getSkill() == 4340012)
				? 3
				: eff.getDotSuperpos();
		RemoveStati((MonsterStatus) null, (MonsterStatusEffect) null, false);
		if (status == MonsterStatus.MS_Burned) {
			...
			if (dotSuperpos < eff.getDotSuperpos()) {
				...
				Ignition ig = new Ignition(c.getPlayer().getId(), effect.getSkill(),
						(value3 > 0L) ? value3 : effect.getValue(), eff.getDotInterval(), effect.getDuration());
				getIgnitions().add(ig);
	...
	public final void cancelStatus(List<Pair<MonsterStatus, MonsterStatusEffect>> cancels) {
		...
		for (Pair<MonsterStatus, MonsterStatusEffect> cancel : cancels) {
			...
			if (((MonsterStatusEffect) cancel.getRight()).getSkill() == 12101024
					|| ((MonsterStatusEffect) cancel.getRight()).getSkill() == 12121002) {
	...
	public final void cancelStatus(MonsterStatus stat, MonsterStatusEffect effect, boolean autocancel) {
		...
		for (Pair<MonsterStatus, MonsterStatusEffect> ms : this.stati) {
			if (ms != null && effect != null && ms.getLeft() == stat
					&& ((MonsterStatusEffect) ms.getRight()).getSkill() == effect.getSkill()) {
				mse = ms.getRight();
				break;
			}
		}
		...
		if (!cancels.isEmpty()) {
			if (con != null) {
				this.map.broadcastMessage(con, MobPacket.cancelMonsterStatus(this, cancels, removeindie),
						getTruePosition());
			...
			if (con != null) {
				if (mse.getSkill() == 12101024 || mse.getSkill() == 12121002) {
					this.map.broadcastMessage(CField.ignitionBomb(12100029, getObjectId(), getTruePosition()));
					this.map.broadcastMessage(CField.EffectPacket.showEffect(con, 0, 12100029, 10, 0, 0, (byte) 0, true,
							getTruePosition(), null, null));
				} else if (mse.getSkill() == 11121004) {
					this.map.broadcastMessage(CField.ignitionBomb(11121013, getObjectId(), getTruePosition()));
					this.map.broadcastMessage(con, CField.EffectPacket.showEffect(con, 0, 11121013, 10, 0, 0, (byte) 0,
							true, getTruePosition(), null, null), false);
					con.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(con, 0, 11121013, 10, 0,
							0, (byte) 0, false, getTruePosition(), null, null));
				}
			}
		}
	}
		...
		public final void killedMob(MapleMap map, long baseExp, boolean mostDamage, int lastSkill) {
			...
			for (Map.Entry<MapleCharacter, OnePartyAttacker> attacker : resolveAttackers().entrySet()) {
				...
				for (MaplePartyCharacter partychar : party.getMembers()) {
					if (((MapleCharacter) attacker.getKey()).getLevel() - partychar.getLevel() <= 5
						...
						if (pchr != null && pchr.isAlive()) {
							boolean enable = true;
							int[] linkMobs = { 9010152, 9010153, 9010154, 9010155, 9010156, 9010157, 9010158, 9010159,
									9010160, 9010161, 9010162, 9010163, 9010164, 9010165, 9010166, 9010167, 9010168,
									9010169, 9010170, 9010171, 9010172, 9010173, 9010174, 9010175, 9010176, 9010177,
									9010178, 9010179, 9010180, 9010181 };
							for (int linkMob : linkMobs) {
								if (MapleMonster.this.getId() == linkMob
										&& pchr.getId() != ((MapleCharacter) attacker.getKey()).getId()) {
									enable = false;
								}
							}
							if (enable) {
				...
				for (MapleCharacter expReceiver : expApplicable) {
					...
					iexp += (int) Math.round(
							(((((MapleCharacter) attacker.getKey()).getId() == expReceiver.getId()) ? 0.6D : 0.0D)
			...
			for (Map.Entry<MapleCharacter, ExpMap> expReceiver : expMap.entrySet()) {
				...
				MapleMonster.this.giveExpToCharacter(expReceiver.getKey(), expmap.exp,
	...
	public void applyMonsterBuff(MapleMap map, List<Pair<MonsterStatus, MonsterStatusEffect>> stats,
			MobSkill mobSkill) {
		if (isBuffed(MonsterStatus.MS_PCounter) || isBuffed(MonsterStatus.MS_MCounter)
				|| isBuffed(MonsterStatus.MS_PImmune) || isBuffed(MonsterStatus.MS_MImmune)) {
			return;
		...
		for (Pair<MonsterStatus, MonsterStatusEffect> e : stats) {
			...
			int time = 0;
			if (((MonsterStatus) e.getLeft()).getFlag() == MonsterStatus.MS_ExchangeAttack.getFlag()
					|| ((MonsterStatus) e.getLeft()).getFlag() == MonsterStatus.MS_PopulatusTimer.getFlag()) {
				time = 60000;
			}
			CancelStatusAction action = new CancelStatusAction(this, e.getLeft(), e.getRight());
			ScheduledFuture<?> schedule = Timer.BuffTimer.getInstance().schedule(() -> action.run(),
					(time > 0) ? time : ((MonsterStatusEffect) e.getRight()).getDuration());
	...
	public final void RemoveStati(MonsterStatus stat, MonsterStatusEffect effect, boolean ig) {
		...
		if (ig && stat != null && effect != null) {
			while (itr.hasNext()) {
				Pair<MonsterStatus, MonsterStatusEffect> skill = itr.next();
				if (stat == skill.getLeft()
						&& ((MonsterStatusEffect) skill.getRight()).getSkill() == effect.getSkill()) {
					remove.add(skill);
					break;
				}
			}
		...
		while (itr.hasNext()) {
			Pair<MonsterStatus, MonsterStatusEffect> skill = itr.next();
			if (System.currentTimeMillis() >= ((MonsterStatusEffect) skill.getRight()).getStartTime()
					+ ((MonsterStatusEffect) skill.getRight()).getDuration()) {
			...
			for (MonsterDropEntry d : dropEntry) {
				if (d.itemId > 0 && d.questid == 0 && d.itemId / 10000 != 238
						&& Randomizer.nextInt(999999) < (int) ((10 * d.chance * chServerrate * chr.getDropMod())
								* (chr.getStat()).dropBuff / 100.0D * showdown / 100.0D)) {
					Item idrop;
					...
					this.map.spawnMobDrop(idrop, this.map.calcDropPos(getPosition(), getTruePosition()), this, chr,
							(byte) 0, 0);
					break;
				}
			}
...
//sequenceDiagram //崩溃复现路径推演
//    participant Server 
//    participant Client 
//    Server->>Client: 发送异常DEMIAN_PHASE_CHANGE包 
//    Client->>Client: 解析unk=3参数失败 
//    Client->>Client: 尝试访问非法内存地址(A5ABAFAB)
//    Client->>System: 触发内存访问违规 
//    System->>Client: 强制终止进程(SendOP -38)
	public void DemainChangePhase(MapleCharacter from) {// 戴米安转换2
		if (!this.demianChangePhase) {
			this.map.removeAllFlyingSword();
			this.map.broadcastMessage(CField.enforceMSG("戴米安獲得了完整的黑暗.", 216, 30000000));
			// this.map.broadcastMessage(MobPacket.ChangePhaseDemian(this, 79));//38
	...
	public void ResetSerenTime(boolean show) {
		...
		if (show) {
			getMap().broadcastMessage(MobPacket.BossSeren.SerenTimer(0, new int[] { 360000, this.SerenNoonTotalTime,
					this.SerenSunSetTotalTime, this.SerenMidNightSetTotalTime, this.SerenDawnSetTotalTime }));
		}
	}

	public void AddSerenTotalTimeHandler(int type, int add, int turn) {
		getMap().broadcastMessage(MobPacket.BossSeren.SerenTimer(1, new int[] { this.SerenNoonTotalTime,
				this.SerenSunSetTotalTime, this.SerenMidNightSetTotalTime, this.SerenDawnSetTotalTime, turn }));
	}
	...
	public void AddSerenTimeHandler(int type, int add) {
		...
		case 2:
			...
			for (MapleCharacter chr : getMap().getAllChracater()) {
				if (chr.isAlive() && chr.getBuffedValue(SecondaryStat.NotDamaged) == null
					...
					chr.addHP(minushp);
					chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, 0, minushp, 36, 0, 0,
		...
		nowtime = (type == 4) ? this.SerenDawnSetNowTime
				: ((type == 3) ? this.SerenMidNightSetNowTime
						: ((type == 2) ? this.SerenSunSetNowTime : this.SerenNoonNowTime));
		MapleMonster seren = null;
		int[] serens = { 8880603, 8880607, 8880609, 8880612, 8880633, 8880637, 8880639, 8880642 }; // [othello] - 익스트림
																									// 보스 세렌 페에즈 넘어가기
		for (int ids : serens) {
		...
		if (nowtime <= 0) {
			if (seren != null) {
				...
				getMap().broadcastMessage(
						MobPacket.BossSeren.SerenChangePhase("Mob/" + seren.getId() + ".img/skill3", 0, seren));
				for (MapleMonster mob : getMap().getAllMonster()) {
					if (mob.getId() == seren.getId() || mob.getId() == 8880605 || mob.getId() == 8880606
							|| mob.getId() == 8880611) {
						getMap().broadcastMessage(MobPacket.ChangePhaseDemian(mob, 79));
						getMap().killMonsterType(mob, 2);
					}
				...
				case 2:
					getMap().broadcastMessage(
							CWvsContext.serverNotice(5, "", "황혼의 불타는 듯한 석양이 회복 효율을 낮추고 지속적으로 피해를 입힙니다."));
				...
				Timer.MapTimer.getInstance().schedule(() -> {
					...
					if (nextid == 8880603 || nextid == 8880633) {
						...
						if (totalseren != null) {
							totalseren.gainShield((long) (totalseren.getStats().getHp() * bonusHp() * 15L / 100L),
									!(totalseren.getShield() > 0L), 0);
						}
					}
