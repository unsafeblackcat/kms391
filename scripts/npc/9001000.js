...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        var choose = "#fn나눔고딕#PRAY 컨텐츠 관련 NPC입니다.#fs15##k\r\n";
		choose += "#fn나눔고딕#현재 컨텐츠 시스템 개편중입니다.#fs11##k\r\n";
		choose += "\r\n\r\n"+ 연보 + "───────────────────────────#k\r\n";
		choose += "#L22#" + 집 + " #fc0xFFFFA648#길드 업무#l";choose += "#L27#" + 집 + " #fc0xFFFFA648#산책로#l"; //choose += "#L6#" + 집 + " #fc0xFFFFA648#승급 시스템#l";
		choose += "#L23#" + 집 + " #fc0xFFFFA648#핫 타임#l\r\n\r\n";
                        choose += "#L25#" + 집 + " #fc0xFF0054FF#월드 보스#l";choose += "#L24#" + 집 + " #fc0xFF0054FF#무릉 도장#l";choose += "#L28#" + 집 + " #fc0xFF0054FF#몬스터 파크#l\r\n\r\n";
		if(cm.getPlayer().getClient().getKeyValue("NEWCHAR2") == null) {
			choose += "#L3##fc0xFFF361A6#초기지원 (계정당 1회)#k#l\r\n";
		}	
		if(cm.getPlayer().getClient().getKeyValue("UnionRE2") == null) {
			choose += "#L4##fc0xFFF361A6#유니온 달성 보상 (계정당 1회)#k#l\r\n\r\n";
		}
		choose += "#fc0xFFD5D5D5#"+ 연보 + "───────────────────────────#k\r\n";
        choose += "#L1#" + 뎀스 + " #fc0xFF6799FF#일일 퀘스트#l　";
		choose += "#L5#" + 뎀스 + " #fc0xFF6799FF#요일 퀘스트#l　";
		//choose += "#L17#" + 뎀스 + " #fc0xFF6799FF#파티 퀘스트#l";
        choose += "\r\n\r\n#fc0xFFD5D5D5#"+ 연보 + "───────────────────────────#k\r\n";
		choose += "#L2#" + 뎀스 + " #fc0xFF6799FF#안개 숲 수련장#l";
		choose += "#L18#" + 컨텐츠 + " #fc0xFF6799FF#메소강화#l";
		choose += "#L21##fc0xFF6B66FF#" + 펫 + " 일반 펫 분양소#l";
        choose += "\r\n\r\n#fc0xFFD5D5D5#"+ 연보 + "───────────────────────────#k\r\n";
                        choose += "#L20#" + 집 + " #fc0xFFFFA648#기본악세제작#l\r\n\r\n";
                        choose += "#L19#" + 집 + " #fc0xFFFFA648#초월파프제작#l";choose += "#L14#" + 집 + " #fc0xFFFFA648#초월아케인제작#l\r\n\r\n";
                        choose += "#L15#" + 집 + " #fc0xFFFFA648#여명의 가엔링 제작#l";choose += "#L16#" + 집 + " #fc0xFFFFA648#제네시스제작#l\r\n\r\n";
                        choose += "#L26#" + 집 + " #fc0xFFFFA648#초월 PRAY 장비 제작#l";
      ...
      var choose = "#fs11#" + 검정 + "일일 퀘스트를 진행하시겠어요??\r\n";
			choose += "#fc0xFFD5D5D5#────────────아케인 지역────────────#k\r\n";
            choose += "#L1#" + 연파랑 + "소멸의 여로#n#l";
            choose += "#L2#" + 연파랑 + "츄츄 아일랜드#n#l";
            choose += "#L3#" + 연파랑 + "레헬른#n#l\r\n";
            choose += "#L4#" + 연파랑 + "아르카나#n#l";
			choose += "#L5#" + 연파랑 + "모라스#n#l";
            choose += "#L6#" + 연파랑 + "에스페라#n#l\r\n\r\n";
		    choose += "#fc0xFFD5D5D5#────────────어센틱 지역────────────#k\r\n";
            choose += "#L7#" + 연파랑 + "세르니움#n#l";
			choose += "#L8#" + 연파랑 + "아르크스#n#l";
            choose += "#L9#" + 연파랑 + "오디움#n#l\r\n\r\n";
			choose += "#fc0xFFD5D5D5#───────────────────────────#k\r\n";
            cm.sendSimpleS(choose, 0x04, 1540105);
        } else if (selection == 2) {
            cm.dispose();
			if(cm.getPlayer().getParty() == null) {
				cm.sendOk("파티가 아니면 입장할 수 없습니다");
				return;
			}  else if (cm.getPlayerCount(993198000) >= 1) {
            cm.sendOk("이미 누군가가 사용중 입니다.\r\n다른채널을 이용 해 주세요.");
            cm.dispose();
            return;
        } else if (!cm.isLeader()) {
            ...
            return;
			 ...
			 cm.resetMap(993198000);
             cm.warpParty(993198000);
			 cm.getClient().getSession().writeAndFlush(Packages.tools.packet.CField.DojangTraining());
			 cm.dispose();
			...
			if(cm.getPlayer().getParty() == null) {
				cm.sendOk("파티가 아니면 입장할 수 없습니다");
				return;
			}  else if (cm.getPlayerCount(910002000) >= 1) {
            cm.sendOk("이미 누군가가 사용중 입니다.\r\n다른채널을 이용 해 주세요.");
            cm.dispose();
            return;
        } else if (!cm.isLeader()) {
            ...
            return;
			 ...
			 cm.resetMap(910002000);
             cm.warpParty(910002000);
			 cm.getClient().getSession().writeAndFlush(Packages.tools.packet.CField.DojangTraining());
			 cm.dispose();
			 
		} else if (selection == 3) {
			cm.dispose();
		 cm.openNpcCustom(cm.getClient(), 1052206, "FirstAccount");
        } else if (selection == 4) {
		if(cm.getPlayer().getAllUnion() < 8000) {
			cm.sendOk("유니온이 모자란거 같아요!");
			cm.dispose();
			return;
			}
        		cm.dispose();
		 cm.openNpcCustom(cm.getClient(), 1052206, "UnionRE");
        } else if (selection == 5) {
          		cm.dispose();
		 cm.openNpcCustom(cm.getClient(), 1052206, "DailyQuest");
        } else if (selection == 6) {
            cm.dispose();
            cm.openNpc(9001027);
        } else if (selection == 7) {
	...
	cm.openNpc(9062296);
            ...
            cm.dispose();
            cm.openNpc(9062605, "여명가엔링");
        } else if (selection == 26) {
            cm.dispose();
            cm.openNpc(9062605, "초월칠흑");
        } else if (selection == 27) {
            cm.dispose();
            cm.openNpc(9062605, "2074115");
        } else if (selection == 28) {
            cm.dispose();
            cm.openNpc(9062605, "9071003");
        } else if (selection == 14) {
            cm.dispose();
            cm.openNpc(9062605, "초월아케인");
        } else if (selection == 18) {
            cm.dispose();
            cm.openNpc(9062605, "MesoEn");
        } else if (selection == 19) {
            cm.dispose();
            cm.openNpc(9062605, "MakeItem");
        } else if (selection == 20) {
            cm.dispose();
            cm.openNpc(9062605, "BaseAcc");
            ...
            cm.dispose();
            cm.openNpc(1530330, "HotTime");
        } else if (selection == 24) {
            cm.dispose();
            cm.openNpc(9310017, "9310017");
        } else if (selection == 25) { // 월보
            cm.dispose();
            cm.warp(680000700, 0);
        }
    } else if (status == 2) {
		...
		switch(selection) {
			case 1:
			   cm.dispose();
                cm.openNpc(3003756, "Today_River");
			break;
				case 2:
				   cm.dispose();
              cm.openNpc(3003756, "Today_Chewchew");
			break;
				case 3:
					   cm.dispose();
              cm.openNpc(3003756, "Today_rehelen");
			break;
				case 4:
					   cm.dispose();
              cm.openNpc(3003756, "Today_arcana");
			break;
				case 5:
			 	   cm.dispose();
              cm.openNpc(3003756, "Today_moras");
			break;
				case 6:
             	   cm.dispose();
              cm.openNpc(3003756, "Today_esfera");
			break;
			case 7:
			   cm.dispose();
              cm.openNpc(3003756, "Today_cernium");
			break;
				case 8:
				  cm.dispose();
              cm.openNpc(3003756, "Today_arcus");
			break;
				case 9:
			   cm.dispose();
              cm.openNpc(3003756, "Today_odium");
			break;
		}
        if (sel == 4) {// 칭호 승급
            if (selection == 3) {
