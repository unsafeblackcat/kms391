...
public class SummonHandler {
	...
	public static final void MoveSummon(final LittleEndianAccessor slea, final MapleCharacter chr) {
		...
		if (sum.getOwner().getId() != chr.getId() || sum.getSkillLevel() <= 0
		...
		if (res.size() > 0) {
			chr.getMap().broadcastMessage(chr, CField.SummonPacket.moveSummon(chr.getId(), sum.getObjectId(), pos, res),
					sum.getTruePosition());
		}
	}
	...
	public static final void DamageSummon(final LittleEndianAccessor slea, final MapleCharacter chr) {
		...
		if (monster != null) {
			...
			case 13111024: {
				monster.applyStatus(chr.getClient(), MonsterStatus.MS_Speed,
						new MonsterStatusEffect(13111024, (int) chr.getBuffLimit(13111024)), -100,
						chr.getBuffedEffect(SecondaryStat.IndieSummon, 13111024));
				break;
			}
			}
		}
		if ((summon.isPuppet() || summon.getSkill() == 3221014) && summon.getOwner().getId() == chr.getId()
				&& damage > 0) {
			summon.addHP(-damage);
			if (summon.getHP() <= 0) {
				remove = true;
			...
			chr.getMap().broadcastMessage(chr,
					CField.SummonPacket.damageSummon(chr.getId(), summon.getSkill(), damage, unkByte, monsterIdFrom),
	...
	// 定义一个静态方法 applyAddAttack，接收玩家对象、怪物对象和攻击技能 ID 作为参数
	public static void applyAddAttack(MapleCharacter player, MapleMonster monster, int attackSkillId) {
		// 检查玩家是否拥有技能 32141000 的增益效果，对应 "데스 VI" 技能
		if (player.getBuffedValue(32141000)) {
			// 如果攻击技能 ID 是 32141000
			if (attackSkillId == 32141000) {
				// 检查被攻击的怪物是否为 BOSS
				if (monster.getStats().isBoss()) {
					// 检查玩家的六芒星精通技能堆栈值是否小于 48
					if (player.hexaMasterySkillStack < 48) {
						// 如果小于 48，则将六芒星精通技能堆栈值增加 12
					...
					// 检查玩家的死亡击杀计数是否达到或超过 4
					if (player.deathKillCount >= 4) {
						// 再次检查玩家的六芒星精通技能堆栈值是否小于 48
						if (player.hexaMasterySkillStack < 48) {
							// 如果小于 48，则将六芒星精通技能堆栈值增加 12
							player.hexaMasterySkillStack += 12;
						}
	...
	public static void SummonAttack(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {// 处理召唤兽的攻击逻辑
		...
		if (summon.getSkill() / 1000000 != 35 && summon.getSkill() != 400011065 && summon.getSkill() != 12120013
				&& summon.getSkill() != 33101008 && summon.getSkill() != 400041038 && sse == null) {
			if (summon.getSkill() != 500001002 && summon.getSkill() != 500001003 && summon.getSkill() != 500001004) { // HEXA
																														// 솔
																														// 야누스
																														// 구현
		...
		map.broadcastMessage(chr, CField.SummonPacket.summonAttack(summon, (skillid2 != 0) ? skillid2 : skillid,
				animation, tbyte, allDamage, chr.getLevel(), pos, false), summon.getTruePosition());
		...
		for (final Pair<Integer, List<Long>> attackEntry : allDamage) {
			...
			case 152101000: {
				if (chr.getSkillLevel(152100012) > 0) {
					final SecondaryStatEffect curseMark = SkillFactory.getSkill(152000010)
							.getEffect(chr.getSkillLevel(152000010));
					final int max = (chr.getSkillLevel(152100012) > 0) ? 5
							: ((chr.getSkillLevel(152110010) > 0) ? 3 : 1);
					...
					statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_CurseMark,
							new MonsterStatusEffect(152000010, curseMark.getDuration(),
									curseMark.getY() * mob.getCustomValue0(152000010)),
							mob.getCustomValue0(152000010)));
					break;
				}
				break;
			}
			case 164121008: {
				...
				if (chr.getSkillCustomValue0(164121008) >= summonEffect.getZ()) {
					...
					if (chr.getSkillCustomValue0(164121009) < summonEffect.getX()) {
						chr.setSkillCustomInfo(164121009,
								chr.getSkillCustomValue0(164121009) + (mob.getStats().isBoss() ? 3 : 1), 0L);
						if (chr.getSkillCustomValue0(164121009) > summonEffect.getX()) {
				...
				chr.getMap().broadcastMessage(
						CField.SummonPacket.AbsorbentEdificeps(chr.getId(), obj.getObjectId(), 10, 5));
				break;
			}
			case 400011077:
			case 400011078: {
			...
			case 35111002: {
				statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_Stun,
						new MonsterStatusEffect(skillid, 5000), 1L));
				break;
			}
			case 1311019:
			case 1321024:
			...
			case 1301014: {
				statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_Stun,
						new MonsterStatusEffect(skillid, (summonEffect.getSubTime() > 0) ? summonEffect.getSubTime()
								: summonEffect.getDuration()),
						1L));
				break;
			}
			case 2111010: {
				statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_Burned,
						new MonsterStatusEffect(skillid, summonEffect.getDOTTime()),
						summonEffect.getDOT() * totDamageToOneMonster / allDamage.size() / 10000L));
			...
			case 2321003: {
				statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_ElementResetBySummon,
						new MonsterStatusEffect(skillid, (summonEffect.getSubTime() > 0) ? summonEffect.getSubTime()
								: summonEffect.getDuration()),
						(long) summonEffect.getX()));
				break;
			}
			case 3111005: {
			...
			case 3211005: {
				if (!mob.getStats().isBoss()) {
					statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_Freeze,
							new MonsterStatusEffect(skillid, 5000), (long) summonEffect.getX()));
					break;
				}
				break;
			}
				...
				if (chr.getSkillLevel(3320000) > 0) {
					...
					if (energyCharge != null) {
						...
						chr.getClient().getSession()
								.writeAndFlush((Object) CWvsContext.BuffPacket.giveBuff(statups, null, chr));
					}
					break;
				}
				break;
			...
			case 23111008: {
				if (!mob.getStats().isBoss()) {
					statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_Freeze,
							new MonsterStatusEffect(skillid, summonEffect.getSubTime()), -10L));
					break;
				}
				break;
			}
			...
			case 400021067: {
				statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_Speed,
						...
						(long) summonEffect.getV()));
				if (mob.getFreezingOverlap() < 5) {
					mob.setFreezingOverlap(
							(byte) (mob.getFreezingOverlap() + ((numAttacked == 1) ? summonEffect.getZ() : 1)));
					break;
				}
				break;
			}
			case 400021033: {
				final SecondaryStatEffect Eff = SkillFactory.getSkill(400021032)
						.getEffect(c.getPlayer().getSkillLevel(400021052));
				statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_ElementResetBySummon,
						new MonsterStatusEffect(skillid, (summonEffect.getSubTime() > 0) ? summonEffect.getSubTime()
								: summonEffect.getDuration()),
						(long) Eff.getQ2()));
			...
			case 400051023: {
				...
				statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_FixdamRBuff,
						new MonsterStatusEffect(skillid, summonEffect.getS2() * 1000), mob.getCustomValue0(400051023)));
				break;
			}
			case 400021069: {
				...
				double nowPlus = mob.getStats().isBoss() ? summonEffect.getZ()
						: ((mob.getHp() <= totDamageToOneMonster) ? 0.2 : 0.0);
				nowPlus *= 1000.0;
				plustime += (int) nowPlus;
				if (nowPlus > 0.0) {
					final MapleAtom atom = new MapleAtom(true, mob.getObjectId(), 29, true, 400021069,
							mob.getTruePosition().x, mob.getTruePosition().y);
			...
			statusz.clear();
			if (chr.getBuffedValue(SecondaryStat.BMageDeath) != null && GameConstants.isBMDarkAtackSkill(skillid)
					&& chr.getBuffedValue(SecondaryStat.AttackCountX) != null) {
			...
			case 33001016: {
				...
				chr.addHP(chr.getStat().getCurrentMaxHp()
						* SkillFactory.getSkill(skillid2).getEffect(chr.getSkillLevel(skillid2)).getQ() / 100L);
				if (mob.getAnotherByte() == 0) {
					...
					statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_JaguarBleeding,
							new MonsterStatusEffect(33000036, 15000), (long) mob.getAnotherByte()));
					break;
				}
			...
			case 33101115: {
				...
				statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_Stun,
						new MonsterStatusEffect(skillid, 3000), 1L));
				if (mob.getAnotherByte() == 0) {
					mob.setAnotherByte(1);
					statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_JaguarBleeding,
							new MonsterStatusEffect(33000036, 15000), (long) mob.getAnotherByte()));
			...
			case 33121017: {
				statusz2.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_Freeze,
						new MonsterStatusEffect(skillid2, 10000), 1L));
				statusz2.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_Smite,
						new MonsterStatusEffect(skillid2, 10000), 1L));
				if (mob.getBuff(MonsterStatus.MS_JaguarBleeding) == null && mob.getAnotherByte() > 0) {
					mob.setAnotherByte(0);
				}
			...
			if (mob.isBuffed(summonSkill.getId()) && GameConstants.isBattleMage(chr.getJob())) {
				...
				if (size < 10 && skillid != 32001014 && skillid != 32141000 && skillid != 2111010 && skillid != 32100010
						&& skillid != 32110017 && skillid != 32120019 && !summon.isNoapply()) { // 데스 VI 구현
		...
		if (!GameConstants.isCaptain(c.getPlayer().getJob()) && summon.getSkill() != 2211011
				&& summon.getSkill() != 3221014 && summon.getSkill() != 25121133 && summon.getSkill() != 400021068
				&& skillid != 400021095 && skillid != 164121008 && skillid != 36121002 && skillid != 36121013
				&& skillid != 11111029 && skillid != 11141002 && skillid != 4111007 && skillid != 14111010
				&& skillid != 5320011 && skillid != 5321004 && skillid != 5211014 && skillid != 14121003
				&& skillid != 400021071 && (skillid < 33001007 || skillid > 33001015)
				&& summonEffect.getCooldown(c.getPlayer()) > 0
				&& c.getPlayer().getCooldownLimit(summon.getSkill()) == 0L) {
			switch (skillid) {
			...
			case 400001064:
				break;
			default:
				c.getPlayer().addCooldown(summon.getSkill(), System.currentTimeMillis(),
						summonEffect.getCooldown(c.getPlayer()));
				c.getSession().writeAndFlush(
						(Object) CField.skillCooldown(summon.getSkill(), summonEffect.getCooldown(c.getPlayer())));
				break;
			}
		}
		if (SkillFactory.getSkill(skillid2) != null && summon.getSkill() != 25121133 && summon.getSkill() != 3221014) {
			final SecondaryStatEffect subSummonEffect = SkillFactory.getSkill(skillid2)
					.getEffect(chr.getSkillLevel(skillid2));
			if (subSummonEffect.getCooldown(c.getPlayer()) > 0 && c.getPlayer().getCooldownLimit(skillid2) == 0L) {
				c.getPlayer().addCooldown(skillid2, System.currentTimeMillis(),
						subSummonEffect.getCooldown(c.getPlayer()));
				c.getSession().writeAndFlush(
						(Object) CField.skillCooldown(skillid2, subSummonEffect.getCooldown(c.getPlayer())));
			}
		}
	...
	public static final void RemoveSummon(final LittleEndianAccessor slea, final MapleClient c) {
		...
		if (summon.getSkill() == 400021047 || summon.getSkill() == 400021063 || summon.getSkill() == 400031047
				|| summon.getSkill() == 400041033 || summon.getSkill() == 400041034) {
			...
			c.getPlayer().getMap().broadcastMessage(c.getPlayer(),
					CField.SummonPacket.getSummonSkillAttackEffect(summon, type, skillid, level, unk1, unk2, bullet,
							pos1, pos2, pos3, unk3, unk4, unk5, skills),
					false);
		}
		if (summon.getSkill() == 400031047 || summon.getSkill() == 400041048 || summon.getSkill() == 400021047
				|| summon.getSkill() == 400021063) {
			return;
		}
		summon.removeSummon(c.getPlayer().getMap(), false);
		if (summon.getSkill() != 35121011 && summon.getSkill() != 400051011) {
			c.getPlayer().cancelEffect(
					SkillFactory.getSkill(summon.getSkill()).getEffect(c.getPlayer().getSkillLevel(summon.getSkill())));
		}
	...
	public static final void SubSummon(final LittleEndianAccessor slea, final MapleCharacter chr) {
		...
		case 5210015: {
			...
			atoms.add(new SecondAtom(0x22, chr.getId(), mobid, 5201017, 4000, 0, 1,
					new Point(sum.getTruePosition().x, sum.getTruePosition().y), Arrays.asList(0)));
			atoms.add(new SecondAtom(0x22, chr.getId(), mobid, 5201017, 4000, 0, 1,
					new Point(sum.getTruePosition().x, sum.getTruePosition().y), Arrays.asList(0)));
		...
		case 36121014: {
			if (chr.getParty() != null) {
				for (final MaplePartyCharacter pchr : chr.getParty().getMembers()) {
					final MapleCharacter chr2 = chr.getClient().getChannelServer().getPlayerStorage()
							.getCharacterByName(pchr.getName());
					if (chr2 != null && pchr.isOnline()
							&& sum.getTruePosition().x + eff.getLt().x < chr2.getTruePosition().x
							&& sum.getTruePosition().x - eff.getLt().x > chr2.getTruePosition().x
							&& sum.getTruePosition().y + eff.getLt().y < chr2.getTruePosition().y
							&& sum.getTruePosition().y - eff.getLt().y > chr2.getTruePosition().y) {
			...
			if (sum.getTruePosition().x + eff.getLt().x < chr.getTruePosition().x
					&& sum.getTruePosition().x - eff.getLt().x > chr.getTruePosition().x
					&& sum.getTruePosition().y + eff.getLt().y < chr.getTruePosition().y
					&& sum.getTruePosition().y - eff.getLt().y > chr.getTruePosition().y) {
		...
		case 400041044: {
			...
			if (chr.getParty() != null) {
				for (final MaplePartyCharacter pchr2 : chr.getParty().getMembers()) {
					final MapleCharacter chr3 = chr.getClient().getChannelServer().getPlayerStorage()
							.getCharacterByName(pchr2.getName());
		...
		case 25121133: {
			...
			chr.getClient().getSession().writeAndFlush((Object) CField.EffectPacket.showEffect(chr, 0, 25121209, 2, 0,
					25121133, (byte) (chr.isFacingLeft() ? 1 : 0), true, null, null, null));
			chr.getMap().broadcastMessage(chr, CField.EffectPacket.showEffect(chr, 0, 25121209, 2, 0, 25121133,
					(byte) (chr.isFacingLeft() ? 1 : 0), false, null, null, null), false);
			eff.applyTo(chr);
			break;
		}
		case 400021032: {
			final SecondaryStatEffect pismaker = SkillFactory.getSkill(400021052)
					.getEffect(chr.getSkillLevel(400021032));
			if (chr.getMapId() != 450013700 && chr.getParty() != null) {
				for (final MaplePartyCharacter Pchar : chr.getParty().getMembers()) {
					final MapleCharacter Pchar2 = chr.getClient().getChannelServer().getPlayerStorage()
							.getCharacterById(Pchar.getId());
					if (Pchar2 != null) {
		...
		case 35120002: {
			...
			if (chr.getParty() != null) {
				for (final MaplePartyCharacter pc : chr.getParty().getMembers()) {
					...
					if (ch > 0) {
						final MapleCharacter player = ChannelServer.getInstance(ch).getPlayerStorage()
						...
						player.getClient().getSession().writeAndFlush(
								(Object) CField.EffectPacket.showSummonEffect(player, sum.getSkill(), true));
						player.getMap().broadcastMessage(player,
								CField.EffectPacket.showSummonEffect(player, sum.getSkill(), false), false);
					}
				}
			} else {
				chr.addHP((int) (chr.getStat().getCurrentMaxHp() * eff.getHp() / 100.0));
				chr.getClient().getSession()
						.writeAndFlush((Object) CField.EffectPacket.showSummonEffect(chr, sum.getSkill(), true));
				chr.getMap().broadcastMessage(chr, CField.EffectPacket.showSummonEffect(chr, sum.getSkill(), false),
						false);
			}
			final List<Pair<MonsterStatus, MonsterStatusEffect>> datas = new ArrayList<Pair<MonsterStatus, MonsterStatusEffect>>();
			datas.add(new Pair<MonsterStatus, MonsterStatusEffect>(MonsterStatus.MS_IndiePdr,
					new MonsterStatusEffect(sum.getSkill(), sum.getDuration(),
							-SkillFactory.getSkill(35111008).getEffect(sum.getSkillLevel()).getW())));
			datas.add(new Pair<MonsterStatus, MonsterStatusEffect>(MonsterStatus.MS_IndieMdr,
					new MonsterStatusEffect(sum.getSkill(), sum.getDuration(),
							-SkillFactory.getSkill(35111008).getEffect(sum.getSkillLevel()).getW())));
		...
		case 1301013: { // todo more
			if (chr.hasSkill(1310016)) {
				...
				if (!partyMembers.isEmpty()) {
					for (MapleCharacter partyMember : partyMembers) {
						...
						partyMember.getMap()
								.broadcastMessage(CField.SummonPacket.summonSkill(chr.getId(), sum.getObjectId(), 14));
					}
				} else {
			...
			if (bHealing.getId() == 1310016) {
				...
				chr.getClient().getSession()
						.writeAndFlush((Object) CField.EffectPacket.showSummonEffect(chr, 1310016, true));
			...
			if (bHealing.getId() == 1301013) {
				...
				if (chr.getSkillLevel(1320045) > 0) {
					healhp += (int) (chr.getStat().getCurrentMaxHp() / 100L
							* SkillFactory.getSkill(1320045).getEffect(1).getX());
				}
				chr.addHP(healhp);
				chr.getClient().getSession()
						.writeAndFlush((Object) CField.EffectPacket.showSummonEffect(chr, 1301013, true));
		...
		case 152101000: {
			...
			if (harmonyLink.getId() == 152111007 && sum.getCrystalSkills().size() >= 2
					&& sum.getCrystalSkills().get(1)) {
		...
		case 400041038: {
			...
			final List<MapleMapObject> objs = chr.getMap().getMapObjectsInRange(sum.getTruePosition(), 500000.0,
			...
			if (monsters.size() > 0) {
				...
				final MapleAtom atom = new MapleAtom(false, chr.getId(), 49, true, 400041038, sum.getTruePosition().x,
						sum.getTruePosition().y - 400);
				atom.setDwSummonObjectId(sum.getObjectId());
				int key = 0;
				for (final Integer m : monsters) {
					for (int k = 0; k < eff.getBulletCount(); ++k) {
						atom.addForceAtom(new ForceAtom(2, Randomizer.rand(40, 44), Randomizer.rand(3, 4),
								360 / (monsters.size() * 5 + 7) * key, 200));
						++key;
					}
				}
				...
				chr.getClient().getSession().writeAndFlush((Object) CField.EffectPacket.showEffect(chr, sum.getSkill(),
						sum.getSkill(), 4, 0, 0, (byte) 0, true, null, null, null));
				chr.getMap().broadcastMessage(chr, CField.EffectPacket.showEffect(chr, sum.getSkill(), sum.getSkill(),
						4, 0, 0, (byte) 0, false, null, null, null), false);
				break;
			}
			break;
		}
	...
	public static void replaceSummon(LittleEndianAccessor slea, MapleClient c) {
		...
		for (MapleSummon s : c.getPlayer().getSummons()) {
			...
			if (skillId != 400031005 || s.getSkill() < 33001007 || s.getSkill() > 33001015
					|| s.getSkill() == GameConstants
							.getSelectJaguarSkillId(GameConstants.getMountItem(33001001, c.getPlayer()))) {
				continue;
			}
			c.getPlayer().getMap().broadcastMessage(CField.SummonPacket.removeSummon(s, false));
			s.setPosition(new Point(c.getPlayer().getPosition().x + Randomizer.rand(-400, 400),
		...
		if (target != null) {
			...
			if (type > 0) {
				c.getPlayer().getMap().broadcastMessage(
						CField.SummonPacket.summonSkill(c.getPlayer().getId(), target.getObjectId(), type));
			}
	...
	public static void specialSummon5th(final LittleEndianAccessor slea, final MapleClient c) {
		...
		final MapleSummon target = chr.getMap().getSummonByOid(objectId);
		if (target != null && System.currentTimeMillis() - target.getLastAttackTime() > SkillFactory.getSkill(skillid)
				.getEffect(c.getPlayer().getSkillLevel(skillid)).getCooldown(chr)) {
