...
/*	if (mode == -1) {
		...
		if (status == 0)
			...
			qm.sendNext("Have you found a small egg in your pocket? That egg is your duty, your responsibility. Luna is hard when you're all by yourself. In times like this, there's nothing quite like having a friend that will be there for you at all times. Have you heard of a #bpet#k?\r\nPeople raise pets to ease the burden, sorrow, and loneliness, because knowing that you have someone, or something in this matter, on your side will really bring a peace of mind. But everything has consequences, and with it comes responsibility...");
		} else if (status == 2) {
			qm.sendNextPrev("Raising a pet requires a huge amount of responsibility. Remember a pet is a form of Luna, as well, so you'll need to feed it, name it, share your thoughts with it, and ultimately form a bond. That's how the owners get attached to these pets.");
		} else if (status == 3) {
			...
			qm.sendNextPrev("Rune Snail is a pet of many skills. It'll pick up items, feed you with potions, and do other things that will astound you. The downside is that since Rune Snail was born out of power of Mana, it's Lunaspan is very short. Once it turns into a doll, it'll never be able to be revived.");
		} else if (status == 5) {
