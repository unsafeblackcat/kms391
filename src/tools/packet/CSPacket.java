...
public class CSPacket {
    ...
    public static byte[] warpCS(MapleClient c) {
        ...
        mplew.write(false);
        mplew.write(false);
        // start of sub_144B194B0
        mplew.writeShort(cs.getAllModInfo().size());
        for (CashItemInfo.CashModInfo info : cs.getAllModInfo()) {

            addModCashItemInfo(mplew, info);
        }
        mplew.writeShort(0);
        int i;
        for (i = 0; i < 0; i++) {
        ...
        mplew.writeInt(cs.getRandomItemInfo().size());
        for (Map.Entry<Integer, List<Integer>> items : cs.getRandomItemInfo().entrySet()) {
            ...
            if (((Integer) items.getKey()).intValue() / 1000 == 5533) {
                mplew.writeInt(((List) items.getValue()).size());
        ...
        // end of sub_144B194B0

        // start of sub_14197F0E0
        mplew.writeInt(cs.getBestItems().size());
        for (Pair<Integer, Integer> item : cs.getBestItems()) {
            mplew.write(((Integer) item.left).intValue());
            mplew.writeInt(((Integer) item.right).intValue());
        }
        // end of sub_14197F0E0

        mplew.writeInt(0); // v1.2.391+
        mplew.writeInt(0); // v1.2.391+
        mplew.writeInt(0); // v1.2.391+

        // start of sub_14197F3F0
        mplew.writeInt(0);
        for (i = 0; i < 0; i++) {
            mplew.write(3);
            mplew.writeInt(110000030);
        }
        // end of sub_14197F3F0

        // start of sub_14197F580
        String[][] strs = {
        ...
        mplew.writeInt(strs.length);
        for (String[] str : strs) {
            mplew.writeMapleAsciiString(str[0]);
            mplew.writeMapleAsciiString(str[1]);
        }
        // end of sub_14197F580
    ...
    public static byte[] getCSInventory(MapleClient c) {
        ...
        mplew.writeShort(0);
        mplew.writeShort(c.loadCharacters(c.getWorld()).size());
        return mplew.getPacket();
    }
    ...
    public static byte[] showBoughtCSItem(Item item, MapleClient c, int sn, int paybackRate, int discountRate) {
        ...
        mplew.writeInt(0); // hasRegisterDate
        mplew.write(0); // receiveBonus
        return mplew.getPacket();
    }
    ...
    public static byte[] showsendNotes(MapleCharacter chr, ResultSet notes, int count) throws SQLException {
        ...
        for (int i = 0; i < count; i++) {
            ...
            mplew.writeLong(notes.getLong("timestamp"));
            notes.next();
        }
    ...
    public static byte[] showNotes(ResultSet notes, int count) throws SQLException {
        ...
        for (int i = 0; i < count; i++) {
            ...
            //mplew.writeInt(notes.getInt("senderid"));
            //mplew.writeInt(0);
            //mplew.writeString(notes.getString("message"));
            //mplew.writeInt(notes.getInt("senderid"));
            notes.next();
        }
    ...
    public static final void addCashItemInfo(MaplePacketLittleEndianWriter mplew, Item item, MapleClient c, int CommodityID, int nPaybackRate, int nDisCountRate) {
        mplew.writeLong(item.getUniqueId());
        mplew.writeInt(c.getAccID());
        mplew.writeInt(c.getPlayer().getId());
        mplew.writeInt(item.getItemId());
        mplew.writeInt(CommodityID);
        mplew.writeShort(item.getQuantity());
        mplew.writeAsciiString(item.getGiftFrom(), 13);
        mplew.writeLong(item.getExpiration() >= 0 ? item.getExpiration() : -1L);
    ...
    public static byte[] coodinationResult(int type, int msg, MapleCharacter chr) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendPacketOpcode.COODINATION_RESULT.getValue());
        mplew.writeInt(type);
        mplew.writeInt(msg);
        mplew.writeInt(chr.getCoodination().size());
