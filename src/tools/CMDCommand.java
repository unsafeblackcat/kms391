					...
					for (ChannelServer cserv : ChannelServer.getAllInstances()) {
						...
						if (player != null) {
							...
							player.getClient().getSession()
									.writeAndFlush(CWvsContext.getTopMsg("[通知]該玩家已達到GM " + command[2] + "級."));
							System.out.println("[通知] " + command[1] + " 已將玩家設定為GM " + command[2] + "級");
							player.setGMLevel(number);
							a = 1;
							continue;
						}
						if (player == null && a == 0) {
							System.out.println("[通知] " + command[1] + " 找不到玩家.");
							a = 1;
						}
					}
				case "패킷":
