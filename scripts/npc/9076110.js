...
function spawnMonster () {
    for(var i = 0; i < Mmobx.length; i++){
        for(var j = 0; j < Mmoby.length-1; j++){
            cm.getPlayer().getMap().spawnMonsterOnGroundBelow(Packages.server.Luna.MapleLunaFactory.getMonster(Mmobid[0]), new java.awt.Point(Mmobx[i], Mmoby[j]));
            cm.getPlayer().getMap().spawnMonsterOnGroundBelow(Packages.server.Luna.MapleLunaFactory.getMonster(Mmobid[1]), new java.awt.Point(Mmobx[i], Mmoby[j]));
        }
    }
}