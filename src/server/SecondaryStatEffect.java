...
public class SecondaryStatEffect implements Serializable {
	...
	public static int parseEval(String data, int level) {
		...
		if (dddd.substring(0, 1).equals("-")) {
			dddd = dddd.substring(1, 2).equals("u") || dddd.substring(1, 2).equals("d")
					? "n(" + dddd.substring(1, dddd.length()) + ")"
					: "n" + dddd.substring(1, dddd.length());
	...
	private static String calcLog(String path, String data, int level) { // Skill.wz 데이터 로그 함수 자동 처리 구현
		...
		while ((topIndex + 4) <= (data.length() - 1)) {
			if (data.substring(bottomIndex, topIndex).equals("log")) {
				double base = SecondaryStatEffect.baseLog(
						Double.parseDouble(
								data.substring((topIndex + 2), level >= 10 ? (topIndex + 4) : (topIndex + 3))),
						Double.parseDouble(data.substring(topIndex, (topIndex + 2))));
	...
	private static SecondaryStatEffect loadFromData(MapleData source, int sourceid, boolean skill, boolean overTime,
		...
		try {
			...
			ret.dotHealHPPerSecondR = (short) SecondaryStatEffect.parseEval("dotHealHPPerSecondR", source, 0, variables,
					level);
			...
			ret.indieIgnoreMobpdpR = (short) SecondaryStatEffect.parseEval("indieIgnoreMobpdpR", source, 0, variables,
					level);
			...
			ret.indieCooltimeReduce = (short) SecondaryStatEffect.parseEval("indieCooltimeReduce", source, 0, variables,
					level);
			...
			if (ret.skill) {
				...
				case 154121042:
				case 155121042: {
					ret.statups.put(SecondaryStat.IndieDamR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieDamR), ret.duration));
					break;
				}

				case 1101013: {
					ret.duration = 0;// 设置持续时间为0秒（表示永久生效或由其他机制控制持续时间）
					ret.statups.put(SecondaryStat.ComboCounter, new Pair<Integer, Integer>(1, ret.duration)); // 2.
																												// 添加连击计数器状态，初始层数为1，持续时间继承ret.duration（即0）

					break; // 跳出条件判断
				}
				case 1121054: { // 발할라
					ret.statups.put(SecondaryStat.IndiePad,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indiePad), ret.duration)); // 添加独立攻击力增益（IndiePad）
					ret.statups.put(SecondaryStat.IndieCr,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieCr), ret.duration)); // 添加独立暴击率增益（IndieCr）
					ret.statups.put(SecondaryStat.Stance, new Pair<Integer, Integer>(100, ret.duration)); // 添加姿态增益（100%伤害减免效果）
					ret.statups.put(SecondaryStat.Asr, new Pair<Integer, Integer>(ret.x, ret.duration)); // 添加异常状态抗性增益（Asr）
					ret.statups.put(SecondaryStat.Ter, new Pair<Integer, Integer>(ret.y, ret.duration)); // 添加属性抗性增益（Ter）
					// ret.statups.put(SecondaryStat.ComboCounter, new Pair<Integer, Integer>(11,
					// -1)); // 這段代碼會影響1101013的BUFF狀態 所以禁用
					break;
				}
				...
				case 1101006: {
					ret.statups.put(SecondaryStat.IndiePad,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indiePad), ret.duration));
				...
				case 1210016: {
					...
					ret.statups.put(SecondaryStat.BlessingArmorIncPad,
							new Pair<Integer, Integer>(Integer.valueOf(ret.epad), ret.duration));
					break;
				}
				case 1211010: {
				...
				case 1221015: {
					ret.statups.put(SecondaryStat.IndiePmdR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indiePmdR), ret.duration));
					break;
				}
				case 1221016: {
				...
				case 1301006: {
					ret.statups.put(SecondaryStat.Pdd,
							new Pair<Integer, Integer>(Integer.valueOf(ret.pdd), ret.duration));
					break;
				}
				case 1301007: {
				...
				case 12101005:
				case 22141016: {
					// ret.statups.put(SecondaryStat.ElementalReset, new Pair<Integer,
					// Integer>(ret.x, ret.duration));
					break;
				}
				case 2111007:
				case 2211007:
				...
				case 2101001: {
					ret.statups.put(SecondaryStat.IndieMad,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieMad), ret.duration));
					break;
				}
				case 2101010: {
					ret.duration = 0;
				...
				case 27111008: {
					ret.duration = 0;
					ret.statups.put(SecondaryStat.TeleportMasteryRange,
							new Pair<Integer, Integer>(ret.x, ret.duration));
					break;
				}
				case 2321054: {
					ret.duration = 0;
				...
				case 33121004:
				case 400001002: {
					ret.statups.put(SecondaryStat.SharpEyes,
							new Pair<Integer, Integer>((ret.x << 8) + ret.y, ret.duration));
					break;
				}
				case 3101004: {
					ret.statups.put(SecondaryStat.SoulArrow,
							new Pair<Integer, Integer>(Integer.valueOf(ret.epad), ret.duration));
				...
				case 3120022: { // 어드밴스드 퀴버
					ret.statups.put(SecondaryStat.AdvancedQuiver,
							new Pair<Integer, Integer>(Integer.valueOf(ret.level), ret.duration));
					break;
				}
				case 3121007: {
					ret.statups.put(SecondaryStat.IndieDex,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieDEX), ret.duration));
				...
				case 3211011: { // 값
					ret.statups.put(SecondaryStat.Asr,
							new Pair<Integer, Integer>(Integer.valueOf(ret.asrR), ret.duration));
					break;
				}
				case 3211012: {
					ret.duration = 0;
				...
				/*
				 * case 3210016: case 3220021: { //인핸스 애로우
				 * ret.statups.put(SecondaryStat.EnhanceArrow, new Pair<Integer, Integer>(ret.x,
				 * ret.duration)); break; }
				 */
				case 3221006: {
					ret.statups.put(SecondaryStat.IndieDex,
				...
				case 3321038: {
					ret.duration = 0;
					ret.statups.put(SecondaryStat.IndieDamageReduce,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieDamReduceR), ret.duration));
					break;
				}
				case 3221054: { // 불스아이
					ret.statups.put(SecondaryStat.IgnoreTargetDEF, new Pair<Integer, Integer>(0, ret.duration));
					ret.statups.put(SecondaryStat.BullsEye,
							new Pair<Integer, Integer>((ret.x << 8) + ret.y, ret.duration));
					ret.statups.put(SecondaryStat.IndieIgnoreMobPdpR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieIgnoreMobpdpR), ret.duration));
					ret.statups.put(SecondaryStat.IndieDamR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieDamR), ret.duration));
					break;
				}
				...
				case 4001005:
				case 14001022: {
					ret.statups.put(SecondaryStat.Speed,
							new Pair<Integer, Integer>(Integer.valueOf(ret.speed), ret.duration));
					ret.statups.put(SecondaryStat.Jump,
							new Pair<Integer, Integer>(Integer.valueOf(ret.jump), ret.duration));
					break;
				}
				case 4111002:
				case 0x404140:
				...
				/*
				 * case 5100015: case 5110014: case 5120018: { ret.duration = 0;
				 * ret.statups.put(SecondaryStat.EnergyCharged, new Pair<Integer, Integer>(1,
				 * ret.duration)); break; }
				 */
				case 5121009: {
				...
				case 5121015: {
					ret.statups.put(SecondaryStat.IndiePadR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indiePadR), ret.duration));
					break;
				}
				case 5221018: {
					ret.statups.put(SecondaryStat.IndiePadR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indiePadR), ret.duration));
					ret.statups.put(SecondaryStat.IndieStance,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieStance), ret.duration));
					ret.statups.put(SecondaryStat.Eva, new Pair<Integer, Integer>(ret.x, ret.duration));
					ret.statups.put(SecondaryStat.IndieAsrR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieAsrR), ret.duration));
					ret.statups.put(SecondaryStat.IndieTerR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieTerR), ret.duration));
					break;
				}
				case 5301003:
				case 5320008: {
					ret.statups.put(SecondaryStat.IndieHp,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieMhp), ret.duration));
					ret.statups.put(SecondaryStat.IndieMp,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieMmp), ret.duration));
					ret.statups.put(SecondaryStat.IndieJump,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieJump), ret.duration));
					ret.statups.put(SecondaryStat.IndieSpeed,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieSpeed), ret.duration));
					ret.statups.put(SecondaryStat.IndieAllStat,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieAllStat), ret.duration));
					break;
				}
				case 5321010: {
				...
				case 11121054: {
					...
					ret.statups.put(SecondaryStat.IndieIgnoreMobPdpR,
							new Pair<>((int) ret.indieIgnoreMobpdpR, ret.duration));
				...
				case 13001022: {
					...
					ret.statups.put(SecondaryStat.IndieDamR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieDamR), 0));
					break;
				}
				case 13101024: {
				...
				case 13121004: {
					ret.statups.put(SecondaryStat.IndieEvaR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.prop), ret.duration));
					break;
				}
				case 13121017:
				case 13121054: {
				...
				case 15121054: {
					ret.statups.put(SecondaryStat.StrikerHyperElectric,
							new Pair<Integer, Integer>(ret.x, ret.duration));
					break;
				}
				case 21001003: {
				...
				case 21001008: {
					ret.duration = 0;
					ret.statups.put(SecondaryStat.BodyPressure,
							new Pair<Integer, Integer>(Integer.valueOf(ret.damage), ret.duration));
					break;
				}
				case 21101005: {
					ret.duration = 0;
				...
				case 22171073: {
					ret.statups.put(SecondaryStat.EnhancedMad,
							new Pair<Integer, Integer>(Integer.valueOf(ret.emad), ret.duration));
					ret.statups.put(SecondaryStat.EnhancedPdd,
							new Pair<Integer, Integer>(Integer.valueOf(ret.epdd), ret.duration));
					break;
				}
				case 23121004: {
					ret.statups.put(SecondaryStat.IndiePadR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indiePadR), ret.duration));
					ret.statups.put(SecondaryStat.EnhancedMaxHp,
							new Pair<Integer, Integer>(Integer.valueOf(ret.emhp), ret.duration));
					break;
				}
				case 23121054: {
					ret.statups.put(SecondaryStat.IndiePad,
				...
				case 27111005: {
					ret.statups.put(SecondaryStat.IndiePdd,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indiePdd), ret.duration));
					break;
				}
				case 27111006: {
					ret.statups.put(SecondaryStat.EnhancedMad,
				...
				case 31211004: {
					ret.statups.put(SecondaryStat.IndieHpR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.getIndieMhpR()), ret.duration));
				...
				case 32121056: {
					ret.statups.put(SecondaryStat.AttackCountX,
							new Pair<Integer, Integer>(Integer.valueOf(ret.attackCount), ret.duration));
					break;
				}
				case 33001007:
				case 33001008:
				...
				case 33001015: {
					ret.duration = 0;
					ret.statups.put(SecondaryStat.JaguarSummoned,
							new Pair<Integer, Integer>((ret.criticaldamage << 8) + ret.asrR, ret.duration));
					break;
				}
				case 33101003: {
					ret.statups.put(SecondaryStat.SoulArrow, new Pair<Integer, Integer>(1, ret.duration));
				...
				case 35001002: {
					ret.duration = 0;
					ret.statups.put(SecondaryStat.EnhancedPad,
							new Pair<Integer, Integer>(Integer.valueOf(ret.epad), ret.duration));
					ret.statups.put(SecondaryStat.EnhancedPdd,
							new Pair<Integer, Integer>(Integer.valueOf(ret.epdd), ret.duration));
				...
				case 35111003: {
					...
					ret.statups.put(SecondaryStat.EnhancedPdd,
							new Pair<Integer, Integer>(Integer.valueOf(ret.epdd), ret.duration));
					// ret.statups.put(SecondaryStat.CriticalIncrease, new Pair<Integer,
					// Integer>(ret.cr, ret.duration));
					ret.statups.put(SecondaryStat.Mechanic, new Pair<Integer, Integer>(30, ret.duration));
				...
				case 36101003: {
					ret.statups.put(SecondaryStat.IndieHpR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.getIndieMhpR()), ret.duration));
					ret.statups.put(SecondaryStat.IndieMpR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.getIndieMhpR()), ret.duration));
					break;
				}
				case 36111004: {
				...
				case 36121003: {
					ret.statups.put(SecondaryStat.IndieBDR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieBDR), ret.duration));
					ret.statups.put(SecondaryStat.IndiePmdR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indiePmdR), ret.duration));
					break;
				}
				...
				case 51120003: {
					ret.statups.put(SecondaryStat.DamageDecreaseWithHP,
							new Pair<Integer, Integer>(ret.y, ret.duration));
					break;
				}
				case 51121006: {
					ret.duration = 0;
				...
				case 61121217: {
					ret.duration = 0;
					ret.statups.put(SecondaryStat.StopForceAtominfo,
							new Pair<Integer, Integer>(ret.cooltime / 1000, ret.duration));
					break;
				}
				case 61111003: {
					ret.statups.put(SecondaryStat.Asr,
							new Pair<Integer, Integer>(Integer.valueOf(ret.asrR), ret.duration));
					ret.statups.put(SecondaryStat.Ter,
							new Pair<Integer, Integer>(Integer.valueOf(ret.terR), ret.duration));
					break;
				}
				case 61111008:
				case 61120008:
				case 61121053: {
					...
					// ret.statups.put(SecondaryStat.Stance, new Pair<Integer, Integer>(100,
					// ret.duration));
					ret.statups.put(SecondaryStat.Speed,
							new Pair<Integer, Integer>(Integer.valueOf(ret.speed), ret.duration));
					ret.statups.put(SecondaryStat.Jump,
				...
				case 64121054: {
					ret.statups.put(SecondaryStat.IndieDamR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieDamR), ret.duration));
					ret.statups.put(SecondaryStat.IndieCr,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieCr), ret.duration));
					break;
				}
				case 60011219: {
					ret.statups.put(SecondaryStat.IndieDamR,
				...
				case 80001455: {
					ret.statups.put(SecondaryStat.IndiePadR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indiePadR), ret.duration));
					ret.statups.put(SecondaryStat.IndieMadR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieMadR), ret.duration));
					break;
				}
				case 80001456: {
					ret.statups.put(SecondaryStat.SetBaseDamageByBuff,
							new Pair<Integer, Integer>(2000000, ret.duration));
					break;
				}
				case 80001457: {
				...
				case 80001460: {
					ret.statups.put(SecondaryStat.IndieHpR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieMhpR), ret.duration));
					break;
				}
				case 80001461: {
				...
				case 91001025: {
					ret.statups.put(SecondaryStat.IndieCD,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieCD), ret.duration));
					break;
				}
				case 131001000:
				case 131001001:
				...
				case 131001206: {
					ret.statups.put(SecondaryStat.IndieExp, new Pair<Integer, Integer>(50, ret.duration));
					ret.statups.put(SecondaryStat.DotHealHPPerSecond,
							new Pair<Integer, Integer>(Integer.valueOf(ret.dotHealHPPerSecondR), ret.duration));
					break;
				}
				case 131001306: {
					ret.statups.put(SecondaryStat.IndieExp, new Pair<Integer, Integer>(50, ret.duration));
				...
				case 142001003: {
					ret.statups.put(SecondaryStat.IndieBooster,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieBooster), ret.duration));
					break;
				}
				case 142001007: {
					ret.duration = 0;
					ret.statups.put(SecondaryStat.KinesisPsychicEnergeShield,
							new Pair<Integer, Integer>(1, ret.duration));
					break;
				}
				case 142111010: {
				...
				case 151111005: {
					ret.statups.put(SecondaryStat.Novility,
							new Pair<Integer, Integer>(Integer.valueOf(ret.level), ret.duration));
					break;
				}
				case 151121001: {
				...
				case 400001025: {
					ret.statups.put(SecondaryStat.IndieReduceCooltime,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieCooltimeReduce), ret.duration));
					ret.statups.put(SecondaryStat.FreudsProtection,
							new Pair<Integer, Integer>(sourceid - 400001024, ret.duration));
					break;
				}
				case 400001026: {
					ret.statups.put(SecondaryStat.IndieReduceCooltime,
				...
				case 400001037: {
					ret.statups.put(SecondaryStat.MagicCircuitFullDrive,
							new Pair<Integer, Integer>(ret.y, ret.duration));
					break;
				}
				case 400001043: {
				...
				/*
				 * case 400011000: { ret.statups.put(SecondaryStat.IndiePmdR, new Pair<Integer,
				 * Integer>(Integer.valueOf(ret.indiePmdR), ret.duration));
				 * ret.statups.put(SecondaryStat.IndieIgnoreMobPdpR, new Pair<Integer,
				 * Integer>(Integer.valueOf(ret.indieIgnoreMobpdpR), ret.duration));
				 * ret.statups.put(SecondaryStat.AuraWeapon, new Pair<Integer, Integer>(ret.z,
				 * ret.duration)); break; }
				 */
				case 400011015: {
				...
				case 400011017: {
					ret.statups.put(SecondaryStat.BonusAttack,
							new Pair<Integer, Integer>(Integer.valueOf(ret.level), ret.duration));
					break;
				}
				case 400011039: {
				...
				case 400011055: {
					ret.statups.put(SecondaryStat.Ellision,
							new Pair<Integer, Integer>(Integer.valueOf(ret.level), ret.duration));
					break;
				}
				case 400011066: {
				...
				case 400011073: {
					ret.statups.put(SecondaryStat.ComboInstict,
							new Pair<Integer, Integer>(Integer.valueOf(ret.level), ret.duration));
					break;
				}
				case 400011109: {
				...
				case 400021003: {
					ret.statups.put(SecondaryStat.Pray,
							new Pair<Integer, Integer>(Integer.valueOf(ret.level), ret.duration));
					break;
				}
				case 400021060: {
				...
				case 400031015: {
					ret.statups.put(SecondaryStat.SplitArrow,
							new Pair<Integer, Integer>(Integer.valueOf(ret.level), ret.duration));
					break;
				}
				case 400031017: {
				...
				case 400041001: {
					ret.statups.put(SecondaryStat.SpreadThrow,
							new Pair<Integer, Integer>(Integer.valueOf(ret.level), ret.duration));
					break;
				}
				case 400041029: {
				...
				case 400041035: {
					ret.statups.put(SecondaryStat.ChainArtsFury,
							new Pair<Integer, Integer>(Integer.valueOf(ret.level), ret.duration));
					break;
				}
				case 400041052: {
				...
				case 400051006: {
					ret.statups.put(SecondaryStat.BulletParty,
							new Pair<Integer, Integer>(Integer.valueOf(ret.level), ret.duration));
					break;
				}
				case 400051007: {
					ret.statups.put(SecondaryStat.IndiePmdR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indiePmdR), ret.duration));
					ret.statups.put(SecondaryStat.Striker1st,
							new Pair<Integer, Integer>(Integer.valueOf(ret.level), ret.duration));
					break;
				}
				case 400051009: {
				...
				case 400051015: {
					ret.duration = 0;
					ret.statups.put(SecondaryStat.SerpentScrew,
							new Pair<Integer, Integer>(ret.damage / 10, ret.duration));
					break;
				}
				case 400051018: {
					ret.statups.put(SecondaryStat.Spotlight,
							new Pair<Integer, Integer>(Integer.valueOf(ret.level), ret.duration));
					break;
				}
				case 400051033: {
				...
				case 400007010: {
					ret.statups.put(SecondaryStat.IndieDamR,
							new Pair<Integer, Integer>(Integer.valueOf(ret.indieDamR), ret.duration * 1000));
					break;
				}
				case 400007003: {
					ret.statups.put(SecondaryStat.IndieCr,
				...
				case 400007007:
				case 400007011: {
					ret.statups.put(SecondaryStat.IndieExp,
							new Pair<Integer, Integer>(Integer.valueOf(ret.IndieExp), ret.duration));
					break;
				}
				case 400007006: {
					ret.statups.put(SecondaryStat.IndieNotDamaged, new Pair<Integer, Integer>(1, ret.duration));
				...
				case 2003576:
				case 2003591: {
					ret.statups.put(SecondaryStat.Inflation,
							new Pair<Integer, Integer>(Integer.valueOf(ret.inflation), ret.duration));
					break;
				}
				case 2450038: {
	...
	public final boolean applyTo(MapleCharacter chr) {
		return this.applyTo(chr, chr, true, chr.getTruePosition(), this.duration, (byte) (chr.isFacingLeft() ? 1 : 0),
				false);
	}
	...
	public final boolean applyToBuff(MapleCharacter chr, int duration) {
		return this.applyTo(chr, chr, true, chr.getTruePosition(), duration, (byte) (chr.isFacingLeft() ? 1 : 0),
				false);

	}

	public final boolean applyTo(MapleCharacter applyfrom, MapleCharacter applyto) {
		return this.applyTo(applyfrom, applyto, true, applyto.getTruePosition(), this.duration,
				(byte) (applyfrom.isFacingLeft() ? 1 : 0), false);
	}

	public final boolean applyTo(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary) {
		return this.applyTo(applyfrom, applyto, primary, applyto.getTruePosition(), this.duration,
				(byte) (applyfrom.isFacingLeft() ? 1 : 0), false);
	}

	public final boolean applyTo(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary, int duration) {
		return this.applyTo(applyfrom, applyto, primary, applyto.getTruePosition(), duration,
				(byte) (applyfrom.isFacingLeft() ? 1 : 0), false);
	}

	public final boolean applyTo(MapleCharacter chr, boolean primary) {
		return this.applyTo(chr, chr, primary, chr.getTruePosition(), this.duration,
				(byte) (chr.isFacingLeft() ? 1 : 0), false);
	}

	public final boolean applyTo(MapleCharacter chr, boolean primary, boolean showEffect) {
		return this.applyTo(chr, chr, primary, chr.getTruePosition(), this.duration,
				(byte) (chr.isFacingLeft() ? 1 : 0), showEffect);
	}
	...
	public final boolean applyTo(MapleCharacter chr, boolean primary, int duration) {
		return this.applyTo(chr, chr, primary, chr.getTruePosition(), duration, (byte) (chr.isFacingLeft() ? 1 : 0),
				false);
	}
	...
	public final boolean applyTo(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary, Point pos,
			int localDuration, byte rltype, boolean showEffect) {
		block196: {
			block197: {
				block195: {
					SummonMovementType summonMovementType;
					if (applyfrom.getMapId() == ServerConstants.warpMap && this.skill && !applyfrom.isGM()
							&& (this.getSummonMovementType() != null || this.isMist())) {
					...
					if (!(this.sourceid != 33101008
							|| applyfrom.getBuffedValue(SecondaryStat.IndieSummon) == null && applyfrom.canSummon())) {
						applyfrom.getClient().getSession().writeAndFlush((Object) CWvsContext.enableActions(applyto));
						return false;
					}
					...
					if (applyto.getSkillLevel(4110012) > 0 && localDuration < 0
							&& applyto.getSkillCustomValue0(4110012) > 0L) {
					...
					if (primary && this.itemConNo != 0 && !applyto.inPVP()) {
						...
						MapleInventoryManipulator.removeById(applyto.getClient(),
								GameConstants.getInventoryType(this.itemCon), this.itemCon, this.itemConNo, false,
								true);
					}
					if (this.isResurrection() && applyto.getId() != applyfrom.getId() && primary) {
						...
						SkillFactory.getSkill(2321006).getEffect(applyfrom.getSkillLevel(2321006)).applyTo(applyfrom,
								applyto, false);
						SkillFactory.getSkill(2321006).getEffect(applyfrom.getSkillLevel(2321006)).applyTo(applyfrom,
								false);
					}
					...
					if (primary || applyfrom.getId() != applyto.getId()) {
						applyto.getClient().getSession().writeAndFlush(
								(Object) CWvsContext.updatePlayerStats(hpmpupdate, !this.skill, false, applyto, false));
					}
					...
					if (this.expinc != 0) {
						...
						applyto.getClient().getSession()
								.writeAndFlush((Object) CField.EffectPacket.showNormalEffect(applyto, 24, true));
						...
						applyto.changeSingleSkillLevel(SkillFactory.getCraft(this.recipe), Integer.MAX_VALUE,
								this.recipeUseCount,
								this.recipeValidDay > 0
										? System.currentTimeMillis()
												+ (long) this.recipeValidDay * 24L * 60L * 60L * 1000L
										: -1L);
					}
					...
					if (this.sourceid == 2121003 && !applyto.getPosionNovas().isEmpty()) {
						applyto.getClient().getSession()
								.writeAndFlush((Object) CField.poisonNova(applyto, applyto.getPosionNovas()));
					...
					} else if ((this.sourceid == 14001027 || this.sourceid == 14121016 || this.sourceid == 14141004
							|| this.sourceid == 14141012) && primary) {
					...
					if ((summonMovementType = this.getSummonMovementType()) != null
							&& ((this.sourceid == 400021071 || this.sourceid == 35111008 || this.sourceid == 35120002
									|| this.sourceid == 35111002 || this.sourceid == 400021092) && primary
									|| this.sourceid != 400021071 && this.sourceid != 35111008
											&& this.sourceid != 35120002 && this.sourceid != 400021092)) {

						// 检查技能源ID是否为400011001（可能是某个特定技能或召唤物）
						if (this.sourceid == 400011001) {
							// 检查角色是否拥有技能ID为1101013的增益效果（可能是连击强化类技能）
							if (applyto.getBuffedValue(1101013)) {
								// 创建一个连击计数器状态，设置连击层数为11（可能是最大层数），持续时间为-1（永久或特殊持续逻辑）
								statups.put(SecondaryStat.ComboCounter, new Pair<Integer, Integer>(11, -1));

								// 向客户端发送状态更新包，通知客户端应用新的连击状态
								applyto.getClient().getSession().writeAndFlush(
										(Object) CWvsContext.BuffPacket.giveBuff(statups, null, applyto));
							}

							// 获取角色召唤的ID为400011001的召唤物对象（可能是技能触发的召唤物）
							...
							if (summon != null) {
								...
								applyto.getMap().broadcastMessage(
										CField.SummonPacket.spawnSummon(summon, true, summon.getDuration()));
								applyto.getClient().getSession()
										.writeAndFlush((Object) CWvsContext.enableActions(applyto, true, false));
								return true;
							}
						...
						} else if (this.sourceid != 35111002 && this.sourceid != 400021047 && this.sourceid != 400011065
								&& this.sourceid != 14000027 && this.sourceid != 14120017 && this.sourceid != 14100027
								&& this.sourceid != 0xD74D4D && this.sourceid != 14120008 && this.sourceid != 151100002
								&& primary) {
							if (applyto.getBuffedValue(this.sourceid)) {
								boolean cancels = true;
								if (applyto.getBuffedEffect(SecondaryStat.SoulMP) != null && applyto
										.getBuffedEffect(SecondaryStat.SoulMP).getSourceId() == this.sourceid) {
									cancels = false;
								}
								if (this.sourceid == 500001002 || this.sourceid == 500001003
										|| this.sourceid == 500001004) { // HEXA 솔 야누스 구현
						...
						if (spawn) {
							...
							if (this.sourceid == 500001002 || this.sourceid == 500001003
								...
								if (applyfrom.getSummons(summId).size() >= maxCount
										&& !applyfrom.skillisCooling(500001002)) {
									applyfrom.getClient().getSession().writeAndFlush(
											CField.skillCooldown(500001002, this.getCooldown(applyfrom)));
									applyfrom.addCooldown(500001002, System.currentTimeMillis(),
											this.getCooldown(applyfrom));
								}
							}
							...
							if (this.sourceid == 3111002
									? applyfrom.getTotalSkillLevel(elite = SkillFactory.getSkill(3120012)) > 0
									: this.sourceid == 3211002 && applyfrom
											.getTotalSkillLevel(elite = SkillFactory.getSkill(3220012)) > 0) {
								return elite.getEffect(applyfrom.getTotalSkillLevel(elite)).applyTo(applyfrom, applyto,
										primary, pos, localDuration, rltype, showEffect);
							}
							if (this.sourceid == 400011012 || this.sourceid == 400011013
									|| this.sourceid == 400011014) {
								pos = applyfrom.getTruePosition();
							} else if ((this.sourceid == 36121002 || this.sourceid == 36121013
									|| this.sourceid == 36121014 || this.sourceid == 400041044
									|| this.sourceid == 400041047) && applyto.getSkillLevel(36120051) > 0) {
								localDuration += SkillFactory.getSkill(36120051)
										.getEffect(applyto.getSkillLevel(36120051)).getDuration();
							}
							if (summId == 400001039) {
								summId = 400001040;
							...
							MapleSummon tosummon = new MapleSummon(applyfrom, summId,
									new Point(pos == null ? applyfrom.getTruePosition() : pos), summonMovementType,
									rltype, aftercancel ? Integer.MAX_VALUE : localDuration);
							...
							if (localDuration != 0 && localDuration < 2100000000 && this.sourceid < 400000000
							...
							applyfrom.dropMessageGM(-8,
									"spawn summon : " + summId + " / duration : "
											+ (aftercancel ? Integer.MAX_VALUE : localDuration) + " / pos : "
											+ tosummon.getTruePosition());
							...
							if (this.sourceid == 131003023 || this.sourceid == 131004023 || this.sourceid == 131005023
									|| this.sourceid == 131006023) {
								tosummon.setLastAttackTime(System.currentTimeMillis());
							}
							if (this.sourceid == 400041028) {
								applyto.getClient().getSession()
										.writeAndFlush((Object) CField.ShadowServentRefresh(applyto, tosummon, 3));
							...
							if (this.sourceid == 400011077) {
								SkillFactory.getSkill(400011078).getEffect(this.level).applyTo(applyto, false,
										localDuration);
							} else if (this.sourceid == 131001022) {
								SkillFactory.getSkill(131002022).getEffect(this.level).applyTo(applyto, false,
										localDuration);
								SkillFactory.getSkill(131003022).getEffect(this.level).applyTo(applyto, false,
										localDuration);
								SkillFactory.getSkill(131004022).getEffect(this.level).applyTo(applyto, false,
										localDuration);
								SkillFactory.getSkill(131005022).getEffect(this.level).applyTo(applyto, false,
										localDuration);
								SkillFactory.getSkill(131006022).getEffect(this.level).applyTo(applyto, false,
										localDuration);
							} else if (this.sourceid == 400031007) {
								SkillFactory.getSkill(400031008).getEffect(this.level).applyTo(applyto, false,
										localDuration);
							} else if (this.sourceid == 400031008) {
								SkillFactory.getSkill(400031009).getEffect(this.level).applyTo(applyto, false,
										localDuration);
								...
								if (applyto.getSkillLevel(5210015) > 0) {
									SkillFactory.getSkill(5220019).getEffect(applyto.getSkillLevel(5220019))
											.applyTo(applyto, false, false);
									summon = applyto.getSummon(5211019);
									if (summon != null) {
										summon.removeSummon(applyto.getMap(), false);
									}
									;
									MapleSummon summon4 = new MapleSummon(applyto, 5211019, applyto.getTruePosition(),
											SummonMovementType.FLAME_SUMMON, (byte) 0, 120000);
								...
								try {
									MapleSummon tosummon1 = new MapleSummon(applyfrom, 400051038,
											new Point(applyfrom.getTruePosition().x, applyfrom.getTruePosition().y),
											summonMovementType, rltype, localDuration);
									applyfrom.getMap().spawnSummon(tosummon1, localDuration);
									applyfrom.addSummon(tosummon1);
									tosummon1.addHP(this.x);
									MapleSummon tosummon2 = new MapleSummon(applyfrom, 400051052,
											new Point(applyfrom.getTruePosition().x + 100,
													applyfrom.getTruePosition().y),
											summonMovementType, rltype, localDuration);
									applyfrom.getMap().spawnSummon(tosummon2, localDuration);
									applyfrom.addSummon(tosummon2);
									tosummon2.addHP(this.x);
									MapleSummon tosummon3 = new MapleSummon(applyfrom, 400051053,
											new Point(applyfrom.getTruePosition().x + 200,
													applyfrom.getTruePosition().y),
											summonMovementType, rltype, localDuration);
								...
								try {
									...
									MapleSummon tosummon2 = new MapleSummon(applyfrom, 5320011, pos1,
											summonMovementType, rltype, localDuration);
									applyfrom.getMap().spawnSummon(tosummon2, localDuration);
									applyfrom.addSummon(tosummon2);
									tosummon2.addHP(this.x);
									if (applyfrom.getSkillLevel(5320045) > 0) {
										...
										MapleSummon tosummon3 = new MapleSummon(applyfrom, 5320011, pos1,
												summonMovementType, rltype, localDuration);
										applyfrom.getMap().spawnSummon(tosummon3, localDuration);
										applyfrom.addSummon(tosummon3);
										tosummon3.addHP(this.x);
								...
								if (count.size() == 3) {
									applyfrom.getMap().broadcastMessage(CField.teslaTriangle(applyfrom.getId(),
											(Integer) count.get(0), (Integer) count.get(1), (Integer) count.get(2)));
								} else if (count.size() < 3) {
									return true;
								} else {
									for (MapleSummon s : applyfrom.getMap().getAllSummonsThreadsafe()) {
										if (s.getSkill() != this.sourceid
												|| s.getOwner().getId() != applyfrom.getId()) {
											continue;
										}
									...
									MapleSummon s = new MapleSummon(applyfrom, this, pos,
								...
								applyfrom.getClient().getSession()
										.writeAndFlush((Object) CWvsContext.enableActions(applyfrom));
							} else if (this.sourceid == 400051017) {
								MapleAtom atom = new MapleAtom(false, applyfrom.getId(), 30, true, this.sourceid, 0, 0);
								List<MapleMapObject> objs = applyfrom.getMap().getMapObjectsInRange(
										applyto.getTruePosition(), 500000.0,
								...
								for (MapleMapObject obj : objs) {
									atom.addForceAtom(
											new server.maps.ForceAtom(1, Randomizer.rand(52, 72), Randomizer.rand(5, 6),
													Randomizer.rand(33, 72), 1440, applyfrom.getTruePosition()));
								...
								server.maps.ForceAtom forceAtom = new server.maps.ForceAtom(1, 49, 5,
										Randomizer.rand(45, 90), 1440);
						...
						MechDoor door = new MechDoor(applyto, new Point(pos == null ? applyto.getTruePosition() : pos),
								newId, localDuration);
					...
					if ((this.sourceid == 500001002 || this.sourceid == 500001003 || this.sourceid == 500001004)
							&& applyfrom.getSummons(summId).size() == 1) { // HEXA 솔 야누스 구현
					...
					if (applyfrom.getId() == applyto.getId() && applyfrom.getParty() != null) {
						Rectangle bounds = this.calculateBoundingBox(applyfrom.getTruePosition(),
								applyfrom.isFacingLeft());
						List<MapleMapObject> affecteds = applyfrom.getMap().getMapObjectsInRect(bounds,
								Arrays.asList(new MapleMapObjectType[] { MapleMapObjectType.PLAYER }));
						for (MapleMapObject affectedmo : affecteds) {
							MapleCharacter affected = (MapleCharacter) affectedmo;
							if (affected.getParty() == null || applyfrom.getId() == affected.getId()
									|| !this.isPartyBuff(applyfrom, affected)
									|| !this.calculateBoundingBox(applyfrom.getTruePosition(), applyfrom.isFacingLeft())
											.contains(applyto.getTruePosition())
									|| applyfrom.getParty().getId() != affected.getParty().getId()) {
								continue;
							}
							this.applyTo(applyto, affected, primary, pos, localDuration, (byte) 0,
									this.sourceid == 2311003);
							affected.getClient().getSession()
									.writeAndFlush((Object) CField.EffectPacket.showEffect(affected, 0, this.sourceid,
											4, 0, 0, (byte) (affected.getTruePosition().x > pos.x ? 1 : 0), true, pos,
											null, null));
							affected.getMap().broadcastMessage(affected,
									CField.EffectPacket.showEffect(affected, 0, this.sourceid, 4, 0, 0,
											(byte) (affected.getTruePosition().x > pos.x ? 1 : 0), false, pos, null,
											null),
									false);
						}
					}
				...
				boolean spawn = false;
				if ((this.sourceid == 400031039 || this.sourceid == 400031040) && primary
						|| (this.sourceid == 400040008 || this.sourceid == 400041008) && !primary
						|| this.sourceid == 2111003
						|| (this.sourceid == 12121005 || this.sourceid == 400001017 || this.sourceid == 100001261
								|| this.sourceid == 151121041 || this.sourceid == 4221006) && primary
						|| this.sourceid != 400031039 && this.sourceid != 400031040 && this.sourceid != 12121005
								&& this.sourceid != 400001017 && this.sourceid != 100001261
								&& this.sourceid != 151121041 && this.sourceid != 400040008
								&& this.sourceid != 400041008 && this.sourceid != 4221006) {
					spawn = true;
				}
				if (!spawn) {
					break block196;
				...
				Rectangle bounds = this.calculateBoundingBox(pos != null ? pos : applyfrom.getTruePosition(),
						applyfrom.isFacingLeft());
				MapleMist mist = new MapleMist(bounds, applyfrom, this,
						this.sourceid == 12121005 ? this.alchemistModifyVal(applyfrom, localDuration, false)
								: localDuration,
						rltype);
				if (this.sourceid == 101120104) {
				...
				if (this.sourceid == 80001839) {
					...
					if (applyfrom.getMap() != null) {
						...
						if (mist != null) {
							...
							for (MapleMapObject monstermo : applyfrom.getMap().getMapObjectsInRange(mist.getPosition(),
									range, Arrays.asList(MapleMapObjectType.MONSTER))) {
								mob = (MapleMonster) monstermo;
								if (mob != null) {
									mob.applyStatus(applyfrom.getClient(), MonsterStatus.MS_Freeze,
											new MonsterStatusEffect(100001261, 15000), 15000, mist.getSource());
								}
							}
						}
					}
				...
				if (this.sourceid == 400051025 || this.sourceid == 400051026 || this.sourceid == 400031012
						|| this.sourceid == 22170093 || this.sourceid == 400041041) {
					mist.setDelay(0);
				}
				applyfrom.getMap().spawnMist(mist, false);
				if (applyfrom.isGM()) {
					applyfrom.dropMessageGM(-8, "spawnMist : " + this.sourceid + " / Duration : " + duration
							+ " / pos : " + mist.getTruePosition());

				}
				if (sourceid == 400051025) {
					applyfrom.getMap().broadcastMessage(
							CField.ICBM(true, sourceid, calculateBoundingBox(pos, applyfrom.isFacingLeft())));
				}
			}
		...
		if (isTimeLeap() && (System.currentTimeMillis() - applyto.lastTimeleapTime) >= duration) {
			for (MapleCoolDownValueHolder i : applyto.getCooldowns()) {
				if (i.skillId != 5121010 && !SkillFactory.getSkill(i.skillId).isHyper()
						&& i.skillId / 10000 <= applyto.getJob()) {
		...
		if (this.rewardItem != null && this.totalprob > 0) {
			for (Triple<Integer, Integer, Integer> reward : this.rewardItem) {
				if (!MapleInventoryManipulator.checkSpace(applyto.getClient(), (Integer) reward.left,
						(Integer) reward.mid, "") || (Integer) reward.right <= 0
						|| Randomizer.nextInt(this.totalprob) >= (Integer) reward.right) {
					continue;
				}
				if (GameConstants.getInventoryType((Integer) reward.left) == MapleInventoryType.EQUIP) {
					...
					item.setGMLog(
							"Reward item (effect): " + this.sourceid + " on " + FileoutputUtil.CurrentReadable_Date());
					MapleInventoryManipulator.addbyItem(applyto.getClient(), item);
					continue;
				}
				MapleInventoryManipulator.addById(applyto.getClient(), (Integer) reward.left,
						((Integer) reward.mid).shortValue(),
						"Reward item (effect): " + this.sourceid + " on " + FileoutputUtil.CurrentReadable_Date());
			}
		}
		return true;
	...
	public final Rectangle calculateBoundingBox(int skillid, int level, Point posFrom, boolean facingLeft) {
		return SecondaryStatEffect.calculateBoundingBox(posFrom, facingLeft,
				SkillFactory.getSkill((int) skillid).getEffect((int) level).lt,
				SkillFactory.getSkill((int) skillid).getEffect((int) level).rb, this.range);
	}
	...
	public final double getMaxDistanceSq() {
		int maxX = Math.max(Math.abs(this.getLt() == null ? 0 : this.getLt().x),
				Math.abs(this.rb == null ? 0 : this.rb.x));
		int maxY = Math.max(Math.abs(this.getLt() == null ? 0 : this.getLt().y),
				Math.abs(this.rb == null ? 0 : this.rb.y));
	...
	public final void silentApplyBuff(MapleCharacter chr, long starttime,
			Map<SecondaryStat, Pair<Integer, Integer>> statup, int cid) {
		...
		for (Map.Entry<SecondaryStat, Pair<Integer, Integer>> statupz : statup.entrySet()) {
			...
			SummonMovementType summonMovementType;
			long remainDuration = (long) ((Integer) statupz.getValue().right).intValue()
			...
			if (remainDuration > 0L) {
				...
				if (summonMovementType == null
						|| (tosummon = new MapleSummon(chr, this, chr.getTruePosition(), summonMovementType))
								.isPuppet()) {
					continue;
				}
	...
	public final void applyKaiserCombo(MapleCharacter applyto, short combo) {
		EnumMap<SecondaryStat, Pair<Integer, Integer>> stat = new EnumMap<SecondaryStat, Pair<Integer, Integer>>(
				SecondaryStat.class);
	...
	/*
	 * Opcode count of 19699 triggered aggressive code reduction. Override with
	 * --aggressivesizethreshold. WARNING - void declaration
	 */
	// 버프 걸리면 호출
	private final void applyBuffEffect(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary,
			int newDuration, Point pos, boolean showEffect) {
...
//        }
		...
		case 1241501: { // 오리진 세이크리드 바스티온 구현
			MapleMist mist = new MapleMist(new Rectangle(pos.x + 50, pos.y + 50, pos.x - 50, pos.y - 50), applyfrom,
					this, localDuration, (byte) 0);
			applyfrom.getMap().spawnMist(mist, false);
		...
		case 2341501: { // 오리진 홀리 어드밴트 구현
			SkillFactory.getSkill(2341502).getEffect(applyfrom.getSkillLevel(2341500)).applyTo(applyfrom,
					new Point(pos.x - 600, pos.y));
			SkillFactory.getSkill(2341503).getEffect(applyfrom.getSkillLevel(2341500)).applyTo(applyfrom,
					new Point(pos.x + 600, pos.y));
		...
		case 4361501: { // 오리진 카르마 블레이드 구현
			...
			localstatups.put(SecondaryStat.Origin_Karma_Blade_Stack,
					new Pair<Integer, Integer>(applyfrom.hexaMasterySkillStack, localDuration));
			break;
		}
			...
			applyfrom.hexaSkillTimer = Timer.WorldTimer.getInstance().register(new Runnable() {
				...
				public void run() {
					if (applyfrom.getBuffedValue(12141500)) {
						...
						applyfrom.getMap()
								.broadcastMessage(CField.bonusAttackRequest(12141501, mobList, true, 0, null));
					} else {
		...
		case 31141501: { // 오리진 나이트메어 구현
			...
			localstatups.put(SecondaryStat.Origin_Nightmare,
					new Pair<Integer, Integer>(applyfrom.hexaMasterySkillStack, localDuration));
			break;
		}
		...
		case 32141000: { // 데스 VI 구현
			localstatups.put(SecondaryStat.BMageDeath,
					new Pair<Integer, Integer>(Integer.valueOf(applyto.getDeath()), localDuration));
			break;
		}
		case 33141004: { // 와일드 발칸 VI 구현
			...
			localstatups.put(SecondaryStat.Unk_764,
					new Pair<Integer, Integer>(applyfrom.hexaMasterySkillStack, localDuration));
			break;
		}
			...
			applyfrom.hexaSkillTimer = Timer.WorldTimer.getInstance().register(new Runnable() {
				...
				public void run() {
					if (applyfrom.getBuffedValue(35141501)) {
						...
						applyfrom.getMap()
								.broadcastMessage(CField.bonusAttackRequest(35141501, mobList, true, 0, null));
					} else {
						if (applyfrom.hexaSkillTimer != null) {
							applyfrom.hexaSkillTimer.cancel(true);
						}
		...
		case 155141501: { // 오리진 가장 오래된 심연 구현
			if (applyfrom.getSkillLevel(155141000) > 0) { // 깨어난 심연 구현
				...
				SecondaryStatEffect eff = SkillFactory.getSkill(155141001)
						.getEffect(applyfrom.getSkillLevel(155141000));
				applyfrom.getClient().getSession()
						.writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups, eff, applyfrom));
			}
		...
		case 162141007: { // 분출 : 해돋이 우물 VI 구현
			SkillFactory.getSkill(162141008).getEffect(applyfrom.getSkillLevel(162141008)).applyTo(applyfrom,
		...
		default: // 모든 6차 스킬 도입부 무적 & 바인딩
			if (applyfrom.getLevel() >= 260) {
				if (this.sourceid == GameConstants.getHexaOriginFirst(applyfrom.getJob())) {
					...
					if (!applyfrom.getBuffedValue(this.sourceid)
							&& applyfrom.getKeyValue(7777777, "Origin Cooltime") <= System.currentTimeMillis()) {
						...
						if (this.sourceid == 15141500) { // 오리진 뇌명벽해파 구현
							...
							SkillFactory.getSkill(15001022).getEffect(applyfrom.getSkillLevel(15001022)).applyTo(
									applyfrom, applyfrom.getBuffedEffect(SecondaryStat.CygnusElementSkill) == null);
							SkillFactory.getSkill(15110025).getEffect(15110025).applyTo(applyfrom);
						}

						if (this.sourceid == 22201500) { // 오리진 조디악 버스트 구현
							SkillFactory.getSkill(22201501).getEffect(applyfrom.getSkillLevel(22201501))
									.applyTo(applyfrom);
						}
						...
						if (eff != null) {
							applyfrom.hexaMasterySkillStack = (int) ((10000 + SkillFactory.getSkill((int) 27120008)
									.getEffect((int) applyto.getSkillLevel((int) 27120008)).duration)
									- (System.currentTimeMillis() % 1000000000L - eff.getStarttime()) + 10000);

							SkillFactory.getSkill(20040219).getEffect(applyfrom.getSkillLevel(20040219))
									.applyTo(applyfrom);
						}
						...
						if (eff != null) {
							...
							SkillFactory.getSkill(20040220).getEffect(applyfrom.getSkillLevel(20040220))
									.applyTo(applyfrom);
						}
						// 오리진 하모닉 패러독스 구현 [E]
						...
						if (eff != null) {
							applyfrom.hexaMasterySkillStack = (int) (eff.getDuration()
									- (System.currentTimeMillis() % 1000000000L - eff.getStarttime()) + 10000);

							SkillFactory.getSkill(61111008).getEffect(applyfrom.getSkillLevel(61111008))
									.applyTo(applyfrom);

							applyfrom.setKaiserCombo((short) 1201);
							SkillFactory.getSkill(61111008).getEffect(1).applyKaiserCombo(applyfrom,
									applyfrom.getKaiserCombo());
						}
						...
						if (eff != null) {
							...
							SkillFactory.getSkill(61120008).getEffect(applyfrom.getSkillLevel(61120008))
									.applyTo(applyfrom);
						...
						if (eff != null) {
							...
							SkillFactory.getSkill(61121053).getEffect(applyfrom.getSkillLevel(61121053))
									.applyTo(applyfrom);
						...
						if (this.sourceid == 155141500) { // 오리진 가장 오래된 심연 구현
							SkillFactory.getSkill(155141501).getEffect(applyfrom.getSkillLevel(155141500))
									.applyTo(applyfrom);
						}
						...
						localstatups.put(SecondaryStat.IndieNotDamaged,
								new Pair<Integer, Integer>(1, ndTime != 0 ? ndTime : 5000));
						// localstatups.put(SecondaryStat.IndieNotMove, new Pair<Integer, Integer>(1,
						// ndTime != 0 ? ndTime : 5000));

						applyfrom.setKeyValue(7777777, "Origin Cooltime",
								(System.currentTimeMillis() + originDuration) + "");
					}
				}
			}
		...
		case 21120019: {
			...
			if (applyfrom.getSkillCustomValue0(21120019) < 6) {
				localstatups.put(SecondaryStat.AranBoostEndHunt,
						new Pair<>((int) applyfrom.getSkillCustomValue0(21120019), localDuration));
		...
		case 1220010: {
			...
			localstatups.put(SecondaryStat.ElementalCharge,
					new Pair<Integer, Integer>(u * applyfrom.getElementalCharge(), localDuration));
			break;
		}
		case 1211010: {
			bufftimeR = false;
			...
			localstatups.clear();
			localstatups.put(SecondaryStat.Listonation,
					new Pair<Integer, Integer>(applyfrom.getListonation() * this.y, localDuration));
			break;
		}
		case 2211015: {
		...
		case 25121133: {
			...
			localstatups.put(SecondaryStat.IndieDamR,
					new Pair<Integer, Integer>(Integer.valueOf(this.indieDamR), localDuration));
			localstatups.put(SecondaryStat.IndiePad,
					new Pair<Integer, Integer>(Integer.valueOf(this.indiePad), localDuration));
			localstatups.put(SecondaryStat.IndieBDR,
					new Pair<Integer, Integer>(Integer.valueOf(this.indieBDR), localDuration));
			localstatups.put(SecondaryStat.IndieIgnoreMobPdpR,
					new Pair<Integer, Integer>(Integer.valueOf(this.indieIgnoreMobpdpR), localDuration));
		...
		case 2221054: { // 아이스오라
			...
			localstatups.put(SecondaryStat.IceAura,
					new Pair<Integer, Integer>(1, localDuration > 0 ? localDuration : 0));
			localstatups.put(SecondaryStat.IndieAsrR,
					new Pair<Integer, Integer>(this.z, localDuration > 0 ? localDuration : 0));
			localstatups.put(SecondaryStat.IndieTerR,
					new Pair<Integer, Integer>(this.w, localDuration > 0 ? localDuration : 0));
		...
		case 2301004: {
			...
			localstatups.put(SecondaryStat.Bless,
					new Pair<Integer, Integer>(Integer.valueOf(this.level), localDuration));
		...
		case 2310013: {
			bufftimeR = false;
			localstatups.put(SecondaryStat.CooltimeHolyMagicShell,
					new Pair<Integer, Integer>(1, SkillFactory.getSkill(2311009).getEffect(this.level).getY() * 1000));
			break;
		}
		case 2311003: {
			if (primary) {
			...
			if (applyfrom.getId() == applyto.getId() && applyfrom.getSkillLevel(2320046) > 0) {
				localstatups.put(SecondaryStat.HolySymbol, new Pair<Integer, Integer>(
						this.x + SkillFactory.getSkill(2320046).getEffect(1).getY(), localDuration));
			} else {
				localstatups.put(SecondaryStat.HolySymbol,
						new Pair<Integer, Integer>(primary ? this.x : this.x / 2, localDuration));
			}
			if (applyfrom.getSkillLevel(2320047) > 0) {
				localstatups.put(SecondaryStat.IndieAsrR,
						new Pair<Integer, Integer>(primary ? asr : asr / 2, localDuration));
				localstatups.put(SecondaryStat.IndieTerR,
						new Pair<Integer, Integer>(primary ? asr : asr / 2, localDuration));
			}
			...
			localstatups.put(SecondaryStat.DropRate, new Pair<Integer, Integer>(
					SkillFactory.getSkill((int) 2320048).getEffect((int) 1).v, localDuration));
			break;
		}
		case 2311009: {
			...
			if (applyfrom.getSkillLevel(2320044) > 0) {
				localDuration += SkillFactory.getSkill(2320044).getEffect(applyfrom.getSkillLevel(2320044))
						.getDuration();
			}
			localstatups.put(SecondaryStat.HolyMagicShell,
					new Pair<Integer, Integer>(Integer.valueOf(applyto.getHolyMagicShell()), localDuration));
			break;
		}
		case 2321001: {
			localstatups.clear();
		...
		case 2321054: {
			for (MapleSummon mapleSummon : applyto.getMap().getAllSummonsThreadsafe()) {
				if (mapleSummon.getOwner().getId() != applyto.getId()
						|| mapleSummon.getSkill() != 400021032 && mapleSummon.getSkill() != 400021033) {
					continue;
				}
			...
			if (applyto.getBuffedEffect(400021032) != null) {
				...
				SkillFactory.getSkill(400021033).getEffect(applyto.getSkillLevel(400021032)).applyTo(applyto, false,
						(int) bufftime);
				break;
			}
			...
			SkillFactory.getSkill(400021032).getEffect(applyto.getSkillLevel(400021032)).applyTo(applyto, false,
					(int) bufftime);
			break;
		}
		case 2321016: {
			...
			localstatups.put(SecondaryStat.IndiePmdR,
					new Pair<Integer, Integer>(Math.min(this.s2, this.u + (this.u * int_)), localDuration));
			break;
		}
		case 3121002:
		case 3221002:
			...
			if (applyfrom.getSkillLevel(3120044) > 0) {
				localstatups.put(SecondaryStat.IndieIgnoreMobPdpR,
						new Pair<Integer, Integer>(
								Integer.valueOf(SkillFactory.getSkill((int) 3120044).getEffect((int) 1).ignoreMobpdpR),
								localDuration));
			}
			if (applyfrom.getSkillLevel(3220044) > 0) {
				localstatups.put(SecondaryStat.IndieIgnoreMobPdpR,
						new Pair<Integer, Integer>(
								Integer.valueOf(SkillFactory.getSkill((int) 3220044).getEffect((int) 1).ignoreMobpdpR),
								localDuration));
			}
			if (applyfrom.getSkillLevel(3320026) > 0) {
				localstatups.put(SecondaryStat.IndieIgnoreMobPdpR,
						new Pair<Integer, Integer>(
								Integer.valueOf(SkillFactory.getSkill((int) 3320026).getEffect((int) 1).ignoreMobpdpR),
								localDuration));
			}
			localstatups.put(SecondaryStat.SharpEyes,
					new Pair<Integer, Integer>(
							((applyfrom.getSkillLevel(3120045) > 0 ? 5
									: (applyfrom.getSkillLevel(3220045) > 0 ? 5
											: (applyfrom.getSkillLevel(3320027) > 0 ? 5 : 0)))
									+ this.x << 8) + this.y,
							localDuration));
			break;
		}
		case 3320008: {
			showEffect = false;
			localstatups.put(SecondaryStat.BonusAttack,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(3320008), localDuration));
			break;
		}
		case 3321034: {
			localstatups.put(SecondaryStat.IndieSummon, new Pair<Integer, Integer>(1, localDuration));
		...
		case 3310006: { // 에인션트 가이던스
			...
			applyto.getClient().getSession().writeAndFlush((Object) CField.EffectPacket.showEffect(applyto, 0, 3310006,
					1, 0, 0, (byte) (applyto.isFacingLeft() ? 1 : 0), true, applyto.getTruePosition(), null, null));
			applyto.getMap().broadcastMessage(applyto,
					CField.EffectPacket.showEffect(applyto, 0, 3310006, 1, 0, 0,
							(byte) (applyto.isFacingLeft() ? 1 : 0), false, applyto.getTruePosition(), null, null),
					false);
			break;
		}
		case 400011038: {
		...
		case 2321005: {
			...
			localstatups.put(SecondaryStat.AdvancedBless,
					new Pair<Integer, Integer>(Integer.valueOf(this.level), localDuration));
			if (applyfrom.getSkillLevel(2320051) > 0) {
				localstatups.put(SecondaryStat.IndieMp,
						new Pair<Integer, Integer>(
								this.indieMmp + SkillFactory.getSkill((int) 2320051).getEffect((int) 1).indieMmp,
								localDuration));
				localstatups.put(SecondaryStat.IndieHp,
						new Pair<Integer, Integer>(
								this.indieMhp + SkillFactory.getSkill((int) 2320051).getEffect((int) 1).indieMhp,
								localDuration));
			} else {
				localstatups.put(SecondaryStat.IndieMp,
						new Pair<Integer, Integer>(Integer.valueOf(this.indieMmp), localDuration));
				localstatups.put(SecondaryStat.IndieHp,
						new Pair<Integer, Integer>(Integer.valueOf(this.indieMhp), localDuration));
			}
			if (applyfrom.getSkillLevel(2320049) > 0) {
				localstatups.put(SecondaryStat.IndieMad, new Pair<Integer, Integer>(
						this.y + SkillFactory.getSkill(2320049).getEffect(1).getX(), localDuration));
				localstatups.put(SecondaryStat.IndiePad, new Pair<Integer, Integer>(
						this.x + SkillFactory.getSkill(2320049).getEffect(1).getX(), localDuration));
				break;
			}
			localstatups.put(SecondaryStat.IndieMad, new Pair<Integer, Integer>(this.y, localDuration));
			localstatups.put(SecondaryStat.IndiePad, new Pair<Integer, Integer>(this.x, localDuration));
		...
		case 80000783: { // 유일 전용 버프
			...
			localstatups.put(SecondaryStat.IndiePmdR,
					new Pair<>(applyfrom.getOneItemEquippedCount() * 3, localDuration));
			break;
		}
		case 2321055: {
			bufftimeR = false;
		...
		case 3101009: {
			...
			case 1: {
				localstatups.put(SecondaryStat.QuiverCatridge,
						new Pair<Integer, Integer>(applyto.getRestArrow()[0], 0));
				break block3;
			}
			case 2: {
				localstatups.put(SecondaryStat.QuiverCatridge,
						new Pair<Integer, Integer>(applyto.getRestArrow()[1], 0));
				break block3;
			}
			}
			break;
		...
		case 3110001: {
			...
			localstatups.put(SecondaryStat.MortalBlow,
					new Pair<Integer, Integer>(Integer.valueOf(applyto.getMortalBlow()), 0));
			break;
		}
		case 3210001: {
			localstatups.clear();
		...
		case 3110012: {
			bufftimeR = false;
			localstatups.put(SecondaryStat.BowMasterConcentration,
					new Pair<Integer, Integer>(applyto.getConcentration() * 5, localDuration));
			break;
		}
		...
		case 14111025: {
			...
			if (newDuration / 10000 == 207 && this.sourceid != 5201008) {
				localstatups.put(SecondaryStat.NoBulletConsume,
						new Pair<Integer, Integer>(newDuration - 2070000 + 1, localDuration));
				break;
			}
			...
			localstatups.put(SecondaryStat.NoBulletConsume,
					new Pair<Integer, Integer>(newDuration - 2330000 + 1, localDuration));
			break;
		}
		/*
		 * case 4200013: case 4220015: { aftercancel = true;
		 * localstatups.put(SecondaryStat.CriticalGrowing, new Pair<Integer,
		 * Integer>(applyto.criticalGrowing, 0)); break; }
		 */
		case 4211016: {
		...
		case 4221054: {
			...
			localDuration = 0;
			localstatups.put(SecondaryStat.IndieDamR,
					new Pair<Integer, Integer>(this.indieDamR * applyto.getFlip(), localDuration));
			localstatups.put(SecondaryStat.IndieCr,
					new Pair<Integer, Integer>(this.x * applyto.getFlip(), localDuration));
			localstatups.put(SecondaryStat.FlipTheCoin,
					new Pair<Integer, Integer>(Integer.valueOf(applyto.getFlip()) + 1, localDuration));
			break;
		}
		case 80002924: {
			if (applyto.isGM()) {
		...
		case 4341002: { // 파이널 컷
			localstatups.clear();
			if (applyto.hasDonationSkill(4341002)
					&& (GameConstants.isDualBlade(applyto.getJob()) || GameConstants.isPhantom(applyto.getJob()))) {
				if (primary) {
					bufftimeR = true;
		...
		case 5321054: { // 벅샷
			...
			if (GameConstants.isCannon(applyto.getJob()) && applyto.hasDonationSkill(5321054)) { // GameConstants.isCannon(applyto.getJob())
				...
				localstatups.put(SecondaryStat.Buckshot,
						new Pair<Integer, Integer>(Integer.valueOf(this.x), localDuration));
		...
		case 400051001: {
			...
			if (applyto.getBuffedValue(SecondaryStat.SelectDice) != null) {
				applyto.getMap().broadcastMessage(applyto, CField.EffectPacket.showDiceEffect(applyto, 0, 5111007,
						applyto.getBuffedValue(SecondaryStat.SelectDice), -1, false), false);
				applyto.getClient().getSession().writeAndFlush((Object) CField.EffectPacket.showDiceEffect(applyto, 0,
						5111007, applyto.getBuffedValue(SecondaryStat.SelectDice), -1, true));
				localstatups.put(SecondaryStat.DiceRoll,
						new Pair<Integer, Integer>(applyto.getBuffedValue(SecondaryStat.SelectDice), localDuration));
				break;
			}
			applyto.getMap().broadcastMessage(applyto,
					CField.EffectPacket.showDiceEffect(applyto, 0, 5111007, 2, -1, false), false);
			applyto.getClient().getSession()
					.writeAndFlush((Object) CField.EffectPacket.showDiceEffect(applyto, 0, 5111007, 2, -1, true));
		...
		case 35120014: {
			...
			localstatups.put(SecondaryStat.DiceRoll,
					new Pair<Integer, Integer>(thirddice * 100 + doubledice * 10 + dice, localDuration));
			int n = bl ? applyto.getBuffSource(SecondaryStat.SelectDice) : (sk = this.sourceid);
			if (bl) {
				applyto.getMap().broadcastMessage(applyto,
						CField.EffectPacket.showDiceEffect(applyto, 0, n, -1, 1, false), false);
				applyto.getClient().getSession()
						.writeAndFlush((Object) CField.EffectPacket.showDiceEffect(applyto, 0, n, -1, 1, true));
			}
			if (this.isDoubleDice() && doubledice > 0) {
				// [othello] - 더블 럭키 다이스 숫자 제대로 출력
				applyto.getMap().broadcastMessage(applyto,
						CField.EffectPacket.showDiceEffect(applyto, 0, n, dice * 10 + doubledice, -1, false), false);
				applyto.getClient().getSession().writeAndFlush(
						(Object) CField.EffectPacket.showDiceEffect(applyto, 0, n, dice * 10 + doubledice, -1, true));
				...
				applyto.getMap().broadcastMessage(applyto,
						CField.EffectPacket.showDiceEffect(applyto, 0, n, dice, -1, false), false);
				applyto.getClient().getSession()
						.writeAndFlush((Object) CField.EffectPacket.showDiceEffect(applyto, 0, n, dice, -1, true));
				if (dice == 1) {
			...
			if (applyto.getBuffedEffect(SecondaryStat.SelectDice) != null) {
				applyto.getMap().broadcastMessage(applyto,
						CField.EffectPacket.showDiceEffect(applyto, 1, n, thirddice, 0, false), false);
				applyto.getClient().getSession()
						.writeAndFlush((Object) CField.EffectPacket.showDiceEffect(applyto, 1, n, thirddice, 0, true));
			}
			applyto.getMap().broadcastMessage(applyto, CField.EffectPacket.showDiceEffect(applyto, 1, n, -1, 2, false),
					false);
			applyto.getClient().getSession()
					.writeAndFlush((Object) CField.EffectPacket.showDiceEffect(applyto, 1, n, -1, 2, true));
			break;
		}
		case 5121055: {
			bufftimeR = false;
			...
			localstatups.put(SecondaryStat.UnityOfPower,
					new Pair<Integer, Integer>(Integer.valueOf(applyto.getUnityofPower()), localDuration));
			break;
		}
		case 5121054: {
		...
		case 11121012: {
			...
			localstatups.put(SecondaryStat.Buckshot, new Pair<Integer, Integer>(
					SkillFactory.getSkill((int) 11001024).getEffect((int) applyto.getSkillLevel((int) 11001024)).x, 0));
			break;
		}
		case 11121011: {
			aftercancel = true;
		...
		case 12120007: {
			...
			// localstatups.put(SecondaryStat.Ember, new Pair<Integer, Integer>(this.x ,
			// localDuration));
			break;
		}
		case 400021042: { // 플레임 디스차지
		...
		case 12121005: { // 버닝리젼
			...
			localstatups.put(SecondaryStat.IndieDamR,
					new Pair<Integer, Integer>(Integer.valueOf(this.indieDamR), localDuration));
			localstatups.put(SecondaryStat.IndieBooster,
					new Pair<Integer, Integer>(Integer.valueOf(this.indieBooster), localDuration));
			break;
		}
		case 12121052: {
			localstatups.put(SecondaryStat.NotDamaged, new Pair<Integer, Integer>(1, 3000));
		...
		case 14120009: {
			if (applyto.siphonVitality == this.x) {
				localstatups.put(SecondaryStat.SiphonVitality,
						new Pair<Integer, Integer>(applyto.siphonVitality, this.subTime));
		...
		case 14120011: {
			if (applyto.getBuffedValue(SecondaryStat.Protective) == null) {
				applyto.setSkillCustomInfo(14120011, SkillFactory.getSkill((int) 14120009)
						.getEffect((int) applyfrom.getSkillLevel((int) 14120009)).y, 0L);
				if (applyto.getSkillLevel(14120049) > 0) {
					applyto.setSkillCustomInfo(14120011, applyto.getSkillCustomValue0(14120011)
							+ (long) SkillFactory.getSkill(14120049).getEffect(1).getX(), 0L);
				}
			}
			localstatups.put(SecondaryStat.Protective,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(14120011), localDuration));
			if (applyto.getSkillLevel(14120050) > 0) {
				localstatups.put(SecondaryStat.IndiePad,
						new Pair<Integer, Integer>(SkillFactory.getSkill(14120050).getEffect(1).getX(), localDuration));
			}
			if (applyto.getSkillLevel(14120051) <= 0) {
				break;
			}
			localstatups.put(SecondaryStat.IndieAsrR,
					new Pair<Integer, Integer>(SkillFactory.getSkill(14120051).getEffect(1).getX(), localDuration));
			break;
		}
		case 14121004: {
		...
		case 15111022: {
			...
			localstatups.put(SecondaryStat.IndieDamR,
					new Pair<Integer, Integer>(applyto.lightning * this.y, localDuration));
			break;
		}
		case 15120003: {
			if (applyto.lightning < 0) {
		...
		case 15001022: { // 엘리멘탈 : 라이트닝
			...
			localstatups.put(SecondaryStat.IgnoreTargetDEF,
					new Pair<>((applyto.getSkillLevel(15121054) > 0 ? 9 : this.x) * applyto.lightning, this.y * 1000));
			localstatups.put(SecondaryStat.IndiePmdR,
					new Pair<>((applyto.getSkillLevel(15121054) > 0 ? 5 : 0) * applyto.lightning, this.y * 1000));
			break;
		}
		case 15110025: { // 연쇄 구현
		...
		case 23110004: {
			...
			localstatups.clear();
			localstatups.put(SecondaryStat.IgnisRore,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(23110005), localDuration));
			break;
		}
		case 23111005: { // 워터실드 - 무적처리
			if (applyfrom.getSkillLevel(23120046) > 0) {
				localstatups.put(SecondaryStat.DamAbsorbShield, new Pair<Integer, Integer>(
						this.x + SkillFactory.getSkill(23120046).getEffect(1).getX(), localDuration));
			} else {
			...
			if (applyfrom.getSkillLevel(23120047) > 0) {
				localstatups.put(SecondaryStat.Asr, new Pair<Integer, Integer>(
						this.asrR + SkillFactory.getSkill(23120047).getEffect(1).getX(), localDuration));
			} else {
				localstatups.put(SecondaryStat.Asr,
						new Pair<Integer, Integer>(Integer.valueOf(this.asrR), localDuration));
			}
			if (applyfrom.getSkillLevel(23120048) > 0) {
				localstatups.put(SecondaryStat.Ter, new Pair<Integer, Integer>(
						this.terR + SkillFactory.getSkill(23120048).getEffect(1).getX(), localDuration));
				break;
			}
		...
		case 20031210: {
			...
			applyto.getClient().getSession()
					.writeAndFlush((Object) CField.updateCardStack(false, applyto.getCardStack()));
			applyto.getClient().getSession().writeAndFlush((Object) CField.EffectPacket.showEffect(applyto, 0,
					this.sourceid, 6, judgement - 1, -1, (byte) 0, true, null, null, null));
			MapleAtom atom = new MapleAtom(false, applyto.getId(), 1, true,
					applyto.getTotalSkillLevel(SkillFactory.getSkill(24120002)) > 0 ? 24120002 : 24100003,
					applyto.getTruePosition().x, applyto.getTruePosition().y);
			int i = 0;
			while (true) {
				if (i >= (this.sourceid == 24100003 ? 5 : 10)) {
					break;
				}
				atom.addForceAtom(new server.maps.ForceAtom(2, Randomizer.rand(15, 29), Randomizer.rand(7, 11),
		...
		case 20040216:
		case 20040217: {
			localstatups.put(SecondaryStat.Larkness,
					new Pair<Integer, Integer>(applyto.getLuminusMorphUse(), localDuration));
			break;
		}
		case 20040219:
		case 20040220: {
			...
			localstatups.clear();
			if (!showEffect) {
				localDuration = 10000 + SkillFactory.getSkill((int) 27120008)
						.getEffect((int) applyto.getSkillLevel((int) 27120008)).duration;
			}
			...
			localstatups.put(SecondaryStat.Larkness,
					new Pair<Integer, Integer>(applyto.getLuminusMorphUse(), localDuration));
			applyto.getClient().getSession().writeAndFlush((Object) CWvsContext.BuffPacket
					.LuminusMorph(applyto.getLuminusMorphUse(), applyto.getLuminusMorph()));
			break;
		}
		case 21000000: {
			aftercancel = true;
			localstatups.put(SecondaryStat.AranCombo,
					new Pair<Integer, Integer>(Integer.valueOf(applyto.getCombo()), 0));
			break;
		}
		case 24111002: {
			localstatups.clear();
		...
		case 24111003: {
			if (applyfrom.getSkillLevel(24120049) > 0) {
				...
				localstatups.put(SecondaryStat.IndieTerR, new Pair<Integer, Integer>(
						this.y + SkillFactory.getSkill(24120049).getEffect(1).getX(), localDuration));
			} else {
				localstatups.put(SecondaryStat.IndieAsrR, new Pair<Integer, Integer>(this.x, localDuration));
				localstatups.put(SecondaryStat.IndieTerR, new Pair<Integer, Integer>(this.y, localDuration));
			}
			if (applyfrom.getSkillLevel(24120050) > 0) {
				localstatups.put(SecondaryStat.IndieHpR, new Pair<Integer, Integer>(
						this.indieMhpR + SkillFactory.getSkill(24120050).getEffect(1).getX(), localDuration));
			} else {
				localstatups.put(SecondaryStat.IndieHpR,
						new Pair<Integer, Integer>(Integer.valueOf(this.indieMhpR), localDuration));
			}
			if (applyfrom.getSkillLevel(24120051) > 0) {
				localstatups.put(SecondaryStat.IndieMpR, new Pair<Integer, Integer>(
						this.indieMmpR + SkillFactory.getSkill(24120051).getEffect(1).getX(), localDuration));
				break;
			}
			localstatups.put(SecondaryStat.IndieMpR,
					new Pair<Integer, Integer>(Integer.valueOf(this.indieMmpR), localDuration));
			break;
		}
		case 25111209: {
			bufftimeR = false;
		...
		case 27100008: { // 블레스 오브 다크니스
			// localstatups.put(SecondaryStat.BlessOfDarkness, new Pair<Integer,
			// Integer>(Integer.valueOf(applyto.getBlessofDarkness()), 0));
			aftercancel = true;
			break;
		}
		case 27111004: {
		...
		case 27120005: {
			...
			localstatups.put(SecondaryStat.StackBuff,
					new Pair<Integer, Integer>(applyto.stackbuff * this.damR, localDuration));
			break;
		}
		case 27121054: {
			bufftimeR = false;
		...
		case 31011001: {
			...
			applyto.getClient().getSession()
					.writeAndFlush((Object) CWvsContext.BuffPacket.cancelBuff(cancelList, applyto));
		...
		case 31121005: {
			localstatups.put(SecondaryStat.IndieHpR,
					new Pair<Integer, Integer>(Integer.valueOf(this.getIndieMhpR()), localDuration));
			localstatups.put(SecondaryStat.IndieDamR,
					new Pair<Integer, Integer>(Integer.valueOf(this.indieDamR), localDuration));
		...
		case 31211003: {
			if (applyfrom.getSkillLevel(31220046) > 0) {
				localstatups.put(SecondaryStat.DamAbsorbShield, new Pair<Integer, Integer>(
						this.x + SkillFactory.getSkill(31220046).getEffect(1).getX(), localDuration));
			} else {
				localstatups.put(SecondaryStat.DamAbsorbShield, new Pair<Integer, Integer>(this.x, localDuration));
			}
			if (applyfrom.getSkillLevel(31220047) > 0) {
				localstatups.put(SecondaryStat.Asr, new Pair<Integer, Integer>(
						this.y + SkillFactory.getSkill(31220047).getEffect(1).getX(), localDuration));
			} else {
				localstatups.put(SecondaryStat.Asr, new Pair<Integer, Integer>(this.y, localDuration));
			}
			if (applyfrom.getSkillLevel(31220048) > 0) {
				localstatups.put(SecondaryStat.Ter, new Pair<Integer, Integer>(
						this.z + SkillFactory.getSkill(31220048).getEffect(1).getX(), localDuration));
				break;
			}
		...
		case 32001016: {
			...
			localstatups.put(SecondaryStat.IndieBooster, new Pair<Integer, Integer>(-1, localDuration));
			localstatups.put(SecondaryStat.IndieSpeed,
					new Pair<Integer, Integer>(Integer.valueOf(this.indieSpeed), localDuration));
			if (!primary) {
				break;
			}
		...
		case 32111012: {
			...
			localstatups.put(SecondaryStat.IndieAsrR,
					new Pair<Integer, Integer>(Integer.valueOf(this.indieAsrR), localDuration));
			if (!primary) {
				break;
			}
			if (applyto.getBuffedValue(32001016) && applyto.getBuffedOwner(32001016) == applyto.getId()) {
		...
		case 32121017: {
			...
			if (applyfrom.getId() == applyto.getId() && applyto.getSkillLevel(32120060) > 0) {
				localstatups.put(SecondaryStat.IndieBDR,
						new Pair<Integer, Integer>(
								Integer.valueOf(SkillFactory.getSkill((int) 32120060).getEffect((int) 1).indieBDR),
								localDuration));
			}
		...
		case 32121018: {
			localstatups.put(SecondaryStat.DebuffAura,
					new Pair<Integer, Integer>(Integer.valueOf(this.level), localDuration));
			if (!primary) {
				break;
			}
		...
		case 33111007: { // 비스트 폼
			...
			if (applyfrom.getSkillLevel(33120044) > 0) {
				localstatups.put(SecondaryStat.IndieHpR, new Pair<Integer, Integer>(
						Integer.valueOf(SkillFactory.getSkill((int) 33120044).getEffect((int) 1).mhpR), localDuration));
			}
			if (applyfrom.getSkillLevel(33120045) > 0) {
				localstatups.put(SecondaryStat.IndieBooster, new Pair<Integer, Integer>(
						-this.w - SkillFactory.getSkill(33120045).getEffect(1).getW(), localDuration));
				break;
			}
		...
		case 33110014: {
			...
			localstatups.put(SecondaryStat.JaguarCount,
					new Pair<Integer, Integer>(size * (this.y << 8) + this.z * size, 0));
			break;
		}
		case 35001002: {
			localstatups.clear();
			...
			localstatups.put(SecondaryStat.EnhancedPad,
					new Pair<Integer, Integer>(Integer.valueOf(pad), localDuration));
			localstatups.put(SecondaryStat.EnhancedPdd,
					new Pair<Integer, Integer>(Integer.valueOf(pdd), localDuration));
		...
		case 36111003: {
			...
			localstatups.put(SecondaryStat.DamAbsorbShield, new Pair<Integer, Integer>(this.z,
					primary ? this.duration : (int) applyto.getBuffLimit(this.sourceid)));
			localstatups.put(SecondaryStat.StackBuff, new Pair<Integer, Integer>(this.x,
					primary ? this.duration : (int) applyto.getBuffLimit(this.sourceid)));
			break;
		}
		case 36121052: {
		...
		case 37000006: {
			aftercancel = true;
			localstatups.clear();
			localstatups.put(SecondaryStat.RwBarrier,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(37000006), 0));
			localDuration = 0;
			break;
		}
		case 37100002:
		...
		case 37110009: {
			...
			localstatups.put(SecondaryStat.RWCombination,
					new Pair<Integer, Integer>(applyto.combinationBuff, localDuration));
		...
		case 37120012: {
			...
			localstatups.put(SecondaryStat.IndieCr,
					new Pair<Integer, Integer>(this.q * applyto.combinationBuff, localDuration));
			localstatups.put(SecondaryStat.RWCombination,
					new Pair<Integer, Integer>(applyto.combinationBuff, localDuration));
			if (applyto.combinationBuff < this.z) {
				break;
		...
		case 51001005: {
			...
			localstatups.put(SecondaryStat.RoyalGuardState,
					new Pair<Integer, Integer>(Integer.valueOf(applyto.getRoyalStack()), localDuration));
			if (!primary) {
				break;
			}
		...
		case 51001010: {
			bufftimeR = false;
			aftercancel = true;
			int duration1 = (applyto.getRoyalStack() != 5) ? ((applyto.getRoyalStack() != 4)
					? ((applyto.getRoyalStack() != 3) ? ((applyto.getRoyalStack() != 2) ? 1420 : 1330) : 1280)
					: 1230) : 150;
			...
			if (applyfrom.getId() == applyto.getId()) {
				...
				if (applyto.getParty() != null) {
					for (MaplePartyCharacter maplePartyCharacter : applyto.getParty().getMembers()) {
						MapleCharacter chr2;
						if (!maplePartyCharacter.isOnline() || !(chr2 = applyfrom.getClient().getChannelServer()
								.getPlayerStorage().getCharacterByName(maplePartyCharacter.getName()))
								.getBuffedValue(51111008)
								&& (applyfrom.getTruePosition().x + this.getLt().x >= chr2.getTruePosition().x
										|| applyfrom.getTruePosition().x - this.getLt().x <= chr2.getTruePosition().x
										|| applyfrom.getTruePosition().y + this.getLt().y >= chr2.getTruePosition().y
										|| applyfrom.getTruePosition().y
												- this.getLt().y <= chr2.getTruePosition().y)) {
							continue;
						}
						++size;
					}
				...
				localstatups.put(SecondaryStat.IndieDamR,
						new Pair<Integer, Integer>(this.indieDamR * (applyto.getParty() == null ? 1 : size), 0));
			} else {
				aftercancel = true;
		...
		case 61121053: {
			...
			final int[] array2;
			final int[] skilllist = array2 = new int[] { 61101002, 61110211, 61120007, 61121217, 61141006, 61141007 }; // 윌
																														// 오브
																														// 소드
																														// VI
																														// 구현
		...
		case 61121054: {
			bufftimeR = false;
			for (MapleCoolDownValueHolder i : applyto.getCooldowns()) {
				if (SkillFactory.getSkill(i.skillId) == null || i.skillId == 61121054
						|| SkillFactory.getSkill(i.skillId).isHyper() || !GameConstants.isKaiser(i.skillId / 10000)) {
					continue;
				}
				applyto.removeCooldown(i.skillId);
				applyto.getClient().getSession().writeAndFlush((Object) CField.skillCooldown(i.skillId, 0));
		...
		case 64120006: {
			bufftimeR = false;
			localstatups.put(SecondaryStat.WeaponVariety,
					new Pair<Integer, Integer>(applyto.weaponChanges1.size(), localDuration));
			break;
		}
		case 64121001: {
			if (primary) {
		...
		case 65101002: {
			...
			localstatups.put(SecondaryStat.PowerTransferGauge,
					new Pair<Integer, Integer>(applyto.getBarrier(), localDuration));
			break;
		}
		case 65111004: {
			localstatups.put(SecondaryStat.Stance,
					new Pair<Integer, Integer>(Integer.valueOf(this.prop), localDuration));
			break;
		}
		case 65120006: {
			bufftimeR = false;
		...
		case 65141000: { // 트리니티 VI 구현
			localstatups.clear();
			int value = (int) (applyto.getSkillCustomValue(this.sourceid) == null ? 0L
					: applyto.getSkillCustomValue0(this.sourceid));
			if (value >= 2) {
				break;
			}
		...
		case 150000017: {
			bufftimeR = false;
			localstatups.put(SecondaryStat.IndieDamR,
					new Pair<Integer, Integer>(this.y * applyto.FlowofFight, localDuration));
		...
		case 150010241: {
			...
			localstatups.put(SecondaryStat.IndieDamR,
					new Pair<Integer, Integer>(this.y * applyto.LinkofArk, localDuration));
		...
		case 80001462: {
			localstatups.put(SecondaryStat.IndieIgnoreMobPdpR,
					new Pair<Integer, Integer>(applyto.getStat().critical_rate * this.x / 100, localDuration));
			break;
		}
		case 80001463: {
			bufftimeR = false;
			...
			} else if (GameConstants.isArcher(applyto.getJob()) || GameConstants.isCaptain(applyto.getJob())
					|| GameConstants.isMechanic(applyto.getJob()) || GameConstants.isAngelicBuster(applyto.getJob())) {
		...
		case 80001465: {
			bufftimeR = false;
			int value = this.x * (applyto.getStat().getStr() + applyto.getStat().getDex() + applyto.getStat().getInt()
					+ applyto.getStat().getLuk()) / 100;
			if (GameConstants.isXenon(applyto.getJob())) {
			...
			if (GameConstants.isArcher(applyto.getJob()) || GameConstants.isCaptain(applyto.getJob())
					|| GameConstants.isMechanic(applyto.getJob()) || GameConstants.isAngelicBuster(applyto.getJob())) {
		...
		case 80001466: {
			localstatups.put(SecondaryStat.IndieStr,
					new Pair<Integer, Integer>((applyto.getLevel() + 1) * this.x / 100, localDuration));
			break;
		}
		case 80001467: {
			localstatups.put(SecondaryStat.IndieDex,
					new Pair<Integer, Integer>((applyto.getLevel() + 1) * this.x / 100, localDuration));
			break;
		}
		case 80001468: {
			localstatups.put(SecondaryStat.IndieInt,
					new Pair<Integer, Integer>((applyto.getLevel() + 1) * this.x / 100, localDuration));
			break;
		}
		case 80001469: {
			localstatups.put(SecondaryStat.IndieLuk,
					new Pair<Integer, Integer>((applyto.getLevel() + 1) * this.x / 100, localDuration));
			break;
		}
		case 80001470: {
			...
			if (eq == null) {
				System.out
						.println("\ubb34\uae30 \uc5c6\uc774 \ubc84\ud504\ub97c \uc5b4\ub5bb\uac8c \uc368 \u3161\u3161 "
								+ applyto.getName());
				break;
			}
			eq = (Equip) MapleItemInformationProvider.getInstance().getEquipById(eq.getItemId());
			localstatups.put(SecondaryStat.IndieStr,
					new Pair<Integer, Integer>(eq.getWatk() * this.x / 100, localDuration));
			break;
		}
		case 80001471: {
			Equip eq = (Equip) applyto.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -11);
		...
		case 80001472: {
			...
			eq = (Equip) MapleItemInformationProvider.getInstance().getEquipById(eq.getItemId());
			localstatups.put(SecondaryStat.IndieInt,
					new Pair<Integer, Integer>(eq.getMatk() * this.x / 100, localDuration));
			break;
		}
		case 80001473: {
			Equip eq = (Equip) applyto.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -11);
		...
		case 80001476: {
			localstatups.put(SecondaryStat.IndieIgnoreMobPdpR,
					new Pair<Integer, Integer>(Integer.valueOf(this.indieIgnoreMobpdpR), localDuration));
			localstatups.put(SecondaryStat.IndiePddR,
					new Pair<Integer, Integer>(Integer.valueOf(this.indiePddR), localDuration));
			break;
		}
		case 80001477: {
		...
		case 80001479: {
			localstatups.put(SecondaryStat.IndiePadR,
					new Pair<Integer, Integer>(Integer.valueOf(this.indiePadR), localDuration));
			localstatups.put(SecondaryStat.IndieMadR,
					new Pair<Integer, Integer>(Integer.valueOf(this.indieMadR), localDuration));
			break;
		}
		...
		case 80002762: {
			...
			localstatups.put(SecondaryStat.EmpiricalKnowledge,
					new Pair<Integer, Integer>(applyto.empiricalStack, localDuration));
			break;
		}
		case 80002770: {
		...
		case 100001263: // 제로 : 디바인오라
		{
			...
			localstatups.put(SecondaryStat.IndiePad,
					new Pair<Integer, Integer>(Integer.valueOf(this.indiePad), localDuration));
			localstatups.put(SecondaryStat.IndieMad,
					new Pair<Integer, Integer>(Integer.valueOf(this.indieMad), localDuration));
			localstatups.put(SecondaryStat.IndiePdd,
					new Pair<Integer, Integer>(Integer.valueOf(this.indiePdd), localDuration));
			localstatups.put(SecondaryStat.IndieTerR,
					new Pair<Integer, Integer>(Integer.valueOf(this.indieTerR), localDuration));
			localstatups.put(SecondaryStat.IndieAsrR,
					new Pair<Integer, Integer>(Integer.valueOf(this.indieAsrR), localDuration));
			localstatups.put(SecondaryStat.IndieSpeed,
					new Pair<Integer, Integer>(Integer.valueOf(this.indieSpeed), localDuration));
			localstatups.put(SecondaryStat.IndieJump,
					new Pair<Integer, Integer>(Integer.valueOf(this.indieJump), localDuration));
			localstatups.put(SecondaryStat.IndieBooster, new Pair<Integer, Integer>(-1, localDuration));
			break;
		}
		...
		case 100000276: {
			...
			localstatups.put(SecondaryStat.TimeFastABuff,
					new Pair<Integer, Integer>(Integer.valueOf(applyto.RapidTimeDetect), localDuration));
			break;
		}
		case 100000277: {
			bufftimeR = false;
			...
			localstatups.clear();
			localDuration = applyto.getBuffedValue(400001045) ? 0 : 20000;
			localstatups.put(SecondaryStat.TimeFastBBuff,
					new Pair<Integer, Integer>(Integer.valueOf(applyto.RapidTimeStrength), localDuration));
		...
		case 100001274: {
			...
			for (MapleCoolDownValueHolder m : applyto.getCooldowns()) {
				int skil = m.skillId;
				if ((skil / 10000 == 40001 || skil / 10000 == 10000) && skil != 100001005 && skil != 100001261
						&& skil != 100001283 || skil == 100001274) {
					continue;
				}
		...
		case 101120109: {
			...
			localstatups.put(SecondaryStat.Stance, new Pair<Integer, Integer>(100, localDuration));
			localstatups.put(SecondaryStat.ImmuneBarrier,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(101120109), localDuration));
			break;
		}
		case 131001004: {
			if (primary) {
		...
		case 131001113: {
			localstatups.put(SecondaryStat.MaxLevelBuff,
					new Pair<Integer, Integer>(Integer.valueOf(this.indieMadR), localDuration));
			break;
		}
		case 131001009: {
		...
		case 131001011: {
			...
			localstatups.put(SecondaryStat.PinkbeanYoYoAttackStack,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(this.sourceid + 100), localDuration));
			break;
		}
		case 131001018: {
			localstatups.put(SecondaryStat.IndieStatR,
					new Pair<Integer, Integer>(applyto.getLevel() / this.y, localDuration));
			break;
		}
		case 142101004: {
		...
		case 151101010: {
			...
			localstatups.put(SecondaryStat.IndieIgnoreMobPdpR,
					new Pair<Integer, Integer>(this.y * applyto.adelResonance, localDuration));
			localstatups.put(SecondaryStat.AdelResonance,
					new Pair<Integer, Integer>(applyto.adelResonance, localDuration));
			break;
		}
		case 151121004: {
			localDuration = 8000;
		...
		case 151121011: {
			localstatups.put(SecondaryStat.IndieNotDamaged,
					new Pair<Integer, Integer>(1, 3000 + (applyto.getSkillLevel(151120039) > 0 ? 1000 : 0)));
			break;
		}
		case 142110009: {
			localstatups.put(SecondaryStat.IndiePdd,
					new Pair<Integer, Integer>(Integer.valueOf(this.indiePdd), 180000));
		...
		case 152111003: {
			...
			localstatups.put(SecondaryStat.IndieBDR,
					new Pair<Integer, Integer>(Integer.valueOf(this.indieBDR), localDuration));
			localstatups.put(SecondaryStat.IndieStance,
					new Pair<Integer, Integer>(Integer.valueOf(this.indieStance), localDuration));
			localstatups.put(SecondaryStat.IndiePmdR,
					new Pair<Integer, Integer>(Integer.valueOf(this.indiePmdR), localDuration));
			...
			if (size > 0) {
				localstatups.put(SecondaryStat.IndieDamR, new Pair<Integer, Integer>(
						SkillFactory.getSkill(400021068).getEffect(applyfrom.getSkillLevel(400021068)).getU() * size,
						localDuration));
			}
		...
		case 152121043: {
			bufftimeR = false;
			localstatups.put(SecondaryStat.IndieNotDamaged,
					new Pair<Integer, Integer>(1, Math.max(1, 4000 + applyfrom.blessMark * 600)));
			break;
		}
		case 152121011: {
		...
		case 155001103: {
			...
			localstatups.put(SecondaryStat.IndieDamR,
					new Pair<Integer, Integer>((localDuration - 1) * this.y, this.z * 1000));
			break;
		}
		case 155001205: {
			localstatups.put(SecondaryStat.IndieFloating, new Pair<Integer, Integer>(1, localDuration / 1000));
		...
		case 155001001: {
			localstatups.put(SecondaryStat.Speed, new Pair<Integer, Integer>(
					this.speed * (applyto.getBuffedValue(155121043) ? 2 : 1), localDuration));
			localstatups.put(SecondaryStat.IndieStance, new Pair<Integer, Integer>(
					this.indieStance * (applyto.getBuffedValue(155121043) ? 2 : 1), localDuration));
			break;
		}
		case 155101003: {
			localstatups.put(SecondaryStat.IndiePad, new Pair<Integer, Integer>(
					this.indiePad * (applyto.getBuffedValue(155121043) ? 2 : 1), localDuration));
			localstatups.put(SecondaryStat.IndieCr, new Pair<Integer, Integer>(
					this.indieCr * (applyto.getBuffedValue(155121043) ? 2 : 1), localDuration));
			break;
		}
		case 155111005: {
			localstatups.put(SecondaryStat.IndieBooster,
					new Pair<Integer, Integer>(applyto.getBuffedValue(155121043) ? -2 : -1, localDuration));
			localstatups.put(SecondaryStat.IndieEvaR, new Pair<Integer, Integer>(
					this.indieEvaR * (applyto.getBuffedValue(155121043) ? 2 : 1), localDuration));
			break;
		}
		case 155111306: {
		...
		case 155120014: {
			localstatups.put(SecondaryStat.FightJazz,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(155120015), localDuration));
			break;
		}
		case 155121005: {
			localstatups.put(SecondaryStat.IndieDamR, new Pair<Integer, Integer>(
					this.indieDamR * (applyto.getBuffedValue(155121043) ? 2 : 1), localDuration));
			localstatups.put(SecondaryStat.IndieBDR, new Pair<Integer, Integer>(
					this.indieBDR * (applyto.getBuffedValue(155121043) ? 2 : 1), localDuration));
			localstatups.put(SecondaryStat.IndieIgnoreMobPdpR, new Pair<Integer, Integer>(
					SkillFactory.getSkill((int) 155121102).getEffect((int) applyto.getSkillLevel((int) 155121102)).s2
							* (applyto.getBuffedValue(155121043) ? 2 : 1),
					localDuration));
			break;
		}
		case 155121043: {
		...
		case 400001010: {
			...
			localstatups.put(SecondaryStat.BlitzShield,
					new Pair<Integer, Integer>(applyto.getBlitzShield(), localDuration));
			break;
		}
		case 400001014:
		case 400001015: {
		...
		case 400001017: {
			...
			localstatups.put(SecondaryStat.IndieAllStatR,
					new Pair<Integer, Integer>(Integer.valueOf(this.indieStatRBasic), 1500));
			break;
		}
		case 400001045: {
			for (MapleCoolDownValueHolder cooldown : applyto.getCooldowns()) {
				if (cooldown.skillId == this.sourceid || !GameConstants.isZero(cooldown.skillId / 10000)
						|| SkillFactory.getSkill(cooldown.skillId).isHyper()
						|| !applyto.skillisCooling(cooldown.skillId)) {
					continue;
				}
		...
		case 400011011: { // 로 아이아스
			...
			if (applyto.getId() == applyfrom.getId()) {
				if (applyfrom.getBuffedEffect(SecondaryStat.RhoAias) != null || !primary) {
					...
					for (MaplePartyCharacter pc : applyto.getParty().getMembers()) {
						...
						if (!pc.isOnline() || pc.getMapid() != applyto.getMapId()
								|| pc.getChannel() != applyto.getClient().getChannel()
								|| (victim = applyto.getClient().getChannelServer().getPlayerStorage()
										.getCharacterByName(pc.getName())) == null
								|| !victim.getBuffedValue(400011011)) {
							continue;
						}
			...
			localstatups.put(SecondaryStat.RhoAias,
					new Pair<Integer, Integer>(
							SkillFactory.getSkill(400011011).getEffect(applyfrom.getSkillLevel(400011011)).getX(),
							localDuration));
			break;
		}
		case 400011016: {
			...
			localstatups.put(SecondaryStat.InstallMaha,
					new Pair<Integer, Integer>(Integer.valueOf(this.level), localDuration));
			localstatups.put(SecondaryStat.IndiePadR,
					new Pair<Integer, Integer>(Integer.valueOf(this.indiePadR), localDuration));
			break;
		...
		case 400011083: {
			if (!applyto.getBuffedValue(400011083)) {
				localstatups.put(SecondaryStat.IndiePadR,
						new Pair<Integer, Integer>(Integer.valueOf(this.indiePadR), localDuration));
				localstatups.put(SecondaryStat.IndieCr,
						new Pair<Integer, Integer>(Integer.valueOf(this.indieCr), localDuration));
				localstatups.put(SecondaryStat.IndieIgnoreMobPdpR,
						new Pair<Integer, Integer>(Integer.valueOf(this.indieIgnoreMobpdpR), localDuration));
		...
		case 400011116: {
			localstatups.put(SecondaryStat.AfterImageShock,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(400011116), localDuration));
			break;
		}
		case 400011127: {
			Rectangle bounds;
			if (primary) {
				applyto.setSkillCustomInfo(400011127,
						(int) (applyto.getStat().getCurrentMaxHp() / 100L * (long) this.x), 0L);
				localstatups.put(SecondaryStat.IndieDamR, new Pair<Integer, Integer>(this.z, this.y * 1000));
				localstatups.put(SecondaryStat.IndieBarrier,
						new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(400011127), localDuration));
				...
				for (MapleMapObject mapleMapObject : affecteds) {
					...
					affected.setSkillCustomInfo(400011127,
							(int) (affected.getStat().getCurrentMaxHp() / 100L * (long) this.x), 0L);
					SkillFactory.getSkill(400011127).getEffect(applyto.getSkillLevel(400011127)).applyTo(applyto,
							affected, false, pos, newDuration, (byte) 0, false);
				}
				break;
			}
			aftercancel = true;
			...
			if (applyfrom.getParty() != null) {
				for (MapleMapObject affectedmo : affecteds) {
					...
					localstatups.put(SecondaryStat.HolyUnity,
							new Pair<Integer, Integer>(affected.getId(), localDuration));
					affected.getMap().broadcastMessage(affected,
							CField.EffectPacket.showEffect(affected, 0, this.sourceid, 1, 0, 0,
									(byte) (affected.getTruePosition().x > pos.x ? 1 : 0), false, pos, null, null),
							false);
					applyto.setSkillCustomInfo(400011003, affected.getId(), 0L);
					SkillFactory.getSkill(400011021).getEffect(applyto.getSkillLevel(400011003)).applyTo(applyfrom,
							affected, primary, pos, newDuration, (byte) 0, false);
					active = true;
					break;
				}
			}
		...
		case 400011047: {
			indieList1.add(new Pair<Integer, Integer>(applyto.getBuffedValue(1301006) ? 1 : 0,
					applyto.getBuffedValue(1301007) ? 1 : 0));
			if (primary) {
			...
			localstatups.put(SecondaryStat.IndieBarrier, new Pair<Integer, Integer>(
					(int) applyto.getSkillCustomValue0(400011048), (int) applyto.getBuffLimit(400011047)));
			localstatups.put(SecondaryStat.DarknessAura,
					new Pair<Integer, Integer>(this.w, (int) applyto.getBuffLimit(400011047)));
			break;
		}
		case 400011053: {
			if (applyto.hasDonationSkill(400011052)) {
				localstatups.put(SecondaryStat.BlessedHammer2,
						new Pair<Integer, Integer>(applyto.getElementalCharge(), 0));
			} else {
				localstatups.put(SecondaryStat.BlessedHammer2, new Pair<Integer, Integer>(applyto.getElementalCharge(),
						SkillFactory.getSkill(400011052).getEffect(this.level).getV() * 1000));
			}
			break;
		}
		...
		case 400011058: {
			...
			localstatups.put(SecondaryStat.WillofSwordStrike,
					new Pair<Integer, Integer>(applyto.ignoreDraco, localDuration));
			break;
		}
		case 400011088: {
		...
		case 400011102: {
			localstatups.put(SecondaryStat.DevilishPower,
					new Pair<Integer, Integer>(Integer.valueOf(this.level), localDuration));
			break;
		}
		case 400011108: {
		...
		case 400021006: {
			localstatups.clear();
			applyto.setSkillCustomInfo(this.sourceid, applyfrom.getId(), 0L);
			localstatups.put(SecondaryStat.UnionAura,
					new Pair<Integer, Integer>(applyfrom.getSkillLevel(400021006), localDuration));
			localstatups.put(SecondaryStat.YellowAura,
					new Pair<Integer, Integer>(applyfrom.getSkillLevel(32001016), localDuration));
			localstatups.put(SecondaryStat.DrainAura,
					new Pair<Integer, Integer>(applyfrom.getSkillLevel(32101009), localDuration));
			localstatups.put(SecondaryStat.BlueAura,
					new Pair<Integer, Integer>(applyfrom.getSkillLevel(32111012), localDuration));
			localstatups.put(SecondaryStat.DarkAura,
					new Pair<Integer, Integer>(applyfrom.getSkillLevel(32121017), localDuration));
			if (primary) {
				localstatups.put(SecondaryStat.DebuffAura,
						new Pair<Integer, Integer>(applyfrom.getSkillLevel(32121018), localDuration));
				localstatups.put(SecondaryStat.IndieMad,
						new Pair<Integer, Integer>(Integer.valueOf(this.indieMad), localDuration));
			}
			localstatups.put(SecondaryStat.IndieDamR,
					new Pair<Integer, Integer>(
							Integer.valueOf(SkillFactory.getSkill((int) 32121017)
									.getEffect((int) applyfrom.getSkillLevel((int) 32121017)).indieDamR),
							localDuration));
			localstatups.put(SecondaryStat.IndieSpeed,
					new Pair<Integer, Integer>(
							Integer.valueOf(SkillFactory.getSkill((int) 32001016)
									.getEffect((int) applyfrom.getSkillLevel((int) 32001016)).indieSpeed),
							localDuration));
			localstatups.put(SecondaryStat.IndieBooster, new Pair<Integer, Integer>(-1, localDuration));
			localstatups.put(SecondaryStat.IndieBDR,
					new Pair<Integer, Integer>(
							Integer.valueOf(SkillFactory.getSkill((int) 32120060)
									.getEffect((int) applyfrom.getSkillLevel((int) 32120060)).indieBDR),
							localDuration));
			localstatups
					.put(SecondaryStat.IndieAsrR,
							new Pair<Integer, Integer>(
									Integer.valueOf(SkillFactory.getSkill((int) 32111012)
											.getEffect((int) applyfrom.getSkillLevel((int) 32111012)).asrR),
									localDuration));
		...
		case 400021008: {
			...
			localstatups.put(SecondaryStat.PsychicTornado,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(400021009), localDuration));
			break;
		}
		case 400021012: {
			localDuration = 10000;
		...
		case 400021052: {
			localstatups.put(SecondaryStat.DamR, new Pair<Integer, Integer>(
					Math.min(this.w + applyto.getStat().getTotalInt() / this.x, this.z), localDuration));
			break;
		}
		/*
		 * case 400021071: { if (!primary) { if (applyto.getPerfusion() < this.x - 1) {
		 * applyto.setPerfusion(applyto.getPerfusion() + 1); aftercancel = true;
		 * localstatups.put(SecondaryStat.LuminousPerfusion, new Pair<Integer,
		 * Integer>(applyto.getPerfusion(), 0)); break; } if (applyto.getPerfusion() !=
		 * this.x - 1) { break; } localstatups.clear(); applyto.setPerfusion(0);
		 * applyto.cancelEffect(this); if (applyto.getCooldownLimit(400021071) <= 0L) {
		 * break; } applyto.removeCooldown(400021071); break; } aftercancel = true;
		 * localstatups.put(SecondaryStat.LuminousPerfusion, new Pair<Integer,
		 * Integer>(applyto.getPerfusion(), 0)); break; }
		 */
		case 400021070: {
			...
			localstatups.put(SecondaryStat.IndieDamR,
					new Pair<Integer, Integer>(this.q2 + applyfrom.peaceMaker * this.w2, localDuration));
			break;
		}
		case 400021077: {
			break;
		...
		case 400021093: {
			localstatups.put(SecondaryStat.IndieMad,
					new Pair<Integer, Integer>(
							(int) ((long) SkillFactory.getSkill((int) 400021092)
									.getEffect((int) applyfrom.getSkillLevel((int) 400021092)).dot
									+ applyto.getSkillCustomValue0(400021093)
											* (long) SkillFactory.getSkill((int) 400021092)
													.getEffect((int) applyfrom.getSkillLevel((int) 400021092)).w2),
							localDuration));
			break;
		}
		case 400021096: {
		...
		case 400021099: {
			...
			localstatups.put(SecondaryStat.CrystalGate,
					new Pair<Integer, Integer>(1, (int) applyto.getBuffLimit(400021099)));
			break;
		}
		case 400021100: {
			localstatups.put(SecondaryStat.IndieMad,
		...
		case 400031006: {
			...
			localstatups.put(SecondaryStat.TrueSniping, new Pair<Integer, Integer>(applyto.trueSniping,
					primary ? localDuration : (int) applyto.getBuffLimit(this.sourceid)));
		...
		case 400031017: { // 팅스킬?
			...
			// localstatups.put(SecondaryStat.RideVehicleExpire, new Pair<Integer,
			// Integer>(SecondaryStatEffect.parseMountInfo(applyto, this.sourceid),
			// localDuration));
//                localstatups.put(SecondaryStat.IgnisRore, new Pair<Integer, Integer>(10, localDuration));
		...
		case 400031055: { // 리피팅 크로스보우 카트리지
			...
			localstatups.put(SecondaryStat.RepeatingCrossbowCatridge,
					new Pair<Integer, Integer>(Cartridge, localDuration));
			break;
		}
		...
		case 400041008: {
			if (!applyto.getBuffedValue(400041008)) {
				localstatups.put(SecondaryStat.ShadowSpear,
						new Pair<Integer, Integer>(Integer.valueOf(this.level), localDuration));
				break;
			}
			return;
		...
		case 400041040: {
			...
			localstatups.put(SecondaryStat.MarkOfPhantomDebuff,
					new Pair<Integer, Integer>(applyto.getMarkofPhantom(), 0));
			localstatups.put(SecondaryStat.MarkOfPhantomStack,
					new Pair<Integer, Integer>(applyto.getMarkofPhantom(), 0));
			break;
		}
		case 400041047: {
			localstatups.put(SecondaryStat.IndieDamR,
					new Pair<Integer, Integer>(
							SkillFactory.getSkill(400041044).getEffect(applyfrom.getSkillLevel(400041044)).getS(),
							localDuration));
			break;
		}
		case 400041048: {
		...
		case 400051010: {
			for (MapleCoolDownValueHolder cooldown : applyto.getCooldowns()) {
				if (SkillFactory.getSkill(cooldown.skillId) == null || SkillFactory.getSkill(cooldown.skillId).isHyper()
						|| !GameConstants.isEunWol(cooldown.skillId / 10000)
						|| SkillFactory.getSkill(cooldown.skillId).isNotCooltimeReset()) {
					continue;
				}
				applyto.removeCooldown(cooldown.skillId);
			}
		...
		case 400051334: {
			...
			aftercancel = false;
			if (primary) {
				localstatups.put(SecondaryStat.MemoryOfSource,
						new Pair<Integer, Integer>(Integer.valueOf(this.level), 30000));
		...
		case 400051058: {
			if (!applyto.getBuffedValue(400051058)) {
				...
				localstatups.put(SecondaryStat.Striker4th,
						new Pair<Integer, Integer>(applyto.striker4thAttack, localDuration));
				break;
			}
			aftercancel = true;
			...
			localstatups.put(SecondaryStat.Striker4th,
					new Pair<Integer, Integer>(applyto.striker4thAttack, (int) applyto.getBuffLimit(400051058)));
			break;
		}
		case 400051072: {
		...
		case 80003070: {
			aftercancel = true;
			localstatups.put(SecondaryStat.NatureFriend,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(this.sourceid), 0));
			break;
		}
		case 162120038: {
			if (primary) {
			...
			localstatups.put(SecondaryStat.IndieBarrier,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(162120038), localDuration));
			break;
		}
		case 162121043: {
			localstatups.put(SecondaryStat.IndieStance,
		...
		case 162111004: {
			...
			localstatups.put(SecondaryStat.IndieDamR,
					new Pair<Integer, Integer>(Integer.valueOf(eff.indieDamR), localDuration));
			break;
		}
		case 162111001: {
			SecondaryStatEffect eff = SkillFactory.getSkill(162111000).getEffect(this.getLevel());
			localstatups.put(SecondaryStat.IndieJump,
					new Pair<Integer, Integer>(Integer.valueOf(eff.indieJump), localDuration));
			localstatups.put(SecondaryStat.IndieSpeed,
					new Pair<Integer, Integer>(Integer.valueOf(eff.indieSpeed), localDuration));
			localstatups.put(SecondaryStat.IndieBooster, new Pair<Integer, Integer>(-1, localDuration));
			break;
		}
		case 80003059: {
		...
		case 3111015: {
			...
			aftercancel = true;
			localstatups.put(SecondaryStat.FlashMirage,
					new Pair<>((int) (1 + applyto.getSkillCustomValue0(3111015)), 0));
			break;
		}
		case 1320016: {
		...
		case 1321020:
			localstatups.put(SecondaryStat.ReincarnationAccept,
					new Pair<>((int) applyto.getSkillCustomValue0(1321020), 0));
			break;
		case 1320019: {
			...
			applyto.getClient().send(CField.EffectPacket.showEffect(applyto, 0, skillid, 1, 0, 0,
					(byte) (applyto.isFacingLeft() ? 1 : 0), true, null, null, null));
			applyto.getMap().broadcastMessage(applyto, CField.EffectPacket.showEffect(applyto, 0, skillid, 1, 0, 0,
					(byte) (applyto.isFacingLeft() ? 1 : 0), false, null, null, null), false);
		...
		case 400021005: {
			...
			SkillFactory.getSkill(20040220).getEffect(1).applyTo(applyfrom, applyto, false, pos, (int) duration,
					(byte) 0, true);
			break;
		}
		case 11001022: {
		...
		case 80001809: {
			int itemid;
			if (applyto.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -29) == null || (itemid = applyto
					.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -29).getItemId()) < 1182001
					|| itemid > 1182005) {
				break;
			}
			if (itemid >= 1182001) {
		...
		case 80002280: {
			...
			final int skilllv = (applyfrom.getSkillLevel(20010294) <= 0) ? applyfrom.getSkillLevel(80000369)
					: applyfrom.getSkillLevel(20010294);
		...
		case 400001052: {
			localstatups.put(SecondaryStat.NewFlying, new Pair<Integer, Integer>(1,
					(int) SkillFactory.getSkill((int) 400001007).getEffect((int) this.level).t * 1000));
			break;
		}
		case 80003046: {
			if (applyfrom.getBuffedValue(this.sourceid)) {
		...
		case 80002625: {
			...
			localstatups.put(SecondaryStat.BlackMageDebuff,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(80002625), 0x200B20));
			break;
		}
		case 80002902: {
			bufftimeR = false;
		...
		case 131001026: {
			...
			localstatups.put(SecondaryStat.PinkBeanMagicShow,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(131001026), 0));
			break;
		}
		case 131001023: {
		...
		case 80001740: {
			localstatups.clear();
			localstatups.put(SecondaryStat.PVPUnk_441,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(80001740), localDuration));
			break;
		}
		case 80002673: {
			localstatups.clear();
		...
		case 80002338: {
			...
			localDuration = 6000;
			localstatups.put(SecondaryStat.BattlePvP_Rude_Stack,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(this.sourceid), localDuration));
			break;
		}
		case 80002342: {
			localstatups.clear();
		...
		case 80001651: {
			...
			localstatups.put(SecondaryStat.Speed,
					new Pair<Integer, Integer>(Integer.valueOf(this.speed), localDuration));
		...
		case 142121004: {
			...
			localstatups.clear();
			localstatups.put(SecondaryStat.IndiePmdR,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(142121004), localDuration));
			break;
		}
		...
		case 135001007: {
			aftercancel = true;
			localstatups.clear();
			localstatups.put(SecondaryStat.VMatrixStackBuff,
					new Pair<Integer, Integer>((int) applyfrom.getSkillCustomValue0(this.sourceid), 0));
			break;
		}
		case 135001015: {
			localstatups.clear();
		...
		case 2300009: {
			localstatups.put(SecondaryStat.BlessingAnsanble,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(2320013), 0));
			break;
		}
		case 2321006: {
			localstatups.clear();
		...
		case 400011126: { // 技能ID: 400011126（对应游戏内技能"剑之幻象"）
			// 检查玩家是否拥有ID为1101013的被动技能增益（通常是"连击大师"被动）
			if (applyto.getBuffedValue(1101013)) {
				// 若被动激活，则为玩家添加独立物理攻击力百分比加成
				// 参数说明:
				// - SecondaryStat.IndiePmdR: 独立物理攻击力百分比（Indie = Independent，独立于基础属性的加成）
				// - 72: 攻击力提升幅度（72%）
				// - 8000: 效果持续时间（8000毫秒 = 8秒）
		...
		case 80001538: {
			...
			SecondaryStat[] stats2 = { SecondaryStat.IndieStr, SecondaryStat.IndieDex, SecondaryStat.IndieInt,
					SecondaryStat.IndieLuk, SecondaryStat.IndiePmdR };
		...
		case 80002419: {
			...
			SecondaryStat[] stats = { SecondaryStat.IndieDamR, SecondaryStat.IndieExp, SecondaryStat.DropRate,
					SecondaryStat.MesoUp, SecondaryStat.IndieCD, SecondaryStat.IndieBDR, SecondaryStat.IndieAllStatR,
		...
		case 1111003: {
			localstatups.clear();
			localstatups.put(SecondaryStat.ScaringSword,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(1111003), localDuration));
			break;
		}
		case 1110009: {
			localstatups.clear();
		...
		case 64001012: {
			localstatups.clear();
			localstatups.put(SecondaryStat.DarkSight,
					new Pair<Integer, Integer>(Integer.valueOf(this.level), localDuration));
			break;
		}
		case 152121041: {
		...
		case 150011077: {
			localstatups.put(SecondaryStat.IndieDamR,
					new Pair<Integer, Integer>(SkillFactory.getSkill(150011074).getEffect(1).getQ(), localDuration));
			applyto.getClient().getSession().writeAndFlush((Object) SLFCGPacket.OnYellowDlg(3001534, 5000,
					"헤이! 왓썹 브로. 롱타임노씨이지만 유와 내 하트가 뜨겁게 뛰고있으니 전혀 걱정하지 않았돠아!\r\n말이 나왔으니 오랜만에 내 뮤직을 리슨.", ""));
			break;
		}
		case 150011076: {
			localstatups.put(SecondaryStat.IndieReduceCooltime,
					new Pair<Integer, Integer>(SkillFactory.getSkill(150011074).getEffect(1).getW(), localDuration));
			applyto.getClient().getSession().writeAndFlush((Object) SLFCGPacket.OnYellowDlg(3001533, 5000,
					"이전 너와 교신한 후 1년 이상 지난 것이다, 치르.\n연락이 너무 뜸하다는 생각이 드는 것이다, 치르.\n바크바크 녀석처럼 마음만 있으면 된다는 생각을 하는 것은 아니라고 믿는다, 치르.",
					""));
			break;
		}
		case 150011075: {
			localstatups.put(SecondaryStat.IndieExp, new Pair<Integer, Integer>(
					SkillFactory.getSkill(150011074).getEffect(1).getS() / 2, localDuration));
			localstatups.put(SecondaryStat.IndieDamR, new Pair<Integer, Integer>(
					SkillFactory.getSkill(150011074).getEffect(1).getQ() / 2, localDuration));
			localstatups.put(SecondaryStat.IndieReduceCooltime, new Pair<Integer, Integer>(
					SkillFactory.getSkill(150011074).getEffect(1).getW() / 2, localDuration));
			applyto.getClient().getSession().writeAndFlush((Object) SLFCGPacket.OnYellowDlg(3001532, 5000,
					"\uc548\ub155! \uc6b0\ub9ac\ub294 \uc0c8\ub85c\uc6b4 \uacf3\uc744 \ubc1c\uacac\ud574\uc11c \uac00\ubcf4\ub824\uace0 \ud558\ub294 \uc911\uc774\uc57c. \uc5b4\ub5a4 \uacf3\uc77c\uc9c0 \ubaa8\ub450\ub4e4 \uae30\ub300\ud558\uace0 \uc788\uc5b4...\uadf8\ub7f0\ub370 \ub610 \ubcc4\ubcfc\uc77c \uc5c6\ub294 \uacf3\uc774\uba74 \uc5b4\uca4c\uc9c0.....\ubaa8\ub450\uac00 \uc2e4\ub9dd\ud558\ub294 \uac78 \ubcf4\uae34 \uc2eb\uc740\ub370... \uadf8\ub807\ub2e4\uace0 \uac00\uc9c0 \uc54a\uc744 \uc218\ub294 \uc5c6\uace0...",
					""));
			break;
		}
		case 150011078: {
			localstatups.put(SecondaryStat.IndieExp,
					new Pair<Integer, Integer>(SkillFactory.getSkill(150011074).getEffect(1).getS(), localDuration));
			applyto.getClient().getSession().writeAndFlush((Object) SLFCGPacket.OnYellowDlg(3001535, 3000,
					"\uc557, \ubc18\uc9dd\ubc18\uc9dd\ud55c \uac78 \uc900 \uc0ac\ub78c, \uc798 \uc788\uc5c8\ub0d0! \ub098 \uc694\uc998 \ubc25 \ub9ce\uc774 \uba39\uc5c8\ub354\ub2c8 \uc774\ub9cc\ud07c \ucef8\ub2e4! \uc5b4\uc81c\ub294 \ub118\uc5b4\uc9c0\uace0 \uc6b8\uc9c0\ub3c4 \uc54a\uc74c! \ud5e4\ud5e4\u2026..(\uc774\uac74 \ube44\ubc00\uc778\ub370 \uc774 \uadfc\ucc98\uc5d0\uc11c \ubc18\uc9dd\ubc18\uc9dd\ud558\ub294 \uac83\ub3c4 \ubc1c\uacac\ud588\ub2e4!)",
					""));
			break;
		}
		case 164101006: {
		...
		case 151111005: {
			bufftimeR = false;
			localstatups.clear();
			localstatups.put(SecondaryStat.Novility,
					new Pair<Integer, Integer>(Integer.valueOf(this.level), localDuration));
			if (applyfrom.getSkillLevel(151120038) > 0) {
				localstatups.put(SecondaryStat.IndieDamR, new Pair<Integer, Integer>(
						SkillFactory.getSkill(151120038).getEffect(1).getX(), localDuration));
			}
			if (primary) {
		...
		case 37121052: {
			aftercancel = true;
			localstatups.clear();
			localstatups.put(SecondaryStat.RwMagnumBlow,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(37121052), 0));
			bufftimeR = false;
			break;
		}
		case 37000011:
		...
		case 30000227: {
			localstatups.clear();
			localstatups.put(SecondaryStat.HiddenPieceOn,
					new Pair<Integer, Integer>((int) (applyto.getKeyValue(19752, "hiddenpiece") <= 0L ? 0L
							: applyto.getKeyValue(19752, "hiddenpiece")), 0));
			break;
		}
		case 30001080: {
			showEffect = false;
		...
		case 400001016: {
			localstatups.clear();
			localstatups.put(SecondaryStat.DemonDamageAbsorbShield,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(400001016), localDuration));
			bufftimeR = false;
			break;
		}
		case 400021105: {
			...
			applyto.getClient().getSession()
					.writeAndFlush((Object) CWvsContext.BuffPacket.cancelBuff(hashMap, applyto));
		...
		case 27101003: {
			// localstatups.put(SecondaryStat.BlessOfDarkness, new Pair<Integer, Integer>(1,
			// 0));
			localDuration = 0;
			break;
		}
		...
		case 50001214: {
			...
			localstatups.put(SecondaryStat.IndieDamR,
					new Pair<Integer, Integer>(Integer.valueOf(this.indieDamR), localDuration));
			localstatups.put(SecondaryStat.MichaelProtectofLight,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(50001214), localDuration));
			break;
		}
		...
		case 400001043: {
			localstatups.clear();
			localstatups.put(SecondaryStat.IndieDamR, new Pair<Integer, Integer>(
					(int) ((long) this.q + applyfrom.getSkillCustomValue0(400001043)), localDuration));
			localstatups.put(SecondaryStat.Bless5th, new Pair<Integer, Integer>(applyfrom.메이플용사, localDuration));
			break;
		}
		case 5221054: {
		...
		case 400031066: {
			aftercancel = true;
			localstatups.clear();
			localstatups.put(SecondaryStat.GripOfAgony,
					new Pair<Integer, Integer>((int) applyfrom.getSkillCustomValue0(400031066), 0));
			break;
		}
		case 400031062: {
			localstatups.clear();
		...
		case 63120001: { // 데스 블레싱
			localstatups.clear();
			localstatups.put(SecondaryStat.DeathBlessing, new Pair<Integer, Integer>(
					this.x + SkillFactory.getSkill(63120001).getEffect(20).getX(), localDuration));
			break;
		}
		case 63111009: {
			localstatups.clear();
		...
		case 80003018: {
			aftercancel = true;
			localstatups.clear();
			localstatups.put(SecondaryStat.PriorPryperation,
					new Pair<Integer, Integer>((int) applyto.getSkillCustomValue0(this.sourceid), 0));
			break;
		}
		case 410115:
		...
		case 1210016: {
			localstatups.clear();
			localstatups.put(SecondaryStat.BlessingArmor,
					new Pair<Integer, Integer>((int) applyfrom.getSkillCustomValue0(this.sourceid), localDuration));
			localstatups.put(SecondaryStat.BlessingArmorIncPad,
					new Pair<Integer, Integer>(Integer.valueOf(this.epad), localDuration));
			break;
		}
		default: {
		...
		if (GameConstants.MovedSkill(this.sourceid) || this.sourceid == 400001050 || this.sourceid == 400051000
				|| this.sourceid == 15001021 || this.sourceid == 63001004 || this.sourceid == 63001006
				|| localstatups.containsKey(SecondaryStat.ReviveOnce)) {
			showEffect = false;
		}
		...
		if (applyto.getKeyValue(800023, "indiepmer3") > 0) {
			...
			// applyto.dropMessage(6, "0번 수치 : " + applyto.getKeyValue(800023,
			// "indiepmer3"));
		}
		...
		if (this.sourceid == 80002924 && applyto.getKeyValue(53714, "atk") != -1L) {
			localstatups.put(SecondaryStat.IndieDamR,
					new Pair<Integer, Integer>((int) applyto.getKeyValue(53714, "atk") + damRs, 0));
		...
		if (this.sourceid == 80002419 && applyto.getKeyValue(800024, "indiepmer") > 0) {
			localstatups.put(SecondaryStat.IndieDamR,
					new Pair<Integer, Integer>((int) applyto.getKeyValue(800024, "indiepmer") + damRs, 0));
			// applyto.dropMessage(6, "2번 수치 : " + applyto.getKeyValue(800023,
			// "indiepmer"));
		...
		if (this.isMonsterRiding() && !localstatups.containsKey(SecondaryStat.RideVehicle)) {
			...
			localstatups.put(SecondaryStat.RideVehicle,
					new Pair<Integer, Integer>(SecondaryStatEffect.parseMountInfo(applyto, this.sourceid), 0));
		} else if (SkillFactory.getSkill(this.sourceid) != null && this.sourceid != 22171080
				&& SkillFactory.getSkill(this.sourceid).getVehicleID() > 0) {
			if (localDuration >= 2100000000 || localDuration < 0) {
				localDuration = 0;
			}
			localstatups.put(SecondaryStat.RideVehicle,
					new Pair<Integer, Integer>(SkillFactory.getSkill(this.sourceid).getVehicleID(), localDuration));
			if (applyfrom.getMapId() == ServerConstants.warpMap && this.sourceid / 10000 == 8000) {
				applyfrom.getClient().getSession().writeAndFlush(SLFCGPacket
						.BlackLabel("#fn나눔고딕 ExtraBold##fs32##b#e잠수포인트 적립을 시작합니다.", 100, 1000, 4, 0, 0, 1, 4));
		...
		if (this.skill && !applyto.isHidden() && !isPetBuff && applyfrom.getId() == applyto.getId()) {
			if (this.sourceid >= 400041009 && this.sourceid <= 400041015 && showEffect
					&& applyfrom.getId() == applyto.getId()) {
				applyto.getClient().getSession()
						.writeAndFlush((Object) CField.EffectPacket.showEffect(applyto, 0, this.sourceid, 1, 0, 0,
								(byte) (applyto.getTruePosition().x > pos.x ? 1 : 0), true, pos, null, null));
				applyto.getMap().broadcastMessage(applyto, CField.EffectPacket.showEffect(applyto, 0, this.sourceid, 1,
						0, 0, (byte) (applyto.getTruePosition().x > pos.x ? 1 : 0), true, pos, null, null), false);
			}
			if (!GameConstants.isLinkMap(applyto.getMapId()) && showEffect
					&& !GameConstants.isKhali(applyto.getJob())) { // [othello]칼리 멀티팅 체크 6만원짜리 지렸따~ &&
																	// !GameConstants.isKhali(applyto.getJob())

				if (this.sourceid != 400051334 && (this.sourceid < 400041009 || this.sourceid > 400041015)) {
					applyto.getMap().broadcastMessage(applyto,
							CField.EffectPacket.showEffect(applyto, 0, this.sourceid, 1, 0, 0,
									(byte) (applyto.getTruePosition().x > pos.x ? 1 : 0), false, pos, null, null),
							false);
				}

			} else if (this.isPartyBuff(applyfrom, applyto)) {
				Rectangle bounds = this.calculateBoundingBox(applyfrom.getTruePosition(), applyfrom.isFacingLeft());
				if (this.sourceid == 155001001 || this.sourceid == 155101003 || this.sourceid == 155111005
						|| this.sourceid == 155121005) {
					bounds = SkillFactory.getSkill(155121043).getEffect(1)
							.calculateBoundingBox(applyfrom.getTruePosition(), applyfrom.isFacingLeft());
				}
				List<MapleMapObject> list = applyfrom.getMap().getMapObjectsInRect(bounds,
						Arrays.asList(new MapleMapObjectType[] { MapleMapObjectType.PLAYER }));
				for (MapleMapObject affectedmo : list) {
					...
					affected.getClient().getSession()
							.writeAndFlush((Object) CField.EffectPacket.showEffect(affected, 0, this.sourceid, 4, 0, 0,
									(byte) (affected.getTruePosition().x > pos.x ? 1 : 0), true, pos, null, null));
					affected.getMap().broadcastMessage(affected,
							CField.EffectPacket.showEffect(affected, 0, this.sourceid, 4, 0, 0,
									(byte) (affected.getTruePosition().x > pos.x ? 1 : 0), false, pos, null, null),
							false);
		...
		for (int FreudsProtection : FreudsProtections = new int[] { 400001025, 400001026, 400001027, 400001028,
				400001029, 400001030 }) {
		...
		for (Map.Entry<SecondaryStat, Pair<Integer, Integer>> statup : localstatups.entrySet()) {
			...
			if (statup.getValue().right > 0) {
				...
				if (this.skill && SkillFactory.getSkill(this.sourceid) != null
						&& !SkillFactory.getSkill(this.sourceid).isHyper() && bufftimeR && this.sourceid < 400000000
						&& statup.getKey() != SecondaryStat.IndieNotDamaged
						&& statup.getKey() != SecondaryStat.NotDamaged && this.getSummonMovementType() == null
						&& GameConstants.getHexaOriginFirst(applyto.getJob()) != this.sourceid
						&& (GameConstants.getHexaOriginFirst(applyto.getJob()) + 1) != this.sourceid
						&& (GameConstants.getHexaOriginFirst(applyto.getJob()) + 2) != this.sourceid
						&& (GameConstants.getHexaOriginFirst(applyto.getJob()) + 3) != this.sourceid
...
//                System.out.println("버프 종착점 : " + SkillFactory.getSkill(this.sourceid).getName() + " / " + SkillFactory.getSkill(this.sourceid).getId());
				addV.add(new Pair(statup.getKey(), new SecondaryStatValueHolder(this, starttime,
						(statup.getValue()).left, statup.getValue().right, applyfrom.getId(), indieList1, indieList2)));
			}
		}
		if (exit) {
			applyto.getClient().getSession().writeAndFlush((Object) CWvsContext.enableActions(applyto, true, false));
		...
		if (showEffect && primary && this.getSummonMovementType() == null) {
			if (SkillFactory.getSkill(this.sourceid) != null && (this.sourceid == 24121003 || this.sourceid == 30010186
					|| this.isHeroWill() || localstatups.size() > 0 || this.isMist() || this.damage > 0
					|| SkillFactory.getSkill(this.sourceid).getType() == 41
					|| SkillFactory.getSkill(this.sourceid).getType() == 51)) {
				applyto.getClient().getSession()
						.writeAndFlush((Object) CWvsContext.enableActions(applyto, true, false));
			} else {
				applyto.getClient().getSession()
						.writeAndFlush((Object) CWvsContext.enableActions(applyto, false, false));
			}
		}

		if (localstatups.size() > 0) {
			applyto.getClient().getSession()
					.writeAndFlush(CWvsContext.BuffPacket.giveBuff(localstatups, this, applyto, SkillList));
			applyto.getMap().broadcastMessage(applyto,
					CWvsContext.BuffPacket.giveForeignBuff(applyto, localstatups, this), false);
		...
		if (localstatups.size() > 0) {
			...
			applyto.getClient().getSession()
					.writeAndFlush((Object) CWvsContext.BuffPacket.giveBuff(localstatups, this, applyto));
			applyto.getMap().broadcastMessage(applyto,
					CWvsContext.BuffPacket.giveForeignBuff(applyto, localstatups, this), false);
			if (showEffect || this.sourceid == 3110001 || this.sourceid == 3210001 || this.sourceid == 3110012
					|| this.sourceid == 3101009) {
				applyto.getClient().getSession()
						.writeAndFlush((Object) CWvsContext.enableActions(applyto, false, true));
			}
				...
				if (this.sourceid != 21121058) {
					...
					for (int i2 = 33001007; i2 < 33001015; ++i2) {
						if (GameConstants.getJaguarSummonId(i2) > 0) {
							if (GameConstants
									.getSelectJaguarSkillId(GameConstants.getMountItem(33001001, applyto)) != i2) {
								final MapleSummon tosummon = new MapleSummon(applyfrom, i2,
										new Point(applyto.getPosition().x + Randomizer.rand(-400, 400),
												applyto.getPosition().y),
										SummonMovementType.SUMMON_JAGUAR, (byte) 0, localDuration);
								tosummon.setPosition(new Point(applyto.getPosition().x + Randomizer.rand(-400, 400),
										applyto.getPosition().y));
	...
	// 와드
	public void giveAnotherBuff(MapleCharacter applyto, MapleCharacter applyfrom,
			Map<SecondaryStat, Pair<Integer, Integer>> localstatups, boolean BuffTimeR) {
		...
		for (Map.Entry<SecondaryStat, Pair<Integer, Integer>> statup : localstatups.entrySet()) {
			if ((Integer) statup.getValue().right > 0) {
				if (this.skill && SkillFactory.getSkill(this.sourceid) != null
						&& !SkillFactory.getSkill(this.sourceid).isHyper() && BuffTimeR && this.sourceid < 400000000
						&& statup.getKey() != SecondaryStat.IndieNotDamaged
						&& statup.getKey() != SecondaryStat.NotDamaged) {
					statup.getValue().right = this.alchemistModifyVal(applyfrom, (Integer) statup.getValue().right,
							false);
				}
				applyto.registerEffect(this, starttime, statup, false, applyfrom.getId(), indieList1, indieList2);
				continue;
			}
			addV.add(new Pair<SecondaryStat, SecondaryStatValueHolder>(statup.getKey(),
					new SecondaryStatValueHolder(this, starttime, (Integer) statup.getValue().left,
							(Integer) statup.getValue().right, applyfrom.getId(), indieList1, indieList2)));
		}
		if (exit) {
			applyto.getClient().getSession().writeAndFlush((Object) CWvsContext.enableActions(applyto, true, false));
			return;
		...
		applyto.getClient().getSession()
				.writeAndFlush((Object) CWvsContext.BuffPacket.giveBuff(localstatups, this, applyto));
		applyto.getMap().broadcastMessage(applyto, CWvsContext.BuffPacket.giveForeignBuff(applyto, localstatups, this),
				false);
	}
	...
	private final int calcHPChange(MapleCharacter applyfrom, boolean primary) {
		...
		if (this.hp != 0 && applyfrom.getBuffedEffect(SecondaryStat.DebuffIncHp) == null) {
			if (!this.skill) {
				hpchange = primary ? (hpchange += this.alchemistModifyVal(applyfrom, this.hp, true))
						: (hpchange += this.hp);
				if (applyfrom.hasDisease(SecondaryStat.Undead)) {
					hpchange = applyfrom.getDisease(SecondaryStat.Undead).getMobskill().getSkillLevel() == 17
							? (hpchange *= -1)
							: (hpchange /= 2);
				}
			} else {
				hpchange += SecondaryStatEffect.makeHealHP((double) this.hp / 100.0,
						applyfrom.getStat().getTotalMagic(), 3.0, 5.0);
				if (applyfrom.hasDisease(SecondaryStat.Undead)) {
					hpchange = -hpchange;
				}
			}
		...
		if (this.hpR != 0.0 && applyfrom.getBuffedEffect(SecondaryStat.DebuffIncHp) == null) {
			hpchange += (int) ((double) applyfrom.getStat().getCurrentMaxHp() * this.hpR)
					/ (applyfrom.hasDisease(SecondaryStat.Undead) ? 2 : 1);
			if (applyfrom.hasDisease(SecondaryStat.Undead)
					&& applyfrom.getDisease(SecondaryStat.Undead).getMobskill().getSkillLevel() == 17) {
				hpchange *= -1;
			}
		}
		...
		if (hpchange > 0 && applyfrom.getMapId() == 410002060
				&& (seren = applyfrom.getMap().getMonsterById(8880602)) != null && seren.getSerenTimetype() == 2) {
		...
		if (applyfrom.getBuffedEffect(SecondaryStat.DebuffIncHp) == null) {
			...
			case 1211010: {
				hpchange = (int) (applyfrom.getStat().getCurrentMaxHp()
						* (long) Math.max(10, this.x - applyfrom.getListonation() * 10) / 100L);
				break;
			}
			}
		}
		if (applyfrom.getSkillLevel(30010242) > 0 && hpchange < 0
				&& (applyfrom.getBuffedEffect(SecondaryStat.DebuffIncHp) != null
						|| applyfrom.getBuffedValue(SecondaryStat.DemonFrenzy) != null
								&& applyfrom.getBuffedEffect(SecondaryStat.DemonFrenzy).getQ2() < applyfrom.getStat()
										.getHPPercent())
				&& this.skill) {
	...
	private static final int makeHealHP(double rate, double stat, double lowerfactor, double upperfactor) {
		return (int) (Math.random()
				* (double) ((int) (stat * upperfactor * rate) - (int) (stat * lowerfactor * rate) + 1)
				+ (double) ((int) (stat * lowerfactor * rate)));
	}

	private final int calcMPChange(MapleCharacter applyfrom, boolean primary) {
		int mpchange = 0;
		if (this.mp != 0) {
			mpchange = primary ? (mpchange += this.alchemistModifyVal(applyfrom, this.mp, false))
					: (mpchange += this.mp);
		}
		if (this.mpR != 0.0) {
		...
		if (primary) {
			if (this.mpCon != 0 && !GameConstants.isDemonSlayer(applyfrom.getJob())) {
				mpchange = applyfrom.getBuffedValue(SecondaryStat.InfinityForce) != null || applyfrom.energyCharge ? 0
						: (int) ((double) mpchange
								- (double) (this.mpCon - this.mpCon * applyfrom.getStat().mpconReduce / 100)
										* ((double) applyfrom.getStat().mpconPercent / 100.0));
			} else if (this.getForceCon() != 0 && GameConstants.isDemonSlayer(applyfrom.getJob())) {
				mpchange = applyfrom.getBuffedValue(SecondaryStat.InfinityForce) != null ? 0
						: (mpchange -= this.getForceCon());
		...
		if ((applyfrom.getBuffedValue(20040217) || applyfrom.getBuffedValue(20040219)
				|| applyfrom.getBuffedValue(20040220))
				&& (GameConstants.isDarkSkills(this.sourceid)
						|| (applyfrom.getBuffedValue(20040219) || applyfrom.getBuffedValue(20040220))
								&& (this.sourceid == 27121303 || this.sourceid == 27111303))) {
			mpchange = 0;
		}
		switch (this.sourceid) {
		case 1320019: {
	...
	public final int calcPowerChange(MapleCharacter applyfrom, boolean primary) {
		...
		if (this.powerCon != 0 && GameConstants.isXenon(applyfrom.getJob())) {
			powerchange = applyfrom.getBuffedValue(SecondaryStat.AmaranthGenerator) != null
					|| applyfrom.getBuffedValue(SecondaryStat.Overload) != null ? 0 : (int) this.powerCon;
		}
		return powerchange;
	}
	...
	private boolean isPartyBuff(MapleCharacter applyfrom, MapleCharacter applyto) {
		...
		if (this.lt == null || this.rb == null || applyfrom.getMapId() == 450013700
				|| applyto.getMapId() == 450013700) {
			return false;
		}
		for (SecondaryStat buff : partybuff = new SecondaryStat[] { SecondaryStat.BasicStatUp,
				SecondaryStat.MaxLevelBuff }) {
			if (!this.statups.containsKey(buff)) {
				continue;
			}
			return true;
		}
		if (SkillFactory.getSkill(this.sourceid).isHyper()
				&& (this.sourceid % 100 == 53 || this.sourceid == 25121132)) {
			int job = this.sourceid / 10000;
			if (job < 1000 && applyfrom.getJob() < 1000 && applyto.getJob() < 1000) {
				return true;
			}
			if (job >= 1000 && job < 2000 && applyfrom.getJob() >= 1000 && applyfrom.getJob() < 2000
					&& applyto.getJob() >= 1000 && applyto.getJob() < 2000) {
				return true;
			}
			if (job >= 2000 && job < 3000 && applyfrom.getJob() >= 1000 && applyfrom.getJob() < 3000
					&& applyto.getJob() >= 2000 && applyto.getJob() < 3000) {
				return true;
			}
			if (job >= 3000 && job < 4000 && applyfrom.getJob() >= 1000 && applyfrom.getJob() < 4000
					&& applyto.getJob() >= 3000 && applyto.getJob() < 4000) {
				return true;
			}
		}
		switch (this.sourceid) {
		...
		if (this.isHeal() || this.isResurrection() || this.isTimeLeap()) {
			return !this.isResurrection() && !this.isHeal() || !GameConstants.보스맵(applyfrom.getMapId())
					|| applyto.getDeathCount() > 0;
		}
		return false;
	}
	...
	public final int getCooldown(final MapleCharacter chra) { // 쿨타임
		...
		if (cooltime > 0) {
			...
			/*
			 * if (cooltime > 10) { localCooltime = (int) (cooltime -
			 * (chra.getStat().reduceCooltime * 1000)); } else { localCooltime = Math.max(5,
			 * (cooltime * (100 - (chra.getStat().reduceCooltime * 5)) / 100)); }
			 */
			...
			if (chra.getBuffedEffect(SecondaryStat.IndieReduceCooltime) != null
					&& !GameConstants.isBeginnerJob(sourceid / 10000) && !SkillFactory.getSkill(sourceid).isVMatrix()
					&& ((sourceid / 1000) != 8000)) {
		...
		if (chra.getSkillLevel(71000231) > 0) {
			localCooltime -= localCooltime
					* SkillFactory.getSkill(71000231).getEffect(chra.getSkillLevel(71000231)).coolTimeR / 100;
		}
		...
		if (chra.getSkillLevel(33120048) > 0
				&& (sourceid == 33111006 || sourceid == 33101215 || sourceid == 33121002)) {
		...
		if (localCooltime > 0 && chra.getBuffedValue(SecondaryStat.FixCooltime) != null && sourceid / 10000 != 8000
				&& sourceid / 10000 <= chra.getJob()) {
	...
	public final boolean isRecovery() {
		return this.skill && (this.sourceid == 1001 || this.sourceid == 10001001 || this.sourceid == 20001001
				|| this.sourceid == 20011001 || this.sourceid == 20021001 || this.sourceid == 11001
				|| this.sourceid == 35121005);
	}
	...
	public final boolean isMonsterRiding_() {
		return skill && (sourceid == 1004 || sourceid == 10001004 || sourceid == 20001004 || sourceid == 20011004
				...
				|| sourceid == 35120000 || sourceid == 400031017 || sourceid == 33001001 || sourceid == 35001002
				|| sourceid == 35111003);
	}
	...
	public final SummonMovementType getSummonMovementType() {
		...
		if (GameConstants.isAngelicBlessSkill(SkillFactory.getSkill(this.sourceid))
				|| GameConstants.isAngelicBlessBuffEffectItem(this.sourceid)) {
	...
	public final boolean isSoaring_Mount() {
		return this.skill && (GameConstants.isBeginnerJob(this.sourceid / 10000) && this.sourceid % 10000 == 1142
