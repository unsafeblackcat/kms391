...
function action(mode, type, sel) {
	...
	if (guild1 != null) {
		if (guild1 == cm.getPlayer().getGuild()) {
			if (!guild1R) {
				if (status == 0) {
					var msg = "#fs15##fn나눔고딕#[컨텐츠 :: 길드 경쟁전]#fs15#"+enter;
					...
					switch (seld) {
						case 2:
							...
							msg += "#r#fs15#※주의! 지금 배팅하는 금액은 즉시 빠져나가며, 경쟁전에서 이기거나, 원정대가 해체되는 경우를 제외하곤 절대 돌려받을 수 없으니 신중하게 선택해주세요.";
		...
		if (status == 0) {
			var msg = "#fs15##fn나눔고딕#[컨텐츠 :: 길드 경쟁전]#fs15##b"+enter;
