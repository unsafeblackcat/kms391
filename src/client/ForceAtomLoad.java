...
public class ForceAtomLoad {
	...
	private final Map<Integer, ForceAtomLoad> forceAtomlist = new ConcurrentHashMap<>();

	public void setForceAtomlist(Map<Integer, ForceAtomLoad> forceAtomlist) {
		this.forceAtomlist.clear();
		this.forceAtomlist.putAll(forceAtomlist); // 线程安全更新
	}
	...
	public static void loadFromWZData() {
		ForceAtomlist.clear();
		MapleData effdata = MapleDataProviderFactory.getDataProvider(MapleDataProviderFactory.fileInWZPath("Effect.wz"))
				.getData("CharacterEff.img");
				...
				for (MapleData ForceAtomInfo : dataType.getChildren()) {
					...
					for (MapleData ForceAtomTypeInfo : ForceAtomInfo.getChildren()) {
						if (ForceAtomTypeInfo.getName().equals("info")) {
							...
							ForceAtomLoad fal = new ForceAtomLoad(firstImpactMin, firstImpactMax, secondImpactMin,
									secondImpactMax);
