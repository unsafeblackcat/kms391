...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (!cm.haveItem(2430215, 1)) {
            cm.sendOk("#fs15#" + 검정 + "#i2430215# #z2430215# 아이템이 없는거 같군.");
            cm.dispose();
            return;
        }
        if (selection == 0) {
            ...
            if (!c) {
                cm.sendOkS("#fs15#" + 검정 + "이봐, #h # 각인서를 사용할 안드로이드가 없는거 같네.", 0x04, 9000030);
                cm.dispose();
            } else {
        ...
        txt += "";
        cm.sendYesNoS(txt, 0x04, 9000030);
    } else if (status == 3) {
        ...
        if (selection == 0) {
            ...
            NPCScriptManager.getInstance().startItem(cm.getPlayer().getClient(), 9000030, "consume_2430215");
        } else if (selection == 1) {
            cm.getClient().removeClickedNPC();
            cm.dispose();
        }
