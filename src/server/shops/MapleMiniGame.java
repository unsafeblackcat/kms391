...
public class MapleMiniGame extends AbstractPlayerStore {
	...
	public void checkExitAfterGame() {
		for (int i = 0; i < 2; i++) {
			if (this.exitAfter[i]) {
				this.exitAfter[i] = false;
				((i == 0) ? getMCOwner() : this.chrs[i - 1].get()).getClient().getSession()
						.writeAndFlush(PlayerShopPacket.shopErrorMessage(0, i));
	...
	public void setPoints(int i, int type) {
		...
		if (z != null) {
			...
			z.getQuestNAdd(MapleQuest.getInstance((this.GameType == 1) ? 122200 : 122210))
