...
public class Skill {
		...
		try {
			...
			if (level != null) {
				...
				for (final MapleData leve : data.getChildByPath("level")) {
					ret.getEffects().add(SecondaryStatEffect.loadSkillEffectFromData(leve, id, isBuff,
			...
			if (reqDataRoot != null) {
				for (final MapleData reqData : reqDataRoot.getChildren()) {
					ret.requiredSkill
							.add(new Pair<String, Integer>(reqData.getName(), MapleDataTool.getInt(reqData, 1)));
				}
			}
			ret.animationTime = 0;
			if (effect != null) {
					...
					if (info3 != null) {
						...
						if (listinfo4 != null) {
							for (final MapleData list3 : listinfo4.getChildren()) {
								skilllist.add(new Pair<Integer, Integer>(Integer.parseInt(list3.getName()),
										MapleDataTool.getInt(list3.getName(), listinfo4, -1)));
							}
						}
	...
	public boolean canBeLearnedBy(final MapleCharacter chr) {
		...
		return jid / 100 == skillForJob / 100 && jid / 1000 == skillForJob / 1000
				&& (!GameConstants.isCannon(skillForJob) || GameConstants.isCannon(job))
				&& (!GameConstants.isDemonSlayer(skillForJob) || GameConstants.isDemonSlayer(job))
				...
				&& (jid / 10 % 10 != 0 || skillForJob / 10 % 10 <= jid / 10 % 10)
				&& (skillForJob / 10 % 10 == 0 || skillForJob / 10 % 10 == jid / 10 % 10)
	...
	public boolean isSpecialSkill(final int skillId) {
		// 第一部分：小于等于5320007的判断
		if (skillId <= 5320007) {
			if (skillId == 5320007) {
				return true;
			...
			if (skillId > 4210012) {
				if (skillId > 5220012) {
					...
					return skillId == 5221022; // 修复点1：直接返回结果
				} else {
					if (skillId == 5220012) {
						return true;
					}
					...
					return skillId == 4340010; // 修复点2：直接返回结果
				}
			} else {
				if (skillId == 4210012) {
					return true;
				...
				if (skillId > 2221009) {
					...
					return skillId == 4110012; // 修复点3：直接返回结果
				} else {
					if (skillId == 2221009 || skillId == 1120012 || skillId == 1320011) {
						return true;
					}
					return skillId == 2121009; // 修复点4：直接返回结果
				}
			}
		}
		...
		if (skillId <= 21120014) {
			if (skillId == 21120014 || skillId == 5321004 || skillId - 5321003 + 1 == 2) {
				return true;
			}
			return skillId - 5321003 + 1 - 2 == 15799005;
		} else {
			if (skillId > 21121008) {
				return skillId == 22171069;
			}
			return skillId == 21121008 || (skillId >= 21120020 && skillId <= 21120021);
		}
	}
	...
	public boolean isFourthJob() {
		final int skillId = this.id;

		if (this.isSpecialSkill(skillId) || (skillId - 92000000 < 1000000 && skillId % 10000 == 0)
				|| this.isLegacySkill(skillId) || this.isVehicleSkill(skillId) || this.isCashShopSkill(skillId)
				|| this.isZeroBetaSkill(skillId)) {
			return false;
		}

		final int jobCategory = this.getJobCategory(skillId);
		final int jobTier = this.getJobTier(jobCategory);

		return jobCategory - 40000 > 5
				&& (this.hasSpecialSkillEffect(skillId) || (jobTier == 4 && !GameConstants.isZero(jobCategory)));
	}

	private static final Set<Integer> SPECIAL_EFFECT_SKILLS = Set.of(101100101, 101110203, 101120104, 4331002, 4311003,
			4321006, 4340007, 101000101, 4330009);

	private static final int SPECIAL_OFFSET_1 = 101120104, SPECIAL_OFFSET_2 = 101110102;

	/**
	 * 增强版特殊效果检测
	 */
	private boolean hasSpecialSkillEffect(int skillId) {
		return SPECIAL_EFFECT_SKILLS.contains(skillId) || isSpecialOffsetSkill(skillId);
	}

	private boolean isSpecialOffsetSkill(int skillId) {
		return skillId - SPECIAL_OFFSET_1 == 100 || skillId - SPECIAL_OFFSET_2 == 98;
	}

	private boolean isLegacySkill(int skillId) {
		final int LEGACY_SKILL_PREFIX = 92000000;
		final int LEGACY_SKILL_RANGE = 1000000;

		// 主判断逻辑：92000000-92999999范围内且能被10000整除的技能
		if (skillId - LEGACY_SKILL_PREFIX < LEGACY_SKILL_RANGE && skillId % 10000 == 0) {
			return true;
		}

		// 次级判断逻辑：处理类似92010000这样的衍生技能
		int baseSkillId = 10000 * (skillId / 10000);
		return baseSkillId - LEGACY_SKILL_PREFIX < LEGACY_SKILL_RANGE && baseSkillId % 10000 == 0;
	}

	/**
	 * 判断是否为坐骑/载具类技能
	 * 
	 * @param skillId 技能ID（如80001001）
	 * @return 是坐骑技能返回true
	 */
	private boolean isVehicleSkill(int skillId) {
		// 坐骑技能范围：800000-800999
		int category = skillId / 10000; // 获取前4位
		if (category == 8000) {
			category = skillId / 100; // 特殊处理8000前缀技能
		}
		return category >= 8000 && category <= 8099;
	}

	/**
	 * 判断是否为现金商城技能
	 * 
	 * @param skillId 技能ID（如95001001）
	 * @return 是商城技能返回true
	 */
	private boolean isCashShopSkill(int skillId) {
		// 商城技能特征：9500开头
		int category = skillId / 10000;
		return category == 9500;
	}

	private static final int BETA_CATEGORY_OFFSET = 40000, ZERO_CATEGORY = 6000, SPECIAL_CATEGORY = 13000;

	/**
	 * 增强版Zero测试技能检测 特征： 1. 职业类别超过40005 2. 符合特殊职业类别规则
	 */
	private boolean isZeroBetaSkill(int skillId) {
		int category = extractSkillCategory(skillId);
		return isOverThreshold(category) && isSpecialZeroCategory(category);
	}

	private int extractSkillCategory(int skillId) {
		return (skillId / 10000 == 8000) ? skillId / 100 : skillId / 10000;
	}

	private boolean isOverThreshold(int category) {
		return category - BETA_CATEGORY_OFFSET > 5;
	}

	private boolean isSpecialZeroCategory(int category) {
		return category == ZERO_CATEGORY || category == SPECIAL_CATEGORY || (category >= 2001 && category <= 2005)
				|| category % 1000 == 0;
	}
	 ...
	 * @return 职业类别代码
	 */
	private int getJobCategory(int skillId) {
		// 特殊处理8000系列的技能
		return (skillId / 10000 == 8000) ? skillId / 100 : skillId / 10000;
	}
	 ...
	 * 获取职业阶位
	 * 
	 * @param jobCategory 职业类别
	 * @return 职业阶位(1-4转)
	 */
	private int getJobTier(int jobCategory) {
		if (jobCategory % 100 == 0 || jobCategory == 501 || jobCategory == 3101) {
			return 1;
		} else if (jobCategory - 2200 < 100 || jobCategory == 2001) {
			// 处理特殊职业阶位
			return calculateSpecialTier(jobCategory);
		}
		return 0;
	}

	private int calculateSpecialTier(int jobCategory) {
		// 阶位计算逻辑实现
		switch (jobCategory) {
		case 2200:
		case 2210:
	...
	public static SecondAtom2 parseSecondAtom(final MapleData d, final int id) { // 이게 제가 짠건데
		...
		return new SecondAtom2(dataIndex, 0, createDelay, enableDelay, expire, 0, attackableCount, pos, rotate,
				aExtraPos, aCustom, localOnly, id);
	}
