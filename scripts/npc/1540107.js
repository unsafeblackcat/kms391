...
"#fc0xFF3B36CF##fs15#1我想確認一下輪椅名單.#k",
"#fc0xFF3B36CF##fs15#2我想確認一下輪椅名單.#k",
"#fc0xFF3B36CF##fs15#3我想確認一下輪椅名單.#k",
"#fc0xFF3B36CF##fs15#4我想確認一下輪椅名單.#k",
"#fc0xFF3B36CF##fs15#5我想確認一下輪椅名單.#k",
"#fc0xFF3B36CF##fs15#6我想確認一下輪椅名單.#k");
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        trade = "#fs15#"+검정+"저는 #fc0xFF000087#[FANCY]#k "+검정+"#b我是負責#k"+검정+"椅子銷售的Ryan.D。"
        trade += "\r\n每個選定的椅子可以購買 #r#z4319999# 10個#k"+검정+"可以通過購買.\r\n\r\n";
        trade += "現時擁有的 #d#z4319999##k 個數 : #e#r"+leaf+"#k個#n\r\n"
        trade += "#fc0xFFD5D5D5#───────────────────────────#k\r\n";
        for (var i = 0; i < itemCategorys.length; i++) {
            trade += "#L"+i+"#"+itemCategorys[i]+"#l\r\n";
        }
        cm.sendOkS(trade,0x86);
    } else if (status == 1) {
        ...
        if (check == 0) {
         ...
         msg = "'"+a.getFullYear()+"年 " + Number(a.getMonth() + 1) + "月 " + a.getDate() + "日 "+a.getHours()+"時 "+a.getMinutes()+"分 "+a.getSeconds()+"秒'\r\n";
         msg += "複製嘗試物品代碼（椅子商店）： "+itemsArray[select]+"\r\n";
         msg += "用戶角色ID： "+cm.getPlayer().getId()+"\r\n";
         msg += "使用者帳戶ID： "+cm.getPlayer().getAccountID()+"\r\n";
         out1.write(msg.getBytes());
         out1.close();
         cm.getPlayer().getWorldGMMsg(cm.getPlayer(), "試圖在椅子商店複製");
         cm.sendOk(itemsArray[select] + "#fs15##r檢查异常路徑.",9062004);
         cm.dispose();
          return;
        }
        var itemsArray = getArray(menuSelect);
        cm.sendYesNoS("#fs15##r真的要購買選擇的物品嗎？#k\r\n\r\n#b#i"+itemsArray[select]+"##z"+itemsArray[select]+"##k",0x86);
    } else if (status == 3) {
        var itemsArray = getArray(menuSelect);
        if (cm.haveItem(4319999, 10) && cm.canHold(itemsArray[select])) {
            cm.gainItem(4319999, -10);
	        cm.gainItem(itemsArray[select], 1);
            말 = "#fs15##b#h ##k您選擇了適合您的椅子！ 有很多多樣的椅子，請再來看看。\r\n\r\n"
            말 += "#fUI/UIWindow2.img/QuestIcon/4/0#\r\n"
            말 += "#i"+itemsArray[select]+"# #b#z"+itemsArray[select]+""
            cm.sendOkS(말,0x86);
            cm.dispose();
        } else {
            cm.sendOkS("#fs15##i4310018# #b#z4310018##k 項目好像不足或沒有。",0x86);
            cm.dispose();
            return;
        }
    }
