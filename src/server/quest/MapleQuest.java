...
public class MapleQuest implements Serializable {
	...
	private static MapleQuest loadQuest(ResultSet rs, PreparedStatement psr, PreparedStatement psa,
		...
		while (rse.next()) {
			...
			((List) ret.partyQuestInfo.get(rse.getString("rank"))).add(new Pair<>(rse.getString("mode"),
					new Pair<>(rse.getString("property"), Integer.valueOf(rse.getInt("value")))));
		}
		rse.close();
		return ret;
	}
	...
	public void complete(MapleCharacter c, int npc, Integer selection, boolean acc) {
		...
		if (c.getMap() != null && (this.autoPreComplete || checkNPCOnMap(c, npc))
				&& canComplete(c, Integer.valueOf(npc))) {
	...
	public void forceStart(MapleCharacter c, int npc, String customData, boolean acc) {
		...
		newStatus.setCompletionTime(
				acc ? c.getClient().getQuest(this).getCompletionTime() : c.getQuest(this).getCompletionTime());
	...
	public void forceComplete(MapleCharacter c, int npc, boolean update, boolean acc) {
		if (getId() != 100855
				&& ((getId() >= 100829 && getId() <= 100863) || (getId() >= 501460 && getId() <= 501592))) {
		...
		case 16408:
			...
			c.setKeyValue(500862, "str", "일일 미션 " + (c.getKeyValue(500862, "openBox") + 1L) + "개 완료! "
					+ (c.getKeyValue(500862, "openBox") + 1L) + "번째 상자를 클릭하세요!");
	...
	public enum MedalQuest {
		...
		Sleepywood(29014, 29015, 50, new int[] { 105040300, 105070001, 105040305, 105090200, 105090300, 105090301,
				105090312, 105090500, 105090900, 105080000 }, "슬리피우드");
