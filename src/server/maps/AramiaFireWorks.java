...
public class AramiaFireWorks {
	...
	private static final int[] arrayMob = new int[] { 9500168, 9500169, 9500170, 9500171, 9500173, 9500174, 9500175,
			9500176, 9500170, 9500171, 9500172, 9500173, 9500174, 9500175, 9400569 };

	private static final int[] arrayX = new int[] { 2100, 2605, 1800, 2600, 3120, 2700, 2320, 2062, 2800, 3100, 2300,
			2840, 2700, 2320, 1950 };

	private static final int[] arrayY = new int[] { 574, 364, 574, 316, 574, 574, 403, 364, 574, 574, 403, 574, 574,
			403, 574 };

	private static final int[] array_X = new int[] { 720, 180, 630, 270, 360, 540, 450, 142, 142, 218, 772, 810, 848,
			232, 308, 142 };

	private static final int[] array_Y = new int[] { 1234, 1234, 1174, 1234, 1174, 1174, 1174, 1260, 1234, 1234, 1234,
			1234, 1234, 1114, 1114, 1140 };
	...
	private final void broadcastServer(MapleCharacter c, int itemid) {
		World.Broadcast.broadcastMessage(CWvsContext.serverNotice(6, "",
				"<Channel " + c.getClient().getChannel() + "> " + c.getMap().getMapName() + " : The amount of {"
						+ MapleItemInformationProvider.getInstance().getName(itemid) + "} has reached the limit!"));
	}
	...
	public final void giveSuns(MapleCharacter c, int kegs) {
		 ...
		 * MapleReactor reactor = map.getReactorByName("mapleTree"); for (int gogo =
		 * kegs + 500; gogo > 0; gogo -= 500) { switch (reactor.getState()) { case 0:
		 * case 1: case 2: case 3: case 4: if (this.sunshines >= 500 * (2 +
		 * reactor.getState())) { reactor.setState((byte)(reactor.getState() + 1));
		 * reactor.setTimerActive(false);
		 * map.broadcastMessage(CField.triggerReactor(reactor, reactor.getState(),
		 * c.getId())); } break; default: if (this.sunshines >= 500)
		 * map.resetReactors(c.getClient()); break; } }
		 */
	 ...
	 * map.startMapEffect("The tree is bursting with sunshine!", 5121010); for (int
	 * i = 0; i < 3; i++) { Timer.EventTimer.getInstance().schedule(new Runnable() {
	 * public final void run() { //AramiaFireWorks.this.spawnItem(map); } }, (5000 +
	 * i * 10000)); } }
	 */
