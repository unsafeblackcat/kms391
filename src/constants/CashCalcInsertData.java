		...
		for (int i = 0; i < strs.length; i++) {
			...
			for (Pair<Integer, String> itemPair : MapleItemInformationProvider.getInstance().getAllItems()) {
				if (((String) itemPair.getRight()).toLowerCase().equals(data[0])) {
					retItems.add(new Triple<>((new StringBuilder()).append(itemPair.getLeft()).append("").toString(),
							itemPair.getRight(), data[1]));
				}
			}
			...
			if (retItems != null && retItems.size() > 0) {
				if (retItems.size() != 1) {
					...
					for (int z = 0; z < retItems.size(); z++) {
						System.out.println((z + 1) + ". " + (String) ((Triple) retItems.get(z)).getLeft() + " - "
								+ (String) ((Triple) retItems.get(z)).getMid());
					}
					int geti = sc.nextInt();
					if (geti != -1) {
