...
function action(mode, type, sel) {
	...
	if (status == 0) {
		var 게임약관 = "#fs11#       팬시 더 리미티드 전직 도우미\r\n";
		게임약관 += "" + 회색 + "────────────────────────────\r\n";
		...
		if (seld >= 1 && seld <= 5) {
			...
			cm.getPlayer().AutoTeachSkill();
			max = seld == 2500 || seld == 430 ? 260 : 260;
			if (seld != 10112) {
			...
			cm.warp(993225700, 0);
			cm.getPlayer().resetStats(4, 4, 4, 4);
         cm.getPlayer().dropMessage(1, "[팬시] 초기 지원 상자가 지급 되었습니다. 인벤토리를 확인해 주세요 !");
         cm.getPlayer().dropMessage(5, "[팬시] 초기 지원 상자가 지급 되었습니다. 인벤토리를 확인해 주세요 !");
			cm.dispose();
		...
		if (adv) {
			...
			if (sel != 330) {
				...
				for (var i = cm.getPlayer().getLevel(); i < 260; i++) {
				cm.gainExp(Packages.constants.GameConstants.getExpNeededForLevel(i));
			}
			cm.getPlayer().setReborns(sel);
			cm.getPlayer().AutoTeachSkill();
...
function SecondJob(i) {
	var adventure = "#fs11##b#h ##k 용사님! #b"+서버이름+" 서버#k에서 즐길 직업을 선택해주세요!" + enter;
	switch (i) {
		case 1:
			//adventure += "너는 #b전사#k를 택했군!\r\n";
			adventure += "#L110# #r파이터#k로 #b"+서버이름+" 서버#k에 참여할게!\r\n";
			adventure += "#L120# #r페이지#k로 #b"+서버이름+" 서버#k에 참여할게!\r\n";
			adventure += "#L130# #r스피어맨#k으로 #b"+서버이름+" 서버#k에 참여할게!\r\n";
			break;
		case 2:
			//adventure += "현재 당신이 선택한 직업군은 #b마법사#k 입니다.\r\n";
			adventure += "#L210# #r불,독위자드#k로 #b"+서버이름+" 서버#k에 참여할게!\r\n";
			adventure += "#L220# #r썬,콜위자드#k로 #b"+서버이름+" 서버#k에 참여할게!\r\n";
			adventure += "#L230# #r클레릭#k으로 #b"+서버이름+" 서버#k에 참여할게!\r\n";
			break;
		case 3:
			adventure += "현재 당신이 선택한 직업군은 #b궁수#k 입니다.\r\n";
			adventure += "#L310# #r헌터#k로 #b"+서버이름+" 서버#k에 참여할게!\r\n";
			adventure += "#L320# #r사수#k로 #b"+서버이름+" 서버#k에 참여할게!\r\n";
			//adventure += "#L330# #r패스파인더#k로 #b"+서버이름+" 서버#k에 참여할게!\r\n";
			break;
		case 4:
			//adventure += "현재 당신이 선택한 직업군은 #b도적#k 입니다.\r\n";
			adventure += "#L410# #r어쌔신#k로 #b"+서버이름+" 서버#k에 참여할게!\r\n";
			adventure += "#L420# #r시프#k로 #b"+서버이름+" 서버#k에 참여할게!\r\n";
			break;
		case 5:
			//adventure += "현재 당신이 선택한 직업군은 #b해적#k 입니다.\r\n";
			adventure += "#L510# #r인파이터#k로 #b"+서버이름+" 서버#k에 참여할게!\r\n";
			adventure += "#L520# #r건슬링거#k로 #b"+서버이름+" 서버#k에 참여할게!\r\n";
			break;
	}
	return adventure;
}
