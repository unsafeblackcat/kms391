...
function action(mode, type, sel) {
    ...
    if (status == 0) {
        ...
        var msg = "#fs15# 您好。#e카오#n 推廣積分發放助手在此。\r\n " + enter;
        msg += "目前 #b#h ##k 先生/女士的推廣積分：#b" + cm.getPlayer().getHPoint() + "P#k" + enter;
        msg += "目前 #b#h ##k 先生/女士的推廣等級：#b" + cm.getPlayer().getPgrades() + "#k#b" + enter;
        msg += "目前 #b#h ##k 先生/女士的累積次數：#b" + cm.getPlayer().getClient().getKeyValue("PNumber") + "회#k#b" + enter + enter;
        msg += "#L1#領取推廣積分" + enter;
        msg += "#L2#查詢推廣積分排名"+enter;
        msg += "#L5#推廣積分商店" + enter;
		msg += "#L8#領取累積次數獎勵" + enter;
        msg += "#L7#領取累積次數獎勵"+enter;
...
function getRanking(asd) {
    var ret = "#fs15#랭킹은 최대 " + asd + "명까지만 보여집니다.\r\n";
    var as = 0;
    var names = [];
...
function getList() {
    ...
    if (ret.equals("")) return "#fs15#현재 수령 가능한 홍보 보상이 없습니다.";
    return ret;
}
...
function getList2(n) {
    ...
    while (rs.next()) {
        ...
        ret += "#fs14#닉네임 : #b" + rs.getString("name") + "#k #fs15#지급 날짜 : #d" + rs.getString("date") + "#k \r\n";
