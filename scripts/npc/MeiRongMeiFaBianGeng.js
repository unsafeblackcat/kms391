...
틀 = "#fs15##fUI/Basic.img/actMark/22#";
髮型 = "#k 髮型 #r";
面型 = "#k 面型 #r";
顔色 = "#k 顔色 #r";
緩存 = "#k 緩存 #r";
...
function action(mode, type, sel) {
	...
	if (status == 0) {
        ...
        var msg = "                    " + 틀 + "#l\r\n";
                msg += "#fc0xFFD5D5D5#──────────────────────────#l\r\n";
		msg += "#L1##fc0xFFFF3636#" + 髮型 + "#l";
		msg += "#L2##fc0xFFFF3636#" + 面型 + "#l";
                msg += "#L3##fc0xFFFF3636#" + 顔色 + "#l\r\n";
		msg += "#L4##fc0xFFFF3636#" + 緩存 + "#l";
		msg += "#L5##fc0xFFFF3636#" + 特別的 + "#l";
		...
		if (sel == 1) {
			...
			msg += "\r\n#fs15#";
			msg += "#L20# #d普通染色#k 夏至 #l\r\n";
			msg += "#L21# #d鏡片染色#k 更改 #l\r\n";
			msg += "#L22# #d膚色#k 更改 #l\r\n";
			msg += "#L23# 角色 #d染色#k 更改 #l\r\n";
			msg += "#L24# 角色 染色 #d重置#k夏至#l\r\n";
			msg += "#L25# #b混合鏡頭#k/#r染色#k 購買 #l\r\n";
			cm.sendSimple(msg);
		...
		if (sel == -1) {
			...
			beauty = 1;
			cm.sendGetNumber("#fs15#輸入想要更改的#r染色#k。\r\n現有染色 #d'0'#k  (0 ~ 1000)", 0, 0, 1000);
		} else if (sel == 24) {
		...
		if(seld >= 12 && seld <= 16) {
			selStr = "#fn돋움##fc0xFFFFFFFF#完全可以換成新的風格。 如果你厭倦了現在的樣子，就慢慢考慮你想換的髮型吧。";
			cm.sendStyle(selStr, codyList);
		}
		if (beauty == 1) {
			if (sel < 0 && sel > 1000) {
				cm.sendOk("您輸入的顏色值不正確.");
				cm.dispose();
				return;
			}
