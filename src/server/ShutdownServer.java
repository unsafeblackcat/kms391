...
public class ShutdownServer implements ShutdownServerMBean {
		...
		public void run() {
			...
			System.out.println(
					"[加載已完成] " + this.LoadingThreadName + " | 使用了 " + (System.currentTimeMillis() - this.StartTime)
							+ " 毫秒. (" + (ShutdownServer.CompletedLoadingThreads.get() + 1) + "/10)");
	...
	private class AllShutdown extends Thread {
		...
		public void run() {
			...
			System.out.println("[Fully Shutdowned in "
					+ (System.currentTimeMillis() - ShutdownServer.this.startTime) / 1000L + " seconds]");
