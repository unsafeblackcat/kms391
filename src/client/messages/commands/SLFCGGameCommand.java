...
public class SLFCGGameCommand {
	...
	public static class SetBingoStep extends CommandExecute {// 宾果游戏管理系统
			...
			if (splitted.length > 1) {
				...
				case "1":
					if (c.getPlayer().getMapId() != 922290000) {
						c.getSession()
								.writeAndFlush(SLFCGPacket.OnYellowDlg(1052230, 3000, "#face1#빙고 대기맵에서 실행해 주세요", ""));
						return 0;
					}
					if (c.getPlayer().getMap().getCharacters().size() < 1) {
						c.getSession().writeAndFlush(
								SLFCGPacket.OnYellowDlg(1052230, 3000, "#face1#빙고 최소 등록 인원은 30명 입니다", ""));
						return 0;
					}
					...
					if (splitted.length == 2) {
						...
						for (Pair<Integer, Integer> item : BingoGame.items) {
							c.getPlayer().dropMessage(6,
									MapleItemInformationProvider.getInstance().getName(((Integer) item.left).intValue())
											+ "아이템 " + item.right + "개");
						}
						return 0;
					}
					BingoGame.items.add(new Pair<>(Integer.valueOf(Integer.parseInt(splitted[2])),
							Integer.valueOf(Integer.parseInt(splitted[3]))));
					c.getPlayer().dropMessage(-7,
							MapleItemInformationProvider.getInstance().getName(Integer.parseInt(splitted[2])) + "아이템 "
									+ splitted[3] + "개가 아이템 리스트에 등록되었습니다.");
					return 0;
				case "4":
					if (splitted.length == 2) {
		...
		public int execute(final MapleClient c, String[] splitted) {
			if (splitted.length > 1) {
				...
				case "0":
					...
					if (c.getPlayer().getMap().getCharacters().size() < 1) {
						c.getSession()
								.writeAndFlush(SLFCGPacket.OnYellowDlg(1052230, 3000, "#face1#OX 최소 등록 인원 30명", ""));
						return 0;
					}
				...
				case "1":
					...
					break;
				case "2":
					channel = (c.getChannel() == 1) ? "1"
							: ((c.getChannel() == 2) ? "20세 이상" : String.valueOf(c.getChannel() - 1));
					for (ChannelServer cs : ChannelServer.getAllInstances()) {
						for (MapleCharacter chr : cs.getPlayerStorage().getAllCharacters().values()) {
							chr.getClient().getSession().writeAndFlush(SLFCGPacket.OnYellowDlg(3003301, 7000,
									"#face3#운영자가 OX퀴즈 게임 참여자를 모집 중이야! " + channel + "채널에서 깸디를 클릭해봐~", ""));
						}
					}
					break;
				}
			...
			if (splitted.length > 1) {
				...
				case "0":
					if (c.getPlayer().getMapId() != 993022000) {
						c.getSession().writeAndFlush(
								SLFCGPacket.OnYellowDlg(1052230, 3000, "#face1#암호 추리 대기맵에서 실행해 주세요", ""));
						return 0;
					}
					if (c.getPlayer().getMap().getCharacters().size() < 30) {
						c.getSession().writeAndFlush(
								SLFCGPacket.OnYellowDlg(1052230, 3000, "#face1#암호 추리 최소 등록 인원은 30명 입니다", ""));
						return 0;
					}
