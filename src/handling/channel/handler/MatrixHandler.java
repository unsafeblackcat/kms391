...
public class MatrixHandler {
					...
					for (final MapleData d : dat) {
						...
						if (!jobs.contains("none")) {
							...
							if (d.getChildByPath("spCoreOption") != null) {
								...
								spOption.setHeal_percent(
										MapleDataTool.getInt("spCoreOption/effect/heal_percent", d, 0));
								spOption.setReducePercent(
										MapleDataTool.getInt("spCoreOption/effect/reducePercent", d, 0));
							}
							final Core core = new Core(-1L, coreid, 0, 1, 0, 1, maxlevel, skillid, skillid2, skillid3,
									-1, spOption);
							...
							case 1: {
								MatrixHandler.activeCores.add(new Pair<Core, List<String>>(
										new Core(-1L, coreid, 0, 1, 0, 1, maxlevel, skillid, 0, 0, -1, spOption),
										jobs));
								continue;
							}
							case 2: {
								MatrixHandler.passiveCores.add(new Pair<Core, List<String>>(
										new Core(-1L, coreid, 0, 1, 0, 1, maxlevel, skillid, 0, 0, -1, spOption),
										jobs));
								continue;
							}
							case 3: {
								MatrixHandler.specialCores.add(new Pair<Core, List<String>>(
										new Core(-1L, coreid, 0, 1, 0, 1, maxlevel, skillid, 0, 0, -1, spOption),
										jobs));
								continue;
							}
	...
	public static boolean ResetCore(final MapleClient c, final Pair<Core, List<String>> origin,
			final Pair<Core, List<String>> fresh, final boolean checkjob) {
		return origin.getLeft().getCoreId() == fresh.getLeft().getCoreId()
				|| fresh.getLeft().getCoreId() / 10000000 != 2
				|| (origin.getLeft().getSkill1() == fresh.getLeft().getSkill1() && origin.getLeft().getSkill1() != 0
						&& fresh.getLeft().getSkill1() != 0)
				|| (origin.getLeft().getSkill2() == fresh.getLeft().getSkill2() && origin.getLeft().getSkill2() != 0
						&& fresh.getLeft().getSkill2() != 0)
				|| (origin.getLeft().getSkill3() == fresh.getLeft().getSkill3() && origin.getLeft().getSkill3() != 0
						&& fresh.getLeft().getSkill3() != 0)
				|| (checkjob && !CheckUseableJobs(origin.getRight(), fresh.getRight())) || !checkUseableJobs(fresh, c);
	}

	public static boolean ResetCore(final MapleClient c, final Pair<Core, List<String>> origin,
			final Pair<Core, List<String>> fresh, final Pair<Core, List<String>> fresh2, final boolean checkjob) {
		return origin.getLeft().getCoreId() == fresh.getLeft().getCoreId()
				|| fresh.getLeft().getCoreId() / 10000000 != 2
				|| (origin.getLeft().getSkill1() == fresh.getLeft().getSkill1() && origin.getLeft().getSkill1() != 0
						&& fresh.getLeft().getSkill1() != 0)
				...
				|| (origin.getLeft().getSkill3() == fresh.getLeft().getSkill3() && origin.getLeft().getSkill3() != 0
						&& fresh.getLeft().getSkill3() != 0)
				|| (origin.getLeft().getSkill1() == fresh2.getLeft().getSkill1() && origin.getLeft().getSkill1() != 0
						&& fresh2.getLeft().getSkill1() != 0)
				|| (origin.getLeft().getSkill2() == fresh2.getLeft().getSkill2() && origin.getLeft().getSkill2() != 0
						&& fresh2.getLeft().getSkill2() != 0)
				|| (origin.getLeft().getSkill3() == fresh2.getLeft().getSkill3() && origin.getLeft().getSkill3() != 0
						&& fresh2.getLeft().getSkill3() != 0)
				|| (checkjob && !CheckUseableJobs(origin.getRight(), fresh.getRight())) || !checkUseableJobs(fresh, c);
	}
	...
	public static void UseCoreJamStone(final MapleClient c, final int itemid, final int value, final int kind) {
		...
		for (int count = 0; count < value; count++) {
			if (c.getPlayer().getCore().size() >= 500) {
				c.getPlayer().dropMessage(1, "覈心最多可持有500個.");
			...
			if (c.getPlayer().getLevel() < 200 || GameConstants.isYeti(c.getPlayer().getJob())
					|| GameConstants.isPinkBean(c.getPlayer().getJob())) {
				return;
			}
			...
			if (c.getPlayer().haveItem(itemid)) {
				...
				final Core core = new Core(crcId, skill1.getLeft().getCoreId(), c.getPlayer().getId(), 1, 0, 1,
						skill1.getLeft().getMaxlevel(), skill1.getLeft().getSkill1(),
						(skill2 == null) ? 0 : skill2.getLeft().getSkill1(),
						(skill3 == null) ? 0 : skill3.getLeft().getSkill1(), -1, spCore);
				if (sp) {
	...
	public static void UseMirrorCoreJamStone(final MapleClient c, final int itemid, final int value, final int kind) {
		...
		for (int count = 0; count < value; count++) {
			...
			if (c.getPlayer().haveItem(itemid)) {
				MapleInventoryManipulator.removeById_Lock(c, GameConstants.getInventoryType(itemid), itemid);
				final Core core = new Core(crcId, 10000024, c.getPlayer().getId(), 1, 0, 1, 25, 400001039, 0, 0, -1,
						null);
				cores.add(core);
				c.getPlayer().getCore().add(core);
				core.setId(c.getPlayer().getCore().indexOf(core));
	...
	public static void UseCraftCoreJamStone(final MapleClient c, final int itemid, final int value, final int kind) {
		...
		for (int count = 0; count < value; count++) {
			...
			if (c.getPlayer().haveItem(itemid)) {
				MapleInventoryManipulator.removeById_Lock(c, GameConstants.getInventoryType(itemid), itemid);
				final Core core = new Core(crcId, 10000031, c.getPlayer().getId(), 1, 0, 1, 25, 400001059, 0, 0, -1,
						null);
				cores.add(core);
				c.getPlayer().getCore().add(core);
				core.setId(c.getPlayer().getCore().indexOf(core));
	...
	public static void gainVCoreLevel(final MapleCharacter player) {
		for (final Pair<Core, List<String>> coreskill : MatrixHandler.activeCores) {
			if (checkOwnUseableJobs(coreskill, player.getClient())) {
				final Core core = new Core(Randomizer.nextLong(), coreskill.getLeft().getCoreId(), player.getId(), 1, 0,
						1, coreskill.getLeft().getMaxlevel(), coreskill.getLeft().getSkill1(), 0, 0, -1,
						coreskill.getLeft().getSpCoreOption());
	...
	public static void updateCore(final LittleEndianAccessor slea, final MapleClient c) {
		...
		case 3: { // 코어 교체 1.2.390 OK.
			...
			if (c.getPlayer().getCooldownLimit(targetCore.getSkill1()) > 0L
					|| (sourceCore != null && c.getPlayer().getCooldownLimit(sourceCore.getSkill1()) > 0L)) {
				c.getPlayer().dropMessage(6, "재사용 대기시간 중인 코어는 해제할 수 없습니다.");
				return;
			}
		...
		case 4: { // 코어 강화 1.2.390 OK.
			...
			c.getSession().writeAndFlush(
					(Object) CWvsContext.OnCoreEnforcementResult(target, exp, prevLevel, core3.getLevel()));
			break;
		}
			...
			for (final Pair<Core, List<String>> skill1 : cores) {
				if (skill1.getLeft().getCoreId() == coreId) {
					if (nCount == 1) {
						...
						final Core core5 = new Core(Randomizer.nextLong(), skill1.getLeft().getCoreId(),
								c.getPlayer().getId(), 1, 0, 1, skill1.getLeft().getMaxlevel(),
								skill1.getLeft().getSkill1(), (skill2 == null) ? 0 : skill2.getLeft().getSkill1(),
								(skill3 == null) ? 0 : skill3.getLeft().getSkill1(), -1,
								skill1.getLeft().getSpCoreOption());
						...
						c.getPlayer().setKeyValue(1477, "count",
								String.valueOf(c.getPlayer().getKeyValue(1477, "count") - lostpont));
					} else {
						for (int i = 0; i < nCount; ++i) {
							...
							final Core core6 = new Core(Randomizer.nextLong(), skill1.getLeft().getCoreId(),
									c.getPlayer().getId(), 1, 0, 1, skill1.getLeft().getMaxlevel(),
									skill1.getLeft().getSkill1(), (skill4 == null) ? 0 : skill4.getLeft().getSkill1(),
									(skill5 == null) ? 0 : skill5.getLeft().getSkill1(), -1,
									skill1.getLeft().getSpCoreOption());
						...
						c.getPlayer().setKeyValue(1477, "count",
								String.valueOf(c.getPlayer().getKeyValue(1477, "count") - nCount * lostpont));
						c.getSession().writeAndFlush((Object) CWvsContext.ViewNewCore(skill1.getLeft(), nCount));
					}
				}
			}
			c.getPlayer().dropMessage(1, "覈心製作完成.");
			break;
		}
	...
	public static void calcSkillLevel(final MapleCharacter player, final int position) {
				...
				if (core.getPosition() >= 28) {// 코어 장착
					...
					if (core.getSkill1() != 0) {
						if (addSkills.containsKey(core.getSkill1())) {
							addSkills.put(core.getSkill1(), addSkills.get(core.getSkill1()) + core.getLevel()
									+ Math.max(0, matrix2.getLevel()));
						} else {
						...
						if (core.getSkill1() == 400051000 && (GameConstants.isStriker(player.getJob())
								|| GameConstants.isArk(player.getJob()) || GameConstants.isEunWol(player.getJob())
								|| GameConstants.isAngelicBuster(player.getJob())
								|| GameConstants.isXenon(player.getJob()))) {
							addSkills.put(400051001, core.getLevel() + Math.max(0, matrix2.getLevel()));
						}
					}
					if (core.getSkill2() != 0) {
						if (addSkills.containsKey(core.getSkill2())) {
							addSkills.put(core.getSkill2(), addSkills.get(core.getSkill2()) + core.getLevel()
									+ Math.max(0, matrix2.getLevel()));
						} else {
					...
					if (addSkills.containsKey(core.getSkill3())) {
						addSkills.put(core.getSkill3(),
								addSkills.get(core.getSkill3()) + core.getLevel() + Math.max(0, matrix2.getLevel()));
					} else {
		...
		for (final Map.Entry<Integer, Integer> addSkill : addSkills.entrySet()) {
			if (SkillFactory.getSkill(addSkill.getKey()) != null) {
				updateSkills.put(SkillFactory.getSkill(addSkill.getKey()), new SkillEntry(addSkill.getValue(),
						(byte) SkillFactory.getSkill(addSkill.getKey()).getMasterLevel(), -1L));
			}
		}
		player.changeSkillsLevel(updateSkills);
		if (position != -1) {
