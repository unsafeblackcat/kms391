	...
	public class MultiYutPlayer {
		...
		public void ThrowYut() {
			...
			for (MultiYutPlayer player : MultiYutGame.this.players) {
				player.getChr().getClient().send(SLFCGPacket.MultiYutGamePacket.ThrowYut(MultiYutGame.this.players,
						type - 1, MultiYutGame.this.whoTurn.getPosition(), onemore));
			}
		}
		...
		public void MoveHorse(int horsePos, int movecount, int yuttype) {
			...
			if (horse != null && horse.getInvposition() == horsePos) {
				...
				if (yuttype == 6) {
					...
					if (beforecount == 26 || beforecount == 21 || beforecount == 28 || beforecount == 1
							|| (beforecount == 20 && movecount == 29) || (beforecount == 23 && movecount == 27)
							|| (beforecount == 15 && movecount == 25)) {
						int moved = (beforecount == 1) ? 20
								: ((beforecount == 21) ? 5
										: ((beforecount == 26) ? 10
												: ((beforecount == 28) ? 23
														: ((beforecount == 20) ? 29
					...
					int startnum = (beforecount == 5) ? 21
							: ((beforecount == 10) ? 26 : ((beforecount == 23) ? 28 : 0));
					if (beforecount == 10) {
						if (yuttype >= 3) {
							horse.getLayout().add(Integer.valueOf(beforecount));
				...
				} else if ((beforecount == 21 || beforecount == 22 || beforecount == 24 || beforecount == 25)
						&& beforecount + yuttype > 25) {
				...
				for (MultiYutPlayer player : MultiYutGame.this.players) {
					player.getChr().getClient()
							.send(SLFCGPacket.MultiYutGamePacket.MovedHorse(MultiYutGame.this.players, this,
									horse.getLayout(), beforecount, MultiYutGame.this.whoTurn.getPosition(), horsePos,
									catched, onemore));
				}
			}
		}
		...
		if (multiYutMagchingQueue.size() == n) {
			...
			Timer.EtcTimer.getInstance().schedule(() -> {
				...
				for (MultiYutPlayer myp : myg.getPlayers()) {
					...
					myp.chr.getClient().getSession()
							.writeAndFlush(SLFCGPacket.MultiYutGamePacket.createUI(me, opponent));
				}
			}, 5000L);
		}
	}
	...
	public static void addQueue(MapleCharacter mapleCharacter, int n) {
		if (mapleCharacter.skillisCooling(100668) && !mapleCharacter.isGM()) {
			...
			mapleCharacter.getClient()
					.send(CField.NPCPacket.getNPCTalk(9062462, (byte) 0, "#r#e" + 초
			...
			mapleCharacter.getClient().send(CField.NPCPacket.getNPCTalk(9062462, (byte) 0, "잠시 후에 다시 시도해 주세요.", "00 00",
					(byte) 0, mapleCharacter.getId()));
		} else if (!multiYutMagchingQueue.contains(mapleCharacter)) {
			multiYutMagchingQueue.add(mapleCharacter);
			mapleCharacter.getClient()
					.send(SLFCGPacket.ContentsWaiting(mapleCharacter, 993189800, new int[] { 11, 2, 1, 18 }));
			if (multiYutMagchingQueue.size() == n) {
				for (MapleCharacter mapleCharacter2 : multiYutMagchingQueue) {
					mapleCharacter2.getClient()
							.send(SLFCGPacket.ContentsWaiting(mapleCharacter2, 0, new int[] { 19, 0, 1, 18 }));
				}
			}
		}
	}
