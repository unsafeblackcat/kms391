...
var status = -1;
var select = -19
var inz = new Array(1212131, 1222124, 1232124, 1242144, 1242145, 1262053, 1302359, 1312215, 1322266, 1332291, 1342110, 1362151, 1372239, 1382276, 1402271, 1412191, 1422199, 1432229, 1442287, 1452269, 1462254, 1472277, 1482234, 1492247, 1522154, 1532159, 1582046, 1272043, 1282043, 2048915, 1592037, 1292030, 1213030);
...
function action(mode, type, selection) {
	...
	if (GameConstants.isLuminous(cm.getPlayer().getJob())) {
		...
		txt += "#L27##i" + inz[27] + ":##z" + inz[27] + ":#\r\n";
	} else if (GameConstants.isIllium(cm.getPlayer().getJob())) {
		txt += "#L28##i" + inz[28] + ":##z" + inz[28] + ":#\r\n";
	} else if (GameConstants.isZero(cm.getPlayer().getJob())) {
		txt += "#L29##i" + inz[29] + ":##z" + inz[29] + ":#\r\n";
	} else if (GameConstants.isPathFinder(cm.getPlayer().getJob())) {
		txt += "#L30##i" + inz[30] + ":##z" + inz[30] + ":#\r\n";
	} else if (GameConstants.isHoyeong(cm.getPlayer().getJob())) {
		txt += "#L31##i" + inz[31] + ":##z" + inz[31] + ":#\r\n";
	} else if (GameConstants.isAdel(cm.getPlayer().getJob())) {
		txt += "#L32##i" + inz[32] + ":##z" + inz[32] + ":#\r\n";
	}
	txt += "\r\n#L100#전체 아이템 리스트를 본다.\r\n#L101#사용 취소";
	cm.sendSimple(txt);
    } else if (status == 1) {
