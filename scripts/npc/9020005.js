...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        st = selection;
        if (st == 3) {
            var talk = "오늘 남은 도전 횟수는 " + (3 - cm.getPlayer().getV(setting[0][0])) + "회야";
            cm.sendOk(talk);
            cm.dispose();
            return;
        }
        if (cm.getParty() == null) {
            ...
            cm.dispose();
            return;
        } else if (cm.getParty().getMembers().size() < 1 && !cm.getPlayer().isGM()) {
            cm.sendOk("파티원이 3명 미만이로군. 이곳은 위험한 곳이야. 적어도 레벨 200이상의 3명이상 파티원이 있어야 입장하실 수 있으니 다시 확인해 봐.");
            cm.dispose();
            return;
        ...
        if (!cm.isBossAvailable(setting[0][0], setting[0][1])) {
            ...
            talk += "#k#n님이 오늘 입장했습니다. [탈출] 파티퀘스트는 하루에 " + setting[0][1] + "번만 도전하실 수 있습니다.";
            cm.sendOk(talk);
            cm.dispose();
            return;
        } else {
