...
public class CWvsContext {
	...
	public static class InventoryPacket {
		...
		public static byte[] ItemSortLockCash(MapleInventoryType type, short slot, int Itemid, int quantity,
				boolean isLock) {
			MaplePacketLittleEndianWriter packet = new MaplePacketLittleEndianWriter();
			packet.writeShort(SendPacketOpcode.INVENTORY_OPERATION.getValue());
			packet.write(1);
		...
		public static byte[] ItemSortLockEtc(MapleInventoryType type, short slot, int Itemid, int quantity,
				boolean isLock) {
			MaplePacketLittleEndianWriter packet = new MaplePacketLittleEndianWriter();
			packet.writeShort(SendPacketOpcode.INVENTORY_OPERATION.getValue());
			packet.write(1);
		...
		public static byte[] ItemSortLockInstall(MapleInventoryType type, short slot, int Itemid, int quantity,
				boolean isLock) {
			MaplePacketLittleEndianWriter packet = new MaplePacketLittleEndianWriter();
			packet.writeShort(SendPacketOpcode.INVENTORY_OPERATION.getValue());
			packet.write(1);
		...
		public static byte[] ItemSortLockCody(MapleInventoryType type, short slot, int Itemid, int quantity,
				boolean isLock) {
			MaplePacketLittleEndianWriter packet = new MaplePacketLittleEndianWriter();
			packet.writeShort(SendPacketOpcode.INVENTORY_OPERATION.getValue());
			packet.write(1);
		...
		public static byte[] ItemSortLockEquip(MapleInventoryType type, short slot, int Itemid, int quantity,
				boolean isLock) {
			MaplePacketLittleEndianWriter packet = new MaplePacketLittleEndianWriter();
			packet.writeShort(SendPacketOpcode.INVENTORY_OPERATION.getValue());
			packet.write(1);
		...
		public static byte[] ItemSortLockConsume(MapleInventoryType type, short slot, int Itemid, int quantity,
				boolean isLock) {
			MaplePacketLittleEndianWriter packet = new MaplePacketLittleEndianWriter();
			packet.writeShort(SendPacketOpcode.INVENTORY_OPERATION.getValue());
			packet.write(1);
		...
		public static byte[] moveInventoryItem(MapleInventoryType type, short source, short target, boolean bag,
				boolean bothBag) {
			MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
			mplew.writeShort(SendPacketOpcode.INVENTORY_OPERATION.getValue());
	...
	public static class BuffPacket {
		...
		public static byte[] giveBuff(Map<SecondaryStat, Pair<Integer, Integer>> statups, SecondaryStatEffect effect,
				MapleCharacter chr) {
			return giveBuff(statups, effect, chr, new ArrayList<>());
		}

		public static byte[] giveBuff(Map<SecondaryStat, Pair<Integer, Integer>> statups, SecondaryStatEffect effect,
				MapleCharacter chr, List<Pair<SecondaryStat, Integer>> buffSkillid) {
			MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
			...
			for (Pair<SecondaryStat, Pair<Integer, Integer>> stat : newstatups) {
				if (!(stat.getLeft()).canStack() && !(stat.getLeft()).isSpecialBuff()
						&& !statups.containsKey(SecondaryStat.PinkbeanRollingGrade)) {
					if (SecondaryStat.isEncode4Byte(statups)
							|| (stat.getLeft() == SecondaryStat.IgnisRore && effect.getSourceId() == 400031017)) {
						mplew.writeInt(((stat.getRight()).left).intValue());
					} else {
						mplew.writeShort(((stat.getRight()).left).intValue());
					}

					if (stat.getLeft() == SecondaryStat.KinesisPsychicPoint
							|| ((SecondaryStat) stat.getLeft()).SpectorEffect()
							|| stat.getLeft() == SecondaryStat.HoyoungThirdProperty
							|| stat.getLeft() == SecondaryStat.TidalForce || stat.getLeft() == SecondaryStat.YetiAnger
							|| stat.getLeft() == SecondaryStat.YetiAngerMode) {
					...
					} else if ((stat.getLeft() == SecondaryStat.Buckshot || stat.getLeft() == SecondaryStat.PoseType)
							&& effect.getSourceId() == 11101032) {
					...
					} else if ((((SecondaryStat) stat.getLeft()).isItemEffect() && effect.getSourceId() != 135001009)
							|| !effect.isSkill() || SkillFactory.getSkill(effect.getSourceId()) == null) {
			...
			boolean unk = false;
			if (statups.containsKey(SecondaryStat.Etherealform) || statups.containsKey(SecondaryStat.Transform)
					|| statups.containsKey(SecondaryStat.IndieJointAttack)
					|| statups.containsKey(SecondaryStat.SwordOfSoulLight)
					|| statups.containsKey(SecondaryStat.RideVehicle) || statups.containsKey(SecondaryStat.SpreadThrow)
					|| statups.containsKey(SecondaryStat.Dike) || statups.containsKey(SecondaryStat.IceAura)
					|| statups.containsKey(SecondaryStat.Novility) || statups.containsKey(SecondaryStat.ThrowBlasting)
					|| statups.containsKey(SecondaryStat.DarknessAura)
					|| statups.containsKey(SecondaryStat.RoyalKnights)
					|| statups.containsKey(SecondaryStat.AbyssalLightning)
					|| statups.containsKey(SecondaryStat.PhotonRay)
					|| statups.containsKey(SecondaryStat.ThanatosDescent)
					|| statups.containsKey(SecondaryStat.DragonPang)
					|| statups.containsKey(SecondaryStat.SalamanderMischief)) {
				unk = true;
			}
			...
			for (Map.Entry<SecondaryStat, Pair<Integer, Integer>> stat : statups.entrySet()) {
				...
				int value = 1;
				if (statups.containsKey(SecondaryStat.NewFlying) && effect.getSourceId() == 36141502) { // 오리진 아티피셜 에볼루션
																										// 구현
					value = 0;
			...
			mplew.write(0);
			mplew.writeInt(0);
			mplew.write(statups.containsKey(SecondaryStat.SalamanderMischief) ? 33
					: (statups.containsKey(SecondaryStat.NewFlying) && effect.getSourceId() == 36141502) ? 1
							: statups.containsKey(SecondaryStat.Origin_Adrenaline_Surge) ? 1 : 0);
		...
		public static byte[] giveDisease(Map<SecondaryStat, Pair<Integer, Integer>> statups, MobSkill skill,
				MapleCharacter chr) {
			MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
			mplew.writeShort(SendPacketOpcode.GIVE_BUFF.getValue());
			...
			for (Pair<SecondaryStat, Pair<Integer, Integer>> pair : newstatups) {
				...
				if (SecondaryStat.isEncode4Byte(statups)) {
					if (pair.left == SecondaryStat.ReturnTeleport) {
						...
						mplew.writeInt(pair.getLeft().getX() != 0 ? pair.getLeft().getX()
								: ((Integer) pair.getRight().left).intValue());
					}
				} else {
		...
		public static void encodeIndieTempStat(MaplePacketLittleEndianWriter mplew,
				List<Pair<SecondaryStat, Pair<Integer, Integer>>> newstatups, MapleCharacter chr) {
			...
			List<Pair<SecondaryStat, List<SecondaryStatValueHolder>>> indiestatz = PacketHelper
					.sortIndieBuffStats(indiestats);
			for (Pair<SecondaryStat, List<SecondaryStatValueHolder>> indiestat : indiestatz) {
				...
				for (SecondaryStatValueHolder indie : indiestat.getRight()) {
					...
					if (indiestat.getLeft().isItemEffect() || !indie.effect.isSkill()
							|| SkillFactory.getSkill(indie.effect.getSourceId()) == null) {
		...
		private static void giveDice(MaplePacketLittleEndianWriter mplew, SecondaryStatEffect effect,
				MapleCharacter chr) {
		...
		public static byte[] cancelBuff(Map<SecondaryStat, Pair<Integer, Integer>> statups, MapleCharacter chr) {
			...
			mplew.write(statups.containsKey(SecondaryStat.Transform)
					|| statups.containsKey(SecondaryStat.IndieJointAttack)
					|| statups.containsKey(SecondaryStat.RideVehicle) || statups.containsKey(SecondaryStat.SpreadThrow)
					|| statups.containsKey(SecondaryStat.PoseType));
			mplew.writeInt(0);
			return mplew.getPacket();
		}

		public static byte[] giveForeignBuff(MapleCharacter chr, Map<SecondaryStat, Pair<Integer, Integer>> statups,
			...
			mplew.writeShort(0);
			mplew.write((statups.containsKey(SecondaryStat.Larkness) || statups.containsKey(SecondaryStat.Transform)
					|| statups.containsKey(SecondaryStat.BlessedHammer) || statups.containsKey(SecondaryStat.IceAura)));
			mplew.writeLong(0);
			return mplew.getPacket();
		}
		...
		public static byte[] cancelForeignBuff(MapleCharacter chr, Map<SecondaryStat, Pair<Integer, Integer>> statups) {
			...
			mplew.write((statups.containsKey(SecondaryStat.PoseType) || statups.containsKey(SecondaryStat.BlessedHammer)
					|| statups.containsKey(SecondaryStat.RideVehicle)));
			return mplew.getPacket();
		}
	}
	...
	public static class InfoPacket {
		...
		public static byte[] updateQuest(MapleQuestStatus quest) {
			...
			case 0:
				// mplew.writeMapleAsciiString((quest.getCustomData() != null) ?
				// quest.getCustomData() : ""); // 1.2.390 Changed.
		 ...
		 */
		public static byte[] GainEXP_Monster(MapleCharacter chr, long gain, boolean white, long flag, int eventBonusExp,
				int weddingExp, int partyExp, int itemEquipExp, int pcExp, int rainbowWeekExp, int boomupExp,
				int portionExp, int skillExp, int buffExp, int restExp, int itemExp, int valueExp, long bonusExp,
				int bloodExp, int iceExp, Pair<Integer, Integer> burningExp, int hpLiskExp, int fieldBonusExp,
				int eventBonusExp2, int fieldBonusExp2) {
			MaplePacketLittleEndianWriter packet = new MaplePacketLittleEndianWriter();
		...
		public static byte[] multiKill(int count, long exp) {// 修改記錄？？
			MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
			mplew.writeShort(SendPacketOpcode.SHOW_STATUS_INFO.getValue());
			mplew.write(35);
			mplew.write(0);
	...
	public static class GuildPacket {
		...
		public static byte[] guildEmblemChange(MapleCharacter chr, short bg, byte bgcolor, short logo, byte logocolor) { // 1.2.391
																															// OK.
			MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
			mplew.writeShort(SendPacketOpcode.GUILD_OPERATION.getValue());
		...
		public static byte[] newGuildHash() { // 1.2.391 OK.
			...
			mplew.writeInt(104);
			mplew.writeMapleAsciiString(
					"kvls4oCsy72x84NzHcMSYDE3cS4rAuGwzggoq_NCXbD~Lzn9b_zLCZjtrtQmpuPy5gvUR3~X6BGRXbZV0f7Vf~OsUY3IYxxyO8JgiHL7gazVu5WbkJWlJCjqBwG1h4V5IczqzdWuU4PQ_FK6gBgPii");
		...
		public static void addNewMemberData(MaplePacketLittleEndianWriter mplew, MapleGuildCharacter mgc) { // 1.2.391
																											// OK.
			mplew.writeInt(mgc.getId());
		...
		public static byte[] guildMemberOnline(MapleGuildCharacter mgc, int gid, int cid, boolean bOnline) { // 1.2.391
																												// OK.
			MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
			mplew.writeShort(SendPacketOpcode.GUILD_OPERATION.getValue());
		...
		public static byte[] genericGuildMessage(int code) {
			...
			if (code == 68 || code == 4 || code == 5 || code == 103 || code == 104 || code == 100 || code == 101
					|| code == 102) {
				mplew.writeMapleAsciiString("");
			}
		...
		public static byte[] showSearchGuildInfo(MapleCharacter chr, List<MapleGuild> gss, String text, byte mode,
				int option) { // 1.2.391 OK.
			MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
			mplew.writeShort(SendPacketOpcode.SEARCH_GUILD.getValue());
	...
	public static class PartyPacket {
		...
		public static void addPartyStatus(MapleCharacter chr, int forchannel, MapleParty party,
				MaplePacketLittleEndianWriter lew) {
			addPartyStatus(chr, forchannel, party, lew, false, false, (byte) 0);
		}

		public static void addPartyStatus(MapleCharacter chr, int forchannel, MapleParty party,
				MaplePacketLittleEndianWriter mplew, boolean leaving, boolean exped, byte type) {
			/*
			 * 0 : None 1 : 파티 가입 2 : 접속 상태 3 : 파티 탈퇴
			 */
			...
			if (type > 0) { // 파티 생성을 제외하고..
				...
				for (MaplePartyCharacter partychar : partymembers) {
					mplew.writeInt(
							partychar.getChannel() == forchannel ? partychar.getMapid() : partychar.getChannel());
				}
				...
				for (MaplePartyCharacter partychar : partymembers) {
					...
					mplew.writeInt(
							partychar.getChannel() == forchannel && !leaving ? partychar.getDoorPosition().x : 0);
					mplew.writeInt(
							partychar.getChannel() == forchannel && !leaving ? partychar.getDoorPosition().y : 0);
				}
		...
		public static byte[] updateParty(int forChannel, MapleParty party, PartyOperation op,
				MaplePartyCharacter target, MapleCharacter chr) {
			MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
			mplew.writeShort(SendPacketOpcode.PARTY_OPERATION.getValue());
	...
	public static class BuddylistPacket {
		...
		public static byte[] requestBuddylistAdd(int cidFrom, int accId, String nameFrom, int levelFrom, int jobFrom,
				MapleClient c, String groupName, String memo) {
			MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
			mplew.writeShort(SendPacketOpcode.BUDDYLIST.getValue());
			mplew.write(26); // 372
			mplew.write(1);
	...
	public static byte[] updatePlayerStats(final Map<MapleStat, Long> mystats, final boolean itemReaction,
			final boolean itemReaction2, final MapleCharacter chr, boolean isPet) {
...
//        mplew.write(0); // 1.2.372 ++
	...
	public static void DamageSkinInfo(MapleCharacter chr, MaplePacketLittleEndianWriter mplew) {
		...
		mplew.write(0);
		mplew.writeMapleAsciiString(
				MapleItemInformationProvider.getInstance().getName(skinid) + "이다.\\r\\n\\r\\n\\r\\n\\r\\n\\r\\n");
		mplew.writeInt(0);
		mplew.writeInt(-1);
		mplew.writeInt(0);
		mplew.write(1);
		...
		for (int i = 0; i < skinsize; i++) {
			...
			mplew.write(0);
			mplew.writeMapleAsciiString(
					MapleItemInformationProvider.getInstance().getName(GameConstants.getItemIdbyNum(skin))
							+ "이다.\\r\\n\\r\\n\\r\\n\\r\\n\\r\\n");
			mplew.writeInt(0);
		}
	}
	...
	public static byte[] charInfoOther(MapleCharacter chr) {
		...
		mplew.writeMapleAsciiString(((chr.getGuild() != null && chr.getGuild().getAllianceId() != 0)
				? World.Alliance.getAlliance(chr.getGuild().getAllianceId()).getName()
				: "")); // 길드 연합
	...
	public static byte[] updateLinkPreset(final MapleCharacter chr, final List<Pair<Integer, Integer>> list,
			final int preset) {
		final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	public static byte[] PsychicGrabAttack(int cid, int skillid, int subskillid, int unk, int id, byte a, int b,
			List<Integer> grab) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	public static byte[] PsychicGrabPreparation(MapleCharacter chr, int skillid, short level, int unk, int speed,
			int[] unk2, int[] mob, short[] unk3, byte[] unk4, Point[] pos1, Point[] pos2, Point[] pos3, Point[] pos4,
			Point[] pos5) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.PSYCHIC_GREP.getValue());
		mplew.writeInt(chr.getId());
	...
	public static byte[] UltimateMaterial(int code, int speed, int unk0, int skill, short level, byte unk1, short unk2,
			short unk3, short unk4, int posx, int posy) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	public static byte[] MatrixSkillMulti(MapleCharacter chr, int skillid, int level, int unk1, int unk2, int bullet,
			boolean data, List<Integer> datas, List<MatrixSkill> skills) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	public static byte[] showWeddingWishGiveToServerResult(List<String> wishes, MapleInventoryType type,
			List<Item> items) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.WEDDING_GIFT.getValue());
		mplew.write(11);
	...
	public static byte[] mixLense(int itemId, int baseFace, int newFace, boolean isDreeUp, boolean isBeta,
			boolean isAlphaBeta, MapleCharacter player) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	public static byte[] unionFreeset(MapleClient c, int presetNum) {
		...
		for (int i = 0; i < 8; i++) {
			mplew.writeInt((c.getCustomData(500627 + presetNum, i + "") == null) ? i
					: Integer.parseInt(c.getCustomData(500627 + presetNum, i + "")));
		}
		...
		for (MapleUnion chr : equipped) {
			...
			mplew.writeInt(chr.getJob());
			mplew.writeInt(chr.getUnk1());
			mplew.writeInt((presetNum == 4) ? chr.getPos4()
					: ((presetNum == 3) ? chr.getPos3()
							: ((presetNum == 2) ? chr.getPos2() : ((presetNum == 1) ? chr.getPos1() : chr.getPos()))));
			mplew.writeInt((presetNum == 4) ? chr.getPriset4()
					: ((presetNum == 3) ? chr.getPriset3()
							: ((presetNum == 2) ? chr.getPriset2()
									: ((presetNum == 1) ? chr.getPriset1() : chr.getPriset()))));
			mplew.writeInt(chr.getUnk3());
			mplew.writeInt(0); // 1.2.390 ++
			mplew.writeInt(0); // 1.2.390 ++
			mplew.writeMapleAsciiString(chr.getName());
	...
	public static byte[] equipmentEnchantResult(int op, Equip item, Equip item2, EquipmentScroll scroll,
			StarForceStats stats, int... args) {
		...
		case 50:
			...
			for (EquipmentScroll es : ess) {
				...
				mplew.writeInt(es.getName().contains("아크") ? 4
						: ((es.getName().contains("이노센트") || es.getName().contains("순백")) ? 1 : 0));
				mplew.writeInt(es.getFlag());

				mplew.writeInt(es.getName().contains("민첩") ? 352
						: es.getName().contains("행운") ? 328
								: es.getName().contains("지력") ? 336
										: es.getName().contains("힘") ? 324
												: es.getName().contains("체력") ? 328
														: es.getName().contains("올스텟") ? 60 : 0);
