...
public class ItemMakerHandler {
	...
	public static final void ItemMaker(LittleEndianAccessor slea, MapleClient c) {
		...
		case 1:
			...
			if (GameConstants.isGem(toCreate)) {
				...
				MapleInventoryManipulator.addById(c, randGemGiven, (short) (byte) ((taken == randGemGiven) ? 9 : 1),
						"Made by Gem " + toCreate + " on " + FileoutputUtil.CurrentReadable_Date());
				...
				c.getPlayer().getMap().broadcastMessage(c.getPlayer(),
						CField.EffectPacket.showItemMakerEffect(c.getPlayer(), 0, false), false);
				break;
			}
			...
			if (stimulator || numEnchanter > 0) {
				if (c.getPlayer().haveItem(create.getStimulator(), 1, false, true)) {
					MapleInventoryManipulator.removeById(c, MapleInventoryType.ETC, create.getStimulator(), 1, false,
							false);
				}
		...
		case 4:
			...
			if (!ii.isDropRestricted(itemId) && !ii.isAccountShared(itemId)) {
				...
				MapleInventoryManipulator.addById(c, arrayOfInt[0], (short) (byte) arrayOfInt[1],
	...
	private static final int checkRequiredNRemove(MapleClient c, List<Pair<Integer, Integer>> recipe) {
		...
		for (Pair<Integer, Integer> p : recipe) {
			if (!c.getPlayer().haveItem(((Integer) p.getLeft()).intValue(), ((Integer) p.getRight()).intValue(), false,
					true)) {
				return 0;
			}
		}
		for (Pair<Integer, Integer> p : recipe) {
			...
			MapleInventoryManipulator.removeById(c, GameConstants.getInventoryType(itemid), itemid,
					((Integer) p.getRight()).intValue(), false, false);
		}
		return itemid;
	}
	...
	public static final void UseBag(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
		...
		if (toUse == null || toUse.getQuantity() < 1 || toUse.getItemId() != itemId
				|| (itemId / 10000 != 433 && itemId / 10000 != 265 && itemId / 10000 != 308)) {
			c.getSession().writeAndFlush(CWvsContext.enableActions(c.getPlayer()));
			return;
		}
		boolean firstTime = !chr.getExtendedSlots().contains(Integer.valueOf(itemId));
		c.getSession().writeAndFlush(
				CField.openBag(chr.getExtendedSlots().indexOf(Integer.valueOf(itemId)), itemId, firstTime));
		c.getSession().writeAndFlush(CWvsContext.enableActions(c.getPlayer()));
	}
	...
	public static final void StartHarvest(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
		MapleReactor reactor = c.getPlayer().getMap().getReactorByOid(slea.readInt());
		if (reactor == null || !reactor.isAlive() || reactor.getReactorId() > 200015
				|| (chr.getStat()).harvestingTool <= 0
				|| reactor.getTruePosition().distanceSq(chr.getTruePosition()) > 10000.0D
				|| c.getPlayer().getFatigue() >= 100) {
			c.getPlayer().dropMessage(1, "當前無法進行採集.");
			return;
		}
	...
	public static final void CraftEffect(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
		...
		if (profession != null && (c.getPlayer().getProfessionLevel(profession.intValue()) > 0
				|| (profession.intValue() == 92040000 && chr.getMap().getExtractorSize() > 0))) {
			int time = slea.readInt();
			if (time > 6000 || time < 3000) {
				time = 4000;
			}
			c.getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, time, 0, 38,
					effect.endsWith("Extract") ? 1 : 0, 0, (byte) 0, true, null, effect, null));
			chr.getMap().broadcastMessage(chr, CField.EffectPacket.showEffect(chr, time, 0, 38,
					effect.endsWith("Extract") ? 1 : 0, 0, (byte) 0, false, null, effect, null), false);
		}
	}
	...
	public static final void CraftComplete(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
		...
		if (type == 1) {
			...
			if ((chr.getMapId() != 910001000 && (craftID != 92028000 || craftID != 92028001 || craftID != 92049000
					|| chr.getMap().getExtractorSize() <= 0)) || ce == null || chr.getFatigue() >= 200) {
				return;
			}
		...
		if (type == 1) {
			...
			for (int abc = 0; abc < itemsize; abc++) {
				...
				if (item == null) {
					item = c.getPlayer().getInventory(MapleInventoryType.getByType((byte) invtype))
							.getItem((short) position);
				}
				...
				MapleInventoryManipulator.addById(c, toGet, quantity,
						"Made by disassemble " + itemId + " on " + FileoutputUtil.CurrentReadable_Date());
				MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.EQUIP, item.getPosition(), (short) 1,
						false);
			}
		...
		if (craftID == 92028001) {
			...
			MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.EQUIP, item1.getPosition(), (short) 1,
					false);
			MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.EQUIP, item2.getPosition(), (short) 1,
					false);
			...
			for (Triple<Integer, Integer, Integer> triple : ce.targetItems) {
				if (!MapleInventoryManipulator.checkSpace(c, ((Integer) triple.left).intValue(),
						((Integer) triple.mid).intValue(), "")) {
					return;
				}
			}
			for (Map.Entry<Integer, Integer> e : ce.reqItems.entrySet()) {
				MapleInventoryManipulator.removeById(c,
						GameConstants.getInventoryType(((Integer) e.getKey()).intValue()),
						((Integer) e.getKey()).intValue(), ((Integer) e.getValue()).intValue(), false, false);
			}
						...
						if (Randomizer.nextInt(100) < ((Integer) triple.right).intValue()) {
							...
							if (ce.needOpenItem) {
								...
								if (mLevel == 1) {
									...
									sa.put(ce, new SkillEntry(2147483647, (byte) (chr.getMasterLevel(craftID) - 1),
											SkillFactory.getDefaultSExpiry(ce)));
								}
							}
							fatigue = ce.incFatigability;
							expGain = (ce.incSkillProficiency == 0)
									? ((fatigue * 20 - (ce.reqSkillLevel - theLevl) * 2) * 1)
