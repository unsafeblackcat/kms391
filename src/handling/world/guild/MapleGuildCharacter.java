...
public class MapleGuildCharacter implements Serializable {
	...
	public MapleGuildCharacter(int id, short lv, String name, byte channel, int job, byte rank, int guildContribution,
			byte allianceRank, int gid, boolean on, int lastattendance) {
		this.level = lv;
		this.id = id;
		this.name = name;
		if (on) {
