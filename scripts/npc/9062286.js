...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (selection == 0) { // 사냥이벤트 아이템교환
            ...
            cm.dispose();
            cm.openNpcCustom(cm.getClient(), 9063144, "Mr2");
        } else if (selection == 3) { // 일퀘
            cm.dispose();
            cm.openNpcCustom(cm.getClient(), 9063144, "Mr3");
        } else if (selection == 4) { // 후원룰렛
            cm.dispose();
            cm.openNpcCustom(cm.getClient(), 9063144, "Mr4");
        } else if (selection == 5) { // 어빌리티
            cm.dispose();
            cm.openNpcCustom(cm.getClient(), 9063144, "Mr5");
        } else if (selection == 6) { //V 코어 매트릭스
            cm.dispose();
            cm.openNpcCustom(cm.getClient(), 9063144, "Mr6");
        } else if (selection == 7) { //자유 전직
            cm.dispose();
            cm.openNpcCustom(cm.getClient(), 9063144, "Mr7");
        } else if (selection == 8) { //판타즈마
            cm.dispose();
            cm.openNpcCustom(cm.getClient(), 9063144, "Mr8");
        } else if (selection == 9) { //아라크노
            cm.dispose();
            cm.openNpcCustom(cm.getClient(), 9063144, "MRSMH");
            ...
            cm.dispose();
            cm.openNpcCustom(cm.getClient(), 9063144, "SaiJiTongXingZhneg");
