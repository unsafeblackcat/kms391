...
function setup(mapid) {
	...
	eim.setInstanceMap(map + 280).resetFully();
    return eim;
}
...
function changedMap(eim, player, mapid) {
    ...
    if (mapid != 450008100 && mapid != 450008150 && mapid != 450008200 && mapid != 450008250 && mapid != 450008300 && mapid != 450008350 && mapid != 450008380) {
...
function playAnimation(player, eim) {
    ...
    if (stage == 0) {
        ...
        player.getClient().getSession().writeAndFlush(CField.showSpineScreen(false, false, false, "Effect/Direction20.img/bossWill/intro_spine/skeleton", "1", 0, true, "skeleton"));
        player.getClient().getSession().writeAndFlush(SLFCGPacket.playSE("Sound/SoundEff.img/esfera/boss/intro1"));
    } else if (stage == 2) {
        player.getClient().getSession().writeAndFlush(CField.showSpineScreen(false, false, false, "Effect/Direction20.img/bossWill/intro_spine/skeleton", "2", 0, true, "skeleton"));
        player.getClient().getSession().writeAndFlush(SLFCGPacket.playSE("Sound/SoundEff.img/esfera/boss/intro2"));
    } else if (stage == 4) {
        player.getClient().getSession().writeAndFlush(CField.showSpineScreen(false, false, false, "Effect/Direction20.img/bossWill/intro_spine/skeleton", "3", 0, true, "skeleton"));
...
function WarptoNextStage(eim) {
    ...
    {
	    spawnMonster(eim, stage, 8950114, 33, 248);
    }
}
