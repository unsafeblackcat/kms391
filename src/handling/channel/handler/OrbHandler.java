...
public class OrbHandler {
	...
	public static void spawnOrb(LittleEndianAccessor slea, MapleClient c) {
		...
		if (chr.getSkillLevel(GameConstants.getLinkedSkill(skillId)) > 0) {
			...
			MapleOrb orb = new MapleOrb(chr.getId(), 1, skillId, effect.getX(), effect.getSubTime() / 1000,
					effect.getDuration() / 1000, delay, unk1, unk2, pos, facing, unk3);
	...
	public static void moveOrb(LittleEndianAccessor slea, MapleClient c) {
		...
		if (chr.getId() == chrId && chr.getMap().getMapObject(objectId, MapleMapObjectType.ORB) != null
				&& chr.getSkillLevel(GameConstants.getLinkedSkill(skillId)) > 0) {
			chr.getMap().broadcastMessage(
					CField.moveOrb(chrId, type, objectId, action, pos, slea.readInt(), slea.readInt(), slea.readInt()));
		}
	}
	...
	public static void removeOrb(LittleEndianAccessor slea, MapleClient c) {
		...
		if (orb != null) {
			if (chr.getSkillLevel(orb.getSkillId()) > 0) {
				SecondaryStatEffect effect = SkillFactory.getSkill(orb.getSkillId())
						.getEffect(chr.getSkillLevel(orb.getSkillId()));
