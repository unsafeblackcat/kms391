...
public class MapleClient {
	...
	public final List<MapleCharacter> loadCharacters(int serverId) {
		...
		for (CharNameAndId cni : loadCharactersInternal(serverId)) {
			...
			chars.add(chr);
			this.charInfo.put(Integer.valueOf(chr.getId()),
					new Pair<>(Short.valueOf(chr.getLevel()), Short.valueOf(chr.getJob())));
	...
	private List<CharNameAndId> loadCharactersInternal(int serverId) {
		...
		try {
			...
			ps = con.prepareStatement(
					"SELECT id, name, gm FROM characters WHERE accountid = ? AND world = ? ORDER BY `order` ASC");
		...
		try {
			if (this.quests != null) {
				...
				for (MapleQuestStatus q : this.quests.values()) {
					ps = con.prepareStatement(
							"INSERT INTO acc_queststatus (`queststatusid`, `accid`, `quest`, `status`, `time`, `forfeited`, `customData`) VALUES (DEFAULT, ?, ?, ?, ?, ?, ?)",
							1);
					ps.setInt(1, this.accId);
	...
	public int login(String login, String pwd, String mac, boolean weblogin, boolean ipMacBanned) {
		...
		try {
			...
			if (rs.next()) {
				...
				if (ServerConstants.ConnectorSetting && !this.isGm()
						&& !this.getSessionIPAddress().equals("192.168.155")
						&& !this.getSessionIPAddress().equals("192.168.155") && !this.isGm()
						&& !this.getSessionIPAddress().equals("192.168.155")) {
					if (allowed != 1 && gmc < 1) {
						// this.session.write(CWvsContext.serverNotice(1, "", "morning 접속기를 통해
						// 접속해주세요."));
						rs.close();
						ps.close();
						loginok = 20;
	...
	public final void updateLoginState(int newstate, String SessionID, String accname) {
		...
		try {
			con = DatabaseConnection.getConnection();
			ps = con.prepareStatement(
					"UPDATE accounts SET loggedin = ?, SessionIP = ?, lastlogin = CURRENT_TIMESTAMP() WHERE name = ?");
			ps.setInt(1, newstate);
			ps.setString(2, SessionID);
	...
	public final void removalTask(boolean shutdown) {
		try {
			...
			if (this.player.getMapId() == 180000002 && !this.player.isIntern()) {
				...
				if (stat1.getCustomData() == null) {
					...
					int seconds = Integer.parseInt(stat2.getCustomData())
							- (int) ((System.currentTimeMillis() - Long.parseLong(stat1.getCustomData())) / 1000L);
					if (seconds < 0) {
						seconds = 0;
					}
			...
			FileoutputUtil.outputFileError("Log_AccountStuck.txt", e);
		}
	}
	...
	public final void disconnect(boolean RemoveInChannelServer, boolean fromCS, boolean shutdown) {
		...
		if (player != null) {
			...
			FileoutputUtil.log(FileoutputUtil. 登出日誌, "[登出] 帳號編號 : " + this.player.getClient().getAccID()  + " | "
	                + this.player.getName()  + "(" + this.player.getId()  + ") 已登出遊戲");
	...
	public final void disconnect(MapleCharacter player, boolean RemoveInChannelServer, boolean fromCS,
			boolean shutdown) {
		if (player != null) {
			FileoutputUtil.log(FileoutputUtil. 登出日誌, "[登出處理] 帳號編號 : " + player.getClient().getAccID()  + " | "
	                + player.getName()  + "(" + player.getId()  + ") 於第二階段完成登出程序");
			MapleMap map = player.getMap();
			MapleParty party = player.getParty();
			String namez = player.getName();
			int idz = player.getId(), messengerid = (player.getMessenger() == null) ? 0 : player.getMessenger().getId(),
	...
	public final int deleteCharacter(int cid) {
		...
		try {
			...
			String data = "캐릭터명 : " + name + " (AccountID : " + this.accId + ", AccountName : " + getAccountName()
					+ ") 이 삭제됨.\r\n";
		...
		try {
			if (this.keyValues_boss != null) {
				...
				PreparedStatement pse = con
						.prepareStatement("INSERT INTO acckeyvalue_boss (`id`, `key`, `value`) VALUES (?, ?, ?)");
		...
		try {
			if (this.customDatas != null) {
				...
				pse = con.prepareStatement(
						"INSERT INTO account_customdata (`accid`, `id`, `key`, `value`) VALUES (?, ?, ?, ?)");
				pse.setInt(1, this.accId);
	...
	public void setCustomData(int id, String key, String value) {
		// 獲取或創建指定ID的數據列表
		List<Pair<String, String>> dataList = this.customDatas.computeIfAbsent(Integer.valueOf(id),
				k -> new ArrayList<>());

		// 查找並更新現有鍵值對
		for (Pair<String, String> datas : dataList) {
			if (datas.getLeft().equals(key)) {
				datas.right = value;
				sendUpdatePackets(id, key, value);
				return;
			}
		}
	...
	// 提取重複的代碼為獨立方法
	private void sendUpdatePackets(int id, String key, String value) {
	...
	public void loadCabinet() {
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				MapleCabinet cb = new MapleCabinet(rs.getInt("accountid"), rs.getInt("itemid"), rs.getInt("count"),
						rs.getString("bigname"), rs.getString("smallname"), rs.getLong("savetime"),
						rs.getInt("delete"));
	...
	public void saveCabiNet(List<MapleCabinet> cabinet) {
		...
		try {
			...
			for (MapleCabinet cn : cabinet) {
				...
				ps = con.prepareStatement(
						"INSERT INTO `cabinet` (`accountid`, `itemid`, `count`, `bigname`, `smallname`, `savetime`, `delete`, `playerid`, `name`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
	...
	public void loadShopLimit() {
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				MapleShopLimit shops = new MapleShopLimit(rs.getInt("accountid"), rs.getInt("charid"),
						rs.getInt("shopid"), rs.getInt("itemid"), rs.getInt("position"), rs.getInt("limitcountacc"),
						rs.getInt("limitcountchr"), rs.getInt("lastbuymonth"), rs.getInt("lastbuyday"));
	...
	public void saveShopLimit(List<MapleShopLimit> shoplimit) {
		...
		try {
			...
			for (MapleShopLimit sl : shoplimit) {
				...
				ps = con.prepareStatement(
						"INSERT INTO `shopitemlimit` (`accountid`, `charid`, `shopid`, `itemid`, `position`, `limitcountacc`, `limitcountchr`, `lastbuymonth`, `lastbuyday`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
				ps.setLong(1, this.accId);
			...
			if (usersend) {
				if (getPlayer().getMapId() == chr.getMapId() && chr.getClient().getChannel() == getChannel()) {
					...
					chr.getMap().broadcastMessage(
							CWvsContext.LieDetectorMulti(chr.getPlayer().getId(), getPlayer().getId()));
				}
			} else {
				chr.setSkillCustomInfo(179875, 0L, 60000);
				chr.getClient().setKeyValue("LIE_DETECT", "" + System.currentTimeMillis());
				chr.getClient().setLieDectctCount(1);
				chr.getMap().broadcastMessage(
						CWvsContext.LieDetectorMulti(chr.getPlayer().getId(), chr.getPlayer().getId()));
				chr.getClient().send(CWvsContext.LieDetector(chr.getClient(), 0, getLieDectctCount()));
			}
		}
	}
