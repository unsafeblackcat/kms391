...
public class MapleItemInformationProvider {
	...
	protected final MapleDataProvider chrData = MapleDataProviderFactory
			.getDataProvider(new File(System.getProperty("wz") + "/Character.wz"));
	...
	protected final MapleDataProvider itemData = MapleDataProviderFactory
			.getDataProvider(new File(System.getProperty("wz") + "/Item.wz"));
	...
	public void runEtc() {
			...
			for (MapleData level : dat.getChildByPath("Effect")) {
				...
				if (level.getChildByPath("activeSkill") != null) {
					for (MapleData skill : level.getChildByPath("activeSkill")) {
						itez.activeSkills.put(Integer.valueOf(MapleDataTool.getIntConvert("id", skill, 0)),
								Byte.valueOf((byte) MapleDataTool.getIntConvert("level", skill, 0)));
					}
				}
		...
		for (MapleDataEntry d : e.getFiles()) {
			...
			this.androids.put(Integer.valueOf(Integer.parseInt(d.getName().substring(0, 4))),
					new Triple<>(new Pair<>(hair, face), skin, Integer.valueOf(gender)));
		}
		...
		for (MapleData d : lifesData) {
			if (d.getChildByPath("info") != null && MapleDataTool.getInt("type", d.getChildByPath("info"), 0) == 1) {
				this.potLife.put(Integer.valueOf(MapleDataTool.getInt("counsumeItem", d.getChildByPath("info"), 0)),
						new Pair<>(Integer.valueOf(Integer.parseInt(d.getName())),
								Integer.valueOf(d.getChildByPath("level").getChildren().size())));
			}
		}
		...
		for (MapleDataEntry b : a.getFiles()) {
			...
			for (Map.Entry<String, Pair<Point, Point>> ez : dummy.entrySet()) {
				if (((String) ez.getKey()).length() > 2 && ((String) ez.getKey())
						.substring(((String) ez.getKey()).length() - 2, ((String) ez.getKey()).length() - 1)
						.equals("D")) {
					thePointK.add(new Triple<>(ez.getKey(), (Point) ((Pair) ez.getValue()).left,
							(Point) ((Pair) ez.getValue()).right));
					continue;
				}
				if (((String) ez.getKey()).contains("PoleArm")) {
					thePointA.add(new Triple<>(ez.getKey(), (Point) ((Pair) ez.getValue()).left,
							(Point) ((Pair) ez.getValue()).right));
					continue;
				}
				thePoint.add(new Triple<>(ez.getKey(), (Point) ((Pair) ez.getValue()).left,
						(Point) ((Pair) ez.getValue()).right));
			}
		...
		try {
			...
			for (Map.Entry<Integer, ItemInformation> entry : this.dataCache.entrySet()) {
				if (GameConstants.getInventoryType(((Integer) entry.getKey()).intValue()) == MapleInventoryType.EQUIP
						|| GameConstants
								.getInventoryType(((Integer) entry.getKey()).intValue()) == MapleInventoryType.CODY) {
	...
	private void addPotential(List<Integer> potentials, List<Triple<Boolean, Integer, Integer>> list, int level,
			boolean additional, int itemtype) {
		for (Triple<Boolean, Integer, Integer> potential : list) {
			if (additional) {
				if (!((Boolean) potential.left).booleanValue()) {
					continue;
				}
				if ((itemtype == 1190 || itemtype == 1191) && ((Integer) potential.right).intValue() % 1000 >= 601
						&& ((Integer) potential.right).intValue() % 1000 <= 604) {
					continue;
				}
	...
	private boolean isWeaponPotential(int itemtype) {
		return (GameConstants.isWeapon(itemtype * 1000) || itemtype == 1098 || itemtype == 1092 || itemtype == 1099
				|| itemtype == 1190 || itemtype == 1191);
	}

	private boolean isAccessoryPotential(int itemtype) {
		return ((itemtype >= 1112 && itemtype <= 1115) || itemtype == 1122 || itemtype == 1012 || itemtype == 1022
				|| itemtype == 1032);
	}
	...
	public void cachePotentialOption() {
		...
		for (MapleData data : potsData) {
			...
			if (this.potentialOpCache.get(Integer.valueOf(type)) == null) {
				...
				potentialIds.add(new Triple<>(Boolean.valueOf(additional), Integer.valueOf(reqLevel),
						Integer.valueOf(potentialID)));
				this.potentialOpCache.put(Integer.valueOf(type), potentialIds);
				continue;
			}
			((List) this.potentialOpCache.get(Integer.valueOf(type))).add(
					new Triple<>(Boolean.valueOf(additional), Integer.valueOf(reqLevel), Integer.valueOf(potentialID)));
		}
	}
	...
	public final List<Pair<Integer, String>> getAllEquips() {
		...
		for (MapleData eqpType : itemsData.getChildren()) {
			for (MapleData itemFolder : eqpType.getChildren()) {
				itemPairs.add(new Pair<>(Integer.valueOf(Integer.parseInt(itemFolder.getName())),
			...
			for (Map.Entry<String, Integer> stat : sta.entrySet()) {
				if (((String) stat.getKey()).equals("STRMin")) {
					nEquip.setStr((short) (nEquip.getStr()
							+ rand(((Integer) stat.getValue()).intValue(), ((Integer) sta.get("STRMax")).intValue())));
					continue;
				}
				if (((String) stat.getKey()).equals("DEXMin")) {
					nEquip.setDex((short) (nEquip.getDex()
							+ rand(((Integer) stat.getValue()).intValue(), ((Integer) sta.get("DEXMax")).intValue())));
					continue;
				}
				if (((String) stat.getKey()).equals("INTMin")) {
					nEquip.setInt((short) (nEquip.getInt()
							+ rand(((Integer) stat.getValue()).intValue(), ((Integer) sta.get("INTMax")).intValue())));
					continue;
				}
				if (((String) stat.getKey()).equals("LUKMin")) {
					nEquip.setLuk((short) (nEquip.getLuk()
							+ rand(((Integer) stat.getValue()).intValue(), ((Integer) sta.get("LUKMax")).intValue())));
					continue;
				}
				if (((String) stat.getKey()).equals("PADMin")) {
					nEquip.setWatk((short) (nEquip.getWatk()
							+ rand(((Integer) stat.getValue()).intValue(), ((Integer) sta.get("PADMax")).intValue())));
					continue;
				}
				if (((String) stat.getKey()).equals("PDDMin")) {
					nEquip.setWdef((short) (nEquip.getWdef()
							+ rand(((Integer) stat.getValue()).intValue(), ((Integer) sta.get("PDDMax")).intValue())));
					continue;
				}
				if (((String) stat.getKey()).equals("MADMin")) {
					nEquip.setMatk((short) (nEquip.getMatk()
							+ rand(((Integer) stat.getValue()).intValue(), ((Integer) sta.get("MADMax")).intValue())));
					continue;
				}
				if (((String) stat.getKey()).equals("MDDMin")) {
					nEquip.setMdef((short) (nEquip.getMdef()
							+ rand(((Integer) stat.getValue()).intValue(), ((Integer) sta.get("MDDMax")).intValue())));
					continue;
				}
				if (((String) stat.getKey()).equals("ACCMin")) {
					nEquip.setAcc((short) (nEquip.getAcc()
							+ rand(((Integer) stat.getValue()).intValue(), ((Integer) sta.get("ACCMax")).intValue())));
					continue;
				}
				if (((String) stat.getKey()).equals("EVAMin")) {
					nEquip.setAvoid((short) (nEquip.getAvoid()
							+ rand(((Integer) stat.getValue()).intValue(), ((Integer) sta.get("EVAMax")).intValue())));
					continue;
				}
				if (((String) stat.getKey()).equals("SpeedMin")) {
					nEquip.setSpeed((short) (nEquip.getSpeed() + rand(((Integer) stat.getValue()).intValue(),
							((Integer) sta.get("SpeedMax")).intValue())));
					continue;
				}
				if (((String) stat.getKey()).equals("JumpMin")) {
					nEquip.setJump((short) (nEquip.getJump()
							+ rand(((Integer) stat.getValue()).intValue(), ((Integer) sta.get("JumpMax")).intValue())));
					continue;
				}
				if (((String) stat.getKey()).equals("MHPMin")) {
					nEquip.setHp((short) (nEquip.getHp()
							+ rand(((Integer) stat.getValue()).intValue(), ((Integer) sta.get("MHPMax")).intValue())));
					continue;
				}
				if (((String) stat.getKey()).equals("MMPMin")) {
					nEquip.setMp((short) (nEquip.getMp()
							+ rand(((Integer) stat.getValue()).intValue(), ((Integer) sta.get("MMPMax")).intValue())));
					continue;
				}
				if (((String) stat.getKey()).equals("MaxHPMin")) {
					nEquip.setHp((short) (nEquip.getHp() + rand(((Integer) stat.getValue()).intValue(),
							((Integer) sta.get("MaxHPMax")).intValue())));
					continue;
				}
				if (((String) stat.getKey()).equals("MaxMPMin")) {
					nEquip.setMp((short) (nEquip.getMp() + rand(((Integer) stat.getValue()).intValue(),
							((Integer) sta.get("MaxMPMax")).intValue())));
				}
			}
	...
	public final boolean canEquip(Map<String, Integer> stats, int itemid, int level, int job, int fame, int str,
			int dex, int luk, int int_, int supremacy) {
		if (str >= (stats.containsKey("reqSTR") ? ((Integer) stats.get("reqSTR")).intValue() : 0)
				&& dex >= (stats.containsKey("reqDEX") ? ((Integer) stats.get("reqDEX")).intValue() : 0)
				&& luk >= (stats.containsKey("reqLUK") ? ((Integer) stats.get("reqLUK")).intValue() : 0)
				&& int_ >= (stats.containsKey("reqINT") ? ((Integer) stats.get("reqINT")).intValue() : 0)) {
	...
	public final Item scrollEquipWithId(Item equip, Item scrollId, boolean ws, MapleCharacter chr) {
		if (equip.getType() == 1) {
			...
			case 2049181: // [othello] - 놀라운 긍정의 Fancy 주문서
				nEquip.setStr((short) (nEquip.getStr() + 6));
				nEquip.setInt((short) (nEquip.getInt() + 6));
				nEquip.setDex((short) (nEquip.getDex() + 6));
				nEquip.setLuk((short) (nEquip.getLuk() + 6));
			...
			case 2049050:
				if (!Randomizer.isSuccess(getSuccess(scrollId.getItemId(), chr, nEquip))) {
					if (Randomizer.isSuccess(getCursed(scrollId.getItemId(), chr))
						...
						chr.dropMessage(5, "因卷轴效果，物品未被摧毁.");
					}
					failed = true;
					break;
				}
			...
			case 2049703:
				if (!Randomizer.isSuccess(getSuccess(scrollId.getItemId(), chr, nEquip))) {
					if (Randomizer.isSuccess(getCursed(scrollId.getItemId(), chr))
							&& ItemFlag.PROTECT_SHIELD.check(nEquip.getFlag())) {
						chr.dropMessage(5, "因卷軸效果，物品未被摧毀.");
					}
					failed = true;
					break;
				}
				...
				break;
			case 2049750: // 罕見潛在能力卷軸80%
			case 2049751: // 罕見潛在能力卷軸60%
			case 2049752: // 罕見潛在能力卷軸30%
			case 2049762: // 罕見潛在能力卷軸100%
				if (!Randomizer.isSuccess(getSuccess(scrollId.getItemId(), chr, nEquip))) {
					if (Randomizer.isSuccess(getCursed(scrollId.getItemId(), chr))
							&& ItemFlag.PROTECT_SHIELD.check(nEquip.getFlag())) {
						chr.dropMessage(5, "因卷軸效果，物品未被摧毀.");
			...
			case 2047406:
				...
				break;
//======================================究極黑暗卷軸=======================================//
			case 2046957: // 究極的黑暗單手武器攻擊力卷軸
			case 2047838: // 究極的黑暗雙手武器攻擊力卷軸
				int WatkH = 14; // 攻击力、STR、DEX、INT、LUK 增加的值
				nEquip.addWatk((short) WatkH);
				nEquip.addStr((short) WatkH);
				nEquip.addDex((short) WatkH);
				nEquip.addInt((short) WatkH);
				nEquip.addLuk((short) WatkH);
				break;

			case 2046958: // 究極的黑暗單手武器魔力卷軸
			case 2047839: // 究極的黑暗雙手武器魔力卷軸
				int MatkH = 14; // 魔力、STR、DEX、INT、LUK 增加的值
				nEquip.addMatk((short) MatkH);
				nEquip.addStr((short) MatkH);
				nEquip.addDex((short) MatkH);
				nEquip.addInt((short) MatkH);
				nEquip.addLuk((short) MatkH);
				break;

			case 2046847: // 究極的黑暗飾品攻擊力卷軸
			case 2047919: // 究極的黑暗防具攻擊力卷軸
			case 2048063: // 究極的黑暗寵物裝備攻擊力卷軸
			case 2049219: // 究極的黑暗肩飾品攻擊力卷軸
				int WatkHg = 9; // 攻击力增加的值
				nEquip.addWatk((short) WatkHg);
				break;

			case 2046848: // 究極的黑暗飾品魔力卷軸
			case 2047920: // 究極的黑暗防具魔力卷軸
			case 2048064: // 究極的黑暗寵物裝備魔力卷軸
			case 2049220: // 究極的黑暗肩飾品魔力卷軸
				int MatkHm = 9; // 魔力增加的值
				nEquip.addMatk((short) MatkHm);
				break;

//======================================璀璨之光卷軸============================================//
			case 2046959: // 璀璨之光單手武器攻擊力捲軸100%
			case 2047840: // 璀璨之光雙手武器攻擊力捲軸100%
				int Cg = 15; // 攻击力增加的值
				int Cgg = 13; // STR、DEX、INT、LUK 增加的值

				nEquip.addWatk((short) Cg);
				nEquip.addStr((short) Cgg);
				nEquip.addDex((short) Cgg);
				nEquip.addInt((short) Cgg);
				nEquip.addLuk((short) Cgg);
				break;

			case 2046960: // 璀璨之光單手武器魔力捲軸100%
			case 2047841: // 璀璨之光雙手武器魔力捲軸100%
				int Cm = 15; // 魔力增加的值
				int Cmg = 13; // STR、DEX、INT、LUK 增加的值

				nEquip.addMatk((short) Cm);
				nEquip.addStr((short) Cmg);
				nEquip.addDex((short) Cmg);
				nEquip.addInt((short) Cmg);
				nEquip.addLuk((short) Cmg);
				break;

			case 2046849: // 璀璨之光飾品攻擊力捲軸100
			case 2047921: // 璀璨之光防具攻擊力捲軸100
			case 2048065: // 璀璨之光寵物裝備攻擊力捲軸100
			case 2049221: // 璀璨之光肩飾品攻擊力捲軸100
				int WatkCg = 10; // 攻击力增加的值
				nEquip.addWatk((short) WatkCg);
				break;

			case 2046850: // 璀璨之光飾品魔力捲軸100
			case 2047922: // 璀璨之光防具魔力捲軸100
			case 2048066: // 璀璨之光寵物裝備魔力捲軸100
			case 2049222: // 璀璨之光飾品魔力捲軸100
				int MatkCg = 10; // 魔力增加的值
				nEquip.addMatk((short) MatkCg);
				break;

//========================================救世卷軸============================================//		
			case 2046974: // 救世單手武器攻擊力卷軸
			case 2047845: // 救世雙手武器攻擊力卷軸
				List<Pair<Integer, Integer>> Jsg = new ArrayList<>();
				Jsg.add(new Pair<>(15, 35)); // 35%概率 +15
				Jsg.add(new Pair<>(16, 30)); // 30%概率 +16
				Jsg.add(new Pair<>(17, 15)); // 15%概率 +17
				Jsg.add(new Pair<>(18, 8)); // 8%概率 +18
				Jsg.add(new Pair<>(19, 7)); // 7%概率 +19
				Jsg.add(new Pair<>(20, 5)); // 5%概率 +20
				int Jsgk = GameConstants.isRandStat(Jsg, 100);
				nEquip.addWatk((short) Jsgk);
				nEquip.addStr((short) Jsgk);
				nEquip.addInt((short) Jsgk);
				nEquip.addDex((short) Jsgk);
				nEquip.addLuk((short) Jsgk);
				break;

			case 2046975: // 救世單手武器魔力卷軸
			case 2047846: // 救世雙手武器魔力卷軸
				List<Pair<Integer, Integer>> Jsm = new ArrayList<>();
				Jsm.add(new Pair<>(15, 35)); // 35%概率 +15
				Jsm.add(new Pair<>(16, 30)); // 30%概率 +16
				Jsm.add(new Pair<>(17, 15)); // 15%概率 +17
				Jsm.add(new Pair<>(18, 8)); // 8%概率 +18
				Jsm.add(new Pair<>(19, 7)); // 7%概率 +19
				Jsm.add(new Pair<>(20, 5)); // 5%概率 +20
				int Jsmk = GameConstants.isRandStat(Jsm, 100);
				nEquip.addMatk((short) Jsmk);
				nEquip.addStr((short) Jsmk);
				nEquip.addInt((short) Jsmk);
				nEquip.addDex((short) Jsmk);
				nEquip.addLuk((short) Jsmk);
				break;

			case 2046853: // 救世飾品攻擊力卷軸
			case 2047925: // 救世防具攻擊力卷軸
			case 2048069: // 救世寵物裝備攻擊力卷軸
			case 2049225: // 救世肩飾品攻擊力卷軸
				List<Pair<Integer, Integer>> Jsspg = new ArrayList<>();
				Jsspg.add(new Pair<>(10, 35)); // 35%概率 +10
				Jsspg.add(new Pair<>(11, 30)); // 30%概率 +11
				Jsspg.add(new Pair<>(12, 15)); // 15%概率 +12
				Jsspg.add(new Pair<>(13, 8)); // 8%概率 +13
				Jsspg.add(new Pair<>(14, 7)); // 7%概率 +14
				Jsspg.add(new Pair<>(15, 5)); // 5%概率 +15
				int Jg = GameConstants.isRandStat(Jsspg, 100);
				nEquip.addWatk((short) Jg);
				break;

			case 2046854: // 救世飾品魔力卷軸
			case 2047926: // 救世防具魔力卷軸
			case 2048070: // 救世寵物裝備魔力卷軸
			case 2049226: // 救世肩飾品魔力卷軸
				List<Pair<Integer, Integer>> Jsspm = new ArrayList<>();
				Jsspm.add(new Pair<>(10, 35)); // 35%概率 +10
				Jsspm.add(new Pair<>(11, 30)); // 30%概率 +11
				Jsspm.add(new Pair<>(12, 15)); // 15%概率 +12
				Jsspm.add(new Pair<>(13, 8)); // 8%概率 +13
				Jsspm.add(new Pair<>(14, 7)); // 7%概率 +14
				Jsspm.add(new Pair<>(15, 5)); // 5%概率 +15
				int Jm = GameConstants.isRandStat(Jsspm, 100);
				nEquip.addWatk((short) Jm);

				// ========================================榮耀卷軸============================================//
			case 2046996: // 榮耀雙手武器攻擊力卷軸
			case 2047818: // 榮耀單手武器攻擊力卷軸
				List<Pair<Integer, Integer>> Gjl = new ArrayList<>();
				Gjl.add(new Pair<>(10, 1)); // 1%概率 +10
				Gjl.add(new Pair<>(11, 2)); // 2%概率 +11
				Gjl.add(new Pair<>(12, 4)); // 4%概率 +12
				Gjl.add(new Pair<>(13, 6)); // 6%概率 +13
				Gjl.add(new Pair<>(14, 31)); // 31%概率 +14
				Gjl.add(new Pair<>(15, 28)); // 28%概率 +15
				Gjl.add(new Pair<>(16, 13)); // 13%概率 +16
				Gjl.add(new Pair<>(17, 7)); // 7%概率 +17
				Gjl.add(new Pair<>(18, 5)); // 5%概率 +18
				Gjl.add(new Pair<>(19, 2)); // 2%概率 +19
				Gjl.add(new Pair<>(20, 1)); // 1%概率 +20
				int Gjl1 = GameConstants.isRandStat(Gjl, 100);
				nEquip.addWatk((short) Gjl1);
				nEquip.addStr((short) Gjl1);
				nEquip.addInt((short) Gjl1);
				nEquip.addDex((short) Gjl1);
				nEquip.addLuk((short) Gjl1);
				break;

			case 2046997:// 榮耀雙手武器魔力卷軸
			case 2047819:// 榮耀單手武器魔力卷軸
				List<Pair<Integer, Integer>> Mfl = new ArrayList<>();
				Mfl.add(new Pair<>(10, 1)); // 1%概率 +10
				Mfl.add(new Pair<>(11, 2)); // 2%概率 +11
				Mfl.add(new Pair<>(12, 4)); // 4%概率 +12
				Mfl.add(new Pair<>(13, 6)); // 6%概率 +13
				Mfl.add(new Pair<>(14, 31)); // 31%概率 +14
				Mfl.add(new Pair<>(15, 28)); // 28%概率 +15
				Mfl.add(new Pair<>(16, 13)); // 13%概率 +16
				Mfl.add(new Pair<>(17, 7)); // 7%概率 +17
				Mfl.add(new Pair<>(18, 5)); // 5%概率 +18
				Mfl.add(new Pair<>(19, 2)); // 2%概率 +19
				Mfl.add(new Pair<>(20, 1)); // 1%概率 +20
				int Mfl1 = GameConstants.isRandStat(Mfl, 100);
				nEquip.addMatk((short) Mfl1);
				nEquip.addStr((short) Mfl1);
				nEquip.addInt((short) Mfl1);
				nEquip.addDex((short) Mfl1);
				nEquip.addLuk((short) Mfl1);
				break;

			case 2046844:// 榮耀飾品攻擊力卷軸
			case 2049212:// 榮耀肩飾攻擊力卷軸
			case 2049214:// 榮耀肩飾增幅卷軸
			case 2047942:// 榮耀防具攻擊力卷軸
			case 2047944:// 榮耀防具增幅卷軸
			case 2048090:// 榮耀寵物裝備攻擊力卷軸
			case 2048092:// 榮耀寵物裝備增幅卷軸
				List<Pair<Integer, Integer>> attackIncreaseList = new ArrayList<>(); // 定义攻击力增加量和对应的概率
				attackIncreaseList.add(new Pair<>(5, 1)); // 1% 概率攻击力 +5
				attackIncreaseList.add(new Pair<>(6, 2)); // 2% 概率攻击力 +6
				attackIncreaseList.add(new Pair<>(7, 4)); // 4% 概率攻击力 +7
				attackIncreaseList.add(new Pair<>(8, 6)); // 6% 概率攻击力 +8
				attackIncreaseList.add(new Pair<>(9, 31)); // 31% 概率攻击力 +9
				attackIncreaseList.add(new Pair<>(10, 28));// 28% 概率攻击力 +10
				attackIncreaseList.add(new Pair<>(11, 13));// 13% 概率攻击力 +11
				attackIncreaseList.add(new Pair<>(12, 7)); // 7% 概率攻击力 +12
				attackIncreaseList.add(new Pair<>(13, 5)); // 5% 概率攻击力 +13
				attackIncreaseList.add(new Pair<>(14, 2)); // 2% 概率攻击力 +14
				attackIncreaseList.add(new Pair<>(15, 1)); // 1% 概率攻击力 +15
				int attackIncrease = GameConstants.isRandStat(attackIncreaseList, 100);
				Equip Sgj = (Equip) equip;
				Equip Sgj1 = null;
				if (GameConstants.isAlphaWeapon(Sgj.getItemId())) {
					Sgj1 = (Equip) chr.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -10);
				} else if (GameConstants.isBetaWeapon(Sgj.getItemId())) {
					Sgj1 = (Equip) chr.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -11);
				}
				Sgj.addWatk((short) attackIncrease);
				if (Sgj1 != null) {
					Sgj1.addWatk((short) attackIncrease);
				}
				break;

			case 2046845:// 榮耀飾品魔力卷軸
			case 2049213:// 榮耀肩飾魔法力卷軸
			case 2047943:// 榮耀防具魔力卷軸
			case 2048091:// 榮耀寵物裝備魔力卷軸
				// 定义魔法力增加量和对应的概率
				List<Pair<Integer, Integer>> magicIncreaseList = new ArrayList<>();
				magicIncreaseList.add(new Pair<>(5, 1)); // 1% 概率魔法力 +5
				magicIncreaseList.add(new Pair<>(6, 2)); // 2% 概率魔法力 +6
				magicIncreaseList.add(new Pair<>(7, 4)); // 4% 概率魔法力 +7
				magicIncreaseList.add(new Pair<>(8, 6)); // 6% 概率魔法力 +8
				magicIncreaseList.add(new Pair<>(9, 31)); // 31% 概率魔法力 +9
				magicIncreaseList.add(new Pair<>(10, 28));// 28% 概率魔法力 +10
				magicIncreaseList.add(new Pair<>(11, 13));// 13% 概率魔法力 +11
				magicIncreaseList.add(new Pair<>(12, 7)); // 7% 概率魔法力 +12
				magicIncreaseList.add(new Pair<>(13, 5)); // 5% 概率魔法力 +13
				magicIncreaseList.add(new Pair<>(14, 2)); // 2% 概率魔法力 +14
				magicIncreaseList.add(new Pair<>(15, 1)); // 1% 概率魔法力 +15
				int magicIncrease = GameConstants.isRandStat(magicIncreaseList, 100);
				Equip Smfl = (Equip) equip;
				Equip Smfl1 = null;
				if (GameConstants.isAlphaWeapon(Smfl.getItemId())) {
					Smfl1 = (Equip) chr.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -10);
				} else if (GameConstants.isBetaWeapon(Smfl.getItemId())) {
					Smfl1 = (Equip) chr.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -11);
				}
				Smfl.addMatk((short) magicIncrease);
				if (Smfl1 != null) {
					Smfl1.addMatk((short) magicIncrease);
				}
				break;

			// ======================================新命運卷軸=======================================//
			case 2046972: // [週年限定]新命運之單手武器攻擊力捲軸100%
			case 2047842: // [週年限定]新命運之雙手武器攻擊力捲軸100%
				List<Pair<Integer, Integer>> Xmyg = new ArrayList<>();
				Xmyg.add(new Pair<>(15, 35)); // 35%概率 +14
				Xmyg.add(new Pair<>(16, 30)); // 30%概率 +15
				Xmyg.add(new Pair<>(17, 15)); // 15%概率 +16
				Xmyg.add(new Pair<>(18, 8)); // 8%概率 +17
				Xmyg.add(new Pair<>(19, 7)); // 7%概率 +18
				Xmyg.add(new Pair<>(20, 5)); // 5%概率 +20

				int XmyW = GameConstants.isRandStat(Xmyg, 100);
				nEquip.addStr((short) XmyW);
				nEquip.addInt((short) XmyW);
				nEquip.addDex((short) XmyW);
				nEquip.addLuk((short) XmyW);
				nEquip.addWatk((short) XmyW);
				break;

			case 2046973: // [週年限定]新命運之單手武器魔力捲軸100%
			case 2047843: // [週年限定]新命運之雙手武器魔力捲軸100%
				List<Pair<Integer, Integer>> XmyM = new ArrayList<>();
				XmyM.add(new Pair<>(15, 35)); // 35%概率 +14
				XmyM.add(new Pair<>(16, 30)); // 30%概率 +15
				XmyM.add(new Pair<>(17, 15)); // 15%概率 +16
				XmyM.add(new Pair<>(18, 8)); // 8%概率 +17
				XmyM.add(new Pair<>(19, 7)); // 7%概率 +18
				XmyM.add(new Pair<>(20, 5)); // 5%概率 +20

				int XmyMF = GameConstants.isRandStat(XmyM, 100);
				nEquip.addStr((short) XmyMF);
				nEquip.addInt((short) XmyMF);
				nEquip.addDex((short) XmyMF);
				nEquip.addLuk((short) XmyMF);
				nEquip.addMatk((short) XmyMF);
				break;

			case 2046851: // [週年限定]新命運之飾品攻擊力捲軸100%
			case 2047923: // [週年限定]新命運之防具攻擊力捲軸100%
			case 2048067: // [週年限定]新命運之寵物裝備攻擊力捲軸100%
			case 2049223: // [週年限定]新命運之飾品攻擊力捲軸100%
				List<Pair<Integer, Integer>> XmySG = new ArrayList<>();
				XmySG.add(new Pair<>(15, 35)); // 35%概率 +15
				XmySG.add(new Pair<>(16, 30)); // 30%概率 +16
				XmySG.add(new Pair<>(17, 15)); // 15%概率 +17
				XmySG.add(new Pair<>(18, 8)); // 8%概率 +18
				XmySG.add(new Pair<>(19, 7)); // 7%概率 +19
				XmySG.add(new Pair<>(20, 5)); // 5%概率 +20

				int Xg = GameConstants.isRandStat(XmySG, 100);
				nEquip.addWatk((short) Xg);
				break;

			case 2046852: // [週年限定]新命運之飾品魔力捲軸100%
			case 2047924: // [週年限定]新命運之防具魔力捲軸100%
			case 2048068: // [週年限定]新命運之寵物裝備魔力捲軸100%
			case 2049224: // [週年限定]新命運之飾品魔力捲軸100%
				List<Pair<Integer, Integer>> XmySM = new ArrayList<>();
				XmySM.add(new Pair<>(15, 35)); // 35%概率 +15
				XmySM.add(new Pair<>(16, 30)); // 30%概率 +16
				XmySM.add(new Pair<>(17, 15)); // 15%概率 +17
				XmySM.add(new Pair<>(18, 8)); // 8%概率 +18
				XmySM.add(new Pair<>(19, 7)); // 7%概率 +19
				XmySM.add(new Pair<>(20, 5)); // 5%概率 +20

				int Xm = GameConstants.isRandStat(XmySM, 100);
				nEquip.addMatk((short) Xm);
				break;

			// ======================================其他卷軸=======================================//

			case 2047949: // 赞助防具攻击力/魔力咒文书 9週年防具卷軸
			case 2046875: // 赞助饰品攻击力/魔力 卷軸 9週年飾品卷軸
			...
			default:
				if (GameConstants.isChaosScroll(scrollId.getItemId())) {
					if (!Randomizer.isSuccess(getSuccess(scrollId.getItemId(), chr, nEquip))) {
						if (Randomizer.isSuccess(getCursed(scrollId.getItemId(), chr, nEquip))
								&& ItemFlag.PROTECT_SHIELD.check(nEquip.getFlag())) {
							chr.dropMessage(5, "因卷軸效果，物品未被破壞.");
						}
						failed = true;
						break;
					}
				...
				if (scrollId.getItemId() == 2049361) {
					...
					if (level > 150) {
						chr.dropMessage(6, "僅限對150級以下的裝備物品使用.");
						break;
					}
				}
				if (scrollId.getItemId() == 2049360) {
					...
					int level = MapleDataTool.getInt("reqLevel", info, 0);
					if (level > 300) {
						chr.dropMessage(6, "僅限對300級以下的裝備物品使用.");
						break;
					}
					...
					if (nEquip.getFire() > 0L) {
						...
						for (int i = 0; i < 4; i++) {
							...
							if (dat == (EquipmentEnchant.isMagicWeapon(GameConstants.getWeaponType(nEquip.getItemId()))
									? 18
									: 17)) {
								int value;
								if (i == 0) {
					...
					while (nEquip.getEnhance() < max) {
						...
						for (Pair<EnchantFlag, Integer> stat : starForceStats.getStats()) {
							if (EnchantFlag.Watk.check(((EnchantFlag) stat.left).getValue())) {
								nEquip.setEnchantWatk(
										(short) (nEquip.getEnchantWatk() + ((Integer) stat.right).intValue()));
								if (zeroEquip != null) {
							...
							if (EnchantFlag.Matk.check(((EnchantFlag) stat.left).getValue())) {
								nEquip.setEnchantMatk(
										(short) (nEquip.getEnchantMatk() + ((Integer) stat.right).intValue()));
								if (zeroEquip != null) {
							...
							if (EnchantFlag.Str.check(((EnchantFlag) stat.left).getValue())) {
								nEquip.setEnchantStr(
										(short) (nEquip.getEnchantStr() + ((Integer) stat.right).intValue()));
								if (zeroEquip != null) {
									zeroEquip.setEnchantStr(
											(short) (zeroEquip.getEnchantStr() + ((Integer) stat.right).intValue()));
								}
							}
							if (EnchantFlag.Dex.check(((EnchantFlag) stat.left).getValue())) {
								nEquip.setEnchantDex(
										(short) (nEquip.getEnchantDex() + ((Integer) stat.right).intValue()));
								if (zeroEquip != null) {
									zeroEquip.setEnchantDex(
											(short) (zeroEquip.getEnchantDex() + ((Integer) stat.right).intValue()));
								}
							}
							if (EnchantFlag.Int.check(((EnchantFlag) stat.left).getValue())) {
								nEquip.setEnchantInt(
										(short) (nEquip.getEnchantInt() + ((Integer) stat.right).intValue()));
								if (zeroEquip != null) {
									zeroEquip.setEnchantInt(
											(short) (zeroEquip.getEnchantInt() + ((Integer) stat.right).intValue()));
								}
							}
							if (EnchantFlag.Luk.check(((EnchantFlag) stat.left).getValue())) {
								nEquip.setEnchantLuk(
										(short) (nEquip.getEnchantLuk() + ((Integer) stat.right).intValue()));
								if (zeroEquip != null) {
									zeroEquip.setEnchantLuk(
											(short) (zeroEquip.getEnchantLuk() + ((Integer) stat.right).intValue()));
								}
							}
							if (EnchantFlag.Wdef.check(((EnchantFlag) stat.left).getValue())) {
								nEquip.setEnchantWdef(
										(short) (nEquip.getEnchantWdef() + ((Integer) stat.right).intValue()));
								if (zeroEquip != null) {
							...
							if (EnchantFlag.Mdef.check(((EnchantFlag) stat.left).getValue())) {
								nEquip.setEnchantMdef(
										(short) (nEquip.getEnchantMdef() + ((Integer) stat.right).intValue()));
								if (zeroEquip != null) {
							...
							if (EnchantFlag.Hp.check(((EnchantFlag) stat.left).getValue())) {
								nEquip.setEnchantHp(
										(short) (nEquip.getEnchantHp() + ((Integer) stat.right).intValue()));
								if (zeroEquip != null) {
									zeroEquip.setEnchantHp(
											(short) (zeroEquip.getEnchantHp() + ((Integer) stat.right).intValue()));
								}
							}
							if (EnchantFlag.Mp.check(((EnchantFlag) stat.left).getValue())) {
								nEquip.setEnchantMp(
										(short) (nEquip.getEnchantMp() + ((Integer) stat.right).intValue()));
								if (zeroEquip != null) {
									zeroEquip.setEnchantMp(
											(short) (zeroEquip.getEnchantMp() + ((Integer) stat.right).intValue()));
								}
							}
							if (EnchantFlag.Acc.check(((EnchantFlag) stat.left).getValue())) {
								nEquip.setEnchantAcc(
										(short) (nEquip.getEnchantAcc() + ((Integer) stat.right).intValue()));
								if (zeroEquip != null) {
									zeroEquip.setEnchantAcc(
											(short) (zeroEquip.getEnchantAcc() + ((Integer) stat.right).intValue()));
								}
							}
							if (EnchantFlag.Avoid.check(((EnchantFlag) stat.left).getValue())) {
								nEquip.setEnchantAvoid(
										(short) (nEquip.getEnchantAvoid() + ((Integer) stat.right).intValue()));
								if (zeroEquip != null) {
				...
				if (GameConstants.isEquipScroll(scrollId.getItemId())) {
					...
					for (int i = 1; i <= MapleDataTool.getIntConvert("info/forceUpgrade",
						...
						if (GameConstants.isSuperior(nEquip.getItemId())) {
							...
							if (senhance < 1) {
								nEquip.setStr((short) (nEquip.getStr()
										+ ((slevel > 70) ? 2 : ((slevel > 100) ? 9 : ((slevel > 140) ? 19 : 1)))));
								nEquip.setDex((short) (nEquip.getDex()
										+ ((slevel > 70) ? 2 : ((slevel > 100) ? 9 : ((slevel > 140) ? 19 : 1)))));
								nEquip.setInt((short) (nEquip.getInt()
										+ ((slevel > 70) ? 2 : ((slevel > 100) ? 9 : ((slevel > 140) ? 19 : 1)))));
								...
								nEquip.setStr((short) (nEquip.getStr()
										+ ((slevel > 70) ? 3 : ((slevel > 100) ? 10 : ((slevel > 140) ? 20 : 2)))));
								nEquip.setDex((short) (nEquip.getDex()
										+ ((slevel > 70) ? 3 : ((slevel > 100) ? 10 : ((slevel > 140) ? 20 : 2)))));
								nEquip.setInt((short) (nEquip.getInt()
										+ ((slevel > 70) ? 3 : ((slevel > 100) ? 10 : ((slevel > 140) ? 20 : 2)))));
								...
								nEquip.setStr((short) (nEquip.getStr()
										+ ((slevel > 70) ? 5 : ((slevel > 100) ? 12 : ((slevel > 140) ? 22 : 4)))));
								nEquip.setDex((short) (nEquip.getDex()
										+ ((slevel > 70) ? 5 : ((slevel > 100) ? 12 : ((slevel > 140) ? 22 : 4)))));
								nEquip.setInt((short) (nEquip.getInt()
										+ ((slevel > 70) ? 5 : ((slevel > 100) ? 12 : ((slevel > 140) ? 22 : 4)))));
								...
								nEquip.setStr((short) (nEquip.getStr()
										+ ((slevel > 70) ? 8 : ((slevel > 100) ? 15 : ((slevel > 140) ? 25 : 7)))));
								nEquip.setDex((short) (nEquip.getDex()
										+ ((slevel > 70) ? 8 : ((slevel > 100) ? 15 : ((slevel > 140) ? 25 : 7)))));
								nEquip.setInt((short) (nEquip.getInt()
										+ ((slevel > 70) ? 8 : ((slevel > 100) ? 15 : ((slevel > 140) ? 25 : 7)))));
								...
								nEquip.setStr((short) (nEquip.getStr()
										+ ((slevel > 70) ? 12 : ((slevel > 100) ? 19 : ((slevel > 140) ? 29 : 11)))));
								nEquip.setDex((short) (nEquip.getDex()
										+ ((slevel > 70) ? 12 : ((slevel > 100) ? 19 : ((slevel > 140) ? 29 : 11)))));
								nEquip.setInt((short) (nEquip.getInt()
										+ ((slevel > 70) ? 12 : ((slevel > 100) ? 19 : ((slevel > 140) ? 29 : 11)))));
								...
								nEquip.setWatk((short) (nEquip.getWatk()
										+ ((slevel > 70) ? 2 : ((slevel > 100) ? 5 : ((slevel > 140) ? 9 : 2)))));
								nEquip.setMatk((short) (nEquip.getMatk()
										+ ((slevel > 70) ? 2 : ((slevel > 100) ? 5 : ((slevel > 140) ? 9 : 2)))));
								nEquip.setEnhance((byte) 6);
							} else if (senhance == 6) {
								nEquip.setWatk((short) (nEquip.getWatk()
										+ ((slevel > 70) ? 3 : ((slevel > 100) ? 6 : ((slevel > 140) ? 10 : 3)))));
								nEquip.setMatk((short) (nEquip.getMatk()
										+ ((slevel > 70) ? 3 : ((slevel > 100) ? 6 : ((slevel > 140) ? 10 : 3)))));
								nEquip.setEnhance((byte) 7);
							} else if (senhance == 7) {
								nEquip.setWatk((short) (nEquip.getWatk()
										+ ((slevel > 70) ? 4 : ((slevel > 100) ? 7 : ((slevel > 140) ? 11 : 5)))));
								nEquip.setMatk((short) (nEquip.getMatk()
										+ ((slevel > 70) ? 4 : ((slevel > 100) ? 7 : ((slevel > 140) ? 11 : 5)))));
								nEquip.setEnhance((byte) 8);
							} else if (senhance == 8) {
								nEquip.setWatk((short) (nEquip.getWatk()
										+ ((slevel > 70) ? 5 : ((slevel > 100) ? 8 : ((slevel > 140) ? 12 : 8)))));
								nEquip.setMatk((short) (nEquip.getMatk()
										+ ((slevel > 70) ? 5 : ((slevel > 100) ? 8 : ((slevel > 140) ? 12 : 8)))));
								nEquip.setEnhance((byte) 9);
							} else if (senhance == 9) {
								nEquip.setWatk((short) (nEquip.getWatk()
										+ ((slevel > 70) ? 6 : ((slevel > 100) ? 9 : ((slevel > 140) ? 13 : 12)))));
								nEquip.setMatk((short) (nEquip.getMatk()
										+ ((slevel > 70) ? 6 : ((slevel > 100) ? 9 : ((slevel > 140) ? 13 : 12)))));
								nEquip.setEnhance((byte) 10);
							} else {
								nEquip.setStr((short) (nEquip.getStr()
										+ ((slevel > 70) ? 15 : ((slevel > 100) ? 20 : ((slevel > 140) ? 30 : 10)))));
								nEquip.setDex((short) (nEquip.getDex()
										+ ((slevel > 70) ? 15 : ((slevel > 100) ? 20 : ((slevel > 140) ? 30 : 10)))));
								nEquip.setInt((short) (nEquip.getInt()
										+ ((slevel > 70) ? 15 : ((slevel > 100) ? 20 : ((slevel > 140) ? 30 : 10)))));
							...
							if (nEquip.getWatk() > 0) {
								nEquip.setWatk(
										(short) (nEquip.getWatk() + getEquipLevel(getReqLevel(nEquip.getItemId()))));
							}
							if (nEquip.getWdef() > 0) {
								nEquip.setWdef((short) (nEquip.getWdef()
							...
							if (nEquip.getMatk() > 0) {
								nEquip.setMatk(
										(short) (nEquip.getMatk() + getEquipLevel(getReqLevel(nEquip.getItemId()))));
							}
							if (nEquip.getMdef() > 0) {
								nEquip.setMdef((short) (nEquip.getMdef()
										+ getEquipLevel(getReqLevel(nEquip.getItemId()) + Randomizer.rand(1, 2))));
			...
			if (!GameConstants.isCleanSlate(scrollId.getItemId())
					...
					&& !GameConstants.isRebirthFireScroll(scrollId.getItemId()) && scrollId.getItemId() != 2049360
					&& scrollId.getItemId() != 2049361) {
				if (ItemFlag.SAFETY_SHIELD.check(nEquip.getFlag()) && failed) {
					chr.dropMessage(5, "因卷軸效果，陞級可用次數未减少.");
				} else {
					nEquip.setUpgradeSlots(
							(byte) (nEquip.getUpgradeSlots() - getUpgradeScrollUseSlot(scrollId.getItemId())));
					if (zeroEquip != null) {
						zeroEquip.setUpgradeSlots(
								(byte) (zeroEquip.getUpgradeSlots() - getUpgradeScrollUseSlot(scrollId.getItemId())));
					}
				}
				if (!failed) {
					...
					if (zeroEquip != null) {
						...
						chr.getClient().send(
								CWvsContext.InventoryPacket.addInventorySlot(MapleInventoryType.EQUIPPED, zeroEquip));
					}
				}
			}
		}
	...
	public final Item getEquipById(int equipId, long ringId, boolean rebirth) {
		...
		if (!isCash(equipId) && rebirth) {
			...
			if (ItemFlag.UNTRADEABLE.check(eqz.getFlag()) && eqz.getKarmaCount() < 0
					&& (isKarmaEnabled(equipId) || isPKarmaEnabled(equipId))) {
	...
	public void initItemAddData(ResultSet sqlAddData) throws SQLException {
		...
		while (sqlAddData.next()) {
			this.tmpInfo.equipAdditions.add(new Triple<>(sqlAddData.getString("key"), sqlAddData.getString("subKey"),
	...
	public void initItemInformation(ResultSet sqlItemData) throws SQLException {
		...
		if (ret.monsterBook > 0 && itemId / 10000 == 238) {
			...
			for (Map.Entry<Integer, Triple<Integer, List<Integer>, List<Integer>>> set : this.monsterBookSets
					.entrySet()) {
	...
	public Pair<String, Boolean> isSuperial(int itemid) {
		...
		if ((itemid >= 1102481 && itemid <= 1102485) || (itemid >= 1072743 && itemid <= 1072747)
