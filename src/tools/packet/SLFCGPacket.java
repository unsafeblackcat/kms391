...
public class SLFCGPacket {
	...
	// public static byte[] BlockGameControlPacket(int velocity, int
	// misplaceallowance) {
//    MaplePacketLittleEndianWriter w = new MaplePacketLittleEndianWriter();
...
//  }
	...
	public static byte[] StarDustUI(int questId, String path, long Point, long Coin) {
		...
		packet.writeInt(0); // 追加每天調整最大值的UI添加
		packet.writeInt(0); // 0正確
		packet.write(1);
		packet.writeDouble(1.0);

		packet.writeInt(Point); // 入場時有剩餘的量規
		packet.writeInt(Coin); // 總數
	...
	public static byte[] DreamBreakerRanking(String name) {
		...
		packet.writeInt(
				DreamBreakerRank.Rank.containsKey(name) ? ((Integer) DreamBreakerRank.Rank.get(name)).intValue() : 0);
	...
	public static byte[] SetIngameDirectionMode(boolean Enable, boolean BlackFrame, boolean ForceMouseOver,
			boolean ShowUI) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	public static class OneCardGamePacket {
		...
		public static byte[] CreateUI(MapleCharacter chr, int position, List<MapleCharacter> chrs) {
			...
			for (int a = 0; a < chrs.size(); a++) {
				mplew.write(a);
				AvatarLook.encodeAvatarLook(mplew, chrs.get(a), (chr.getId() == ((MapleCharacter) chrs.get(a)).getId()),
						false, true);
		...
		public static byte[] onStart(List<OneCardGame.OneCardPlayer> players) {
			...
			for (int a = 0; a < players.size(); a++) {
				...
				mplew.writeInt(0);
				mplew.writeInt(0); // playCount等級相關數据包
		...
		public static byte[] onUserPossibleAction(OneCardGame.OneCardPlayer player, List<OneCardGame.OneCard> cards,
				boolean bGetCardFromGraves, boolean bClockWiseTurn) {
			MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
			mplew.writeShort(SendPacketOpcode.PLAYER_INTERACTION.getValue());
			mplew.write(160);
			mplew.write(15); // time
			...
			mplew.write(bClockWiseTurn); // 順時針方向與否呵呵
	...
	public static class MultiYutGamePacket {
		...
		public static byte[] MovedHorse(List<MultiYutGame.MultiYutPlayer> players, MultiYutGame.MultiYutPlayer owner,
				List<Integer> layout, int nowpoint, int whoturn, int movehorsepos, boolean catched, boolean onemore) {
			MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
			mplew.writeShort(SendPacketOpcode.MAPLE_YUT.getValue());
			mplew.write(1);
			mplew.writeInt(3);
	...
	public static class MonsterPyramidPacket {
		...
		public static byte[] createUI(List<MapleCharacter> list) {
			...
			(maplePacketLittleEndianWriter = new MaplePacketLittleEndianWriter())
					.writeShort(SendPacketOpcode.MONSTER_PYRAMID.getValue());
		...
		public static byte[] onInit(List<MapleCharacter> list) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				maplePacketLittleEndianWriter.writeInt(1);
				for (int j = 0; j < 6; j++) {
					maplePacketLittleEndianWriter.writeInt(((MapleCharacter) list.get(i)).getMonsterPyramidInstance()
							.getPlayer(list.get(i)).getSelectBlockType(j));
				}
			}
			maplePacketLittleEndianWriter.write(0);
			return maplePacketLittleEndianWriter.getPacket();
		...
		public static byte[] Handler(List<MapleCharacter> list, boolean first, int... args) {
			...
			case 3:
				if (!first) {
					for (int j = 0; j < list.size(); j++) {
						...
						mplew.writeInt(((MapleCharacter) list.get(j)).getMonsterPyramidInstance().getPlayer(list.get(j))
								.getPoint());
						mplew.writeInt(((MapleCharacter) list.get(j)).getMonsterPyramidInstance().getPlayer(list.get(j))
								.getRank());
						for (int m = 0; m < 6; m++) {
							mplew.writeInt(((MapleCharacter) list.get(j)).getMonsterPyramidInstance()
									.getPlayer(list.get(j)).getSelectBlockType(m));
						}
					}
					mplew.write(3);
					mplew.writeInt(args[4]);
			...
			case 5:
				for (i = 0; i < list.size(); i++) {
					...
					mplew.writeInt(((MapleCharacter) list.get(i)).getMonsterPyramidInstance().getPlayer(list.get(i))
							.getPoint());
					mplew.writeInt(((MapleCharacter) list.get(i)).getMonsterPyramidInstance().getPlayer(list.get(i))
							.getRank());
					for (int j = 0; j < 6; j++) {
						mplew.writeInt(((MapleCharacter) list.get(i)).getMonsterPyramidInstance().getPlayer(list.get(i))
								.getSelectBlockType(j));
					}
				}
				for (k = 0; k < 7; k++) {
					for (int l = 0; l < 8 - k; l++) {
	...
	public static class MultiOthelloGamePacket {
		...
		public static byte[] createUI(List<MapleCharacter> list, MapleCharacter mapleCharacter, int i) {
			MaplePacketLittleEndianWriter maplePacketLittleEndianWriter;
			(maplePacketLittleEndianWriter = new MaplePacketLittleEndianWriter())
					.writeShort(SendPacketOpcode.PLAYER_INTERACTION.getValue());
			...
			for (i = 0; i < list.size(); i++) {
				maplePacketLittleEndianWriter.write(((MapleCharacter) list.get(i)).getBattleReverseInstance()
						.getPlayer(((MapleCharacter) list.get(i)).getId()).getStoneId());
				AvatarLook.encodeAvatarLook(maplePacketLittleEndianWriter, list.get(i),
						(mapleCharacter.getId() == ((MapleCharacter) list.get(i)).getId()), false, true);
		...
		public static byte[] onInit(List<BattleReverse.BattleReverseStone> list, int n) {
			MaplePacketLittleEndianWriter maplePacketLittleEndianWriter;
			(maplePacketLittleEndianWriter = new MaplePacketLittleEndianWriter())
					.writeShort(SendPacketOpcode.PLAYER_INTERACTION.getValue() + 1);
		...
		public static byte[] onBoardUpdate(boolean b, Point point, int n, int n2,
				List<BattleReverse.BattleReverseStone> list, byte turn) {
			MaplePacketLittleEndianWriter maplePacketLittleEndianWriter;
			(maplePacketLittleEndianWriter = new MaplePacketLittleEndianWriter())
					.writeShort(SendPacketOpcode.PLAYER_INTERACTION.getValue() + 1);
	...
	public static final byte[] BlackLabelE(String msg, int delay, int textspeed, int type, int x, int y, int type1,
			int type2, int type3, String t) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.SHOW_EFFECT.getValue());
		mplew.write(61);
		mplew.writeMapleAsciiString(msg);
	...
	public static class TyoonKitchenPacket {
		...
		public static byte[] Menu(List<MapleTyoonKitchen.MapleTyoonKitchenRecipe> mty, List<MapleTyoonKitchen> mtkc) {
			...
			mplew.write(HexTool.getByteArrayFromHexString(
					"08 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 F4 01 00 00 00 00 E8 FE FF FF 81 FF FF FF 42 FF FF FF DB FF FF FF 54 E2 1E 00 21 01 00 00 02 00 BB A7 01 00 00 00 01 00 00 00 00 00 00 00 00 00 00 00 F4 01 00 00 00 00 E8 FE FF FF F6 FF FF FF 42 FF FF FF 50 00 00 00 55 E2 1E 00 21 01 00 00 04 00 B0 ED B1 E2 02 00 00 00 02 00 00 00 00 00 00 00 00 00 00 00 F4 01 00 00 00 00 E8 FE FF FF 11 FF FF FF 42 FF FF FF 6B FF FF FF 56 E2 1E 00 21 01 00 00 04 00 