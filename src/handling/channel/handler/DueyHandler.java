...
public class DueyHandler {
	...
	public static final void DueyOperation(LittleEndianAccessor slea, MapleClient c) {
		...
		case 3:
			...
			if (!c.getPlayer().isGM()) {
				c.getPlayer().dropMessage(1, "目前僅可領取道具及遊戲幣.");
				return;
			}
					...
					if (accid != c.getAccID()) {
						...
						if (inventId > 0) {
							...
							if ((GameConstants.isThrowingStar(item.getItemId())
									|| GameConstants.isBullet(item.getItemId())) && item.getQuantity() == 0) {
								amount = 0;
							}
							if (c.getPlayer().getItemQuantity(item.getItemId(), false) >= amount) {
								...
								if (!ii.isDropRestricted(item.getItemId()) && !ii.isAccountShared(item.getItemId())) {
									...
									if (addItemToDB(toSend, mesos, c.getPlayer().getName(), cid, recipientOn, letter,
											qq, quickdelivery)) {
										if (GameConstants.isThrowingStar(toSend.getItemId())
												|| GameConstants.isBullet(toSend.getItemId())) {
											MapleInventoryManipulator.removeFromSlot(c, inv, (short) (byte) itemPos,
													toSend.getQuantity(), true, false);
										} else {
											MapleInventoryManipulator.removeFromSlot(c, inv, (short) (byte) itemPos,
													amount, true, false);
										}
						...
						} else if (addMesoToDB(mesos, c.getPlayer().getName(), cid, recipientOn, letter,
								quickdelivery)) {
							c.getSession().writeAndFlush(CField.sendDuey((byte) 19, null, null));
						} else {
							c.getSession().writeAndFlush(CField.sendDuey((byte) 17, null, null));
						...
						if (recipientOn && rClient != null && quickdelivery) {
							rClient.getSession()
									.writeAndFlush(CField.receiveParcel(c.getPlayer().getName(), quickdelivery));
						}
					} else {
		...
		case 5:
			...
			if (dp.getItem() != null && c.getPlayer().haveItem(dp.getItem().getItemId(), 1, true, true)
					&& MapleItemInformationProvider.getInstance().isPickupRestricted(dp.getItem().getItemId())) {
				c.getSession().writeAndFlush(CField.sendDuey((byte) 18, null, null));
				return;
			}
	...
	public static final boolean addItemToDB(Item item, int mesos, String sName, int recipientID, boolean isOn,
		...
		try {
			...
			if (rs.next()) {
				ItemLoader.DUEY.saveItems(Collections.singletonList(item), con, rs.getInt(1),
						GameConstants.getInventoryType(item.getItemId()), true);
			}
			rs.close();
			ps.close();
			return true;
