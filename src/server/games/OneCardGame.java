...
public class OneCardGame {
	...
	public class OneCard {
		...
		/*
		 * type 0 ~ 5 -> 1 ~ 6 6 : 공격기 2개 7 : 공격기 3개 8 : 색변환 카드 9 : 1 + 1 10 : 건너뛰기 11 :
		 * 리버스 12 : 특수 공격기
		 */
		/*
		 * color 0 : 빨강 1 : 노랑 2 : 파랑 3 : 초록
		 */
	...
	private void StartGame(MapleCharacter chr) {
		...
		for (OneCard card : first.cards) {
			if (card.getType() <= 5) {
				if ((card.getColor() == getLastCard().getColor() || card.getType() == getLastCard().getType())
						&& (getLastCard().getType() <= 5
								|| (getLastCard().getType() >= 8 && getLastCard().getType() <= 11))
						&& getFire() == 0) {
		...
		sendPacketToPlayers(SLFCGPacket.OneCardGamePacket.onUserPossibleAction(first, possibleCards,
				first.cards.size() < 17, isbClockWiseTurn()));
	...
	public void skipPlayer() {
		...
		if (getLastPlayer().getCards().size() >= 17) { // 파산
			...
			getLastPlayer().chr.getClient().getSession()
					.writeAndFlush(CField.playSound("Sound/MiniGame.img/oneCard/gameover"));
		...
		if (nextPlayer != null) {
			...
			if (getOneCardDeckInfo().size() == 0 || getOneCardDeckInfo().isEmpty()) {
				...
				sendPacketToPlayers(SLFCGPacket.OneCardGamePacket.onUserPossibleAction(nextPlayer, possibleCards,
		...
		for (OneCardPlayer bp : this.getPlayers()) {
			if (bp.getPlayer().getId() == winner.getPlayer().getId()) {
				...
				if (bp.getPlayer().getKeyValue(501215, "today") >= 50) {
					...
					bp.getPlayer().gainCabinetItemPlayer(4310029, 10, 1,
							"미니게임 보상 입니다. 보관 기간 내에 수령하지 않을 시 보관함에서 사라집니다.");
				}
	...
	public void playerDead(OneCardPlayer player, boolean exit) {
		...
		if (this.getPlayers().size() == 1) {
			...
			for (int i = 0; i < this.getPlayers().size(); ++i) {
				if (this.getPlayers().get(i).position == (isbClockWiseTurn() ? (player.getPosition() + 1)
			...
			if (check) {
				...
				if (getLastCard().getColor() == 4) {
					...
					for (OneCard card : getLastPlayer().getCards()) {
						if (card.getType() <= 5) {
							if ((card.getColor() == getLastCard().getColor()
									|| card.getType() == getLastCard().getType()) && getFire() == 0) {
								possibleCards.add(card);
							}
						} else if (card.getType() <= 7) {
							if (getFire() == 0) {
								if (card.getColor() == getLastCard().getColor()
										|| card.getType() == getLastCard().getType()) {
									possibleCards.add(card);
								}
							} else {
								if (card.getType() == 6) {
									if (getLastCard().getType() == 6) {
										possibleCards.add(card);
									} else if (getLastCard().getType() == 7
											&& card.getColor() == getLastCard().getColor()) {
										possibleCards.add(card);
									}
								} else {
									if (card.getColor() == getLastCard().getColor()) {
				...
				sendPacketToPlayers(SLFCGPacket.OneCardGamePacket.onUserPossibleAction(getLastPlayer(), possibleCards,
						(possibleCards.size() == 0 && getLastPlayer().getCards().size() == 16)
								|| getLastPlayer().getCards().size() < 16,
						isbClockWiseTurn()));
				getLastPlayer().getPlayer().getClient().getSession()
						.writeAndFlush(CField.playSound("Sound/MiniGame.img/oneCard/myturn"));
				getLastPlayer().getPlayer().getClient().getSession()
						.writeAndFlush(CWvsContext.enableActions(getLastPlayer().getPlayer()));
			}
		}
	}
			...
			if (oneCardMatchingQueue.size() == req) {
				...
				Timer.EventTimer.getInstance().schedule(() -> {
					...
					for (OneCardPlayer ocp : ocg.getPlayers()) {
						ocp.chr.getClient().getSession()
								.writeAndFlush(SLFCGPacket.OneCardGamePacket.CreateUI(ocp.chr, ocp.position, chrs));
					}
				...
				World.Broadcast.broadcastSmega(CWvsContext.serverNotice(19, "",
						chr.getName() + "님이 원카드 대기열에 캐릭터를 등록했습니다. 남은 인원 : " + (req - oneCardMatchingQueue.size())));
			}
		}
	}
	...
	public static void addQueueParty(MapleCharacter leader) {
		...
		Timer.EventTimer.getInstance().schedule(() -> {
			...
			for (OneCardPlayer ocp : ocg.getPlayers()) {
				ocp.chr.getClient().getSession()
						.writeAndFlush(SLFCGPacket.OneCardGamePacket.CreateUI(ocp.chr, ocp.position, players));
			}
