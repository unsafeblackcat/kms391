...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        talk = "#fs23#爲了打倒斯烏，移動到黑色天堂核心?\r\n\r\n"
        //        if (cm.getPlayer().isGM()) {
        //talk += "#L4##b블랙헤븐 코어(익스트림 모드)#k로 이동 한다.(레벨 190이상)#l\r\n"
        //      }
        talk += "#L1#移動到黑色天堂核心(#b普通模式#k).(等級190以上)#l\r\n"
        ...
        st = selection;
        if (cm.getParty() == null) {
            cm.sendOkS("必須組成1人以上的隊伍才能進入。", 0x24);
            cm.getPlayer().dropMessage(6, "在這裡？ 1");
            cm.dispose();
            return;
            ...
            cm.getPlayer().dropMessage(6, "在這裡? 2");
            cm.dispose();
            return;
        } else if (!cm.isLeader()) {
            cm.sendOkS("只有隊長才能申請進入。", 0x24);
            cm.dispose();
            return;
        ...
        if (!cm.isBossAvailable(setting[st][0], setting[st][1]) && st != 2 && st != 3) {
            ...
            talk += "#k#n的等級不足。 斯烏 " + name[st] + "模式需要" + setting[st][3] + " 級以上才能進入。";
