...
public class ExternalCodeTableGetter {
	...
	private final <T extends Enum<? extends WritableIntValueHolder>> short getValue(String name, T[] values,
			short def) {
		...
		if (prop != null && prop.length() > 0) {
			...
			short base = 0;
			if (args.length == 2) {
				base = ((WritableIntValueHolder) ((Object) ExternalCodeTableGetter.valueOf((String) args[0], values)))
						.getValue();
				if (base == def) {
	...
	public static final <T extends Enum<? extends WritableIntValueHolder>> void populateValues(Properties properties,
			T[] values) {
