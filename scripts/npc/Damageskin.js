...
function action(mode, type, sel) {
    ...
    if (status == 0) {
        var msg = "#fs15#" + enter;
        msg += "#fc0xFF000000#這裡準備了能讓狩獵更加豐富多彩的各種傷害皮膚。#b" + enter;
        msg += "隨機抽取 - #i" + need + "##z" + need + "# " + qty + "個" + enter;
        msg += "選擇抽取 - #i" + 選擇需求 + "##z" + 選擇需求 + "# " + 選擇數量 + "個#b" + enter;
        msg += "#L1##fs15##d查看物品清單" + enter;
        msg += "#L2#我要隨機抽取\r\n";
        msg += "#L3#我要選擇傷害皮膚!!";
        ...
        switch (sel) {
            ...
            case 2:
                ...
                a = Randomizer.rand(0,  (傷害皮膚.length - 1));
                cm.gainItem(need,  -qty);
                cm.gainItem( 傷害皮膚[a], 1);
                cm.sendOkS("#fs15# 你獲得了以下物品：#fs15##b" + enter + "#i" + 傷害皮膚[a] + "##z" + 傷害皮膚[a] + "#",0x86);
                cm.dispose(); 
                break;
            case 3:
                var msg = "請選擇你想要的傷害皮膚：#fs15##b" + enter;
                for (i = 0; i < 傷害皮膚.length; i++) 
                    msg += "#L" + i + "##i" + 傷害皮膚[i] + "##z" + 傷害皮膚[i] + "#" + enter;
                cm.sendSimpleS(msg,0x86); 
                break;
        }
    } else if (status == 2) {
        if (!cm.haveItem( 選擇需求, 選擇數量)) {
            cm.sendOkS(" 所需材料不足。",0x86);
            cm.dispose(); 
            return;
        }
        a = sel;
        cm.gainItem( 選擇需求, -選擇數量);
        cm.gainItem( 傷害皮膚[a], 1);
        cm.sendOkS(" 你獲得了以下物品！#fs15##b" + enter + "#i" + 傷害皮膚[a] + "##z" + 傷害皮膚[a] + "#",0x86);
        cm.dispose(); 
    }
}