...
function setup(eim) {
    ...
    var eim = em.newInstance("URS");
    var map = eim.setInstanceMap(970072000); //측정맵
    map.resetFully();
    map.killMonster(9833376); //몬스터코드
    var mob = em.getMonster(9833376); //몬스터코드
    mob.setHp(9000000000000);
    eim.registerMonster(mob);
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(200, 100)); //좌표
    eim.startEventTimer(12000); //시간 (1s = 1000)
    return eim;
}

function playerEntry(eim, player) {
    var map = eim.getMapFactory().getMap(970072000); //측정맵
    player.changeMap(map, map.getPortal(0));
    em.setProperty("entry", "false");
}

function changedMap(eim, player, mapid) {
    if (mapid != 970072000) { //측정맵
        em.setProperty("entry", "true");
...
function end(eim) {
        ...
        var map = eim.getMapFactory().getMap(680000710); //나갈맵
        player.changeMap(map, map.getPortal(0));
