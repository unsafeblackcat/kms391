...
public class MapleMapItem extends MapleMapObject {
	...
	public MapleMapItem(Item item, Point position, MapleMapObject dropper, MapleCharacter owner, byte type,
			boolean playerDrop, Equip equip) {
		setPosition(position);
		this.item = item;
		this.dropper = dropper;
		this.owner = owner;
	...
	public void sendSpawnData(MapleClient client) {
		if ((this.questid <= 0 || client.getPlayer().getQuestStatus(this.questid) == 1)
			...
			client.getSession().writeAndFlush(
					CField.dropItemFromMapObject(client.getPlayer().getMap(), this, null, getTruePosition(), (byte) 2,
							(client.getPlayer().getBuffedEffect(SecondaryStat.PickPocket) != null)));
		}
	}
	...
	public void expire(MapleMap map) {
		...
		if (this.owner != null) {
			...
			if (pickPocket != null) {
				...
				this.owner.getClient().getSession().writeAndFlush(
						CWvsContext.BuffPacket.giveBuff(pickPocket.getStatups(), pickPocket, this.owner));
			}
		}
	}
