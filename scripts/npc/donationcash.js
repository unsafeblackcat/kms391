...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        말 = "#fs15#" + 검정 + "안녕하세요 #b#h ##k" + 검정 + "님! [후원] 치장아이템 스탯부여 NPC 입니다.\r\n\r\n"
        ...
        sssss = selection;
        if (selection == 0) {
            var txt = "#fs15#" + 검정 + "어떤 아이템을 강화하실건가요?\r\n";
            txt += "#fc0xFFD5D5D5#─────────────────────────────#k\r\n";
        ...
        cm.sendNextS("#fs15#정말 #r#i" + item.getItemId() + ":# #z" + item.getItemId() + ":##k을 강화 하시겠나요? 선택은 되돌릴 수 없으니 신중하게 선택하여 주세요.", 4, 9075000);
    } else if (status == 3) {
        if (sssss == 1) {
            ...
            if (item.getItemId() >= 1112000 && item.getItemId() <= 1112099 || item.getItemId() >= 1112800 && item.getItemId() <= 1112899) {
                cm.sendOkS("#fs15#" + 검정 + "커플링과 우정링은 강화가 불가능해요.", 0x04, 9075000);
                cm.dispose();
                return;
            }
            if (item.getStr() > 499 || item.getDex() > 499 || item.getInt() > 499 || item.getLuk() > 499 || item.getWatk() > 299 || item.getMatk() > 299 || item.getHp() > 2490) {
                cm.sendOkS("#fs15#" + 검정 + "더 이상 강화는 불가능해요! 한 아이템에 최대 #r100회#d 까지만 강화할 수 있습니다.", 0x04, 9075000);
                cm.dispose();
                return;
            }
            if (item.getItemId() >= 1802000 && item.getItemId() <= 1802999) {
                cm.sendOkS("#fs15#" + 검정 + "펫 장비는 강화가 불가능해요.", 0x04, 9075000);
                cm.dispose();
                return;
            }
            if (!cm.isCash(item.getItemId())) {
                cm.sendOkS("#fs15#" + 검정 + "비정상적인 아이템인것 같습니다.", 0x04, 9075000);
                cm.dispose();
                return;
            }
            var suc = false;
            ...
            if (!suc) {
                ...
                말 = "#fs15#" + 검정 + "강화 실패시 뜨는 문구인데 뜨면 운영자에게 스샷과 함께 복구신청 부탁드립니다.\r\n"
                말 += "#fc0xFFD5D5D5#───────────────────────────#k#l\r\n";
                ...
                cm.getPlayer().forceReAddItem(item, Packages.client.inventory.MapleInventoryType.getByType(6));
                말 = "#fs15#" + 검정 + "강화가 완료되었습니다. #b#h ##k" + 검정 + "님이 강화하신 아이템의 옵션입니다. \r\n"
                말 += "#fc0xFFD5D5D5#───────────────────────────#k#l\r\n";
                말 += "#i" + item.getItemId() + "# #fc0xFF4641D9##z" + item.getItemId() + "##k\r\n"
                말 += "#fc0xFF6799FF#STR : + " + (item.getStr() + item.getEnchantStr()) + "\r\n"
                말 += "DEX : + " + (item.getDex() + item.getEnchantDex()) + "\r\n"
