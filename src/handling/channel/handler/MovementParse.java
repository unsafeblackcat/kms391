...
public class MovementParse {
	...
	public static List<LifeMovementFragment> parseMovement(final LittleEndianAccessor rh, final int kind,
			MapleCharacter chr) {
		...
		for (byte i = 0; i < numCommands; i++) {
			...
			case 28: {
				...
				final UnknownMovement3 um = new UnknownMovement3(command, new Point(0, 0), duration, newstate,
						(short) 0, (byte) force);
				um.setUnow(nnow);
				res.add(um);
				break;
			...
			case 49: { // 342
				...
				final short duration = rh.readShort();
				final byte unk = rh.readByte();
				final UnknownMovement2 um = new UnknownMovement2(command, new Point(xpos, ypos), duration, newstate,
						unk);
			...
			case 68: {
				...
				final short duration = rh.readShort();
				final byte unk = rh.readByte();
				final UnknownMovement um = new UnknownMovement(command, new Point(xpos, ypos), duration, newstate, fh,
						unk);
				um.setPixelsPerSecond(new Point(xwobble, ywobble));
				res.add(um);
				break;
			...
			default: {
				if (command == 78 || command == 80) {
					...
					final short xoffset = rh.readShort();
					final short yoffset = rh.readShort();
					final UnknownMovement4 alm = new UnknownMovement4(command, new Point(xpos, ypos), 0, 0, fh,
							(byte) 0);
					alm.setPixelsPerSecond(new Point(xwobble, ywobble));
					alm.setOffset(new Point(xoffset, yoffset));
					res.add(alm);
