...
public class PlayerStats implements Serializable {
	...
	private static final int[] allJobs = new int[] { 0, 10000, 10000000, 20000000, 20010000, 20020000, 30000000,
			30010000 };
	public static final int[] pvpSkills = new int[] { 1000007, 2000007, 3000006, 4000010, 5000006, 5010004, 11000006,
			12000006, 13000005, 14000006, 15000005, 21000005, 22000002, 23000004, 31000005, 32000012, 33000004,
			35000005 };
	...
	public final boolean setHp(long newhp, boolean silent, MapleCharacter chra, boolean igskill) {
		...
		if (chra != null) {
			...
			if (!igskill && GameConstants.isDemonAvenger(chra.getJob()) && chra.getBuffedValue(400011112)
					&& this.hp <= 0L) {
				...
				chra.getClient().getSession().writeAndFlush((Object) CWvsContext.BuffPacket.giveBuff(statups,
						chra.getBuffedEffect(SecondaryStat.Revenant), chra));
				chra.updateSingleStat(MapleStat.HP, this.hp);
				return true;
			}
			if (oldHp > this.hp && !chra.isAlive()) {
				if (!igskill) {
					if (!chra.skillisCooling(1320016) && !chra.skillisCooling(1320019) && chra.getBuffedValue(1321020)
							&& chra.getSkillLevel(1320016) > 0) {
						SecondaryStatEffect rein = SkillFactory.getSkill(1320019)
								.getEffect(chra.getSkillLevel(1320016));
						...
						if (chra.getBuffedEffect(SecondaryStat.ReviveOnce).getSourceId() == 24111002) {
							chra.getClient().getSession()
									.writeAndFlush((Object) CField.EffectPacket.showEffect(chra,
											chra.getBuffSource(SecondaryStat.ReviveOnce),
											chra.getBuffSource(SecondaryStat.ReviveOnce), 1, 0, 0, (byte) 0, true, null,
											null, null));
							chra.getMap().broadcastMessage(chra,
									CField.EffectPacket.showEffect(chra, chra.getBuffSource(SecondaryStat.ReviveOnce),
											chra.getBuffSource(SecondaryStat.ReviveOnce), 1, 0, 0, (byte) 0, false,
											null, null, null),
									false);
						} else if (chra.getJob() == 1412) { // 다크니스 어센션
							chra.getClient().getSession()
									.writeAndFlush((Object) CField.EffectPacket.showEffect(chra, 3000, 14110030, 63, 0,
											0, (byte) (chra.isFacingLeft() ? 1 : 0), true, chra.getTruePosition(), null,
											null));
							chra.getMap().broadcastMessage(chra,
									CField.EffectPacket.showEffect(chra, 3000, 14110030, 63, 0, 0,
											(byte) (chra.isFacingLeft() ? 1 : 0), false, chra.getTruePosition(), null,
											null),
									false);
						}
						this.hp = chra.getStat().getCurrentMaxHp() / 2L;
						...
						chra.getClient().getSession().writeAndFlush(
								(Object) CField.skillCooldown(effect.getSourceId(), effect.getCooldown(chra)));
...
//        this.reduceCooltime = new ArrayList<Integer>();
	...
	public void recalcLocalStats(final boolean first_login, final MapleCharacter chra) {
		...
		int jokerItemId = 0, starforce = 0, plushp = 0;
			...
			while (itera.hasNext()) {
				...
				for (final Integer potential : potentials) {
					...
					if (potential != 0 && ii.getPotentialInfo(potential) != null) {
						...
						localmaxhp_ += ii.getPotentialInfo(potential).get(lv).incMHP
								/ (GameConstants.isDemonAvenger(chra.getJob()) ? 2 : 1);
						...
						if ((potential >= 201 && potential <= 212) || potential == 391 || potential == 10081
								|| potential == 12081 || potential == 12082 || potential == 12801) {
				...
				if (GameConstants.isDemonAvenger(chra.getJob()) && equip.getEnhance() > 0) {
					...
					if (starforce > 0 && starforce <= 425) {
						plushp = ((starforce > 391) ? 168
								: ((starforce > 380 && starforce <= 390) ? 166
										: ((starforce > 370 && starforce <= 380) ? 164
												: ((starforce > 360 && starforce <= 370) ? 162
														: ((starforce > 350 && starforce <= 360) ? 160
																: ((starforce > 340 && starforce <= 350) ? 158
																		: ((starforce > 330 && starforce <= 340) ? 156
																				: ((starforce > 320 && starforce <= 330)
																						? 154
																						: ((starforce > 310
																								&& starforce <= 320)
																										? 152
				...
				if (GameConstants.isArcaneSymbol(equip.getItemId())
					...
					localmaxhp_ += ((GameConstants.isDemonAvenger(chra.getJob())
							|| (GameConstants.isZero(chra.getJob()) && GameConstants.isWeapon(equip.getItemId())))
									? (equip.getHp() / 2)
									: equip.getHp());
				...
				this.starforce += (GameConstants.isLongcoat(equip.getItemId()) ? (equip.getEnhance() * 2)
						: equip.getEnhance());
				...
				if (ii.getEquipStats(equip.getItemId()) != null
						&& ii.getEquipStats(equip.getItemId()).get("MHPr") != null) {
					this.percent_hp += ii.getEquipStats(equip.getItemId()).get("MHPr");
				}
				if (ii.getEquipStats(equip.getItemId()) != null
						&& ii.getEquipStats(equip.getItemId()).get("MMPr") != null) {
			...
			if (set2 != null) {
				...
				if (set2.jokerPossible && jokerItemId > 0 && entry.getValue() < set2.completeCount) {
					for (final int itemId : set2.itemIDs) {
						...
						if (!GameConstants.isWeapon(itemId) && !GameConstants.isWeapon(jokerItemId)
								&& itemId / 10000 == jokerItemId / 10000
								&& chra.getInventory(MapleInventoryType.EQUIPPED).findById(itemId) == null) {
							entry.setValue(entry.getValue() + 1);
							break;
						}
					}
		...
		this.wdef += (int) Math
				.floor(this.localstr * 1.2 + (this.localdex + this.localluk) * 0.5 + this.localint_ * 0.4);
		this.mdef += (int) Math
				.floor(this.localstr * 0.4 + (this.localdex + this.localluk) * 0.5 + this.localint_ * 1.2);
	...
	private Pair<Long, Long> handleUnionSkills(MapleCharacter chra) {
		...
		try {
			for (MapleUnion union : chra.getUnions().getUnions()) {
				...
				if (UnionHandler.cardSkills.containsKey(union.getJob() / 10) && (jobSkillEffect = SkillFactory
						.getSkill(jobSkill = UnionHandler.cardSkills.get(union.getJob() / 10).intValue())
						.getEffect(level)) != null) {
					switch (jobSkill) {
					case 71000013: {
				...
				if (!UnionHandler.characterSizes.containsKey(type)
						|| !UnionHandler.characterSizes.get(type).containsKey(level - 1)) {
					continue;
				}
				...
				Point main = null;
				int indexof = 0;
				int Jobtype = GameConstants.isWarrior(union.getJob()) ? 1
						: (GameConstants.isMagician(union.getJob()) ? 2
								: (GameConstants.isArcher(union.getJob()) ? 3
										: (GameConstants.isThief(union.getJob()) ? 4 : 5)));
				...
				for (Map.Entry<Integer, Integer> entry : skills.entrySet()) {
					SecondaryStatEffect fieldEffect = SkillFactory.getSkill((Integer) entry.getKey())
							.getEffect((Integer) entry.getValue());
					if (fieldEffect == null) {
						continue;
					}
					int check = fieldEffect.getSourceId() % 10;
					if (chra.getKeyValue(18791, check + "") != -1L
							&& chra.getKeyValue(18791, check + "") != (long) check) {
	...
	/*
	 * Opcode count of 22319 triggered aggressive code reduction. Override with
	...
	private Pair<Long, Long> handlePassiveSkills(MapleCharacter chra) {
		...
		int[] up2 = skilllist = new int[] { 400001000, 400001001, 400001002, 400001003, 400001004, 400001006, 400001007,
				400001021, 400011066, 400021068, 400021095, 400031005, 400041032, 400051000, 400051072 };
		i = up2.length;
		...
		int[] i2 = skilllist1 = new int[] { 80000654, 80000655, 80000656, 80000657, 80000658, 80000659, 80000660,
				80000661 };
		n = i2.length;
		...
		int[] linkskills = new int[] { 80002766, 80002774, 80000055, 80000002, 80000005, 80000050, 80000001, 80000047,
				80000110, 80000188, 80002857, 80000609, 80000006 };
		...
		case 132: {
			...
			if (bof > 0) {
				int mastery;
				int n2 = chra.getSkillLevel(1120003) > 0
						? (int) SkillFactory.getSkill(1120003).getEffect(chra.getSkillLevel(1120003)).getMastery()
						: (mastery = 0);
			...
			if (bof > 0) {
				byte mastery;
				byte by = chra.getSkillLevel(1220018) > 0
						? SkillFactory.getSkill(1220018).getEffect(chra.getSkillLevel(1220018)).getMastery()
						: (mastery = 0);
			...
			if (bof > 0) {
				byte mastery;
				byte by = chra.getSkillLevel(1320018) > 0
						? SkillFactory.getSkill(1320018).getEffect(chra.getSkillLevel(1320018)).getMastery()
						: (mastery = 0);
				this.Mastery += by > 0 ? (double) by : (double) bx.getEffect(bof).getMastery();
			...
			if (bof > 0) {
				...
				if (chra.getStat().getHPPercent() >= eff.getX()) {
					this.critical_rate = (short) (this.critical_rate + eff.getCr());
					this.critical_damage = (short) (this.critical_damage + bx.getEffect(bof).getCriticalDamage());
					short up4 = chra.getBuffedValue(SecondaryStat.Reincarnation) != null
							&& chra.getTotalSkillLevel(1320046) > 0
									? SkillFactory.getSkill(1320046).getEffect(1).getDamage()
									: (short) 0;
		...
		case 232: {
			...
			if (bof > 0) {
				byte mas = 0;
				byte by = chra.getTotalSkillLevel(2121005) > 0
			...
			if (bof > 0) {
				byte mas = 0;
				byte by = chra.getTotalSkillLevel(2221005) > 0
			...
			if (bof > 0) {
				byte mas = 0;
				byte by = chra.getTotalSkillLevel(2310008) > 0
		...
		case 322: {
			...
			if (bof > 0) {
				byte mas = 0;
				--this.attackSpeed;
				byte by = chra.getTotalSkillLevel(3120005) > 0
			...
			if (bof > 0) {
				byte mas = 0;
				byte by = chra.getTotalSkillLevel(3220004) > 0
...
//                }
		...
		case 332: {
			...
			if (bof > 0) {
				byte mas = 0;
				--this.attackSpeed;
				byte by = chra.getTotalSkillLevel(3320010) > 0
		...
		case 422: {
			...
			if (chra.getJob() >= 410 && chra.getJob() <= 412) {
				...
				if (bof > 0) {
					byte mas = 0;
					byte by = chra.getTotalSkillLevel(4120012) > 0
			...
			if (bof > 0) {
				byte mas = 0;
				byte by = chra.getTotalSkillLevel(4220012) > 0
		...
		case 434: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(4340013) > 0
		...
		case 522: {
			...
			if (GameConstants.isViper(chra.getJob())) {
				...
				if (bof > 0) {
					byte mas = 0;
					eff = bx.getEffect(bof);
					byte by = chra.getTotalSkillLevel(5121015) > 0
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(5220020) > 0
		...
		case 532: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(5320009) > 0
		...
		case 1111:
		case 1112: {
			bx = SkillFactory.getSkill(11000021); // 靈魂衛士
			bof = chra.getTotalSkillLevel(bx);
			if (bof > 0) {
				eff = bx.getEffect(bof);
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(11120007) > 0
		...
		case 1212: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(12120009) > 0
		...
		case 1312: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(13120006) > 0
		...
		case 1412: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(14120005) > 0
		...
		case 1512: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(15120006) > 0
		...
		case 2112: {
			...
			if (bof > 0) {
				...
				if (chra.getCombo() >= 50) {
					...
					int n3 = chra.getSkillLevel(21110000) > 0
							? SkillFactory.getSkill(21110000).getEffect(chra.getSkillLevel(21110000)).getZ()
							: (pluswatk = 0);
					int pluscri = chra.getSkillLevel(21110000) > 0
							? SkillFactory.getSkill(21110000).getEffect(chra.getSkillLevel(21110000)).getY()
							: 0;
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(21120001) > 0
		...
		case 2218: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(22170071) > 0
		...
		case 2312: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(23120009) > 0
		...
		case 2412: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(24120006) > 0
		...
		case 2512: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(25120113) > 0
		...
		case 2712: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(27120007) > 0
		...
		case 3112: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(31120008) > 0
		...
		case 3122: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(31220006) > 0
		...
		case 3212: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(32120016) > 0
		...
		case 3312: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(33120000) > 0
		...
		case 3512: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(35120000) > 0
			...
			if (bof > 0) {
				...
				int[] a = skillid2 = new int[] { 35121003, 35101012, 35121009, 35120002, 35111008, 35111002, 400051068,
						400051009 };
				int n4 = a.length;
				for (int j = 0; j < n4; ++j) {
					Integer s = a[j];
		...
		case 3612: {
			...
			if (bof > 0) {
				eff = bx.getEffect(bof);
				final int mas = (chra.getTotalSkillLevel(36120006) <= 0) ? 0
						: SkillFactory.getSkill(36120006).getEffect(chra.getSkillLevel(36120006)).getMastery();
		...
		case 3712: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(37120010) > 0
		...
		case 5112: {
			...
			// othello
			bx = SkillFactory.getSkill(51120003); // 高級靈魂盾
			bof = chra.getTotalSkillLevel(bx);
			if (bof > 0) {
				eff = bx.getEffect(bof);
			...
			bx = SkillFactory.getSkill(50000001); // 靈魂遮罩
			bof = chra.getTotalSkillLevel(bx);
			if (bof > 0) {
				eff = bx.getEffect(bof);
				this.percent_hp += eff.getX();
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(51120001) > 0
		...
		case 6112: {
			...
			if (bof > 0) {
				eff = bx.getEffect(bof);
				this.percent_hp += eff.getPercentHP();
				int up6 = chra.getKaiserCombo() >= 300 ? 3
						: (chra.getKaiserCombo() >= 200 ? 2 : (chra.getKaiserCombo() >= 100 ? 1 : 0));
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(61120012) > 0
		...
		case 6312: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(63120013) > 0
		...
		case 6412: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(64120008) > 0
		...
		case 6512: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(65120005) > 0
		...
		case 14212: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(142120013) > 0
		...
		case 15112: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(151120007) > 0
		...
		case 15212: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(152120015) > 0
		...
		case 15512: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(155120010) > 0
		...
		case 16412: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(164120010) > 0
		...
		case 16212: {
			...
			if (bof > 0) {
				byte mas = 0;
				eff = bx.getEffect(bof);
				byte by = chra.getTotalSkillLevel(162120025) > 0
		...
		case 15412: { // 卡利被動處理
			...
			bx = SkillFactory.getSkill(154100007); // 體能訓練
			bof = chra.getTotalSkillLevel(bx);
			if (bof > 0) {
				eff = bx.getEffect(bof);
				this.localluk += eff.getLukX();
			}
			bx = SkillFactory.getSkill(154000004); // 喇叭
			bof = chra.getTotalSkillLevel(bx);
			if (bof > 0) {
				eff = bx.getEffect(bof);
				this.watk += bx.getEffect(bof).getX();
			...
			bx = SkillFactory.getSkill(154000005); // 頁面-添加回避率
			bof = chra.getTotalSkillLevel(bx);
			if (bof > 0) {
				eff = bx.getEffect(bof);
				this.speed += eff.getPsdSpeed();
			...
			bx = SkillFactory.getSkill(154000005); // 衝擊力
			bof = chra.getTotalSkillLevel(bx);
			if (bof > 0) {
				eff = bx.getEffect(bof);
				this.critical_rate = (short) (this.critical_rate + eff.getCr());
			...
			bx = SkillFactory.getSkill(154110008); // 插圖
			bof = chra.getTotalSkillLevel(bx);
			if (bof > 0) {
				eff = bx.getEffect(bof);
				this.watk += eff.getWatk();
				this.InsertFinalDamage(eff.getPdR());
				this.ignoreMobpdpR.add(Integer.valueOf(eff.getIgnoreMob()));
				this.InsertDamageIncrease(154101001, bx.getEffect(bof).getDAMRate()); // 藝術許可證
			}
			bx = SkillFactory.getSkill(154110009); // 維吉爾倫斯
			bof = chra.getTotalSkillLevel(bx);
			if (bof > 0) {
				eff = bx.getEffect(bof);
				this.percent_hp += bx.getEffect(bof).getPercentHP();
		...
		if (GameConstants.isResist(chra.getJob())
				&& (bof = chra.getTotalSkillLevel(bx = SkillFactory.getSkill(30000002))) > 0) {
	...
	private Pair<Long, Long> handleBuffStats(MapleCharacter chra) {
		...
		if ((buff = chra.getBuffedValue(SecondaryStat.DiceRoll)) != null) {
			...
			this.BossDamage = (int) ((double) this.BossDamage
					* (((double) GameConstants.getDiceStat(buff, 5) + 100.0) / 100.0));
		}
		// 퍼번트 드레인
		...
		if (chra.getBuffedEffect(SecondaryStat.DarkSight) != null) {
			...
			up = chra.getSkillLevel(skillid) > 0
					? SkillFactory.getSkill(skillid).getEffect(chra.getSkillLevel(skillid)).getY()
					: 0;
			...
			this.watk += chra.getBuffedEffect(SecondaryStat.UnkBuffStat707) != null ? effect.getWatk()
					: effect.getWatk() / 2;
		...
		block145: for (Pair<SecondaryStat, SecondaryStatValueHolder> buffs : chra.getEffects()) {
			...
			case "FlipTheCoin": {
				this.critical_rate = (short) (this.critical_rate
						+ chra.getBuffedEffect(SecondaryStat.FlipTheCoin).getX());
				break;
			}
			case "EnrageCr": {
			...
			case "BullsEye": {
				...
				this.critical_damage = (short) (this.critical_damage
						+ chra.getBuffedEffect(SecondaryStat.BullsEye).getY());
				break;
			}
			case "IndieAsrR":
			case "Asr": {
			...
			case "WeaponVariety": {
				int skillid2 = chra.getSkillLevel(64120006) > 0 ? 64120006
						: (chra.getSkillLevel(64110005) > 0 ? 64110005 : 64100004);
				this.InsertFinalDamage(SkillFactory.getSkill(skillid2).getEffect(chra.getSkillLevel(skillid2)).getX()
						* chra.weaponChanges1.size());
				break;
			}
			case "CrossOverChain":
			...
			case "Infinity": {
				this.InsertFinalDamage(chra.getBuffedEffect(buffs.getLeft()).getQ()
						+ (chra.getInfinity() - 1) * chra.getBuffedEffect(buffs.getLeft()).getDamage());
			...
			case "ReshuffleSwitch": {
				...
				// 获取基础buff效果（添加空指针检查）
				SecondaryStatEffect buffEffect = chra.getBuffedEffect(buffs.getLeft());
				if (buffEffect == null) {
					break;
				}

				// 初始化属性加成
				int watk = buffEffect.getAttackX();
				int cr = buffEffect.getCr();
				int bdr = buffEffect.getBdR();

				// 使用final直接定义技能数组（移除冗余变量）
				final int[] reshuffleSkills = { 61100008, 61110010, 61120013 };

				// 处理每个技能加成
				for (int skillId : reshuffleSkills) {
					if (chra.getSkillLevel(skillId) <= 0) {
						continue;
					}

					SecondaryStatEffect skillEffect = SkillFactory.getSkill(skillId)
							.getEffect(chra.getSkillLevel(skillId));

					// 基础加成
					watk += skillEffect.getAttackX();
					cr += skillEffect.getCr();
					bdr += skillEffect.getBdR();

					// 61120013技能的特殊加成条件
					if (skillId == 61120013 && (chra.getBuffedValue(61111008) || chra.getBuffedValue(61120008)
							|| chra.getBuffedValue(61121053))) {
						bdr += skillEffect.getX();
					}
				}
			...
			case "IgnisRore": {
				this.InsertFinalDamage((int) (chra.getSkillCustomValue0(23110005)
						* (long) chra.getBuffedEffect(buffs.getLeft()).getX()));
				break;
			}
			case "BlessOfDarkness": {
				int up3 = chra.getBlessofDarkness() == 1 ? chra.getBuffedEffect(buffs.getLeft()).getU()
						: (chra.getBlessofDarkness() == 2 ? chra.getBuffedEffect(buffs.getLeft()).getV()
								: (chra.getBlessofDarkness() == 3 ? chra.getBuffedEffect(buffs.getLeft()).getY() : 0));
				this.magic += up3;
				break;
			}
			case "JaguarSummoned": {
				this.critical_damage = (short) (this.critical_damage
						+ chra.getBuffedEffect(buffs.getLeft()).getCriticalDamage());
			...
			if (eqq != null && eqq.getDurability() == 0 && eqq.getPosition() < 0) {
				if (chr.getInventory(MapleInventoryType.EQUIP).isFull()) {
					...
					chr.getClient().getSession()
							.writeAndFlush((Object) CWvsContext.InventoryPacket.getShowInventoryFull());
					return false;
				}
		...
		if (atk == 0) {
			...
			if (weapon_item != null) {
				...
				if (player.getJob() == 110) {
					...
					this.sword = this.skill > 0
							? (double) ((this.skil.getEffect(player.getSkillLevel(this.skil)).getMastery() * 5 + 10)
									/ 100)
							: 0.1;
				} else {
				...
				this.skill = player.getSkillLevel(this.skil);
				this.axe = this.skill > 0
						? (double) ((this.skil.getEffect(player.getSkillLevel(this.skil)).getMastery() * 5 + 10) / 100)
						: 0.1;
				this.skil = SkillFactory.getSkill(1200001);
				this.skill = player.getSkillLevel(this.skil);
				this.blunt = this.skill > 0
						? (double) ((this.skil.getEffect(player.getSkillLevel(this.skil)).getMastery() * 5 + 10) / 100)
						: 0.1;
				this.skil = SkillFactory.getSkill(1300000);
				this.skill = player.getSkillLevel(this.skil);
				this.spear = this.skill > 0
						? (double) ((this.skil.getEffect(player.getSkillLevel(this.skil)).getMastery() * 5 + 10) / 100)
						: 0.1;
				this.skil = SkillFactory.getSkill(1300001);
				this.skill = player.getSkillLevel(this.skil);
				this.polearm = this.skill > 0
						? (double) ((this.skil.getEffect(player.getSkillLevel(this.skil)).getMastery() * 5 + 10) / 100)
						: 0.1;
				this.skil = SkillFactory.getSkill(3200000);
				this.skill = player.getSkillLevel(this.skil);
				this.CROSSBOW = this.skill > 0
						? (double) ((this.skil.getEffect(player.getSkillLevel(this.skil)).getMastery() * 5 + 10) / 100)
						: 0.1;
				this.skil = SkillFactory.getSkill(3100000);
				this.skill = player.getSkillLevel(this.skil);
				this.bow = this.skill > 0
						? (double) ((this.skil.getEffect(player.getSkillLevel(this.skil)).getMastery() * 5 + 10) / 100)
						: 0.1;
				if (weapon == MapleWeaponType.CROSSBOW) {
					minbasedamage = (int) ((double) this.localdex * 0.9 * 3.6 * this.CROSSBOW + (double) this.localstr)
							/ 100 * (atk + 15);
				}
				if (weapon == MapleWeaponType.BOW) {
					minbasedamage = (int) ((double) this.localdex * 0.9 * 3.4 * this.bow + (double) this.localstr) / 100
							* (atk + 15);
				}
				if (player.getJob() == 400 && weapon == MapleWeaponType.DAGGER) {
					minbasedamage = (int) ((double) this.localluk * 0.9 * 3.6 * this.dagger + (double) this.localstr
							+ (double) this.localdex) / 100 * atk;
				}
				if (player.getJob() != 400 && weapon == MapleWeaponType.DAGGER) {
					minbasedamage = (int) ((double) this.localstr * 0.9 * 4.0 * this.dagger + (double) this.localdex)
							/ 100 * atk;
				}
				if (player.getJob() == 400 && weapon == MapleWeaponType.CLAW) {
					minbasedamage = (int) ((double) this.localluk * 0.9 * 3.6 * this.claw + (double) this.localstr
							+ (double) this.localdex) / 100 * (atk + 15);
				}
				if (weapon == MapleWeaponType.SPEAR) {
					minbasedamage = (int) ((double) this.localstr * 0.9 * 3.0 * this.spear + (double) this.localdex)
							/ 100 * atk;
				}
				if (weapon == MapleWeaponType.POLE_ARM) {
					minbasedamage = (int) ((double) this.localstr * 0.9 * 3.0 * this.polearm + (double) this.localdex)
							/ 100 * atk;
				}
				if (weapon == MapleWeaponType.SWORD1H) {
					minbasedamage = (int) ((double) this.localstr * 0.9 * 4.0 * this.sword + (double) this.localdex)
							/ 100 * atk;
				}
				if (weapon == MapleWeaponType.SWORD2H) {
					minbasedamage = (int) ((double) this.localstr * 0.9 * 4.6 * this.sword + (double) this.localdex)
							/ 100 * atk;
				}
				if (weapon == MapleWeaponType.AXE1H) {
					minbasedamage = (int) ((double) this.localstr * 0.9 * 3.2 * this.axe + (double) this.localdex) / 100
							* atk;
				}
				if (weapon == MapleWeaponType.BLUNT1H) {
					minbasedamage = (int) ((double) this.localstr * 0.9 * 3.2 * this.blunt + (double) this.localdex)
							/ 100 * atk;
				}
				if (weapon == MapleWeaponType.AXE2H) {
					minbasedamage = (int) ((double) this.localstr * 0.9 * 3.4 * this.axe + (double) this.localdex) / 100
							* atk;
				}
				if (weapon == MapleWeaponType.BLUNT2H) {
					minbasedamage = (int) ((double) this.localstr * 0.9 * 3.4 * this.blunt + (double) this.localdex)
							/ 100 * atk;
				}
				if (weapon == MapleWeaponType.STAFF || weapon == MapleWeaponType.WAND) {
					minbasedamage = (int) ((double) this.localstr * 0.9 * 3.0 * this.staffwand + (double) this.localdex)
							/ 100 * atk;
				}
			}
		}
	...
	public final float calculateMaxBaseDamage(int watk) {
		...
		if (watk == 0) {
			...
			maxbasedamage = (weapon.getMaxDamageMultiplier() * (float) mainstat + (float) secondarystat) * (float) watk
					/ 100.0f;
		}
	...
	public final void relocHeal(MapleCharacter chra) {
		...
		this.shouldHealMP = GameConstants.isDemonSlayer(chra.getJob()) ? 0.0f
				: (float) (3 + this.mpRestore + this.recoverMP + this.localint_ / 10);
		...
		if (playerjob == 111 || playerjob == 112) {
			...
			if (lvl4 > 0) {
				...
				&& (lvl = chra.getSkillLevel(effect = SkillFactory.getSkill(31110009))) > 0) {
	...
	public Pair<Long, Long> handleEquipAdditions(final MapleItemInformationProvider ii, final MapleCharacter chra,
			final boolean first_login, final Map<Skill, SkillEntry> sData, final int itemId) {
		final List<Triple<String, String, String>> additions = ii.getEquipAdditions(itemId);
		// final Map<Integer, Map<String, Integer>> addz =
		// ii.getEquipIncrements(itemId);//（预留接口）
		int skillid = 0;
		int skilllevel = 0;
		...
		if (additions != null) {
			for (final Triple<String, String, String> add : additions) {
				...
				case "critical": {
					...
					// 职业要求检查（支持多职业逗号分隔）
					String jobReq = ii.getEquipAddReqs(itemId, "critical", "job");
					if (jobReq != null) {
						meetsRequirements = Arrays.stream(jobReq.split(",")).map(String::trim)
								.filter(segment -> !segment.isEmpty()).map(Integer::parseInt)
								.anyMatch(jobId -> jobId == chra.getJob());
					}

					// 等级要求检查
					String levelReq = ii.getEquipAddReqs(itemId, "critical", "level");
					if (meetsRequirements && levelReq != null) {
						meetsRequirements = chra.getLevel() >= Integer.parseInt(levelReq);
					}

					// 2. 应用加成
					if (meetsRequirements) {
						switch (add.getMid()) {
						case "prop" -> this.critical_rate = clampShortAdd(this.critical_rate, right);
						case "damage" -> this.critical_damage = clampShortAdd(this.critical_damage, right);
						}
					}
					continue;
				}
	...
	private short clampShortAdd(short base, int increment) {
		return (short) Math.min(Short.MAX_VALUE, Math.max(Short.MIN_VALUE, base + increment));
	}
	...
	public final boolean isRangedJob(int job) {
		return GameConstants.isArcher(job) || GameConstants.isCannon(job) || GameConstants.isNightLord(job)
				|| GameConstants.isCaptain(job) || GameConstants.isNightWalker(job) || GameConstants.isMechanic(job)
	...
	/*
	 * public List<Integer> getReduceCooltime() { return this.reduceCooltime; }
	 */
	...
	private double getJobMasteryBonus(short job) {
		double bonus = 0;
		if (GameConstants.isMagician(job)) {
			bonus = 25.0;
		} else if (isRangedJob(job)) { // 提取远程职业判断
			bonus = 15.0;
		} else {
			bonus = 20.0;
		}
	...
	private double applyKinesisCombo(double mastery, MapleCharacter chr) {
		if (!GameConstants.isKinesis(chr.getJob()))
			return mastery;

		int comboBonus = chr.getCombo() / 30;
		return Math.min(mastery + comboBonus, 99.0);
	}

	public int BeforeStatWatk(MapleCharacter chr) {
		// 参数校验
		if (chr == null || this.AfterStatWatk(chr) <= 0)
			return 0;

		// 基础计算
		double mastery = this.Mastery + getJobMasteryBonus(chr.getJob());

		// 特殊职业处理
		mastery = applyKinesisCombo(mastery, chr);

		// 最终计算
		return (int) Math.round(mastery / 100.0 * this.AfterStatWatk(chr));
	}

	public int AfterStatWatk(final MapleCharacter chr) {
		int statwatk = 0;
		int mainstat = GameConstants.isWarrior(chr.getJob()) ? this.getTotalStr()
				: (GameConstants.isMagician(chr.getJob()) ? this.getTotalInt()
						: (GameConstants.isArcher(chr.getJob()) ? this.getTotalDex()
								: (GameConstants.isThief(chr.getJob()) ? this.getTotalLuk() : 0)));
		int secondstat = GameConstants.isWarrior(chr.getJob()) ? this.getTotalDex()
				: (GameConstants.isMagician(chr.getJob()) ? this.getTotalLuk()
						: (GameConstants.isArcher(chr.getJob()) ? this.getTotalStr()
								: (GameConstants.isThief(chr.getJob()) ? this.getTotalDex() : 0)));
		...
		if (GameConstants.isXenon(chr.getJob())) {
			statwatk = (int) ((this.getTotalStr() + this.getTotalDex() + this.getTotalLuk()) * 4 * 0.01 * atk
					* GameConstants.getWeponInt(weapon.getItemId()) * GameConstants.getJobInt(chr.getJob())
					* ((100.0 + atkpercent) / 100.0) * ((100.0 + this.DamagePercent) / 100.0)
					* ((100.0 + this.getFinalDamage()) / 100.0));
		} else {
			if (GameConstants.isPirate(chr.getJob())) {
				if (GameConstants.isYeti(chr.getJob()) || GameConstants.isViper(chr.getJob())
						|| GameConstants.isCannon(chr.getJob()) || GameConstants.isStriker(chr.getJob())
						|| GameConstants.isEunWol(chr.getJob()) || GameConstants.isArc(chr.getJob())) {
			...
			statwatk = (int) Math
					.round((mainstat * 4 + secondstat) * 0.01 * atk * GameConstants.getWeponInt(weapon.getItemId())
							* GameConstants.getJobInt(chr.getJob()) * ((100.0 + atkpercent) / 100.0)
							* ((100.0 + this.DamagePercent) / 100.0) * ((100.0 + this.getFinalDamage()) / 100.0));
		}
		return statwatk;
	}
	...
	public Pair<Long, Long> handleInnerSkills(MapleCharacter chra) {
		...
		for (InnerSkillValueHolder ISVH : chra.getInnerSkills().get(chra.getInnerSkillsPreset())) {
			...
			case 70000014: {
				this.critical_rate = (short) (this.critical_rate
						+ SkillFactory.getSkill(ISVH.getSkillId()).getEffect(x).getCr());
				break;
			}
			case 70000015:
			case 70000044: {
	...
	public long test(final MapleCharacter chr, final MapleMonster mob, final int skillid) {
		...
		if (effect != null) {
			...
			if (GameConstants.isPirate(chr.getJob())) {
				if (GameConstants.isViper(chr.getJob()) || GameConstants.isCannon(chr.getJob())
						|| GameConstants.isStriker(chr.getJob()) || GameConstants.isEunWol(chr.getJob())
						|| GameConstants.isArc(chr.getJob())) {
					mainstat = this.getTotalStr();
					secondstat = this.getTotalDex();
				} else {
					mainstat = this.getTotalDex();
			...
			final double igint = 100.0
					- (mob.getStats().getPDRate() - mob.getStats().getPDRate() * (this.getIgnoreMobpdpR() / 100.0));
			final double own = (mainstat * 4 + secondstat) * atk * GameConstants.getWeponInt(weapon.getItemId())
					* GameConstants.getJobInt(weapon.getItemId()) / 100.0;
			final double two = effect.getDamage() / 100 * criint * ((100.0 + atkpercent) / 100.0)
					* ((100.0 + this.DamagePercent + this.BossDamage) / 100.0);
