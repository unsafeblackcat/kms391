...
function action(M, type, selection) {
        ...
        if (St == 0) {
            ...
            if (b == null || b < 0) {
                for (i = 0; i < 3; i++) {
                    ...
                    cm.getPlayer().addKV("야영지" + questlist[rd][0] + "_count", "0");
                    cm.getPlayer().addKV("야영지" + questlist[rd][0] + "_" + questlist[rd][2] + "q", questlist[rd][1]);
                    cm.getPlayer().addKV("야영지" + questlist[rd][0] + "_isclear", "0");
            ...
            cm.getPlayer().addKV("arcane_quest_" + qnum, 0);
                ...
                while (listed < 3) {
                    for (i = 0; i < questlist.length; i++) {
                        if (cm.getPlayer().getV("야영지" + qarr[i][0] + "_" + qarr[i][2]+"q") > 0) {
            ...
            for (i = 0; i < qarr.length; i++) {
                if (cm.getPlayer().getV("야영지" + qarr[i][0] + "_mobq") > 0) {
                    dialogue += "#b#e[일일 퀘스트] #o" + qarr[i][0] + "# " + qarr[i][1] + "마리 퇴치 \r\n"
                    questarray.push(qarr[i]);
                } else if (cm.getPlayer().getV("야영지" + qarr[i][0] + "_itemq") > 0) {
            ...
            for (i = 0; i < questarray.length; i++) {
                if (cm.getPlayer().getV("야영지" + questarray[i][0] + "_mobq") > 0) {
                    dialogue += "#L" + i + "##" + color[i] + "#e[일일 퀘스트] #o" + questarray[i][0] + "# " + questarray[i][1] + "마리 퇴치#k#n#l\r\n"
                } else if (cm.getPlayer().getV("야영지" + questarray[i][0] + "_itemq") > 0) {
	...
	talk+= "이번 주 자네에게 부탁할 일은 아래와 같다네.\r\n\r\n";
            for (i = 0; i < 3; i++) {
                ...
                cm.getPlayer().addKV("야영지" + questarray[i][0] + "_count", 0);
                cm.getPlayer().addKV("야영지" + questarray[i][0] + "_" + questarray[i][2] + "q", questarray[i][1]);
                cm.getPlayer().addKV("야영지" + questarray[i][0] + "_isclear", 0);
            }
            cm.sendNext(talk);
	    ...
	    if (a >= 6) {
            ...
            for (i = 0; i < questlist.length; i++) {
                if (cm.getPlayer().getV("야영지" + questlist[i][0] + "_mobq") > 0 && cm.getPlayer().getV("야영지" + questlist[i][0] + "_isclear") < 2) {
                    선택지 = "#L" + i + "##d[일일 퀘스트] #o" + questlist[i][0] + "# " + questlist[i][1] + "마리 퇴치#k"
                    if (cm.getPlayer().getV("야영지" + questlist[i][0] + "_count") >= cm.getPlayer().getV("야영지" + questlist[i][0] + "_mobq")) {
                        count1++;
                        dialogue2 += 선택지 + " (완료 가능)\r\n"
                    } else {
                        count2++;
                ...
                } else if (cm.getPlayer().getV("야영지" + questlist[i][0] + "_itemq") > 0 && cm.getPlayer().getV("야영지" + questlist[i][0] + "_isclear") < 2) {
                    선택지 = "#L" + i + "##d[일일 퀘스트] #z" + questlist[i][0] + "# " + questlist[i][1] + "개 수집#k"
                    if (cm.itemQuantity(questlist[i][0]) >= cm.getPlayer().getV("야영지" + questlist[i][0] + "_itemq")) {
                        count1++;
                        dialogue2 += 선택지 + " (완료 가능)\r\n"
                    } else {
                        count2++;
            ...
            if ((cm.getPlayer().getV("야영지" + questlist[selection][0] + "_mobq") > 0 && cm.getPlayer().getV("야영지" + questlist[selection][0] + "_count") >= cm.getPlayer().getV("야영지" + questlist[selection][0] + "_mobq")) ||
                (cm.getPlayer().getV("야영지" + questlist[selection][0] + "_itemq") > 0 && cm.itemQuantity(questlist[selection][0]) >= cm.getPlayer().getV("야영지" + questlist[selection][0] + "_itemq"))) {
	    ...
	    text2 += "#i4001868# #b#z4001868# #r#e3 개#k#n\r\n";
                cm.sendOk(text2);
                if (cm.getPlayer().getV("야영지" + questlist[selection][0] + "_itemq") > 0) {
                    cm.gainItem(questlist[selection][0], -cm.getPlayer().getV("야영지" + questlist[selection][0] + "_itemq"));
                }
                cm.getPlayer().addKV("야영지" + questlist[selection][0] + "_isclear", 2);
...
function clearquest(paramint) {
    cm.getPlayer().addKV("야영지" + paramint + "_count", -1);
    cm.getPlayer().addKV("야영지" + paramint + "_mobq", -1);
    cm.getPlayer().addKV("야영지" + paramint + "_itemq", -1);
    cm.getPlayer().addKV("야영지" + paramint + "_isclear", -1);
}
