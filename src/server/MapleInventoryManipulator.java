...
public class MapleInventoryManipulator {
	...
	public static void addRing(final MapleCharacter chr, final int itemId, final long ringId, final int sn,
		...
		chr.getClient().getSession().writeAndFlush((Object) CSPacket.sendBoughtRings(GameConstants.isCrushRing(itemId),
				ring, sn, chr.getClient(), partner));
	}
	...
	public static short addbyItem(final MapleClient c, final Item item, final boolean sort, final boolean fromcs) {
		...
		if (item.getQuantity() >= 300 && !sort) {
			final String data = c.getPlayer().getName() + " | " + item.getItemId() + " (x" + item.getQuantity()
					+ ")를 addbyItem을 통해 얻음.\r\n";
	...
	public static long getUniqueId(final int itemId, final MaplePet pet) {
		...
		} else if (GameConstants.getInventoryType(itemId) == MapleInventoryType.CODY
	...
	public static boolean addById(final MapleClient c, final int itemId, final short quantity, final String owner,
			final MaplePet pet, final String gmLog) {
		return addById(c, itemId, quantity, owner, pet, 0L, gmLog, false);
	}

	public static boolean addById(final MapleClient c, final int itemId, final short quantity, final String owner,
			final MaplePet pet, final long period, final String gmLog) {
		return addId(c, itemId, quantity, owner, pet, period, gmLog, false) >= 0;
	}

	public static boolean addById(final MapleClient c, final int itemId, final short quantity, final String owner,
			final MaplePet pet, final long period, final String gmLog, final boolean special) {
		return addId(c, itemId, quantity, owner, pet, period, gmLog, special) >= 0;
	}

	public static byte addId(final MapleClient c, final int itemId, short quantity, final String owner,
			final MaplePet pet, final long period, final String gmLog, final boolean special) {
		if (quantity >= 300) {
		    final String data = c.getPlayer().getName()  + " | " + itemId + " (x" + quantity + ") 通過 addbyId 獲得。\r\n";
		    NPCConversationManager.writeLog("Log/Item.log",  data, true);
		}
					...
					while (quantity > 0 && i.hasNext()) {
						...
						if (oldQ < slotMax && (eItem.getOwner().equals(owner) || owner == null)
							...
							c.getSession().writeAndFlush(
									(Object) CWvsContext.InventoryPacket.updateInventorySlot(type, eItem, false));
						}
					}
				}
				while (quantity > 0) {
	...
	public static Item addId_Item(final MapleClient c, final int itemId, short quantity, final String owner,
			final MaplePet pet, final long period, final String gmLog, final boolean special) {
		final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
		if ((ii.isPickupRestricted(itemId) && c.getPlayer().haveItem(itemId, 1, true, false))
				|| !ii.itemExists(itemId)) {
				...
				Label_0581: {
					if (quantity > 0) {
						...
						if (newQ2 != 0) {
							...
							c.getSession()
									.writeAndFlush((Object) CWvsContext.InventoryPacket.addInventorySlot(type, nItem));
							if (GameConstants.isRechargable(itemId) && quantity == 0) {
								break Label_0581;
							}
						} else {
				...
				if (existing.size() > 0) {
					...
					while (quantity > 0 && i.hasNext()) {
						...
						if (oldQ < slotMax) {
							...
							c.getSession().writeAndFlush(
									(Object) CWvsContext.InventoryPacket.updateInventorySlot(type, nItem, false));
						}
					}
				}
				while (quantity > 0) {
	...
	public static boolean addFromDrop(final MapleClient c, Item item, final boolean show, final boolean enhance,
			final boolean pet, final boolean sort) {
		final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
		if (c.getPlayer() == null
				|| (ii.isPickupRestricted(item.getItemId()) && !sort
				...
				if (existing.size() > 0) {
					...
					while (quantity > 0 && i.hasNext()) {
						...
						if (oldQ < slotMax && item.getOwner().equals(eItem.getOwner())
								&& item.getExpiration() == eItem.getExpiration() && item.getFlag() == eItem.getFlag()
								&& (!type.equals(MapleInventoryType.CASH) || (type.equals(MapleInventoryType.CASH)
										&& item.getUniqueId() == eItem.getUniqueId()))) {
							final short newQ = (short) Math.min(oldQ + quantity, slotMax);
							quantity -= (short) (newQ - oldQ);
							eItem.setQuantity(newQ);
							c.getSession().writeAndFlush(
									(Object) CWvsContext.InventoryPacket.updateInventorySlot(type, eItem, !pet));
						}
					}
				}
				while (quantity > 0) {
					...
					c.getSession()
							.writeAndFlush((Object) CWvsContext.InventoryPacket.addInventorySlot(type, nItem, !pet));
				}
			} else {
		...
		if (show) {
			c.getSession().writeAndFlush(
					(Object) CWvsContext.InfoPacket.getShowItemGain(item.getItemId(), item.getQuantity(), false));
		}
		if (item.getItemId() == 4001886) {
			c.getSession().writeAndFlush((Object) CWvsContext.setBossReward(c.getPlayer()));
		}
	...
	private static Item checkEnhanced(final Item before, final MapleCharacter chr) {
		if (before instanceof Equip) {
			...
			if (eq.getState() == 0 && (eq.getUpgradeSlots() >= 1 || eq.getLevel() >= 1)
					&& GameConstants.canScroll(eq.getItemId()) && Randomizer.nextInt(100) >= 80) {
				eq.resetPotential();
			}
		}
		return before;
	...
	public static boolean removeFromSlot(final MapleClient c, final MapleInventoryType type, final short slot,
		...
		if (item != null) {
			...
			if (item.getQuantity() == 0 && !allowZero) {
				c.getSession().writeAndFlush(
						(Object) CWvsContext.InventoryPacket.clearInventoryItem(type, item.getPosition(), fromDrop));
			} else {
				c.getSession()
						.writeAndFlush((Object) CWvsContext.InventoryPacket.updateInventorySlot(type, item, fromDrop));
			}
			if (item.getItemId() == 4001886) {
				c.getSession().writeAndFlush((Object) CWvsContext.setBossReward(c.getPlayer()));
			}
	...
	public static boolean removeFromSlot_Lock(final MapleClient c, final MapleInventoryType type, final short slot,
		...
		final Item item = c.getPlayer().getInventory(type).getItem(slot);
		return item != null && !ItemFlag.LOCK.check(item.getFlag()) && !ItemFlag.UNTRADEABLE.check(item.getFlag())
	...
	public static void move(final MapleClient c, final MapleInventoryType type, final short src, final short dst) {// 修改記錄？？调用
																													// move
																													// 方法时传入
																													// MapleClient
																													// 对象
		...
		c.getPlayer().getInventory(type).move(src, dst, slotMax, c);
		if (type != MapleInventoryType.EQUIP && type != MapleInventoryType.CASH && initialTarget != null
			...
			if (olddstQ + oldsrcQ > slotMax || source.getExpiration() > 0L || initialTarget.getExpiration() > 0L) {
				c.getSession().writeAndFlush(
						(Object) CWvsContext.InventoryPacket.moveAndCombineWithRestItem(source, initialTarget));
			} else {
				c.getSession()
						.writeAndFlush((Object) CWvsContext.InventoryPacket.moveAndCombineItem(source, initialTarget));
			}
		} else {
			c.getSession().writeAndFlush(
					(Object) CWvsContext.InventoryPacket.moveInventoryItem(type, src, dst, false, false));
		}
	}
	...
	public static void equip(final MapleClient c, final short src, short dst, final MapleInventoryType type) {
		...
		if (((dst <= -1200 && dst > -1300) || (dst >= -999 && dst < -99)) && !stats.containsKey("cash")
		...
		if (source.getItemId() != 1342069 && GameConstants.isWeapon(source.getItemId()) && !stats.containsKey("cash")
				&& dst != -10 && dst != -11) {
			c.getSession().writeAndFlush((Object) CWvsContext.enableActions(c.getPlayer()));
			return;
		}
		...
		if (GameConstants.isArcaneSymbol(source.getItemId())) {
			...
			for (short i = -1600; i >= -1605; --i) {
				if (chr.getInventory(MapleInventoryType.EQUIPPED).getItem(i) != null
						&& chr.getInventory(MapleInventoryType.EQUIPPED).getItem(i).getItemId() == source.getItemId()) {
					isequiped = true;
				}
			}
			if (isequiped) {
		...
		if (GameConstants.isAuthenticSymbol(source.getItemId())) {// 检查玩家是否已经装备了同类型的"原初徽章"
			boolean isequiped = false;
			for (short i = -1700; i >= -1705; --i) {// 原初徽章专用装备槽位范围为-1700到-1705
				if (chr.getInventory(MapleInventoryType.EQUIPPED).getItem(i) != null
						&& chr.getInventory(MapleInventoryType.EQUIPPED).getItem(i).getItemId() == source.getItemId()) {
					isequiped = true;
				}
					...
					for (final Integer j : s.id) {
						if (theList.contains(j)) {
							c.getPlayer().dropMessage(1,
									"無法裝備此物品，已經擁有 " + StringUtil.makeEnumHumanReadable(s.name()) + " equipped.");
							c.getSession().writeAndFlush((Object) CWvsContext.enableActions(c.getPlayer()));
							return;
						}
					}
		...
		for (int itemId : ContentsByItem.ExpBuffMedal.keySet()) {
			if (source.getItemId() == itemId) {
				...
				c.getPlayer().dropMessage(5, "[Fancy] 因佩戴勳章，經驗值獲取量增加 " + incPercent + "% .");
		...
		for (int itemId : ContentsByItem.ExpBuffPendent.keySet()) {
			if (source.getItemId() == itemId) {
				...
				c.getPlayer().dropMessage(5, "[Fancy] 因佩戴項鍊，經驗值獲取量增加 " + incPercent + "% .");
		...
		if (MapleItemInformationProvider.getInstance().getName(source.getItemId()).startsWith("\uc815\ub839\uc758")) {
			...
			final String nowtime = kc.getYeal() % 100 + kc.getMonths() + kc.getDays() + kc.getHours() + kc.getMins()
					+ kc.getMins();
		...
		for (final Integer potential : potentials) {
			...
			if (potential != 0 && ii.getPotentialInfo(potential) != null) {
				...
				c.getPlayer().changeSkillLevel(
						GameConstants.getOrdinaryJobNumber(c.getPlayer().getJob()) * 10000 + usefulSkill, (byte) 1,
						(byte) 1);
			}
		}
		int flag = source.getFlag();
		if (stats.get("equipTradeBlock") != null || source.getItemId() / 10000 == 167
				|| source.getItemId() / 10000 == 166) {
			if ((source.getItemId() / 10000 == 167 || source.getItemId() / 10000 == 166)
					&& ItemFlag.KARMA_EQUIP.check(flag)) {
				flag -= ItemFlag.KARMA_EQUIP.getValue();
				source.setFlag(flag);
			}
			...
			if (android != null) {
				...
				if (ItemFlag.ANDROID_ACTIVATED.check(androidflag)) {
					...
					c.getSession().writeAndFlush((Object) CWvsContext.InventoryPacket.updateInventoryItem(false,
							MapleInventoryType.EQUIP, android));
		...
		if (target != null) {
			if (MapleItemInformationProvider.getInstance().getName(target.getItemId())
					.startsWith("\uc815\ub839\uc758")) {
			...
			for (final Integer potential2 : potentials) {
				...
				if (potential2 != 0 && ii.getPotentialInfo(potential2) != null) {
					...
					c.getPlayer().changeSkillLevel(
							GameConstants.getOrdinaryJobNumber(c.getPlayer().getJob()) * 10000 + usefulSkill2,
							(byte) (-1), (byte) 0);
				}
			}
		...
		if (source.getItemId() >= 1113098 && source.getItemId() <= 1113128) {
			chr.changeSkillLevel(SkillFactory.getSkill(80001455 + (source.getItemId() - 1113098)),
					(byte) source.getBaseLevel(), (byte) 4);
		}
		if (target != null && target.getItemId() >= 1113098 && target.getItemId() <= 1113128) {
			chr.changeSkillLevel(SkillFactory.getSkill(80001455 + (target.getItemId() - 1113098)), (byte) (-1),
					(byte) 0);
		}
		...
		c.getSession()
				.writeAndFlush((Object) CWvsContext.InventoryPacket.moveInventoryItem(type, src, dst, false, false));
	...
	public static void unequip(final MapleClient c, short src, final short dst, final MapleInventoryType type) {
		...
		if (target != null) {
			if (src > -1200 && src < -999 && !GameConstants.isEvanDragonItem(target.getItemId())
					&& !GameConstants.isMechanicItem(target.getItemId())) {
				c.getSession().writeAndFlush((Object) CWvsContext.enableActions(c.getPlayer()));
				return;
			}
			...
			if (target.getItemId() != 1342069 && GameConstants.isWeapon(target.getItemId()) && src != -10
					&& src != -11) {
				c.getSession().writeAndFlush((Object) CWvsContext.enableActions(c.getPlayer()));
				return;
			}
			...
			if (GameConstants.isEvanDragonItem(target.getItemId())
					&& (c.getPlayer().getJob() < 2200 || c.getPlayer().getJob() > 2218)) {
				c.getSession().writeAndFlush((Object) CWvsContext.enableActions(c.getPlayer()));
				return;
			}
			if (GameConstants.isMechanicItem(target.getItemId())
					&& (c.getPlayer().getJob() < 3500 || c.getPlayer().getJob() > 3512)) {
				c.getSession().writeAndFlush((Object) CWvsContext.enableActions(c.getPlayer()));
				return;
			}
			if (ii.isOnlyEquip(target.getItemId()) && c.getPlayer().hasEquipped(target.getItemId())) {
				c.getPlayer().dropMessage(1,
						"\uace0\uc720\uc7a5\ucc29 \uc544\uc774\ud15c\uc740 1\uac1c\ub9cc \ucc29\uc6a9\ud560 \uc218 \uc788\uc2b5\ub2c8\ub2e4.");
				c.getSession().writeAndFlush((Object) CWvsContext.enableActions(c.getPlayer()));
				return;
			}
			...
			if ((stats.get("equipTradeBlock") != null || source.getItemId() / 10000 == 167
					|| source.getItemId() / 10000 == 166) && !ItemFlag.UNTRADEABLE.check(flag)) {
				flag += ItemFlag.UNTRADEABLE.getValue();
				target.setFlag(flag);
			}
		}
		...
		if (target != null && ItemFlag.KARMA_EQUIP.check(target.getFlag())
				&& !ItemFlag.UNTRADEABLE.check(target.getFlag())) {
			target.setFlag(target.getFlag() - ItemFlag.KARMA_EQUIP.getValue() + ItemFlag.UNTRADEABLE.getValue());
		}
		if (source.getItemId() >= 1113098 && source.getItemId() <= 1113128) {
			c.getPlayer().changeSkillLevel(SkillFactory.getSkill(80001455 + (source.getItemId() - 1113098)),
					(byte) (-1), (byte) 0);
		}
		c.getSession()
				.writeAndFlush((Object) CWvsContext.InventoryPacket.moveInventoryItem(type, src, dst, false, false));
	...
	public static boolean drop(final MapleClient c, MapleInventoryType type, final short src, final short quantity,
		...
		final Item source = c.getPlayer().getInventory(type).getItem(src);
		if (quantity < 0 || source == null || (!npcInduced && GameConstants.isPet(source.getItemId()))
		...
		if (type == MapleInventoryType.EQUIP) {
			...
			if ((equip.getEnchantBuff() & 0x88) != 0x0) {
				c.getSession().writeAndFlush((Object) CWvsContext.enableActions(c.getPlayer()));
				c.getPlayer().dropMessage(1, "無法丟棄裝備的痕迹.");
				return false;
			}
		}
		final Point dropPos = new Point(c.getPlayer().getPosition());
		FileoutputUtil.log(FileoutputUtil.物品掉落日誌,
				"[드롭] 계정번호 : " + c.getAccID() + " | " + c.getPlayer().getName() + "이 " + c.getPlayer().getMapId()
						+ " 에서 " + ii.getName(source.getItemId()) + "(" + source.getItemId() + ")를 "
						+ source.getQuantity() + "개를 드롭");
		if (quantity < source.getQuantity() && !GameConstants.isRechargable(source.getItemId())) {
