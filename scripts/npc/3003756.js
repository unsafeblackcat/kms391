...
var daily = [ // 첫번째
	{'mobid' : 8644500, 'qty' : 201, 't' : 1}, 
{'mobid' : 8644501, 'qty' : 202, 't' : 1},
{'mobid' : 8644502, 'qty' : 203, 't' : 1},
{'mobid' : 8644503, 'qty' : 204, 't' : 1},
{'mobid' : 8644504, 'qty' : 205, 't' : 1}, 
{'mobid' : 8644505, 'qty' : 206, 't' : 1}
]
	...
	if (status == 0) {
		...
		if (isclear == 5) {
			if ((isitem && (cm.haveItem(getk("mobid"), getk("mobq")))) || (!isitem && (getk("count") >= getk("mobq")))) {
				if (isitem) cm.gainItem(getk("mobid"), -getk("mobq"));
				setk("isclear", "6");
				var msg = "오늘의 명령을 완수하셨군요!."+enter;
				msg += "여기, 보상을 지급해드리겠습니다."+enter;
				msg += getReward()+enter;
				msg += "#k앞으로도 잘 부탁해."+enter;
				msg += "의뢰를 하나 더 완수하면 일일 보상을 지급해드리겠습니다.\r\n다시 한번 말을 걸어주십시오.\r\n보상을 받기전 장비창을 20칸 이상 비워주세요.";
				cm.sendOk(msg);
				cm.dispose();
			}
		}
		switch (isclear) {
			case 1:
				...
				msg += "#fUI/UIWindow2.img/UtilDlgEx/list1#\r\n#d#L1#(Lv."+제한레벨+") #b#e[일일 퀘스트]#n#k #d에스페라 연구 명령";

				cm.sendSimple(msg);
			break;
			case 2:
				var msg = "안녕하십니까! 오늘의 명령에는 이런 것들이 있습니다."+enter+enter;
				msg += "#fUI/UIWindow2.img/UtilDlgEx/list0#\r\n#r#e[일일 퀘스트] : 에스페라 연구 명령#n#k"+enter;
			...
			case 4:
				var msg = "시그너스 기사단 스트라이커 부대 소속 올리입니다!"+enter+enter;
				msg += "#fUI/UIWindow2.img/UtilDlgEx/list1#\r\n#d#L1#(Lv."+제한레벨+") #b#e[일일 퀘스트]#n#k #d에스페라 연구 명령 2";
		...
		if (getk("isclear") == 1 || getk("isclear") == 4) {
			var msg = "#k안녕 #b#h #!#k 안녕하십니까! 오늘의 명령에는 이런 것들이 있습니다."+enter+enter;
			msg += isitem ? "#r#e[일일 퀘스트] : 에스페라 연구 명령#n#k"+enter : "#fUI/UIWindow2.img/UtilDlgEx/list0#\r\n#r#e[일일 퀘스트] : 에스페라 연구 명령#n#k"+enter;
		...
		var msg = "안녕 #b#h #!#k 안녕하십니까! 오늘의 명령에는 이런 것들이 있습니다."+enter+enter;
		msg += isitem ? "#fUI/UIWindow2.img/UtilDlgEx/list0#\r\n#r#e[일일 퀘스트] : 에스페라 연구 명령#n#k"+enter : "#fUI/UIWindow2.img/UtilDlgEx/list0#\r\n#r#e[일일 퀘스트] : 에스페라 연구 명령#n#k"+enter;
		msg += isitem ? "\r\n목표 전리품 : #b#i"+getk("mobid")+"##z"+getk("mobid")+"# "+getk("mobq")+"개#k"+enter+enter : "처치 대상 : #b#o"+getk("mobid")+"# "+getk("mobq")+"마리#k"+enter+enter;
		msg += "그럼, 일이 끝난 후에 다시 와줘.\r\n보상을 받기전 장비창을 20칸 이상 비워주세요.";
		cm.sendOk(msg);
		cm.dispose();
