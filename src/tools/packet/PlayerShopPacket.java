...
public class PlayerShopPacket {
	...
	public static final byte[] getPlayerStore(MapleCharacter chr, boolean firstTime) {
		...
		AvatarLook.encodeAvatarLook(mplew, ((MaplePlayerShop) ips).getMCOwner(), false,
				(GameConstants.isZero(chr.getJob()) && chr.getGender() == 1), true);
		...
		for (Pair<Byte, MapleCharacter> storechr : ips.getVisitors()) {
			...
			AvatarLook.encodeAvatarLook(mplew, (MapleCharacter) storechr.right, false,
					(GameConstants.isZero(((MapleCharacter) storechr.right).getJob())
							&& ((MapleCharacter) storechr.right).getGender() == 1),
					true);
	...
	public static byte[] getMiniGame(MapleClient c, MapleMiniGame minigame) {
		...
		AvatarLook.encodeAvatarLook(mplew, minigame.getMCOwner(), false,
				(GameConstants.isZero(c.getPlayer().getJob()) && c.getPlayer().getGender() == 1), true);
		...
		for (Pair<Byte, MapleCharacter> visitorz : minigame.getVisitors()) {
			...
			AvatarLook.encodeAvatarLook(mplew, visitorz.getRight(), false,
					(GameConstants.isZero(((MapleCharacter) visitorz.right).getJob())
							&& ((MapleCharacter) visitorz.right).getGender() == 1),
					true);
	...
	public static byte[] getMiniGameReady(boolean ready) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.PLAYER_INTERACTION.getValue());
		mplew.write(ready ? PlayerInteractionHandler.Interaction.READY.action
