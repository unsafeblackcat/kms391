...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        talk = "#r#e#fs15#<魯塔比斯北花園入口>#k#n\r\n"
        talk += "這是通往北邊封印守護者 #r貝倫#k 守護的庭院的門.\r\n#r普通記錄將在正常情況下於當天午夜初始化\r\n混沌記錄於每週四午夜初始化。#k#n\r\n\r\n"
        talk += "#L0#使用#i4033611#進入#r普通模式#k.（100級以上）#l\r\n"
        cm.sendSimple(talk);
    } else if (status == 1) {
        st = selection;
        if (cm.getPlayer().getParty() == null) {
            cm.sendNextS("#fs15#好像只有舉辦隊伍才能入場。 找個隊伍吧.", 2)
            cm.dispose();
            return;
        } else if (!cm.isLeader()) {
            ...
            cm.sendNext("#fs15#已經有其他隊伍進入裡面，正在挑戰貝倫.");
            cm.dispose();
            return;
        ...
        if (!cm.isBossAvailable(setting[st][0], setting[st][1]) && st != 2) {
            ...
            talk += "#k#n您的等級不足.貝倫只能挑戰等級 " + setting[st][3] + "以上.";
        } else {
