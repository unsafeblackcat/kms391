...
public class MapleCharacterUtil {
	...
	private static final Pattern name2Pattern = Pattern.compile("^[가-힣a-zA-Z0-9_]{4,12}$");
	...
	public static final boolean isEligibleCharNameTwo(String name, boolean gm) {
		...
		// 使用 name2Pattern 進行驗證
		if (!name2Pattern.matcher(name).matches()) {
			return false;
		}
		for (String z : GameConstants.RESERVED) {
			if (name.indexOf(z) != -1) {
	...
	public static Triple<Integer, Integer, Integer> getInfoByName(String name, int world) {
		...
		try {
			...
			Triple<Integer, Integer, Integer> id = new Triple<>(Integer.valueOf(rs.getInt("id")),
					Integer.valueOf(rs.getInt("accountid")), Integer.valueOf(rs.getInt("gender")));
			rs.close();
			ps.close();
			con.close();
			return id;
	...
	public static void sendNote(String to, String name, String msg, int fame, int type, int senderid) {
		...
		try {
			...
			ps = con.prepareStatement(
					"INSERT INTO notes (`to`, `from`, `message`, `timestamp`, `gift`, `show`, `type`, `senderid`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
			ps.setString(1, to);
			ps.setString(2, name);
			ps.setString(3, msg);
		...
		if (World.Find.findChannel(to) >= 0) {
			MapleCharacter chr = ChannelServer.getInstance(World.Find.findChannel(to)).getPlayerStorage()
					.getCharacterByName(to);
			if (chr != null) {
	...
	public static Triple<Boolean, Integer, Integer> getNXCodeInfo(String code) throws SQLException {
		...
		try {
			...
			rs = ps.executeQuery();
			if (rs.next()) {
				ret = new Triple<>(Boolean.valueOf((rs.getInt("valid") > 0)), Integer.valueOf(rs.getInt("type")),
						Integer.valueOf(rs.getInt("item")));
			}
			rs.close();
			ps.close();
			con.close();
