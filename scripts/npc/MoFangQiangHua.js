...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        chat += "#fs18#旋轉魔方有以下規則！" + enter;

        chat += "1. 所有魔方只能使用等級限制在100以上的傳奇選項的物品。" + enter;
        chat += "2. 選擇<舊的屬性>選項可以在保留當前選項的情况下重新旋轉魔方。" + enter;
        chat += "選擇<新的屬性>選項後，物品的選項將更改爲選中的選項。";

        chat += enter;
        chat += "#L" + CubeStatus.BLACK_CUBE + "##i5062010# 黑魔方" ;
        chat += "#L" + CubeStatus.RED_CUBE + "##i5062009# 紅魔方" ;
        chat += "#L" + CubeStatus.MASTERCRAFT_CUBE + "##i2711004# 名匠魔方" ;
        chat += "#L99##fs15##i5062500#附加魔方";
        cm.sendSimpleS(chat, talkType);
    } else if (status == 1) {
            if (selection == 99) { // 跳转选项的判断 
        cm.dispose(); 
        cm.openNpc(9000347); 
        return;
    }
        ...
        if (!checkCost(cubeState)) {
            ...
            if (cubeItemList.length > 0) {
                chat += "#fs15#請選擇要轉魔方的物品！" + enter + enter;
                chat += getItemCatalog(cubeItemList);
                cm.sendSimpleS(chat, talkType);
            } else {
                chat += "#fs15#沒有可以旋轉魔方的物品";
                cm.sendSimpleS(chat, talkType);
                cm.dispose();
            }
        }
        ...
        if (selection == -1) {
            cm.sendSimple("#fs15#沒有符合條件的物品");
            cm.dispose();
        }

        if (!cm.getPlayer().getMeso() > 메소) {
            cm.sendOk("#fs15#金幣不足.");
            cm.dispose();
            return;
        }
        ...
        if (!checkCost(cubeState)) {
            ...
            chat += "#fs15##b#z" + newItem.getItemId() + "# #k";
            chat += "#fs15##r#L0#<舊的屬性>#k" + enter;
            chat += getItemPotential(
                originalPotentialData,
                cubeState == CubeStatus.ADDITIONAL_CUBE
            );
            chat += "#fs15##b#z" + newItem.getItemId() + "# #k";
            chat += "#fs15##r#L1#<新的屬性>#k" + enter;
            chat += getItemPotential(
                newPotentialsData,
                cubeState == CubeStatus.ADDITIONAL_CUBE
            );
