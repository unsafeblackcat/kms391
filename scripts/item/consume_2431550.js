...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			var ask = " #fs15#안녕! 나 맹맹이! 나 배달 잘해. 어느 타입의 아이템을 선물할래?\r\n";
			...
			if (selection >= 1 && selection <=5) {
				...
				cm.sendOk("#fs15# 선물시스템은 아이템의 종류나 옵션에 관계없이 누구에게나 선물할수 있는 시스템이야. 선물을 하기 위해선 같은 채널에 접속중이여야 해. 잘못된 사용으로 인한 문제는 운영진 측에서 책임지지 않아.");
				cm.dispose();
			}
		} else if (status == 2) {
			if (operation == 1) {
