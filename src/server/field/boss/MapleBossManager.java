...
public class MapleBossManager {
	...
	public static void changePhase(MapleMonster monster) {
		if (monster.getId() >= 8800102 && monster.getId() <= 8800110) {
			...
			// [othello] - 익스트림 보스
		} else if (monster.getId() == 8880012 || monster.getId() == 8880000 || monster.getId() == 8880002
				|| monster.getId() == 8880010) {
			byte phase;
			...
			if (monster.getPhase() != phase) {
				monster.getMap().broadcastMessage(
						MobPacket.getSmartNotice(monster.getId(), 0, 5, 1, "瑪格納斯控制古瓦的力量削弱了.古瓦爾的氣息變得更强."));
			...
			if (monster.getPhase() != phase) {
				monster.getMap().broadcastMessage(
						MobPacket.getSmartNotice(monster.getId(), 0, 5, 1, "被選中的賽蓮（極境）對麥格納斯的古瓦爾控制力量減弱了。古瓦爾的氣勢變得更為強大。"));
				monster.setPhase(phase);
				monster.getMap().broadcastMessage(MobPacket.changePhase(monster));
			}
		}
		...
		for (MobSkill msi : monster.getSkills()) {
			if (time - monster.getLastSkillUsed(msi.getSkillId(), msi.getSkillLevel()) >= 0L
				...
				if (msi.isOnlyFsm()) {
					...
					if (monster.getPhase() == 3 && msi.getSkillId() == 176 && msi.getSkillLevel() == 27
	...
	public static void ZakumArmHandler(MapleMonster monster, MapleCharacter chr, int actionAndDir) {
		if ((monster.getId() == 8800002 || (monster.getId() == 8800102 && monster.getPhase() <= 2))
			...
			if (monster.getPhase() == 1) {
				...
				if (monster.getCustomValue0(8800003) == 2L || monster.getCustomValue0(8800003) == 3L
						|| monster.getCustomValue0(8800003) == 4L) {
					...
					if (monster.getCustomValue0(8800004) == 2L) {
						...
						if (monster.getCustomValue0(8800005) == 2L) {
							...
							for (MapleMonster mob : monster.getMap().getAllMonster()) {
								if ((mob.getId() >= 8800002 && mob.getId() <= 8800010)
										|| (mob.getId() >= 8800102 && mob.getId() <= 8800110)) {
									mob.setPhase((byte) 2);
				...
				if (!changephase) {
					...
					for (MapleMonster mob : monster.getMap().getAllMonster()) {
						for (Map.Entry<Integer, Integer> mobs : list.entrySet()) {
							if (((Integer) mobs.getKey()).intValue() == mob.getId()) {
								if (monster.getId() == 8800102) {
									MobSkill msi = mob.getStats().getSkill(176,
											(monster.getCustomValue0(8800003) == 6L) ? 26 : 25);
									msi.setMobSkillDelay(chr, mob,
											(monster.getCustomValue0(8800003) == 6L) ? 2430 : 1420, (short) 0,
											((actionAndDir & 0x1) != 0));
								} else {
									MobSkill msi = mob.getStats().getSkill(176,
											(monster.getCustomValue0(8800003) == 6L) ? 34 : 33);
									msi.setMobSkillDelay(chr, mob,
											(monster.getCustomValue0(8800003) == 6L) ? 2430 : 1420, (short) 0,
											((actionAndDir & 0x1) != 0));
								}
								mob.getMap().broadcastMessage(MobPacket.setAttackZakumArm(mob.getObjectId(),
										(monster.getCustomValue0(8800003) == 6L) ? 1 : 0));
							}
						}
					}
				} else if (changephase) {
					...
					Timer.MapTimer.getInstance().schedule(() -> {
						...
						monster.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(monsterid),
								new Point(-60, 86));
					}, 1500L);
	...
	public static void magnusHandler(MapleMonster monster, int type, int actionAndDir) {
		if (monster != null) {
			...
			if (type == 0) {
				...
				int size = Randomizer.rand(monster.getPhase() == 1 ? 5 : 3 * monster.getPhase(),
						8 + monster.getPhase());
				...
				for (count = 0; count < size; ++count) {
					...
					Obstacle ob;
					if (type <= 5) {
						ob = new Obstacle(obtype, new Point(x, -2000), new Point(x, -1347), 25,
								monster.getId() == 8880010 ? 10 : 50, 1459, Randomizer.rand(80, 230), 1, 653, 0);
					} else if (type <= 7) {
						ob = new Obstacle(obtype, new Point(x, -2000), new Point(x, -1347), 45,
								monster.getId() == 8880010 ? 30 : 100, 1481, Randomizer.rand(80, 230), 1, 653, 0);
					} else {
						ob = new Obstacle(obtype, new Point(x, -2000), new Point(x, -1347), 65,
								monster.getId() == 8880010 ? 50 : 100, 542, Randomizer.rand(50, 270), 2, 653, 0);
					}
				...
				if (monster.getCustomValue(monster.getId()) == null) {
					...
					if (objtype == 1) {
						...
						Iterator<Integer> var18 = listed.iterator();
						...
						while (var18.hasNext()) {
							...
							if (delay > 0) {
								Timer.MapTimer.getInstance().schedule(() -> {
									monster.getMap().broadcastMessage(
											CField.DebuffObjON(randlist2, monster.getId() == 8880000));
								}, (long) delay);
							} else {
								monster.getMap()
										.broadcastMessage(CField.DebuffObjON(randlist2, monster.getId() == 8880000));
							}
					...
					monster.setCustomInfo(monster.getId(), 0,
							monster.getPhase() > 0 ? time - monster.getPhase() * 1000 : 30000);
				}
			} else if (type == 2) {
				if (actionAndDir == 27 || actionAndDir == 26) {
					monster.getMap()
							.broadcastMessage(MobPacket.getSmartNotice(monster.getId(), 0, 5, 1, "梅格努斯試圖震退周圍的敵人."));
				}

				if (actionAndDir == 34 || actionAndDir == 35) {
					monster.getMap().broadcastMessage(
							MobPacket.getSmartNotice(monster.getId(), 0, 5, 1, "梅格努斯發動連續攻擊,意圖消滅剩餘敵人."));
				}

				if (actionAndDir == 60) {
					monster.getMap().broadcastMessage(
							MobPacket.getSmartNotice(monster.getId(), 0, 5, 1, "梅格努斯正對減速的敵人蓄力,準備發動強力一擊."));
				}

				if (actionAndDir == 62) {
					monster.getMap().broadcastMessage(
							MobPacket.getSmartNotice(monster.getId(), 0, 5, 1, "梅格努斯察覺敵人正在恢復/強化,準備應對措施."));
				}
	...
	public static void lotusHandler(MapleMonster monster) {
		if (monster != null) {
			...
			for (int i = 0; i < size; i++) {
				...
				if (type == 48) {
					ob = new Obstacle(48, new Point(x, -520), new Point(x, -16), 36, 10, Randomizer.rand(0, 1000),
							Randomizer.rand(250, 450), 1, 504, 0);
					if (!hard) {
						ob.setEffect(false);
					}
				} else if (type == 49) {
					ob = new Obstacle(49, new Point(x, -520), new Point(x, -16), 51, hard ? 20 : 10,
							Randomizer.rand(0, 1000), Randomizer.rand(200, 350), 1, 504, 0);
				} else if (type == 50) {
					ob = new Obstacle(50, new Point(x, -520), new Point(x, -16), 51, hard ? 30 : 20,
							Randomizer.rand(0, 1000), Randomizer.rand(100, 200), 2, 504, 0);
				} else if (type == 51) {
					ob = new Obstacle(51, new Point(x, -520), new Point(x, -16), 65, hard ? 50 : 40,
							Randomizer.rand(0, 1000), Randomizer.rand(100, 200), 2, 504, 0);
				} else {
					ob = new Obstacle(52, new Point(x, -520), new Point(x, -16), 190, 100, 190,
							Randomizer.rand(150, 250), 1, 504, 0);
				}
				obs.add(ob);
			}
	...
	public static void duskHandler(MapleMonster dusk, MapleMap map) {
		map.broadcastMessage(SLFCGPacket.OnYellowDlg(0, 3000, "觸手在保護眼睛，輸出降低%80.", ""));
		map.broadcastMessage(CField.enforceMSG("看到恐懼是無法被塡滿的！ 如果受不了的話，恐懼就會轉移，請注意!", 250, 3000));
			...
			if (map.getId() == 450009400 || map.getId() == 450009450) {
				...
				if (dusk != null) {
					...
					for (MapleCharacter cchr : map.getCharacters()) {
						if (cchr.isDuskBlind()) {
							if (time - cchr.getLastSpawnBlindMobTime() >= 3000) {
								for (int i = 0; i < 3; i++) {
									...
									map.spawnMonsterOnGroundBelow(mob,
											new Point(Randomizer.rand(-650, 650), Randomizer.rand(-500, -200)));
						...
						cchr.getClient().getSession().writeAndFlush(
								MobPacket.handleDuskGauge(cchr.isDuskBlind(), cchr.getDuskGauge(), 1000));
					}
					...
					if (dusk.getPhase() == 1) {
						if (time - dusk.getLastSpecialAttackTime() >= 60000) {
							...
							map.broadcastMessage(
									MobPacket.BossDusk.spawnTempFoothold(map.getId() == 450009450 ? 3 : 1));
							map.broadcastMessage(CField.enforceMSG("用防禦的觸手進行强力攻擊！ 如果堅持下去的話，可以攻擊暴露出來的空的眼睛!", 250, 3000));
						}
					}
					if (dusk.getPhase() == 0) {
						if (time - dusk.getLastSpecialAttackTime() >= 25000) {
							if (dusk.isUseSpecialSkill()) {
								...
								map.broadcastMessage(MobPacket.moveMonsterResponse(dusk.getObjectId(), (short) 0,
										dusk.getMp(), true, dusk.getNextSkill(), dusk.getNextSkillLvl(), 0));
					...
					if (att != null) {
						if (dusk.getPhase() == 1 && !dusk.isUseSpecialSkill()) {
							if (time - att.getLastSeedCountedTime() >= 10000) {
								map.broadcastMessage(MobPacket.BossDusk.spawnDrillAttack(Randomizer.rand(-650, 650),
										Randomizer.nextInt(100) < 50, map.getId() == 450009450 ? 4 : 2));
								att.setLastSeedCountedTime(time);
							}
					...
					if (time - dusk.getLastStoneTime() >= 3000) {
						...
						for (int i = 0; i < size; ++i) {
							...
							if (type == 65) {
								ob = new Obstacle(type, new Point(x, -1055), new Point(x, -157), 24,
										map.getId() == 450009450 ? 30 : 15, 157, Randomizer.rand(300, 850), 1, 898, 0);
							} else if (type == 66) {
								ob = new Obstacle(type, new Point(x, -1055), new Point(x, -157), 24,
										map.getId() == 450009450 ? 30 : 15, 185, Randomizer.rand(300, 850), 1, 898, 0);
							} else if (type == 67) {
								ob = new Obstacle(type, new Point(x, -1055), new Point(x, -157), 24,
										map.getId() == 450009450 ? 30 : 15, 718, Randomizer.rand(300, 850), 1, 898, 0);
							} else { // 67 나올리는없을듯.
								ob = new Obstacle(type, new Point(x, -1055), new Point(x, -157), 24,
										map.getId() == 450009450 ? 30 : 15, 718, Randomizer.rand(300, 850), 1, 898, 0);
							}
	...
	public static void DemianStigmaGive(MapleMonster monster) {
		...
		String[] EffectMsgs = { "戴米安給最有威脅的敵人打上了烙印.", "戴米安給烙印最多的敵人烙印.", "戴米安給烙印最少的敵人烙印.", "不知道戴米安會給誰打上了烙印." };
	...
	public static MapleCharacter RandCharacter(MapleMonster monster, int type, boolean first) {
		...
		for (int i = 0; i < a2.size() - 1; i++) {
			for (int j = 0; j < a2.size() - i - 1; j++) {
				if (((Long) ((Pair) a2.get(j)).getRight()).longValue() < ((Long) ((Pair) a2.get(j + 1)).getRight())
						.longValue()) {
	...
	public static void pierreHandler(MapleMonster monster) {
		monster.setSchedule(Timer.MobTimer.getInstance().register(() -> {
			if ((monster.getEventInstance() == null || !monster.isAlive())
					&& monster.getMap().getCharacters().size() <= 0) {
			...
			String monstercolor = (monster.getId() % 10 == 0) ? "Purple"
					: ((monster.getId() % 10 == 1) ? "Red" : "Blue");
			int time = Chaos ? trans.getDuration() : (monstercolor.equals("Purple") ? 10 : 20);
			if ((System.currentTimeMillis() - monster.lastCapTime >= (time * 1000) && monster.lastCapTime != 0L)
				...
				if (Trans && !Devide) {
					if ((monstercolor.equals("Purple") || monstercolor.equals("Red") || monstercolor.equals("Blue"))
							&& TransFormHpPercent >= monster.getHPPercent()) {
						MapleMonster copy = MapleLifeFactory
								.getMonster(Randomizer.nextBoolean() ? (monster.getId() + 1) : (monster.getId() + 2));
						...
						for (MapleCharacter chr : copy.getMap().getAllCharactersThreadsafe()) {
							Pair<Integer, Integer> skill = new Pair<>(Integer.valueOf(Randomizer.rand(189, 190)),
									Integer.valueOf(1));
							if (chr.isAlive()) {
								...
								if (chr.getBuffedValue(SecondaryStat.NotDamaged) == null
									...
									MobSkillFactory
											.getMobSkill(((Integer) skill.left).intValue(),
													((Integer) skill.right).intValue())
											.applyEffect(chr, copy, true, copy.isFacingLeft());
								}
							}
						}
					}
					...
					if (monster.getCustomValue(8900000) == null || monster.getCustomValue0(8900000) == 0L) {
						...
						monster.getMap().broadcastMessage(MobPacket.showBossHP(monster.getId(), -1L, 0L));
						for (MapleCharacter chr : copy.getMap().getAllCharactersThreadsafe()) {
							Pair<Integer, Integer> skill = trans.getSkills()
									.get(Randomizer.nextInt(trans.getSkills().size()));
							if (chr.isAlive()) {
								chr.cancelDisease(SecondaryStat.CapDebuff);
								if (chr.getBuffedValue(SecondaryStat.NotDamaged) == null
										&& chr.getBuffedValue(SecondaryStat.IndieNotDamaged) == null) {
						...
						MapleMonster copy = MapleLifeFactory
								.getMonster(monster.getId() + (monstercolor.equals("Red") ? 1 : -1));
						List<Pair<MonsterStatus, MonsterStatusEffect>> stats = new ArrayList<>();
						stats.add(new Pair<>(MonsterStatus.MS_IndiePdr, new MonsterStatusEffect(187, 210000000, 70L)));
						stats.add(new Pair<>(MonsterStatus.MS_IndieMdr, new MonsterStatusEffect(187, 210000000, 70L)));
						stats.add(new Pair<>(MonsterStatus.MS_Pad, new MonsterStatusEffect(187, 210000000, 17000L)));
						...
						if (copy != null) {
							...
							if (size <= 1) {
								MapleMonster other = MapleLifeFactory
										.getMonster((mobid == 8900001) ? 8900001 : 8900002);
								...
								Point mobpos2 = new Point((monster.getTruePosition()).x - 150,
										(monster.getTruePosition()).y);
								if (monster.getMap().getFootholds().findBelow(mobpos2) == null) {
									mobpos2 = monster.getPosition();
								}
	...
	public static void blackMageHandler(MapleMonster monster) {
		monster.setSchedule(Timer.MobTimer.getInstance().register(() -> {
			...
			for (MapleCharacter chr : map.getAllChracater()) {
				...
				if ((chr.getMapId() == 450013100 && chr.getMap().getMobsSize(8880500) > 0
						&& chr.getMap().getMobsSize(8880501) > 0)
						|| (chr.getMapId() == 450013300 && chr.getMap().getMobsSize(8880502) > 0)
						|| (chr.getMapId() == 450013500 && chr.getMap().getMobsSize(8880503) > 0)
						|| (chr.getMapId() == 450013700 && chr.getMap().getMobsSize(8880504) > 0)) {
					if (chr.getMapId() != 450013700 && chr.getMapId() != 450013500
							&& chr.getSkillCustomValue(45001310) == null && map.getCustomValue0(45001316) == 0) {
					...
					if (chr.getSkillCustomValue(45001311) == null && chr.hasDisease(SecondaryStat.CurseOfCreation)) {
						...
						if (chr.isAlive()) {
							chr.getClient().send(
									CField.getFieldSkillAdd(100015, (chr.getMapId() == 450013700) ? 2 : 1, false));
						}
					}
				}
			}
			if (map.getCustomValue0(45001317) > 0) {
				...
				for (MapleCharacter chr2 : map.getAllCharactersThreadsafe()) {
					if (((chr2.getTruePosition()).x <= bx || afterx <= (chr2.getTruePosition()).x
							|| (chr2.getPosition()).y <= -168) && chr2.getBuffedEffect(SecondaryStat.NotDamaged) == null
							&& chr2.getBuffedEffect(SecondaryStat.IndieNotDamaged) == null) {
						chr2.getPercentDamage(monster, 999, 999, 12500, true);
					}
				}
			} else if (map.getCustomValue0(45001360) == 1) {
				for (MapleCharacter chr2 : map.getAllCharactersThreadsafe()) {
					if ((chr2.getPosition()).y == 85 && chr2.getMapId() == 450013500 && chr2.isAlive()
							&& chr2.getBuffedEffect(SecondaryStat.NotDamaged) == null
							&& chr2.getBuffedEffect(SecondaryStat.IndieNotDamaged) == null) {
			...
			if ((map.getId() == 450013100 && map.getMobsSize(8880500) > 0 && map.getMobsSize(8880501) > 0)
					|| (map.getId() == 450013300 && map.getMobsSize(8880502) > 0)
					|| (map.getId() == 450013500 && map.getMobsSize(8880503) > 0 && map.getCharactersSize() > 0)) {
				if (map.getCustomValue0(45001316) == 0 && map.getCustomValue(45001310) == null) {
					...
					for (int i = 0; i < (map.getRight() - map.getLeft()) / 150; i++) {
						if (type == 1) {
							Obstacle ob = new Obstacle(75, new Point(map.getRight() - i * 150, -540),
									new Point(map.getRight() - 300, 16), 50, 50, 0, i * 530, 82, 4, 573, 0);
							obs.add(ob);
						} else {
							Obstacle ob = new Obstacle(75, new Point(map.getLeft() + i * 150, -540),
									new Point(map.getLeft() - 300, 16), 50, 50, 0, i * 530, 82, 4, 573, 0);
							obs.add(ob);
						}
					}
					map.CreateObstacle(monster, obs);
				} else if (map.getCustomValue(45001311) == null && map.getId() == 450013100
						&& map.getMobsSize(8880500) > 0 && map.getMobsSize(8880501) > 0) {
					map.setCustomInfo(45001311, 0, 63000);
					map.broadcastMessage(CField.enforceMSG("不祥的紅色閃電襲擊落下，扼制移動.", 265, 3000));
				...
				} else if (map.getCustomValue(45001315) == null && map.getId() == 450013300
						&& map.getMobsSize(8880502) > 0) {
					boolean use = true;
					int randt = 2;
					for (MapleMonster mob : map.getAllMonster()) {
						if (mob.getId() == 8880502) {
							...
							map.broadcastMessage(MobPacket.UseSkill(mob.getObjectId(),
									(randt == 1) ? 11 : ((randt == 2) ? 10 : 12)));
							break;
						}
					}
					if (use) {
						...
						Timer.MapTimer.getInstance().schedule(() -> {
							...
							// "Sound/Etc.img/BlackMageShield","UI/UIWindow8.img/BlackMageShield/mobEffect0"));//38
							map.getMonsterById(8880502).gainShield(3000000000000L, true, 30);
						}, 16000L);
					}
					// mush
				} else if (map.getCustomValue(45001312) == null && map.getId() == 450013100
						&& map.getMobsSize(8880500) > 0 && map.getMobsSize(8880501) > 0) {
					List<Point> pt = new ArrayList<>();
					int mobsize = map.getMobsSize(8880507) + map.getMobsSize(8880508);
					int basex = 2172;
					map.setCustomInfo(45001312, 0, 50000);
					map.broadcastMessage(CField.enforceMSG("痛哭的輧障升起，蠶食空間.", 265, 3000));
					if (mobsize < 16) {
					...
					for (MapleMonster mob : map.getAllMonster()) {
						if (mob.getId() == 8880501 || mob.getId() == 8880500) {
							...
							mob.getMap()
									.broadcastMessage(MobPacket.moveMonsterResponse(mob.getObjectId(),
											(short) (int) mob.getCustomValue0(99991), mob.getMp(), true, 170,
											(mob.getId() == 8880500) ? 62 : 64, 0));
					...
					Timer.MapTimer.getInstance().schedule(() -> {
						if (mobsize < 16) {
							...
							map.spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8880507),
									new Point(-basex + map.getCustomValue0(45001313) * 174, 85));
							map.spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8880508),
									new Point(basex - map.getCustomValue0(45001313) * 174, 85));
						}
						...
						for (MapleMonster mob : map.getAllMonster()) {
							...
							if (mob.getId() == 8880500 || mob.getId() == 8880501) {
								// map.broadcastMessage(MobPacket.mobBarrierEffect(mob.getObjectId(),
								// "UI/UIWindow8.img/BlackMageShield/mobEffect",
				...
				} else if (map.getCustomValue0(45001316) == 0 && map.getCustomValue(45001313) == null
						&& map.getId() == 450013300 && map.getMobsSize(8880502) > 0) {
				...
				} else if (map.getCustomValue0(45001316) == 0 && map.getCustomValue(45101313) == null
						&& map.getId() == 450013300 && map.getMobsSize(8880502) > 0) {
					for (MapleMonster mob : map.getAllMonster()) {
				...
				} else if (map.getCustomValue0(45001316) == 0 && map.getCustomValue(45001314) == null
						&& ((map.getId() == 450013300 && map.getMobsSize(8880502) > 0)
								|| (map.getId() == 450013500 && map.getMobsSize(8880503) > 0))) {
					map.setCustomInfo(45001314, 0, Randomizer.rand(45000, 51000));
					map.getObtacles(Randomizer.rand(1, 2));
				} else if (map.getCustomValue0(45001316) == 0 && map.getCustomValue(45001320) == null
						&& map.getId() == 450013500 && map.getMobsSize(8880503) > 0) {
					List<Triple<Point, Integer, Integer>> skillinfo = new ArrayList<>();
					for (int i = 0; i < Randomizer.rand(3, 5); i++) {
						skillinfo.add(new Triple<>(new Point(Randomizer.rand(-970, 970), Randomizer.rand(-400, 60)),
								Integer.valueOf(Randomizer.rand(0, 180)),
								Integer.valueOf(100 + Randomizer.rand(50, 100) * i)));
					}
				...
				} else if (map.getCustomValue0(45001316) == 0 && map.getCustomValue(45001321) == null
						&& map.getId() == 450013500 && map.getMobsSize(8880503) > 0) {
					Point pos = null;
					int type;
					for (type = Randomizer.rand(1, 6); map.getCustomValue0(45001350 + type) == 1; type = Randomizer
							.rand(1, 6))
						;
					switch (type) {
					case 1:
				...
				} else if (map.getCustomValue0(45001316) == 0
						&& (map.getCustomValue(45001322) == null || map.getCustomValue(45001323) == null)
						&& map.getId() == 450013500 && map.getMobsSize(8880503) > 0) {
					List<Obstacle> obs = new ArrayList<>();
				...
				} else if (map.getCustomValue(45001324) == null && map.getId() == 450013500
						&& map.getMobsSize(8880503) > 0) {
					List<Triple<Point, String, Integer>> skillinfo = new ArrayList<>();
					int type3 = 1;
					map.broadcastMessage(CField.enforceMSG("黑魔法師使用創造和破壞的權能。 要選擇上和下，藏到那一邊.", 265, 4000));
					...
					Timer.MapTimer.getInstance().schedule(() -> {
						...
						for (int i = 0; i < 45; ++i) {
							...
							Obstacle ob = new Obstacle(78, new Point(x, -600), new Point(x, -85), 100, 999, 0, 450, 0,
									Randomizer.rand(400, 600), Randomizer.rand(1, 3), cya);
							obs.add(ob);
						}
					...
					for (int type = 1; type <= 6; type++) {
						if (map.getCustomValue0(45001350 + type) == 1) {
							...
							skillinfo
									.add(new Triple<>(pos, (type == 1) ? "foot1" : ("foo" + type), Integer.valueOf(0)));
						}
					}
					Timer.MapTimer.getInstance().schedule(() -> {
						...
						map.broadcastMessage(CField.getFieldFootHoldAdd(100013, 1, skillinfo, false));
						// map.broadcastMessage(MobPacket.mobBarrierEffect(map.getMonsterById(0x878177).getObjectId(),
						// "UI/UIWindow8.img/BlackMageShield/mobEffect_210",
						// "Sound/Etc.img/BlackMageShield",
						// "UI/UIWindow8.img/BlackMageShield/mobEffect0_210"));//38
						...
						Timer.MapTimer.getInstance().schedule(() -> {
							map.broadcastMessage(MobPacket.FieldSummonAttack(3, true, pos,
									map.getMonsterById(0x878177).getObjectId(), damage));
							map.setCustomInfo(45001327,
									map.getMonsterById((int) 0x878177).getPosition().x + Randomizer.rand(200, 500), 0);
							map.setCustomInfo(45001328, -304, 0);
							map.broadcastMessage(MobPacket
									.FieldSummonTeleport(new Point(map.getCustomValue0(45001327), -304), true));
						}, 2000L);
					}, 5000L);
				} // mush
				if (map.getCustomValue(45021328) == null && map.getId() == 450013500 && map.getMobsSize(8880503) > 0
						&& map.getCustomValue0(45011328) < 3) {
					int mobid = 0;
					map.broadcastMessage(CField.enforceMSG("破壞天使從無到有.", 265, 4000));
				...
				if (map.getCustomValue(45001326) == null && map.getId() == 450013500 && map.getMobsSize(8880503) > 0) {
					...
					map.broadcastMessage(MobPacket.FieldSummonAttack(3, false, new Point(0, 0),
							map.getMonsterById(8880503).getObjectId(), damage));
				}
			}
			if (map.getId() == 450013700 && map.getMobsSize(8880504) > 0) {
				if (map.getCustomValue(45004445) == null) {
					...
					map.broadcastMessage(CField.enforceMSG("表現出接近神的權能。創造和破壞，選擇一種想要的力量.", 265, 5000));
					map.setCustomInfo(45004445, 0, 30000);
					map.setCustomInfo(45004446, 1, 0);
					skillinfo.add(new Triple<>(new Point(-801, -404), new Point(-681, 238),
							Integer.valueOf(Randomizer.rand(0, 3))));
					skillinfo.add(new Triple<>(new Point(-681, -404), new Point(-561, 238),
							Integer.valueOf(Randomizer.rand(0, 3))));
					skillinfo.add(new Triple<>(new Point(-561, -404), new Point(-441, 238),
							Integer.valueOf(Randomizer.rand(0, 3))));
					skillinfo.add(new Triple<>(new Point(-441, -404), new Point(-321, 238),
							Integer.valueOf(Randomizer.rand(0, 3))));
					skillinfo.add(new Triple<>(new Point(-321, -404), new Point(-201, 238),
							Integer.valueOf(Randomizer.rand(0, 3))));
					skillinfo.add(new Triple<>(new Point(-201, -404), new Point(-81, 238),
							Integer.valueOf(Randomizer.rand(0, 3))));
					skillinfo.add(new Triple<>(new Point(-81, -404), new Point(39, 238),
							Integer.valueOf(Randomizer.rand(0, 3))));
					skillinfo.add(new Triple<>(new Point(39, -404), new Point(159, 238),
							Integer.valueOf(Randomizer.rand(0, 3))));
					skillinfo.add(new Triple<>(new Point(159, -404), new Point(279, 238),
							Integer.valueOf(Randomizer.rand(0, 3))));
					skillinfo.add(new Triple<>(new Point(279, -404), new Point(399, 238),
							Integer.valueOf(Randomizer.rand(0, 3))));
					skillinfo.add(new Triple<>(new Point(399, -404), new Point(519, 238),
							Integer.valueOf(Randomizer.rand(0, 3))));
					skillinfo.add(new Triple<>(new Point(519, -404), new Point(639, 238),
							Integer.valueOf(Randomizer.rand(0, 3))));
					skillinfo.add(new Triple<>(new Point(639, -404), new Point(759, 238),
							Integer.valueOf(Randomizer.rand(0, 3))));
					skillinfo.add(new Triple<>(new Point(759, -404), new Point(879, 238),
							Integer.valueOf(Randomizer.rand(0, 3))));
					...
					Timer.MapTimer.getInstance().schedule(() -> {
						...
						// map.broadcastMessage(MobPacket.mobBarrierEffect(map.getMonsterById(0x878178).getObjectId(),
						// "UI/UIWindow8.img/BlackMageShield/mobEffect_211",
						// "Sound/Etc.img/BlackMageShield",
						// "UI/UIWindow8.img/BlackMageShield/mobEffect0_211"));//38
					}, 5000L); // mush
					...
					for (int i = 0; i < Randomizer.rand(3, 4); i++) {
						...
						skillinfo.add(new Triple<>(new Point(ranx - 150, rany - 150), new Point(ranx + 150, rany + 150),
								Integer.valueOf(Randomizer.rand(0, 1))));
	...
	public static void SerenHandler(MapleMonster monster) {
		...
		case 8880630:
			monster.setSchedule(Timer.MobTimer.getInstance().register(() -> {
				...
				for (int i = 0; i < 3; i++) {
					...
					Obstacle ob = new Obstacle(84, new Point(x, -440), new Point(x, 275), 30, 15, i * 1000,
							Randomizer.rand(16, 32), 3, 715, 0);
					obs.add(ob);
				}
				monster.getMap().CreateObstacle(monster, obs);
			...
			monster.setSchedule(Timer.MobTimer.getInstance().register(() -> {
				...
				if (monster != null) {
					for (int i = 0; i < 2; ++i) {
						...
						Timer.MobTimer.getInstance().schedule(() -> {
							monster.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8880606),
									new Point(470, 305));
							monster.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8880606),
									new Point(-10, 305));
							monster.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8880606),
		...
		case 8880637: {
			monster.setSchedule(Timer.MobTimer.getInstance().register(() -> {
				MapleMap mapleMap = monster.getMap();
				FieldSkillFactory.getInstance();
				mapleMap.broadcastMessage(
						MobPacket.useFieldSkillExtremeSeren(FieldSkillFactory.getFieldSkill(100023, 1)));
			}, Randomizer.rand(5000, 10000)));
			break;
		}
		case 8880605:
		case 8880635: // [othello] 익스트림 보스
			monster.setSchedule(Timer.MobTimer.getInstance().register(() -> {
				for (MapleCharacter chr : monster.getMap().getAllChracater()) {
					if (chr.isAlive() && (chr.getPosition()).x - 300 <= (monster.getPosition()).x
							&& (chr.getPosition()).x + 300 >= (monster.getPosition()).x) {
		...
		case 8880640: // [othello] 익스트림 보스
			monster.setSchedule(Timer.MobTimer.getInstance().register(() -> {
				if (monster != null) {
					...
					if (type == 0) {
						monster.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8880611),
								new Point(-320, 305));
						monster.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8880611),
								new Point(470, 305));
					} else {
						monster.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8880611),
								(type == 1) ? new Point(-320, 305) : new Point(470, 305));
					}
				}
		...
		case 8880643: // [othello] 익스트림 보스
			...
			monster.setSchedule(Timer.MobTimer.getInstance().register(
					() -> monster.getMap().broadcastMessage(MobPacket.enableOnlyFsmAttack(monster, 1, 0)), time));
			break;
		}
	}
		...
		case 8880166: {
			monster.setSchedule(Timer.MobTimer.getInstance().register(() -> {
				...
				for (int i = 0; i < count; ++i) {
					...
					int monsterid = monster.getMap().getId() == 450004450 ? 8880180
							: (monster.getMap().getId() == 450003840 ? 8880186 : 8880160);
		...
		case 8880155: {
			monster.setSchedule(Timer.MobTimer.getInstance().register(() -> {
				...
				if (monster.getCustomValue0(23888) == 0L) {
					if ((monster.getId() == 8880150 || monster.getId() == 8880151 || monster.getId() == 8880153
							|| monster.getId() == 8880155) && monster.getCustomValue(8880150) == null) {
						...
						MapleMonster monste3r = MapleLifeFactory.getMonster(monster.getId() == 8880155 ? 8880194
								: (monster.getId() == 8880150 ? 8880170 : 8880182));
						...
						MapleMonster monste3r4 = MapleLifeFactory.getMonster(monster.getId() == 8880155 ? 8880195
								: (monster.getId() == 8880150 ? 8880171 : 8880183));
						Point pos2 = pos;
						Timer.MapTimer.getInstance().schedule(() -> {
					...
					if (monster != null && monster.getCustomValue(8880141) == null) {
						Butterfly bf = new Butterfly(Randomizer.rand(0, 8), Butterfly.getPosition(
								monster.getId() != 8880150 && monster.getId() != 8880151 && monster.getId() != 8880155,
								Randomizer.rand(0, 49)));
						...
						if (monster.getCustomValue0(8880140) == 20L) {
							monster.getMap().broadcastMessage(CField.enforceMSG("夢想正在變强.小心點!", 222, 3000));
						} else if (monster.getCustomValue0(8880140) >= 40L) {
							if (monster.getId() == 8880150 || monster.getId() == 8880151
									|| monster.getId() == 8880155) {
						...
						if (monster.getId() == 8880140 || monster.getId() == 8880141 || monster.getId() == 8880150
								|| monster.getId() == 8880151 || monster.getId() == 8880142
								|| monster.getId() == 8880155) {
							if (monster.getPhase() == 0 && monster.getHPPercent() <= 75) {
								monster.setPhase((byte) 1);
								monster.getMap().broadcastMessage(CField.enforceMSG("路西德正在發揮力量!", 222, 5000));
							} else if (monster.getPhase() == 1 && monster.getHPPercent() <= 50) {
								monster.setPhase((byte) 2);
								monster.getMap().broadcastMessage(CField.enforceMSG("路西德發揮更强大的力量!", 222, 5000));
							} else if (monster.getPhase() == 2 && monster.getHPPercent() <= 25) {
								monster.setPhase((byte) 3);
								monster.getMap().broadcastMessage(CField.enforceMSG("路西德好像非常憤怒!", 222, 5000));
							}
						}
						monster.setCustomInfo(8880141, 0, monster.getPhase() == 0 ? 4000
								: (monster.getPhase() == 1 ? 3000
										: (monster.getPhase() == 2 ? 2000 : (monster.getPhase() == 3 ? 1000 : 3000))));
					}
					if (monster.getId() == 8880153 && monster.getCustomValue(8880153) == null) {
						monster.getMap().broadcastMessage(CWvsContext.getTopMsg("路西德將持續全範圍攻擊."));
						for (MapleCharacter achr : monster.getMap().getAllCharactersThreadsafe()) {
							if (achr == null || achr.getBuffedValue(SecondaryStat.NotDamaged) != null
									|| achr.getBuffedValue(SecondaryStat.IndieNotDamaged) != null) {
								continue;
							}
	...
	public static void AggressIve(MapleMonster monster) {
		if (monster.getCustomValue(987654) == null) {
			...
			String txt = (monster.getStats().getName().equals("戴米安") || monster.getStats().getName().equals("班.雷昻")
					|| monster.getStats().getName().equals("卞熊")) ? "將" : "將";
			monster.getMap().broadcastMessage(MobPacket.getSmartNotice(monster.getId(), 1, 3, 0,
					monster.getStats().getName() + txt + " " + chr.getName() + "視爲最具有威脅的敵人."));
	...
	public static void dunkelHandler(MapleMonster dunkel, MapleMap map) {
		dunkel.setSchedule(Timer.MobTimer.getInstance().register(() -> {
			if ((dunkel.getEventInstance() == null || !dunkel.isAlive())
					&& dunkel.getMap().getCharacters().size() <= 0) {
			...
			if (dunkel.isAlive()) {
				if (dunkel.getCustomValue(dunkel.getId()) == null) {
					...
					for (int i = 0; i < radomcount; i++) {
						...
						dunkel.getMap().spawnMonsterOnGroundBelow(fallenWarrior,
								new Point(Randomizer.rand(-782, 774), 29));
						List<Pair<MonsterStatus, MonsterStatusEffect>> stats = new ArrayList<>();
				...
				if (dunkel.getCustomValue(dunkel.getId() + 1) == null) {
					...
					for (int i = 0; i < size; i++) {
						...
						if (type == 72) {
							ob = new Obstacle(type, new Point(x, -815), new Point(x, 29), 24, 10, 180,
									Randomizer.rand(300, 850), 1, 844, 0);
						} else if (type == 73) {
							ob = new Obstacle(type, new Point(x, -815), new Point(x, 29), 24, 10, 601,
									Randomizer.rand(300, 850), 1, 844, 0);
						} else if (type == 74) {
							ob = new Obstacle(type, new Point(x, -815), new Point(x, 29), 24, 10, 1377,
									Randomizer.rand(300, 850), 1, 844, 0);
						} else {
							ob = new Obstacle(type, new Point(x, -815), new Point(x, 29), 24, 10, 0,
									Randomizer.rand(300, 850), 1, 844, 0);
						}
						obs.add(ob);
					}
				...
				if (dunkel.getCustomValue(dunkel.getId() + 2) == null) {
					...
					for (int i = 0; i < count; i++) {
						...
						for (int j = 0; j < i; j++) {
							if (((DunkelEliteBoss) eliteBosses.get(i))
									.getbosscode() == ((DunkelEliteBoss) eliteBosses.get(j)).getbosscode()) {
								eliteBosses.set(i, getEliteBossAttack(dunkel));
								j--;
							}
						}
					}
					dunkel.getMap().broadcastMessage(
							MobPacket.BossDunKel.eliteBossAttack(dunkel, eliteBosses, null, Randomizer.nextBoolean()));
	...
	public static DunkelEliteBoss getEliteBossAttack(MapleMonster dunkel) {
		...
		if (type == 0) {
			eboss = new DunkelEliteBoss((short) type, (short) 1, 2800, 1440, 1, 3, 0, 0, 0, (short) 0, (short) 0,
					(short) 0, (byte) 0, isLeft, new Point(-280, -220), new Point(10, 10), cp, (short) 0, (short) 0);
		} else if (type == 1) {
			eboss = new DunkelEliteBoss((short) type, (short) 2, 3000, 1620, 1, 4, 300, 0, 1200, (short) 65,
					(short) 100, (short) 0, (byte) 0, (byte) 1, new Point(-100, -75), new Point(0, 0), new Point(0, 0),
					(short) 0, (short) 2);
		} else if (type == 2) {
			eboss = new DunkelEliteBoss((short) type, (short) 2, 3000, 1800, 1, 5, 0, 1, 1600, (short) 35, (short) 600,
					(short) 0, (byte) 0, (byte) 0, new Point(-45, -20), new Point(0, 0), new Point(0, 0), (short) 0,
					(short) 2);
		} else if (type == 3) {
			eboss = new DunkelEliteBoss((short) type, (short) 1, 2900, 1500, 1, 6, 0, 0, 0, (short) 0, (short) 0,
					(short) 0, (byte) 0, isLeft, new Point(-620, -135), new Point(50, 5), cp, (short) 0, (short) 0);
		} else if (type == 4) {
			eboss = new DunkelEliteBoss((short) type, (short) 3, 3300, 1710, 5, 7, 0, 0, 0, (short) 0, (short) 0,
					(short) 0, (byte) 1, isLeft, new Point(-40, -80), new Point(40, 0), cp,
					(short) Randomizer.rand(5, 8), (short) 0);
		} else if (type == 5) {
			eboss = new DunkelEliteBoss((short) type, (short) 1, 4800, 3630, 1, 8, 0, 0, 0, (short) 0, (short) 0,
					(short) 1, (byte) 1, isLeft, new Point(-290, -420), new Point(270, 25), cp, (short) 0, (short) 0);
		} else if (type == 6) {
			eboss = new DunkelEliteBoss((short) type, (short) 3, 3000, 2160, 7, 11, 0, 0, 0, (short) 0, (short) 0,
					(short) 1, (byte) 1, isLeft, new Point(-50, -170), new Point(50, 5), new Point(0, 0),
					(short) Randomizer.rand(5, 9), (short) 0);
		} else if (type == 7) {
			eboss = new DunkelEliteBoss((short) type, (short) 1, 4800, 3630, 1, 8, 0, 0, 0, (short) 0, (short) 0,
					(short) 1, (byte) 1, isLeft, new Point(-290, -420), new Point(270, 25), cp, (short) 0, (short) 0);
		} else if (type == 8) {
			eboss = new DunkelEliteBoss((short) type, (short) 1, 2800, 840, 1, 12, 0, 0, 0, (short) 0, (short) 0,
					(short) 0, (byte) 0, isLeft, new Point(-360, -155), new Point(10, 10), cp, (short) 0, (short) 0);
		} else if (type == 9) {
			eboss = new DunkelEliteBoss((short) type, (short) 1, 2700, 840, 1, 10, 0, 0, 0, (short) 0, (short) 0,
					(short) 0, (byte) 0, isLeft, new Point(-350, -155), new Point(10, 10), cp, (short) 0, (short) 0);
		} else {
			eboss = null;
		}
		return eboss;
	...
	public static void JinHillaGlassTime(MapleMonster jinhilla, int time) {
		...
		jinhilla.setSchedule(Timer.MobTimer.getInstance().schedule(() -> {
			if (jinhilla != null && jinhilla.isAlive()) {
				...
				jinhilla.getMap().broadcastMessage(MobPacket.moveMonsterResponse(jinhilla.getObjectId(),
						(short) (int) jinhilla.getCustomValue0(99991), jinhilla.getMp(), true, 247, 1, 0));
				msi.applyEffect(player, jinhilla, true, true);
				jinhilla.setLastSkillUsed(msi, System.currentTimeMillis(), (time * 1000));
				JinHillaGlassTime(jinhilla,
						(jinhilla.getHPPercent() >= 60) ? 150 : ((jinhilla.getHPPercent() >= 30) ? 120 : 100));
			}
		}, (time * 1000)));
	}
	...
	public static void chaosBlackMageHandler(MapleMonster monster) {
		monster.setSchedule(Timer.MobTimer.getInstance().register(() -> {
			...
			for (MapleCharacter chr : map.getAllChracater()) {
				...
				if ((chr.getMapId() == 450017100 && chr.getMap().getMobsSize(8880520) > 0
						&& chr.getMap().getMobsSize(8880521) > 0)
						|| (chr.getMapId() == 450017300 && chr.getMap().getMobsSize(8880522) > 0)
						|| (chr.getMapId() == 450017500 && chr.getMap().getMobsSize(8880523) > 0)
						|| (chr.getMapId() == 450017700 && chr.getMap().getMobsSize(8880524) > 0)) {
					if (chr.getMapId() != 450017700 && chr.getMapId() != 450017500
							&& chr.getSkillCustomValue(45001310) == null && map.getCustomValue0(45001316) == 0) {
						chr.setSkillCustomInfo(45001310, 0L, Randomizer.rand(7000, 11000));
						for (MapleMonster mob : chr.getMap().getAllMonster()) {
					...
					if (chr.getSkillCustomValue(45001311) == null && chr.hasDisease(SecondaryStat.CurseOfCreation)) {
						...
						if (chr.isAlive()) {
							chr.getClient().send(
									CField.getFieldSkillAdd(100015, (chr.getMapId() == 450017700) ? 2 : 1, false));
						}
					}
				}
			}
			if (map.getCustomValue0(45001317) > 0) {
				...
				for (MapleCharacter chr2 : map.getAllCharactersThreadsafe()) {
					if ((chr2.getPosition()).y == 85 && chr2.getMapId() == 450017500 && chr2.isAlive()
							&& chr2.getBuffedEffect(SecondaryStat.NotDamaged) == null
							&& chr2.getBuffedEffect(SecondaryStat.IndieNotDamaged) == null) {
						chr2.addHP(-chr2.getStat().getCurrentMaxHp() * 10L);
						chr2.getClient().send(CField.DamagePlayer2((int) chr2.getStat().getCurrentMaxHp() * 10));
			...
			if ((map.getId() == 450017100 && map.getMobsSize(8880520) > 0 && map.getMobsSize(8880521) > 0)
					|| (map.getId() == 450017300 && map.getMobsSize(8880522) > 0)
					|| (map.getId() == 450017500 && map.getMobsSize(8880523) > 0 && map.getCharactersSize() > 0)) {
				if (map.getCustomValue0(45001316) == 0 && map.getCustomValue(45001310) == null) {
					int type = Randomizer.rand(1, 2);
					List<Obstacle> obs = new ArrayList<>();
					map.setCustomInfo(45001310, 0, 60000);
					...
					map.CreateObstacle(monster, obs);
				} else if (map.getCustomValue(45001311) == null && map.getId() == 450017100
						&& map.getMobsSize(8880520) > 0 && map.getMobsSize(8880521) > 0) {
					map.setCustomInfo(45001311, 0, 63000);
					map.broadcastMessage(CField.enforceMSG("不祥的紅色閃電襲擊落下，扼制移動.", 265, 3000));
					map.spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8880506), new Point(-1600, 85));
					map.spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8880506), new Point(400, 85));
				...
				} else if (map.getCustomValue(45001315) == null && map.getId() == 450017300
						&& map.getMobsSize(8880522) > 0) {
					boolean use = true;
					int randt = 2;
					for (MapleMonster mob : map.getAllMonster()) {
						if (mob.getId() == 8880522) {
					...
					if (use) {
						...
						Timer.MapTimer.getInstance().schedule(() -> {
							...
							map.broadcastMessage(MobPacket.mobBarrierEffect(map.getMonsterById(8880522).getObjectId(),
									"UI/UIWindow8.img/BlackMageShield/mobEffect", "Sound/Etc.img/BlackMageShield",
									"UI/UIWindow8.img/BlackMageShield/mobEffect0"));
							map.getMonsterById(8880522).gainShield(3000000000000L, true, 30);
						}, 16000L);
					}
					// mush
				} else if (map.getCustomValue(45001312) == null && map.getId() == 450017100
						&& map.getMobsSize(8880520) > 0 && map.getMobsSize(8880521) > 0) {
					List<Point> pt = new ArrayList<>();
					int mobsize = map.getMobsSize(8880507) + map.getMobsSize(8880508);
					int basex = 2172;
					...
					for (MapleMonster mob : map.getAllMonster()) {
						...
						&& ((map.getId() == 450017300 && map.getMobsSize(8880522) > 0)
								|| (map.getId() == 450017500 && map.getMobsSize(8880523) > 0))) {
					map.setCustomInfo(45001314, 0, Randomizer.rand(45000, 51000));
					map.getObtacles(Randomizer.rand(1, 2));
				} else if (map.getCustomValue0(45001316) == 0 && map.getCustomValue(45001320) == null
						&& map.getId() == 450017500 && map.getMobsSize(8880523) > 0) {
				...
				} else if (map.getCustomValue(45001324) == null && map.getId() == 450017500
						&& map.getMobsSize(8880523) > 0) {
					List<Triple<Point, String, Integer>> skillinfo = new ArrayList<>();
					int type3 = 1;
					map.broadcastMessage(CField.enforceMSG("黑魔法師使用創造和破壞的權能。 要選擇上和下，藏到那一邊.", 265, 4000));
					map.setCustomInfo(45001324, 0, Randomizer.rand(60000, 75000));
					...
					Timer.MapTimer.getInstance().schedule(() -> {
						...
						map.broadcastMessage(CField.getFieldFootHoldAdd(100013, 1, skillinfo, false));
						map.broadcastMessage(MobPacket.mobBarrierEffect(map.getMonsterById(8880523).getObjectId(),
								"UI/UIWindow8.img/BlackMageShield/mobEffect_210", "Sound/Etc.img/BlackMageShield",
								"UI/UIWindow8.img/BlackMageShield/mobEffect0_210"));
						...
						Timer.MapTimer.getInstance().schedule(() -> {
							map.broadcastMessage(MobPacket.FieldSummonAttack(3, true, pos,
									map.getMonsterById(8880523).getObjectId(), damage));
							map.setCustomInfo(45001327,
									map.getMonsterById((int) 8880523).getPosition().x + Randomizer.rand(200, 500), 0);
							map.setCustomInfo(45001328, -304, 0);
							map.broadcastMessage(MobPacket
									.FieldSummonTeleport(new Point(map.getCustomValue0(45001327), -304), true));
						}, 2000L);
				...
				if (map.getCustomValue(45021328) == null && map.getId() == 450017500 && map.getMobsSize(8880523) > 0
						&& map.getCustomValue0(45011328) < 3) {
					int mobid = 0;
					map.broadcastMessage(CField.enforceMSG("破壞天使從無到有.", 265, 4000));
					map.setCustomInfo(45011328, map.getCustomValue0(45011328) + 1, 0);
			...
			if (map.getId() == 450017700 && map.getMobsSize(8880524) > 0) {
				if (map.getCustomValue(45004445) == null) {
					...
					Timer.MapTimer.getInstance().schedule(() -> {
						map.setCustomInfo(45004446, 0, 0);
						map.getMonsterById(0x878187).gainShield(3000000000000L, true, 0);
						map.broadcastMessage(MobPacket.mobBarrierEffect(map.getMonsterById(8880524).getObjectId(),
								"UI/UIWindow8.img/BlackMageShield/mobEffect_211", "Sound/Etc.img/BlackMageShield",
								"UI/UIWindow8.img/BlackMageShield/mobEffect0_211"));
					}, 5000L); // mush
				} else if (map.getCustomValue(45004444) == null && map.getCustomValue0(45004446) == 0) {
					map.setCustomInfo(45004444, 0, Randomizer.rand(7000, 12000));
					for (int i = 0; i < Randomizer.rand(3, 4); i++) {
	...
	public static void SlimeHandler(MapleMonster monster) {
		...
		monster.setSchedule(Timer.MobTimer.getInstance().register(() -> {
			if (monster.isAlive()) {
				if (monster.getCustomValue(monster.getId()) == null) {
					...
					for (int i = 0; i < radomcount; i++) {
						...
						if (monster.getId() == 8880700) {
							...
							monster.getMap().spawnMonsterOnGroundBelow(silme,
									new Point(Randomizer.rand(828, 868), -1385));
						}
					}
				}
				MapleMap mapleMap = monster.getMap();
				FieldSkillFactory.getInstance();
				mapleMap.broadcastMessage(
						MobPacket.BossSlime.useFieldSkillSlime(FieldSkillFactory.getFieldSkill(100023, 2)));
			}
		}, 5000L));
	}
}
