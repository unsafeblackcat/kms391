...
public class MapleInventory implements Iterable<Item>, Serializable {
	...
	public void move(short sSlot, short dSlot, short slotMax, MapleClient client) {// 修改記錄？？記錄GM移動物品信息
		Item source = this.inventory.get(Short.valueOf(sSlot));
		Item target = this.inventory.get(Short.valueOf(dSlot));
		if (source == null) {
			throw new InventoryException("试图移动空插槽");
		}
		// 判断玩家是否为GM，如果是则打印物品ID、名称和日期
		if (client != null && client.getPlayer() != null && client.getPlayer().isGM()) {
			MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
			String itemName = ii.getName(source.getItemId());
			// 获取当前日期和时间
