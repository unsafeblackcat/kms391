...
public class MapleAndroid implements Serializable {
	...
	public final void saveToDb() {
		...
		try {
			...
			ps = con.prepareStatement(
					"UPDATE androids SET hair = ?, face = ?, name = ?, skin = ?, gender = ?, ear = ? WHERE uniqueid = ?");
	...
	public static final MapleAndroid create(int itemid, long uniqueid) {
		Triple<Pair<List<Integer>, List<Integer>>, List<Integer>, Integer> aInfo = MapleItemInformationProvider
				.getInstance().getAndroidInfo(GameConstants.getAndroidType(itemid));
		if (aInfo == null) {
			return null;
		}
		return create(itemid, uniqueid,
				((Integer) ((List<Integer>) ((Pair) aInfo.left).left)
						.get(Randomizer.nextInt(((List) ((Pair) aInfo.left).left).size()))).intValue(),
				((Integer) ((List<Integer>) ((Pair) aInfo.left).right)
						.get(Randomizer.nextInt(((List) ((Pair) aInfo.left).right).size()))).intValue(),
				((Integer) ((List<Integer>) aInfo.mid).get(Randomizer.nextInt(((List) aInfo.mid).size()))).intValue(),
				((Integer) aInfo.right).intValue());
	}
	...
	public static final MapleAndroid create(int itemid, long uniqueid, int hair, int face, int skin, int gender) {
		...
		try {
			...
			pse = con.prepareStatement(
					"INSERT INTO androids (uniqueid, hair, face, skin, gender, name, ear) VALUES (?, ?, ?, ?, ?, ?, ?)");
