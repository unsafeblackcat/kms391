...
public class BountyHunting {
		...
		Timer.EventTimer.getInstance().schedule(new Runnable() {
			public void run() {
				if (c.getPlayer().getMapId() == 993000000) {
					c.getSession()
							.writeAndFlush(CField.environmentChange("defense/wave/" + BountyHunting.this.stage, 16));
					c.getSession().writeAndFlush(CField.environmentChange("killing/first/start", 16));
					for (Iterator<Integer> iterator = ((List) BountyHunting.this.waveData
							.get(BountyHunting.this.stage - 1)).iterator(); iterator.hasNext();) {
						int wave = ((Integer) iterator.next()).intValue();
						c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(wave),
								new Point(Randomizer.rand(500, 700) * -1, 126));
						c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(wave),
								new Point(Randomizer.rand(500, 700), 126));
					}
				}
			}
		}, 3000L);
	...
	public void start(final MapleClient c) {
		...
		insertWaveData();
		c.getSession()
				.writeAndFlush(CField.startMapEffect("놈들이 사방에서 몰려오는군! 녀석들을 처치하면 막대한 경험치를 얻을 수 있다!", 5120159, true));
