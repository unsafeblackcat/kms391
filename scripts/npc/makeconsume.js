...
function action(mode, type, sel) {
   ...
   if (status == 0) {
      var msg = "#fs15#소비 아이템을 제작 하고싶으신가요?#k#n"+enter;
      msg += "";
      for (i = 0; i < items.length; i++)
         msg += "#fs15##b#L"+i+"##i"+items[i]['itemid']+"##z"+items[i]['itemid']+"# "+items[i]['qty']+"개#k"+enter;
      ...
      var msg = "#fs15#선택한 아이템은 다음과 같습니다.#fs15##b"+enter;
      msg += "#fs15#아이템 : #i"+item['itemid']+"##z"+item['itemid']+"# "+item['qty']+"개"+enter;
      ...
      msg += enter;
      msg += "#fs15##k선택한 아이템을 제작하기 위한 레시피입니다.#fs15##d"+enter+enter;
      ...
      if (Packages.server.Randomizer.rand(1, 100) <= item['chance']) {
         gainItem(item);
         cm.sendOk("#fs15#제작에 성공하였습니다.");
      } else {
         cm.sendOk("#fs15#제작에 실패했습니다.");
         gainFail(item);
      }
      cm.dispose();
   }
