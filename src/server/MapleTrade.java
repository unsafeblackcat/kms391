...
public class MapleTrade {
	...
	public final void CompleteTrade() {
		if (this.exchangeItems != null) {
			...
			for (Item item : itemz) {
				item.setGMLog(
						StringUtil.getAllCurrentTime() + "에 " + this.partner.getChr().getName() + "과의 교환으로 얻은 아이템.");
				...
				FileoutputUtil.log(FileoutputUtil.交換物品日誌,
						"[교환 완료] " + ((MapleCharacter) this.chr.get()).getName() + "이 "
								+ this.partner.getChr().getName() + "의 교환 창에서 "
								+ MapleItemInformationProvider.getInstance().getName(item.getItemId()) + "을 "
								+ item.getQuantity() + "개 획득.");
			}
		...
		if (this.exchangeMeso > 0L) {
			...
			FileoutputUtil.log(FileoutputUtil.交換楓幣日誌,
					"[교환 완료] " + ((MapleCharacter) this.chr.get()).getName() + "이 " + this.partner.getChr().getName()
							+ "의 교환 창에서 " + (this.exchangeMeso - GameConstants.getTaxAmount(this.exchangeMeso))
							+ "를 획득.");
		}
	...
	public final void setMeso(long meso) {
		...
		if (((MapleCharacter) this.chr.get()).getMeso() >= meso) {
			...
			FileoutputUtil.log(FileoutputUtil.交換楓幣日誌, "[교환 메소 올림] " + ((MapleCharacter) this.chr.get()).getName() + "이 "
					+ this.partner.getChr().getName() + "의 교환 창에서 " + this.meso + "를 올림.");
		}
	}
	...
	public final void addItem(Item item) {
		...
		FileoutputUtil.log(FileoutputUtil.交換物品日誌,
				"[교환 아이템 올림] " + ((MapleCharacter) this.chr.get()).getName() + "이 " + this.partner.getChr().getName()
						+ "의 교환 창에서 " + MapleItemInformationProvider.getInstance().getName(item.getItemId()) + "을 "
						+ item.getQuantity() + "개 올림.");
	}

	public final void chat(String message) {
		if (!CommandProcessor.processCommand(((MapleCharacter) this.chr.get()).getClient(), message,
				ServerConstants.CommandType.TRADE)) {
			((MapleCharacter) this.chr.get()).getClient().getSession()
					.writeAndFlush(PlayerShopPacket.shopChat(this.chr.get(),
							((MapleCharacter) this.chr.get()).getName(), ((MapleCharacter) this.chr.get()).getId(),
							message, (((MapleCharacter) this.chr.get()).getTrade()).tradingslot));
			if (this.partner != null) {
				this.partner.getChr().getClient().getSession()
						.writeAndFlush(PlayerShopPacket.shopChat(this.chr.get(),
								((MapleCharacter) this.chr.get()).getName(), ((MapleCharacter) this.chr.get()).getId(),
		...
		DBLogger.getInstance().logChat(LogType.Chat.Trade, ((MapleCharacter) this.chr.get()).getId(),
				((MapleCharacter) this.chr.get()).getName(), message, "수신 : " + this.partner.getChr().getName());
		if (((MapleCharacter) this.chr.get()).getClient().isMonitored()) {
			World.Broadcast.broadcastGMMessage(CWvsContext.serverNotice(6, ((MapleCharacter) this.chr.get()).getName(),
					((MapleCharacter) this.chr.get()).getName() + " said in trade with "
	...
	public final void chatAuto(String message) {
		...
		if (this.partner != null) {
			this.partner.getChr().getClient().getSession().writeAndFlush(PlayerShopPacket.shopChat(
					this.partner.getChr(), this.partner.getChr().getName(), this.partner.getChr().getId(), message, 1));
		}
		if (((MapleCharacter) this.chr.get()).getClient().isMonitored()) {
			World.Broadcast.broadcastGMMessage(CWvsContext.serverNotice(6, ((MapleCharacter) this.chr.get()).getName(),
					((MapleCharacter) this.chr.get()).getName() + " said in trade [Automated] with "
							+ this.partner.getChr().getName() + ": " + message));
		} else if (this.partner != null && this.partner.getChr() != null
				&& this.partner.getChr().getClient().isMonitored()) {
			World.Broadcast.broadcastGMMessage(CWvsContext.serverNotice(6, ((MapleCharacter) this.chr.get()).getName(),
	...
	public final boolean setItems(MapleClient c, Item item, byte targetSlot, int quantity) {
		...
		if (this.partner == null || target == -1 || isLocked()
		...
		if ((ItemFlag.UNTRADEABLE.check(flag) || GameConstants.isPet(item.getItemId())
				|| ii.isDropRestricted(item.getItemId()) || ii.isAccountShared(item.getItemId()))
		...
		if (item.getType() == 1) {
			...
			if ((equip.getEnchantBuff() & 0x88) != 0) {
				c.getSession().writeAndFlush(CWvsContext.enableActions(c.getPlayer()));
				c.getPlayer().dropMessage(1, "無法兌換裝備的痕迹.");
				return false;
			}
		}
	...
	private final int check() {
		...
		if (this.exchangeItems != null) {
			...
			if (((MapleCharacter) this.chr.get()).getInventory(MapleInventoryType.EQUIP).getNumFreeSlot() < eq
					...
					|| ((MapleCharacter) this.chr.get()).getInventory(MapleInventoryType.SETUP).getNumFreeSlot() < setup
	...
	public static final void startTrade(MapleCharacter c, boolean isTrade) {
		if (c.getTrade() == null) {
			...
			c.getClient().getSession().writeAndFlush(
					CField.InteractionPacket.getTradeStart(c.getClient(), c.getTrade(), (byte) 0, isTrade));
			c.isTrade = isTrade;
		} else {
	...
	public static final void visitTrade(MapleCharacter c1, MapleCharacter c2, boolean isTrade) {
		if (c2 != null && c1.getTrade() != null && c1.getTrade().getPartner() == c2.getTrade() && c2.getTrade() != null
			...
			c1.getClient().getSession().writeAndFlush(
					CField.InteractionPacket.getTradeStart(c1.getClient(), c1.getTrade(), (byte) 1, isTrade));
		} else {
			c1.getClient().getSession().writeAndFlush(CWvsContext.serverNotice(5, "", "다른 유저와 교환중인 유저입니다."));
		}
	}
