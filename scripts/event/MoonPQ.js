...
function spawnMonster(eim) {
    ...
    for(var i = 0; i < Mmobx.length; i++){
        for(var j = 0; j < Mmoby.length-1; j++){
            map.spawnMonsterOnGroundBelow(Packages.server.Luna.MapleLunaFactory.getMonster(Mmobid[0]), new java.awt.Point(Mmobx[i], Mmoby[j]));
            map.spawnMonsterOnGroundBelow(Packages.server.Luna.MapleLunaFactory.getMonster(Mmobid[1]), new java.awt.Point(Mmobx[i], Mmoby[j]));
        }
    }
}
