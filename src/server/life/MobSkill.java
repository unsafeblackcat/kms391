...
public class MobSkill {
	...
	public boolean checkCurrentBuff(MapleCharacter player, MapleMonster monster) {
		...
		case 144:
		case 145:
			stop = (monster.isBuffed(MonsterStatus.MS_Hardskin) || monster.isBuffed(MonsterStatus.MS_PowerImmune)
					|| monster.isBuffed(MonsterStatus.MS_MImmune) || monster.isBuffed(MonsterStatus.MS_PImmune));
			break;
		case 129:
		...
		case 184:
			if (monster.getId() == 8910000) {
				if (this.skillLevel == 1 && (monster.getHPPercent() < 10
						|| (monster.getHPPercent() <= 100 && monster.getHPPercent() >= 70))) {
					stop = true;
				}
				break;
			}
			if (monster.getId() == 8910100 && (monster.getHPPercent() < 10
					|| (monster.getHPPercent() <= 100 && monster.getHPPercent() >= 70))) {
				stop = true;
			}
			break;
	...
	public void applyEffect(MapleCharacter player, final MapleMonster monster, boolean skill, boolean isFacingLeft,
			int RectCount) {
		try {
			BanishInfo info;
			List<Point> list;
			...
			if (monster != null && player != null) {
				player.dropMessageGM(5, monster.getStats().getName() + "(" + monster.getId() + ")의 스킬 : " + this.skillId
						+ " / " + this.skillLevel);
			}
			switch (this.skillId) {
			case 100:
				stats.add(new Pair<>(MonsterStatus.MS_Pad,
						new MonsterStatusEffect(this.skillId, (int) this.duration, this.x)));
				break;
			case 101:
				stats.add(new Pair<>(MonsterStatus.MS_Mad,
						new MonsterStatusEffect(this.skillId, (int) this.duration, this.x)));
				break;
			case 102:
				stats.add(new Pair<>(MonsterStatus.MS_Pdr,
						new MonsterStatusEffect(this.skillId, (int) this.duration, this.x)));
				break;
			case 103:
				stats.add(new Pair<>(MonsterStatus.MS_Mdr,
						new MonsterStatusEffect(this.skillId, (int) this.duration, this.x)));
				break;
			case 105:
			...
			case 114:
				if (this.lt != null && this.rb != null && skill && monster != null) {
					...
					for (MapleMapObject mons : objects) {
						...
						player.getMap().broadcastMessage(MobPacket
								.healEffectMonster(((MapleMonster) mons).getObjectId(), this.skillId, this.skillLevel));
					}
					break;
				}
				if (monster != null) {
					...
					player.getMap().broadcastMessage(
							MobPacket.healEffectMonster(monster.getObjectId(), this.skillId, this.skillLevel));
				}
				break;
			case 115:
				stats.add(new Pair<>(MonsterStatus.MS_Speed,
						new MonsterStatusEffect(this.skillId, (int) this.duration, this.x)));
				break;
			case 120:// 卡熊38
				// diseases.put(SecondaryStat.Seal, new Pair<>(Integer.valueOf(this.x),
				// Integer.valueOf((int) this.duration)));
				// if (this.skillLevel == 37) {
				// diseases.put(SecondaryStat.KawoongDebuff, new Pair<>(Integer.valueOf(20),
				// Integer.valueOf((int) this.duration)));
				// }
				break;
			case 121:
				diseases.put(SecondaryStat.Darkness,
						new Pair<>(Integer.valueOf(this.x), Integer.valueOf((int) this.duration)));
				break;
			case 122:
				diseases.put(SecondaryStat.Weakness,
						new Pair<>(Integer.valueOf(this.x), Integer.valueOf((int) this.duration)));
				if (this.skillLevel == 20) {
					diseases.put(SecondaryStat.Slow,
							new Pair<>(Integer.valueOf(60), Integer.valueOf((int) this.duration)));
				}
				break;
			case 123:
				...
				break;
			case 124:
				diseases.put(SecondaryStat.Curse,
						new Pair<>(Integer.valueOf(this.x), Integer.valueOf((int) this.duration)));
				break;
			case 125:
				diseases.put(SecondaryStat.Poison,
						new Pair<>(Integer.valueOf(this.x), Integer.valueOf((int) this.duration)));
				break;
			case 126:
				diseases.put(SecondaryStat.Slow,
			...
			case 127:
				...
				break;
			case 128:
				diseases.put(SecondaryStat.Attract,
						new Pair<>(Integer.valueOf(this.x), Integer.valueOf((int) this.duration)));
				break;
			case 129:
				if (monster == null || (monster.getEventInstance() != null
			...
			case 131: {
				...
				if (monster.getId() / 1000 != 8950) {
					mist = new MapleMist(calculateBoundingBox(monster.getTruePosition(), true), monster, this,
							(int) this.duration);
				}
			...
			case 132:
				diseases.put(SecondaryStat.ReverseInput,
						new Pair<>(Integer.valueOf(1), Integer.valueOf((int) this.duration)));
				break;
			case 133:
				diseases.put(SecondaryStat.Undead,
						new Pair<>(Integer.valueOf(this.x), Integer.valueOf((int) this.duration)));
				break;
			case 134:
				diseases.put(SecondaryStat.StopPortion,
						new Pair<>(Integer.valueOf(this.x), Integer.valueOf((int) this.duration)));
				break;
			case 135:
				diseases.put(SecondaryStat.StopMotion,
						new Pair<>(Integer.valueOf(this.x), Integer.valueOf((int) this.duration)));
				break;
			case 136:
				diseases.put(SecondaryStat.Fear,
						new Pair<>(Integer.valueOf(this.x), Integer.valueOf((int) this.duration)));
				break;
			case 137:
				diseases.put(SecondaryStat.Frozen,
						new Pair<>(Integer.valueOf(this.x), Integer.valueOf((int) this.duration)));
				break;
			case 138:
				diseases.put(SecondaryStat.DispelItemOption,
						new Pair<>(Integer.valueOf(this.x), Integer.valueOf((int) this.duration)));
				break;
			case 140:
				stats.add(new Pair<>(MonsterStatus.MS_PImmune,
						new MonsterStatusEffect(this.skillId, (int) this.duration, this.x)));
				break;
			case 141:
				stats.add(new Pair<>(MonsterStatus.MS_MImmune,
						new MonsterStatusEffect(this.skillId, (int) this.duration, this.x)));
				break;
			case 142:
				stats.add(new Pair<>(MonsterStatus.MS_Hardskin,
						new MonsterStatusEffect(this.skillId, (int) this.duration, this.x)));
				break;
			case 143:
				stats.add(new Pair<>(MonsterStatus.MS_PImmune,
						new MonsterStatusEffect(this.skillId, (int) this.duration, this.x)));
				stats.add(new Pair<>(MonsterStatus.MS_PCounter,
						new MonsterStatusEffect(this.skillId, (int) this.duration, this.x)));
				break;
			case 144:
				stats.add(new Pair<>(MonsterStatus.MS_MImmune,
						new MonsterStatusEffect(this.skillId, (int) this.duration, this.x)));
				stats.add(new Pair<>(MonsterStatus.MS_MCounter,
						new MonsterStatusEffect(this.skillId, (int) this.duration, this.x)));
				break;
			case 145:
				stats.add(new Pair<>(MonsterStatus.MS_PImmune,
				...
				if (monster.getId() == 8840000 || monster.getId() == 8840007 || monster.getId() == 8840014) {
					monster.getMap().broadcastMessage(
							MobPacket.getSmartNotice(monster.getId(), 1, 5, 0, "근 거리의 포위당한 반 레온이 반격을 시전합니다."));
				}
				break;
			case 146:
				if (monster.getId() == 8850010 || monster.getId() == 8850110) {
					...
					if (cyguns != null) {
						stats.add(new Pair<>(MonsterStatus.MS_PowerImmune,
								new MonsterStatusEffect(this.skillId, (int) this.duration, this.x)));
			...
			case 153:
				...
				break;
			case 154:
				stats.add(new Pair<>(MonsterStatus.MS_Acc,
						new MonsterStatusEffect(this.skillId, (int) this.duration, this.x)));
				break;
			case 155:
				stats.add(new Pair<>(MonsterStatus.MS_Eva,
						new MonsterStatusEffect(this.skillId, (int) this.duration, this.x)));
				break;
			case 156:
				stats.add(new Pair<>(MonsterStatus.MS_Speed,
				...
				break;
			case 157:
				stats.add(new Pair<>(MonsterStatus.MS_Seal,
						new MonsterStatusEffect(this.skillId, (int) this.duration, this.x)));
				break;
			case 170:
				...
				case 3:
					if (monster.getId() == 8840000 || monster.getId() == 8840007 || monster.getId() == 8840014) {
						monster.getMap().broadcastMessage(MobPacket.getSmartNotice(monster.getId(), 1, 5, 0,
								monster.getStats().getName() + "이 자신에게 가장 큰 위협을 준 적에게 순간이동하여 다가갑니다."));
						monster.setPosition(player.getTruePosition());
						player.getClient().getSession()
								.writeAndFlush(MobPacket.TeleportMonster(monster, false, 2, monster.getTruePosition()));
						break;
					}
					monster.setPosition(player.getTruePosition());
					player.getClient().getSession()
				...
				case 10:
					monster.setPosition(player.getTruePosition());
					monster.getMap()
							.broadcastMessage(MobPacket.TeleportMonster(monster, false, 5, monster.getTruePosition()));
					break;
				case 13:
					list = new ArrayList<>();
					...
					if (isFacingLeft) {
						...
						monster.getMap()
								.broadcastMessage(MobPacket.TeleportMonster(monster, false, 3, new Point(965, 420)));
					} else {
						monster.setPosition(new Point(-1750, 420));
						monster.getMap()
								.broadcastMessage(MobPacket.TeleportMonster(monster, false, 3, new Point(-1750, 420)));
					}
					monster.getMap().broadcastMessage(CWvsContext.getTopMsg("벨룸이 깊은 숨을 들이쉽니다."));
					monster.getMap()
							.broadcastMessage(MobPacket.setAfterAttack(monster.getObjectId(), 9, 1, 21, isFacingLeft));
				...
				case 42:
					...
					for (int is = 0; is < 3; ++is) {
						mda.add(new MapleDelayedAttack(monster,
								new Point(xs + (is * 250 * (facingleft ? -1 : 1)), -113), Randomizer.rand(80, 130),
								facingleft));
					}

					monster.getMap().spawnDelayedAttack(monster, MobSkillFactory.getMobSkill(skillId, skillLevel), mda);
					x = isFacingLeft ? -900 : 900;
					if (monster.getMap().getFootholds()
							.findBelow(new Point((monster.getPosition()).x + x, (monster.getPosition()).y)) == null) {
						x = 0;
					}
					monster.getMap().broadcastMessage(MobPacket.TeleportMonster(monster, false, 10,
							new Point((monster.getPosition()).x + x, 17)));
					break;
				case 44:
					if (monster.getId() == 8880409) {
						monster.getMap().broadcastMessage(MobPacket.TeleportMonster(monster, false, 12,
								new Point(Randomizer.rand(-700, 700), 16)));
						break;
					}
					monster.getMap().broadcastMessage(
							MobPacket.TeleportMonster(monster, false, 12, new Point(Randomizer.rand(-40, 1400), 16)));
					break;
				case 45:
					x = isFacingLeft ? -680 : 680;
					monster.getMap().broadcastMessage(MobPacket.TeleportMonster(monster, false, 12,
							new Point((monster.getPosition()).x + x, 16)));
					break;
				case 46:
				...
				case 50:
					monster.getMap().broadcastMessage(
							MobPacket.TeleportMonster(monster, true, 1, new Point((monster.getPosition()).x, 16)));
					monster.getMap().broadcastMessage(MobPacket.setAttackZakumArm(monster.getObjectId(), 7));
					monster.getMap().broadcastMessage(CField.enforceMSG("需在戴米安暴走墮落世界樹之力前造成大量傷害封锁他.", 216, 9500000));
					monster.getMap().broadcastMessage(MobPacket.demianRunaway(monster, (byte) 0,
							MobSkillFactory.getMobSkill(214, 14), 10000, false));
					monster.setCustomInfo(8880111, 1, 0);
					monster.setNextSkill(0);
					monster.setNextSkillLvl(0);
					monster.setSkillForbid(true);
					monster.setLastSkillUsed(MobSkillFactory.getMobSkill(214, 14), System.currentTimeMillis(),
							MobSkillFactory.getMobSkill(214, 14).getDuration());
						...
						if (monster.isAlive()) {
							...
							if (suc) {
								...
								Timer.MobTimer.getInstance().schedule(() -> {
									if (monster.isAlive()) {
										monster.getMap().broadcastMessage(
												MobPacket.setAttackZakumArm(monster.getObjectId(), 8));
										monster.getMap().broadcastMessage(MobPacket.demianRunaway(monster, (byte) 1,
												MobSkillFactory.getMobSkill(214, 14), 10000, true));
										monster.setSkillForbid(false);
									}
								}, 500L);
								...
								monster.getMap()
										.broadcastMessage(MobPacket.setAttackZakumArm(monster.getObjectId(), 8));
								monster.setSkillForbid(false);
							}
						}
					}, 10000L);
				...
				case 58:
					monster.setPosition(player.getTruePosition());
					monster.getMap()
							.broadcastMessage(MobPacket.TeleportMonster(monster, false, 2, monster.getTruePosition()));
					break;
				case 62:
					rchar = monster.getController().getClient().getRandomCharacter();
					if (rchar != null) {
						monster.getMap()
								.broadcastMessage(MobPacket.TeleportMonster(monster, false, 2, rchar.getPosition()));
					}
				...
				case 64:
					...
					if (mapleCharacter1 != null) {
						...
						monster.getMap().broadcastMessage(MobPacket.TeleportMonster(monster, false, 2,
								(pos2 != null) ? pos2 : mapleCharacter1.getPosition()));
					}
					break;
				case 63:
					monster.setPosition(player.getTruePosition());
					...
					break;
				case 77:
					monster.setPosition(monster.isFacingLeft()
							? new Point((monster.getPosition()).x - 400, (monster.getPosition()).y)
							: new Point((monster.getPosition()).x + 400, (monster.getPosition()).y));
					player.getClient().getSession()
							.writeAndFlush(MobPacket.TeleportMonster(monster, false, 16, monster.getTruePosition()));
			...
			case 171:
				diseases.put(SecondaryStat.TimeBomb,
						new Pair<>(Integer.valueOf(this.x), Integer.valueOf((int) this.duration)));
				break;
			case 172:
				diseases.put(SecondaryStat.Morph,
						new Pair<>(Integer.valueOf(this.x), Integer.valueOf((int) this.duration)));
				if (monster.getId() == 8850011 || monster.getId() == 8850111) {
					monster.getMap().broadcastMessage(
							MobPacket.getSmartNotice(monster.getId(), 0, 5, 1, "시그너스가 버프 스킬을 비웃으며 적을 리본돼지로 변이하려 합니다."));
				}
				break;
			case 173:
				diseases.put(SecondaryStat.DarkTornado,
						new Pair<>(Integer.valueOf(this.x), Integer.valueOf((int) this.duration)));
				break;
			case 174:
				if (this.skillLevel == 14) {
				...
				diseases.put(SecondaryStat.Lapidification,
						new Pair<>(Integer.valueOf(this.x), Integer.valueOf((int) this.duration)));
				break;
			case 175:
				cancels.add(SecondaryStat.DeathMark);
				if (player.getDebuffValue(SecondaryStat.DeathMark).intValue() == 1) {
					diseases.put(SecondaryStat.DeathMark,
							new Pair<>(Integer.valueOf(2), Integer.valueOf((int) this.duration)));
					break;
				}
				if (player.getDebuffValue(SecondaryStat.DeathMark).intValue() == 2) {
					diseases.put(SecondaryStat.DeathMark,
							new Pair<>(Integer.valueOf(3), Integer.valueOf((int) this.duration)));
					break;
				}
				if (player.getDebuffValue(SecondaryStat.DeathMark).intValue() <= 2) {
					diseases.put(SecondaryStat.DeathMark,
			...
			case 176:
					...
					minx = (type == 3) ? -85
							: ((type == 4) ? -215
									: ((type == 5) ? -340
											: ((type == 6) ? -465
													: ((type == 7) ? 60
															: ((type == 8) ? 200 : ((type == 9) ? 350 : 500))))));
					for (MapleCharacter mapleCharacter : player.getMap().getAllCharactersThreadsafe()) {
						if (minx - 70 < (mapleCharacter.getPosition()).x && minx + 70 > (mapleCharacter.getPosition()).x
								&& (mapleCharacter.getPosition()).y > -15 && mapleCharacter.isAlive()) {
							int percent = (monster.getId() >= 8800023 && monster.getId() <= 8800030) ? 50
									: ((monster.getId() >= 8800003 && monster.getId() <= 8800010) ? 90
											: ((monster.getId() >= 8800103 && monster.getId() <= 8800110) ? 100 : 0));
				...
				case 27:
					...
					for (MapleCharacter mapleCharacter : player.getMap().getAllCharactersThreadsafe()) {
						if (mapleCharacter.isAlive()) {
							...
							if (type == 3 || type == 7) {
								if (mapleCharacter.getFH() == 20 || mapleCharacter.getFH() == 19
										|| mapleCharacter.getFH() == 18 || mapleCharacter.getFH() == 15
										|| mapleCharacter.getFH() == 16 || mapleCharacter.getFH() == 17) {
									damage = true;
								}
							} else if (type == 4 || type == 8) {
								if (mapleCharacter.getFH() == 14 || mapleCharacter.getFH() == 13
										|| mapleCharacter.getFH() == 12 || mapleCharacter.getFH() == 11
										|| mapleCharacter.getFH() == 10 || mapleCharacter.getFH() == 9) {
									damage = true;
								}
							} else if ((type == 5 || type == 9)
									&& (mapleCharacter.getFH() == 5 || mapleCharacter.getFH() == 4
											|| mapleCharacter.getFH() == 3 || mapleCharacter.getFH() == 6
											|| mapleCharacter.getFH() == 7 || mapleCharacter.getFH() == 8)) {
								damage = true;
							}
							if (damage) {
				...
				if ((monster.getId() == 8860000 || monster.getId() == 8860005) && this.skillLevel >= 1
						&& this.skillLevel <= 4) {
					value = this.x;
								...
								if (mapleCharacter.isAlive()) {
									if (mapleCharacter.getBuffedEffect(SecondaryStat.HolyMagicShell) != null) {
										if (mapleCharacter.getHolyMagicShell() > 1) {
											mapleCharacter
													.setHolyMagicShell((byte) (mapleCharacter.getHolyMagicShell() - 1));
											Map<SecondaryStat, Pair<Integer, Integer>> statups = new HashMap<>();
											statups.put(SecondaryStat.HolyMagicShell, new Pair<>(
													Integer.valueOf(mapleCharacter.getHolyMagicShell()),
													Integer.valueOf((int) mapleCharacter.getBuffLimit(mapleCharacter
															.getBuffSource(SecondaryStat.HolyMagicShell)))));
											mapleCharacter.getClient().getSession()
													.writeAndFlush(
															CWvsContext.BuffPacket.giveBuff(statups,
																	mapleCharacter.getBuffedEffect(
																			SecondaryStat.HolyMagicShell),
																	mapleCharacter));
											mapleCharacter.getClient().getSession()
													.writeAndFlush(CField.EffectPacket.showEffect(mapleCharacter, 1, 0,
															36, 0, 0, (byte) 0, true, null, null, null));
											mapleCharacter.getMap().broadcastMessage(mapleCharacter,
													CField.EffectPacket.showEffect(mapleCharacter, 1, 0, 36, 0, 0,
										...
										mapleCharacter.getClient().getSession()
												.writeAndFlush(CField.EffectPacket.showEffect(mapleCharacter, 1, 0, 36,
														0, 0, (byte) 0, true, null, null, null));
										mapleCharacter.getMap()
												.broadcastMessage(
														mapleCharacter, CField.EffectPacket.showEffect(mapleCharacter,
																1, 0, 36, 0, 0, (byte) 0, false, null, null, null),
														false);
									} else if (mapleCharacter.getBuffedEffect(SecondaryStat.WindWall) != null) {
										int windWall = Math.max(0, mapleCharacter.getBuffedValue(SecondaryStat.WindWall)
												.intValue()
												- 100 * mapleCharacter.getBuffedEffect(SecondaryStat.WindWall).getZ());
										if (windWall > 1) {
											mapleCharacter.setBuffedValue(SecondaryStat.WindWall, windWall);
											Map<SecondaryStat, Pair<Integer, Integer>> statups = new HashMap<>();
											statups.put(SecondaryStat.WindWall, new Pair<>(Integer.valueOf(windWall),
													Integer.valueOf((int) mapleCharacter.getBuffLimit(400031030))));
											mapleCharacter.getClient().getSession()
													.writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups,
															mapleCharacter.getBuffedEffect(SecondaryStat.WindWall),
															mapleCharacter));
										} else {
											mapleCharacter.cancelEffectFromBuffStat(SecondaryStat.WindWall);
										}
									} else if (mapleCharacter.getBuffedEffect(SecondaryStat.TrueSniping) == null
											&& mapleCharacter.getBuffedEffect(SecondaryStat.Etherealform) == null
											&& mapleCharacter.getBuffedEffect(SecondaryStat.IndieNotDamaged) == null
											&& mapleCharacter.getBuffedEffect(SecondaryStat.NotDamaged) == null) {
										int reduce = 0;
										if (mapleCharacter.getBuffedEffect(SecondaryStat.IndieDamageReduce) != null) {
											reduce = mapleCharacter.getBuffedValue(SecondaryStat.IndieDamageReduce)
													.intValue();
										} else if (mapleCharacter
												.getBuffedEffect(SecondaryStat.IndieDamReduceR) != null) {
											reduce = -mapleCharacter.getBuffedValue(SecondaryStat.IndieDamReduceR)
													.intValue();
										}
										...
										int minushp = -value;
										mapleCharacter.getClient().getSession()
												.writeAndFlush(CField.EffectPacket.showEffect(mapleCharacter,
														this.skillId, this.skillLevel, 45, 0, 0, (byte) 0, true, null,
														null, null));
										mapleCharacter.getMap().broadcastMessage(mapleCharacter,
												CField.EffectPacket.showEffect(mapleCharacter, this.skillId,
														this.skillLevel, 45, 0, 0, (byte) 0, false, null, null, null),
												false);
										mapleCharacter.getClient().getSession()
												.writeAndFlush(CField.EffectPacket.showEffect(mapleCharacter, 0,
														minushp, 36, 0, 0, (byte) 0, true, null, null, null));
										mapleCharacter.getMap().broadcastMessage(mapleCharacter,
												CField.EffectPacket.showEffect(mapleCharacter, 0, minushp, 36, 0, 0,
														(byte) 0, false, null, null, null),
												false);
										mapleCharacter.addHP(minushp);
									}
								}
								mapleCharacter.getClient().getSession().writeAndFlush(
										CField.screenAttack(monster.getId(), this.skillId, this.skillLevel, value));
							}
						}
					}
				}
				break;
			case 177:
				diseases.put(SecondaryStat.VenomSnake,
						new Pair<>(Integer.valueOf(this.x), Integer.valueOf((int) this.duration)));
				break;
			case 179:
				...
				setDuration(10000L);
				diseases.put(SecondaryStat.PainMark,
						new Pair<>(Integer.valueOf(this.skillLevel), Integer.valueOf((int) this.duration)));
				break;
			case 180:
				diseases.put(SecondaryStat.VampDeath,
						new Pair<>(Integer.valueOf((int) this.duration), Integer.valueOf((int) this.duration)));
				diseases.put(SecondaryStat.VampDeathSummon,
						new Pair<>(Integer.valueOf((int) this.duration), Integer.valueOf((int) this.duration)));
				break;
			case 181:
				allchr = true;
			...
			case 183:
				...
				break;
			case 184:
				diseases.put(SecondaryStat.ReturnTeleport,
						new Pair<>(Integer.valueOf(1), Integer.valueOf((int) this.duration)));
				break;
			case 186:
				if (this.skillLevel == 1 || this.skillLevel == 4) {
					...
					for (MapleCharacter chr5 : players) {
						MapleMist mapleMist = new MapleMist(
								calculateBoundingBox(new Point((chr5.getPosition()).x, (monster.getPosition()).y),
										chr5.isFacingLeft()),
								monster, this, (int) getDuration());
				...
				if (this.skillLevel == 2) {
					MapleMist mapleMist = new MapleMist(
							calculateBoundingBox(monster.getPosition(), monster.isFacingLeft()), monster, this,
							(int) getDuration());
				...
				if (this.skillLevel == 3) {
					MapleMist mapleMist = new MapleMist(new Rectangle(-1870, 133, 1150, 463), monster, this,
							(int) getDuration());
				...
				if (this.skillLevel == 5) {
					...
					MapleMist mapleMist = new MapleMist(new Rectangle(-1695, -357, 365, 463), monster, this,
							(int) getDuration());
				...
				if (this.skillLevel == 11) {
					...
					server.Timer.MapTimer.getInstance().schedule(() -> {
						MapleMist DustMist = new MapleMist(new Rectangle(-664, -940, 1387, 803), monster, this,
								(int) getDuration());
				...
				if (player != null && this.lt != null && this.rb != null) {
					MapleMist mapleMist = new MapleMist(
							calculateBoundingBox(player.getTruePosition(), player.isFacingLeft()), monster, this,
							(int) this.duration);
					if (mapleMist != null) {
			...
			case 188:
				...
				diseases.put(SecondaryStat.Stance,
						new Pair<>(Integer.valueOf(100), Integer.valueOf((int) this.duration)));
				diseases.put(SecondaryStat.Attract,
						new Pair<>(Integer.valueOf(((monster.getPosition()).x < (player.getPosition()).x) ? 1 : 2),
								Integer.valueOf((int) this.duration)));
				...
				break;
			case 189:
				diseases.put(SecondaryStat.CapDebuff,
						new Pair<>(Integer.valueOf(100), Integer.valueOf((int) this.duration)));
				break;
			case 190:
				diseases.put(SecondaryStat.CapDebuff,
						new Pair<>(Integer.valueOf(200), Integer.valueOf((int) this.duration)));
				break;
			case 191:
			...
			case 201:
				...
				if ((this.skillId == 201 && this.skillLevel == 40) || (this.skillId == 201 && this.skillLevel == 237)
						|| this.skillLevel == 162) {
					return;
				}
				if (this.skillId == 200) {
					if (this.skillLevel == 228) {
						monster.getMap().broadcastMessage(
								MobPacket.getSmartNotice(monster.getId(), 0, 2, 8, "시그너스가 상대의 행동에 분노를 느낍니다."));
						monster.getMap().broadcastMessage(CWvsContext.getTopMsg("시그너스가 상대의 행동에 분노를 느낍니다."));
					} else if (this.skillLevel == 223) {
						monster.getMap().broadcastMessage(
								MobPacket.getSmartNotice(monster.getId(), 0, 2, 2, "시그너스가 자신의 기사단을 위해 신수를 소환합니다."));
				...
				if (this.skillId == 201 && this.skillLevel == 211) {
					monster.getMap().broadcastMessage(MobPacket.getSmartNotice(monster.getId(), 1, 5, 0,
							monster.getStats().getName() + "이 적들의 잠재능력에 위협을 느껴 무효화 된 틈을타 부하 몬스터를 소환합니다."));
				} else if ((this.skillId == 200 && this.skillLevel == 212)
						|| (this.skillId == 201 && this.skillLevel == 210)) {
					monster.getMap().broadcastMessage(MobPacket.getSmartNotice(monster.getId(), 0, 5, 1,
							monster.getStats().getName() + "이 주변에 체력을 흡수할 몬스터가 없어 새롭게 소환을 시도합니다."));
				...
				for (Integer mobId : getSummons()) {
					...
					try {
						toSpawn = MapleLifeFactory
								.getMonster(GameConstants.getCustomSpawnID(monster.getId(), mobId.intValue()));
					} catch (RuntimeException e) {
						continue;
					}
					if (toSpawn == null || toSpawn.getStats().getName().contains("악몽의 골렘")
							|| toSpawn.getStats().getName().equals("악몽의 골렘")) {
						continue;
					}
					...
					if (getAfterDead() > 0) {
						...
						if ((toSpawn.getId() >= 8920000 && toSpawn.getId() <= 8920003)
								|| (toSpawn.getId() >= 8920100 && toSpawn.getId() <= 8920103)) {
							if (toSpawn.getId() == 8920000 || toSpawn.getId() == 8920100) {
								...
								player.getMap().broadcastMessage(CField.removeMapEffect());
								player.getMap()
										.broadcastMessage(CField.startMapEffect("내 고통을 느끼게 해줄게요.", 5120102, true));
							}
						}
					...
					case 8920005:
					case 8920105:
						toSpawn.setLastSkillUsed(MobSkillFactory.getMobSkill(188, 1), System.currentTimeMillis(),
								5000L);
					...
					case 8510100:
						if (Math.ceil(Math.random() * 5.0D) == 1.0D) {
							ypos = 78;
							xpos = (int) (0.0D + Math.ceil(Math.random() * 5.0D))
									+ ((Math.ceil(Math.random() * 2.0D) == 1.0D) ? 180 : 0);
							break;
						}
						xpos = (int) (monster.getTruePosition().getX() + Math.ceil(Math.random() * 1000.0D) - 500.0D);
						break;
					...
					case 8880408:
						monster.getMap()
								.broadcastMessage(CField.enforceMSG("힐라가 죽음의 밑바닥에서 스우의 사령을 끌어올리는 소리가 들린다.", 254, 6000));
						break;
					case 8880409:
						monster.getMap().broadcastMessage(
								CField.enforceMSG("힐라가 죽음의 밑바닥에서 데미안의 사령을 끌어올리는 소리가 들린다.", 254, 6000));
						break;
					}
					...
					int objectId = monster.getMap().spawnMonsterWithEffect(toSpawn,
							(effect > 0) ? -1 : ((getSpawnEffect() == 77) ? 1 : getSpawnEffect()),
							flying ? new Point(xpos, ypos)
				...
				break;
			case 203:
				target = ChannelServer.getInstance(player.getClient().getChannel()).getMapFactory()
						.getMap(monster.getMap().getId() + 10);
				target.resetFully();
				target.spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(monster.getId() + 1),
						new Point(Randomizer.rand(-1026, 690), 245));
				...
				break;
			case 211:
				mist3 = new MapleMist(calculateBoundingBox(monster.getPosition(), monster.isFacingLeft()), monster,
						this, 30000);
				...
				for (z = 1; z <= 4; z++) {
					Timer.MobTimer.getInstance().schedule(() -> {
						...
						for (int iz = 0; iz < 3; iz++) {
							Obstacle ob = new Obstacle((this.skillLevel == 2) ? 58 : 59,
									new Point((monster.getPosition()).x, -505),
									new Point((monster.getPosition()).x + obsplusx[iz],
											(this.skillLevel == 2) ? 16 : 17),
									50, 90, 539, 105, 1, angle[iz], unk[iz]);
							obs2.add(ob);
						}
					...
					server.Timer.MobTimer.getInstance().schedule(() -> {
						...
						for (int iz = 0; iz < 5; iz++) {
							Obstacle ob = new Obstacle((this.skillLevel == 2) ? 58 : 59,
									...
									new Point((monster.getPosition()).x + obsplusx[iz],
											(this.skillLevel == 2) ? 16 : 17),
									50, 90, 964, 76, 1, angle[iz], unk[iz]);
							obs2.add(ob);
						}
						player.getMap().broadcastMessage(MobPacket.createObstacle(monster, obs2, (byte) 4));
					}, (z * 2000));
			...
			case 220:
				...
				break;
			case 221:
				diseases.put(SecondaryStat.Seal,
						new Pair<>(Integer.valueOf(this.x), Integer.valueOf((int) this.duration)));
				break;
			case 223:
				if (!monster.isBuffed(MonsterStatus.MS_Laser)) {
					...
					monster.setLastSkillUsed(this, System.currentTimeMillis(), 10000L);
					stats.add(new Pair<>(MonsterStatus.MS_Laser,
							new MonsterStatusEffect(this.skillId, (int) this.duration)));
					break;
				}
			...
			case 226:
				...
				break;
			case 227:
				Swoomist = new MapleMist(calculateBoundingBox(monster.getTruePosition(), true), monster, this,
						(int) this.duration);
				if (Swoomist != null) {
			...
			case 228:
				...
				case 6:
					...
					monster.getMap().broadcastMessage(MobPacket.LaserHandler(monster.getObjectId(),
							monster.getEnergycount(), monster.getEnergyspeed(), monster.isEnergyleft() ? 0 : 1));
					Timer.MobTimer.getInstance().schedule(() -> {
						monster.setSkillForbid(false);
						monster.setEnergyspeed(1);
				...
				case 7:
					...
					Timer.MobTimer.getInstance().schedule(() -> {
						monster.setEnergycount(
								monster.getEnergycount() + (monster.isEnergyleft() ? -(9 * monster.getEnergyspeed())
										: (9 * monster.getEnergyspeed())));
						monster.setEnergyspeed(1);
					}, 30000L);
					break;
				}
			case 230:
				if (monster != null && !monster.getRectangles().isEmpty()) {
					...
					for (MapleCharacter chrs : monster.getMap().getAllChracater()) {
						if (chrs != null && chrs.isAlive() && rect.x - 50 < (chrs.getPosition()).x
								&& rect.x + 150 > (chrs.getPosition()).x && -16 >= (chrs.getPosition()).y
								&& -316 <= (chrs.getPosition()).y) {
			...
			case 234:
				...
				diseases.put(SecondaryStat.Contagion,
						new Pair<>(Integer.valueOf(this.x), Integer.valueOf((int) this.duration)));
				break;
			case 235: {
				break;
			...
			case 237: {
				...
				player.getClient().getSession()
						.writeAndFlush((Object) CWvsContext.BuffPacket.giveDisease(dds, this, player));
				...
				player.getClient().getSession()
						.writeAndFlush((Object) SLFCGPacket.playSE("SoundEff/BossDemian/incStigma"));
				if (player.Stigma == 7 && player.getBuffedEffect(SecondaryStat.NotDamaged) == null
						&& player.getBuffedEffect(SecondaryStat.IndieNotDamaged) == null) {
					player.Stigma = 0;
					player.getMap().setStigmaDeath(player.getMap().getStigmaDeath() + 1);
					player.playerIGDead();
					player.getMap()
							.broadcastMessage(MobPacket.CorruptionChange((byte) 0, player.getMap().getStigmaDeath()));
					...
					if (monster.getId() != 8880111 && monster.getId() != 8880101) {
						player.getMap().broadcastMessage(CField.enforceMSG("烙印完成，戴米安逐漸墮入黑暗.", 216, 30000000));
					}
					Timer.MobTimer.getInstance().schedule(() -> {
						if (monster.isAlive()) {
					...
					player.getClient().getSession()
							.writeAndFlush((Object) CWvsContext.BuffPacket.cancelBuff(dds, player));
					player.getMap().broadcastMessage(player, CWvsContext.BuffPacket.cancelForeignBuff(player, dds),
							false);
					if (player.getMap().getStigmaDeath() >= 7 && monster.getId() != 8880111
							&& monster.getId() != 8880101) {
			...
			case 238:
				...
				case 2:
				case 3:
					player.getMap().broadcastMessage(MobPacket.BossLucid.doFlowerTrapSkill(this.skillLevel,
							Randomizer.nextInt(3), 1000, 48, Randomizer.nextBoolean()));
					break;
				case 4:
				case 10: {
					ArrayList<FairyDust> arrayList = new ArrayList<FairyDust>() {
						{
							...
							for (int i = 0; i < max; ++i) {
								...
								this.add(new FairyDust(Randomizer.nextInt(3), u, v + Randomizer.nextInt(v2),
										x + Randomizer.rand(s, s2)));
							}
						}
					};
					monster.setLastSkillUsed(this, System.currentTimeMillis(), Randomizer.rand(10, 20) * 1000);
					player.getMap().broadcastMessage(
							MobPacket.BossLucid.doFairyDustSkill(this.skillLevel, (List<FairyDust>) arrayList));
				...
				case 6:
					player.getClient().getSession()
							.writeAndFlush(MobPacket.BossLucid.doForcedTeleportSkill(Randomizer.nextInt(8)));
					break;
				case 7:
					isLeft = Randomizer.isSuccess(50);
					if (monster.getMap().getId() == 450004150 || monster.getMap().getId() == 450004450
							|| monster.getMap().getId() == 450003840) {
						...
						player.getMap().broadcastMessage(
								MobPacket.BossLucid.createDragon(2, posX, posY, createPosX, createPosY, isLeft));
					}
				...
				case 9:
					...
					player.getMap().broadcastMessage(
							MobPacket.BossLucid.setStainedGlassOnOff(false, FieldLucid.STAINED_GLASS));
					player.getMap().broadcastMessage(
							MobPacket.BossLucid.setButterflyAction(Butterfly.Mode.MOVE, new int[] { 600, -500 }));
					...
					player.getMap()
							.broadcastMessage(MobPacket.BossLucid.doSpiralShoot(4, 390, 225, 13, 3, 3500, 10, 10, 1));
					player.getMap()
							.broadcastMessage(MobPacket.BossLucid.doSpiralShoot(5, 10, 15, 20, 40, 4000, 13, 0, 0));
					...
					Timer.MobTimer.getInstance().schedule(() -> {
						...
						player.getMap().broadcastMessage(
								MobPacket.BossLucid.setStainedGlassOnOff(true, FieldLucid.STAINED_GLASS));
			...
			case 241:
				...
				case 2:
					...
					stats.add(new Pair<>(MonsterStatus.MS_PopulatusTimer,
							new MonsterStatusEffect(this.skillId, (int) this.duration, 1L)));
				...
				case 4:
					...
					if (chr3 != null) {
						chr3.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr3,
								this.skillLevel, this.skillId, 73, 0, 0, (byte) 0, true, null, null, null));
						chr3.getMap().broadcastMessage(chr3, CField.EffectPacket.showEffect(chr3, this.skillLevel,
								this.skillId, 73, 0, 0, (byte) 0, false, null, null, null), false);
						Timer.MobTimer.getInstance().schedule(() -> {
							chr3.getClient().send(CField.onUserTeleport(chr3, Randomizer.rand(-880, 1030), 100));
							chr3.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr3,
									this.skillLevel, this.skillId, 73, 1, 0, (byte) 0, true, null, null, null));
							chr3.getMap().broadcastMessage(chr3, CField.EffectPacket.showEffect(chr3, this.skillLevel,
									this.skillId, 73, 1, 0, (byte) 0, false, null, null, null), false);
						}, 1000L);
					}
					break;
				case 5:
					diseases.put(SecondaryStat.PapulBomb,
							new Pair<>(Integer.valueOf(this.x), Integer.valueOf((int) this.duration)));
					break;
				case 7:
					ms = this;
					...
					stats.add(new Pair<>(MonsterStatus.MS_PopulatusInvincible,
							new MonsterStatusEffect(this.skillId, 610000, this.x)));
					...
					monster.getMap().broadcastMessage(
							CField.startMapEffect("파풀라투스가 시간 이동을 할 수 없도록 차원의 균열을 봉인해야 합니다.", 5120177, true));
					...
					Timer.MapTimer.getInstance().schedule(new Runnable() {
						public void run() {
							...
							if (hppercent > 0) {
								...
								monster.getMap()
										.broadcastMessage(MobPacket.damageMonster(monster.getObjectId(), hp, true));
							}
							for (MapleReactor reactor : monster.getMap().getAllReactor()) {
								if (reactor != null && reactor.getReactorId() == 2208011) {
							...
							monster.getMap().broadcastMessage(
									MobPacket.BossPapuLatus.PapulLatusTime(monster.getPatten() * 1000, false, false));
							monster.cancelStatus(MonsterStatus.MS_PopulatusInvincible,
									monster.getBuff(MonsterStatus.MS_PopulatusInvincible));
						}
					}, 60000L);
					break;
				case 8:
					diseases.put(SecondaryStat.Stun,
							new Pair<>(Integer.valueOf(1), Integer.valueOf((int) this.duration)));
					break;
				}
				break;
			case 242:
				...
				case 3:
					...
					player.getMap()
							.broadcastMessage(MobPacket.BossWill.WillSpiderAttack(id, skillId, skillLevel, 0, idx));
					break;
				case 4: { // 1페이즈 : 주시하는 눈
					monster.getMap().broadcastMessage(
							CField.enforceMSG("눈동자를 공격해서 다른 공간에 달빛을 흘려보내세요. 어서 달빛 보호막을 생성해야 해요.", 245, 28000));
					...
					for (MapleCharacter chr : monster.getMap().getAllCharactersThreadsafe()) {
						chr.getClient().getSession().writeAndFlush(MobPacket.BossWill.WillSpiderAttack(monster.getId(),
								skillId, skillLevel, Randomizer.rand(1, 2), null));
					}

					monster.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8880305),
							new Point(0, -2020));
					...
					server.Timer.MobTimer.getInstance().schedule(() -> {
						...
						for (MapleMist mi : monster.getMap().getAllMistsThreadsafe()) {
							if (mi.getMobSkill() != null && mi.getMobSkill().getSkillId() == skillId
									&& mi.getMobSkill().getSkillLevel() == skillLevel) {
								mists.add(mi);
							} else if (mi.getSource() != null && (mi.getSource().getSourceId() == 400031039
									|| mi.getSource().getSourceId() == 400031040)) {
								mists.add(mi);
							}
						}
						...
						for (MapleCharacter chr : monster.getMap().getAllCharactersThreadsafe()) {
							...
							chr.getClient().getSession()
									.writeAndFlush(CField.EffectPacket.showWillEffect(chr, 1, skillId, skillLevel));
							chr.getClient().getSession().writeAndFlush(
									MobPacket.BossWill.WillSpiderAttack(monster.getId(), skillId, skillLevel, 0, null));
						...
						for (MapleCharacter chr : targets) {
							if (chr.getBuffedEffect(SecondaryStat.HolyMagicShell) != null) {
								if (chr.getHolyMagicShell() > 0) {
									...
									Map<SecondaryStat, Pair<Integer, Integer>> statups = new HashMap<>();
									statups.put(SecondaryStat.HolyMagicShell, new Pair<>((int) chr.getHolyMagicShell(),
								...
								int windWall = (int) Math.max(0, chr.getBuffedValue(SecondaryStat.WindWall)
								...
								if (windWall > 0) {
									...
									statups.put(SecondaryStat.WindWall,
											new Pair<>(windWall, (int) chr.getBuffLimit(400031030)));
									chr.getClient().getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups,
											chr.getBuffedEffect(SecondaryStat.WindWall), chr));
								} else {
									chr.cancelEffectFromBuffStat(SecondaryStat.WindWall);
								}
							} else if (chr.getBuffedEffect(SecondaryStat.TrueSniping) == null
						...
						if (monster.isUseSpecialSkill()) {
							...
							monster.getMap().broadcastMessage(MobPacket.BossWill.setWillHp(monster.getWillHplist(),
									monster.getMap(), monster.getId(), monster.getId() + 3, monster.getId() + 4));
				...
				case 5: {// mush
					...
					monster.getMap().broadcastMessage(MobPacket.BossWill.WillSpiderAttack(monster.getId(), this.skillId,
							this.skillLevel, type, null));
					Timer.MobTimer.getInstance().schedule(() -> {
						...
						for (MapleMist mi : monster.getMap().getAllMistsThreadsafe()) {
							if (mi.getSource() == null || mi.getSource().getSourceId() != 400031039
									&& mi.getSource().getSourceId() != 400031040) {
								continue;
							}
							mists.add(mi);
						...
						for (MapleCharacter chr : monster.getMap().getAllCharactersThreadsafe()) {
							boolean add = true;
							chr.getClient().getSession().writeAndFlush(
									(Object) CField.EffectPacket.showWillEffect(chr, 1, this.skillId, this.skillLevel));
						...
						for (MapleCharacter chr : targets) {
							if (chr.getBuffedEffect(SecondaryStat.HolyMagicShell) != null) {
								if (chr.getHolyMagicShell() > 1) {
									...
									statups.put(SecondaryStat.HolyMagicShell, new Pair<>(
											Integer.valueOf(chr.getHolyMagicShell()),
											(int) chr.getBuffLimit(chr.getBuffSource(SecondaryStat.HolyMagicShell))));
									chr.getClient().getSession().writeAndFlush((Object) CWvsContext.BuffPacket
											.giveBuff(statups, chr.getBuffedEffect(SecondaryStat.HolyMagicShell), chr));
									continue;
								}
								chr.cancelEffectFromBuffStat(SecondaryStat.HolyMagicShell);
								continue;
							...
							if (chr.getBuffedValue(4341052)) {
								chr.getClient().getSession().writeAndFlush((Object) CField.EffectPacket.showEffect(chr,
										1, 0, 36, 0, 0, (byte) 0, true, null, null, null));
								chr.getMap().broadcastMessage(chr, CField.EffectPacket.showEffect(chr, 1, 0, 36, 0, 0,
							...
							if (chr.getBuffedEffect(SecondaryStat.WindWall) != null) {
								...
								if (windWall > 0) {
									...
									statups.put(SecondaryStat.WindWall,
											new Pair<>(windWall, (int) chr.getBuffLimit(400031030)));
									chr.getClient().getSession().writeAndFlush((Object) CWvsContext.BuffPacket
											.giveBuff(statups, chr.getBuffedEffect(SecondaryStat.WindWall), chr));
									continue;
								}
								chr.cancelEffectFromBuffStat(SecondaryStat.WindWall);
								continue;
							}
							if (chr.getBuffedEffect(SecondaryStat.TrueSniping) != null
									|| chr.getBuffedEffect(SecondaryStat.Etherealform) != null
									|| chr.getBuffedEffect(SecondaryStat.IndieNotDamaged) != null
									|| chr.getBuffedEffect(SecondaryStat.NotDamaged) != null) {
								continue;
				...
				case 7: {
					monster.getMap()
							.broadcastMessage(CField.enforceMSG("거짓의 거울은 공격을 반전시켜요. 균열이 나타나면 공격을 마주하세요.", 245, 26000));
					monster.getMap().broadcastMessage(MobPacket.BossWill.willUseSpecial());
					...
					Timer.MobTimer.getInstance().schedule(() -> MobSkillFactory.getMobSkill(242, 14).applyEffect(player,
							monster, skill, isFacingLeft), 5000L);
					break;
				}
				case 8: {
					...
					if (monster.getId() == 8880300 || monster.getId() == 8880340) {
						...
						if (will1 != null) {
							monster.getMap()
									.broadcastMessage(MobPacket.forcedSkillAction(will1.getObjectId(), 3, false));
						}
						if ((will2 = monster.getMap().getMonsterById(monster.getId() + 4)) != null) {
							monster.getMap()
									.broadcastMessage(MobPacket.forcedSkillAction(will2.getObjectId(), 3, false));
						}
					} else {
				...
				case 9:
					...
					player.getMap().broadcastMessage(
							MobPacket.BossWill.posion(player, (int) player.getSkillCustomValue0(24219), 0, 0, 0));
					Timer.MapTimer.getInstance().schedule(() -> {
						if (player.getSkillCustomValue0(24219) > 0L) {
							player.getMap().broadcastMessage(
									MobPacket.BossWill.removePoison(player, (int) player.getSkillCustomValue0(24219)));
					...
					break;
				case 10:
					player.getMap().broadcastMessage(MobPacket.BossWill.WillSpiderAttack(monster.getId(), this.skillId,
							this.skillLevel, 1, null));
					break;
				case 11:
					player.getMap().broadcastMessage(MobPacket.BossWill.WillSpiderAttack(monster.getId(), this.skillId,
							this.skillLevel, 1, null));
				...
				case 12:
					...
					if (monster.getId() == 8880325 || monster.getId() == 8880326) {
						...
						if (player.getMapId() == 450008250 || player.getMapId() == 450008850) {
							...
							if (Randomizer.nextBoolean()) {
								player.getMap().broadcastMessage(
										MobPacket.BossWill.createBulletEyes(new int[] { 0, id, -492, -370 }));
								player.getMap().broadcastMessage(
										MobPacket.BossWill.createBulletEyes(new int[] { 0, id, -8, -370 }));
								player.getMap().broadcastMessage(
										MobPacket.BossWill.createBulletEyes(new int[] { 0, id, 501, -370 }));
							} else {
								player.getMap().broadcastMessage(
										MobPacket.BossWill.createBulletEyes(new int[] { 0, id, -300, -370 }));
								player.getMap().broadcastMessage(
										MobPacket.BossWill.createBulletEyes(new int[] { 0, id, 300, -370 }));
							}
							...
							if (Randomizer.nextBoolean()) {
								player.getMap().broadcastMessage(
										MobPacket.BossWill.createBulletEyes(new int[] { 0, id, -300, -400 }));
							} else {
								player.getMap().broadcastMessage(
										MobPacket.BossWill.createBulletEyes(new int[] { 0, id, -415, -400 }));
								player.getMap().broadcastMessage(
										MobPacket.BossWill.createBulletEyes(new int[] { 0, id, 300, -400 }));
							}
						}
					...
					if (id != 8880341 && id != 8880342 && id != 8880302 && id != 8880301) {
						if (Randomizer.nextBoolean()) {
							player.getMap().broadcastMessage(
									MobPacket.BossWill.createBulletEyes(new int[] { 0, id, -250, -370 }));
							player.getMap().broadcastMessage(
									MobPacket.BossWill.createBulletEyes(new int[] { 0, id, 250, -370 }));
						} else {
							player.getMap().broadcastMessage(
									MobPacket.BossWill.createBulletEyes(new int[] { 0, id, -470, -440 }));
							player.getMap().broadcastMessage(
									MobPacket.BossWill.createBulletEyes(new int[] { 0, id, 470, -440 }));
						}
					}
					if ((monster.getTruePosition()).y < 0) {
						player.getMap().broadcastMessage(MobPacket.BossWill
								.createBulletEyes(new int[] { 1, id, 300, 100, -690, -2634, 695, -2019 }));
						break;
					}
					player.getMap().broadcastMessage(
							MobPacket.BossWill.createBulletEyes(new int[] { 1, id, 300, 100, -690, -455, 695, 500 }));
					break;
				case 13: {
				...
				case 14: {
					...
					Timer.MapTimer.getInstance().schedule(
							() -> this.willSpider(monster.getController(), monster, a == 0 ? 1 : 0, true), 11000L);
					Timer.MapTimer.getInstance().schedule(() -> {
				...
				case 15:
					for (MapleCharacter mapleCharacter : monster.getMap().getAllCharactersThreadsafe()) {
						monster.getMap().broadcastMessage(
								CField.EffectPacket.showWillEffect(mapleCharacter, 0, this.skillId, this.skillLevel));
					}
					Timer.MobTimer.getInstance().schedule(() -> {
						for (MapleCharacter chr : monster.getMap().getAllCharactersThreadsafe()) {
							// chr.getClient().getSession().writeAndFlush(CField.portalTeleport(Randomizer.nextBoolean()
							// ? "ptup" : "ptdown"));
							chr.getClient().getSession().writeAndFlush(MobPacket.BossWill.WillSpiderAttack(
									monster.getId(), this.skillId, this.skillLevel, Randomizer.nextInt(2), null));
						}
					}, 3000L);
					break;
				}
			case 246:
				...
				monster.getMap().broadcastMessage(
						MobPacket.jinHillahBlackHand(monster.getObjectId(), this.skillId, this.skillLevel, datas));
				break;
			case 247:
				...
				monster.getMap().broadcastMessage(CField.JinHillah(4,
						(monster.getController() == null) ? player : monster.getController(), monster.getMap()));
				...
				for (MapleCharacter mapleCharacter : monster.getMap().getAllCharactersThreadsafe()) {
					...
					mapleCharacter.getClient().getSession()
							.writeAndFlush(CField.JinHillah(3, mapleCharacter, monster.getMap()));
				...
				break;
			case 248:
				mapleMist1 = new MapleMist(
						new Rectangle(Randomizer.rand(-400, 300), Randomizer.rand(-400, 100), 400, 400), monster, this,
						8000);
				if (mapleMist1 != null) {
			...
			case 249:
				...
				case 1:
					...
					break;
				case 2:
					diseases.put(SecondaryStat.CurseOfDestruction,
							new Pair<>(Integer.valueOf(10), Integer.valueOf(6000)));
					break;
				}
				if ((player.hasDisease(SecondaryStat.CurseOfCreation) && this.skillLevel == 2)
						|| (player.hasDisease(SecondaryStat.CurseOfDestruction) && this.skillLevel == 1)) {
			...
			case 260:
				...
				case 3:
					...
					for (i1 = 0; i1 < n; i1++) {
						...
						Obstacle ob = new Obstacle(bool1 ? 81 : 82, new Point(xstart, i6), new Point(xgoal, 29), 110,
								30, 0, 5, 2, 260, 1);
						ob.setVperSec(1000);
						monster.getMap().CreateObstacle2(null, ob, (byte) 5);
						player.getClient().getSession()
								.writeAndFlush(MobPacket.TeleportMonster(monster, true, 9, monster.getTruePosition()));
					}
					break;
				case 5:
					obs = new ArrayList<>();
					...
					for (i5 = 0; i5 < size_; i5++) {
						...
						y += 80;
						Obstacle ob = new Obstacle(i6, new Point(i3, -y), new Point(i3, 305), 80, 40, 0,
								Randomizer.rand(2, 3), Randomizer.rand(2, 3), y + 305, 0);
						obs.add(ob);
					}
			...
			case 262:
				...
				case 1:
					if (player.getSkillCustomValue(26201) == null) {
						...
						player.getClient().getSession()
								.writeAndFlush(SLFCGPacket.MakeBlind(new int[] { 1, 255, 240, 240, 240, 400, 0 }));
						Timer.BuffTimer.getInstance()
								.schedule(() -> player.getClient().getSession()
										.writeAndFlush(SLFCGPacket.MakeBlind(new int[] { 0, 0, 0, 0, 0, 300, 0 })),
										1000L);
					}
					break;
				}
				break;
			case 263:
				monster.getMap().broadcastMessage(MobPacket.BossSeren.SerenSpawnOtherMist(monster.getObjectId(),
						isFacingLeft, monster.getPosition()));
				break;
			case 264:
				monster.getMap().broadcastMessage(MobPacket.BossSeren.SerenMobLazer(monster, this.skillLevel,
						(this.skillLevel == 1) ? 1800 : 1500));
				break;
			case 265:
				...
				for (k = 0; k < 8; k++) {
					...
					monster.getMap().spawnMonsterWithEffect(MapleLifeFactory.getMonster(8870106), 43,
							new Point(posx, (monster.getPosition()).y));
				}
				break;
			case 266: // 슬라임 소환
				for (int slime = 0; slime < 3; slime++) {
					monster.getMap().spawnMonsterOnGroundBelow(
							MapleLifeFactory.getMonster(Randomizer.rand(8880715, 8880719)), monster.getPosition());
				}
				break;
			}
	...
	public void setMobSkillDelay(MapleCharacter chr, MapleMonster monster, int skillAfter, short option,
		...
		try {
			...
			case 145: {
				...
				case 12: {
					...
					chr.getMap().broadcastMessage(
							MobPacket.SpeakingMonster(monster, monster.getId() == 8870100 ? 3 : 2, 0));
				}
				}
				chr.getClient().getSession().writeAndFlush((Object) MobPacket.MobSkillDelay(monster.getObjectId(),
						this.skillId, this.skillLevel, skillAfter, option, skillRectInfo));
				break;
			}
			case 213: {
				switch (this.skillLevel) {
				case 10: {
					...
					monster.getMap()
							.broadcastMessage(CField.enforceMSG("더스크 중심으로 주변의 에너지가 빠른 속도로 빨려 들어갑니다.", 249, 2000));
					Timer.MapTimer.getInstance().schedule(() -> {
			...
			case 215: {
				chr.getMap().broadcastMessage(MobPacket.MobSkillDelay(monster.getObjectId(), this.skillId,
						this.skillLevel, skillAfter, option, skillRectInfo));
				break;
			}
			case 217: {
				switch (this.skillLevel) {
				...
				case 16: {
					...
					chr.getMap()
							.broadcastMessage(CField.getUpdateEnvironment(chr.getMap().getNodez().getEnvironments()));
					for (int i = 0; i < 7; ++i) {
					...
					Timer.MapTimer.getInstance().schedule(() -> {
						...
						if (monster.getBuff(MonsterStatus.MS_Freeze) == null) {
							...
							if (mistskillid != 0 && mistskilllv != 0
									&& (mist = new MapleMist(new Rectangle(-684, -29, 1365, 15), monster,
											MobSkillFactory.getMobSkill(mistskillid, mistskilllv),
											(int) MobSkillFactory.getMobSkill(mistskillid, mistskilllv)
													.getDuration())) != null
									&& monster != null && monster.getMap() != null && monster.isAlive()) {
								monster.getMap().spawnMist(mist, false);
							}
						}
					...
					Timer.MapTimer.getInstance().schedule(() -> {
						...
						chr.getMap().broadcastMessage(
								CField.getUpdateEnvironment(chr.getMap().getNodez().getEnvironments()));
					}, 12800L);
					break block1;
				}
				case 7:
				...
				case 23: {
					chr.getMap().broadcastMessage(MobPacket.MobSkillDelay(monster.getObjectId(), this.skillId,
							this.skillLevel, skillAfter, option, skillRectInfo));
					Timer.MobTimer.getInstance()
							.schedule(() -> chr.getMap().spawnEnergySphere(monster.getObjectId(), this.skillLevel,
									new MapleEnergySphere(monster.getTruePosition().x, 10, 20000, 0, true, true)),
									3000L);
					break block1;
				}
				case 21: {
			...
			case 230: {
				...
				chr.getMap().broadcastMessage(MobPacket.MobSkillDelay(monster.getObjectId(), this.skillId,
						this.skillLevel, 960, option, skillRectInfo));
				break;
			}
			case 238: {
				monster.getMap().getLucidDream().clear();
				ArrayList<Point> pos = new ArrayList<Point>();
				if (monster.getMap().getId() == 450003920 || monster.getMap().getId() == 450004250
						|| monster.getMap().getId() == 450004550) {
			...
			case 241: {
				chr.getMap().broadcastMessage(MobPacket.MobSkillDelay(monster.getObjectId(), this.skillId,
						this.skillLevel, 1, option, skillRectInfo));
				break;
			}
			case 140:
			case 141: {
			...
			chr.dropMessageGM(6, this.skillId + " / " + this.skillLevel + " 發送的延遲數据包 : " + skillAfter);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void willSpider(MapleCharacter player, MapleMonster monster, int type, boolean solo) {
		...
		if (spider.size() > 0) {
			player.getMap()
					.broadcastMessage(MobPacket.BossWill.WillSpiderAttackPaten(monster.getObjectId(), type, spider));
		}
	}
}
