...
public enum ItemLoader {
		...
		try {
			...
			while (rs.next()) {
					...
					if (!login) {
						...
						if (this.table_enchant != null && type.getType() != 6) {
							ps1 = con.prepareStatement(
									"SELECT * FROM `" + this.table_enchant + "` WHERE inventoryitemid = ?");
				...
				if (item.getItemId() == 4001886) {
					item.setReward(new BossReward(rs.getInt("objectid"), rs.getInt("mobid"), rs.getInt("partyid"),
							rs.getInt("price")));
				}
	...
	public void saveItems(List<Item> items, Connection con, int id, MapleInventoryType type, boolean dc) {
		...
		try {
			...
			if (this.value <= 1 && type.getType() == 6) {
				...
				pse = con.prepareStatement("INSERT INTO " + this.table_equip
						+ " VALUES (DEFAULT, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
			}
			...
			while (iter.hasNext()) {
				...
				if (item.getItemId() / 1000000 == 1) {
					...
					if (this.table_enchant != null && type.getType() != 6) {
						PreparedStatement ps2 = con.prepareStatement("INSERT INTO `" + this.table_enchant
								+ "` VALUES (DEFAULT, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
						ps2.setLong(1, iid);
	...
	public static boolean isCanMadeItem(Equip equip) {
		...
		if (equip.getFire() > 0L && (GameConstants.isRing(equip.getItemId()) || equip.getItemId() / 1000 == 1092
				|| equip.getItemId() / 1000 == 1342 || equip.getItemId() / 1000 == 1713
				|| equip.getItemId() / 1000 == 1712 || equip.getItemId() / 1000 == 1152
				|| equip.getItemId() / 1000 == 1143 || equip.getItemId() / 1000 == 1672
				|| GameConstants.isSecondaryWeapon(equip.getItemId()) || equip.getItemId() / 1000 == 1190
				|| equip.getItemId() / 1000 == 1182 || equip.getItemId() / 1000 == 1662
		...
		if (equip.getItemId() == 1182285 || equip.getItemId() == 1122430
				|| ii.getEquipStats(equip.getItemId()).get("undecomposable") != null
				|| ii.getEquipStats(equip.getItemId()).get("unsyntesizable") != null) {
			return true;
		}
		EquipmentEnchant.checkEquipmentStats(null, equip);
	...
	public void saveCSItems(List<Pair<Item, MapleInventoryType>> items, Connection con, int id, MapleClient c)
			throws SQLException {
		StringBuilder query = new StringBuilder();
		query.append("DELETE FROM `");
		query.append(this.table);
			...
			while (iter.hasNext()) {
				...
				if (item != null) {
					...
					if (mit.equals(MapleInventoryType.EQUIP) || mit.equals(MapleInventoryType.EQUIPPED)
							|| mit.equals(MapleInventoryType.CODY)) {
						Equip equip = (Equip) item;
						if (!isCanMadeItem(equip)) {
							continue;
						}
	...
	public Map<Long, Pair<Item, MapleInventoryType>> loadCSItems(boolean login, int id) throws SQLException {
		...
		query.append("` LEFT JOIN `");
		query.append(this.table_equip);
		query.append("` USING " + ((getValue() != 7) ? "(`inventoryitemid`)" : "(`auctionid`)")
				+ ((getValue() != 7) ? " WHERE `type` = ?" : ""));
		if (getValue() != 7) {
			query.append(" AND `");
			query.append(this.arg);
			query.append("` = ?");
