...
public class ReactorActionManager extends AbstractPlayerInteraction {
	...
	public void dropItems(boolean meso, int mesoChance, int minMeso, int maxMeso, int minItems, boolean suc) {
		...
		for (ReactorDropEntry d : items) {
			if (d.itemId == 0) {
				...
				int mesoDrop = Randomizer.nextInt(range)
				...
				if (GameConstants.getInventoryType(d.itemId) != MapleInventoryType.EQUIP) {
					drop = new Item(d.itemId, (short) 0,
							(short) ((d.Maximum != 1) ? Randomizer.rand(d.Minimum, d.Maximum) : 1), 0);
				} else {
					drop = ii.getEquipById(d.itemId);
				}
				drop.setGMLog(
						"Dropped from reactor " + this.reactor.getReactorId() + " on map " + getPlayer().getMapId());
				try {
	...
	public void doHarvest() {
		...
		if (getPlayer().getFatigue() >= 200 || (getPlayer().getStat()).harvestingTool <= 0
				|| getReactor().getTruePosition().distanceSq(getPlayer().getTruePosition()) > 10000.0D) {
