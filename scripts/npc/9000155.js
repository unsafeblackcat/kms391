...
function NormalUnboxing() {
    ...
    for (i = 0; i < NormalReward.length; i++) {
        ...
        if (tchance <= NormalReward[i]['chance']) {
            ...
            ab++;
            boxmsg += "#b#i" + NormalReward[i]['item'] + "##z" + NormalReward[i]['item'] + "# " + qb + "恭喜#k" + enter;
        }
    }
    if (ab == 0) {
        boxmsg += "很可惜落空了！";
...
function AdvancedUnboxing() {
    ...
    for (i = 0; i < AdvancedReward.length; i++) {
        ...
        if (tchance <= AdvancedReward[i]['chance']) {
            ...
            ab++;
            boxmsg += "#b#i" + AdvancedReward[i]['item'] + "##z" + AdvancedReward[i]['item'] + "# " + qb + "恭喜#k 中獎!" + enter;
            if (AdvancedReward[i]['item'] == 2431156 || AdvancedReward[i]['item'] == 2431157 || AdvancedReward[i]['item'] == 2633620 || AdvancedReward[i]['item'] == 1113997 ||
                ...
                for (ac = 0; ac < ab; ++ac) {
                    if (AdvancedReward[i]['item'] == 1122998) {
                        World.Broadcast.broadcastMessage(CField.getGameMessage(8, cm.getPlayer().getName() + " 您在轉盤使用券中獲得了稀有物品."));
                    } else {
                        World.Broadcast.broadcastMessage(CField.getGameMessage(8, cm.getPlayer().getName() + " 님在這個轉盤使用券上 " + cm.getItemName(AdvancedReward[i]['item']) + "獲得了."));
                    }
                    if (ab >= 2) {
                        return;
                    }
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        var chat = "#fs15#"
        chat += "#h0#考驗你的運氣！" + enter
        chat += "挑戰普通使用券或高級使用券！" + enter + enter
        chat += "#L0#抽取普通使用券#l" + enter
        chat += "#L1#抽取高級使用券#l"
        cm.sendSimple(chat);
    } else if (status == 1) {
        switch (selection) {
            case 0:
                if (!cm.haveItem(4001780, 1)) {
                    cm.sendOkS("#i4001780# #z4001780# 你沒有相應的道具..", 0x24);
                    cm.dispose();
                    break;
                } else {
            ...
            case 1:
                if (!cm.haveItem(4319998, 1)) {
                    cm.sendOkS("#i4319998# #z4319998# 你沒有相應的道具..", 0x24);
                    cm.dispose();
                    break;
                } else {
