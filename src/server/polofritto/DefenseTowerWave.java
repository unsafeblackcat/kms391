		...
		Timer.EventTimer.getInstance().schedule(new Runnable() {
			...
			public void run() {// 1
				c.getSession().writeAndFlush(
						(Object) CField.environmentChange("defense/wave/" + DefenseTowerWave.this.wave, 16));
				c.getSession().writeAndFlush((Object) CField.environmentChange("killing/first/start", 16));
				int time = 0;
				if (c.getPlayer().getMapId() == 993000100) {
					Iterator iterator = ((List) DefenseTowerWave.this.waveData.get(DefenseTowerWave.this.wave - 1))
							.iterator();
