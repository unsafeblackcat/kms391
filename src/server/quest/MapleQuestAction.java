...
public class MapleQuestAction implements Serializable {
	...
	public MapleQuestAction(MapleQuestActionType type, ResultSet rse, MapleQuest quest, PreparedStatement pss,
		...
		case item:
			...
			rs = psi.executeQuery();
			while (rs.next()) {
				this.items.add(new QuestItem(rs.getInt("itemid"), rs.getInt("count"), rs.getInt("period"),
		...
		case skill:
			...
			rs = pss.executeQuery();
			while (rs.next()) {
				this.skill.add(new Triple<>(Integer.valueOf(rs.getInt("skillid")),
						Integer.valueOf(rs.getInt("skillLevel")), Integer.valueOf(rs.getInt("masterLevel"))));
			}
			rs.close();
			break;
		}
				...
				if (item.itemid == itemid) {
					if (!c.haveItem(item.itemid, item.count, true, false)) {
						MapleInventoryManipulator.addById(c.getClient(), item.itemid, (short) item.count,
								"Obtained from quest (Restored) " + this.quest.getId() + " on "
	...
	public void runStart(MapleCharacter c, Integer extSelection) {
		...
		case exp:
			...
			c.gainExp(
					(this.intStore * GameConstants.getExpRate_Quest(c.getLevel()) * (c.getStat()).questBonus
							* (c.getTrait(MapleTrait.MapleTraitType.sense).getLevel() * 3 / 10 + 100) / 100),
					true, true, true);
			break;
		case item:
			props = new HashMap<>();
			...
			for (QuestItem item : this.items) {
				...
				int id = item.itemid;
				if (item.prop != -2
						&& ((item.prop == -1) ? (extSelection != null && extSelection.intValue() != extNum++)
								: (id != selection))) {
					continue;
				}
				...
				MapleInventoryManipulator.addById(c.getClient(), id, count, "", null, period,
						"퀘스트 보상을 통해 얻은 아이템 입니다. // 퀘스트 번호 : " + this.quest.getId() + " "
								+ FileoutputUtil.CurrentReadable_Date(),
						false);
				c.getClient().getSession().writeAndFlush(CField.EffectPacket.showCharmEffect(c, id, 1, true, ""));
			}
		...
		case quest:
			for (Pair<Integer, Integer> q : this.state) {
				c.updateQuest(new MapleQuestStatus(MapleQuest.getInstance(((Integer) q.left).intValue()),
						((Integer) q.right).intValue()));
			}
			break;
		case skill:
			sa = new HashMap<>();
		...
		case item:
			...
			for (QuestItem item : this.items) {
				...
				if (MapleItemInformationProvider.getInstance().isPickupRestricted(id)
					...
					c.dropMessage(1,
							"You have this item already: " + MapleItemInformationProvider.getInstance().getName(id));
					return false;
				}
