...
function action(mode, type, selection, selection2) {
    ...
    if (status == 0) {
        var change = "#fs16#\r\n";
        change += "                           " + 보라 + " #k#fc0xFF3B00DB#關於搭配#k " + 보라 + "\r\n#fc0xFF7112FF#         ";
        change += "#L9# 改變髮型#l";
        change += "#L10# #更改整形#l";
        change += "#L999# 緩存掃描#l\r\n\r\n\r\n";
        change += "                           " + 보라 + " #k#fc0xFFC900A7#染色相關#k " + 보라 + "\r\n#fc0xFFF361A6#         ";
        change += "#L0# 皮膚變更#l";
        change += "#L7# 混合染色#l";
        change += "#L4# 更改鏡頭#l\r\n\r\n\r\n";
        change += "                           " + 보라 + " #k#fc0xFF6E2FC7#其他相關#k " + 보라 + "\r\n#fc0xFF9253EB#         ";
        change += "#L11# 各種打扮#l";
        change += "#L8# 轉換性別#l";
        change += "#L6# 安卓裝扮#l";
        ...
        if (selection == 0) { // 스킨
            ...
            if (cm.getPlayer().getHair() >= 39600 && cm.getPlayer().getHair() <= 39997) {
                cm.sendOk("#fs15##b스페셜 헤어#k 상태에서는 믹스염색이 불가능하답니다.");
                cm.dispose();
                return;
            }
            st = selection;
            ...
            if (cm.getPlayer().getJob() == 10112 || cm.getPlayer().getJob() == 6001 || (cm.getPlayer().getJob() >= 6500 && cm.getPlayer().getJob() <= 6512)) {
                cm.sendNext("#fs15##r현재 직업군은 사용할 수 없는 기능입니다.");
                cm.dispose();
                return;
            }
            말 = "#fs15# #d남자 캐릭터#k로 변경 시,\r\n"
        ...
        if (st == 7) {
            ...
            if (cm.getPlayer().getJob() == 10112 || cm.getPlayer().getJob() == 6001 || (cm.getPlayer().getJob() >= 6500 && cm.getPlayer().getJob() <= 6512)) {
                cm.sendNext("#fs15#현재 직업군은 사용할 수 없는 기능입니다.");
                cm.dispose();
                return;
            }
            ...
            cm.sendOk("#fs15##b성별#k이 정상적으로 변경 되었습니다.");
            cm.dispose();
        } else {
