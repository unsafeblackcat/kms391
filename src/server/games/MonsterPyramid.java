...
public class MonsterPyramid {
	...
	public void skipPlayer(PyramidPlayer p) {
		...
		for (PyramidPlayer pp : getPlayers()) {
			if (pp.b.getId() == p.b.getId()) {
				...
				if (floor == -2) {
					for (Triple<Integer, Integer, Integer> check : pyramidcheck) {
						if (((Integer) check.getLeft()).intValue() > 0
								&& ((Integer) check.getRight()).intValue() == -1) {
							for (Pair<Integer, Integer> block : (Iterable<Pair<Integer, Integer>>) pp.blocktype) {
								if (test(((Integer) check.getLeft()).intValue(), ((Integer) check.getMid()).intValue(),
										((Integer) block.getLeft()).intValue())
										&& ((Integer) block.getRight()).intValue() > 0) {
									floor = ((Integer) check.getLeft()).intValue();
									pos = ((Integer) check.getMid()).intValue();
									type = ((Integer) block.getLeft()).intValue();
								}
				...
				p.b.getClient().getSession().writeAndFlush(CField.UIPacket.detailShowInfo(
						"제한시간 총 " + p.b.getKeyValue(100668, "피라미드제한") + "번 이상 되셨습니다.(3번 이상 퇴장 및 패널티)", 3, 20, 20));
				...
				for (PyramidPlayer p2 : getPlayers()) {
					p2.b.getClient().send(SLFCGPacket.MonsterPyramidPacket.Handler(chrs, false,
							new int[] { 1, 4, 0, pp.turn, 7, 0, 0 }));
				}
	...
	public void endGame(MapleCharacter chr, boolean gang) {
		...
		for (PyramidPlayer p : getPlayers()) {
			if (p != null) {
				if (gang) {
					p.b.getClient().getSession().writeAndFlush(
							CField.UIPacket.detailShowInfo(chr.getName() + "님이 전장을 이탈하여 게임이 종료 되었습니다.", 3, 20, 20));
				}
				p.b.warp(993186500);
	...
	public static void StartGame(int n) {
		if (monsterPyramidMatchingQueue.size() == n) {
			MonsterPyramidMatchingInfo monsterPyramidMatchingInfo = new MonsterPyramidMatchingInfo(
					monsterPyramidMatchingQueue);
			...
			for (MapleCharacter mapleCharacter2 : monsterPyramidMatchingInfo.players) {
				...
				mapleCharacter2.getClient()
						.send(SLFCGPacket.ContentsWaiting(mapleCharacter2, 0, new int[] { 11, 5, 1, 24 }));
			}
			pyramidcheck.clear();
			for (int i = 0; i < 8; i++) {
	...
	public static void addQueue(MapleCharacter mapleCharacter, int n) {
		if (mapleCharacter.skillisCooling(100668) && !mapleCharacter.isGM()) {
			...
			mapleCharacter.getClient()
					.send(CField.NPCPacket.getNPCTalk(9062354, (byte) 0, "#r#e" + 초
							+ "분 후#n#k에 미니게임에 참여할 수 있어요!\r\n\r\n#b(미니게임 이용 중 포기를 하는 경우 잠시 동안 미니게임을 이용할 수 없습니다.)#k",
							"00 00", (byte) 0, mapleCharacter.getId()));
		} else if (monsterPyramidMatchingQueue.size() >= 3) {
			mapleCharacter.getClient().send(CField.NPCPacket.getNPCTalk(9062354, (byte) 0, "잠시 후에 다시 시도해 주세요.", "00 00",
			...
			mapleCharacter.getClient()
					.send(SLFCGPacket.ContentsWaiting(mapleCharacter, 993186400, new int[] { 11, 2, 1, 24 }));
			if (monsterPyramidMatchingQueue.size() == n) {
				for (MapleCharacter mapleCharacter2 : monsterPyramidMatchingQueue) {
					mapleCharacter2.getClient()
							.send(SLFCGPacket.ContentsWaiting(mapleCharacter2, 0, new int[] { 19, 0, 1, 24 }));
						...
						public void run() {
							...
							for (MapleCharacter chr2 : MonsterPyramid.monsterPyramidMatchingQueue) {
								if (mapleCharacter2.getId() != chr2.getId()) {
									remover.add(chr2);
									chr2.getClient()
											.send(SLFCGPacket.ContentsWaiting(chr2, 0, new int[] { 11, 5, 1, 24 }));
								}
							}
							for (MapleCharacter chr2 : remover) {
		...
		public List<Pair<Integer, Integer>> randblock(List<Pair<Integer, Integer>> blockadd, PyramidPlayer p, int n) {
			...
			if (n < 2) {
					...
					if (beforepblocksize < 12) {
						for (Pair<Integer, Integer> blc : blockadd) {
							if (((Integer) blc.getRight()).intValue() > 0) {
								...
								if (rand > 0) {
									blockadd.set(type, new Pair<>(Integer.valueOf(type),
											Integer.valueOf(((Integer) blc.getRight()).intValue() - rand)));
									beforepblocksize += rand;
									for (Pair<Integer, Integer> bt : p.blocktype) {
										if (((Integer) bt.getLeft()).intValue() == type) {
											p.blocktype.set(type, new Pair<>(Integer.valueOf(type),
													Integer.valueOf(((Integer) bt.getRight()).intValue() + rand)));
										}
									}
								}
							}
					...
					if (beforepblocksize >= 12 || after > 100000) {
						if (after > 100000) {
							for (PyramidPlayer pp : MonsterPyramid.this.getPlayers()) {
								pp.b.getClient().getSession().writeAndFlush(
										CField.UIPacket.detailShowInfo("오류발생! 게임을 다시 시작하여 주세요!", 3, 20, 20));
							}
				...
				for (Pair<Integer, Integer> bt : p.blocktype) {
					for (Pair<Integer, Integer> blc : blockadd) {
						if (bt.getLeft() == blc.getLeft()) {
							p.blocktype.set(((Integer) blc.getLeft()).intValue(),
									new Pair<>(blc.getLeft(), blc.getRight()));
						}
					}
				}
			}
	...
	public void setBlock(MapleCharacter chr, int floor, int pos, int type) {
		...
		int index = 0;
		for (Triple<Integer, Integer, Integer> check : pyramidcheck) {
			if (((Integer) check.getLeft()).intValue() == floor && ((Integer) check.getMid()).intValue() == pos
					&& ((Integer) check.getRight()).intValue() == -1) {
				pyramidcheck.set(index,
						new Triple<>(Integer.valueOf(floor), Integer.valueOf(pos), Integer.valueOf(type)));
				break;
			}
			index++;
		}
		...
		if (passcheck) {
			...
			for (PyramidPlayer pp : getPlayers()) {
				pp.b.getClient().send(SLFCGPacket.MonsterPyramidPacket.Handler(chrs, true,
						new int[] { 1, 3, nttturn, this.round, 0, 0, 0, 3, 3, 0 }));
			}
			ntturn++;
			if (ntturn >= 3) {
				ntturn = 0;
			...
			for (PyramidPlayer pp : getPlayers()) {
				if (pp.turn == ntturn) {
					...
					if (nextTurnCheck(pp) && ntturn >= 3) {
						ntturn = 0;
						pp.b.getClient().send(SLFCGPacket.MonsterPyramidPacket.Handler(chrs, true,
								new int[] { 1, 3, ntturn, this.round, 0, 0, 0, 3, 3, 0 }));
					}
					break;
				}
			}
		...
		for (PyramidPlayer pp : getPlayers()) {
			pp.b.getClient().send(SLFCGPacket.MonsterPyramidPacket.Handler(chrs, false,
					new int[] { 1, 3, ntturn, this.round, floor, pos, type, 13, 1, 6 }));
		}
		int blockcount = 0;
		for (Pair<Integer, Integer> block : (Iterable<Pair<Integer, Integer>>) (getPlayer(chr)).blocktype) {
		...
		if (passcount >= 3 || blockcount == 0) {
			for (PyramidPlayer pp : getPlayers()) {
				if (this.round < 3) {
					if (blockcount == 0) {
						pp.b.getClient().send(SLFCGPacket.MonsterPyramidPacket.Handler(chrs, true,
								new int[] { 1, 6, ntturn, this.round, ntturn - 1 }));
					}
					pp.b.getClient().send(SLFCGPacket.MonsterPyramidPacket.Handler(chrs, true,
							new int[] { 1, 4, pp.turn, this.round, 5, 0, 9 }));
				}
			}
	...
	public void nextStage(int ntturn, boolean used) {
		...
		if (this.round >= 4) {
			...
			for (PyramidPlayer pp : getPlayers()) {
				pp.b.getClient().send(SLFCGPacket.MonsterPyramidPacket.Handler(list, true,
						new int[] { 1, 4, pp.turn, this.round, 5, 0, 9 }));
				pp.setRank(2);
				for (PyramidPlayer ppp : getPlayers()) {
					if (pp.getPoint() > ppp.getPoint()) {
				...
				if (pp.getRank() == 0) {
					...
					pp.b.getClient().send(SLFCGPacket.MonsterPyramidPacket.Handler(list, true,
							new int[] { 1, 3, pp.turn, 3, 0, 0, 0, 11, 0, 0, 0 }));
					continue;
				}
				if (pp.getRank() == 1) {
					pp.b.setKeyValue(100668, "Rank", "2");
					pp.b.getClient().send(SLFCGPacket.MonsterPyramidPacket.Handler(list, true,
							new int[] { 1, 3, pp.turn, 3, 0, 0, 0, 12, 0, 0, 0 }));
					continue;
				}
			...
			server.Timer.EtcTimer.getInstance().schedule(() -> {
				for (MapleCharacter p : list) {
					p.getClient().send(SLFCGPacket.MonsterPyramidPacket.Handler(list, false,
							new int[] { 1, 5, ntturn, this.round }));
				}
			}, 4000L);
			server.Timer.EtcTimer.getInstance().schedule(() -> {
				for (MapleCharacter p : list) {
					p.getClient().send(SLFCGPacket.MonsterPyramidPacket.Handler(list, true,
							new int[] { 1, 4, ntturn, this.round, 0, 0, 0, 8, 3, 6 }));
					p.getClient().send(SLFCGPacket.MonsterPyramidPacket.Handler(list, true,
							new int[] { 1, 4, ntturn, this.round, 8, 0, 6 }));
					p.getClient().send(SLFCGPacket.MonsterPyramidPacket.Handler(list, true,
							new int[] { 1, 3, ntturn, this.round, 0, 0, 0, 8, 3, 6 }));
				}
			}, 6000L);
		}
	}
	...
	public boolean test(int floor, int pos, int type) {
		...
		for (Triple<Integer, Integer, Integer> check : pyramidcheck) {
			if (floor - 1 == ((Integer) check.getLeft()).intValue()
					&& ((Integer) check.getMid()).intValue() == pos + 1) {
