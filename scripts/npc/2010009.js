...
function action(mode, type, selection) {
	...
	if (status == 0) {
	    ...
	    if (selection == 0) {
			...
			if (!cm.isLeader()) {
				cm.sendOk("파티장만 할 수 있습니다.");
				cm.dispose();
			} else if (cm.getPlayer().getParty() == null || partymembers == null || partymembers.size() != 2 || !cm.isLeader()) {
				cm.sendOk("파티에 최소 두 명의 길드 마스터가 있어야합니다.");
				cm.dispose();
			} else if (partymembers.get(0).getGuildId() <= 0 || partymembers.get(0).getGuildRank() > 1) {
				cm.sendOk("파티원 중에 길드가 없거나 길드 마스터가 아닌 사람이 있습니다." + partymembers.get(0).getName() + "/" + partymembers.get(0).getGuildRank());
				cm.dispose();
