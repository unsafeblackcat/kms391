...
public class MarriageMiniBox extends AbstractPlayerStore {
	...
	public MarriageMiniBox(MapleCharacter owner, int itemId, String description, String pass, int GameType,
			int partnerid) {
	...
	public void StartMarriage() {
		...
		if (getPlayer1() == null || getPlayer1().getMarriageId() > 0 || getPlayer2() == null
				|| getPlayer2().getMarriageId() > 0 || getPlayer2().getId() != getPartnerId() || !getPlayer1().isAlive()
				|| !getPlayer2().isAlive()) {
			closeMarriageBox(true, 3);
			return;
		}
		if (!MapleInventoryManipulator.checkSpace(getPlayer1().getClient(), 1112744, 1, "")
				|| !MapleInventoryManipulator.checkSpace(getPlayer2().getClient(), 1112744, 1, "")) {
	...
	public byte getVisitorSlot(MapleCharacter visitor) {
		for (byte i = 0; i < this.chrs.length; i = (byte) (i + 1)) {
			if (this.chrs[i] != null && this.chrs[i].get() != null
					&& ((MapleCharacter) this.chrs[i].get()).getId() == visitor.getId()) {
				return (byte) (i + 1);
			}
		}
