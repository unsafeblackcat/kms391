...
public class EventInstanceManager {//修改記錄？？
	...
	public void registerPlayer(final MapleCharacter chr) {
		...
		try {
			...
			FileoutputUtil.log("Log_Script_Except.txt", "事件名稱" + this.em.getName() + ", 實例名稱 : "
			        + this.name + ", 方法名稱 : playerEntry:\n" + ex2);
			System.out.println("事件名稱" + this.em.getName() + ", 實例名稱 : " + this.name
			        + ", 方法名稱 : playerEntry:\n" + ex2);
		}
	}
			...
			public void run() {
				...
				try {
					...
					FileoutputUtil.log("Log_Script_Except.txt",
					        "事件名稱 : " + EventInstanceManager.this.em.getName() + ", 實例名稱 : "
					                + EventInstanceManager.this.name + ", 方法名稱 : scheduledTimeout:\n" + ex);
					System.out.println("事件名稱 : " + EventInstanceManager.this.em.getName() + ", 實例名稱 : "
					        + EventInstanceManager.this.name + ", 方法名稱 : scheduledTimeout:\n" + ex);
				}
			}
		}, delay);
	...
	public void restartEventTimerMillSecond(final int time) {
		try {
			...
			FileoutputUtil.outputFileError("Log_Script_Except.txt", ex);
			System.out.println("事件名稱 : " + this.em.getName() + ", 實例名稱 : " 
			        + this.name + ", 方法名稱 : restartEventTimerMillSecond:\n");
			ex.printStackTrace();
		}
	}
	...
	public void restartEventTimer(final long time, final int type) {
		try {
			...
			for (final MapleCharacter chr : this.getPlayers()) {
				if (this.name.startsWith("PVP")) {
					chr.getClient().getSession().writeAndFlush(
							(Object) CField.getPVPClock(Integer.parseInt(this.getProperty("type")), timesend));
				} else if (type == 4 || type == 5) {
					chr.getClient().getSession()
							.writeAndFlush((Object) CField.getVanVanClock((byte) ((type != 4) ? 1 : 0), timesend));
				} else {
			...
			FileoutputUtil.outputFileError("Log_Script_Except.txt", ex);
			System.out.println("事件名稱 : " + this.em.getName() + ", 實例名稱 : " 
			        + this.name + ", 方法名稱 : restartEventTimer:\n");
			ex.printStackTrace();
		}
	}
		...
		try {
			...
			for (final MapleCharacter chr : this.getPlayers()) {
				if (this.name.startsWith("PVP")) {
					...
					chr.getClient().getSession()
							.writeAndFlush((Object) CField.getClockMilliEvent(this.timeStarted + this.eventTime));
				} else {
					chr.getClient().getSession().writeAndFlush((Object) CField.getClock(timesend));
				}
			}
	...
	public void unregisterMonster(final MapleMonster mob) {
		...
		if (this.mobs.isEmpty()) {
			try {
				...
				FileoutputUtil.log("Log_Script_Except.txt", "事件名稱 : " + this.em.getName() + ", 實例名稱 : "
				        + this.name + ", 方法名稱 : allMonstersDead:\n" + ex);
				System.out.println("事件名稱 : " + this.em.getName() + ", 實例名稱 : " + this.name
				        + ", 方法名稱 : allMonstersDead:\n" + ex);
			}
		}
	}
	...
	public void monsterKilled(final MapleCharacter chr, final MapleMonster mob) {// 怪物死亡後調用此方法，更新击杀该怪物的角色的击杀计数。
		if (this.disposed) { 
			return;
		}
		try { 
			...
			System.out.println("事件名稱 : " + ((this.em == null) ? "null" : this.em.getName()) + ", 實例名稱 : "
			        + this.name + ", 方法名稱 : monsterValue:\n" + ex);
			FileoutputUtil.log("Log_Script_Except.txt", "事件名稱 : " + ((this.em == null) ? "null" : this.em.getName())
			        + ", 實例名稱 : " + this.name + ", 方法名稱 : monsterValue:\n" + ex);
			...
			ex3.printStackTrace();
			FileoutputUtil.outputFileError("Log_Script_Except.txt", ex3);
		}
	}
	...
	public void monsterDamaged(final MapleCharacter chr, final MapleMonster mob, final long damage) {
		final List<Integer> mobs = Collections
				.unmodifiableList((List<? extends Integer>) Arrays.asList(9700037, 8850011));
	...
	public void dispose_NoLock() {
		...
		try {
			...
			System.out.println("原因 : " + emN + " 實例名稱: " + this.name + " 方法: dispose:");
			e.printStackTrace();
		...
		Timer.EventTimer.getInstance().schedule(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					System.out.println("事件名稱 : " + EventInstanceManager.this.em.getName() + ", 實例名稱 : "
					        + EventInstanceManager.this.name + ", 方法名稱 : " + methodName + ":\n" + ex);
					FileoutputUtil.log("Log_Script_Except.txt",
					        "事件名稱 : " + EventInstanceManager.this.em.getName() + ", 實例名稱 : "
					                + EventInstanceManager.this.name + ", 方法名稱(排程) : " + methodName + " :\n" + ex);
				}
			}
		}, delay);
	}
	...
	public final void leftParty(final MapleCharacter chr) {
		...
		try {
			...
			System.out.println("事件名稱 : " + this.em.getName() + ", 實例名稱 : " + this.name + ", 方法名稱 : leftParty:\n" + ex);
			FileoutputUtil.log("Log_Script_Except.txt", "事件名稱 : " + this.em.getName() + ", 實例名稱 : " + this.name + ", 方法名稱 : leftParty:\n" + ex);
		}
	}
	...
	public final void disbandParty() {
		...
		try {
			...
			System.out.println("事件名稱 : " + this.em.getName() + ", 實例名稱 : " + this.name + ", 方法名稱 : disbandParty:\n" + ex);
			FileoutputUtil.log("Log_Script_Except.txt", "事件名稱 : " + this.em.getName() + ", 實例名稱 : " + this.name + ", 方法名稱 : disbandParty:\n" + ex);
	...
	public final void finishPQ() {
		...
		try {
			...
			System.out.println("事件名稱 : " + this.em.getName() + ", 實例名稱 : " + this.name + ", 方法名稱 : clearPQ:\n" + ex);
			FileoutputUtil.log("Log_Script_Except.txt", "事件名稱 : " + this.em.getName() + ", 實例名稱 : " + this.name + ", 方法名稱 : clearPQ:\n" + ex);
	...
	public final void removePlayer(final MapleCharacter chr) {
		...
		try {
			...
			System.out.println("事件名稱 : " + this.em.getName() + ", 實例名稱 : " + this.name + ", 方法名稱 : playerExit:\n" + ex);
			FileoutputUtil.log("Log_Script_Except.txt", "事件名稱 : " + this.em.getName() + ", 實例名稱 : " + this.name + ", 方法名稱 : playerExit:\n" + ex);
	...
	public void onMapLoad(final MapleCharacter chr) {
		...
		try {
			...
			System.out.println("事件名稱 : " + this.em.getName() + ", 實例名稱 : " + this.name + ", 方法名稱 : onMapLoad:\n" + ex);
			FileoutputUtil.log("Log_Script_Except.txt", "事件名稱 : " + this.em.getName() + ", 實例名稱 : " + this.name + ", 方法名稱 : onMapLoad:\n" + ex);
		} catch (NoSuchMethodException ex2) {
		}
	}
