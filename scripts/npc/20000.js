...
function action(mode, type, selection) {
    ...
    if (!cm.haveItem(4033970,1)) {
        john += "#fs15##fUI/UIWindow2.img/UtilDlgEx/list1#\r\n#fn굴림##L1##d존의 진심이 담겨있는 꽃";
    } else {
        john += "#fs15##fUI/UIWindow2.img/UtilDlgEx/list3#\r\n#fn굴림##L2##d존의 진심이 담겨있는 꽃";
    }
        cm.sendSimple(john);
    } else {
        cm.dispose();
	...
	cm.dispose();
        ...
        cm.sendNextS("#fn나눔고딕 Extrabold##b슈미를 도와주다가 떨어져 죽는줄 알았어 으으..\r\n다음은... #fs15#존#fs15# 이라는 사람이였지..?",2);
        }
    } else if (status == 2) {
        cm.sendNextPrevS("#fn나눔고딕 Extrabold##b안녕하세요! #fs15#존#fs15# 님 맞으신가요??\r\n저는 #fs15##h ##fs15# (이)라고 합니다.#k",2);
    } else if (status == 3) {
        cm.sendNextPrev("#fn나눔고딕 Extrabold#응? 못보던 젊은이로군…그래, 무슨일로 날 찾아왔지?..");
    } else if (status == 4) {
        cm.sendNextS("#fn나눔고딕 Extrabold##b아! 저는 #fs15#존#fs15# 님을 … \r\n\r\n#fs15##L1#진심으로 돕고 싶어 소문듣고 찾아왔습니다.\r\n#L2#도와드리라고 의뢰를 받아서 왔습니다.",2);
    } else if (status == 5) {
            if (selection == 2) {
