...
public class DumpQuests {// 修改记录
	...
	public DumpQuests(boolean update) throws Exception {
		...
		// 添加调试输出：确认Quest.wz文件路径
		File questFile = new File("Wz/Quest.wz");
		System.out.println("尝试加载文件: " + questFile.getAbsolutePath());
		System.out.println("文件是否存在: " + questFile.exists());
		System.out.println("文件是否可读: " + questFile.canRead());

		this.quest = MapleDataProviderFactory.getDataProvider(questFile);
		if (this.quest == null) {
			this.hadError = true;
			System.out.println("错误: 无法加载Quest.wz数据提供程序");
		}
	}
	...
	public void dumpQuests() throws Exception {
		if (!this.hadError) {
			PreparedStatement psai = this.con.prepareStatement(
					"INSERT INTO wz_questactitemdata(uniqueid, itemid, count, period, gender, job, jobEx, prop) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
			PreparedStatement psas = this.con.prepareStatement(
					"INSERT INTO wz_questactskilldata(uniqueid, skillid, skillLevel, masterLevel) VALUES (?, ?, ?, ?)");
			PreparedStatement psaq = this.con
					.prepareStatement("INSERT INTO wz_questactquestdata(uniqueid, quest, state) VALUES (?, ?, ?)");
			PreparedStatement ps = this.con.prepareStatement(
					"INSERT INTO wz_questdata(questid, name, autoStart, autoPreComplete, viewMedalItem, selectedSkillID, blocked, autoAccept, autoComplete, autoCompleteAction) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
			...
			PreparedStatement psq = this.con.prepareStatement(
			...
			System.out.println("Check.img 是否成功加载: " + (checkz != null));

			MapleData actz = this.quest.getData("Act.img");
			System.out.println("Act.img 是否成功加载: " + (actz != null));

			MapleData infoz = this.quest.getData("QuestInfo.img");
			System.out.println("QuestInfo.img 是否成功加载: " + (infoz != null));

			MapleData pinfoz = this.quest.getData("PQuest.img");
			System.out.println("PQuest.img 是否成功加载: " + (pinfoz != null));

			System.out.println("========================\n");

			// 检查QuestInfo.img为空的情况
			if (infoz == null) {
				System.out.println("错误: QuestInfo.img 未找到，跳过任务数据处理");
				this.hadError = true;
				return;
			}
		}
						...
						if (req.getName().equals("job")) {
							...
							for (int x = 0; x < child.size(); x++) {
								dataStore.add(new Pair<>(Integer.valueOf(i),
										Integer.valueOf(MapleDataTool.getInt(child.get(x), -1))));
							}
	...
	public static void main(String[] args) {
		try {
			...
			System.out.println(
					"Finished" + withErrors + " in " + elapsedMinutes + " minutes " + elapsedSecs + " seconds");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
