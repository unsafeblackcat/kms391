...
function end(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        qm.sendNext("#b[일일 퀘스트] 호기심 많은 학자 유령 1,000마리 사냥#k 임무를 완수하고 돌아왔군.");
    } else if(status == 1) {
        qm.sendNext("#b#e#i1713000# #z1713000# 5개#k#n를 지급했다. 장비창을 환인해 봐라");
    } else if (status == 2) {
        for (a = 0; a < 5; a++) {
            qm.gainItem(1713000, 1);
        }
        qm.forceCompleteQuest();
        str = qm.getPlayer().getV("Authentic1");
        ab = str.split(",");
        var clear = 0;
        ...
        qm.getPlayer().setKeyValue(39819, "count", (clear + 2) + "");
        qm.dispose();
    }
}