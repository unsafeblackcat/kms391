...
function action(mode, type, selection) {
	...
	for (var i = 60000; i < 60015; i++) {
        ...
        var choose = "#fn나눔고딕#PRAY ! 무릉 컨텐츠 NPC입니다.#fs15#\r\n";
        ...
        if (selection == 0) {
            ...
            말 = "#fs15#" + 검정 + "어서오게! #b페스티벌#k" + 검정 + "은 재밌게 즐기고 있나? 더 재밌게 즐기기 위한 #r승급제도#k" + 검정 + "가 있다네. 자네도 한번 해볼텐가?\r\n";
            말 += "#fc0xFFD5D5D5#───────────────────────────#k\r\n";
            ...
            var choose = "#fs15#" + 검정 + "초월자의 힘에 다가가 볼텐가?\r\n";
...
cm.openNpc(9062147);
            ...
            var choose = "#fs15#" + 검정 + "자네를 위해 다양한 퀘스트가 준비되어 있다네!\r\n";
            choose += "#fc0xFFD5D5D5#───────────────────────────#k\r\n";
...
cm.openNpc(9062296);
        ...
        if (selection == 100) {
            ...
            if (leftslot < 2) {
                cm.sendOkS("#fs15##r기타칸 2 칸 이상을 확보하고 다시 말을 걸어주게.", 0x04, 9401232);
                cm.dispose();
                return;
            }
            if (cm.getPlayer().getLevel() < 230) {
                cm.sendOkS("#fs15##r레벨 230 이상이 되어야 승급이 가능하다네.", 0x4, 9401232);
                cm.dispose();
                return;
            }
            if (cm.getPlayer().getMeso() < ssssitemmeso || !cm.haveItem(ssssitem2, ssssitemc2) || !cm.haveItem(ssssitem, ssssitemc)) {   //재료체크
                cm.sendOkS("#fs15##r승급에 필요한 재료가 부족한거 같네.", 0x04, 9401232);
                cm.dispose();
                return;
            }
            ...
            if (sell == 5) {
                if (cm.getPlayer().getMeso() < 700000000 || !cm.haveItem(4310010, 50)) {   //재료체크
                    cm.sendOkS("#fs15##b#z1182000##k를 제작하기 위한 재료 아이템이 부족한거 같군.", 0x04, 9401232);
                    cm.dispose();
                    return;
                }
                ...
                if (Math.floor(Math.random() * 100) <= 50) {// 성공했을때
                    ...
                    cm.gainItem(4310010, -50);
                    cm.getPlayer().AddBloomingCoin(-350, null);
                    cm.sendOkS("#fs15##r아쉽게도 제작에 실패했다네..", 0x04, 9401232);
                    cm.dispose();
                    return;
                }
            } else if (sell == 6) {
        ...
        if (sel == 5) {
            ...
            if (!cm.haveItem(needitem1, needitem1count)) {   //재료체크
                cm.sendOkS("#fs15##b#z" + sucitemid + "##k를 제작하기 위한 재료 아이템이 부족한거 같군.", 0x04, 9401232);
                cm.dispose();
                return;
            }
