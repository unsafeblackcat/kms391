...
public class BattleReverse {
	...
	private void InitBoard() {
		...
		for (int a = 0; a < 5; ++a) {
			for (boolean temp = false; !temp; temp = this
					.MakeHole(new Point(Randomizer.nextInt(8), Randomizer.nextInt(8)))) {
			}
		}
	}
	...
	public boolean isValidMove(final MapleCharacter chr, final Point pt) {
		...
		if (player != null && (opponent != null & pt != null) && (player.getStoneId() == 0 || player.getStoneId() == 1)
				&& (opponent.getStoneId() == 0 || opponent.getStoneId() == 1) && pt.x >= 0 && pt.x < 8 && pt.y >= 0
				&& pt.y < 8) {
	...
	public void skipPlayer(final MapleCharacter mapleCharacter) {
		...
		if (!(placeablePoints = mapleCharacter.getBattleReverseInstance()
				.getPlaceablePoints(mapleCharacter.getBattleReverseInstance().getPlayer(mapleCharacter.getId())))
				.isEmpty()) {
	...
	public void placeStone(final Point lastPoint, final MapleCharacter mapleCharacter) {
		...
		int n3 = 0;
		while (x >= 0 && y >= 0
				&& (x == n || (this.Board[x][y].getStoneId() != -1 && this.Board[x][y].getStoneId() != 3))) {
		...
		int n5;
		for (n4 = n, n5 = n2; n4 >= 0
				&& (n4 == n || (this.Board[n4][n5].getStoneId() != -1 && this.Board[n4][n5].getStoneId() != 3)); --n4) {
		...
		for (int n6 = n; n6 >= 0 && n5 <= 7 && (n6 == n
				|| (this.Board[n6][n5].getStoneId() != -1 && this.Board[n6][n5].getStoneId() != 3)); --n6, ++n5) {
		...
		int n7 = n;
		for (int n8 = n2; n8 <= 7 && (n8 == n2
				|| (this.Board[n7][n8].getStoneId() != -1 && this.Board[n7][n8].getStoneId() != 3)); ++n8) {
		...
		for (int n9 = n2; n7 <= 7 && n9 <= 7 && (n7 == n
				|| (this.Board[n7][n9].getStoneId() != -1 && this.Board[n7][n9].getStoneId() != 3)); ++n7, ++n9) {
		...
		int n11;
		int n12;
		for (n11 = n, n12 = n2; n11 <= 7 && (n11 == n
				|| (this.Board[n11][n12].getStoneId() != -1 && this.Board[n11][n12].getStoneId() != 3)); ++n11) {
		...
		for (int n14 = n; n14 <= 7 && n12 >= 0 && (n14 == n
				|| (this.Board[n14][n12].getStoneId() != -1 && this.Board[n14][n12].getStoneId() != 3)); ++n14, --n12) {
		...
		final int n16 = n;
		for (int n17 = n2; n17 >= 0 && (n17 == n2
				|| (this.Board[n16][n17].getStoneId() != -1 && this.Board[n16][n17].getStoneId() != 3)); --n17) {
		...
		if (opponent.getHP() <= 0) {
			for (final BattleReversePlayer battleReversePlayer4 : this.Players) {
				battleReversePlayer4.chr.getClient().getSession()
						.writeAndFlush((Object) SLFCGPacket.MultiOthelloGamePacket.onBoardUpdate(
								opponent.getPlayer().getId() == battleReversePlayer4.getPlayer().getId(), lastPoint,
								player.getStoneId(), this.getOpponent(player.getPlayer().getId()).getHP(),
								this.getStones(), (byte) 1));
			}
	...
	public void setTurn(final MapleCharacter mapleCharacter, final BattleReversePlayer battleReversePlayer,
			final BattleReversePlayer battleReversePlayer2, final Point point) {
		if (this.getPlaceablePoints(this.getOpponent(mapleCharacter.getId())).size() != 0) {
			for (final BattleReversePlayer battleReversePlayer3 : this.Players) {
				battleReversePlayer3.chr.getClient().getSession()
						.writeAndFlush((Object) SLFCGPacket.MultiOthelloGamePacket.onBoardUpdate(
								battleReversePlayer2.getPlayer().getId() == battleReversePlayer3.getPlayer().getId(),
								point, battleReversePlayer.getStoneId(),
								this.getOpponent(battleReversePlayer.getPlayer().getId()).getHP(), this.getStones(),
								(byte) 1));
			}
			...
			this.BattleReverseTimer = Timer.EventTimer.getInstance()
					.schedule(() -> this.skipPlayer(battleReversePlayer2.chr), 11200L);
			return;
		}
		if (this.getPlaceablePoints(this.getPlayer(mapleCharacter.getId())).size() == 0) {
			for (final BattleReversePlayer battleReversePlayer4 : this.Players) {
				battleReversePlayer4.chr.getClient().getSession()
						.writeAndFlush((Object) SLFCGPacket.MultiOthelloGamePacket.onBoardUpdate(
								battleReversePlayer2.getPlayer().getId() == battleReversePlayer4.getPlayer().getId(),
								point, battleReversePlayer.getStoneId(),
								this.getOpponent(battleReversePlayer.getPlayer().getId()).getHP(), this.getStones(),
								(byte) 1));
			}
		...
		while (iterator3.hasNext()) {
			...
			(battleReversePlayer5 = iterator3.next()).chr.getClient().getSession()
					.writeAndFlush((Object) CField.UIPacket.detailShowInfo(
							"\ub458 \uc218 \uc788\ub294 \uacf3\uc774 \uc5c6\uc5b4 \ud134\uc774 \ub118\uc5b4\uac11\ub2c8\ub2e4.",
							3, 20, 20));
			battleReversePlayer5.chr.getClient().getSession()
					.writeAndFlush((Object) SLFCGPacket.MultiOthelloGamePacket.onBoardUpdate(
							battleReversePlayer.getPlayer().getId() == battleReversePlayer5.getPlayer().getId(), point,
							this.getOpponent(mapleCharacter.getId()).getStoneId(),
							this.getPlayer(mapleCharacter.getId()).getHP(), this.getStones(), (byte) 0));
		}
	}
	...
	public void endGame(final MapleCharacter mapleCharacter, final boolean b) {
		...
		if (!b) {
			if (player.getHP() <= 0) {
				...
				player.chr.getClient().getSession()
						.writeAndFlush((Object) SLFCGPacket.MultiOthelloGamePacket.onResult(2));
				opponent.chr.getClient().getSession()
						.writeAndFlush((Object) SLFCGPacket.MultiOthelloGamePacket.onResult(2));
			}
		} else {
			opponent.chr.getClient().getSession()
					.writeAndFlush((Object) SLFCGPacket.MultiOthelloGamePacket.onResult(4));
			opponent.chr.getClient().getSession().writeAndFlush((Object) CField.UIPacket.detailShowInfo(
					"\uc0c1\ub300\ubc29\uc774 \ubbf8\ub2c8\uac8c\uc784\uc744 \uc885\ub8cc\ud558\uc5ec \uac8c\uc784\uc774 \uc885\ub8cc\ub429\ub2c8\ub2e4.",
					3, 20, 20));
			opponent.chr.getClient().getSession()
					.writeAndFlush((Object) SLFCGPacket.playSE("Sound/MiniGame.img/oneCard/victory"));
		}
		if (battleReversePlayer != null && battleReversePlayer2 != null) {
			battleReversePlayer.chr.getClient().getSession()
					.writeAndFlush((Object) SLFCGPacket.MultiOthelloGamePacket.onResult(4));
			battleReversePlayer.chr.getClient().getSession()
					.writeAndFlush((Object) CWvsContext.onSessionValue("svBattleResult", "win"));
			battleReversePlayer.chr.getClient().getSession()
					.writeAndFlush((Object) CField.playSound("Sound/MiniGame.img/oneCard/victory"));
			battleReversePlayer2.chr.getClient().getSession()
					.writeAndFlush((Object) SLFCGPacket.MultiOthelloGamePacket.onResult(4));
			battleReversePlayer2.chr.getClient().getSession()
					.writeAndFlush((Object) CWvsContext.onSessionValue("svBattleResult", "lose"));
			battleReversePlayer2.chr.getClient().getSession()
					.writeAndFlush((Object) CField.playSound("Sound/MiniGame.img/oneCard/gameover"));
			battleReversePlayer.chr.getClient().getSession().writeAndFlush(
					(Object) CField.UIPacket.detailShowInfo(battleReversePlayer.chr.getName() + "님의 승리!", 3, 20, 20));
			battleReversePlayer2.chr.getClient().getSession().writeAndFlush(
					(Object) CField.UIPacket.detailShowInfo(battleReversePlayer.chr.getName() + "님의 승리!", 3, 20, 20));
			...
			for (int j = 0; j < 8; ++j) {
				if (this.Board[i][j].getStoneId() == opponent.getStoneId()) {
					...
					if (i - 1 >= 0 && j - 1 >= 0 && i + 1 <= 7 && j + 1 <= 7
							&& this.Board[i + 1][j + 1].getStoneId() == battleReversePlayer.getStoneId()) {
						while (i - 1 >= 0 && j - 1 >= 0) {
					...
					int n3 = n;
					int n4 = n2;
					if (n3 + 1 <= 7 && n4 - 1 >= 0 && n3 - 1 >= 0 && n4 + 1 <= 7
							&& this.Board[n3 - 1][n4 + 1].getStoneId() == battleReversePlayer.getStoneId()) {
						while (n3 + 1 <= 7 && n4 - 1 >= 0) {
					...
					int n5 = n;
					int n6 = n2;
					if (n5 + 1 <= 7 && n6 + 1 <= 7 && n5 - 1 >= 0 && n6 - 1 >= 0
							&& this.Board[n5 - 1][n6 - 1].getStoneId() == battleReversePlayer.getStoneId()) {
						while (n5 + 1 <= 7 && n6 + 1 <= 7) {
					...
					int n7 = n;
					int n8 = n2;
					if (n7 - 1 >= 0 && n8 + 1 <= 7 && n7 + 1 <= 7 && n8 - 1 >= 0
							&& this.Board[n7 + 1][n8 - 1].getStoneId() == battleReversePlayer.getStoneId()) {
						while (n7 - 1 >= 0 && n8 + 1 <= 7) {
					...
					final int n9 = n;
					int n10;
					if ((n10 = n2) - 1 >= 0 && n10 + 1 <= 7
							&& this.Board[n9][n10 + 1].getStoneId() == battleReversePlayer.getStoneId()) {
						while (n10 - 1 >= 0) {
					...
					final int n11 = n;
					int n12;
					if ((n12 = n2) + 1 <= 7 && n12 - 1 >= 0
							&& this.Board[n11][n12 - 1].getStoneId() == battleReversePlayer.getStoneId()) {
						while (n12 + 1 <= 7) {
					...
					final int n14 = n2;
					if (n13 + 1 <= 7 && n13 - 1 >= 0
							&& this.Board[n13 - 1][n14].getStoneId() == battleReversePlayer.getStoneId()) {
						while (n13 + 1 <= 7) {
					...
					final int n16 = n2;
					if (n15 - 1 >= 0 && n15 + 1 <= 7
							&& this.Board[n15 + 1][n16].getStoneId() == battleReversePlayer.getStoneId()) {
						while (n15 - 1 >= 0) {
	...
	public static void addQueue(final MapleCharacter mapleCharacter, final int n) {
		if (BattleReverse.BattleReverseMatchingQueue.size() >= 2) {
			mapleCharacter.getClient()
					.send(CField.NPCPacket.getNPCTalk(9062354, (byte) 0,
							"\uc7a0\uc2dc \ud6c4\uc5d0 \ub2e4\uc2dc \uc2dc\ub3c4\ud574 \uc8fc\uc138\uc694.", "00 00",
							(byte) 0, mapleCharacter.getId()));
				...
				for (final MapleCharacter mapleCharacter2 : BattleReverse.BattleReverseMatchingQueue) {
					...
					(mapleCharacter2.ConstentTimer = new java.util.Timer()).schedule(new TimerTask() {
						...
						public void run() {
							...
							mapleCharacter2.getClient()
									.send(SLFCGPacket.ContentsWaiting(mapleCharacter2, 0, 11, 5, 1, 23));
	...
	public static void addQueue(MapleCharacter mapleCharacter, boolean b) {
		block2: {
			block1: {
				...
				BattleReverseMatchingInfo battleReverseMatchingInfo = new BattleReverseMatchingInfo(
						BattleReverseMatchingQueue.get(0), BattleReverseMatchingQueue.get(1));
				...
				Timer.MapTimer.getInstance().schedule(() -> {
					...
					br.InitBoard();
					for (MapleCharacter p : chrs) {
						p.getClient().getSession().writeAndFlush((Object) SLFCGPacket.MultiOthelloGamePacket
								.createUI(chrs, p, br.getPlayer(p.getId()).getStoneId()));
					}
			...
			for (MaplePartyCharacter maplePartyCharacter : mapleCharacter.getParty().getMembers()) {
				...
				BattleReverse.addQueue(ChannelServer.getInstance(channel).getPlayerStorage()
						.getCharacterById(maplePartyCharacter.getId()), false);
			}
		}
	}
	...
	public static void StartGame2() {
		if (BattleReverseMatchingQueue.size() >= 2) {
			...
			battleReverseMatchingInfo.p1.warp(993186700);
			battleReverseMatchingInfo.p2.warp(993186700);
			battleReverseMatchingInfo.p1.getClient()
					.send(SLFCGPacket.ContentsWaiting(battleReverseMatchingInfo.p1, 0, 11, 5, 1, 23));
			battleReverseMatchingInfo.p2.getClient()
					.send(SLFCGPacket.ContentsWaiting(battleReverseMatchingInfo.p2, 0, 11, 5, 1, 23));
			Timer.MapTimer.getInstance().schedule(() -> {
				ArrayList<MapleCharacter> chrs = new ArrayList<MapleCharacter>();
				chrs.add(battleReverseMatchingInfo.p1);
				chrs.add(battleReverseMatchingInfo.p2);
				...
				br.InitBoard();
				for (MapleCharacter p : chrs) {
					p.getClient().getSession().writeAndFlush((Object) SLFCGPacket.MultiOthelloGamePacket.createUI(chrs,
							p, br.getPlayer(p.getId()).getStoneId()));
				}
				Timer.EventTimer.getInstance().schedule(() -> br.StartGame(), 3000L);
			}, 5000L);
		}
	...
	private void StartGame() {
		...
		for (final BattleReversePlayer bp : this.Players) {
			bp.chr.getClient().getSession()
					.writeAndFlush((Object) SLFCGPacket.MultiOthelloGamePacket.onInit(this.getStones(), First.StoneId));
		}
