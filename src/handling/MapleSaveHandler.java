...
public class MapleSaveHandler implements Runnable {
	...
	public static String[][] weekKeyValues = new String[][] { { "db_lastweek", "0" }, { "dojo", "0" },
			{ "dojo_time", "0" } };

	public static String[] clientDateKeyValues = new String[] { "dailyGiftComplete", "hotelMapleToday", "ht", "mPark",
			"mpark_t", "jump_1", "jump_2", "jump_3", "day_reborn_1", "day_reborn_2", "day_qitem", "day_summer_a",
			"day_summer_e", "day_colorlens", "day_MedalC", "day_MedalD", "minigame" };
	...
	public MapleSaveHandler() {
		...
		System.out.println("[加載已完成] 啟動冒險島保存處理系統");
	...
	public void run() {
		...
		if (time - this.lastSaveAuctionTime >= 900000L) {
			...
			System.out.println("[通知]服務器打開後經過了 " + (count * 15 / 60) + "“時間 " + (count * 15 % 60) + "分鐘.");
		...
		if ((new Date()).getHours() >= 10 && (new Date()).getHours() <= 24)
			...
			} else if ((new Date()).getMinutes() != 15 && (new Date()).getMinutes() != 45
					&& this.minigamegive == true) {
		...
		if ((new Date()).getHours() == 21 && (new Date()).getMinutes() == 0 && !this.give) {
			...
			for (MapleCharacter csplayer : CashShopServer.getPlayerStorage().getAllCharacters().values()) {
				if (csplayer != null && csplayer.getName() != null && !chrs.contains(csplayer))
					chrs.add(csplayer);
			}
			...
			for (MapleCharacter chr2 : chrs) {
				...
				chr2.dropMessage(1, "[HOT] Fancy熱力時間-登入奬勵已發放，請査看冒險島保管箱.");
				chr2.dropMessage(6, "[HOT] Fancy熱力時間-登入奬勵已發放，請査看冒險島保管箱.");
			}

		}

		if (((new Date()).getHours() == 19 && (new Date()).getMinutes() == 50)
				|| ((new Date()).getHours() == 20 && (new Date()).getMinutes() == 0)
				|| ((new Date()).getHours() == 21 && (new Date()).getMinutes() == 50)
				|| ((new Date()).getHours() == 22 && (new Date()).getMinutes() == 0))
			for (ChannelServer cserv : ChannelServer.getAllInstances()) {
				for (MapleCharacter hp : cserv.getPlayerStorage().getAllCharacters().values()) {
					...
					if ((new Date()).getHours() == 21 && (new Date()).getMinutes() == 50) {
						if (hp.getSkillCustomValue0(20200720) == 0L || hp.getSkillCustomValue0(20200720) == 1L
								|| hp.getSkillCustomValue0(20200720) == 2L) {
					...
					if ((new Date()).getHours() == 22 && (new Date()).getMinutes() == 0) {
						...
						if (hp.getSkillCustomValue0(20200720) == 0L || hp.getSkillCustomValue0(20200720) == 1L
								|| hp.getSkillCustomValue0(20200720) == 2L || hp.getSkillCustomValue0(20200720) == 3L) {
	...
	public static void reset(Iterator<ChannelServer> channels) {
		...
		for (MapleGuild g : World.Guild.getGuilds()) {
			...
			GregorianCalendar clear = new GregorianCalendar((int) keys / 10000, (int) (keys % 10000L / 100L) - 1,
					(int) keys % 100);
			...
			for (int i = 0; i < n; ++i) {
				...
				g.broadcast(
						CWvsContext.GuildPacket.guildSkillPurchased(g.getId(), skillid, 0, -1L, "메이플 운영자", "메이플 운영자"));
			}
			g.writeToDB(false);
		...
		if (Calendar.getInstance().get(7) == 2) {
			...
			try {
				...
				preparedStatement1 = connection
						.prepareStatement("SELECT * FROM dojorankings order by floor DESC, time DESC", 1005, 1008);
				...
				for (int i = 1; i <= resultSet1.getRow(); i++) {
					...
					if (World.getChar(id) != null) {
						World.getChar(id).gainCabinetItemPlayer(itemid, 1, 7,
								"무릉도장 랭커 보상 입니다. 보관 기간 내에 수령하지 않을 시 보관함에서 사라집니다.");
						World.getChar(id).gainCabinetItemPlayer(itemid2, 1, 7,
								"무릉도장 랭커 보상 입니다. 보관 기간 내에 수령하지 않을 시 보관함에서 사라집니다.");
						World.getChar(id).setKeyValue(100466, "Score", "0");
						World.getChar(id).setKeyValue(100466, "Floor", "0");
					} else {
						ps1 = connection.prepareStatement(
								"UPDATE questInfo SET customData = ? WHERE quest = ? and characterid = ?");
	...
	static {
		...
		MapleSaveHandler.weekKeyValues = new String[][] { { "db_lastweek", "0" }, { "dojo", "0" },
				{ "dojo_time", "0" } };
		MapleSaveHandler.clientDateKeyValues = new String[] { "dailyGiftComplete", "hotelMapleToday", "ht", "mPark",
				"mpark_t", "jump_1", "jump_2", "jump_3", "day_reborn_1", "day_reborn_2", "day_qitem", "day_summer_a",
				"day_summer_e", "day_colorlens", "day_MedalC", "day_MedalD", "minigame" };
