...
function start(mode, type, selection) {
	...
	if (mode != 1) {
	    if(type == 1 && mode == 0)
			...
			qm.sendNext("어, 뭐야. 음흉한 여우가 무서운 거야? 내 동생이 이렇게 겁쟁이일 줄이야.");
			qm.dispose();
			return;
		}
	}
	if (status == 0)
		qm.sendAcceptDecline("좀 이상하지 않아? 요새 닭들이 예전 같지 않잖아. 예전에는 달걀을 이거보다 훨씬 많이 낳았는데 점점 줄어들고 있어. 혹시 여우가 많아진 게 원인일까? 그렇다면 얼른 손을 써야 할텐데. 안 그래?");
	else if (status == 1){
		qm.forceStartQuest();
...
function end(mode, type, selection) {
	...
	if (status == 12){
		...
		qm.gainExp(910);
		qm.sendNextPrev("#b마법사들의 공격무기인 완드#k야. 뭐, 딱히 너한테 필요한 물건은 아닐 것 같지만 들고 다니면 멋지긴 할 거야. 아하하.");
	} if (status == 13){
