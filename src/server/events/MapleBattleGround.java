...
public class MapleBattleGround {
	...
	// 定义一个静态方法 Attack，用于处理攻击逻辑。接收一个 LittleEndianAccessor 对象 slea 用于读取数据包，以及一个
	// MapleClient 对象 c 表示客户端
	public static void Attack(LittleEndianAccessor slea, MapleClient c) {// 攻击逻辑
		// 声明一个存储三元组的列表 info，三元组包含三个整数类型的数据，用于存储技能相关的信息
		List<Triple<Integer, Integer, Integer>> info;
		// 声明一个整数变量 cooltime，用于存储技能的冷却时间
		int cooltime;
		// 声明一个存储二元组的列表 coollist，二元组包含两个整数类型的数据，可能用于存储冷却相关的信息
		List<Pair<Integer, Integer>> coollist;
		// 声明一个存储整数的列表 mob，用于存储被攻击的怪物对象 ID
		List<Integer> mob = new ArrayList<>();
		// 初始化一些变量，用于存储攻击的相关属性
		int range = 0; // 攻击范围
		int attackimg = 0; // 攻击动画的图像 ID
		int imgafter = 0; // 攻击后特效的持续时间
		int speed = 0; // 攻击速度
		int delay = 400; // 攻击延迟时间，初始值为 400
		int delay2 = 0; // 第二个攻击延迟时间
		int cool = 0; // 技能冷却时间
		int stack = 0; // 技能叠加层数
		...
		// 检查技能 ID 是否为 80001739 且玩家的该技能自定义值不为空
		if (skillid == 80001739 && c.getPlayer().getSkillCustomValue(skillid) != null) {
			// 如果满足条件，向客户端发送允许玩家进行操作的数据包
			c.send(CWvsContext.enableActions(c.getPlayer()));
			// 直接返回，结束当前方法
			return;
		}

		// 从数据包中读取一个字节的数据，但未对该数据进行使用
		slea.readByte();

		// 从数据包中读取一个字节的数据，用于表示攻击方向（可能 0 表示右，1 表示左）
		int left = slea.readByte();

		// 从数据包中读取技能等级
		int skilllv = slea.readInt();

		// 从数据包中读取一个整数坐标点，作为攻击前玩家的位置
		Point oldpos = slea.readIntPos();

		// 从数据包中读取一个整数坐标点，作为攻击后玩家的位置
		Point newpos = slea.readIntPos();

		// 从数据包中读取一个整数，表示第一批被攻击怪物对象 ID 的数量
		int psize = slea.readInt();

		// 循环读取第一批被攻击怪物的对象 ID，并添加到 mob 列表中
		for (int i = 0; i < psize; i++) {
			mob.add(Integer.valueOf(slea.readInt()));
		}

		// 从数据包中读取一个整数，表示第二批被攻击怪物对象 ID 的数量
		int size = slea.readInt();

		// 循环读取第二批被攻击怪物的对象 ID，并添加到 mob 列表中
		for (int j = 0; j < size; j++) {
			mob.add(Integer.valueOf(slea.readInt()));
		}

		// 从数据包中读取一个整数，但未对该数据进行使用
		slea.readInt();

		// 从数据包中读取一个整数，表示玩家是否在移动状态
		int moveing = slea.readInt();
		// 后续可能会根据不同的技能 ID 对上述变量进行赋值和处理，以实现不同技能的攻击逻辑
		...
		case 80001733:
			...
			c.getPlayer().getMap().broadcastMessage(c.getPlayer(),
					BattleGroundPacket.UpdateAvater(c.getPlayer().getBattleGroundChr(),
							GameConstants.BattleGroundJobType(c.getPlayer().getBattleGroundChr())),
					false);
			c.getPlayer().getBattleGroundChr().setTeam(1);
			c.getPlayer().getClient().send(BattleGroundPacket.UpdateAvater(c.getPlayer().getBattleGroundChr(),
					GameConstants.BattleGroundJobType(c.getPlayer().getBattleGroundChr())));
			break;
		case 80001734:
			attackimg = 1175;
		...
		case 80003002:
			...
			c.getPlayer().getBattleGroundChr().setTeam(2);
			c.getPlayer().getMap().broadcastMessage(c.getPlayer(),
					BattleGroundPacket.UpdateAvater(c.getPlayer().getBattleGroundChr(), 24), false);
			c.getPlayer().getBattleGroundChr().setTeam(1);
			...
			c.getPlayer().getBattleGroundChr().getSkillList()
					.add(new Triple<>(Integer.valueOf(2), Integer.valueOf(1), Integer.valueOf(1)));
			c.getPlayer().getBattleGroundChr().getSkillList()
					.add(new Triple<>(Integer.valueOf(0), Integer.valueOf(80001678), Integer.valueOf(1)));
			c.getPlayer().getBattleGroundChr().getSkillList()
					.add(new Triple<>(Integer.valueOf(0), Integer.valueOf(80003001), Integer.valueOf(1)));
			c.getPlayer().getBattleGroundChr().getSkillList()
					.add(new Triple<>(Integer.valueOf(0), Integer.valueOf(80003002), Integer.valueOf(0)));
			c.getPlayer().getBattleGroundChr().getSkillList()
					.add(new Triple<>(Integer.valueOf(0), Integer.valueOf(80003003), Integer.valueOf(0)));
			c.getPlayer().getBattleGroundChr().getSkillList()
					.add(new Triple<>(Integer.valueOf(0), Integer.valueOf(80003004), Integer.valueOf(0)));
			c.getPlayer().getBattleGroundChr().getSkillList()
					.add(new Triple<>(Integer.valueOf(0), Integer.valueOf(80003005), Integer.valueOf(0)));
			c.getPlayer().getBattleGroundChr().getSkillList()
					.add(new Triple<>(Integer.valueOf(0), Integer.valueOf(80003006), Integer.valueOf(0)));
			c.getPlayer().getBattleGroundChr().getSkillList()
					.add(new Triple<>(Integer.valueOf(0), Integer.valueOf(80003007), Integer.valueOf(0)));
			c.getPlayer().getBattleGroundChr().getSkillList()
					.add(new Triple<>(Integer.valueOf(0), Integer.valueOf(80003008), Integer.valueOf(0)));
			c.getPlayer().getBattleGroundChr().getSkillList()
					.add(new Triple<>(Integer.valueOf(0), Integer.valueOf(80003009), Integer.valueOf(0)));
			c.getPlayer().getBattleGroundChr().getSkillList()
					.add(new Triple<>(Integer.valueOf(0), Integer.valueOf(80003010), Integer.valueOf(0)));
			c.getPlayer().getBattleGroundChr().getSkillList()
					.add(new Triple<>(Integer.valueOf(0), Integer.valueOf(80003011), Integer.valueOf(0)));
			c.getPlayer().getBattleGroundChr().getSkillList()
					.add(new Triple<>(Integer.valueOf(0), Integer.valueOf(80003012), Integer.valueOf(0)));
			c.getPlayer().getBattleGroundChr().getSkillList()
					.add(new Triple<>(Integer.valueOf(0), Integer.valueOf(80003013), Integer.valueOf(1)));
			...
			if (c.getPlayer().getBattleGroundChr().getLevel() >= 8) {
				info.add(new Triple<>(Integer.valueOf(80003005),
						Integer.valueOf((c.getPlayer().getBattleGroundChr().getLevel() >= 11) ? 2 : 1),
						Integer.valueOf(50000)));
				info.add(new Triple<>(Integer.valueOf(80003012),
						Integer.valueOf((c.getPlayer().getBattleGroundChr().getLevel() >= 11) ? 2 : 1),
						Integer.valueOf(50000)));
			}
			...
			for (Pair<Integer, Integer> cooll : coollist) {
				if (c.getPlayer().skillisCooling(((Integer) cooll.getLeft()).intValue())) {
					c.send(BattleGroundPacket.CoolDown(((Integer) cooll.getLeft()).intValue(),
							((Integer) cooll.getRight()).intValue() * 1000,
							(int) ((((Integer) cooll.getRight()).intValue() * 1000)
									- c.getPlayer().getCooldownLimit(((Integer) cooll.getLeft()).intValue()))));
					if (((Integer) cooll.getLeft()).intValue() == 80003012) {
						c.send(BattleGroundPacket.CoolDown(80003005, ((Integer) cooll.getRight()).intValue() * 1000,
								(int) ((((Integer) cooll.getRight()).intValue() * 1000)
										- c.getPlayer().getCooldownLimit(((Integer) cooll.getLeft()).intValue()))));
					}
				}
		...
		case 80003007:
			...
			c.getPlayer().getBattleGroundChr().setTeam(2);
			c.getPlayer().getMap().broadcastMessage(c.getPlayer(),
					BattleGroundPacket.UpdateAvater(c.getPlayer().getBattleGroundChr(), 23), false);
			c.getPlayer().getBattleGroundChr().setTeam(1);
			...
			c.getPlayer().getBattleGroundChr().getSkillList()
					.add(new Triple<>(Integer.valueOf(0), Integer.valueOf(80003001), Integer.valueOf(1)));
			c.getPlayer().getBattleGroundChr().getSkillList()
					.add(new Triple<>(Integer.valueOf(0), Integer.valueOf(80003002), Integer.valueOf(1)));
			c.getPlayer().getBattleGroundChr().getSkillList()
					.add(new Triple<>(Integer.valueOf(0), Integer.valueOf(80003003), Integer.valueOf(0)));
			c.getPlayer().getBattleGroundChr().getSkillList()
					.add(new Triple<>(Integer.valueOf(0), Integer.valueOf(80003004), Integer.valueOf(0)));
		...
		if (stack > 0) {
			c.getPlayer().getMap().broadcastMessage(BattleGroundPacket.AttackSkillStack(c.getPlayer(), oldpos, newpos,
					skillid, attackimg, delay, delay2, imgafter, speed, left, range, stack));
		} else {
			c.getPlayer().getMap().broadcastMessage(BattleGroundPacket.AttackSkill(c.getPlayer(), oldpos, newpos,
					skillid, attackimg, delay, delay2, imgafter, speed, left, range, mob, moveing));
		}
	...
	public static void AttackRefresh(LittleEndianAccessor slea, MapleClient c) {
		...
		if (stype == 1) {
			...
			c.getPlayer().getMap().broadcastMessage(
					BattleGroundPacket.AttackRefresh(c.getPlayer(), stype, count, type, new int[] { 0 }));
		} else if (stype == 2) {
			...
			c.getPlayer().getMap().broadcastMessage(
					BattleGroundPacket.AttackRefresh(c.getPlayer(), stype, unk1, unk2, new int[] { unk3, unk4 }));
		}
	}
	...
	public static void AttackDamage(LittleEndianAccessor slea, MapleClient c) {
		...
		case 80001738:
			...
			if (c.getPlayer().getSkillCustomValue0(skillid) >= 3L) {
				c.getPlayer().removeSkillCustomInfo(skillid);
				Item toDrop = new Item((c.getPlayer().getBattleGroundChr().getTeam() == 1) ? 4001849 : 4001847,
						(short) 0, (short) 1, 0);
				c.getPlayer().getMap().spawnItemDrop(c.getPlayer(), c.getPlayer(), toDrop,
						new Point((c.getPlayer().getTruePosition()).x, (c.getPlayer().getTruePosition()).y), true,
						false);
			}
			break;
		}
		...
		for (i = 0; i < peoplecount; i++) {
			...
			int basedam = Randomizer.rand(c.getPlayer().getBattleGroundChr().getMindam(),
					c.getPlayer().getBattleGroundChr().getMaxdam());
			...
			if (chred != null) {
				...
				if (chred.getChr().getBuffedValue(80001655) && chred.getChr().getSkillCustomValue0(80001655) > 0L
						&& damage > 0) {
					int shield = (int) chred.getChr().getSkillCustomValue0(80001655);
					if (shield - damage > 0) {
						chred.getChr().setSkillCustomInfo(80001655,
								chred.getChr().getSkillCustomValue0(80001655) - damage, 0L);
						damage = 1;
					} else {
						damage -= shield;
				...
				if (chred.getChr().getBuffedValue(80001654)) {
					...
					c.getPlayer().getMap().broadcastMessage(CField.updatePartyMemberHP(c.getPlayer().getId(),
							c.getPlayer().getBattleGroundChr().getHp(), c.getPlayer().getBattleGroundChr().getMaxHp()));
					c.send(CWvsContext.updatePlayerStats(statup, false, c.getPlayer()));
					if (c.getPlayer().getBattleGroundChr().getHp() <= 0
							&& c.getPlayer().getBattleGroundChr().isAlive()) {
						...
						if (!BattleGroundGameHandler.isEndOfGame()) {
							...
							if (alivechr == 0) {
								...
								chred.getChr().getClient()
										.send(CField.environmentChange("Map/Effect2.img/PvP/Win", 16));
								chred.getChr().getClient().send(SLFCGPacket.playSE("Sound/MiniGame.img/BattlePvp/Win"));
								Timer.EtcTimer.getInstance().schedule(() -> {
									c.getPlayer().getMap().broadcastMessage(CField.UIPacket
						...
						c.getPlayer().getMap().broadcastMessage(CWvsContext
								.getTopMsg("[" + chred.getName() + "]님이 [" + c.getPlayer().getName() + "]님을 제압 했습니다."));
						MapleBattleGroundCharacter custom = chred;
						custom.setTeam(2);
						chred.getChr().getMap().broadcastMessage(chred.getChr(),
								BattleGroundPacket.UpdateAvater(custom, GameConstants.BattleGroundJobType(custom)),
								false);
						chred.getChr().getClient()
								.send(BattleGroundPacket.UpdateAvater(chred, GameConstants.BattleGroundJobType(chred)));
						custom = c.getPlayer().getBattleGroundChr();
						custom.setTeam(2);
						c.getPlayer().getMap().broadcastMessage(c.getPlayer(),
								BattleGroundPacket.UpdateAvater(custom, GameConstants.BattleGroundJobType(custom)),
								false);
						c.send(BattleGroundPacket.UpdateAvater(c.getPlayer().getBattleGroundChr(),
								GameConstants.BattleGroundJobType(c.getPlayer().getBattleGroundChr())));
					}
				}
				boolean debuff = true;
				if (chred.getChr().getBuffedValue(80001740)) {
					...
					chred.getChr().getMap().broadcastMessage(BattleGroundPacket.TakeDamageEffect(chred,
							(int) chred.getChr().getSkillCustomValue0(80001740)));
				...
				MapleBattleGroundMobInfo minfo2 = new MapleBattleGroundMobInfo(oid, cid, skid, damage, slea.readInt(),
						slea.readInt(), slea.readInt(), slea.readByte(), slea.readIntPos(), slea.readIntPos());
				if (!BattleGroundGameHandler.isNotDamage()) {
					...
					minfo.add(minfo2);
					c.getPlayer().getMap()
							.broadcastMessage(BattleGroundPacket.AttackDamage(c.getPlayer(), pos1, pos2, pos3, pos4,
									pos5, minfo, true, new int[] { attackcount, skillid, unk1, unk2, unk, unk4, unk5,
											unk6, unk7, unk8, unk9, unk10, unk11, unk12, 1 }));
					chred.setHp(chred.getHp() - damage);
					chred.getChr().getMap().broadcastMessage(
							CField.updatePartyMemberHP(chred.getChr().getId(), chred.getHp(), chred.getMaxHp()));
					Map<MapleStat, Long> statup = new EnumMap<>(MapleStat.class);
					...
					if (chred.getHp() <= 0 && chred.isAlive()) {
						...
						c.getPlayer().getMap().broadcastMessage(CWvsContext
								.getTopMsg("[" + c.getPlayer().getName() + "]님이 [" + chred.getName() + "]님을 제압 했습니다."));
						if (!BattleGroundGameHandler.isEndOfGame()) {
							c.getPlayer().getBattleGroundChr()
									.setMoney(c.getPlayer().getBattleGroundChr().getMoney() + 500);
						} else {
							...
							if (alivechr == 0) {
								...
								c.getPlayer().addKV("BattlePVPLevel",
										c.getPlayer().getBattleGroundChr().getLevel() + "");
								c.getPlayer().addKV("BattlePVPKill", c.getPlayer().getBattleGroundChr().getKill() + "");
								c.send(CField.environmentChange("Map/Effect2.img/PvP/Win", 16));
								c.send(SLFCGPacket.playSE("Sound/MiniGame.img/BattlePvp/Win"));
								c.getPlayer().getMap()
										.broadcastMessage(SLFCGPacket.playSE("Sound/MiniGame.img/BattlePvp/Win"));
								Timer.EtcTimer.getInstance().schedule(() -> {
									c.getPlayer().getMap().broadcastMessage(CField.UIPacket
											.detailShowInfo("게임이 종료 되었습니다 잠시 후 퇴장맵으로 이동 됩니다.", 3, 20, 20));
									c.getPlayer().getMap().broadcastMessage(CField.getClock(5));
						...
						case 80002673:
							...
							c.getPlayer().setSkillCustomInfo(skillid, c.getPlayer().getSkillCustomValue0(skillid) - 1L,
									0L);
							break;
						}
					}
		...
		for (i = 0; i < mobcount; i++) {
			...
			if (monster != null) {
				...
				if (monster.getStats().getLevel() > c.getPlayer().getBattleGroundChr().getLevel()) {
					...
					int percent = (chi == 1) ? 85
							: ((chi == 2) ? 70
									: ((chi == 3) ? 50
											: ((chi == 4) ? 40 : ((chi == 5) ? 25 : ((chi == 6) ? 10 : 0)))));
					if (chi >= 7) {
						damage = 1;
					} else {
				...
				switch (skillid) {
				case 80001649:
					applys.add(new Pair<>(MonsterStatus.MS_Stun,
							new MonsterStatusEffect(skillid, effect.getSubTime(), 1L)));
					break;
				case 80001675:
					dam = (c.getPlayer().getBattleGroundChr().getLevel() >= 11) ? 60
							: ((c.getPlayer().getBattleGroundChr().getLevel() >= 9) ? 50
									: ((c.getPlayer().getBattleGroundChr().getLevel() >= 7) ? 40
											: ((c.getPlayer().getBattleGroundChr().getLevel() >= 5) ? 30 : 20)));
				...
				case 80002338:
					if (monster.getBuff(MonsterStatus.MS_Stun) == null) {
						if (monster.getBuff(MonsterStatus.MS_PVPRude_Stack) == null
								&& monster.getCustomValue0(skillid) > 0L) {
						...
						applys.add(new Pair<>(MonsterStatus.MS_PVPRude_Stack,
								new MonsterStatusEffect(skillid, 6000, monster.getCustomValue0(skillid))));
					}
					break;
				}
				...
				minfo.add(minfo2);
				c.getPlayer().getMap()
						.broadcastMessage(BattleGroundPacket.AttackDamage(c.getPlayer(), pos1, pos2, pos3, pos4, pos5,
								minfo, false, new int[] { attackcount, skillid, unk1, unk2, unk, unk4, unk5, unk6, unk7,
										unk8, unk9, unk10, unk11, unk12, 0 }));
	...
	public static void TakeDamage(LittleEndianAccessor slea, MapleClient c) {
		...
		if (monster != null) {
			randdam = Randomizer.rand(monster.getStats().getPhysicalAttack() - 30,
					monster.getStats().getPhysicalAttack());
			if (c.getPlayer().getBuffedValue(80001655) && c.getPlayer().getSkillCustomValue0(80001655) > 0L) {
				...
				if (shield - randdam > 0) {
					c.getPlayer().setSkillCustomInfo(80001655, c.getPlayer().getSkillCustomValue0(80001655) - randdam,
							0L);
					randdam = 1;
				} else {
					randdam -= shield;
			...
			if (c.getPlayer().getBuffedValue(80001740) && healed) {
				...
				c.getPlayer().getMap()
						.broadcastMessage(BattleGroundPacket.TakeDamage(c.getPlayer(), oid, randdam, type, heal));
				c.getPlayer().getMap().broadcastMessage(BattleGroundPacket.TakeDamageEffect(
						c.getPlayer().getBattleGroundChr(), (int) c.getPlayer().getSkillCustomValue0(80001740)));
				...
				c.getPlayer().getMap()
						.broadcastMessage(BattleGroundPacket.TakeDamage(c.getPlayer(), oid, randdam, type, 0));
			}
		}
	}
	...
	public static void MainTime(LittleEndianAccessor slea, MapleClient c) {
		if (c.getPlayer().getBattleGroundChr() != null && c.getPlayer().getBattleGroundChr().getHp() > 0) {
			...
			if (c.getPlayer().getBuffedValue(80002341)) {
				c.getPlayer().getBattleGroundChr().setHp(c.getPlayer().getBattleGroundChr().getHp()
						+ c.getPlayer().getBattleGroundChr().getMaxHp() / 100 * 3);
			}
			if (c.getPlayer().getBuffedValue(80002671)) {
				c.getPlayer().getBattleGroundChr().setHp(c.getPlayer().getBattleGroundChr().getHp()
						+ c.getPlayer().getBattleGroundChr().getMaxHp() / 100 * 8);
			}
			if (heal) {
				if (c.getPlayer().getBattleGroundChr().getHp() < c.getPlayer().getBattleGroundChr().getMaxHp()) {
					c.getPlayer().getBattleGroundChr().setHp(c.getPlayer().getBattleGroundChr().getHp()
							+ c.getPlayer().getBattleGroundChr().getHpRegen());
				}
				if (c.getPlayer().getBattleGroundChr().getMp() < c.getPlayer().getBattleGroundChr().getMaxMp()) {
					c.getPlayer().getBattleGroundChr().setMp(c.getPlayer().getBattleGroundChr().getMp()
							+ c.getPlayer().getBattleGroundChr().getMpRegen());
				}
				if (c.getPlayer().getBattleGroundChr().getMp() > c.getPlayer().getBattleGroundChr().getMaxMp()) {
	...
	public static void UpgradeSkill(LittleEndianAccessor slea, MapleClient c) {
		...
		slea.skip(4);
		slea.skip(4);
		skilllevel = (slot == 0) ? chr.getAttackUp()
				: ((slot == 1) ? chr.getHpMpUp()
						: ((slot == 2) ? chr.getCriUp()
								: ((slot == 3) ? chr.getSpeedUp() : ((slot == 4) ? chr.getRegenUp() : 0))));
		switch (skilllevel) {
		case 1:
			need = 460;
			break;
		...
		if (chr != null && chr.getHp() > 0 && need <= chr.getMoney()) {
			if (slot == 0) {
				int attackup = (chr.getAttackUp() == 1) ? 2
						: ((chr.getAttackUp() == 2) ? 5
								: ((chr.getAttackUp() == 3) ? 10
										: ((chr.getAttackUp() == 4) ? 15 : ((chr.getAttackUp() == 5) ? 20 : 0))));
				...
				int hpmpup = (chr.getHpMpUp() == 1) ? 2
						: ((chr.getHpMpUp() == 2) ? 7
								: ((chr.getHpMpUp() == 3) ? 15
										: ((chr.getHpMpUp() == 4) ? 20 : ((chr.getHpMpUp() == 5) ? 30 : 0))));
				chr.setSkillHPUP(chr.getMaxHp() / 100 * hpmpup);
				chr.setSkillMPUP(chr.getMaxMp() / 100 * hpmpup);
				chr.setHpMpUp(chr.getHpMpUp() + 1);
			} else if (slot == 2) {
				int criup = (chr.getCriUp() == 1) ? 3
						: ((chr.getCriUp() == 2) ? 6
								: ((chr.getCriUp() == 3) ? 12
										: ((chr.getCriUp() == 4) ? 18 : ((chr.getCriUp() == 5) ? 25 : 0))));
				chr.setCritical(criup);
				chr.setCriUp(chr.getCriUp() + 1);
			} else if (slot == 3) {
				int attackspeed = (chr.getSpeedUp() == 5) ? -20 : -10;
				int speed = (chr.getSpeedUp() == 1) ? 6
						: ((chr.getSpeedUp() == 2) ? 6
								: ((chr.getSpeedUp() == 3) ? 12
										: ((chr.getSpeedUp() == 4) ? 12 : ((chr.getSpeedUp() == 5) ? 12 : 0))));
				int jump = (chr.getSpeedUp() == 1) ? 2
						: ((chr.getSpeedUp() == 2) ? 4
								: ((chr.getSpeedUp() == 3) ? 2
										: ((chr.getSpeedUp() == 4) ? 4 : ((chr.getSpeedUp() == 5) ? 3 : 0))));
				...
				int hpup = (chr.getHpMpUp() == 1) ? 14
						: ((chr.getHpMpUp() == 2) ? 17
								: ((chr.getHpMpUp() == 3) ? 20
										: ((chr.getHpMpUp() == 4) ? 23 : ((chr.getHpMpUp() == 5) ? 25 : 0))));
				int mpup = (chr.getHpMpUp() == 1) ? 6
						: ((chr.getHpMpUp() == 2) ? 7
								: ((chr.getHpMpUp() == 3) ? 8
										: ((chr.getHpMpUp() == 4) ? 10 : ((chr.getHpMpUp() == 5) ? 11 : 0))));
			...
			chr.getChr().getMap()
					.broadcastMessage(BattleGroundPacket.UpgradeSkillEffect(chr, slot * 100 + skilllevel + 1));
	...
	public static void SelectAvater(LittleEndianAccessor slea, MapleClient c) {
		...
		if (chr.getBattleGroundChr() == null) {
			...
			chr.getMap()
					.broadcastMessage(BattleGroundPacket.ChangeAvater(gchr, GameConstants.BattleGroundJobType(gchr)));
			...
			t.setTeam(2);
			chr.getMap().broadcastMessage(chr, BattleGroundPacket.ChangeAvater(t, GameConstants.BattleGroundJobType(t)),
					false);
			chr.getClient().send(CField.ImageTalkNpc(9001153, 4000,
					"싱크로 완료. 몬스터와 보스 몬스터를 처치해 더욱 강해질 수 있습니다.\r\n게임시작 후 1분간은 #r무적#k 상태 입니다."));
		}
	}
}
