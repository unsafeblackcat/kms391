	...
	public void loadFromDb(int accId) throws SQLException {
		...
		try {
			...
			ps = con.prepareStatement(
					"SELECT u.unk1, u.unk3, sum(ive.enhance) as starforce, c.name as unionname, c.id as id, c.job as unionjob, c.level as unionlevel, u.position, u.position2, u.priset, u.priset1, u.priset2, u.priset3, u.priset4, u.pos, u.pos1, u.pos2, u.pos3, u.pos4 FROM unions as u, characters as c, inventoryitems as iv, inventoryequipment as ive WHERE c.id = u.id && c.accountid = ? && characterid = c.id && iv.position < 0 && ive.inventoryitemid = iv.inventoryitemid group by unionname");
			ps.setInt(1, accId);
			rs = ps.executeQuery();
			while (rs.next()) {
				if (GameConstants.isZero(rs.getInt("unionjob")) ? (rs.getInt("unionlevel") < 130)
						: (rs.getInt("unionlevel") < 60)) {
					continue;
				}
				this.unions.add(new MapleUnion(rs.getInt("id"), rs.getInt("unionlevel"), rs.getInt("unionjob"),
						rs.getInt("unk1"), rs.getInt("position2"), rs.getInt("position"), rs.getInt("unk3"),
						rs.getString("unionname"), rs.getInt("starforce"), rs.getInt("priset"), rs.getInt("priset1"),
						rs.getInt("priset2"), rs.getInt("priset3"), rs.getInt("priset4"), rs.getInt("pos"),
						rs.getInt("pos1"), rs.getInt("pos2"), rs.getInt("pos3"), rs.getInt("pos4")));
			}
			rs.close();
			ps.close();
			con.close();
	...
	public void savetoDB(Connection con, int accId) throws SQLException {
		...
		try {
			for (MapleUnion union : this.unions) {
				...
				ps.close();
				ps = con.prepareStatement(
						"INSERT INTO unions (id, unk1, unk3, position, position2, priset, priset1, priset2, priset3, priset4, pos, pos1, pos2, pos3, pos4) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
				ps.setInt(1, union.getCharid());
