...
function action(mode, type, sel) {
	...
	if (status == 0) {
		var msg ="#fs15#안녕하세요 #b#h 0##k님! 잠재능력이 없는 아이템에 잠재능력 부여가 가능하다는 사실을 알고 계시나요? #fs15##b"+enter;
		msg += "#L1#뱃지 아이템 잠재능력 부여"+enter;
		msg += "#L2#포켓 아이템 잠재능력 부여"+enter;
		cm.sendSimple(msg);
	} else if (status == 1) {
		seld = sel;
		var msg = "#fs15# 어떤 포인트를 이용해 잠재능력을 부여하시겠습니까?\r\n";
		...
		var 
                        msg ="#fs15#안녕하세요 #b#h 0##k님! "+seldi+"에 잠재능력 부여가 가능하다는 사실을 알고 계시나요? 원하시는 아이템을 골라주세요.\r\n#r(에디셔널은 개방되지 않습니다)#k"+enter;
		msg += "#fs15#현재 #b#h 0##k님의 #d"+pt+"포인트#k는 "+p+"P 입니다.#fs15#"+enter+enter;
		switch (seld) {
			case 1:
