...
function start(mode, type, selection) {
	...
	if (status == 7)
		qm.PlayerToNpc("#b에에에엑! 자, 잠깐! 잘은 모르겠지만 그 말은... 내가 무조건 널 돌봐줘야 한다는 거야?");
	if (status == 8)
	...
	if (status == 13){
		qm.forceStartQuest();
		qm.sendNext("당장 내게 #r돼지#k들을 공격하라는 명령을 내려! 그럼 단번에 돼지를 잡아 보이겠어! 드래곤으로서의 내 능력을 증명해 보일테다! 자, 돌격!");
	}if (status == 14)
		qm.sendNextPrev("아, 아니지 잠깐! 그 전에 먼저 AP 분배는 한 거야? 난 마스터가 가진 #bINT와 LUK#k에 영향을 받아! 내 진정한 능력을 보고 싶거든 AP를 제대로 분배하고 #b마법사 장비 착용#k 후에 명령해!");
	if (status == 15){
...
function end(mode, type, selection) {
	...
	if(status == 0)
		qm.sendOk("음핫핫핫! 어떠냐? 내 능력이! 너, 아니 마스터는 앞으로 내가가진 이 모든 능력을 마음대로 사용할 수 있게 되었다고! 계약자란 건 그런 거니까! 어때? 훌륭하지? 멋지지? 이 몸을 마구 우러러보고 싶어지지? ");
	if(status == 1){
		qm.forceCompleteQuest();
		qm.gainExp(900);
		qm.getPlayer().gainSP(1, 0);
		qm.sendOk("아... 근데 깨어난 지 얼마 되지도 않은 상태로 힘을 썼더니 배가 고파...");
		qm.dispose();
	}
}