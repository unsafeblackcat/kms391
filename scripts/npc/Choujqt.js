...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (selection == 0) { // 사냥이벤트 아이템교환
            ...
            cm.dispose();
            cm.openNpcCustom(cm.getClient(), 9063144, "JYJPCZ");
        } else if (selection == 2) { //핫타임보상
            cm.dispose();
            cm.openNpcCustom(cm.getClient(), 9063144, "specialchair");
        } else if (selection == 3) { // 일퀘
            cm.dispose();
            cm.openNpcCustom(cm.getClient(), 9063144, "Chenhaocq");
        } else if (selection == 4) { // 후원룰렛
            cm.dispose();
            cm.openNpcCustom(cm.getClient(), 9063144, "Hoonjang2");
        } else if (selection == 5) { // 어빌리티
            cm.dispose();
            cm.openNpcCustom(cm.getClient(), 9063144, "NEOCH");
        } else if (selection == 6) { //V 코어 매트릭스
            cm.dispose();
            cm.openNpcCustom(cm.getClient(), 9063144, "NEOHZ");
        } else if (selection == 7) { //자유 전직
            cm.dispose();
            cm.openNpcCustom(cm.getClient(), 9063144, "ZJMBCZ");
