	...
	private void reload() {
		...
		for (MapleGuild g : World.Guild.getGuilds()) {
			if (g.getWeekReputation() > 0) {
				honorranks.add(
						new Triple<>(g.getName(), Integer.valueOf(g.getWeekReputation()), Integer.valueOf(g.getId())));
			}
			if (g.getGuildScore() > 0.0D) {
				culvertRanks.add(new Triple<>(g.getName(), Integer.valueOf((int) g.getGuildScore()),
						Integer.valueOf(g.getId())));
			}
		}
		int i;
		for (i = 0; i < honorranks.size() - 1; i++) {
			for (int j = 0; j < honorranks.size() - i - 1; j++) {
				if (((Integer) ((Triple) honorranks.get(j)).getMid())
						.intValue() < ((Integer) ((Triple) honorranks.get(j + 1)).getMid()).intValue()) {
		...
		for (i = 0; i < culvertRanks.size() - 1; i++) {
			for (int j = 0; j < culvertRanks.size() - i - 1; j++) {
				if (((Integer) ((Triple) culvertRanks.get(j)).getMid())
						.intValue() < ((Integer) ((Triple) culvertRanks.get(j + 1)).getMid()).intValue()) {
		...
		for (Triple<String, Integer, Integer> list : honorranks) {
			this.honorRank.add(new GuildRankingInfo(list.getLeft(), ((Integer) list.getMid()).intValue(),
					((Integer) list.getRight()).intValue()));
		}
		for (Triple<String, Integer, Integer> list : culvertRanks) {
			this.culvertRank.add(new GuildRankingInfo(list.getLeft(), ((Integer) list.getMid()).intValue(),
					((Integer) list.getRight()).intValue()));
		}
	}
