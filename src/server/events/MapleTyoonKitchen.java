...
public class MapleTyoonKitchen {
	...
	public static void startPartyGame(MapleCharacter chr) {
		if (chr.getParty() != null) {
					...
					if (pm.getPlayer() != null) {
						...
						for (Pair<SecondaryStat, SecondaryStatValueHolder> data : pm.getPlayer().getEffects()) {
							...
							if (SkillFactory.getSkill(mbsvh.effect.getSourceId()) != null
									&& mbsvh.effect.getSourceId() != 80002282
									&& mbsvh.effect.getSourceId() != 2321055) {
								pm.getPlayer().cancelEffect(mbsvh.effect,
			...
			chr.getMap().broadcastMessage(
					CField.environmentChange("UI/TyoonKitchen_UI.img/TyoonKitchen_UI/effect/countdown", 16));
	...
	public class MapleTyoonKitchenRecipe {
		...
		public void reset(boolean first, int number, boolean update) {
			...
			int maketime = 0;
			if (level == 1) {
				this.RecipeInfo.add(new Triple<>(Integer.valueOf(Randomizer.rand(2024020, 2024024)),
						Integer.valueOf(number), Integer.valueOf(0)));
				this.RecipeInfo.add(new Triple<>(Integer.valueOf(Randomizer.rand(2024020, 2024024)),
						Integer.valueOf(number), Integer.valueOf(0)));
				this.RecipeInfo.add(new Triple<>(Integer.valueOf(Randomizer.rand(2024025, 2024027)),
						Integer.valueOf(number), Integer.valueOf(0)));
				this.RecipeInfo.add(new Triple<>(Integer.valueOf(Randomizer.rand(2024020, 2024024)),
						Integer.valueOf(number), Integer.valueOf(0)));
				this.money = 3000;
				...
				setMakeTime(Timer.EventTimer.getInstance().schedule(() -> {
					...
					if (this.RecipeInfo != null) {
						if (MapleTyoonKitchen.this.cooker.getMapId() != 993194500) {
							...
							String[] msg = { "방금 손님은 다시는 우리 가게에 오지 않겠군.", "기다리고 있는 손님이 불쌍하지도 않나?", "이대로라면 식당이 곧 파산하겠군.",
									"주문이 취소되었네!" };
							MapleTyoonKitchen.this.cooker.getMap()
									.startMapEffect(msg[Randomizer.rand(0, msg.length - 1)], 5120216, 4000);
							...
							MapleTyoonKitchen.this.cooker.getMap()
									.broadcastMessage(CField.environmentMove("cookPlate" + (getMakenum() + 1), 0));
							getMakeTime().cancel(true);
							setMakeTime(null);
							this.type = 0;
							this.type2 = -1;
							...
							this.limittime = 0L;
							this.money = 0;
							MapleTyoonKitchen.this.cooker.getMap().broadcastMessage(
									SLFCGPacket.TyoonKitchenPacket.Menu(MapleTyoonKitchen.this.recipes,
											MapleTyoonKitchen.getAllMtk(MapleTyoonKitchen.this.cooker)));
							Timer.EventTimer.getInstance().schedule(() -> {
			...
			if (update) {
				MapleTyoonKitchen.this.cooker.getMap().broadcastMessage(SLFCGPacket.TyoonKitchenPacket.Menu(
						MapleTyoonKitchen.this.recipes, MapleTyoonKitchen.getAllMtk(MapleTyoonKitchen.this.cooker)));
			}
		}// mush
