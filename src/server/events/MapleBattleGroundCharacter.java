...
public class MapleBattleGroundCharacter implements Comparable<MapleBattleGroundCharacter> {
	...
	public void levelup() {
		...
		if (this.level == 6) {
			...
			this.chr.getClient()
					.send(CField.ImageTalkNpc(9001153, 3000, "시스템 성장 임계점에 도달하였습니다. 더욱 적극적으로 상대방을 격파 하십시오."));
		}
		this.maxmp += 100;
		this.mpregen++;
		this.skillmindamup = 0;
		this.skillmaxdamup = 0;
		this.chr.getClient().getSession()
				.writeAndFlush(CField.EffectPacket.showEffect(this.chr, 0, 0, 29, 0, 0,
						(byte) (this.chr.isFacingLeft() ? 1 : 0), true, this.chr.getTruePosition(),
						"Effect/BasicEff.img/PvpLevelUp", null));
		this.chr.getMap().broadcastMessage(this.chr,
				CField.EffectPacket.showEffect(this.chr, 0, 0, 29, 0, 0, (byte) (this.chr.isFacingLeft() ? 1 : 0),
						false, this.chr.getTruePosition(), "Effect/BasicEff.img/PvpLevelUp", null),
				false);
		this.chr.getMap().broadcastMessage(SLFCGPacket.playSE("Sound/Game.img/PvpLevelUp"));
		int attackup = (getAttackUp() == 1) ? 2
				: ((getAttackUp() == 2) ? 5
						: ((getAttackUp() == 3) ? 10 : ((getAttackUp() == 4) ? 15 : ((getAttackUp() == 5) ? 20 : 0))));
		...
		int hpmpup = (getHpMpUp() == 1) ? 2
				: ((getHpMpUp() == 2) ? 7
						: ((getHpMpUp() == 3) ? 15 : ((getHpMpUp() == 4) ? 20 : ((getHpMpUp() == 5) ? 30 : 0))));
