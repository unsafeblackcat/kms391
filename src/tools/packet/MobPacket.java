...
public class MobPacket {
	...
	public static class BossDunKel {
		...
		public static byte[] eliteBossAttack(MapleMonster dunkel, List<DunkelEliteBoss> eliteBosses, MapleCharacter chr,
				boolean isLeft) {
			MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
			...
			for (DunkelEliteBoss eb : eliteBosses) {
				...
				if (eb.getv5() == 2) {
					mplew.write(HexTool.getByteArrayFromHexString(
							"49 FD FF FF FB FF FF FF 00 00 00 00 E4 02 00 00 FB FF FF FF 01 00 00 00"));
				}
			}
			mplew.write(0);
			return mplew.getPacket();
	...
	public static byte[] moveMonster(byte bOption, int skill, long targetInfo, int tEncodedGatherDuration, int oid,
			Point startPos, Point startWobble, List<LifeMovementFragment> moves, List<Point> multiTargetForBall,
			List<Short> randTimeForAreaAttack, boolean cannotUseSkill) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	public static byte[] spawnMonster(MapleMonster life, int spawnType, int link, int control) {
		...
		mplew.writeInt(
				!life.getStats().isMobZone() ? 0 : ((per == 25) ? 4 : ((per == 50) ? 3 : ((per == 75) ? 2 : 1))));
		mplew.writeInt(0);
		mplew.writeInt(-1);
		mplew.writeInt(0);
		...
		if (life.getId() == 8881000) {
			...
			for (Triple<String, Integer, Integer> skeleton : (Iterable<Triple<String, Integer, Integer>>) life
					.getStats().getSkeleton()) {
				mplew.writeMapleAsciiString((String) skeleton.left);
	...
	public static byte[] spawnMonsterFake(MapleMonster life, int spawnType, int link, int control,
			List<Pair<Integer, Integer>> list) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.SPAWN_MONSTER.getValue());
		mplew.write(HexTool.getByteArrayFromHexString(
				"00 73 00 00 00 01 E5 7F 87 00 01 00 80 6C CE 7B 41 00 00 A0 86 01 00 80 B1 4F 01 F0 55 00 00 C0 5D 00 00 2C 01 00 00 2C 01 00 00 0F 27 00 00 EE 02 00 00 00 CA 9A 3B C4 FF FF FF D2 00 00 00 00 75 2B 7D 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 CF 02 11 00 05 00 00 00 00 FE FF 00 80 6C CE 7B 41 00 00 00 00 00 00 00 00 00 00 01 00 00 00 00 00 00 00 00 FF FF FF FF 00 00 00 00 00 00 00 00 01 00 00 00 00 64 00 00 00 FF FF FF FF "));
		mplew.write(false);
		mplew.write(false);
		if (life.getId() == 8880101 || life.getId() == 8880111) {
			int size = 0;
	...
	public static byte[] controlMonster(MapleMonster life, boolean newSpawn, boolean aggro) {
		...
		for (Triple<String, Integer, Integer> skeleton : (Iterable<Triple<String, Integer, Integer>>) life.getStats()
				.getSkeleton()) {
			mplew.writeMapleAsciiString((String) skeleton.left);
			mplew.write(((Integer) skeleton.mid).intValue());
			mplew.writeInt(((Integer) skeleton.right).intValue());
	...
	public static byte[] applyMonsterStatus(MapleMonster mons, List<Pair<MonsterStatus, MonsterStatusEffect>> mse,
			boolean fromMonster, SecondaryStatEffect effect) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		...
		mplew.write(getMse(mse, MonsterStatus.MS_Origin_Bind) || getMse(mse, MonsterStatus.MS_PowerImmune) ? 3
				: getMse(mse, MonsterStatus.MS_Freeze) ? 2 : 0);
	...
	private static void encodeTemporary(MaplePacketLittleEndianWriter mplew, MapleMonster mob,
			List<Pair<MonsterStatus, MonsterStatusEffect>> mse, SecondaryStatEffect effect) {
		List<Pair<MonsterStatus, MonsterStatusEffect>> newstatups = PacketHelper.sortMBuffStats(mse);
		...
		for (Pair<MonsterStatus, MonsterStatusEffect> ms : newstatups) { // 1.2.391 ok.
			...
			if (!((MonsterStatusEffect) ms.getRight()).isMobskill()
					&& mobStat.getFlag() <= MonsterStatus.MS_PopulatusInvincible.getFlag()
					&& !((MonsterStatus) ms.getLeft()).isStacked() && !checked.contains(ms.getLeft())) {
		...
		for (Pair<MonsterStatus, MonsterStatusEffect> ms : newstatups) { // 1.2.391 ok.
			...
			if (((MonsterStatusEffect) ms.getRight()).isMobskill()
				...
				if (((MonsterStatus) ms.getLeft()).getFlag() == MonsterStatus.MS_PopulatusTimer.getFlag()) {
					...
					mplew.writeInt((mob.getCustomTime(2412).intValue() != 0) ? mob.getCustomTime(2412).intValue()
							: ((MonsterStatusEffect) ms.getRight()).getDuration());
				} else {
					mplew.writeShort(((MonsterStatusEffect) ms.getRight()).getDuration() / 1000);
				}
				checked.add(ms.getLeft());
		...
		if (getMse(mse, MonsterStatus.MS_AdddamParty)) { // 1.2.391 ok.
			...
			mplew.writeInt((eff != null) ? ((eff.getChr() != null)
					? ((eff.getChr().getParty() == null) ? 0 : eff.getChr().getParty().getId())
					: 0) : 0);
	...
	public static byte[] cancelMonsterStatus(MapleMonster mons, List<Pair<MonsterStatus, MonsterStatusEffect>> mse,
			List<MonsterStatusEffect> indielist) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	public static byte[] dropCap() {
		...
		case 0:
			mplew.write(HexTool.getByteArrayFromHexString(
					"09 00 43 61 70 45 66 66 65 63 74 00 00 00 00 09 00 00 00 B0 04 00 00 27 02 00 00 58 02 00 00 27 02 00 00 E8 03 00 00 27 02 00 00 20 03 00 00 27 02 00 00 90 01 00 00 27 02 00 00 40 06 00 00 27 02 00 00 08 07 00 00 27 02 00 00 C8 00 00 00 27 02 00 00 78 05 00 00 27 02 00 00"));
			break;
		case 1:
			mplew.write(HexTool.getByteArrayFromHexString(
					"09 00 43 61 70 45 66 66 65 63 74 02 00 00 00 06 00 00 00 3A 06 00 00 27 02 00 00 AA 04 00 00 27 02 00 00 E2 03 00 00 27 02 00 00 32 FF FF FF 27 02 00 00 8A 01 00 00 27 02 00 00 FA FF FF FF 27 02 00 00"));
			break;
		case 2:
			mplew.write(HexTool.getByteArrayFromHexString(
					"09 00 43 61 70 45 66 66 65 63 74 02 00 00 00 07 00 00 00 31 02 00 00 27 02 00 00 49 FE FF FF 27 02 00 00 F1 FB FF FF 27 02 00 00 D9 FF FF FF 27 02 00 00 81 FD FF FF 27 02 00 00 B9 FC FF FF 27 02 00 00 11 FF FF FF 27 02 00 00"));
			break;
		case 3:
			mplew.write(HexTool.getByteArrayFromHexString(
					"09 00 43 61 70 45 66 66 65 63 74 02 00 00 00 07 00 00 00 BA 00 00 00 27 02 00 00 42 FB FF FF 27 02 00 00 4A 02 00 00 27 02 00 00 D2 FC FF FF 27 02 00 00 9A FD FF FF 27 02 00 00 2A FF FF FF 27 02 00 00 82 01 00 00 27 02 00 00"));
			break;
		case 4:
			mplew.write(HexTool.getByteArrayFromHexString(
					"09 00 43 61 70 45 66 66 65 63 74 00 00 00 00 06 00 00 00 05 FD FF FF 27 02 00 00 95 FE FF FF 27 02 00 00 3D FC FF FF 27 02 00 00 ED 00 00 00 27 02 00 00 CD FD FF FF 27 02 00 00 B5 01 00 00 27 02 00 00"));
			break;
		case 5:
			mplew.write(HexTool.getByteArrayFromHexString(
					"09 00 43 61 70 45 66 66 65 63 74 01 00 00 00 08 00 00 00 1A FC FF FF 27 02 00 00 3A FF FF FF 27 02 00 00 72 FE FF FF 27 02 00 00 52 FB FF FF 27 02 00 00 AA FD FF FF 27 02 00 00 02 00 00 00 27 02 00 00 CA 00 00 00 27 02 00 00 92 01 00 00 27 02 00 00"));
			break;
		case 6:
			mplew.write(HexTool.getByteArrayFromHexString(
					"09 00 43 61 70 45 66 66 65 63 74 01 00 00 00 08 00 00 00 66 FF FF FF 27 02 00 00 46 FC FF FF 27 02 00 00 2E 00 00 00 27 02 00 00 0E FD FF FF 27 02 00 00 7E FB FF FF 27 02 00 00 9E FE FF FF 27 02 00 00 D6 FD FF FF 27 02 00 00 F6 00 00 00 27 02 00 00"));
			break;
		case 7:
			mplew.write(HexTool.getByteArrayFromHexString(
					"09 00 43 61 70 45 66 66 65 63 74 00 00 00 00 08 00 00 00 B6 FE FF FF 27 02 00 00 7E FF FF FF 27 02 00 00 CE FA FF FF 27 02 00 00 EE FD FF FF 27 02 00 00 96 FB FF FF 27 02 00 00 46 00 00 00 27 02 00 00 0E 01 00 00 27 02 00 00 26 FD FF FF 27 02 00 00"));
			break;
		case 8:
			mplew.write(HexTool.getByteArrayFromHexString(
					"09 00 43 61 70 45 66 66 65 63 74 01 00 00 00 09 00 00 00 23 00 00 00 27 02 00 00 93 FE FF FF 27 02 00 00 EB 00 00 00 27 02 00 00 03 FD FF FF 27 02 00 00 3B FC FF FF 27 02 00 00 5B FF FF FF 27 02 00 00 73 FB FF FF 27 02 00 00 B3 01 00 00 27 02 00 00 CB FD FF FF 27 02 00 00"));
			break;
		case 9:
			mplew.write(HexTool.getByteArrayFromHexString(
					"09 00 43 61 70 45 66 66 65 63 74 01 00 00 00 08 00 00 00 E4 FD FF FF 27 02 00 00 8C FB FF FF 27 02 00 00 AC FE FF FF 27 02 00 00 3C 00 00 00 27 02 00 00 CC 01 00 00 27 02 00 00 54 FC FF FF 27 02 00 00 04 01 00 00 27 02 00 00 1C FD FF FF 27 02 00 00"));
			break;
		case 10:
			mplew.write(HexTool.getByteArrayFromHexString(
					"09 00 43 61 70 45 66 66 65 63 74 00 00 00 00 07 00 00 00 5E 02 00 00 27 02 00 00 96 01 00 00 27 02 00 00 CE 00 00 00 27 02 00 00 1E FC FF FF 27 02 00 00 3E FF FF FF 27 02 00 00 76 FE FF FF 27 02 00 00 06 00 00 00 27 02 00 00"));
			break;
		case 11:
			mplew.write(HexTool.getByteArrayFromHexString(
					"09 00 43 61 70 45 66 66 65 63 74 01 00 00 00 09 00 00 00 A8 00 00 00 27 02 00 00 30 FB FF FF 27 02 00 00 E0 FF FF FF 27 02 00 00 18 FF FF FF 27 02 00 00 88 FD FF FF 27 02 00 00 70 01 00 00 27 02 00 00 68 FA FF FF 27 02 00 00 F8 FB FF FF 27 02 00 00 50 FE FF FF 27 02 00 00"));
			break;
		case 12:
			mplew.write(HexTool.getByteArrayFromHexString(
					"09 00 43 61 70 45 66 66 65 63 74 01 00 00 00 07 00 00 00 29 00 00 00 27 02 00 00 B9 01 00 00 27 02 00 00 09 FD FF FF 27 02 00 00 79 FB FF FF 27 02 00 00 61 FF FF FF 27 02 00 00 99 FE FF FF 27 02 00 00 D1 FD FF FF 27 02 00 00"));
			break;
		case 13:
			mplew.write(HexTool.getByteArrayFromHexString(
					"09 00 43 61 70 45 66 66 65 63 74 00 00 00 00 07 00 00 00 8F FF FF FF 27 02 00 00 1F 01 00 00 27 02 00 00 77 03 00 00 27 02 00 00 CF 05 00 00 27 02 00 00 07 05 00 00 27 02 00 00 C7 FE FF FF 27 02 00 00 E7 01 00 00 27 02 00 00"));
			break;
		case 14:
			mplew.write(HexTool.getByteArrayFromHexString(
					"09 00 43 61 70 45 66 66 65 63 74 01 00 00 00 08 00 00 00 8F FF FF FF 27 02 00 00 AF 02 00 00 27 02 00 00 1F 01 00 00 27 02 00 00 CF 05 00 00 27 02 00 00 07 05 00 00 27 02 00 00 57 00 00 00 27 02 00 00 3F 04 00 00 27 02 00 00 E7 01 00 00 27 02 00 00"));
			break;
		}
		return mplew.getPacket();
	}
	...
	public static byte[] incinerateObject(MapleIncinerateObject mio, boolean isSpawn) {// 戴米安BOSS战中的网络封包生成逻辑焚烧效果 飞剑系统
																						// 延迟攻击
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	public static byte[] onDemianDelayedAttackCreate(MapleMonster mob, int skillId, int skillLevel,
			List<MapleDelayedAttack> mda) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.DEMIAN_ATTACK_CREATE.getValue());
		mplew.writeInt(mob.getObjectId());
		mplew.writeInt(skillId);
	...
	public static class BossLucid {
		...
		public static byte[] createDragon(int phase, int posX, int posY, int createPosX, int createPosY,
				boolean isLeft) {
			MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		...
		public static byte[] doFlowerTrapSkill(int level, int pattern, int x, int y, boolean flip) {
			...
			/*
			 * EE 00 00 00 03 00 00 00 00 00 00 00 E8 03 00 00 2B 00 00 00 01
			 */
			mplew.writeInt(238);
			mplew.writeInt(level);
		...
		public static byte[] doSpiralShoot(int type, int angle, int angleRate, int angleDiff, int speed, int interval,
				int shotCount, int bulletAngleRate, int bulletSpeedRate) {
			MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
			mplew.writeShort(SendPacketOpcode.LUCID2_WELCOME_BARRAGE.getValue());
			mplew.writeInt(type);
			mplew.writeInt(angle);
	...
	public static class BossWill {
		...
		public static byte[] WillSpiderAttack(int id, int skill, int level, int type,
				List<Triple<Integer, Integer, Integer>> values) {
			MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
			mplew.writeShort(SendPacketOpcode.WILL_SPIDER_ATTACK.getValue());
			mplew.writeInt(id);
			mplew.writeInt(skill);
	...
	public static byte[] ChangePhaseDemian(MapleMonster mob, int unk) {// 戴米安(Demian)BOSS阶段转换的网络数据包生成方法
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	public static byte[] jinHillahBlackHand(int objectId, int skillId, int skillLv,
			List<Triple<Point, Integer, List<Rectangle>>> points) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	public static byte[] NujukDamage(MapleMonster monster, MapleCharacter chr, long damage, int skillid,
			int attackcount) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	public static class BossPapuLatus {
		...
		public static final byte[] PapulLatusLaser(boolean del, int type) {
			...
			if (del) {
				...
				case 1:
					mplew.write(HexTool.getByteArrayFromHexString(
							"00 00 00 00 02 00 00 00 04 00 00 00 01 00 00 00 00 00 00 00 00 00 00 00 00 00 00 3E 40 05 00 00 00 01 00 00 00 00 00 C0 72 40 00 00 00 00 00 00 24 C0"));
					break;
				case 2:
					mplew.write(HexTool.getByteArrayFromHexString(
							"00 00 00 00 02 00 00 00 02 00 00 00 01 00 00 00 00 00 40 60 40 00 00 00 00 00 00 34 C0 03 00 00 00 01 00 00 00 00 00 40 60 40 00 00 00 00 00 00 3E 40"));
					break;
				case 3:
					mplew.write(HexTool.getByteArrayFromHexString(
							"00 00 00 00 02 00 00 00 06 00 00 00 01 00 00 00 00 00 00 00 00 00 00 00 00 00 00 59 40 07 00 00 00 01 00 00 00 00 00 C0 72 40 00 00 00 00 00 00 24 C0"));
					break;
				}
			}
			mplew.writeZeroBytes(100);
	...
	public static byte[] AfterAttack(MapleMonster monster, int skillid, int skilllevel, boolean left, int unk1,
			int unk2) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.DEMIAN_ATTACK_CREATE.getValue());
		mplew.writeInt(monster.getObjectId());
	...
	public static byte[] AfterAttacklist(MapleMonster monster, int skillid, int skilllevel, boolean left, int unk1,
			List<Point> data) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.DEMIAN_ATTACK_CREATE.getValue());
		mplew.writeInt(monster.getObjectId());
	...
	public static byte[] DemianTranscendenTalSet(MapleMonster demian, MapleMonster TranscendenMob) {// 戴米安BOSS进入超越形态(Transcendental
																									// Form)的网络通信协议生成方法
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	public static byte[] ShowBlackMageSkill(int type) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		mplew.writeShort(SendPacketOpcode.SHOW_EFFECT.getValue());// 黑魔法師全屏播放封包 好像不起作用
	...
	public static byte[] FieldSummonAttack(int type, boolean useskill, Point pos, int objid,
			List<Pair<Long, Integer>> damageinfo) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
