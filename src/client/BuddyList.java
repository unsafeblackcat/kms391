...
public class BuddyList implements Serializable {
	...
	public void loadFromTransfer(Map<CharacterNameAndId, Boolean> data) {
		for (Map.Entry<CharacterNameAndId, Boolean> qs : data.entrySet()) {
			...
			put(new BuddylistEntry(buddyid.getName(), buddyid.getRepName(), buddyid.getAccId(), buddyid.getId(),
					buddyid.getGroupName(), -1, true, buddyid.getLevel(), buddyid.getJob(), buddyid.getMemo()));
		}
	}
	...
	public void loadFromDb(int accId) throws SQLException {
		...
		try {
			...
			ps = con.prepareStatement(
					"SELECT b.buddyaccid, b.pending, c.name as buddyname, c.id as buddyid, c.job as buddyjob, c.level as buddylevel, b.repname, b.groupname, b.memo FROM buddies as b, characters as c WHERE c.accountid = b.buddyaccid AND b.accid = ?");
			ps.setInt(1, accId);
			rs = ps.executeQuery();
			while (rs.next()) {
				...
				if (rs.getInt("pending") == 1) {
					getPendingRequests().push(new CharacterNameAndId(rs.getInt("buddyid"), buddyid, buddyname,
							rs.getString("repname"), rs.getInt("buddylevel"), rs.getInt("buddyjob"),
							rs.getString("groupname"), rs.getString("memo")));
					continue;
				}
				put(new BuddylistEntry(buddyname, rs.getString("repname"), buddyid, rs.getInt("buddyid"),
						rs.getString("groupname"), -1, true, rs.getInt("buddylevel"), rs.getInt("buddyjob"),
						rs.getString("memo")));
			}
			rs.close();
			ps.close();
	...
	public void addBuddyRequest(MapleClient c, int accid, int cidFrom, String nameFrom, String repName, int channelFrom,
			int levelFrom, int jobFrom, String groupName, String memo) {
		put(new BuddylistEntry(nameFrom, repName, accid, cidFrom, groupName, channelFrom, false, levelFrom, jobFrom,
				memo));
		if (getPendingRequests().isEmpty()) {
			c.getSession().writeAndFlush(CWvsContext.BuddylistPacket.requestBuddylistAdd(cidFrom, accid, nameFrom,
					levelFrom, jobFrom, c, groupName, memo));
		} else {
			getPendingRequests().push(
					new CharacterNameAndId(cidFrom, accid, nameFrom, repName, levelFrom, jobFrom, groupName, memo));
		}
	}
