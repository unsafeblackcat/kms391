...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        // 初始對話框 
        말 = "#fs15#\r\n";
        말 += "#L0##i3700514# #d隨機抽取稱號#l\r\n\r\n#y#　└ #b#i2632800##z2632800##k 需要80個\r\n";
        말 += "#L1##i3700514# #r查看所有稱號列表#l";
        cm.sendSimple(말); 
    } else if (status == 1) {
        if (selection == 0) {
            // 確認抽取對話框 
            cm.sendYesNo(" 確定要使用 #b#z"+코인+"# #r"+개수+" 個#k 來抽取嗎？");
        } else if (selection == 1) {
            // 顯示所有稱號列表 
            말 = "#fs15##b＃. 可以獲得以下稱號\r\n\r\n"
        ...
        if (cm.haveItem(코인,  개수)) {
            if (cm.canHold(뽑은템))  {
                ...
                cm.gainItem(뽑은템,  1);
                cm.gainItem(코인,  -개수);
                cm.sendOk("#fs15# 你抽到的物品是！ : \r\n\r\n#i"+뽑은템+"# #d#z"+뽑은템+"#");
                //cm.broadcastSmega(7,"[ 公告] "+ cm.getPlayer().getName()+"  進行了稱號抽取。")
                cm.dispose(); 
            } else {
                cm.sendOk("#fs15#  #r請清空裝備欄位#k。");
                cm.dispose(); 
            }
        } else { 
            cm.sendOk(" 硬幣不足");
