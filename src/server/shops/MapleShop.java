...
public class MapleShop {
	...
	public void sendShop(MapleClient c, int id2) {
		...
		if (MapleShopFactory.getInstance().getShop(this.id).getRechargeShop() == 1) {
			...
			for (MapleShopItem item : MapleShopFactory.getInstance().getShop(this.id).getItems()) {
				...
				for (MapleShopLimit shl : c.getShopLimit()) {
					...
					GregorianCalendar baseCal = new GregorianCalendar(ocal.get(1), shl.getLastBuyMonth(),
							shl.getLastBuyDay());
					...
					if (shl.getItemid() != item.getItemId()
							|| shl.getShopId() != MapleShopFactory.getInstance().getShop(this.id).getId()
		...
		c.getSession().writeAndFlush((Object) CField.NPCPacket.getNPCShop(id2 == 0 ? this.getNpcId() : id2,
				MapleShopFactory.getInstance().getShop(this.id), c));
		block13: for (MapleShopItem item : this.getItems()) {
			if (item.getReCharge() <= 0) {
				continue;
			}
			for (MapleShopLimit shl : c.getShopLimit()) {
				if (shl.getItemid() != item.getItemId() || shl.getShopId() != this.getId()
						|| shl.getPosition() != item.getPosition()) {
					continue;
				}
				c.send(CField.NPCPacket.getShopLimit(this.getNpcId(), item.getPosition() - 1, item.getItemId(),
						shl.getLimitCountAcc()));
				continue block13;
			}
		}
	}
	...
	public void buy(MapleClient c, int itemId, short quantity, short position) {
		...
		if (index >= 0) {
			...
			long price = (long) Math.max(Math.ceil(ii2.getPrice(itemId)
					* (double) (GameConstants.isRechargable(itemId) ? (short) 1 : i.getQuantity())), 0.0);
			if (price >= 0 && c.getPlayer().getMeso() >= (long) price) {
				if (MapleInventoryManipulator.checkSpace(c, itemId, i.getQuantity(), i.getOwner())) {
					...
					c.getSession().writeAndFlush((Object) CField.NPCPacket.confirmShopTransactionItem((byte) 0, this, c,
							index, itemId, true, false, 999999));
				} else {
					c.getSession().writeAndFlush(
							(Object) CField.NPCPacket.confirmShopTransactionItem((byte) 7, this, c, -1, itemId));
				}
			} else {
				c.getSession().writeAndFlush(
						(Object) CField.NPCPacket.confirmShopTransactionItem((byte) 2, this, c, -1, itemId));
			}
			return;
		}
		MapleShopItem item = this.findById(itemId, position);
		if (item == null || item.getItemId() != itemId) {
			c.getPlayer().dropMessage(1, "加載物品資訊時發生錯誤.");
		...
		if (item.getReCharge() > 0) {
			...
			for (MapleShopLimit shl : c.getShopLimit()) {
				...
				c.send(CField.NPCPacket.getShopLimit(this.getNpcId(), item.getPosition() - 1, itemId,
						shl.getLimitCountAcc()));
				break;
			}
			if (add) {
				MapleShopLimit Sl = new MapleShopLimit(c.getPlayer().getAccountID(), c.getPlayer().getId(),
						this.getId(), item.getItemId(), item.getPosition(), 1, 1, item.getReChargeMonth(),
						item.getReChargeDay());
		...
		if (item != null && item.getPrice() > 0L && item.getPriceQuantity() == 0 && this.coinKey == 0) {
			if (c.getPlayer().getMeso() >= item.getPrice() * (long) quantity) {
				...
				if (MapleInventoryManipulator.checkSpace(c, itemId, (short) (quantity * item.getQuantity()), "")) {
					...
					if (GameConstants.isPet(itemId)) {
						MapleInventoryManipulator.addById(c, itemId, (short) (quantity * item.getQuantity()), null,
								MaplePet.createPet(itemId, item.getPeriod()), 0L,
								StringUtil.getAllCurrentTime() + "에 " + (this.id + " 상점에서 구매한 아이템."));
					} else {
						...
						if (GameConstants.isRechargable(itemId)) {
							...
							c.getPlayer().gainItem(itemId, (short) (item.getQuantity() * quantity), false,
									item.getPeriod() <= 0 ? 0L : (long) item.getPeriod(), "상점에서 구입한 아이템");
						}
					}

					c.getSession().writeAndFlush(
							(Object) CField.NPCPacket.confirmShopTransactionItem((byte) 0, this, c, -1, itemId));
				} else {
					c.getSession().writeAndFlush(
							(Object) CField.NPCPacket.confirmShopTransactionItem((byte) 7, this, c, -1, itemId));
				}
			...
			if (c.getPlayer().haveItem((int) item.getPrice(), item.getPriceQuantity() * quantity, false, true)) {
				if (MapleInventoryManipulator.checkSpace(c, itemId, (short) (quantity * item.getQuantity()), "")) {
					if (GameConstants.isPet(itemId)) {
						MapleInventoryManipulator.removeById(c, GameConstants.getInventoryType((int) item.getPrice()),
								(int) item.getPrice(), item.getPriceQuantity(), false, false);
						MapleInventoryManipulator.addById(c, itemId, (short) (quantity * item.getQuantity()), null,
								MaplePet.createPet(itemId, item.getPeriod()), 30L,
								StringUtil.getAllCurrentTime() + "에 " + (this.id + " 상점에서 구매한 아이템."));
					} else {
						ii = MapleItemInformationProvider.getInstance();
						if (GameConstants.isRechargable(itemId)) {
							...
							MapleInventoryManipulator.removeById(c,
									GameConstants.getInventoryType((int) item.getPrice()), (int) item.getPrice(),
									item.getPriceQuantity(), false, false);
							MapleInventoryManipulator.addById(c, itemId, (short) (quantity * item.getQuantity()), null,
									null, item.getPeriod(),
									StringUtil.getAllCurrentTime() + "에 " + (this.id + " 상점에서 구매한 아이템."));
						} else {
							MapleInventoryManipulator.removeById(c,
									GameConstants.getInventoryType((int) item.getPrice()), (int) item.getPrice(),
									item.getPriceQuantity() * quantity, false, false);
							if (GameConstants.getInventoryType(item.getItemId()) == MapleInventoryType.EQUIP
									|| GameConstants.getInventoryType(item.getItemId()) == MapleInventoryType.CODY) {
								for (int i = 0; i < quantity; ++i) {
									c.getPlayer().gainItem(itemId, (short) 1, false,
											item.getPeriod() <= 0 ? 0L
													: (long) item.getPeriod() * 60L * 1000L
															+ System.currentTimeMillis(),
											StringUtil.getAllCurrentTime() + "에 " + (this.id + " 상점에서 구입한 아이템"));
								}
							} else {
								c.getPlayer().gainItem(itemId, (short) (item.getQuantity() * quantity), false,
										item.getPeriod() <= 0 ? 0L : (long) item.getPeriod(),
										StringUtil.getAllCurrentTime() + "에 " + (this.id + " 상점에서 구입한 아이템"));
							}
						}
					}
				...
				c.getSession().writeAndFlush(
						(Object) CField.NPCPacket.confirmShopTransactionItem((byte) 1, this, c, -1, itemId));
			}
			...
			if (item.getPrice() >= 0L && (long) Integer.parseInt(c.getKeyValue("유니온코인")) >= item.getPrice()) {
				if (MapleInventoryManipulator.checkSpace(c, itemId, (short) (quantity * item.getQuantity()), "")) {
					c.getPlayer().setKeyValue(this.coinKey, "point",
							c.getPlayer().getKeyValue(this.coinKey, "point") - item.getPrice() + "");
					c.setKeyValue("유니온코인", "" + ((long) Integer.parseInt(c.getKeyValue("유니온코인")) - item.getPrice()));
					MapleInventoryManipulator.addById(c, itemId, (short) (quantity * item.getQuantity()), null, null,
							item.getPeriod() <= 0 ? 0L : (long) item.getPeriod(), "");
					c.getSession().writeAndFlush(
							(Object) CField.NPCPacket.confirmShopTransactionItem((byte) 0, this, c, -1, itemId));
				} else {
			...
			if (item.getPrice() >= 0L && (long) Integer.parseInt(c.getKeyValue("WishCoinGain")) >= item.getPrice()) {
				if (MapleInventoryManipulator.checkSpace(c, itemId, (short) (quantity * item.getQuantity()), "")) {
					c.getPlayer().setKeyValue(this.coinKey, "point",
							c.getPlayer().getKeyValue(this.coinKey, "point") - item.getPrice() + "");
					c.setKeyValue("WishCoinGain",
							"" + ((long) Integer.parseInt(c.getKeyValue("WishCoinGain")) - item.getPrice()));
					MapleInventoryManipulator.addById(c, itemId, (short) (quantity * item.getQuantity()), null, null,
							item.getPeriod() <= 0 ? 0L : (long) item.getPeriod(), "");
					c.getSession().writeAndFlush(
							(Object) CField.NPCPacket.confirmShopTransactionItem((byte) 0, this, c, -1, itemId));
			...
			if (item.getPrice() >= 0L && (long) Integer.parseInt(c.getKeyValue("RecommendPoint")) >= item.getPrice()) {
				if (MapleInventoryManipulator.checkSpace(c, itemId, (short) (quantity * item.getQuantity()), "")) {
					c.getPlayer().setKeyValue(this.coinKey, "point",
							c.getPlayer().getKeyValue(this.coinKey, "point") - item.getPrice() + "");
					c.setKeyValue("RecommendPoint",
							"" + ((long) Integer.parseInt(c.getKeyValue("RecommendPoint")) - item.getPrice()));
					MapleInventoryManipulator.addById(c, itemId, (short) (quantity * item.getQuantity()), null, null,
							item.getPeriod() <= 0 ? 0L : (long) item.getPeriod(), "");
					c.getSession().writeAndFlush(
							(Object) CField.NPCPacket.confirmShopTransactionItem((byte) 0, this, c, -1, itemId));
			...
			if (item.getPrice() >= 0L
					&& (long) Integer.parseInt(c.getCustomData(501368, "spoint")) >= item.getPrice()) {
				if (MapleInventoryManipulator.checkSpace(c, itemId, (short) (quantity * item.getQuantity()), "")) {
					c.getPlayer().setKeyValue(501368, "point",
							c.getPlayer().getKeyValue(501368, "point") - item.getPrice() + "");
					c.setCustomData(501368, "spoint",
							"" + ((long) Integer.parseInt(c.getCustomData(501368, "spoint")) - item.getPrice()));
					MapleInventoryManipulator.addById(c, itemId, (short) (quantity * item.getQuantity()), null, null,
							item.getPeriod() <= 0 ? 0L : (long) item.getPeriod(), "");
					c.getSession().writeAndFlush(
			...
			if (item.getPrice() >= 0L && c.getCustomKeyValue(501468, "point") >= item.getPrice()) {
				if (MapleInventoryManipulator.checkSpace(c, itemId, (short) (quantity * item.getQuantity()), "")) {
					c.setCustomKeyValue(this.coinKey, "point",
							c.getCustomKeyValue(501468, "point") - item.getPrice() + "");
					MapleInventoryManipulator.addById(c, itemId, (short) (quantity * item.getQuantity()), null, null,
							item.getPeriod() <= 0 ? 0L : (long) item.getPeriod(), "");
					c.getSession().writeAndFlush(
							(Object) CField.NPCPacket.confirmShopTransactionItem((byte) 0, this, c, -1, itemId));
			...
			if (!MapleInventoryManipulator.checkSpace(c, itemId, (short) (quantity * item.getQuantity()), "")) {
				...
				int buyable = (int) c.getPlayer().getKeyValue(this.getQuestEx() + 100000,
						item.getShopItemId() + "_buyed");
				if (item.getBuyQuantity() != 0 && item.getBuyQuantity() - buyable <= 0) {
					c.getSession().writeAndFlush(
							(Object) CField.NPCPacket.confirmShopTransactionItem((byte) 20, this, c, -1, itemId));
					return;
				}
				if (MapleInventoryManipulator.checkSpace(c, itemId, (short) (quantity * item.getQuantity()), "")) {
					c.getPlayer().setKeyValue(this.coinKey, "point",
							String.valueOf(c.getPlayer().getKeyValue(this.coinKey, "point") - item.getPrice()));
					if (GameConstants.isPet(itemId)) {
						MapleInventoryManipulator.addById(c, itemId, (short) (quantity * item.getQuantity()), null,
								MaplePet.createPet(itemId, item.getPeriod()), 0L, "");
					} else {
						...
						if (GameConstants.isRechargable(itemId)) {
							...
							MapleInventoryManipulator.addById(c, itemId, (short) (quantity * item.getQuantity()), null,
									null, item.getPeriod() <= 0 ? 0L : (long) item.getPeriod(), "");
						}
					}
					if (item.getBuyQuantity() != 0) {
						if (c.getPlayer().getKeyValue(this.getQuestEx() + 100000,
								item.getShopItemId() + "_buyed") == -1L) {
							c.getPlayer().setKeyValue(this.getQuestEx() + 100000, item.getShopItemId() + "_buyed", "0");
						}
						c.getPlayer().setKeyValue(this.getQuestEx() + 100000, item.getShopItemId() + "_buyed",
								c.getPlayer().getKeyValue(this.getQuestEx() + 100000, item.getShopItemId() + "_buyed")
										+ 1L + "");
						buyable = (int) c.getPlayer().getKeyValue(this.getQuestEx() + 100000,
								item.getShopItemId() + "_buyed");
						c.getSession().writeAndFlush((Object) CField.NPCPacket.confirmShopTransactionItem((byte) 0,
								this, c, -1, itemId, item.getBuyQuantity() - buyable));
						c.getSession().writeAndFlush((Object) CField.NPCPacket.ShopItemInfoReset(this, c, itemId,
								buyable, item.getPosition()));
					} else {
						c.getSession().writeAndFlush(
								(Object) CField.NPCPacket.confirmShopTransactionItem((byte) 0, this, c, -1, itemId));
					}
				...
				c.getSession().writeAndFlush(
						(Object) CField.NPCPacket.confirmShopTransactionItem((byte) 3, this, c, -1, itemId));
			}
			return;
		}
	}
	...
	public void sell(MapleClient c, MapleInventoryType type, short slot, short quantity) {
		...
		if (item.getType() == 1) {
			...
			if (eq.getEnchantBuff() > 0) {
				c.getPlayer().dropMessage(1, "裝備痕迹無法轉移.");
		...
		if (quantity <= iQuant && iQuant > 0) {
			...
			c.getSession().writeAndFlush(
					CField.NPCPacket.confirmShopTransactionItem((byte) 11, this, c, -1, item.getItemId()));
		}
	}
	...
	public void recharge(MapleClient c, short slot) {
		...
		if (item.getQuantity() < slotMax) {
			...
			if (c.getPlayer().getMeso() >= price) {
				...
				c.getSession().writeAndFlush(
						CWvsContext.InventoryPacket.updateInventorySlot(MapleInventoryType.USE, item, false));
				c.getPlayer().gainMeso(-price, false, true);
				c.getSession().writeAndFlush(
						CField.NPCPacket.confirmShopTransactionItem((byte) 0, this, c, -1, item.getItemId()));
			}
		}
	}
	...
	public static MapleShop createFromDB(int id, boolean isShopId) {
		...
		try {
			...
			if (rs.next()) {
				...
				ret = new MapleShop(shopId, rs.getInt("npcid"), rs.getInt("coinKey"), rs.getString("shopString"),
						rs.getString("saleString"), rs.getInt("rechargeshop"));
				rs.close();
				ps.close();
			} else {
				rs.close();
			...
			while (rs.next()) {
				if (GameConstants.isThrowingStar(rs.getInt("itemid")) || GameConstants.isBullet(rs.getInt("itemid"))) {
					MapleShopItem starItem = new MapleShopItem(rs.getInt("shopitemid"), (short) 1, rs.getInt("itemid"),
							rs.getInt("price"), rs.getInt("pricequantity"), (byte) rs.getInt("Tab"),
							rs.getInt("quantity"), rs.getInt("period"), i, rs.getInt("itemrate"),
							rs.getInt("buyquantity"), rs.getInt("rechargemonth"), rs.getInt("rechargeday"),
							rs.getInt("resetday"), rs.getInt("recharge"), rs.getInt("rechargecount"));
					ret.addItem(starItem);
					if (rechargeableItems.contains(Integer.valueOf(starItem.getItemId()))) {
						recharges.remove(Integer.valueOf(starItem.getItemId()));
					}
				} else {
					ret.addItem(new MapleShopItem(rs.getInt("shopitemid"), (short) 1000, rs.getInt("itemid"),
							rs.getInt("price"), rs.getInt("pricequantity"), (byte) rs.getInt("Tab"),
							rs.getInt("quantity"), rs.getInt("period"), i, rs.getInt("itemrate"),
							rs.getInt("buyquantity"), rs.getInt("rechargemonth"), rs.getInt("rechargeday"),
							rs.getInt("resetday"), rs.getInt("recharge"), rs.getInt("rechargecount")));
			...
			for (Integer recharge : recharges) {
				ret.addItem(new MapleShopItem(0, (short) 1000, recharge.intValue(), 0L, 0, (byte) 0, 0, 0, i, 0, 0, 0,
						0, 0, 0, 0));
				i++;
			}
			rs.close();
