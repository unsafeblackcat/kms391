...
public class MapleDoor extends MapleMapObject {
	...
	public final void sendSpawnData(MapleClient client) {
		...
		if (this.target.getId() == client.getPlayer().getMapId() || getOwnerId() == client.getPlayer().getId()
			...
			client.getSession()
					.writeAndFlush(CField.spawnDoor(getOwnerId(),
							(this.target.getId() == client.getPlayer().getMapId()) ? this.targetPosition
									: this.townPortal.getPosition(),
							true));
			if (getOwner() != null && getOwner().getParty() != null && client.getPlayer().getParty() != null
					&& (getOwnerId() == client.getPlayer().getId()
							|| getOwner().getParty().getId() == client.getPlayer().getParty().getId())) {
				client.getSession()
						.writeAndFlush(CWvsContext.PartyPacket.partyPortal(this.town.getId(), this.target.getId(),
								this.skillId,
								(this.target.getId() == client.getPlayer().getMapId()) ? this.targetPosition
										: this.townPortal.getPosition(),
								true));
			...
			client.getSession()
					.writeAndFlush(CWvsContext.spawnPortal(this.town.getId(), this.target.getId(), this.skillId,
							(this.target.getId() == client.getPlayer().getMapId()) ? this.targetPosition
									: this.townPortal.getPosition()));
		}
	}
		...
		if (this.target.getId() == client.getPlayer().getMapId() || getOwnerId() == client.getPlayer().getId()
			...
			if (getOwner() != null && getOwner().getParty() != null && client.getPlayer().getParty() != null
				...
				client.getSession().writeAndFlush(
						CWvsContext.PartyPacket.partyPortal(999999999, 999999999, 0, new Point(-1, -1), false));
			}
