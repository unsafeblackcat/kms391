...
public class ChannelServer {
	...
	public final void run_startup_configurations() {
		...
		try {
			...
			bootstrap.group((EventLoopGroup) bossGroup, workerGroup).channel(NioServerSocketChannel.class)
					.childHandler(new ChannelInitializer<SocketChannel>() {
						...
						public void initChannel(SocketChannel ch) throws Exception {
							...
							ch.pipeline().addLast("handler",
									new MapleNettyHandler(ServerType.CHANNEL, ChannelServer.this.channel));
						}
			...
			System.out.println("[通知]頻道 " + (getChannel() == 1 ? 1 : getChannel() == 2 ? "20歲以上" : getChannel() - 1)
					+ " 服務器成功打開了 " + port + " 埠.");
			this.eventSM.init();
		} catch (InterruptedException e) {
			System.out.println("[錯誤]通道服務器無法打開 " + this.port + " 埠.");
			e.printStackTrace();
		}
	}
	...
	public static final void startChannel_Main() {
		...
		try {
			...
			System.out.println("[錯誤]頻道服務器打開失敗.");
			e.printStackTrace();
		}
	}
