...
public class CommandProcessor {
	...
	public static boolean processCommand(MapleClient c, String line, ServerConstants.CommandType type) {
		...
		if (line.charAt(0) == ServerConstants.PlayerGMRank.NORMAL.getCommandPrefix()
				|| (c.getPlayer().getGMLevel() > ServerConstants.PlayerGMRank.NORMAL.getLevel()
						&& line.charAt(0) == ServerConstants.PlayerGMRank.DONATOR.getCommandPrefix())) {
			...
			if (commandObject == null || commandObject.getType() != type) {
				sendDisplayMessage(c, "當前輸入的播放機命令不存在.", type);
				return true;
			}
			if (c.getPlayer().getMapId() == 121212121) {
				sendDisplayMessage(c, "當前地圖上無法使用命令.", type);
				return true;
			}
			try {
		...
		if (c.getPlayer().getGMLevel() <= ServerConstants.PlayerGMRank.NORMAL.getLevel()
				|| (line.charAt(0) != ServerConstants.PlayerGMRank.SUPERGM.getCommandPrefix()
		...
		if (co == null) {
			...
			sendDisplayMessage(c, "當前輸入的管理員命令不存在.", type);
			return true;
		}
		if (c.getPlayer().getGMLevel() >= co.getReqGMLevel()) {
			...
			try {
				...
				sendDisplayMessage(c, "該命令未正確使用: " + x, type);
			} catch (Exception e) {
			...
			sendDisplayMessage(c, "使用該命令的許可權級別不够.", type);
		}
		return true;
	}
	...
	static {
		Class[] array = { PlayerCommand.class, InternCommand.class, GMCommand.class, AdminCommand.class,
				DonatorCommand.class, SuperDonatorCommand.class, SuperGMCommand.class, SLFCGGameCommand.class },
				CommandFiles = array;
		for (Class<?> clasz : array) {
			try {
				ServerConstants.PlayerGMRank rankNeeded = (ServerConstants.PlayerGMRank) clasz
						.getMethod("getPlayerLevelRequired", new Class[0]).invoke(null, (Object[]) null);
						...
						if (!Modifier.isAbstract(c.getModifiers()) && !c.isSynthetic()) {
							...
							if (o instanceof CommandExecute && enabled) {
								...
								commands.put(rankNeeded.getCommandPrefix() + c.getSimpleName().toLowerCase(),
										new CommandObject((CommandExecute) o, rankNeeded.getLevel()));
								if (rankNeeded.getCommandPrefix() != ServerConstants.PlayerGMRank.GM.getCommandPrefix()
										&& rankNeeded.getCommandPrefix() != ServerConstants.PlayerGMRank.NORMAL
												.getCommandPrefix()) {
									commands.put("!" + c.getSimpleName().toLowerCase(), new CommandObject(
											(CommandExecute) o, ServerConstants.PlayerGMRank.GM.getLevel()));
								}
							}
						}
					} catch (Exception ex) {
				...
				ex2.printStackTrace();
				FileoutputUtil.outputFileError("Log_Script_Except.txt", ex2);
			}
		}
	}
}
