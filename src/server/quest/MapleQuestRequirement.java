	...
	public MapleQuestRequirement(MapleQuest quest, MapleQuestRequirementType type, ResultSet rse) throws SQLException {
		...
		case job:
			...
			if (first.length <= 0 && rse.getString("intStoresFirst").length() > 0) {
				this.dataStore.add(new Pair<>(Integer.valueOf(Integer.parseInt(rse.getString("intStoresFirst"))),
						Integer.valueOf(Integer.parseInt(rse.getString("intStoresSecond")))));
			}
			for (i = 0; i < first.length; i++) {
				if (first[i].length() > 0 && second[i].length() > 0) {
					this.dataStore.add(new Pair<>(Integer.valueOf(Integer.parseInt(first[i])),
							Integer.valueOf(Integer.parseInt(second[i]))));
				}
			}
			break;
		case partyQuest_S:
