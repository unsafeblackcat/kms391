...
public class DumpItems {// 修改記錄 修改了數據庫字段集從韓國模式改爲uft8模式
	...
	public void dumpItems() throws Exception {
		...
		if (!this.hadError) {
			PreparedStatement psa = this.con.prepareStatement(
					"INSERT INTO wz_itemadddata(itemid, `key`, `subKey`, `value`) VALUES (?, ?, ?, ?)");
			PreparedStatement psr = this.con.prepareStatement(
					"INSERT INTO wz_itemrewarddata(itemid, item, prob, quantity, period, worldMsg, effect) VALUES (?, ?, ?, ?, ?, ?, ?)");
			PreparedStatement ps = this.con.prepareStatement(
					"INSERT INTO wz_itemdata(itemid, name, msg, `desc`, slotMax, price, wholePrice, stateChange, flags, karma, meso, itemMakeLevel, questId, scrollReqs, consumeItem, totalprob, incSkill, replaceId, replaceMsg, `create`, afterImage, `forceUpgrade`, `chairType`, `nickSkill`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
			PreparedStatement pse = this.con.prepareStatement(
	...
	public void dumpItem(PreparedStatement psa, PreparedStatement psr, PreparedStatement ps, PreparedStatement pse,
		...
		if (this.doneIds.contains(Integer.valueOf(this.id))
				|| GameConstants.getInventoryType(this.id) == MapleInventoryType.UNDEFINED) {
			return;
		}
		...
		if (dat != null) {
			for (MapleData child : dat) {
				((Map<String, Integer>) equipStats.get(Integer.valueOf(-1))).put(
						"mob" + MapleDataTool.getIntConvert("id", child, 0),
						Integer.valueOf(MapleDataTool.getIntConvert("prob", child, 0)));
			}
		}
			...
			for (MapleData info : dat) {
				...
				for (MapleData data : info) {
					if (data.getName().length() > 3 && data.getType() != MapleDataType.CANVAS) {
						((Map<String, Integer>) equipStats.get(Integer.valueOf(lv))).put(data.getName().substring(3),
								Integer.valueOf(MapleDataTool.getIntConvert(data, 0)));
					}
				}
			}
		}
				...
				if (stat.equals("canLevel")) {
					...
					if (stat.equals("skill")) {
						for (int i = 0; i < mapleData.getChildren().size(); i++) {
							rett.put("skillid" + i,
									Integer.valueOf(MapleDataTool.getIntConvert(Integer.toString(i), mapleData, 0)));
						}
					} else {
		...
		for (Map.Entry<Integer, Map<String, Integer>> stats : equipStats.entrySet()) {
			...
			for (Map.Entry<String, Integer> stat : (Iterable<Map.Entry<String, Integer>>) ((Map) stats.getValue())
					.entrySet()) {
						...
						if (subKey.getName().equals("con")) {
							for (MapleData conK : subKey.getChildren()) {
								...
								case "job":
									...
									psa.setString(2, mapleData.getName().equals("elemBoost") ? "elemboost"
											: mapleData.getName());
								...
								psa.setString(2,
										mapleData.getName().equals("elemBoost") ? "elemboost" : mapleData.getName());
