...
public class BingoGame {
	...
	public BingoGame(MapleCharacter owner, boolean isByAdmin) {
		isRunning = true;
		Owner = owner;
		String channel = owner.getClient().getChannel() == 1 ? "1"
				: owner.getClient().getChannel() == 2 ? "20세 이상" : String.valueOf(owner.getClient().getChannel() - 1);
		for (MapleCharacter chr : World.getAllCharacters()) {
			if (isByAdmin) {
				chr.getClient().getSession().writeAndFlush(SLFCGPacket.OnYellowDlg(3003301, 7000,
						"#face3#운영자가 빙고 게임 참여자를 모집 중이야! " + channel + "채널에서 @빙고입장을 통해 입장해봐~", ""));
			} else {
				chr.getClient().getSession().writeAndFlush(SLFCGPacket.OnYellowDlg(3003301, 7000, "#face2##b#e"
						+ Owner.getName() + "#k#n가 빙고 게임 참여자를 모집 중이야! " + channel + "채널에서 @빙고입장을 통해 입장해봐~", ""));
			}
		}
	}
	...
	public void StartGame() {
		...
		setBingoTimer(Timer.EventTimer.getInstance().register(new Runnable() {
			public void run() {
				...
				if (BingoGame.this.hostednumbers.size() == 75 || BingoGame.this.rank.size() == 30
						|| BingoGame.this.rank.size() >= BingoGame.this.players.size()) {
					BingoGame.this.StopBingo();
					return;
				}
				while (true) {
					...
					if (!BingoGame.this.hostednumbers.contains(Integer.valueOf(number)) && number <= 75) {
						...
						for (MapleCharacter chr : BingoGame.this.players.keySet()) {
							...
							chr.getClient().getSession().writeAndFlush(
									SLFCGPacket.BingoHostNumber(number, 75 - BingoGame.this.hostednumbers.size()));
						}
						return;
					}
				}
			...
			if (chr != null) {
				...
				if (!this.rank.contains(chr)) {
					for (Pair<Integer, Integer> item : items) {
						if (chr.getInventory(GameConstants.getInventoryType(((Integer) item.left).intValue()))
								.getNumFreeSlot() > 0) {
						...
						chr.dropMessage(-7,
								"인벤토리 공간이 부족하여 "
										+ MapleItemInformationProvider.getInstance()
												.getName(((Integer) item.left).intValue())
										+ "아이템 " + item.right + "개를 지급받지 못했습니다. 이 메세지를 캡쳐하여 GM에게 문의해주세요.");
					}
				}
			}
		}
